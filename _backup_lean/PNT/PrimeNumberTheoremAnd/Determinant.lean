import PrimeNumberTheoremAnd.Common
import PrimeNumberTheoremAnd.WeierstrassProduct
...
namespace PrimeNumberTheoremAnd.Hadamard
...
end PrimeNumberTheoremAnd.Hadamard
...
theorem det2_AF_analytic_on_halfPlaneReGtHalf :
  ...
  have h_norm_conv : ∀ᶠ s in 𝓝 s0, Summable (fun p : Prime => a p s) := by
    ...
    have hsum : Summable (fun p : Prime => (p.1 : ℝ) ^ (-(2 : ℝ) * σ)) := by
      ...
      simpa using PrimeNumberTheoremAnd.Common.real_prime_rpow_summable (r := (2 : ℝ) * σ) this
  ...
  -- Now prove analyticity of the `tsum` (Weierstrass M-test route).
  have h_tsum_analytic : AnalyticAt ℂ (fun s => ∑' (p : Prime), a p s) s0 := by
    ...
    have hdiff_tsum : DifferentiableOn ℂ (fun s => ∑' p : Prime, a p s) (Metric.ball s0 r) := by
      apply differentiableOn_tsum_of_summable_norm
      · -- Summable bound
        exact (PrimeNumberTheoremAnd.Common.real_prime_rpow_summable h2σ).mul_left
          (((1 - (2 : ℝ) ^ (-σ))⁻¹) / 2 + 2⁻¹)
  ...
  have : AnalyticAt ℂ det2_AF s0 :=
    PrimeNumberTheoremAnd.Common.AnalyticAt.congr_of_eventuallyEq h_eq_exp' (h_eq_exp.mono (by intro s hs; symm; simpa using hs))
  ...
  have hId : det2_AF s = ∏' (p : Prime), Complex.exp (a p) := by
    ...
    have hfactor : ∀ p : Prime, det2EulerFactor s p = Complex.exp (a p) := by
      ...
      have hlam_lt : ‖lam‖ < 1 := by
        ...
        have hneg : -s.re < 0 := by
          have hspos : (0 : ℝ) < s.re :=
            lt_trans (by norm_num : (0 : ℝ) < 1 / 2) (lt_trans hσhalf hσ)
          exact neg_lt_zero.mpr hspos
        have hlogpos : 0 < Real.log (p.1 : ℝ) := by
...
theorem det2_AF_nonzero_on_critical_line :
  ...
  have hsum_tail : Summable (fun p : Prime => (p.1 : ℝ) ^ (-((3 : ℝ) / 2))) := by
    simpa [neg_div] using
      PrimeNumberTheoremAnd.Common.real_prime_rpow_summable (r := (3 : ℝ) / 2) (by norm_num)
  have hsum_a : Summable a := by
