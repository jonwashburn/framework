...
lemma hasLogCurvature_of_contDiff
  (H : ℝ → ℝ) (h_diff : ContDiff ℝ 2 H) (h0 : H 0 = 1) (h1 : deriv H 0 = 0) :
  HasLogCurvature H (deriv (deriv H) 0) := by
  have hs : Convex ℝ (Set.univ : Set ℝ) := convex_univ
  have hx0 : (0 : ℝ) ∈ (Set.univ : Set ℝ) := by simp
  have h_taylor := Real.taylor_tendsto (f := H) (x₀ := 0) (n := 2) (s := Set.univ) hs hx0
    (h_diff.contDiffOn)
  have h_taylor' :
      Filter.Tendsto (fun x => (H x - taylorWithinEval H 2 Set.univ 0 x) / x^2)
        (nhds 0) (nhds (0 : ℝ)) := by
    simpa [nhdsWithin_univ] using h_taylor
  have h_rewrite :
      (fun x => (H x - taylorWithinEval H 2 Set.univ 0 x) / x^2) =
        (fun x => (H x - 1) / x^2 - (deriv (deriv H) 0) / 2) := by
    funext x
    by_cases hx : x = 0
    · subst hx
      simp [h0, h1]
    · have hx2 : x^2 ≠ 0 := by nlinarith [hx]
      rw [taylorWithinEval_two_univ]
      field_simp [hx2, h0, h1]
      ring
  have h_taylor'' :
      Filter.Tendsto (fun x => (H x - 1) / x^2 - (deriv (deriv H) 0) / 2)
        (nhds 0) (nhds (0 : ℝ)) := by
    simpa [h_rewrite] using h_taylor'
  have h_limit :
      Filter.Tendsto (fun x => (H x - 1) / x^2)
        (nhds 0) (nhds ((deriv (deriv H) 0) / 2)) :=
    (tendsto_sub_nhds_zero_iff).1 h_taylor''
  have h_scaled :
      Filter.Tendsto (fun x => 2 * (H x - 1) / x^2)
        (nhds 0) (nhds (deriv (deriv H) 0)) := by
    have h_mul : Filter.Tendsto (fun x => (2 : ℝ) * ((H x - 1) / x^2))
        (nhds 0) (nhds ((2 : ℝ) * ((deriv (deriv H) 0) / 2))) :=
      tendsto_const_nhds.mul h_limit
    simpa [div_eq_mul_inv, mul_assoc, mul_left_comm, mul_comm] using h_mul
  simpa [HasLogCurvature] using h_scaled
