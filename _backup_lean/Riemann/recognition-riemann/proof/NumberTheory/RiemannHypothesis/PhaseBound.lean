import Mathlib
import Proof.Foundation.DiscretenessForcing
...
theorem re_positive_of_phase_bound (z : ℂ) (hz : z ≠ 0)
    (h_phase : Complex.abs z > 0 → 0 ≤ z.re) :
    0 ≤ z.re := by
  apply h_phase
  exact Complex.abs.pos hz

theorem re_nonneg_from_phase_bound :
    PhaseBoundCondition →
    ∃ B : ℝ, 0 ≤ B ∧ B < Real.pi / 2 := by
  intro pbc
  refine ⟨pbc.B_prime + pbc.B_ho + pbc.B_pf, ?_, pbc.total_lt_half_pi⟩
  linarith [pbc.B_prime_nonneg, pbc.B_ho_nonneg, pbc.B_pf_nonneg]
...
theorem RH_from_recognition_science :
    ...
    -- Conclusion: there exists a phase bound condition with strict half-pi bound
    ∃ pbc : PhaseBoundCondition, pbc.B_prime + pbc.B_ho + pbc.B_pf < Real.pi / 2 := by
  ...
  }, by simp; linarith [Real.pi_gt_three]⟩
