...
Souls are **forced back into substrate** to "discharge" accumulated phase pressure.

## Key Insight

The Light Field is NOT infinitely capacious. There's a saturation threshold (φ^45,
derived from Gap-45 scaling) above which the phase pressure forces re-embodiment.
...
/-- **SATURATION THRESHOLD**
    ...
    Derived from Gap-45 scaling: φ^45

    This is the "capacity" of the Light Field per unit volume.
    Beyond this density, patterns must discharge into substrate. -/
...
/-- Saturation threshold is very large (φ^45 ≈ 1.8 × 10^9 > 10^8) -/
...
noncomputable def NonExistenceCost (density : ℝ) : ℝ :=
  ...
  else (density - SaturationThreshold) / SaturationThreshold
...
theorem nonExistenceCost_nonneg (density : ℝ) (hd : 0 ≤ density) :
  ...
  · push_neg at h
    apply div_nonneg
    · linarith
    · exact le_of_lt saturationThreshold_pos
...
/-- **BIRTH FROM SATURATION THEOREM**
    ...
    This is the formal statement of "high phase density forces re-embodiment." -/
...
/-- **CYCLE COMPLETENESS THEOREM**
    ...
    The cycle is complete: Life → Death → Light → Saturation → Rebirth → Life. -/
