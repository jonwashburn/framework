...
lemma Jcost_comp_exp_eq_Jlog : (fun t : ℝ => Jcost (Real.exp t)) = Jlog := by
  funext t
  simp [Jcost_exp, Jlog]
...
theorem Jcost_comp_exp_second_deriv_at_zero :
  ...
  -- Use Jcost ∘ exp = Jlog and the established Jlog curvature
  simpa [Jcost_comp_exp_eq_Jlog] using Jlog_second_deriv_at_zero
