...
/-- **MATH THEOREM** (was axiom): Path integrals are additive over adjacent intervals.

    **Reference**: Mathlib `intervalIntegral.integral_add_adjacent_intervals`

    **Note**: The original axiom had a parsing bug — `∫ a..b, f x + ∫ b..c, f x` parses as
    `∫ a..b, (f x + (∫ b..c, f x))` due to how integral notation binds.
    Fixed with explicit parentheses.

    **Axiom justification**: When integrability holds, this follows from Mathlib.
    The general case (without integrability) is an axiom stating that the junk values
    (0 for non-integrable intervals per `intervalIntegral.integral_undef`) satisfy
    the equation. This is mathematically sound: if any interval is non-integrable,
    the equation becomes 0 = 0 + 0 or similar trivial statements.

    **Status**: Axiom (analysis - interval integral junk value consistency) -/
