...
private noncomputable def JcostDeriv (x : ℝ) : ℝ := (1 - x ^ (-2 : ℤ)) / 2
...
private noncomputable def JcostDeriv' (x : ℝ) : ℝ := x ^ (-3 : ℤ)
...
lemma Jcost_as_composition {x : ℝ} (hx : 0 < x) :
  ...
  -- Jcost x = (x + x⁻¹)/2 - 1
  -- Jlog t = (exp t + exp(-t))/2 - 1 = cosh t - 1
  -- Jlog (log x) = (exp(log x) + exp(-log x))/2 - 1 = (x + x⁻¹)/2 - 1
  unfold Jcost Jlog
  have h1 : Real.exp (Real.log x) = x := Real.exp_log hx
  have h2 : Real.exp (-Real.log x) = x⁻¹ := by
    rw [Real.exp_neg, h1]
  rw [h1, h2]
