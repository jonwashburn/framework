...
theorem derivative_eq_neg_linSum_on_agent_bonds_theorem
  ...
  Consent.directional_derivative_value i j (ActionSpec.directionSpecOfAction action hσ)
    ...
    - (bondsOfAgent baseline j).sum
        (fun b => linBondDelta (baseline.bond_multipliers b) (action.logStrain b)) := by
  -- Step 1: For action-based specs, the derivative equals -mechContributionOn
  rw [derivative_eq_neg_mechOn_of_actionSpec i j p baseline κ hκ action hσ h_agent h_support_mem]
  -- Step 2: mechContributionOn equals the sum of linBondDelta
  rw [mechContributionOn_eq_linSum_ofAction baseline j action hσ]
