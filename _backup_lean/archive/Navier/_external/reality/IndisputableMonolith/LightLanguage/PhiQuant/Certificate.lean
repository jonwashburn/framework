...
namespace LightLanguage.PhiQuant
...
def PhiBandThreshold : ℝ :=
  let j_second_deriv_at_phi := 1 / Constants.phi ^ 2 + 1 / Constants.phi ^ 4
...
def phiPairDistance (windows : List (List ℂ))
  ...
  Real.abs ((wi.length : ℝ) - (wj.length : ℝ))
...
@[simp] lemma phi_band_is_parameter_free :
    PhiBandThreshold = 1 / Real.sqrt (1 / Constants.phi ^ 2 + 1 / Constants.phi ^ 4) := by
  rfl
