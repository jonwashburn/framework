...
noncomputable def recognitionProfile (ϑ : ℝ) : ℝ :=
  1 + 2 * Real.tan ϑ + Real.sqrt ((1 + 2 * Real.tan ϑ) ^ 2 - 1)

/-- The argument to arcosh is at least 1 for ϑ ∈ [0, π/2] -/
lemma arcosh_arg_ge_one (ϑ : ℝ) (hϑ : 0 ≤ ϑ ∧ ϑ ≤ π/2) :
  1 ≤ 1 + 2 * Real.tan ϑ := by
  have ht : 0 ≤ Real.tan ϑ :=
    tan_nonneg_of_nonneg_of_le_pi_div_two hϑ.1 hϑ.2
  have hprod : 0 ≤ 2 * Real.tan ϑ := by
    have htwo : 0 ≤ (2 : ℝ) := by norm_num
    exact mul_nonneg htwo ht
  have : 1 ≤ 1 + 2 * Real.tan ϑ := by
...
lemma recognitionProfile_pos (ϑ : ℝ) (hϑ : 0 ≤ ϑ ∧ ϑ ≤ π/2) :
  ...
  have hypos : 0 < 1 + 2 * Real.tan ϑ := lt_of_lt_of_le zero_lt_one hy
  have hs : 0 ≤ Real.sqrt ((1 + 2 * Real.tan ϑ) ^ 2 - 1) := Real.sqrt_nonneg _
...
theorem kernel_match_pointwise (ϑ : ℝ) (hϑ : 0 ≤ ϑ ∧ ϑ ≤ π/2) :
  ...
  have hy_sub : y - 1 = 2 * Real.tan ϑ := by simp [y]
  calc
