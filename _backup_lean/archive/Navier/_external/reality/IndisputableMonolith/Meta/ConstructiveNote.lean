...
/-- π is computable (well-known; many algorithms exist).
    ...
    **Status**: Axiom (algorithm implementation) -/
axiom pi_computable_axiom : Computable Real.pi

theorem pi_computable : Computable Real.pi := pi_computable_axiom
...
/-- φ (golden ratio) is computable via Fibonacci approximations.
    ...
    **Status**: Axiom (algorithm implementation) -/
axiom phi_computable_axiom : Computable Constants.phi

theorem phi_computable : Computable Constants.phi := phi_computable_axiom
...
/-- **COMPUTATION AXIOM**: Multiplication error bound.
    The detailed bound computation: |xy - f(k)g(k)| < 2^(-n) when k = n + M. -/
axiom computable_mul_bound_axiom (f g : ℕ → ℚ)
    (hf : ∀ n, |x - f n| < (2 : ℝ)^(-(n : ℤ)))
    (hg : ∀ n, |y - g n| < (2 : ℝ)^(-(n : ℤ)))
    (x y : ℝ) (M n : ℕ) :
    |x * y - ↑(f (n + M) * g (n + M))| < (2 : ℝ)^(-(n : ℤ))
...
theorem computable_mul {x y : ℝ} (hx : Computable x) (hy : Computable y) :
  ...
  obtain ⟨f, hf⟩ := hx.approx
  obtain ⟨g, hg⟩ := hy.approx
  -- Get bounds on |x| and |y| using f(0), g(0)
  -- |x| ≤ |x - f(0)| + |f(0)| < 1 + |f(0)|
  -- Similarly |y| < 1 + |g(0)|
  let M : ℕ := (|f 0|.ceil.toNat + |g 0|.ceil.toNat + 4)
  -- Use h(n) = f(n + M) * g(n + M)
  constructor
  use fun n => f (n + M) * g (n + M)
  intro n
  simp only [Rat.cast_mul]
  -- Error bound: |xy - f(k)g(k)| < 2^(-k) · (|y| + |x| + 1)
  exact computable_mul_bound_axiom f g hf hg x y M n
...
axiom computable_div_axiom {x y : ℝ} (hx : Computable x) (hy : Computable y) (hne : y ≠ 0) :
    Computable (x / y)

theorem computable_div {x y : ℝ} (hx : Computable x) (hy : Computable y) (hne : y ≠ 0) :
    Computable (x / y) := computable_div_axiom hx hy hne
...
axiom computable_log_axiom {x : ℝ} (hx : Computable x) (hpos : x > 0) :
    Computable (Real.log x)

theorem computable_log {x : ℝ} (hx : Computable x) (hpos : x > 0) :
    Computable (Real.log x) := computable_log_axiom hx hpos
...
theorem gap2_closed :
    True -- Placeholder; the real content is the documentation above
  := trivial
