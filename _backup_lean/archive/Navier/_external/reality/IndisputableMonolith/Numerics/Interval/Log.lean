...
def logIntervalMono (I : Interval) (_hI_pos : 0 < I.lo)
    (lo_bound hi_bound : ℚ)
    (_h_lo : lo_bound ≤ log I.lo)
    (_h_hi : log I.hi ≤ hi_bound)
...
theorem logIntervalMono_contains_log {I : Interval} (hI_pos : 0 < I.lo)
    ...
    {x : ℝ} (hx : I.contains x) :
    (logIntervalMono I hI_pos lo_bound hi_bound (by exact_mod_cast h_lo)
      (by exact_mod_cast h_hi) h_valid).contains (log x) := by
