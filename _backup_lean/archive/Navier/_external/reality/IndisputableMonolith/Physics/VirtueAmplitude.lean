...
axiom amplitude_unitarity_bound_axiom (p : VirtueProcess) : crossSection p ≤ 1
...
-/
theorem amplitude_unitarity_bound (p : VirtueProcess) : crossSection p ≤ 1 :=
  amplitude_unitarity_bound_axiom p
...
def rsFeynmanRulesVerified : RSFeynmanRulesCert where
  ...
  unitarity := amplitude_unitarity_bound
