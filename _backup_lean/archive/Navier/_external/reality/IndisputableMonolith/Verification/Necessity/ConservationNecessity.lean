...
theorem mp_implies_recognition_meaningful (hMP : MP) :
    ∀ (A B : Type) [Inhabited A] [Inhabited B], ∃ _ : Recognize A B, True := by
  intro A B hA hB
  exact ⟨⟨default, default⟩, trivial⟩
...
/-- **Weakened Axiom Replacement**: If we assume the flow carries recognition
    ...
    This is provable without sorry! -/
