...
/-- Canonical constant observables used in the K-gate identity. -/
@[simp] def K_A_obs : Observable :=
  { f := fun _ => Constants.K
  , dimless := by
      intro U U' _
      simp }

@[simp] def K_B_obs : Observable :=
  { f := fun _ => Constants.K
  , dimless := by
      intro U U' _
      simp }

/-- Dimensionless observables are invariant under anchor rescalings. -/
@[simp] lemma anchor_invariance
  (O : Observable) {U U' : RSUnits}
  (h : UnitsRescaled U U') :
  BridgeEval O U = BridgeEval O U' :=
  O.dimless h

/-- The K-gate bridge equality is immediate because both observables evaluate to
the same constant display. -/
@[simp] theorem K_gate_bridge (U : RSUnits) :
  BridgeEval K_A_obs U = BridgeEval K_B_obs U := rfl
