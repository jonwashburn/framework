...
/-- Universal reformation rate constant (inverse timescale of pattern formation) -/
noncomputable def reformation_rate : ℝ := 1 / τ₀

/-- Time to reformation - proportional to pattern complexity.
    This measures how long it takes for a pattern to find a suitable substrate. -/
noncomputable def time_to_reformation (lm : LightMemoryState) : ℝ :=
  -- Time proportional to complexity (larger patterns take longer to reform)
  (lm.pattern.complexity : ℝ) / reformation_rate
