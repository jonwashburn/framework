...
lemma Jcost_deriv (x : ℝ) (hx : x ≠ 0) :
  ...
  -- The derivative of J(x) = (x + 1/x)/2 - 1 is (1 - 1/x²)/2
  have hJcost_eq : Jcost = fun y => (y + y⁻¹) / 2 - 1 := rfl
  simp only [hJcost_eq]
  have step1 : HasDerivAt (fun y => (y + y⁻¹) / 2 - 1) ((1 - x⁻¹ ^ 2) / 2) x := by
...
lemma Jcost_strict_mono_on_one_infty (x y : ℝ) (hx : 0 < x) (hy : 0 < y)
  ...
  have hmain : (x - 1) ^ 2 * (2 * y) < (y - 1) ^ 2 * (2 * x) := by
    ...
    -- Define f(t) = (t-1)²/t inline
    have hf_mono : ∀ a b : ℝ, 1 ≤ a → a < b → (a - 1) ^ 2 / a < (b - 1) ^ 2 / b := by
      intro a b ha hab
      ...
      have : (a - 1) ^ 2 * b - (b - 1) ^ 2 * a < 0 := by
        ...
        nlinarith
      linarith
    have hf_result := hf_mono x y hx1 hxy
    have hx0' : 0 < x := hx
    have hy0' : 0 < y := hy
    rw [div_lt_div_iff₀ hx0' hy0'] at hf_result
    -- hf_result : (x - 1) ^ 2 * y < (y - 1) ^ 2 * x
