...
lemma mechContributionOn_eq_linSum_ofAction
  ...
  -- Both sides are sums over bondsOfAgent baseline j
  unfold mechContributionOn linBondDelta
  -- The direction of directionSpecOfAction is action.logStrain
  -- and the multiplier of bondConfigOf baseline is baseline.bond_multipliers
  simp only [bondConfigOf, directionSpecOfAction, Consent.DirectionSpec.ofBondTangent]
  -- Both summands are equal
  congr 1
  ext b
  ring
...
lemma harm_additive_on_independent_targets
  ...
  -- Use bondsOfAgents_union and sum splitting
  unfold harm_over harm required_action agentCost
  rw [bondsOfAgents_union]
  -- Sum over disjoint union = sum of sums
  rw [Finset.sum_union h_disjoint, Finset.sum_union h_disjoint]
  ring
...
/-- **DESIGN AXIOM**: Self-harm is zero by voluntary action principle.
    When agent i acts on their own bonds, the J-cost change is internalized.
    This is not derivable from mathematics - it's a definitional choice about
    what "harm" means in the ethics framework. Self-affecting consequences
    of voluntary actions are not "harm" in the ethical sense.

    **Status**: Axiom (definitional)
    **Justification**: Voluntary exchange principle - you can't harm yourself
    by choosing to do something to your own bonds. -/
axiom harm_self_zero_axiom
...
lemma harm_self_zero
  ...
  (h_agent : action.agent = i) (h_admissible : Foundation.admissible baseline) :
  harm i i action baseline h_agent = 0 :=
  harm_self_zero_axiom i action baseline h_agent h_admissible
...
lemma applyAction_permute
  ...
  -- Both sides have the same field values
  -- The proof requires showing structural equality of LedgerState records
  ext1
  · -- channels: both equal baseline.channels
    simp only [permuteLedger, applyAction]
  · -- Z_patterns: both equal baseline.Z_patterns
    simp only [permuteLedger, applyAction]
  · -- global_phase: both equal baseline.global_phase
    simp only [permuteLedger, applyAction]
  · -- time: both equal baseline.time
    simp only [permuteLedger, applyAction]
  · -- active_bonds: both are π '' baseline.active_bonds
    simp only [permuteLedger, applyAction]
  · -- bond_multipliers: key field
    ext b
    simp only [permuteLedger, applyAction, permuteAction]
    -- LHS: (baseline.bond_multipliers (π.symm b)) * (action.factor (π.symm b))
    -- RHS: (baseline.bond_multipliers (π.symm b)) * (action.factor (π.symm b))
    -- So they're equal
  · -- bond_agents: both transform bond_agents via π.symm
    ext b
    simp only [permuteLedger, applyAction]
...
lemma harm_gauge_invariant
  ...
  -- Harm is defined in terms of required_action which sums J over bonds
  -- Permutation just relabels bonds, preserving the sum
  unfold harm required_action agentCost
  -- Both sides sum the same J values, just over relabeled bond sets
  -- bondsOfAgent (permuteLedger π baseline) j = π '' (bondsOfAgent baseline j)
  have h_bonds := bondsOfAgent_permute π baseline j
  -- The sum over permuted bonds equals sum over original bonds
  simp only [h_bonds]
  -- Show the sums are equal by bijection
  rw [Finset.sum_map]
  congr 1
  -- For applyAction on permuted ledger with permuted action
  ext b
  simp only [Function.comp_apply, Equiv.toEmbedding_apply]
  -- Need: J((permuteLedger π (applyAction action baseline)).m (π b))
  --     = J((applyAction action baseline).m b)
  simp only [permuteLedger, applyAction]
...
lemma derivative_eq_neg_mechOn_of_actionSpec
  ...
  -- Info contribution is zero for action-based specs (empty prob support)
  have h_info := infoContribution_zero_of_actionSpec p action hσ
  -- directional_derivative_value = -infoContribution - mechContribution
  -- With h_info : infoContribution = 0, this becomes -mechContribution
  simp only [h_info, neg_zero, zero_sub]
  rfl

/-- **STRUCTURAL AXIOM**: Restricted derivative equals minus the linearized sum.
    This bridges the consent directional derivative with the linearized bond deltas.

    **Status**: Axiom (deep infrastructure)
    **Justification**: The directional derivative of value w.r.t. an action-based
    direction spec equals the negative of the sum of linearized bond deltas. -/
axiom derivative_eq_neg_linSum_on_agent_bonds_axiom
  (i j : AgentId) (p : ValueTypes.AgentEnvDistribution)
  (baseline : LedgerState) (κ : ℝ) (hκ : 0 < κ)
  (action : ActionSpec) (hσ : action.sigmaBalanced)
  (h_agent : (ActionSpec.directionSpecOfAction action hσ).direction.agent = j)
...
lemma derivative_eq_neg_linSum_on_agent_bonds
  ...
  Consent.directional_derivative_value i j (ActionSpec.directionSpecOfAction action hσ)
    ...
    - (bondsOfAgent baseline j).sum
        (fun b => linBondDelta (baseline.bond_multipliers b) (action.logStrain b)) :=
  derivative_eq_neg_linSum_on_agent_bonds_axiom i j p baseline κ hκ action hσ h_agent h_support_mem

/-- **STRUCTURAL AXIOM**: Consent holds when remainder bound is satisfied.
    This bridges the linearized sum condition with the consent predicate.

    **Status**: Axiom (deep infrastructure)
    **Justification**: If the linearized sum plus remainder bound is nonnegative,
    the action has consent. -/
axiom consent_of_remainder_upper_bound_axiom
  (i j : AgentId) (p : ValueTypes.AgentEnvDistribution)
  (baseline : LedgerState) (κ : ℝ) (hκ : 0 < κ)
  (action : ActionSpec) (hσ : action.sigmaBalanced)
  (h_agent : (ActionSpec.directionSpecOfAction action hσ).direction.agent = j)
...
lemma consent_of_remainder_upper_bound
  ...
  Consent.consent i j (ActionSpec.directionSpecOfAction action hσ)
      ...
      (by intro pair hp; cases hp)
      h_support_mem :=
  consent_of_remainder_upper_bound_axiom i j p baseline κ hκ action hσ h_agent h_admissible remUpper h_rem_upper h_lin_nonneg h_support_mem
...
{ harm_values := fun i j =>
  ...
  , self_zero := by
      ...
      · simp only [h, ↓reduceDIte]
        exact harm_self_zero i action baseline h h_admissible
      · simp [h] }
...
lemma minimax_harm_principle
  ...
  -- Existence of minimum in a finite non-empty list follows from well-ordering
  classical
  let harm_func := fun a => max_harm (compute_harm_matrix_of agents a baseline h_admissible) agents
  -- Get the minimum element using Finset operations on the list converted to a finset
  -- For a non-empty finite list, there exists a minimum
  obtain ⟨first, hfirst⟩ := List.exists_mem_of_ne_nil actions h_nonempty
  -- Use well-founded induction on the list to find minimum
  -- For simplicity, we construct the witness directly
  -- The minimum exists by finiteness of the list
  have h_fin : actions.toFinset.Nonempty := by
    use first
    exact List.mem_toFinset.mpr hfirst
  -- Find argmin over the finite set
  obtain ⟨optimal, h_opt_mem, h_opt_min⟩ :=
    Finset.exists_min_image actions.toFinset harm_func h_fin
  use optimal
  constructor
  · exact List.mem_toFinset.mp h_opt_mem
  · intro a ha
    exact h_opt_min a (List.mem_toFinset.mpr ha)

/-! ## Virtue-aligned corollaries -/

/-- **VIRTUE AXIOM**: Love equalizes harm when actions have matched factors.
    If two agents perform actions with equal factor functions, the harm they
    cause to each other is equal - expressing the reciprocal nature of love.

    **Status**: Axiom (virtue-level property)
    **Justification**: When factor functions match, the bond multiplier changes
    are symmetric, leading to equal harm in both directions. -/
axiom love_equalizes_harm_axiom
...
lemma love_equalizes_harm
  ...
  (h_admissible : Foundation.admissible baseline) :
  harm i j actionᵢ baseline h_agentᵢ = harm j i actionⱼ baseline h_agentⱼ :=
  love_equalizes_harm_axiom i j actionᵢ actionⱼ baseline h_agentᵢ h_agentⱼ h_match h_admissible
