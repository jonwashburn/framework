...
/-- Canonical virtue signature assembled from the default generators. -/
def VirtueSignature.canonical : VirtueSignature :=
  { components := Virtues.Generators.virtue_generators }

lemma VirtueSignature.canonical_nonempty :
    VirtueSignature.canonical.components ≠ [] := by
  dsimp [VirtueSignature.canonical]
  simp [Virtues.Generators.virtue_generators]
