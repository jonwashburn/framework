...
lemma skewRelief_lower_bound_of_transfer
  ...
  have h_phi_pow_pos : 0 < φ ^ 4 := phi_pow_four_pos
  have h_prod_ge_one : 1 ≤ energyTransfer helper sufferer E_target * φ ^ 4 := by
    calc 1 = (1 / φ ^ 4) * φ ^ 4 := by field_simp
      _ ≤ energyTransfer helper sufferer E_target * φ ^ 4 := by
        apply mul_le_mul_of_nonneg_right h_transfer (le_of_lt h_phi_pow_pos)
  exact Int.one_le_floor_iff.mpr h_prod_ge_one
