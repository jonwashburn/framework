      ...
      | cons head tail =>
        ...
        have h_tail_nil : tail = [] := by
          ...
          have h_moves_prim_unique : ∀ a b, a ∈ moves → b ∈ moves → a.primitive = b.primitive → a = b := by
            ...
            obtain ⟨pa, hpa_mem, ha_some⟩ := ha
            obtain ⟨pb, hpb_mem, hb_some⟩ := hb
          ...
          have h_head_prim : head.primitive = prim := by
            ...
            exact this.2
          have h_x_prim : x.primitive = prim := by
            rw [List.mem_filter] at h_x_mem
            simp only [decide_eq_true_eq] at h_x_mem
            exact h_x_mem.2
          have h_head_x_eq : head = x := h_moves_prim_unique head x h_head_in h_x_in (by rw [h_head_prim, h_x_prim])
...
theorem aggCoeff_rowMoves_of_ne (nf : NormalForm) {pair k : ℕ} {prim : Primitive}
  ...
  have h_filter_empty : (rowMoves nf k).filter (fun m => decide (m.pair = pair) && decide (m.primitive = prim)) = [] := by
    rw [List.filter_eq_nil_iff]
    intro m hm
...
theorem aggCoeff_flatMap (nf : NormalForm) (pair : ℕ) (prim : Primitive)
  ...
  | cons h t ih =>
    ...
    rw [aggCoeff_append]
    have hnodup_t : t.Nodup := List.Nodup.of_cons hl
    have h_not_mem : h ∉ t := List.Nodup.notMem hl
...
Proof: Both sides produce sorted lists of the same elements.
- pairList nf = nf.supportPairs.sort (· ≤ ·)
- (List.range 16).filter (fun k => k ∈ nf.supportPairs) filters a sorted list

Since supportPairs ⊆ Finset.range 16, both contain exactly the elements of supportPairs,
and both are sorted by (· ≤ ·).

Both sorted lists contain the same elements; use perm + sorted comparison. -/
theorem pairList_eq_filtered_range_theorem (nf : NormalForm)
  ...
  · -- Permutation: same elements
    ...
    · intro x
      rw [Finset.mem_sort, List.mem_filter, List.mem_range]
      exact ⟨fun hx => ⟨Finset.mem_range.mp (hw hx), hx⟩, fun ⟨_, hx⟩ => hx⟩
    · exact Finset.sort_nodup (· ≤ ·) nf.supportPairs
    · exact List.Nodup.filter _ (List.nodup_range 16)
  · exact Finset.sort_sorted (· ≤ ·) nf.supportPairs
  · exact List.Pairwise.filter _ (List.pairwise_lt_range 16)
