...
/-- Modulo 4 predicate. -/
def mod4 (b : BondId) : ℕ := b % 4

/-- Modulo 8 predicate. -/
def mod8 (b : BondId) : ℕ := b % 8
...
/-- Wisdom: Period-4 pattern (1, 1, -1, -1). -/
def assignWisdom (scope : Finset BondId) : BondId → ℝ :=
  maskTo scope (fun b => if mod4 b < 2 then 1 else -1)

/-- Courage: High-frequency alternating pattern (1, -1, 1, -1). -/
def assignCourage (scope : Finset BondId) : BondId → ℝ :=
  maskTo scope (fun b => if isEven b then 1 else -1)

/-- Patience: Late activation (0, 0, 0, 0, 0, 0, 1, 1) in 8-tick window. -/
def assignPatience (scope : Finset BondId) : BondId → ℝ :=
  maskTo scope (fun b => if mod8 b >= 6 then 1 else 0)

/-- Gratitude: Center activation (0, 0, 1, 1, 1, 1, 0, 0). -/
def assignGratitude (scope : Finset BondId) : BondId → ℝ :=
  maskTo scope (fun b => if mod8 b >= 2 ∧ mod8 b < 6 then 1 else 0)

/-- Humility: Inverse magnitude (1, 1/2, 1/3, ...). -/
noncomputable def assignHumility (scope : Finset BondId) : BondId → ℝ :=
  maskTo scope (fun b => 1 / ((mod8 b : ℝ) + 1))

/-- Hope: Geometric growth (1, φ, φ², ...). -/
noncomputable def assignHope (scope : Finset BondId) : BondId → ℝ :=
  maskTo scope (fun b => Foundation.φ ^ (mod8 b))

/-- Prudence: Variance bound (1, -1, 0, 0, 0, 0, 0, 0). -/
def assignPrudence (scope : Finset BondId) : BondId → ℝ :=
  maskTo scope (fun b => if mod8 b = 0 then 1 else if mod8 b = 1 then -1 else 0)

/-- Compassion: Relief pattern (1, 0, 0, ..., 0, 1). -/
def assignCompassion (scope : Finset BondId) : BondId → ℝ :=
  maskTo scope (fun b => if mod8 b = 0 ∨ mod8 b = 7 then 1 else 0)

/-- Creativity: Novelty pattern (sin-like). -/
noncomputable def assignCreativity (scope : Finset BondId) : BondId → ℝ :=
  maskTo scope (fun b => if mod4 b = 0 then 1 else if mod4 b = 2 then -1 else 0)
...
noncomputable def basisAssigns (scope : Finset BondId) : List (BondId → ℝ) := [
  ...
  assignWisdom scope,
  assignCourage scope,
  assignPatience scope,
  assignGratitude scope,
  assignHumility scope,
  assignHope scope,
  assignPrudence scope,
  assignCompassion scope,
  assignCreativity scope
]
...
noncomputable def hookFromPrimitiveLift
  ...
  let coeffs : List ℝ := [
    ...
    0,           -- Wisdom (Placeholder)
    0,           -- Courage (Placeholder)
    0,           -- Temperance (Placeholder - wait, index mismatch in comment)
    0,           -- Prudence (Placeholder)
    0,           -- Compassion (Placeholder)
    0,           -- Gratitude (Placeholder)
    0,           -- Patience (Placeholder)
    0,           -- Humility (Placeholder)
    0,           -- Hope (Placeholder)
    0,           -- Creativity (Placeholder)
  ]
  coeffs
