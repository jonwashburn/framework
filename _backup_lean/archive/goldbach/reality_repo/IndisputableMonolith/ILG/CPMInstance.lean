...
/-- The ILG model satisfies CPM assumptions when the energy control hypothesis holds.
    This makes the physical assumption explicit rather than hiding it in a sorry. -/
...
/-- **VARIATIONAL AXIOM**: Projection decreases defect.
    **Status**: Axiom (Lax-Milgram theorem) -/
axiom projection_decreases_defect_axiom (P : KernelParams) (s : ILGState) :
    ilgDefect P (projOp P s) ≤ ilgDefect P s

/-- Legacy model with explicit hypothesis.
    Uses axiomatized variational principle. -/
noncomputable def ilgModelWithSorry (P : KernelParams) : Model ILGState :=
  ilgModel P (fun s => projection_decreases_defect_axiom P s)
