...
def jGreedySchedule (code : Array LInstr) : List JGreedyWindow :=
  let deltas := deltaJPerWindow code
  let deltasList := deltas.toList
  let windows := (List.zip (List.range deltasList.length) deltasList).map (fun (idx, delta) =>
    { window := idx,
      gray := PhiIR.grayIndex idx,
      predictedDelta := delta,
      actualDelta := delta : JGreedyWindow })
  windows.mergeSort (fun a b => a.predictedDelta > b.predictedDelta)
