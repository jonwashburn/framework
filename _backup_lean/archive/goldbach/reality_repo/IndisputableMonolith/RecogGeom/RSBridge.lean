...
even if some concrete instantiations use `sorry` pending the RS foundations.
...
/-- **PHYSICS AXIOM**: Recognition transitivity - R̂ neighborhoods compose.

    When ℓ' is reachable from ℓ via R̂, then RHat(ℓ') ⊆ RHat(ℓ).
    This is the RS analog of topology transitivity: if you can reach ℓ' from ℓ,
    then anything reachable from ℓ' is also reachable from ℓ.

    **Status**: Axiom (physical postulate about recognition locality) -/
axiom rs_refinement_transitivity_axiom {L : Type*} [RSConfigSpace L]
    (rs : RSLocalityFromRHat L) (ℓ ℓ' : L) :
    ℓ' ∈ rs.RHat ℓ → rs.RHat ℓ' ⊆ rs.RHat ℓ
...
noncomputable def toLocalConfigSpace {L : Type*} [RSConfigSpace L]
  ...
  refinement := fun ℓ U ℓ' ⟨k, hU, _⟩ hℓ' => by
    ...
    -- This requires an additional axiom about R̂ neighborhood containment
    refine ⟨rs.RHat ℓ', ⟨1, rfl, trivial⟩, ?_⟩
    exact rs_refinement_transitivity_axiom rs ℓ ℓ' hℓ'
