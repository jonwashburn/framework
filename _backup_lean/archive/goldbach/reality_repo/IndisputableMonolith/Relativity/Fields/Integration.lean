...
theorem integrate_smul (c : ℝ) (f : (Fin 4 → ℝ) → ℝ) (g : MetricTensor) (vol : VolumeElement) :
  ...
  unfold integrate_scalar
  -- Goal: Δx4 * Σ (sqrt * (c * f)) = c * (Δx4 * Σ (sqrt * f))
  -- Rewrite using sum_mul and ring
  simp only [← Finset.sum_mul]
  have h : ∀ i ∈ Finset.range 10,
    sqrt_minus_g g (fun _ => ↑i * vol.grid_spacing) * (c * f (fun _ => ↑i * vol.grid_spacing)) =
    c * (sqrt_minus_g g (fun _ => ↑i * vol.grid_spacing) * f (fun _ => ↑i * vol.grid_spacing)) := by
    intros; ring
