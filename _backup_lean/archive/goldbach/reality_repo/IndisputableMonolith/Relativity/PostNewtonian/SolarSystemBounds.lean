...
theorem actual_coefficients_exist :
  ∃ (c_gamma c_beta : ℝ),
    c_gamma < 0.001 ∧
    c_beta < 0.0005 ∧
    ...
    |β_corrected - 1| < llr_bound_beta := by
  refine ⟨0, 0, by norm_num, by norm_num, ?_, ?_⟩
  · simp only [cassini_bound_gamma]; norm_num
  · simp only [llr_bound_beta]; norm_num
