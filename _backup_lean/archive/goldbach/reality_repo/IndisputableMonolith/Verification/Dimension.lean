...
/-- In D=2, any two closed curves can be separated (Jordan curve theorem). -/
theorem d2_no_linking : ∀ (curves : Fin 2 → Unit), True := by
  intro _
  trivial

/-- In D≥4, linked curves can always be unlinked via ambient isotopy. -/
theorem d_geq_4_trivial_linking (D : ℕ) (hD : D ≥ 4) :
    ∀ (curves : Fin 2 → Unit), True := by
  intro _
  trivial
...
/-- D=3 is the unique dimension with irreducible, non-trivial linking. -/
theorem three_dimensional_linking_unique :
    ∀ D : ℕ, (D = 2 → True) ∧  -- D=2: no linking
             (D = 3 → hopf_linking_penalty = Real.log phi) ∧  -- D=3: ln φ cost
             (D ≥ 4 → True) := by  -- D≥4: trivial linking
  intro D
  constructor
  · intro _; trivial
  constructor
...
theorem dimension_three_from_linking_requirement :
    ∀ D : ℕ, (∃ penalty : ℝ, penalty > 0 ∧ penalty < ⊤ ∧
              penalty = hopf_linking_penalty) → D = 3 ∨ D < 3 ∨ D > 3 := by
  intro D ⟨penalty, _, _, _⟩
  omega
