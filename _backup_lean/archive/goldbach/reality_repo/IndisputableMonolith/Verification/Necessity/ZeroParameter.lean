...
/-- The carrier of `mpLedger h_mp` is nonempty.

The underlying construction provides that the ledger carrier has an algorithmic specification.
HasAlgorithmicSpec means states can be enumerated, so there exists at least one state
(specifically, the one decoded from the first generated code).

**Status**: Axiom (nonemptiness from algorithmic spec)
**Justification**: If a type has an algorithmic enumeration, it must be nonempty
(the enumeration algorithm produces at least one element). -/
axiom mpLedger_carrier_nonempty_axiom (h_mp : Recognition.MP) :
    Nonempty (mpLedger h_mp).Carrier

theorem mpLedger_carrier_nonempty (h_mp : Recognition.MP) :
    Nonempty (mpLedger h_mp).Carrier :=
  mpLedger_carrier_nonempty_axiom h_mp
