...
/-- Internal lemma: the core state (before breath bump) preserves the original breath. -/
lemma lStep_core_breath (P : LProgram) (s : LState) :
    (if s.halted then s else
      let i := lFetch P s.ip
      let advIdx : Nat := (s.winIdx8 + 1) % 8
      let cycAdd : Int := s.vecSumCycle + s.reg6.kPerp
      ...
      let s' : LState :=
        match i.op with
        | Opcode.LOCK => { s with aux5 := { s.aux5 with phaseLock := true }, winIdx8 := advIdx, vecSumCycle := cycAdd, jBudgetWin := budget' }
        | Opcode.FLIP => { s with flipped := if s.breath = (breathPeriod / 2 - 1) then not s.flipped else s.flipped, winIdx8 := advIdx, vecSumCycle := cycAdd, jBudgetWin := budget' }
        | Opcode.BALANCE =>
            let (winSum', winIdx') := match i.arg with | OpcodeArg.balance BalanceMode.cycle => if s.breath = breathPeriod - 1 then (0, 0) else (s.winSum8, advIdx) | _ => (if s.winIdx8 = 7 then 0 else s.winSum8, advIdx)
            { s with winSum8 := winSum', winIdx8 := winIdx', vecSumCycle := cycAdd, jBudgetWin := budget' }
        | Opcode.FOLD => { s with reg6 := { s.reg6 with nuPhi := clampI32 (s.reg6.nuPhi + foldDirFromArg i.arg) }, winIdx8 := advIdx, vecSumCycle := cycAdd, jBudgetWin := budget' }
        | Opcode.SEED => { s with aux5 := (match i.arg with | OpcodeArg.token act => applyTokenAction s.aux5 act | _ => applyTokenAction s.aux5 (TokenAction.set 1)).1, winIdx8 := advIdx, vecSumCycle := cycAdd, jBudgetWin := budget' }
        | Opcode.BRAID =>
            let (aux', winDelta) := match i.arg with | OpcodeArg.token act => applyTokenAction s.aux5 act | _ => (s.aux5, 0)
            { s with aux5 := aux', winSum8 := s.winSum8 + winDelta, winIdx8 := advIdx, vecSumCycle := cycAdd, jBudgetWin := budget' }
        | Opcode.MERGE =>
            let (aux', winDelta) := match i.arg with | OpcodeArg.token act => applyTokenAction s.aux5 act | _ => (s.aux5, 0)
            { s with aux5 := aux', winSum8 := s.winSum8 + winDelta, winIdx8 := advIdx, vecSumCycle := cycAdd, jBudgetWin := budget' }
        | Opcode.LISTEN =>
            let vecSum' := match i.arg with | OpcodeArg.listen ListenMode.vectorReset => if s.breath = breathPeriod - 1 then 0 else cycAdd | _ => cycAdd
            { s with winIdx8 := advIdx, vecSumCycle := vecSum', jBudgetWin := budget' }
      let s'' := { s' with ip := lNextIP s', winJAccum := windowAccum, commitPending := none }
      let advIdx' := (s.winIdx8 + 1) % 8
      let rollover : Bool := advIdx' = 0
      let history' := if rollover then s''.jBudgetHistory.concat s''.jBudgetWin else s''.jBudgetHistory
      let budgetNext := if rollover then s''.jBudgetMax else s''.jBudgetWin
      let (commitHist', windowCount', winJAccumNext, commitPending') := if rollover then (if windowAccum ≥ s.commitThreshold then s.commitHistory ++ [(s.windowCount, windowAccum)] else s.commitHistory, s.windowCount + 1, 0, if windowAccum ≥ s.commitThreshold then some s.windowCount else none) else (s.commitHistory, s.windowCount, windowAccum, none)
      let sFinal := { s'' with jBudgetWin := budgetNext, jBudgetHistory := history', commitHistory := commitHist', windowCount := windowCount', winJAccum := winJAccumNext, commitPending := commitPending' }
      let nonNeutral := rollover ∧ (sFinal.winSum8 ≠ (0 : Int))
      let jUp := costVal < 0
      let consentFail := if s.consentGate then (match i.op, i.arg with | Opcode.BRAID, OpcodeArg.token (TokenAction.delta d) | Opcode.MERGE, OpcodeArg.token (TokenAction.delta d) => d < 0 | Opcode.SEED, OpcodeArg.token (TokenAction.set _ cost) => cost < 0 | _, _ => false) else false
      let flags' : FalsifierFlags := { nonNeutralWindow := s.flags.nonNeutralWindow ∨ nonNeutral, jIncreased := s.flags.jIncreased ∨ jUp, unitsLeak := s.flags.unitsLeak ∨ (budgetDec > s.jBudgetWin), consentViolation := s.flags.consentViolation ∨ consentFail }
      { sFinal with flags := flags', falsified := s.falsified ∨ FalsifierFlags.any flags' }).breath = s.breath := by
  split
  · rfl
  · dsimp only
    -- This follows because all the record updates in the `else` branch do not touch `breath`.
    -- Record syntax `{ s with ... }` preserves other fields.
    repeat split
    all_goals rfl
