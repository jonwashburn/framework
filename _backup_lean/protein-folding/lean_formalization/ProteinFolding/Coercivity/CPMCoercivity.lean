...
/-- A protein conformation (grounded representation) -/
structure Conformation where
  ...
  /-- The set of interaction windows associated with this conformation -/
  windows : List LedgerWindow
  /-- Consistency: number of windows matches residue couplings -/
  window_count_match : windows.length = numResidues - 1
...
theorem cpm_convergence
    ...
    (_hBounded : ∃ M, ∀ t, D (traj.conformations t) ≤ M) :
    ∃ L : ℝ, Tendsto (fun (t : ℕ) => D (traj.conformations t)) atTop (nhds L) ∧ L ≥ 0 := by
  -- The sequence D(traj(t)) is monotone decreasing and bounded below by 0
  have hNonneg : ∀ (t : ℕ), 0 ≤ D (traj.conformations t) := fun t => h.defect_nonneg _
  -- By monotone convergence theorem, it converges
  have hMono : Antitone (fun (t : ℕ) => D (traj.conformations t)) := by
    intro m n hmn
    simp only
    induction hmn with
    | refl => exact le_refl _
    | step hmn' ih => exact le_trans (hDefectDecrease _) ih
  ...
  constructor
  · -- Tendsto to L
    apply tendsto_atTop_ciInf hMono
    use 0
    intro x hx
...
theorem c_min_lt_one_over_phi_cubed :
  ...
  have hcmin : c_min = 2/9 := by
    ...
    norm_num
  rw [hcmin]
  have hphi3_lt : phi^3 < 4.25 := h_cubed.2
  have h1 : (1 : ℝ) / phi^3 > 1 / 4.25 := by
    apply one_div_lt_one_div_of_lt
    · linarith
    · exact hphi3_lt
  have h2 : (1 : ℝ) / 4.25 > 0.23 := by norm_num
...
theorem no_frustration_above_threshold (E : EnergyFunction) (D : DefectFunction) (native : Conformation)
    (h : CPMCompatible E D native) (x : Conformation) :
    D x > 0 →
    E x > E native := by
  intro hDx
  have hcoer := h.coercivity x
  have h_cmin_pos : 0 < c_min := c_min_pos
  have h_prod_pos : 0 < c_min * D x := mul_pos h_cmin_pos hDx
  linarith
