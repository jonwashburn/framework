...
private noncomputable def floatToReal (x : Float) : ℝ :=
  -- Converting Float to ℝ is inherently noncomputable in Lean because:
  -- 1. Float is a finite machine representation (IEEE 754)
  -- 2. ℝ is the Dedekind/Cauchy reals (infinite precision)
  -- At runtime, the verification layer uses Float; at proof level, we use ℝ.
  -- The bridge is axiomatic: we assume the Float accurately represents the intended ℝ.
  -- For formal verification, use interval arithmetic bounds on the Float values.
  match x.toRatParts with
  | some (s, e) =>
      -- s * 2^e gives the exact rational representation of the Float
      (s : ℝ) * (2 : ℝ) ^ (e : ℤ)
  | none => 0  -- Handle NaN/infinity by mapping to 0

private noncomputable def floatsToReals (xs : List Float) : List ℝ :=
  xs.map floatToReal

/-- Upgrade a floating-point observation to real-valued lists. -/
noncomputable def toReal (obs : SpectralObservation) : RealObservation := {
...
noncomputable def syntheticReal : RealObservation := synthetic.toReal
...
/-- Load real-valued spectral data with fallback.
    NOTE: This is noncomputable because ℝ is not runtime-representable.
    For executable code, use `loadWithFallback` and keep data as Float. -/
noncomputable def loadRealWithFallback (source? : Option System.FilePath := none) : IO RealObservation := do
