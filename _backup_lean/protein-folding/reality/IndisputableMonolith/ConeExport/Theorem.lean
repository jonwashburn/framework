...
class ConeEntropyFacts : Prop where
  cone_entropy_bound :
    ∀ {α : Type _} (cone : LightCone α) (area : ℝ),
      entropy cone ≤ area / (4 * lambda_rec^2)
...
/-- Cone entropy bound: Entropy in a cone is bounded by area over 4 lambda_rec².

    This is currently an axiom (typeclass assumption). A full proof would require:
    - Voxel counting: number of voxels ~ area / lambda_rec²
    - Entropy per voxel: ~ ln φ from ledger structure
    - Holographic principle: caps total at area/(4lambda_rec²)
