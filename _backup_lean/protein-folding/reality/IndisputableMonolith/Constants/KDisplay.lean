...
theorem K_gate_units_invariant (U : RSUnits) (α : ℝ) (hα : 0 < α) (hτ : 0 < U.tau0) :
  let U' : RSUnits := { tau0 := α * U.tau0, ell0 := α * U.ell0, c := U.c, c_ell0_tau0 := by field_simp; ring }
...
def K_gate_tolerance (U : RSUnits) (σ_tau σ_ell : ℝ) : ℝ :=
...
def K_gate_check (tau_meas lambda_meas : ℝ) (U : RSUnits) (tolerance : ℝ) : Bool :=
...
theorem display_speed_positive (U : RSUnits) (h : 0 < U.tau0) :
...
theorem display_null_condition (U : RSUnits) (h : 0 < U.tau0) :
  ...
  have : (lambda_kin_display U) / (tau_rec_display U) = U.c := display_speed_eq_c U h
  rw [this]
...
theorem UnitsEquivalent.symm {U1 U2 : RSUnits} (h : UnitsEquivalent U1 U2) :
  ...
  refine ⟨hc.symm, α⁻¹, ?_, ?_, ?_⟩
  · exact inv_ne_zero hα
  · field_simp [hα]; exact hτ.symm
  · field_simp [hα]; exact hℓ.symm
...
theorem displays_invariant_under_equivalence {U1 U2 : RSUnits}
  ...
  rw [tau_rec_display_ratio U1 hτ1, tau_rec_display_ratio U2 (by obtain ⟨_, α, hα, hτ, _⟩ := h; omega)]
...
def validateKGate (meas : KGateMeasurement) : Bool :=
...
def falsifier_K_gate_mismatch (meas : KGateMeasurement) : Bool :=
  ¬validateKGate meas

/-! Bridge Factorization -/

/-- Observable displays factor through units quotient -/
theorem observable_factors_through_quotient (O : RSUnits → ℝ)
    (hQuot : ∀ U α, α ≠ 0 → O {tau0 := α * U.tau0, ell0 := α * U.ell0, c := U.c, c_ell0_tau0 := by field_simp; rw [mul_comm]; exact U.c_ell0_tau0} = O U) :
...
/-- Standard K-gate verification procedure.
    NOTE: Placeholder. Full verification requires empirical measurement infrastructure.
    Procedure:
...
theorem k_gate_verification_procedure : True := trivial
