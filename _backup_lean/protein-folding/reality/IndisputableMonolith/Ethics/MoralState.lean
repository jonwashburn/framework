...
theorem globally_admissible_append {xs ys : List MoralState}
  ...
  let f : ℤ → MoralState → ℤ := fun acc s => acc + s.skew
...
/-- Helper lemma for folding sums (used in total_energy_positive_of_nonempty). -/
lemma total_energy_foldl_add_const (xs : List MoralState) (acc : ℝ) :
  ...
  | nil => simp
  | cons x xs ih =>
      -- Fold one element, then use the induction hypothesis at the updated accumulator.
      simp [List.foldl, ih (acc + x.energy), ih x.energy, add_assoc, add_left_comm, add_comm]
  ...
  have total_nonneg : ∀ xs, 0 ≤ MoralState.total_energy xs := by
    ...
    | cons s ss ih =>
        ...
        have hrec : MoralState.total_energy (s :: ss) =
          ...
          simp only [MoralState.total_energy, List.foldl]
          rw [total_energy_foldl_add_const]
          ring
