...
/--
Certificate layer gluing the φ-scheduler execution to symmetry-ledger
...
def passes (ratios : ModeRatios (Mode := Mode)) : Prop :=
  Fusion.SymmetryLedger.pass cert.ledgerCfg cert.bounds cert.ledgerThreshold ratios

def authorizes (exec : cert.scheduler.Execution) (ratios : ModeRatios (Mode := Mode)) : Prop :=
  passes (cert := cert) ratios
...
lemma authorizes_of_unity
  ...
  have := Fusion.SymmetryLedger.unity_pass (cfg := cert.ledgerCfg)
