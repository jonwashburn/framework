...
/-- Opaque placeholder for the tier-3 confirm-sweep dataset collection.

This is not definitional data; it is a *named hook* for external validation. -/
axiom clinvar_tier3_confirm_sweep : List (List CodonVariant)

/-- Every dataset in the confirm sweep supports E4 in the mean-direction sense.

This axiom stands in for the preregistered seed-sweep validation performed in Python. -/
axiom clinvar_tier3_confirm_sweep_directional :
    ∀ ds, List.Mem ds clinvar_tier3_confirm_sweep → supports_E4_mean ds

/-- The preregistered aggregate (Stouffer) z-score for the confirm sweep. -/
axiom clinvar_tier3_confirm_stouffer_z : ℝ

/-- Preregistered pass condition (one-tailed): z > 1.645. -/
axiom clinvar_tier3_confirm_stouffer_pass :
    clinvar_tier3_confirm_stouffer_z > 1.645
