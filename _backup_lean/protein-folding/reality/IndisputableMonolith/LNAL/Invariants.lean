...
/-- Single step: breath advances by 1 mod breathPeriod.
    Note: The proof is direct from `l_breath_lt_period` in VM.lean.
    TODO: complete tactic proof (currently requires navigating the complex lStep definition). -/
lemma lStep_breath_eq (P : LProgram) (s : LState) :
  ...
  -- where lBumpBreath s := (s.breath + 1) % breathPeriod
  -- Direct extraction requires case-splitting through the full lStep structure.
  unfold lStep
  by_cases hH : s.halted
  · -- halted branch: core = s, breath bumps deterministically
    simp [hH, lBumpBreath, breathPeriod]
  ·
    -- running branch: all opcode branches keep `breath := s.breath`; bump applied at end
    cases hop : (P s.ip).op <;> simp [hH, lFetch, hop, lBumpBreath, breathPeriod]

/-- Breath position after n steps is (breath + n) % breathPeriod.
    TODO: prove by induction using lStep_breath_eq once that is completed. -/
lemma lStep_breath_after (P : LProgram) (s : LState) (hB : BreathBound s) :
  ...
  | zero =>
      ...
      -- (We unfold the compat `Function.iterate` shim explicitly.)
      -- Need the *symmetry* of `Nat.mod_eq_of_lt` because the goal is `b = b % m`.
  ...
  | succ n ih =>
      ...
      have h1 : 1 % breathPeriod = 1 := by
        ...
        exact Nat.mod_eq_of_lt (by decide : (1 : Nat) < breathPeriod)
      calc
...
/-- After breathPeriod steps, breath returns to initial value (requires BreathBound).
    TODO: follows from lStep_breath_after with n = breathPeriod. -/
lemma lStep_breath_periodic (P : LProgram) (s : LState) (hB : BreathBound s) :
  ...
  -- h : (iterate (lStep P) breathPeriod s).breath = (s.breath + breathPeriod) % breathPeriod
...
lemma token_delta_unit (P : LProgram) (s : LState)
  ...
  have hdiff_le : y - x ≤ (1 : Int) := by
    ...
    have h1 : y + (-x) ≤ y + 0 := add_le_add_left hnegx y
    have h2 : y - x ≤ y := by simpa [sub_eq_add_neg] using h1
  ...
  have hdiff_ge : (-1 : Int) ≤ y - x := by
    ...
    have hnegx_le : -x ≤ y - x := by
      have h : (0 : Int) + (-x) ≤ y + (-x) := add_le_add_right hy0 (-x)
