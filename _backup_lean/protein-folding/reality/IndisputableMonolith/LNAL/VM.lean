...
def lStep (P : LProgram) (s : LState) : LState :=
  let core :=
    if s.halted then s else
      ...
      let rollover : Bool := decide (advIdx = 0)
      ...
      let nonNeutral : Bool := rollover && decide (sFinal.winSum8 ≠ (0 : Int))
      let jUp : Bool := decide (costVal < 0)
      let consentFail :=
...
@[simp] def programOfArray (code : Array LInstr) : LProgram :=
  fun idx =>
    if h : idx < code.size then
      -- Safe indexing: `code[idx]` uses the bound proof `h`.
      code[idx]
    else
...
def eightBeatVector : List (Nat × Nat × Nat × Bool) :=
  (enumerate eightBeatTrace).map fun
...
-- Note: keep `#eval` out of library builds (it fails if any dependency contains `sorry`).
