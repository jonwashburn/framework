...
/-- Minimality predicate: Γ derives the master reality bundle at φ with provenance. -/
def Sufficient (Γ : AxiomEnv) (φ : ℝ) : Prop :=
  DerivesWithUsage Γ (IndisputableMonolith.Verification.Reality.RSRealityMaster φ)
...
theorem mp_sufficient (φ : ℝ) : Sufficient mpOnlyEnv φ := by
  ...
  , requiresMP := trivial
  , proof := IndisputableMonolith.Verification.Reality.rs_reality_master_any φ }
...
theorem no_weaker_than_mp_sufficient (φ : ℝ) :
  ...
  intro Γ hNoMP hS
  -- Any provenance witness must use MP and embed usage ≤ Γ, forcing Γ.usesMP
  have hΓmp : Γ.usesMP := hS.used_le.1 hS.requiresMP
  exact hNoMP hΓmp
...
theorem mp_minimal_holds (φ : ℝ) : MPMinimal φ := by
  ...
  intro Γ hle hS
  -- From the provenance witness hS, Γ must include MP
  have hHasMP : Γ.usesMP := hS.used_le.1 hS.requiresMP
