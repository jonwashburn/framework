...
lemma stretch_bound (c : ℝ) : |c| + 1 > 0 := by have : 0 ≤ |c| := abs_nonneg _; exact add_pos_of_nonneg_of_pos this (by norm_num)

/-- Stub small-o specification (placeholder). -/
def IsLittleO (_f g : ℝ → ℝ) (_a : ℝ) : Prop := True

@[simp] lemma isLittleO_mul {f g h : ℝ → ℝ} {a : ℝ}
  (_hf : IsLittleO f g a) (_hg : True) : True := trivial
