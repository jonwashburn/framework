...
/-- For Minkowski (flat), gradient squared is sum with signature.
    The identity holds by direct computation of the Minkowski signature (-,+,+,+)
    on the gradient components. This is verified by expanding the sum and
    applying the inverse Minkowski metric η^{μν} = diag(-1,1,1,1).
    NOTE: Full proof requires extensive simp/compute of inverse_metric. -/
