...
axiom H_cross_octave_validation (k : Nat) (obs : List FoldObs) :
  PreregisteredDataset obs → CrossOctaveViolationsAtMost k obs
...
/-- Paper-ready statement: the hypothesis instantiated at the default violation budget. -/
theorem H_cross_octave_validation_default (obs : List FoldObs) :
    PreregisteredDataset obs → CrossOctaveViolationsAtMost (kDefault obs) obs := by
  intro h
  exact H_cross_octave_validation (k := kDefault obs) (obs := obs) h
