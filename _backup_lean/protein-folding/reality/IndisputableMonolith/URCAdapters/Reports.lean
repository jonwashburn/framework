...
def lambda_report : String := "URC lambda_rec uniqueness: OK"
...
/-- #eval-friendly report: show the axiom-usage provenance for RSRealityMaster at φ
    (constructed with MP-only usage). -/
def mp_provenance_report : String :=
  let φ : ℝ := IndisputableMonolith.Constants.phi
  let Γ := IndisputableMonolith.Meta.AxiomLattice.mpOnlyEnv
  let _ := IndisputableMonolith.Meta.FromMP.mp_derives_reality_master_with_usage Γ (by trivial) φ
  String.intercalate "\n"
    [ "Provenance: RSRealityMaster derived with MP-only usage"
    , "  - MP: True"
    , "  - AtomicTick: False"
    , "  - Continuity: False"
...
/-- #eval-friendly physical witness for lambda_rec identities requiring Physical B. -/
...
def k_gate_report : String :=
  ...
  let obsWitness :=
    IndisputableMonolith.Verification.Identifiability.observe_eq_ud φ F
  have _ := obsWitness
  ...
  "KGateCert: OK (observe_eq_ud via T4/T5 ⇒ K-gate)"
...
/-- #eval-friendly report for ScheduleNeutralityCert. -/
def schedule_neutrality_cert_report : String :=
  let cert : URCGenerators.ScheduleNeutralityCert := {}
  have _ : URCGenerators.ScheduleNeutralityCert.verified cert :=
    URCGenerators.ScheduleNeutralityCert.verified_any _
  "ScheduleNeutralityCert: OK"

/-- #eval-friendly report for CostCeilingCert. -/
def cost_ceiling_cert_report : String :=
  let cert : URCGenerators.CostCeilingCert := {}
  have _ : URCGenerators.CostCeilingCert.verified cert :=
    URCGenerators.CostCeilingCert.verified_any _
  "CostCeilingCert: OK"

/-- #eval-friendly report for OpcodeSoundnessCert. -/
def opcode_soundness_cert_report : String :=
  let cert : URCGenerators.OpcodeSoundnessCert := {}
  have _ : URCGenerators.OpcodeSoundnessCert.verified cert :=
    URCGenerators.OpcodeSoundnessCert.verified_any _
  "OpcodeSoundnessCert: OK"

/-- #eval-friendly report for SU3MaskCert. -/
def su3_mask_cert_report : String :=
  let cert : URCGenerators.SU3MaskCert := {}
  have _ : URCGenerators.SU3MaskCert.verified cert :=
    URCGenerators.SU3MaskCert.verified_any _
  "SU3MaskCert: OK"
...
def absolute_layer_invariant_report : String :=
  ...
  let A : IndisputableMonolith.RH.RS.Anchors :=
    { a1 := U.c
    , a2 := U.ell0
    , consistent := by
        intro h
        have h' : U.ell0 = U.c * U.tau0 := by simpa using U.c_ell0_tau0.symm
        simpa [h] using h' }
...
/-- #eval-friendly report for LambdaRecUncertaintyCert (u_rel(lambda_rec)=½u_rel(G)). -/
...
def certificates_manifest : String :=
  String.intercalate "\n"
    ...
    , schedule_neutrality_cert_report
    , cost_ceiling_cert_report
    , opcode_soundness_cert_report
    , su3_mask_cert_report
    , dec_dd_zero_report
    , dec_bianchi_report
...
def identifiability_cost_report : String :=
  ...
  "IdentifiabilityCost: OK (observe_eq_ud via T4/T5 ⇒ costOf = 0)"
...
/-- #eval-friendly consolidated audit identities report (K‑gate, K identities, lambda_rec identity, single‑inequality). -/
...
def weakfield_ilg_report : String :=
  let cert : URCGenerators.RouteAGateIdentityCert := {}
  have h := URCGenerators.RouteAGateIdentityCert.verified_any cert
  let P : IndisputableMonolith.ILG.Params :=
    { alpha := 1 / 5
    , Clag := 1 / 10
    , A := 1 / 10
    , r0 := 1
    , p := 1
    , hz_over_Rd := 1 / 4 }
  have hP : IndisputableMonolith.ILG.ParamProps P :=
    { alpha_nonneg := by norm_num
    , Clag_nonneg := by norm_num
    , Clag_le_one := by norm_num
    , A_nonneg := by norm_num
    , r0_pos := by norm_num
    , p_pos := by norm_num }
  let h_core := h.right
  have _ := h_core P hP (1 / 2) 1
  have _ := h_core P hP 2 1
  let ψ : IndisputableMonolith.Relativity.ILG.RefreshField := fun _ => 0
  have _ := IndisputableMonolith.Relativity.ILG.gamma_nl_bound ψ P
  have _ := IndisputableMonolith.Relativity.ILG.beta_nl_bound ψ P
  "WeakfieldILGCert: OK"
...
def lensing_band_report : String :=
  let cert : URCGenerators.LensingBandCert := {}
  have _ : URCGenerators.LensingBandCert.verified cert :=
    URCGenerators.LensingBandCert.verified_any _
  "LensingBandCert: OK (strength/shear/deflection/time-delay bounds)"
...
/-- #eval-friendly report confirming the ILG time-kernel form properties. -/
def ilg_kernel_report : String :=
  let cert : URCGenerators.RouteAGateIdentityCert := {}
  have h := URCGenerators.RouteAGateIdentityCert.verified_any cert
  let _ := h.right -- time-kernel bundle
  "ILGKernelFormCert: OK"
