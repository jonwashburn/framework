...
/-- There exists a unique scale `φ` satisfying both the φ-selection predicate and the
    recognition-closure obligations. This packages the algebraic characterization of φ
    (unique positive root of `x² = x + 1`) together with the uniform closure witness. -/
...
/-- Certificate asserting φ-selection uniqueness together with recognition closure. -/
...
@[simp] theorem PhiSelectionSpecCert.verified_any (c : PhiSelectionSpecCert) :
  ...
  phi_selection_unique_with_closure
...
/‑! Planck length identity: lambda_rec = L_P/√π with L_P^2 = ħG/c^3. -/

/-- Certificate asserting lambda_rec = L_P / √π where
...
@[simp] theorem PlanckLengthIdentityCert.verified_any (c : PlanckLengthIdentityCert) :
  ...
  intro B H
  -- Start from the definition lambda_rec = √(ħ G / (π c^3)) and separate √π.
...
@[simp] def RouteAGateIdentityCert.verified (_c : RouteAGateIdentityCert) : Prop :=
  ...
  (∀ (P : IndisputableMonolith.ILG.Params)
      (hP : IndisputableMonolith.ILG.ParamProps P)
      (Tdyn τ0 : ℝ),
      0 ≤ IndisputableMonolith.ILG.w_t P Tdyn τ0 ∧
      IndisputableMonolith.ILG.w_t P τ0 τ0 = 1 ∧
      (∀ {c : ℝ}, 0 < c →
        IndisputableMonolith.ILG.w_t P (c * Tdyn) (c * τ0)
          = IndisputableMonolith.ILG.w_t P Tdyn τ0) ∧
      IndisputableMonolith.ILG.w_t P Tdyn τ0
        = 1 + P.Clag * (Real.rpow (max IndisputableMonolith.ILG.εt (Tdyn / τ0)) P.alpha - 1) ∧
      (∀ {v_baryon2 : ℝ},
        IndisputableMonolith.Relativity.ILG.v_model2 v_baryon2
            (IndisputableMonolith.ILG.w_t P Tdyn τ0)
          = IndisputableMonolith.ILG.w_t P Tdyn τ0 * v_baryon2) ∧
      (0 < τ0 → Tdyn ≤ τ0 →
          IndisputableMonolith.ILG.w_t P Tdyn τ0 ≤ 1) ∧
      (0 < τ0 → τ0 ≤ Tdyn →
          1 ≤ IndisputableMonolith.ILG.w_t P Tdyn τ0) ∧
      (P.Clag = 0 → IndisputableMonolith.ILG.w_t P Tdyn τ0 = 1) ∧
      (P.alpha = 0 → IndisputableMonolith.ILG.w_t P Tdyn τ0 = 1) ∧
      (∀ (ψ : IndisputableMonolith.Relativity.ILG.RefreshField),
        |IndisputableMonolith.Relativity.ILG.gamma_nl ψ P - 1|
          ≤ (1/10 : ℝ) * |P.Clag * P.alpha|
            + (1/100 : ℝ) * (|P.Clag * P.alpha| * |P.Clag * P.alpha|)) ∧
      (∀ (ψ : IndisputableMonolith.Relativity.ILG.RefreshField),
        |IndisputableMonolith.Relativity.ILG.beta_nl ψ P - 1|
          ≤ (1/20 : ℝ) * |P.Clag * P.alpha|
            + (1/400 : ℝ) * (|P.Clag * P.alpha| * |P.Clag * P.alpha|)))

@[simp] theorem RouteAGateIdentityCert.verified_any (c : RouteAGateIdentityCert) :
  RouteAGateIdentityCert.verified c := by
  refine And.intro ?hPlanck ?hKernel
  · intro B hτ
  ...
  · intro P hP Tdyn τ0
    refine And.intro ?hNonneg ?hrest
    · exact IndisputableMonolith.Relativity.ILG.time_kernel_nonneg P hP Tdyn τ0
    · refine And.intro ?href ?hmore
      · simpa using IndisputableMonolith.Relativity.ILG.time_kernel_reference P τ0
      · refine And.intro ?hrescale ?hexplicit
        · intro c hc
          simpa using IndisputableMonolith.Relativity.ILG.time_kernel_rescale P c Tdyn τ0 hc
        · refine And.intro ?hformula ?hrotation
          · simpa using IndisputableMonolith.Relativity.ILG.time_kernel_explicit P Tdyn τ0
          · intro v_baryon2
            simpa using
              IndisputableMonolith.Relativity.ILG.time_kernel_rotation_consistent P
                v_baryon2 Tdyn τ0

/‑! lambda_rec relative scaling under G rescaling: √k scaling (⇒ u_rel(lambda_rec)=½u_rel(G)). -/

/-- Certificate asserting: if one rescales G ↦ k·G with k>0 (holding ħ and c fixed),
    then lambda_rec scales as √k. This implies dλ/λ = (1/2) dG/G and hence
    u_rel(lambda_rec) = 1/2 · u_rel(G). -/
...
@[simp] theorem LambdaRecUncertaintyCert.verified_any (c : LambdaRecUncertaintyCert) :
  ...
  intro B H k hk
  -- lambda_rec(B') with G' = k·G equals √k · lambda_rec(B)
  dsimp [IndisputableMonolith.BridgeData.lambda_rec]
  -- Positivity
...
/-! lambda_rec identity (Planck-side normalization) -/

/-- Certificate asserting the Planck-side identity (c^3 · lambda_rec^2)/(ħ G) = 1/π. -/
...
@[simp] def PlanckGateToleranceCert.verified (_c : PlanckGateToleranceCert) : Prop :=
  -- Exact Planck-side normalization: zero tolerance on (c^3 lambda_rec^2)/(ħ G) − 1/π.
  (∀ (B : IndisputableMonolith.BridgeData) (H : IndisputableMonolith.BridgeData.Physical B),
      Real.abs ((B.c ^ 3) * (IndisputableMonolith.BridgeData.lambda_rec B) ^ 2 / (B.hbar * B.G) - (1 / Real.pi)) ≤ 0)
  ∧
  -- Uncertainty scaling: G ↦ k·G ⇒ lambda_rec ↦ √k·lambda_rec (positivity k>0).
  (∀ (B : IndisputableMonolith.BridgeData) (H : IndisputableMonolith.BridgeData.Physical B)
      (k : ℝ), 0 < k →
      IndisputableMonolith.BridgeData.lambda_rec ({ B with G := k * B.G })
        = Real.sqrt k * IndisputableMonolith.BridgeData.lambda_rec B)
  ∧
  -- Negative control: additive offset on lambda_rec breaks the identity on a physical witness.
  (∃ (B : IndisputableMonolith.BridgeData) (H : IndisputableMonolith.BridgeData.Physical B),
...
@[simp] theorem PlanckGateToleranceCert.verified_any (c : PlanckGateToleranceCert) :
  ...
  · refine ⟨{ G := 1, hbar := 1, c := 1, tau0 := 1, ell0 := 1 }, { c_pos := by norm_num, hbar_pos := by norm_num, G_pos := by norm_num }, ?_⟩
    -- For this B, (lambda_rec)^2 = 1/π, hence (lambda_rec+1)^2 = 1/π + 2 lambda_rec + 1 > 1/π
...
/-- Certificate asserting the lensing small-coupling band condition. -/
...
@[simp] def LensingSmallCouplingCert.verified (_c : LensingSmallCouplingCert) : Prop :=
  (∀ (ψ : IndisputableMonolith.Relativity.ILG.RefreshField)
      ...
      |IndisputableMonolith.Relativity.ILG.lensing_strength ψ p - 1|
        ≤ (1 / 20 : ℝ) * |p.cLag * p.alpha|
          + (1 / 200 : ℝ) * (|p.cLag * p.alpha| * |p.cLag * p.alpha|))
  ∧ (∀ (ψ : IndisputableMonolith.Relativity.ILG.RefreshField)
      (p : IndisputableMonolith.Relativity.ILG.ILGParams)
      (ℓ κ : ℝ),
      0 ≤ κ → |p.cLag * p.alpha| ≤ κ →
        |IndisputableMonolith.Relativity.ILG.deflection ψ p ℓ
            - IndisputableMonolith.Relativity.ILG.deflection_GR ℓ|
          ≤ ((1 / 20 : ℝ) * κ + (1 / 200 : ℝ) * κ * κ) * |ℓ|)
@[simp] theorem LensingSmallCouplingCert.verified_any (c : LensingSmallCouplingCert) :
  LensingSmallCouplingCert.verified c := by
  constructor
  · intro ψ p
    simpa using IndisputableMonolith.Relativity.ILG.lensing_strength_bound ψ p
  · intro ψ p ℓ κ hκ hbound
    simpa using
      IndisputableMonolith.Relativity.ILG.deflection_bound_of_kappa ψ p ℓ κ hκ hbound
...
/‑! Weak‑field ε‑expansion and EL/zero‑path certificates -/
...
/‑-! FRW existence certificate (scaffold: existence and healthy kinetic) -/
structure FRWExistenceCert where deriving Repr
...
@[simp] def ClusterLensingDeriveCert.verified (_c : ClusterLensingDeriveCert) : Prop :=
  ∀ (ψ : IndisputableMonolith.Relativity.ILG.RefreshField)
    (p : IndisputableMonolith.Relativity.ILG.ILGParams) (ℓ κ : ℝ),
      0 ≤ κ → |p.cLag * p.alpha| ≤ κ →
      |IndisputableMonolith.Relativity.ILG.deflection ψ p ℓ
          - IndisputableMonolith.Relativity.ILG.deflection_GR ℓ|
        ≤ ((1 / 20 : ℝ) * κ + (1 / 200 : ℝ) * κ * κ) * |ℓ|
      ∧ |IndisputableMonolith.Relativity.ILG.time_delay ψ p ℓ
          - IndisputableMonolith.Relativity.ILG.time_delay_GR ℓ|
        ≤ ((1 / 20 : ℝ) * κ + (1 / 200 : ℝ) * κ * κ) * |ℓ|
@[simp] theorem ClusterLensingDeriveCert.verified_any (c : ClusterLensingDeriveCert) :
  ...
  · simpa using
      IndisputableMonolith.Relativity.ILG.deflection_bound_of_kappa ψ p ℓ κ hκ hbound
  · simpa using
      IndisputableMonolith.Relativity.ILG.time_delay_bound_of_kappa ψ p ℓ κ hκ hbound
