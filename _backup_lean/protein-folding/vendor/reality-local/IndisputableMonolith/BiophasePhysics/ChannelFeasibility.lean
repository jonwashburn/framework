...
/-! ## Physical Realizability Notes

The exclusion of non-EM channels is handled by axioms in `Consciousness.BioPhaseSNR`:
- `gravitational_fails_biophase`: Gravitational cross-section too small
- `neutrino_fails_biophase`: Weak interaction cross-section too small
- `other_channels_fail_biophase`: Unmodeled channels excluded by default

These are physical claims about cross-sections and SNR calculations.
-/
...
/-- Gravitational fails BIOPHASE (hypothesis-based).
    The original axiom was mathematically false because PassesBiophase
    doesn't depend on channel type. Now uses explicit hypothesis. -/
theorem gravitational_fails_biophase_proven (spec : BiophaseSpec)
    (h : Consciousness.gravitational_fails_biophase_hypothesis spec) :
    ¬PassesBiophase spec ChannelType.Gravitational := h

/-- Neutrino fails BIOPHASE (hypothesis-based). -/
theorem neutrino_fails_biophase_proven (spec : BiophaseSpec)
    (h : Consciousness.neutrino_fails_biophase_hypothesis spec) :
    ¬PassesBiophase spec ChannelType.Neutrino := h

/-! ## Channel Classification -/

/-- At BIOPHASE conditions, only EM is feasible (hypothesis-based). -/
theorem biophase_channel_selection (spec : BiophaseSpec)
    (h_grav : Consciousness.gravitational_fails_biophase_hypothesis spec)
    (h_nu : Consciousness.neutrino_fails_biophase_hypothesis spec) :
  ...
  constructor
  · exact gravitational_fails_biophase_proven spec h_grav
  · exact neutrino_fails_biophase_proven spec h_nu

/-- Main theorem: Only EM satisfies BIOPHASE acceptance (hypothesis-based). -/
theorem only_em_feasible (spec : BiophaseSpec)
    (h_grav : Consciousness.gravitational_fails_biophase_hypothesis spec)
    (h_nu : Consciousness.neutrino_fails_biophase_hypothesis spec)
    (h_other : Consciousness.other_channels_fail_biophase_hypothesis spec) :
  ∀ (channel : ChannelType),
...
/-- StandardBiophase excludes gravitational (hypothesis-based). -/
theorem standard_biophase_grav_fails
    (h : Consciousness.gravitational_fails_biophase_hypothesis StandardBiophase) :
  ¬PassesBiophase StandardBiophase ChannelType.Gravitational :=
  gravitational_fails_biophase_proven StandardBiophase h

/-- StandardBiophase excludes neutrino (hypothesis-based). -/
theorem standard_biophase_nu_fails
    (h : Consciousness.neutrino_fails_biophase_hypothesis StandardBiophase) :
  ¬PassesBiophase StandardBiophase ChannelType.Neutrino :=
  neutrino_fails_biophase_proven StandardBiophase h
