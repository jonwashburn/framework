...
/-- A photon channel satisfies the dimensional consistency requirement.
    This follows from the WellFormed constraint on PhotonChannel. -/
theorem photon_channel_dimensionless (pc : PhotonChannel) [wf : PhotonChannel.WellFormed pc]
    (hτ : pc.units.tau0 ≠ 0) (hℓ : pc.units.ell0 ≠ 0) : wf.dimensionless hτ hℓ :=
  wf.dimensionless hτ hℓ
...
theorem conscious_to_photon_witness (cp : ConsciousProcess) [wf : ConsciousProcess.WellFormed cp]
    ...
    (∀ x, let □ := @HasCoboundary.d mesh cb 2 ∘ @HasHodge.star mesh hd 2 ∘ @HasCoboundary.d mesh cb 1
          (□ A) x = 0) → -- Wave equation
    ∃ (pc : PhotonChannel),
      pc.units = cp.units ∧
      pc.bridge = cp.bridge ∧
      PhotonChannel.WellFormed pc := by
  intro F hBianchi hCont hWave
