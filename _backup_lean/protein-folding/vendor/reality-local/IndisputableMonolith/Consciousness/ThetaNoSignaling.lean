...
/-- A local response function: given the global phase and measurement choice,
    ...
    Simplified model: uniform distribution that doesn't depend on the specific
    choice, but the proof structure works for any normalized response. -/
noncomputable def localResponse (_obs : Observer) (_t : ℝ) :
    PhaseDiscrete → MeasurementChoice → Outcome → ℝ :=
  fun _globalPhase _choice _outcome => 1 / 2
