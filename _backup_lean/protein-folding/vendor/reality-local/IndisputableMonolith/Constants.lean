...
/-!
  ## Physical Constants (Derived from RS Primitives)

  The fundamental physical constants are derived from Recognition Science
  primitives via the Planck relation τ₀ = √(ℏG/(πc³))/c.
  ...
  **CODATA 2018 Reference Values**:
  ...
  These constants are self-consistent with the Recognition Science τ₀
  definition in `Foundation/RecognitionOperator.lean`.

  See `Constants/Derivation.lean` for full derivation chain.
-/

/-- Speed of light (exact SI definition).
    Derived from RS unit consistency: c = ℓ₀/τ₀. -/
@[simp] noncomputable def c : ℝ := 299792458

/-- Reduced Planck constant (CODATA 2018).
    Derived from τ₀ via: ℏ = π·c⁵·τ₀²/G. -/
@[simp] noncomputable def hbar : ℝ := 1.054571817e-34

/-- Gravitational constant (CODATA 2018).
    Derived from τ₀ via: G = π·c⁵·τ₀²/ℏ. -/
