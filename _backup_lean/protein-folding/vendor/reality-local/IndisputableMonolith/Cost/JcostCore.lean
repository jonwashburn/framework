...
lemma Jcost_deriv (x : ℝ) (hx : x ≠ 0) :
  ...
  unfold Jcost
  simp only [deriv_sub_const, deriv_div_const]
  have hdiff : DifferentiableAt ℝ (fun x => x + x⁻¹) x := by
    apply DifferentiableAt.add differentiableAt_id (differentiableAt_inv hx)
  rw [deriv_add differentiableAt_id (differentiableAt_inv hx)]
  simp only [deriv_id'', deriv_inv']
  ring
...
lemma Jcost_strict_mono_on_one_infty (x y : ℝ) (hx : 0 < x) (hy : 0 < y)
  ...
  have hmain : (x - 1) ^ 2 * (2 * y) < (y - 1) ^ 2 * (2 * x) := by
    ...
    have f := fun t => (t - 1) ^ 2 / t
    have hf_mono : ∀ a b, 1 ≤ a → a < b → f a < f b := by
      intro a b ha hab
      simp only [f]
