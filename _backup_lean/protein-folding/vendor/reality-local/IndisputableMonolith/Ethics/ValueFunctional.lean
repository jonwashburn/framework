...
lemma agent_states_nonempty (p : AgentEnvDistribution) :
  ...
  have hsum :
    ...
    simp [Finset.not_nonempty_iff.mp h]
...
lemma sum_agentMarginal (p : AgentEnvDistribution) :
  ...
  have hsum :=
    p.prob_normalized
  refine Finset.sum_congr rfl ?_ ▸ hsum
  intro a ha
  exact (agentMarginal_eq_sum (p := p) (a := a) ha).symm

lemma sum_envMarginal (p : AgentEnvDistribution) :
    p.env_states.sum (fun e => p.envMarginal e) = 1 := by
  classical
  have hprod :=
    AgentEnvDistribution.sum_prob_product (p := p)
  have hrewrite :
      p.env_states.sum (fun e => p.envMarginal e) =
        ∑ se ∈ p.agent_states.product p.env_states, p.prob se.1 se.2 := by
    unfold envMarginal
    simp [Finset.sum_product, Finset.sum_sigma', Finset.sum_attach]
  simpa [hrewrite] using hprod

noncomputable def envDefault (p : AgentEnvDistribution) : {e // e ∈ p.env_states} :=
  Classical.choice (env_states_nonempty (p := p))
...
lemma sum_agentWeights (p : AgentEnvDistribution) :
  ...
  classical
  have hsum :=
    AgentEnvDistribution.sum_agentMarginal (p := p)
