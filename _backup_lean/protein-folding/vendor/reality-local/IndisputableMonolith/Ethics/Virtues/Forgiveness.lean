  ...
  let creditor' : MoralState := {
    ...
    energy_pos := by
      have h_transfer_bound : transfer_amount ≤ 50 := by
        ...
        exact le_trans this h_reasonable
      have h_penalty_bound : energy_penalty ≤ creditor.energy / 2 := by
        unfold energy_penalty
        have h_ratio : (transfer_amount : ℝ) / 100 ≤ 1 / 2 := by
          apply div_le_div_of_nonneg_right
          · exact_mod_cast h_transfer_bound
          · norm_num
        calc creditor.energy * (transfer_amount : ℝ) / 100
            = creditor.energy * ((transfer_amount : ℝ) / 100) := by ring
          _ ≤ creditor.energy * (1 / 2) := by
              apply mul_le_mul_of_nonneg_left h_ratio (le_of_lt creditor.energy_pos)
