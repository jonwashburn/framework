...
/-! ### Closed Form and Bounds -/

lemma iterate_update_cooperation_closed_form (p : ℝ) (n : ℕ) :
  (update_cooperation^[n]) p =
    1 - (1 - p) * (1 - 1 / Foundation.φ) ^ n := by
  induction n with
  | zero => simp
  | succ k ih =>
    -- ih : Nat.iterate update_cooperation k p = 1 - (1 - p) * r^k
    -- Goal: Nat.iterate update_cooperation (k+1) p = 1 - (1 - p) * r^(k+1)
    have hφ_pos : 0 < Foundation.φ := Support.GoldenRatio.phi_pos
    have hφ_ne_zero : Foundation.φ ≠ 0 := ne_of_gt hφ_pos
    simp only [Function.iterate_succ', Function.comp_apply]
    simp only [ih, update_cooperation, pow_succ]
    field_simp
    ring

lemma iterate_update_cooperation_gap (p : ℝ) :
  ∀ n : ℕ,
    1 - (update_cooperation^[n]) p =
...
lemma iterate_update_cooperation_bounds
  (p : ℝ) (hp : 0 ≤ p ∧ p ≤ 1) :
  ∀ n : ℕ, 0 ≤ (update_cooperation^[n]) p ∧
      (update_cooperation^[n]) p ≤ 1 := by
  intro n
  rw [iterate_update_cooperation_closed_form]
  constructor
  · -- 0 ≤ 1 - (1-p) * r^n where r = 1 - 1/φ ∈ (0,1)
    ...
    have h_inv_lt_one : 1 / Foundation.φ < 1 := by
      ...
      exact hφ_gt_one
    have h_r_pos : 0 < 1 - 1 / Foundation.φ := by linarith
    have h_inv_pos : 0 ≤ 1 / Foundation.φ := le_of_lt (div_pos one_pos hφ_pos)
...
theorem gratitude_phi_rate_optimal :
  ...
  have hφ_pos : 0 < Foundation.φ := Support.GoldenRatio.phi_pos
  have hφ_gt_one : 1 < Foundation.φ := Support.GoldenRatio.one_lt_phi
  refine ⟨?_, ?_, Support.GoldenRatio.phi_squared_eq_phi_plus_one⟩
...
theorem gratitude_stabilizes_cooperation
  ...
  1 - (gratitude_iterate (n + 1) state).propensity =
    (1 - (gratitude_iterate n state).propensity) * (1 - 1 / Foundation.φ) := by
  rw [gratitude_iterate_propensity, gratitude_iterate_propensity]
  rw [iterate_update_cooperation_closed_form, iterate_update_cooperation_closed_form]
  ring
...
theorem gratitude_monotonic
  (state : CooperationState) (n : ℕ) :
  (gratitude_iterate n state).propensity ≤
    (gratitude_iterate (n + 1) state).propensity := by
  rw [gratitude_iterate_succ, gratitude_step_propensity]
...
theorem gratitude_enables_cooperation_equilibrium (state : CooperationState) :
  ...
  --   update_cooperation^[n] state.propensity
  have h_eq : ∀ n, (gratitude_iterate n state).propensity =
      (update_cooperation^[n]) state.propensity := fun n => gratitude_iterate_propensity n state
  simp_rw [h_eq]
