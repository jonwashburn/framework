...
theorem pairList_eq_filtered_range_theorem (nf : NormalForm)
  ...
  unfold pairList
  -- Both lists are sorted by < and have the same elements.
  apply List.Sorted.ext (Finset.sort_sorted (· ≤ ·) _)
  · apply List.Sorted.filter
    exact List.range_sorted 16
  · intro x
    rw [Finset.mem_sort, List.mem_filter, List.mem_range]
    constructor
    · intro hx; exact ⟨hw hx |> Finset.mem_range.mp, hx⟩
    · intro hx; exact hx.2
