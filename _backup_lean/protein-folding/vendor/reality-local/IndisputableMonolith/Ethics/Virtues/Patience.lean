...
theorem totalWait_lt_of_impatient
  ...
  | cons a as ih =>
    rw [totalWait_cons, List.length_cons]
    have h_a : interArrival a < patience_threshold := by
      have h := h_impatient a (List.mem_cons_self _ _)
      unfold is_patient at h
      omega
    ...
    · have h_as : ∀ a' ∈ as, ¬ is_patient a'.time_executed a'.time_last_action := by
        ...
        apply h_impatient
        simp [ha']
      have ih_applied := ih h_as h_as_empty
      calc a.interArrival + totalWait as
          < patience_threshold + totalWait as := by linarith
        _ < patience_threshold + patience_threshold * as.length := by linarith
...
theorem patience_enables_wisdom
  ...
  have h_weight_pos : 0 < weight := by
    ...
    linarith
  exact (mul_right_inj' h_weight_pos.ne').mp heq
...
theorem patience_implements_wisdom
  ...
  · intro i h1 h2
    ...
    rw [periodic_shift stream h_periodic n (timed_action.time_last_action + i)]
    ring_nf
