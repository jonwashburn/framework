...
open Patterns Constants Consciousness
...
theorem mutation_is_adjacency (c : Codon) :
    ...
    let c' : Codon := { c with third := .G }
    c.third = .A → qualia_adjacent c c' := by
  intro h
