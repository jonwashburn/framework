...
theorem scaleGate_nonempty : ∃ w : List ℂ, ScaleGate w := by
  ...
  let λrec : ℝ := Real.sqrt (Constants.hbar * Constants.G / (Real.pi * Constants.c ^ 3))
  let a : ℂ := (λrec + 1 : ℝ)
  ...
  · -- Neutral window
    ...
    simp [NeutralWindow, witness, htol.le]
  · -- Lambda rec threshold
    -- `energy = 8 * (λrec + 1)^2` and `λrec ≥ 0`, so the bound is automatic.
    simp [LambdaRecThreshold, witness, a, λrec, pow_two]
    have hλ : 0 ≤ λrec := Real.sqrt_nonneg _
    nlinarith
