...
lemma gap_1332_bounds
  ...
  simp only [gap]
  have hphi := phi_bounds
  have hlog := log_phi_bounds h_low_phi h_high_phi
  ...
  have h_log_lower : (6.7144 : ℝ) < Real.log (1 + 1332 / phi) := by
    have h_arg : 1 + 1332 / (1.618034 : ℝ) < 1 + 1332 / phi := by
      exact add_lt_add_left (div_lt_div_of_pos_left (by norm_num) (by positivity) hphi.2) 1
  ...
  have h_log_upper : Real.log (1 + 1332 / phi) < (6.7145 : ℝ) := by
    have h_arg : 1 + 1332 / phi < 1 + 1332 / (1.618033 : ℝ) := by
      exact add_lt_add_left (div_lt_div_of_pos_left (by norm_num) (by norm_num) hphi.1) 1
    exact lt_trans (Real.log_lt_log (by positivity) h_arg) (log_824_upper h_high)
  constructor
