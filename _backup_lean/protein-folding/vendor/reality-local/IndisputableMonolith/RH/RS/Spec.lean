...
/-- Canonical (spec-level) dimensionless α default at scale `φ`. -/
@[simp] def alphaDefault (φ : ℝ) : ℝ := (1 - 1 / φ) / 2

-- Canonical (spec-level) φ-power mass ratios (toy list).
@[simp] def massRatiosDefault (φ : ℝ) : List ℝ :=
  [φ, 1 / (φ ^ (2 : Nat))]

-- Canonical (spec-level) mixing angles (toy list).
@[simp] def mixingAnglesDefault (φ : ℝ) : List ℝ :=
  [1 / φ]

-- Canonical (spec-level) g-2 muon value (toy formula).
...
-- `dimlessPack_explicit`) live in `RH/RS/InevitabilityScaffold.lean` and are intentionally
