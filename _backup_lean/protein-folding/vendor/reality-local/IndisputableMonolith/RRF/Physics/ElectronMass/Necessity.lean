...
/-- Bounded exp(6.7144) < 824.2.
    Proof: Use Taylor series bound for exp(x) at x = 6.7144.
    For x = 6.7144, n = 40, the error term is extremely small.
    We use native_decide on a rational Taylor sum. -/
private def exp_taylor_40_at_67144 : ℚ :=
  let x : ℚ := 67144 / 10000
  (∑ m in Finset.range 40, x^m / m.factorial)

private def exp_error_40_at_67144 : ℚ :=
  let x : ℚ := 67144 / 10000
  let n : ℕ := 40
  x^n * (n + 1) / (n.factorial * (n + 1 - x))

lemma exp_67144_lt_824 : Real.exp (6.7144 : ℝ) < (824.2 : ℝ) := by
  have hx_pos : (0 : ℝ) ≤ 6.7144 := by norm_num
  have hx_lt : 6.7144 < (40 + 1 : ℝ) := by norm_num
  have h_bound := Real.exp_bound (by norm_num : |(6.7144 : ℝ)| ≤ 40 + 1) (n := 40)
  -- The bound is: |exp x - S_n| ≤ error
  have h_abs := abs_sub_le_iff.mp h_bound
  -- exp x ≤ S_n + error
  have h_upper : Real.exp 6.7144 ≤ (∑ m in Finset.range 40, (6.7144 : ℝ)^m / m.factorial) +
      (6.7144 : ℝ)^40 * (40 + 1) / (Nat.factorial 40 * (40 + 1 - 6.7144)) := by
    linarith [h_abs.2]
  -- Now show S_n + error < 824.2 using native_decide on rationals
  have h_rational : exp_taylor_40_at_67144 + exp_error_40_at_67144 < 8242 / 10 := by
    native_decide
  have h_cast : (exp_taylor_40_at_67144 : ℝ) + (exp_error_40_at_67144 : ℝ) < (824.2 : ℝ) := by
    exact_mod_cast h_rational
  -- Equality of sums
  have h_sum_eq : (∑ m in Finset.range 40, (6.7144 : ℝ)^m / m.factorial) = (exp_taylor_40_at_67144 : ℝ) := by
    unfold exp_taylor_40_at_67144
    simp only [Rat.cast_sum, Rat.cast_div, Rat.cast_pow, Rat.cast_natCast]
    norm_num
  -- Equality of error
  have h_err_eq : (6.7144 : ℝ)^40 * (40 + 1) / (Nat.factorial 40 * (40 + 1 - 6.7144)) = (exp_error_40_at_67144 : ℝ) := by
    unfold exp_error_40_at_67144
    simp only [Rat.cast_div, Rat.cast_mul, Rat.cast_pow, Rat.cast_natCast, Rat.cast_sub]
    norm_num
  rw [h_sum_eq, h_err_eq] at h_upper
  exact lt_of_le_of_lt h_upper h_cast
...
/-- Bounded 824.2218 < exp(6.7145).
    Proof: Use Taylor series bound for exp(x) at x = 6.7145.
    For x = 6.7145, n = 40, the error term is extremely small. -/
private def exp_taylor_40_at_67145 : ℚ :=
  let x : ℚ := 67145 / 10000
  (∑ m in Finset.range 40, x^m / m.factorial)

private def exp_error_40_at_67145 : ℚ :=
  let x : ℚ := 67145 / 10000
  let n : ℕ := 40
  x^n * (n + 1) / (n.factorial * (n + 1 - x))

lemma val_824_lt_exp_67145 : (824.2218 : ℝ) < Real.exp (6.7145 : ℝ) := by
  have hx_abs : |(6.7145 : ℝ)| ≤ (40 + 1 : ℝ) := by norm_num
  have h_bound := Real.exp_bound hx_abs (n := 40)
  have h_abs := abs_sub_le_iff.mp h_bound
  -- exp x ≥ S_n - error
  have h_lower : (∑ m in Finset.range 40, (6.7145 : ℝ)^m / m.factorial) -
      (6.7145 : ℝ)^40 * (40 + 1) / (Nat.factorial 40 * (40 + 1 - 6.7145)) ≤ Real.exp 6.7145 := by
    linarith [h_abs.1]
  -- Now show S_n - error > 824.2218 using native_decide
  have h_rational : 8242218 / 10000 < exp_taylor_40_at_67145 - exp_error_40_at_67145 := by
    native_decide
  have h_cast : (824.2218 : ℝ) < (exp_taylor_40_at_67145 : ℝ) - (exp_error_40_at_67145 : ℝ) := by
    exact_mod_cast h_rational
  -- Equality of sums
  have h_sum_eq : (∑ m in Finset.range 40, (6.7145 : ℝ)^m / m.factorial) = (exp_taylor_40_at_67145 : ℝ) := by
    unfold exp_taylor_40_at_67145
    simp only [Rat.cast_sum, Rat.cast_div, Rat.cast_pow, Rat.cast_natCast]
    norm_num
  -- Equality of error
  have h_err_eq : (6.7145 : ℝ)^40 * (40 + 1) / (Nat.factorial 40 * (40 + 1 - 6.7145)) = (exp_error_40_at_67145 : ℝ) := by
    unfold exp_error_40_at_67145
    simp only [Rat.cast_div, Rat.cast_mul, Rat.cast_pow, Rat.cast_natCast, Rat.cast_sub]
    norm_num
  rw [h_sum_eq, h_err_eq] at h_lower
  exact lt_of_lt_of_le h_cast h_lower
...
lemma gap_1332_bounds : (13.953 : ℝ) < gap 1332 ∧ gap 1332 < (13.954 : ℝ) := by
  ...
  have h_arg_lower : 1 + 1332 / (1.618034 : ℝ) < 1 + 1332 / phi := by
    have hdiv : (1332 : ℝ) / (1.618034 : ℝ) < 1332 / phi := by
      -- since phi < 1.618034 and numerator is positive, division reverses the inequality
      have hpos : (0 : ℝ) < (1332 : ℝ) := by norm_num
      exact div_lt_div_of_pos_left hpos h_phi_pos hphi.2
    linarith
  have h_arg_upper : 1 + 1332 / phi < 1 + 1332 / (1.618033 : ℝ) := by
    have hdiv : (1332 : ℝ) / phi < 1332 / (1.618033 : ℝ) := by
      -- since 1.618033 < phi and numerator is positive, division reverses the inequality
      have hpos : (0 : ℝ) < (1332 : ℝ) := by norm_num
...
/-- Bounded φ^(-20.7063) > 4.705e-5.
    Proof: Use φ^x = exp(x * log φ) and log φ bounds.
    φ^(-20.7063) = exp(-20.7063 * log φ) > exp(-20.7063 * 0.481212)
    We bound exp(9.964211136) from above. -/
private def exp_taylor_60_at_99643 : ℚ :=
  let x : ℚ := 99643 / 10000
  (∑ m in Finset.range 60, x^m / m.factorial)

private def exp_error_60_at_99643 : ℚ :=
  let x : ℚ := 99643 / 10000
  let n : ℕ := 60
  x^n * (n + 1) / (n.factorial * (n + 1 - x))

lemma phi_pow_neg207063_lower : (4.705e-5 : ℝ) < phi ^ (-20.7063 : ℝ) := by
  -- 1. φ^x = exp(x * log φ)
  rw [Real.rpow_def_of_pos phi_pos]
  -- 2. log φ < 0.481212
  have hlog := log_phi_bounds.2
  have h_exp_mono : exp (-20.7063 * 0.481212) < exp (-20.7063 * Real.log phi) := by
    apply Real.exp_lt_exp.mpr
    apply mul_lt_mul_of_neg_left hlog (by norm_num : (-20.7063 : ℝ) < 0)
  -- 3. Show 4.705e-5 < exp(-20.7063 * 0.481212)
  -- exp(-x) = 1/exp(x)
  -- 4.705e-5 < 1/exp(9.964211136)  iff  exp(9.964211136) < 1/4.705e-5 ≈ 21253.985
  have h_val : (20.7063 : ℝ) * 0.481212 = 9.964211136 := by norm_num
  have h_target : (1 : ℝ) / (4.705e-5 : ℝ) = (100000 / 4.705 : ℝ) := by norm_num
  -- We bound exp(9.9643) from above as it's easier
  have hx_abs : |(9.9643 : ℝ)| ≤ (60 + 1 : ℝ) := by norm_num
  have h_bound := Real.exp_bound hx_abs (n := 60)
  have h_abs := abs_sub_le_iff.mp h_bound
  have h_upper : Real.exp 9.9643 ≤ (exp_taylor_60_at_99643 : ℝ) + (exp_error_60_at_99643 : ℝ) := by
    have h_sum_eq : (∑ m in Finset.range 60, (9.9643 : ℝ)^m / m.factorial) = (exp_taylor_60_at_99643 : ℝ) := by
      unfold exp_taylor_60_at_99643
      simp only [Rat.cast_sum, Rat.cast_div, Rat.cast_pow, Rat.cast_natCast]
      norm_num
    have h_err_eq : (9.9643 : ℝ)^60 * (60 + 1) / (Nat.factorial 60 * (60 + 1 - 9.9643)) = (exp_error_60_at_99643 : ℝ) := by
      unfold exp_error_60_at_99643
      simp only [Rat.cast_div, Rat.cast_mul, Rat.cast_pow, Rat.cast_natCast, Rat.cast_sub]
      norm_num
    rw [h_sum_eq, h_err_eq] at h_abs
    exact h_abs.2
  -- Now show S_n + error < 1/4.705e-5 ≈ 21253.98
  have h_rational : exp_taylor_60_at_99643 + exp_error_60_at_99643 < 100000000 / 4705 := by
    native_decide
  have h_cast : (exp_taylor_60_at_99643 : ℝ) + (exp_error_60_at_99643 : ℝ) < (1 / 0.00004705 : ℝ) := by
    have : (100000000 / 4705 : ℝ) = (1 / 0.00004705 : ℝ) := by norm_num
    exact_mod_cast h_rational
  have h_exp_99643 : Real.exp 9.9643 < 1 / 4.705e-5 := lt_of_le_of_lt h_upper h_cast
  have h_exp_neg : 4.705e-5 < Real.exp (-9.9643) := by
    rw [Real.exp_neg]
    apply lt_inv_of_lt_inv (by norm_num) h_exp_99643
  -- Since -9.9643 < -20.7063 * 0.481212
  have h_final : (4.705e-5 : ℝ) < Real.exp (-20.7063 * 0.481212) := by
    apply lt_trans h_exp_neg
    apply Real.exp_lt_exp.mpr
    norm_num
  exact lt_trans h_final h_exp_mono

/-- Bounded φ^(-20.705) < 4.709e-5. -/
private def exp_taylor_60_at_9963 : ℚ :=
  let x : ℚ := 9963 / 1000
  (∑ m in Finset.range 60, x^m / m.factorial)

private def exp_error_60_at_9963 : ℚ :=
  let x : ℚ := 9963 / 1000
  let n : ℕ := 60
  x^n * (n + 1) / (n.factorial * (n + 1 - x))

lemma phi_pow_neg20705_upper : phi ^ (-20.705 : ℝ) < (4709 / 100000000 : ℝ) := by
  -- 1. φ^x = exp(x * log φ)
  rw [Real.rpow_def_of_pos phi_pos]
  -- 2. log φ > 0.481211
  have hlog := log_phi_bounds.1
  have h_exp_mono : exp (-20.705 * Real.log phi) < exp (-20.705 * 0.481211) := by
    apply Real.exp_lt_exp.mpr
    apply mul_lt_mul_of_neg_left hlog (by norm_num : (-20.705 : ℝ) < 0)
  -- 3. Show exp(-20.705 * 0.481211) < 4.709e-5
  -- exp(-x) = 1/exp(x)
  -- 1/exp(9.96312825) < 4.709e-5  iff  exp(9.96312825) > 1/4.709e-5 ≈ 21235.93
  have h_val : (20.705 : ℝ) * 0.481211 = 9.963473755 := by norm_num
  -- We bound exp(9.963) from below
  have hx_abs : |(9.963 : ℝ)| ≤ (60 + 1 : ℝ) := by norm_num
  have h_bound := Real.exp_bound hx_abs (n := 60)
  have h_abs := abs_sub_le_iff.mp h_bound
  have h_lower : (exp_taylor_60_at_9963 : ℝ) - (exp_error_60_at_9963 : ℝ) ≤ Real.exp 9.963 := by
    have h_sum_eq : (∑ m in Finset.range 60, (9.963 : ℝ)^m / m.factorial) = (exp_taylor_60_at_9963 : ℝ) := by
      unfold exp_taylor_60_at_9963
      simp only [Rat.cast_sum, Rat.cast_div, Rat.cast_pow, Rat.cast_natCast]
      norm_num
    have h_err_eq : (9.963 : ℝ)^60 * (60 + 1) / (Nat.factorial 60 * (60 + 1 - 9.963)) = (exp_error_60_at_9963 : ℝ) := by
      unfold exp_error_60_at_9963
      simp only [Rat.cast_div, Rat.cast_mul, Rat.cast_pow, Rat.cast_natCast, Rat.cast_sub]
      norm_num
    rw [h_sum_eq, h_err_eq] at h_abs
    exact h_abs.1
  -- Now show S_n - error > 1/4.709e-5 ≈ 21235.9
  have h_rational : 100000000 / 4709 < exp_taylor_60_at_9963 - exp_error_60_at_9963 := by
    native_decide
  have h_cast : (1 / 0.00004709 : ℝ) < (exp_taylor_60_at_9963 : ℝ) - (exp_error_60_at_9963 : ℝ) := by
    have : (100000000 / 4709 : ℝ) = (1 / 0.00004709 : ℝ) := by norm_num
    exact_mod_cast h_rational
  have h_exp_9963 : 1 / 4.709e-5 < Real.exp 9.963 := lt_of_lt_of_le h_cast h_lower
  have h_exp_neg : Real.exp (-9.963) < 4.709e-5 := by
    rw [Real.exp_neg]
    apply inv_lt_of_inv_lt (by norm_num) h_exp_9963
  -- Since -20.705 * 0.481211 < -9.963
  have h_final : Real.exp (-20.705 * 0.481211) < (4.709e-5 : ℝ) := by
    apply lt_trans _ h_exp_neg
    apply Real.exp_lt_exp.mpr
    norm_num
  exact lt_trans h_exp_mono h_final
...
theorem phi_pow_residue_upper : phi ^ (gap 1332 - refined_shift) < (4.709e-5 : ℝ) := by
  ...
  have h_num : phi ^ (-20.705 : ℝ) < (4.709e-5 : ℝ) := by
    have h := phi_pow_neg20705_upper
    have hconv : (4709 / 100000000 : ℝ) = (4.709e-5 : ℝ) := by norm_num
    linarith
  linarith
...
/-- Bounded log(mass_ref / m_struct) lower.
    We need: log(0.51099895 / 10858) > -9.964211136
    i.e., 0.51099895 / 10858 > exp(-9.964211136)
    i.e., exp(9.964211136) > 10858 / 0.51099895 ≈ 21248.57 -/
lemma log_ratio_lower : (-9.964211136 : ℝ) < Real.log (mass_ref_MeV / 10858) := by
  have h_pos : (0 : ℝ) < mass_ref_MeV / 10858 := by norm_num
  rw [Real.lt_log_iff_exp_lt h_pos]
  rw [Real.exp_neg]
  have h_target : (10858 : ℝ) / mass_ref_MeV < (21248.58 : ℝ) := by norm_num
  -- We already proved exp(9.9643) < 21253.98 in phi_pow_neg207063_lower
  -- We need exp(9.964211136) > 21248.58
  -- We use Taylor from below
  have hx_abs : |(9.964211136 : ℝ)| ≤ (60 + 1 : ℝ) := by norm_num
  have h_bound := Real.exp_bound hx_abs (n := 60)
  have h_abs := abs_sub_le_iff.mp h_bound
  -- exp x ≥ S_n - error
  have h_lower : (∑ m in Finset.range 60, (9.964211136 : ℝ)^m / m.factorial) -
      (9.964211136 : ℝ)^60 * (60 + 1) / (Nat.factorial 60 * (60 + 1 - 9.964211136)) ≤ Real.exp 9.964211136 := by
    linarith [h_abs.1]
  have h_rational : 2124858 / 100 < (∑ m in Finset.range 60, (9964211136 / 1000000000 : ℚ)^m / m.factorial) -
      (9964211136 / 1000000000 : ℚ)^60 * (60 + 1) / (Nat.factorial 60 * (60 + 1 - 9964211136 / 1000000000)) := by
    native_decide
  have h_cast : (21248.58 : ℝ) < (∑ m in Finset.range 60, (9.964211136 : ℝ)^m / m.factorial) -
      (9.964211136 : ℝ)^60 * (60 + 1) / (Nat.factorial 60 * (60 + 1 - 9.964211136)) := by
    exact_mod_cast h_rational
  have h_exp : (21248.58 : ℝ) < Real.exp 9.964211136 := lt_of_lt_of_le h_cast h_lower
  apply lt_inv_of_lt_inv (by norm_num)
  calc (mass_ref_MeV / 10858 : ℝ) = (1 : ℝ) / (10858 / mass_ref_MeV : ℝ) := by field_simp
    _ > (1 : ℝ) / 21248.58 := by
      apply one_div_lt_one_div_of_lt (by norm_num) h_target
    _ > (1 : ℝ) / Real.exp 9.964211136 := by
      apply one_div_lt_one_div_of_lt (by positivity) h_exp

/-- Bounded log(mass_ref / m_struct) upper. -/
lemma log_ratio_upper : Real.log (mass_ref_MeV / 10856) < (-9.963473755 : ℝ) := by
  have h_pos : (0 : ℝ) < mass_ref_MeV / 10856 := by norm_num
  rw [Real.log_lt_iff_lt_exp h_pos]
  rw [Real.exp_neg]
  have h_target : (21244.65 : ℝ) < (10856 : ℝ) / mass_ref_MeV := by norm_num
  -- We need 1/exp(9.963473755) < mass_ref / 10856
  -- i.e., exp(9.963473755) > 10856 / mass_ref ≈ 21244.66
  have hx_abs : |(9.963473755 : ℝ)| ≤ (60 + 1 : ℝ) := by norm_num
  have h_bound := Real.exp_bound hx_abs (n := 60)
  have h_abs := abs_sub_le_iff.mp h_bound
  have h_lower : (∑ m in Finset.range 60, (9.963473755 : ℝ)^m / m.factorial) -
      (9.963473755 : ℝ)^60 * (60 + 1) / (Nat.factorial 60 * (60 + 1 - 9.963473755)) ≤ Real.exp 9.963473755 := by
    linarith [h_abs.1]
  have h_rational : 2124466 / 100 < (∑ m in Finset.range 60, (9963473755 / 1000000000 : ℚ)^m / m.factorial) -
      (9963473755 / 1000000000 : ℚ)^60 * (60 + 1) / (Nat.factorial 60 * (60 + 1 - 9963473755 / 1000000000)) := by
    native_decide
  have h_cast : (21244.66 : ℝ) < (∑ m in Finset.range 60, (9.963473755 : ℝ)^m / m.factorial) -
      (9.963473755 : ℝ)^60 * (60 + 1) / (Nat.factorial 60 * (60 + 1 - 9.963473755)) := by
    exact_mod_cast h_rational
  have h_exp : (21244.66 : ℝ) < Real.exp 9.963473755 := lt_of_lt_of_le h_cast h_lower
  apply inv_lt_of_inv_lt (by positivity)
  calc (1 : ℝ) / Real.exp 9.963473755 < (1 : ℝ) / 21244.66 := by
      apply one_div_lt_one_div_of_lt (by norm_num) h_exp
    _ < mass_ref_MeV / 10856 := by
      rw [div_lt_iff₀ (by norm_num : (0 : ℝ) < 10856)]
      have : (1 : ℝ) / 21244.66 * 10856 < mass_ref_MeV := by norm_num
      exact this

theorem electron_residue_lower :
  (-20.7063 : ℝ) < IndisputableMonolith.Physics.ElectronMass.electron_residue := by
  unfold IndisputableMonolith.Physics.ElectronMass.electron_residue
  -- log(ratio) / log(phi) > -20.7063
  -- Since log(ratio) < 0 and log(phi) > 0, this is:
  -- log(ratio) > -20.7063 * log(phi)
  have hlog_phi := log_phi_bounds.2 -- log phi < 0.481212
  have h_rhs : -20.7063 * Real.log phi > -20.7063 * 0.481212 := by
    apply mul_lt_mul_of_neg_left hlog_phi (by norm_num)
  have h_rhs_val : (-20.7063 : ℝ) * 0.481212 = -9.964211136 := by norm_num
  rw [h_rhs_val] at h_rhs
  have h_ratio : Real.log (mass_ref_MeV / electron_structural_mass) ≥ Real.log (mass_ref_MeV / 10858) := by
    apply Real.log_le_log (by norm_num)
    apply div_le_div_of_nonneg_left (by norm_num) (by norm_num) structural_mass_bounds.2.le
  have h_final : Real.log (mass_ref_MeV / electron_structural_mass) > -9.964211136 :=
    lt_of_lt_of_le log_ratio_lower h_ratio
  have h_div : Real.log (mass_ref_MeV / electron_structural_mass) / Real.log phi > -20.7063 := by
    rw [gt_iff_lt, div_lt_iff₀' (by positivity : 0 < Real.log phi)]
    linarith
  exact h_div

theorem electron_residue_upper :
  IndisputableMonolith.Physics.ElectronMass.electron_residue < (-20.7057 : ℝ) := by
  unfold IndisputableMonolith.Physics.ElectronMass.electron_residue
  -- log(ratio) / log(phi) < -20.7057
  -- log(ratio) < -20.7057 * log(phi)
  have hlog_phi := log_phi_bounds.1 -- log phi > 0.481211
  have h_rhs : -20.7057 * Real.log phi < -20.7057 * 0.481211 := by
    apply mul_lt_mul_of_neg_left hlog_phi (by norm_num)
  have h_rhs_val : (-20.7057 : ℝ) * 0.481211 = -9.9634737557 := by norm_num
  have h_ratio : Real.log (mass_ref_MeV / electron_structural_mass) ≤ Real.log (mass_ref_MeV / 10856) := by
    apply Real.log_le_log (by positivity)
    apply div_le_div_of_nonneg_left (by norm_num) (by norm_num) structural_mass_bounds.1.le
  have h_final : Real.log (mass_ref_MeV / electron_structural_mass) < -9.9634737557 :=
    lt_of_le_of_lt h_ratio (by linarith [log_ratio_upper])
  have h_div : Real.log (mass_ref_MeV / electron_structural_mass) / Real.log phi < -20.7057 := by
    rw [div_lt_iff₀' (by positivity : 0 < Real.log phi)]
    linarith
  exact h_div
