...
def klein_gordon_solution_exists_hypothesis (scale : ScaleFactor) (m_squared : ℝ) : Prop :=
...
theorem massless_scalar_not_exactly_radiation (scale : ScaleFactor) (psi : ℝ → ℝ) :
  -- Massless scalar has p = ρ (stiff), not p = ρ/3 (radiation)
  -- This is correct for scalar field
  ∀ t, pressure_scalar scale psi 0 t = energy_density_scalar scale psi 0 t := by
  intro t
  simp only [pressure_scalar, energy_density_scalar]
  ring
