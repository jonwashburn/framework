...
theorem G_00_is_laplacian_Phi (ng : NewtonianGaugeMetric) (hreg : WeakFieldBounds ng) (x : Fin 4 → ℝ) :
  ...
  have h_eq : linearized_G_00 minkowski.toMetricTensor (to_perturbation ng) x - laplacian ng.Φ x
  ...
  calc |linearized_G_00 minkowski.toMetricTensor (to_perturbation ng) x - laplacian ng.Φ x|
      ≤ |linearized_ricci minkowski.toMetricTensor (to_perturbation ng) x 0 0 - laplacian ng.Φ x|
        + ((1/2) * |linearized_ricci_scalar minkowski.toMetricTensor (to_perturbation ng) x|) := this
    _ < 0.05 + (1 / 2) * 0.1 := by
