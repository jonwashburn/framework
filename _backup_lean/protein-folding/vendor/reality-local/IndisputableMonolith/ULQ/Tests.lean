...
/-- Test: deriveQualia returns Some for non-DC WTokens (verified in Bridge.lean) -/
theorem derivation_exists_for_nonDC (w : LightLanguage.WToken) (h : w.tau.val ≠ 0) :
    (deriveQualia w).isSome = true :=
  deriveQualia_some_of_tau_nonzero w h

/-- Test: deriveQualia returns None for DC WTokens (verified in Bridge.lean) -/
theorem derivation_none_for_DC (w : LightLanguage.WToken) (h : w.tau.val = 0) :
    (deriveQualia w) = none :=
  deriveQualia_none_of_tau_zero w h
