...
@[simp] def UniqueCalibration.verified (_ : UniqueCalibration) : Prop :=
  ∀ (L : RH.RS.Ledger) (B : RH.RS.Bridge L) (A : RH.RS.Anchors), RH.RS.UniqueCalibration L B A

@[simp] def MeetsBands.verified (_ : MeetsBands) : Prop :=
  ∀ (L : RH.RS.Ledger) (B : RH.RS.Bridge L) (U : IndisputableMonolith.Constants.RSUnits),
...
@[simp] theorem AbsoluteLayer.verified_any (A : AbsoluteLayer) :
  ...
  constructor
  · intro L B Anchors
    exact RH.RS.uniqueCalibration_any L B Anchors
  · intro L B U
