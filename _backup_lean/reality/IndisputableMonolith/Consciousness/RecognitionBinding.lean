...
After T2.1 update, `phase_alignment` now depends on boundary extent via φ-ladder coordinates.
The global phase Θ is shared, but local phase includes a ladder-dependent offset.

For the degenerate case (extent ≤ 0 or lam_rec ≤ 0), phase falls back to global_phase.
For non-degenerate boundaries, phase = wrapPhase(Θ + ladder_coordinate).
-/

/-- In degenerate case, phase_alignment falls back to global_phase. -/
theorem projection_preserves_theta_degenerate (B : BoundaryCondition) (ψ : UniversalField)
    (h_degen : ¬(B.extent > 0 ∧ lam_rec > 0)) :
    phase_alignment B ψ = ψ.global_phase := by
  unfold phase_alignment
  simp [h_degen]

/-- Phase alignment is always in [0, 1). -/
theorem projection_phase_bounded (B : BoundaryCondition) (ψ : UniversalField)
    (hb : B.extent > 0) (hL : lam_rec > 0) :
    0 ≤ phase_alignment B ψ ∧ phase_alignment B ψ < 1 :=
  phase_alignment_bounds B ψ hb hL
