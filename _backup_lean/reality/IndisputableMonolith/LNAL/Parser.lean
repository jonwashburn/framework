...
private def tokenizeLine (s : String) : List String :=
  let s := ((cutComment s).trimAscii).toString
  if s.isEmpty then []
  else
    -- Normalize all whitespace to spaces so we can use the stable `splitOn` API.
    let normalized := s.map (fun c => if c.isWhitespace then ' ' else c)
    normalized.splitOn " " |>.filter (fun tok => ¬ tok.isEmpty)

/-- Macro definitions rewritten in terms of the macrocore primitives. -/
def expandMacro (name : String) : Option (Array LInstr) :=
  let _mk := fun op arg => #[LInstr.mk op arg]
  match name with
  | "PHOTON_EMIT" =>
...
private def parseLine (line : String) (lineNum : Nat) :
  ...
  for tok in toks do
    ...
    if let some insts := expandToken tok then
      for (idx, inst) in List.zip (List.range insts.size) insts.toList do
    ...
    -- Unknown token
    throw (ParseError.unknownOpcode loc tok)
...
private def resolveLabels (instrs : Array IntermediateInstr) :
  ...
  let instrList := instrs.toList
  for (idx, instr) in List.zip (List.range instrList.length) instrList do
    if let some label := instr.label then
      if labels.contains label then
        let firstLoc := (instrList.find? (fun i => i.label = some label)).getD { instr := LInstr.simple Opcode.LOCK, loc := { line := 0, column := 0 }, label := none } |>.loc
        throw (ParseError.duplicateLabel label firstLoc instr.loc)
...
def mkProgram (code : Array LInstr) : LProgram :=
  fun ip => if h : ip < code.size then code[ip] else { op := Opcode.LOCK }
...
def findOpcodes (code : Array LInstr) (target : Opcode) : List (Nat × LInstr) :=
  ...
  (List.zip (List.range codeList.length) codeList).filter (fun (_, i) => i.op = target)

/-- Pretty-print a program for debugging -/
def prettyPrint (code : Array LInstr) : String :=
  let codeList := code.toList
  let lines := (List.zip (List.range codeList.length) codeList).map fun (idx, instr) =>
    s!"{idx}: {instrToString instr}"
  "\n".intercalate lines
