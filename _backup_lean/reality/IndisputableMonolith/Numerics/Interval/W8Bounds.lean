...
theorem w8_computed_gt : (2.490564399 : ℝ) < IndisputableMonolith.Constants.w8_from_eight_tick := by
  ...
  have hφ_hi : IndisputableMonolith.Constants.phi < (1.6180340 : ℝ) := by
    -- use tight φ bound from PhiBounds (goldenRatio)
    simpa [IndisputableMonolith.Constants.phi, Real.goldenRatio] using phi_lt_16180340
...
theorem w8_computed_lt : IndisputableMonolith.Constants.w8_from_eight_tick < (2.490572090 : ℝ) := by
  ...
  have hφ_lo : (1.61803395 : ℝ) ≤ IndisputableMonolith.Constants.phi := by
    simpa [IndisputableMonolith.Constants.phi, Real.goldenRatio] using (le_of_lt phi_gt_161803395)
