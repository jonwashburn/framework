...
theorem neutral_windows_are_zero_and_four (beat : Beat) :
  ...
  simp only [isNeutralWindow, Set.mem_insert_iff, Set.mem_singleton_iff, decide_eq_true_eq]
...
theorem small_protein_always_allowed (beat : Beat) (n : ℕ) (hn : n ≤ 45) :
  ...
  simp only [topologyMoveAllowed, decide_eq_true_eq]
  left; exact hn
...
theorem large_protein_needs_window (beat : Beat) (n : ℕ) (hn : 45 < n)
  ...
  simp only [topologyMoveAllowed, isNeutralWindow, decide_eq_false_iff_not] at hbeat ⊢
  push_neg
  exact ⟨by omega, hbeat, by decide⟩
...
theorem neutral_window_pattern (n : ℕ) :
  ...
  simp only [atNeutralWindow, isNeutralWindow, currentBeat, decide_eq_true_eq]
