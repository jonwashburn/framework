...
theorem ehrenfest_structure {H : Type*} [RSHilbertSpace H]
  ...
  · -- This algebraic identity is proven by expanding the inner product ⟨ψ', Oψ'⟩
    ...
    -- The expansion matches the claimed decomposition exactly.
    -- Expand ψ' and use linearity of inner product and O.op
    simp only [inner_sub_left, inner_sub_right, inner_smul_left, inner_smul_right]
    -- Use star(ε) = -ε from h_eps
    have h_star : star ε = -ε := by linarith [h_eps]
    rw [h_star]
    -- The algebra matches by direct calculation
    ring
  · constructor
    · rfl
    · rfl
