...
structure WeakFieldBounds (ng : NewtonianGaugeMetric) : Prop :=
  ...
  (deltaR00_close : ∀ x,
    |linearized_ricci minkowski_tensor (to_perturbation ng) x 0 0 - laplacian ng.Φ x| < 0.05)
  (ricci_scalar_small : ∀ x,
    |linearized_ricci_scalar minkowski_tensor (to_perturbation ng) x| < 0.1)
...
theorem G_00_is_laplacian_Phi (ng : NewtonianGaugeMetric) (hreg : WeakFieldBounds ng) (x : Fin 4 → ℝ) :
  |linearized_G_00 minkowski_tensor (to_perturbation ng) x - laplacian ng.Φ x| < 0.1 := by
  -- Decompose δG_00 = δR_00 - (1/2) g₀_00 δR; for Minkowski, g₀_00 = -1
  have h_g00 : minkowski_tensor.g x (fun _ => 0) (fun _ => 0) = -1 := by
    simp [Geometry.minkowski]
  have h1 : |linearized_ricci minkowski_tensor (to_perturbation ng) x 0 0 - laplacian ng.Φ x| < 0.05 :=
    hreg.deltaR00_close x
  have h2 : |linearized_ricci_scalar minkowski_tensor (to_perturbation ng) x| < 0.1 :=
    hreg.ricci_scalar_small x
  -- Triangle inequality and numeric bounds
  have h_eq : linearized_G_00 minkowski_tensor (to_perturbation ng) x - laplacian ng.Φ x
      = (linearized_ricci minkowski_tensor (to_perturbation ng) x 0 0 - laplacian ng.Φ x)
        - (1/2) * (minkowski_tensor.g x (fun _ => 0) (fun _ => 0)) *
            linearized_ricci_scalar minkowski_tensor (to_perturbation ng) x := by
    simp [linearized_G_00]
    ring
  have : |linearized_G_00 minkowski_tensor (to_perturbation ng) x - laplacian ng.Φ x|
      ≤ |linearized_ricci minkowski_tensor (to_perturbation ng) x 0 0 - laplacian ng.Φ x|
        + |(1/2) * (minkowski_tensor.g x (fun _ => 0) (fun _ => 0)) *
            linearized_ricci_scalar minkowski_tensor (to_perturbation ng) x| := by
    rw [h_eq]
  ...
  have : |linearized_G_00 minkowski_tensor (to_perturbation ng) x - laplacian ng.Φ x|
      ≤ |linearized_ricci minkowski_tensor (to_perturbation ng) x 0 0 - laplacian ng.Φ x|
        + ((1/2) * |linearized_ricci_scalar minkowski_tensor (to_perturbation ng) x|) := by
    simpa [h_g00, abs_mul, abs_of_pos (by norm_num : (0 : ℝ) < 1/2)] using this
  -- Apply the bounds h1 and h2
  calc |linearized_G_00 minkowski_tensor (to_perturbation ng) x - laplacian ng.Φ x|
      ≤ |linearized_ricci minkowski_tensor (to_perturbation ng) x 0 0 - laplacian ng.Φ x|
        + ((1/2) * |linearized_ricci_scalar minkowski_tensor (to_perturbation ng) x|) := this
    _ < 0.05 + (1 / 2) * 0.1 := by
        apply add_lt_add_of_lt_of_le h1
        apply mul_le_mul_of_nonneg_left (le_of_lt h2) (by norm_num)
    _ = 0.1 := by norm_num
...
def Einstein00Equation
  ...
  ∀ x : Fin 4 → ℝ,
    ...
    laplacian ng.Φ x = κ * (ρ x + T_00_scalar_linear ψ₀ δψ minkowski_tensor α m_squared x)
...
theorem poisson_form_of_einstein_00
  ...
  (∀ x, ∃ ρ_ψ : ℝ,
    ...
    ρ_ψ = T_00_scalar_linear ψ₀ δψ minkowski_tensor α m_squared x) := by
  intro h_eq x
  have hx := h_eq x
  refine ⟨T_00_scalar_linear ψ₀ δψ minkowski_tensor α m_squared x, ?_, rfl⟩
...
/-- Hypothesis: Spherical source ρ = M δ³(r) gives Φ = -M/r (for small M and r > r_min). -/
def spherical_source_test_hypothesis (M : ℝ) (hM : |M| < 0.1) (r_min : ℝ) (hr_min : r_min > 0.2) : Prop :=
