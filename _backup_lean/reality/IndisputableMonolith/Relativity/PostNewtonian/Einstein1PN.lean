...
theorem equations_reduce_to_GR (pots : PPNPotentials) (params : PPNParameters)
  ...
  intro h
  have h_grad_zero : ∀ x, Fields.gradient Fields.zero x = fun _ => 0 := by
    intro x; funext μ; unfold Fields.gradient directional_deriv Fields.zero Fields.constant; simp
  have h_psi_zero : ∀ x, Fields.zero.ψ x = 0 := fun _ => rfl
  constructor
  · intro x
    have h00 := h.eq_00 x
    unfold T_00_1PN at h00
    simp [h_grad_zero, h_psi_zero, one_mul] at h00
    exact h00
  · constructor
    · intro x i
      have h0i := h.eq_0i x i
      unfold T_0i_1PN at h0i
      simp [h_grad_zero, h_psi_zero, one_mul] at h0i
      exact h0i
    · intro x i j
      have hij := h.eq_ij x i j
      unfold T_ij_1PN at hij
      simp [h_grad_zero, h_psi_zero, one_mul] at hij
      exact hij
