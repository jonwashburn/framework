...
/-- Convert a boundary's 8-tick pattern to a complex vector.
    The repo’s `period_structure` is already an integer contribution per tick,
    so we embed it into `ℂ`. -/
noncomputable def boundaryPatternVector (b : StableBoundary) : Fin 8 → ℂ :=
  fun t => (b.pattern.period_structure t : ℂ)

/-- Alternative encoding (legacy): keep the same integer embedding. -/
noncomputable def boundaryPatternVectorSigned (b : StableBoundary) : Fin 8 → ℂ :=
  boundaryPatternVector b
