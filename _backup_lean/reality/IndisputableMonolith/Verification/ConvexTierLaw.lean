...
class ConvexTierAxioms where
  kkt_stationarity :
    ...
    (hfeas : C.emit_budget_ok ∧ C.assembly_schedule_ok ∧ C.observable_ok) → ∃ λ_emit : ℝ, λ_emit > 0
  tier_certificate :
    (catalog : List StarSystem) → (C : Constraints) →
    (hfeas : C.emit_budget_ok ∧ C.assembly_schedule_ok ∧ C.observable_ok) → ∃ Δn : ℤ, ∀ s ∈ catalog, M_over_L s = Constants.phi ^ Δn
...
theorem mass_to_light_tier_law (s : StarSystem) (C : Constraints)
  ...
  ∃ Δn : ℤ, M_over_L s = Constants.phi ^ Δn := by
  obtain ⟨Δn, h⟩ := tier_certificate [s] C hfeas
  use Δn
  apply h
  simp
