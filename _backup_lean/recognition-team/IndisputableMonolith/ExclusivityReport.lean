...
/-- Pretty-print the MP-only usage lattice. -/
def mp_only_usage_summary : String :=
  String.intercalate "\n"
...
def exclusivity_proof_report_with_usage : String :=
  ...
  have _ : URCGenerators.ExclusivityProofCert.verified cert :=
    URCGenerators.ExclusivityProofCert.verified_any cert
  -- Build the provenance witness using only MP
  let φ : ℝ := IndisputableMonolith.Constants.phi
  let Γ := IndisputableMonolith.Meta.AxiomLattice.mpOnlyEnv
  let _ := IndisputableMonolith.Meta.FromMP.mp_derives_reality_master_with_usage Γ (by
