...
theorem planck_mass_eq : planck_mass_rs = φ_val^(-5 : ℤ) := by
  unfold planck_mass_rs ℏ_rs E_coh τ₀ c_rs ℓ₀ G_rs
  -- √(φ^(-5) * 1 * 1 / (1 * φ^5)) = √(φ^(-10)) = φ^(-5)
  simp only [mul_one, one_mul, div_one, one_div]
  -- The calculation: √(φ^(-5) / φ^5) = √(φ^(-10)) = φ^(-5)
  sorry -- Technical zpow calculation
