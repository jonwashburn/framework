...
the cost foundation (d'Alembert composition law).
...
/-- **T5: J IS UNIQUE**
    ...
    The d'Alembert composition law + normalization + calibration
...
theorem t6_holds : T6_Phi_Forced := {
  ...
  phi_unique := fun r hr heq => PhiForcing.golden_constraint_unique hr heq
}
...
/-- **THE COMPLETE FORCING CHAIN**
    ...
    |-------|-----------|--------|
    | T0 | Logic from cost | ✓ FORCED |
    | T1 | MP from cost | ✓ FORCED |
    | T2 | Discreteness from J | ✓ FORCED |
    | T3 | Ledger from J-symmetry | ✓ FORCED |
    | T4 | Recognition from observables | ✓ FORCED |
    | T5 | J unique from d'Alembert | ✓ FORCED |
    | T6 | φ unique from self-similarity | ✓ FORCED |
    | T7 | 8-tick from D=3 | ✓ FORCED |
    | T8 | D=3 from linking | ✓ FORCED |

    **NO FREE PARAMETERS.** -/
structure CompleteForcingChain : Prop where
  t0 : T0_Logic_Forced
  t1 : T1_MP_Forced
...
theorem complete_forcing_chain : CompleteForcingChain := {
  t0 := t0_holds
  t1 := t1_holds
  t2 := t2_holds
  t3 := t3_holds
