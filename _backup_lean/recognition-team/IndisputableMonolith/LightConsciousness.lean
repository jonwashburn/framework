...
def lightConsciousnessCert : UniversalCostCertificate := {
  j_unique := by
    intro F hSymm hUnit hConvex hCalib hCont hCosh x hx
    -- This is proven in CostUniqueness.T5_uniqueness_complete
    exact T5_uniqueness_complete F hSymm hUnit hConvex hCalib hCont hCosh hx
...
theorem certificate_verified : ∃ (cert : UniversalCostCertificate),
    cert.minimal_period 3 = ((∃ w : CompleteCover 3, w.period = 8) ∧
    (∀ (T : ℕ) (pass : Fin T → Pattern 3), Function.Surjective pass → 8 ≤ T)) :=
  ⟨lightConsciousnessCert, rfl⟩
