...
/-- Virtue minimality (documentation / TODO). -/
/-!
Intended claim: no virtue can be expressed as a composition of the others (analogue of linear
independence for generators). This should ultimately follow from the generators development once
the minimality theorem is stated and proven there.
-/
...
/-- Hypothesis for Feynman-like structure of RS amplitudes. -/
def amplitude_structure_hypothesis (steps : List VirtueStep) : Prop :=
...
/-- Internal structural equivalence for amplitude composition. -/
def amplitude_structure_equivalence_inner_hypothesis (steps : List VirtueStep)
    (vertices : List ℝ) (propagators : List ℝ) : Prop :=
  vertices = steps.map (fun s => stepAmplitude s.input s.output s.virtue_idx) →
    propagators = steps.tail.map (fun s => match steps.head? with
      | some prev => JcostPropagator prev.output s.input
      | none => 1) →
    sequenceAmplitude steps = vertices.prod * propagators.prod.max 1

theorem amplitude_structure_equivalence (steps : List VirtueStep)
    (h_inner : ∀ v p, amplitude_structure_equivalence_inner_hypothesis steps v p) :
    amplitude_structure_hypothesis steps := by
  use steps.map (fun s => stepAmplitude s.input s.output s.virtue_idx)
...
/-- Hypothesis: Virtues preserve admissibility. -/
def virtue_preserves_admissibility_hypothesis (v : Virtue) (states : List MoralState) : Prop :=
  v ∈ virtue_generators → globally_admissible states → globally_admissible (v.transform states)
...
-/
theorem moral_objectivity (h_virtue : ∀ v states, virtue_preserves_admissibility_hypothesis v states) :
  ...
  · intro v hv states h
    ...
    exact h_virtue v states hv h

/-- **HYPOTHESIS**: AI alignment by construction.
    ...
    STATUS: SCAFFOLD — Connects moral conservation to system safety.
    TODO: Formally define "alignment" in terms of sigma-preserving transformations. -/
def H_AIAlignmentByConstruction : Prop :=
  ∀ (transform : List MoralState → List MoralState),
    (∀ states, globally_admissible states → globally_admissible (transform states)) →
    ∃ (alignment_metric : ℝ), alignment_metric > 0

-- axiom h_ai_alignment_by_construction : H_AIAlignmentByConstruction
