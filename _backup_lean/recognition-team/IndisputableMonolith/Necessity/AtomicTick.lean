...
noncomputable def atomicTickFromZeroParams
  ...
  : Recognition.AtomicTick M :=
  let hNontrivial : DiscreteNecessity.SpecNontrivial M.U := { inhabited := hNonempty }
  let hExists := @DiscreteNecessity.zero_params_has_discrete_skeleton M.U hZero hNontrivial
  let D := Classical.choose hExists
  let hSpec := Classical.choose_spec hExists
  let ι : D → M.U := Classical.choose hSpec
  let hRest := Classical.choose_spec hSpec
  let hSurj : Function.Surjective ι := hRest.1
  let hCount : Countable D := hRest.2
  -- Build an inhabitant of D by pulling back a witness from the target
  let u := Classical.choice hNonempty
  let d := Classical.choose (hSurj u)
  let _ : Inhabited D := ⟨d⟩
  atomicTickFromCountableSurjection M D hCount ι hSurj
