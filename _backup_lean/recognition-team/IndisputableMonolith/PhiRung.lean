...
def rs_pres_prop : Prop :=
  ∀ x : ℝ, 0 ≤ x → 0 ≤ IndisputableMonolith.RecogSpec.PhiPow x
...
def massCanonUnits (U : IndisputableMonolith.Constants.RSUnits) (r Z : ℤ) : ℝ :=
  baseMass U Z * IndisputableMonolith.RecogSpec.PhiPow ((r : ℝ))
...
lemma massCanonUnits_rshift (U : IndisputableMonolith.Constants.RSUnits) (r Z : ℤ) :
  ...
  have hadd : IndisputableMonolith.RecogSpec.PhiPow (((r + 1 : ℤ) : ℝ))
            = IndisputableMonolith.RecogSpec.PhiPow ((r : ℝ)) * IndisputableMonolith.RecogSpec.PhiPow (1 : ℝ) := by
    have : (((r + 1 : ℤ) : ℝ)) = (r : ℝ) + 1 := by
      simp [Int.cast_add, Int.cast_one]
    simpa [this] using IndisputableMonolith.RecogSpec.PhiPow_add (x:=(r : ℝ)) (y:=(1 : ℝ))
  -- PhiPow 1 = φ
  have hφpos : 0 < IndisputableMonolith.Constants.phi := IndisputableMonolith.Constants.phi_pos
  have hφ1 : IndisputableMonolith.RecogSpec.PhiPow (1 : ℝ)
            = IndisputableMonolith.Constants.phi := by
    unfold IndisputableMonolith.RecogSpec.PhiPow
    simpa using (Real.exp_log hφpos)
  -- Conclude the rung shift
  calc
    baseMass U Z * IndisputableMonolith.RecogSpec.PhiPow (((r + 1 : ℤ) : ℝ))
        = baseMass U Z * (IndisputableMonolith.RecogSpec.PhiPow ((r : ℝ)) * IndisputableMonolith.RecogSpec.PhiPow (1 : ℝ)) := by
          simpa [hadd]
    _ = (baseMass U Z * IndisputableMonolith.RecogSpec.PhiPow ((r : ℝ))) * IndisputableMonolith.RecogSpec.PhiPow (1 : ℝ) := by
          simp [mul_assoc, mul_left_comm, mul_comm]
    _ = (baseMass U Z * IndisputableMonolith.RecogSpec.PhiPow ((r : ℝ))) * IndisputableMonolith.Constants.phi := by
          simpa [hφ1]
    _ = IndisputableMonolith.Constants.phi * (baseMass U Z * IndisputableMonolith.RecogSpec.PhiPow ((r : ℝ))) := by
          simp [mul_comm]
...
def RecogSpec.Inevitability_dimless (φ : ℝ) : Prop :=
  IndisputableMonolith.RecogSpec.Inevitability_dimless φ
def RecogSpec.Witness.inevitability_dimless_partial (φ : ℝ) : RecogSpec.Inevitability_dimless φ :=
  IndisputableMonolith.RecogSpec.Witness.inevitability_dimless_partial φ
...
def strengthen_to_Recognition_Closure (φ : ℝ) :
  URC.AE.B () → RecogSpec.Inevitability_dimless φ :=
  fun _ => RecogSpec.Witness.inevitability_dimless_partial φ
...
theorem AE_chain_and_export (φ : ℝ) (C : URC.Certificates)
  ...
  ∧ RecogSpec.Inevitability_dimless φ := by
