...
theorem vacuum_1PN_cancellation (M : ℝ) (x : Fin 4 → ℝ) (hx : spatialRadius x ≠ 0) :
  ...
  let U := fun y => -M * radialInv 1 y
  let U_2 := fun y => M^2 * (radialInv 1 y)^2
  -- 1. Laplacian of U_2 = M^2 * laplacian (radialInv 2).
  -- 2. From our secondDeriv_radialInv proof, laplacian (radialInv 2) = 2/r^4.
  -- 3. Gradient squared of Newtonian potential (∇U)^2 = M^2 / r^4.
  -- 4. Cancellation follows identically.
  sorry
