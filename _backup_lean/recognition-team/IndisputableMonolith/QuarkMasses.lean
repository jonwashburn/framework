...
/-- **CERTIFICATE: Quark Quarter-Ladder**
    Packages the proofs for top, bottom, and charm mass matches. -/
structure QuarkMassCert where
  top_match : abs (predicted_mass res_top - mass_top_exp) / mass_top_exp < 0.0005
  bottom_match : abs (predicted_mass res_bottom - mass_bottom_exp) / mass_bottom_exp < 0.01
  charm_match : abs (predicted_mass res_charm - mass_charm_exp) / mass_charm_exp < 0.02

theorem quark_mass_verified : QuarkMassCert where
  top_match := top_mass_match
  bottom_match := bottom_mass_match
  charm_match := charm_mass_match
