...
lemma partialDeriv_v2_radialInv_n (n : ℕ) (μ : Fin 4) (x : Fin 4 → ℝ) (hx : spatialRadius x ≠ 0) :
    partialDeriv_v2 (radialInv n) μ x = (if μ.val = 1 ∨ μ.val = 2 ∨ μ.val = 3 then - n * x μ / (spatialRadius x)^(n+2) else 0) := by
  unfold radialInv
  have h_norm_diff : DifferentiableAt ℝ (fun t => spatialNormSq (coordRay x μ t)) 0 := by
    apply DifferentiableAt.add
    · apply DifferentiableAt.add
  ...
  have h_rad_diff : DifferentiableAt ℝ (fun t => spatialRadius (coordRay x μ t)) 0 :=
    DifferentiableAt.sqrt h_norm_diff h_norm_pos.ne'

  unfold partialDeriv_v2
  rw [deriv_one_div (h_rad_diff.pow n) (pow_ne_zero n hx)]
  rw [deriv_pow h_rad_diff n]
  by_cases hμ : μ.val = 1 ∨ μ.val = 2 ∨ μ.val = 3
  · rw [if_pos hμ]
    rw [deriv_spatialRadius_coordRay_spatial x μ hμ hx]
    simp only [coordRay_zero]
    field_simp [hx]
    ring
  · rw [if_neg hμ]
    ...
    have h_deriv : deriv (fun t => spatialRadius (coordRay x μ t)) 0 = 0 := by
      rw [h_const]; exact deriv_const _ _
    rw [h_deriv]
    simp

lemma secondDeriv_radialInv_n (n : ℕ) (i : Fin 4) (hi : i.val = 1 ∨ i.val = 2 ∨ i.val = 3) (x : Fin 4 → ℝ) (hx : spatialRadius x ≠ 0) :
    secondDeriv (radialInv n) i i x = - n * (1 / (spatialRadius x)^(n+2) - (n + 2) * (x i)^2 / (spatialRadius x)^(n+4)) := by
  -- The core calculation is:
  -- ∂_i (-n * x_i / r^(n+2)) = -n * [1/r^(n+2) + x_i * ∂_i (r^-(n+2))]
  -- = -n * [1/r^(n+2) + x_i * (-(n+2) * r^-(n+3) * x_i/r)]
  -- = -n * [1/r^(n+2) - (n+2) * x_i^2 / r^(n+4)]
  -- This follows from repeated application of the chain and quotient rules.
  sorry

lemma laplacian_radialInv_n (n : ℕ) (x : Fin 4 → ℝ) (hx : spatialRadius x ≠ 0) :
    laplacian (radialInv n) x = - n * (3 - (n + 2)) / (spatialRadius x)^(n+2) := by
  unfold laplacian
  rw [secondDeriv_radialInv_n n 1 (by decide) x hx]
  rw [secondDeriv_radialInv_n n 2 (by decide) x hx]
  rw [secondDeriv_radialInv_n n 3 (by decide) x hx]
  field_simp [hx]
  unfold spatialRadius spatialNormSq
  ring_nf
  -- -n * (1/r^(n+2) - (n+2)*x1^2/r^(n+4)) + ...
  -- = -n * (3/r^(n+2) - (n+2)*(x1^2+x2^2+x3^2)/r^(n+4))
  -- = -n * (3/r^(n+2) - (n+2)*r^2/r^(n+4))
  -- = -n * (3/r^(n+2) - (n+2)/r^(n+2))
  -- = -n * (3 - (n+2)) / r^(n+2)
  -- For n=1, this is -1 * (3 - 3) / r^3 = 0.
  sorry
