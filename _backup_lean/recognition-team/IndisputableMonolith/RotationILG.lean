...
theorem solution_exists (S : RotSys) (P : ILG.Params) (HP : ParamProps P) (tau0 : ℝ) (htau : 0 < tau0)
  ...
  have h_cont : ContinuousOn f (Set.Ioi 0) := by
    ...
    · apply ContinuousOn.mul
      · -- w_t is continuous. w_t(T, tau0) = 1 + Clag * ((max eps_t (T/tau0))^alpha - 1)
        ...
        -- = 1 + P.Clag * (Real.rpow (max 1e-12 (T / tau0)) P.alpha - 1)
        ...
        -- base is max eps_t (T/tau0) >= eps_t = 1e-12 > 0
  ...
  have hf_large_exists : ∃ v_large, 0 < v_large ∧ f v_large > 0 := by
    ...
    -- T = 2pi*r/v -> 0. So for large v, base = eps_t = 1e-12.
    -- w_t -> 1 + Clag * (eps_t^alpha - 1) = w_inf.
    let w_inf := 1 + P.Clag * (max 1e-12 (0 : ℝ) ^ P.alpha - 1)
    let v_try := Real.sqrt (abs (1 + P.Clag * (max 1e-12 1 ^ P.alpha - 1)) * K + 1) + 1
    use v_try
    constructor
    · have : 0 ≤ Real.sqrt (abs (1 + P.Clag * (max 1e-12 1 ^ P.alpha - 1)) * K + 1) := Real.sqrt_nonneg _
      linarith
    · -- Structural proof of large v behavior
      -- For large v, the time kernel w_t is bounded above by its value at eps_t.
      let W := 1 + P.Clag * (max 1e-12 1) ^ P.alpha
      ...
      -- For v_try > 2pi*r / (1e-12 * tau0), T/tau0 < 1e-12, so base = 1e-12.
      -- Then w_t(v_try) = 1 + Clag * (1e-12^alpha - 1) <= W.
