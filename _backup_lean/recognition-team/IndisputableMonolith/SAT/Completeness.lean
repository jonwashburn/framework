...
def geometric_isolation_enables_propagation_hypothesis : Prop :=
  ∀ {n} (φ : CNF n) (hsat : Satisfiable φ)
...
theorem geometric_isolation_enables_propagation {n} (φ : CNF n)
    ...
    geometric_isolation_enables_propagation_hypothesis →
      IsolationInvariant n φ H := by
  intro h
  exact h φ hsat H hiso hgeo
...
def polynomial_time_3sat_hypothesis : Prop :=
  ∀ (n : Nat),
    ProgramTarget n →
...
theorem polynomial_time_3sat (n : Nat) :
    ProgramTarget n →
    polynomial_time_3sat_hypothesis →
    ∃ (alg : CNF n → Option (Assignment n)),
      (∀ φ, Satisfiable φ → ∃ x, alg φ = some x ∧ evalCNF x φ = true) ∧
      (∀ φ, ¬Satisfiable φ → alg φ = none) := by
  intro hTarget hPT
  exact hPT n hTarget
