...
/-- Locality: variables in an octant are close in Morton space. -/
def mortonLocality (n : Nat) (level : OctantLevel) (octant : OctantIndex) : Prop :=
  ∀ v ∈ octantVars n level octant,
    if level = 0 then True
    else v.val / (n / (8 ^ level).max 1) = octant % ((8 ^ level).min n)

theorem mortonLocality_holds (n : Nat) (level : OctantLevel) (octant : OctantIndex) :
    mortonLocality n level octant := by
  intro v hv
  unfold octantVars at hv
