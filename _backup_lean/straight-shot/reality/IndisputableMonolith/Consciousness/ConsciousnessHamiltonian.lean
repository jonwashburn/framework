...
/-- **C=2A HYPOTHESIS**: Recognition cost equals twice gravitational residual action.

    This is a PHYSICAL HYPOTHESIS, not a definition. It asserts that the J-cost
    geometry and gravitational phase are linked by a factor of 2 due to branch
    superposition in quantum measurement. -/
def recognition_equals_twice_gravity_hypothesis (b : StableBoundary) : Prop :=
...
/-! THE C=2A BRIDGE: Recognition cost equals twice gravitational residual action
    ...
    **Derivation sketch**:
    ...
    4. In branch superposition, potential difference Φ₁₂ is double-counted
    5. This gives factor of 2: C = 2A

    **Status**: Physical hypothesis (not derivable from definitions alone)

    **Why a hypothesis?**: The definitions of BoundaryCost and ResidualAction are
    *independent* — BoundaryCost uses J-cost geometry, ResidualAction uses
    gravitational potential. The C=2A identity asserts these are linked
    by a factor of 2, which comes from the physics of branch superposition.

    **Falsification**: If experimental data showed C ≠ 2A for any boundary,
    the Local-Collapse theory's branch counting would be wrong.

    **Testable prediction**: Collapse threshold at C=1 corresponds to A=1/2,
    which matches the Penrose objective reduction threshold. -/
/-- **HYPOTHESIS**: Recognition cost equals twice gravitational residual action.

    **Mathematical justification:** In a branch superposition model of quantum
    measurement, the potential difference Φ₁₂ between branches is double-counted
    relative to the single-branch case. The energy-time uncertainty gauge E = ℏ/τ
    then links this doubled gravitational phase to the J-cost derived from the
    φ-ladder geometry. -/
theorem recognition_equals_twice_gravity_bridge (b : StableBoundary)
    (h : recognition_equals_twice_gravity_hypothesis b) :
    BoundaryCost b = 2 * (b.coherence_time * ResidualAction b) := h

/-! ## Special Case: Using the explicit hypothesis type -/

/-- Recognition equals twice gravity: theorem version using the explicit hypothesis. -/
...
theorem threshold_coincidence (b : StableBoundary) (h_tau : b.coherence_time = 1)
    (hC2A : recognition_equals_twice_gravity_hypothesis b) :
    (BoundaryCost b ≥ 1) ↔ (ResidualAction b ≥ 1/2) := by
  have h' : BoundaryCost b = 2 * ResidualAction b := by
    -- Specialize C=2τA to τ=1.
    simpa [recognition_equals_twice_gravity_hypothesis, h_tau] using hC2A
  constructor
  · intro hC
    have : 2 * ResidualAction b ≥ 1 := by simpa [h'] using hC
    linarith
  · intro hA
    have : 2 * ResidualAction b ≥ 1 := by linarith
    simpa [h'] using this
...
theorem THEOREM_consciousness_from_gravity_measurement_unity
  ...
  constructor
  · exact recognition_equals_twice_gravity b hC2A
  · constructor
    · exact threshold_coincidence b h_tau hC2A
