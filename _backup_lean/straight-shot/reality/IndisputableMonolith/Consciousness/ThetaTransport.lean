...
/-- The saturation threshold in "patterns" (dimensionless count).
    `Θcrit` is imported from `PhaseSaturation.SaturationThreshold` (currently `φ^45`).
    Interpreting this as a Light‑Field capacity requires extra structure; see
    `LightFieldCapacityGap45` for a **conditional** capacity-scale bridge. -/
...
/-- The empirical claim: RS predicts post-extinction recovery ~10^4 to 10^5 years.
    ...
    Replaces the vacuous `∃ p, True` with a meaningful bound. -/
def recoveryTimePrediction_hypothesis : Prop :=
  ∃ p : RecoveryTimePrediction, 4 ≤ p.order_of_magnitude ∧ p.order_of_magnitude ≤ 5
...
/-- If the initial density is above Θcrit, the relaxed solution stays above Θcrit for all t. -/
theorem relaxedDensity_above_crit (ρ₀ : ThetaDensity) (hρ₀ : Θcrit < ρ₀) (t : ℝ) :
    Θcrit < relaxedDensity ρ₀ t := by
  unfold relaxedDensity
  have hdiff : 0 < ρ₀ - Θcrit := sub_pos.mpr hρ₀
  have hexp_pos : 0 < Real.exp (-t / τ_relax) := Real.exp_pos _
  have hterm_pos : 0 < (ρ₀ - Θcrit) * Real.exp (-t / τ_relax) := mul_pos hdiff hexp_pos
  linarith

/-- If ρ₀ ≥ Θcrit and t ≥ 0, the relaxed solution is bounded above by the initial density. -/
theorem relaxedDensity_le_initial (ρ₀ : ThetaDensity) (hρ₀ : Θcrit ≤ ρ₀) {t : ℝ} (ht : 0 ≤ t) :
    relaxedDensity ρ₀ t ≤ ρ₀ := by
  have hdiff : 0 ≤ ρ₀ - Θcrit := sub_nonneg.mpr hρ₀
  have hτ : 0 < τ_relax := τ_relax_pos
  have hneg : -t / τ_relax ≤ 0 := by
    -- since t ≥ 0 and τ_relax > 0, we have t/τ_relax ≥ 0 and hence -t/τ_relax ≤ 0
    have htdiv : 0 ≤ t / τ_relax := div_nonneg ht (le_of_lt hτ)
    -- rewrite `-t/τ` as `-(t/τ)` and apply `neg_nonpos`.
    simpa [neg_div] using (neg_nonpos.mpr htdiv)
  have hexp_le : Real.exp (-t / τ_relax) ≤ 1 := (Real.exp_le_one_iff).2 hneg
  unfold relaxedDensity
  have hmul :
      (ρ₀ - Θcrit) * Real.exp (-t / τ_relax) ≤ (ρ₀ - Θcrit) * 1 :=
    mul_le_mul_of_nonneg_left hexp_le hdiff
  -- Add Θcrit on both sides and simplify.
  have hadd :
      Θcrit + (ρ₀ - Θcrit) * Real.exp (-t / τ_relax) ≤ Θcrit + (ρ₀ - Θcrit) * 1 :=
    add_le_add_right hmul Θcrit
  -- RHS is `Θcrit + (ρ₀ - Θcrit)` which is `ρ₀`.
  simpa [mul_one, add_sub_cancel] using hadd
