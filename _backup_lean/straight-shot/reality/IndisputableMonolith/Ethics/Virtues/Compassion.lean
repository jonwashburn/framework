...
lemma helperBurden_nonneg
  ...
  exact div_nonneg h_cast_nonneg (le_of_lt h_pos)
...
lemma skewRelief_lower_bound_of_transfer
    (helper sufferer : MoralState) (E_target : ℝ)
    (h_transfer : (1 : ℝ) / (φ ^ 4) ≤ energyTransfer helper sufferer E_target) :
    (1 : ℤ) ≤ skewRelief helper sufferer E_target := by
  unfold skewRelief
  have hφ_pos : 0 < φ := phi_pos
  have hφ4_pos : 0 < φ ^ 4 := pow_pos hφ_pos 4
  apply Int.le_floor.mpr
  have h_prod_ge : ↑1 ≤ energyTransfer helper sufferer E_target * φ ^ 4 := by
    calc (1 : ℝ) = (1 / φ ^ 4) * φ ^ 4 := by field_simp [ne_of_gt hφ4_pos]
      _ ≤ energyTransfer helper sufferer E_target * φ ^ 4 := mul_le_mul_of_nonneg_right h_transfer (le_of_lt hφ4_pos)
  exact_mod_cast h_prod_ge
...
noncomputable def Compassion
  ...
  let helper' : MoralState := {
    ...
    skew := helper.skew + (helper_delta : ℝ)
  ...
  let sufferer' : MoralState := {
    ...
    skew := sufferer.skew - (skew_relief : ℝ)
...
theorem compassion_not_conservative
  ...
  (E_target : ℝ)
  (h_suffering : 0 < sufferer.skew ∧ sufferer.energy < E_target) :
  (Compassion helper sufferer E_target h_suffering).1.skew +
  (Compassion helper sufferer E_target h_suffering).2.skew ≤
  helper.skew + sufferer.skew := by
  unfold Compassion
  simp only
  have h_le := CompassionSpec.helperSkewDelta_le_relief helper sufferer E_target h_suffering
  have h_le_real : (CompassionSpec.helperSkewDelta helper sufferer E_target : ℝ) ≤ (CompassionSpec.skewRelief helper sufferer E_target : ℝ) := by
    exact_mod_cast h_le
  linarith
...
theorem compassion_total_skew_strict
  ...
  have h_lt_real : (CompassionSpec.helperSkewDelta helper sufferer E_target : ℝ) < (CompassionSpec.skewRelief helper sufferer E_target : ℝ) := by
    exact_mod_cast h_lt
  linarith
...
theorem compassion_reduces_suffering
  ...
  (E_target : ℝ)
  (h_suffering : 0 < sufferer.skew ∧ sufferer.energy < E_target) :
  (sufferer.skew - (Compassion helper sufferer E_target h_suffering).2.skew =
      (CompassionSpec.skewRelief helper sufferer E_target : ℝ)) ∧
  (Compassion helper sufferer E_target h_suffering).2.skew ≤ sufferer.skew := by
  unfold Compassion
  simp only
  constructor
  · ring
  · have h_nonneg := CompassionSpec.skewRelief_nonneg helper sufferer E_target h_suffering
    have h_nonneg_real : 0 ≤ (CompassionSpec.skewRelief helper sufferer E_target : ℝ) := by exact_mod_cast h_nonneg
    linarith
...
theorem compassion_costs_helper
  ...
  (E_target : ℝ)
  (h_suffering : 0 < sufferer.skew ∧ sufferer.energy < E_target) :
  (Compassion helper sufferer E_target h_suffering).1.energy < helper.energy := by
  unfold Compassion
  simp only
...
theorem compassion_phi2_transfer_rate
  ...
  (E_target : ℝ)
  (h_suffering : 0 < sufferer.skew ∧ sufferer.energy < E_target) :
  (Compassion helper sufferer E_target h_suffering).2.energy - sufferer.energy ≤
  helper.energy / (Foundation.φ * Foundation.φ) := by
  have h_le := CompassionSpec.energyTransfer_le_helper helper sufferer E_target
  unfold Compassion
  simp only
  calc
    sufferer.energy + min (helper.energy / (φ * φ)) (E_target - sufferer.energy) - sufferer.energy
      = min (helper.energy / (φ * φ)) (E_target - sufferer.energy) := by ring
    _ ≤ helper.energy / (φ * φ) := min_le_left _ _
...
theorem compassion_asymmetric
  ...
  (E_target : ℝ)
  (h_suffering : 0 < sufferer.skew ∧ sufferer.energy < E_target) :
  (Compassion helper sufferer E_target h_suffering).1.energy - helper.energy ≠
  (Compassion helper sufferer E_target h_suffering).2.energy - sufferer.energy := by
  unfold Compassion; simp only
  have h_pos := CompassionSpec.energyTransfer_pos helper sufferer E_target h_suffering
  intro h_eq
  linarith
