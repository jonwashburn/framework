...
/-- Motif distance is bounded by epsilon_net.

    Proof: `motifDistance` is defined as a left fold with `min` starting from
    `epsilon_net = 0.18`. Taking `min` with any value is always ≤ the initial
    accumulator, so the result is bounded by `epsilon_net` for all signals. -/
theorem catalogue_covers_with_epsilon (signal : Fin tauZero → ℂ) :
    motifDistance signal ≤ epsilon_net := by
  unfold motifDistance motifSignatureList motifSignatureRealList epsilon_net
  -- After unfolding, the foldl has exactly two distances; simplifying shows
  -- `min (min 0.18 d1) d2 ≤ 0.18`, which follows by `min_le_left`.
  simp [List.foldl_cons, List.foldl, min_le_left]

/-- **EMPIRICAL AXIOM**: CPM distance control.
    For any signal, there exists a motif within epsilon_net. -/
def CPM_distance_control_hypothesis : Prop :=
...
theorem CPM_distance_control (signal : Fin tauZero → ℂ) :
    CPM_distance_control_hypothesis →
      ∃ motif ∈ motifSignatureList, l2Dist signal motif ≤ epsilon_net := by
  intro h
  exact h signal
