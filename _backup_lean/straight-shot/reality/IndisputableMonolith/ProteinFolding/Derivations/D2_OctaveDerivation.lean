...
theorem phi_2_5_approx : 3.2 < phi^(2.5 : ℝ) ∧ phi^(2.5 : ℝ) < 3.5 := by
  ...
  -- Rewrite φ^2.5 = φ² × φ^0.5
  have h_rewrite : phi^(2.5 : ℝ) = phi^2 * Real.sqrt phi := by
  ...
  have hs_pos : 0 < Real.sqrt phi := Real.sqrt_pos.mpr phi_pos
  constructor
  · -- 2.618 × 1.26 = 3.298 > 3.2
    calc (3.2 : ℝ) < 2.618 * 1.26 := by norm_num
      _ < phi^2 * 1.26 := by nlinarith
      _ < phi^2 * Real.sqrt phi := by nlinarith
  · -- 2.7 × 1.28 = 3.456 < 3.5
    calc phi^2 * Real.sqrt phi < 2.7 * 1.28 := by nlinarith
      _ = 3.456 := by norm_num
      _ < 3.5 := by norm_num
