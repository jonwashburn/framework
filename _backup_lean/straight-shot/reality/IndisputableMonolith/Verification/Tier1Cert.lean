...
@[simp] def Tier1Cert.verified (_c : Tier1Cert) : Prop :=
  ...
  ∧ (∀ {StateSpace : Type}
        ...
        ∃ (Recognizer Recognized : Type),
          Nonempty (IndisputableMonolith.Recognition.Recognize Recognizer Recognized))
  ...
  ∧ (∀ (E : Verification.Necessity.LedgerNecessity.DiscreteEventSystem)
        ...
        (∃ f : Verification.Necessity.LedgerNecessity.FlowFS E ev,
          ...
          ∃ L : IndisputableMonolith.RecogSpec.Ledger,
            Nonempty (E.Event ≃ L.Carrier))
  -- C04: T5 cost uniqueness on ℝ₊ (extension lemma; assumes exp-axis agreement via JensenSketch)
  ∧ (∀ {F : ℝ → ℝ} [IndisputableMonolith.Cost.JensenSketch F] {x : ℝ},
        0 < x → F x = IndisputableMonolith.Cost.Jcost x)
...
@[simp] theorem Tier1Cert.verified_any (c : Tier1Cert) : Tier1Cert.verified c := by
  refine And.intro IndisputableMonolith.Recognition.mp_holds ?_
  ...
  · intro F _ x hx
    exact IndisputableMonolith.Cost.T5_cost_uniqueness_on_pos (F := F) hx
  refine And.intro ?C05 ?_
  · intro x
    exact IndisputableMonolith.PhiSupport.phi_unique_pos_root x
  refine And.intro ?C06 ?_
  · intro d
    exact IndisputableMonolith.Patterns.cover_exact_pow d
  refine And.intro ?C06min ?_
  · intro d T pass hsurj
    exact IndisputableMonolith.Patterns.min_ticks_cover (d := d) (T := T) pass hsurj
  refine And.intro ?C06_8 ?_
  · exact IndisputableMonolith.Patterns.period_exactly_8
  refine And.intro ?C06_8min ?_
  · intro T pass hsurj
    exact IndisputableMonolith.Patterns.eight_tick_min (T := T) pass hsurj
  · intro D h
