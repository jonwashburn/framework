...
\begin{itemize}
\item $H^0(X,L^m)$: the complex vector space of global holomorphic sections of $L^{\otimes m}$.
\item $J_x^k(L^m)$: the $k$--jet space at $x$, i.e. germs of holomorphic sections of $L^{\otimes m}$ modulo those vanishing to order $k+1$ at $x$.
Equivalently $J_x^k(L^m)\cong \mathcal{O}_X(L^{\otimes m})_x/\mathfrak{m}_x^{k+1}$.
\end{itemize}
...
Using Corollary~\ref{cor:global-flat-weighted}, \editamir{for $p<n/2$} choose $j$ large enough so that
\[
...
\]
\editamir{In the borderline case $p=n/2$, the present bounds do \emph{not} imply the existence of such $j$ (see Lemma~\ref{lem:borderline-p-half}); the gluing step must therefore be supplemented by an additional middle-dimensional closure input.}
\end{editamirblockNEW}
\end{itemize}
\begin{editamirblockNEW}
...
\]
and the packing bound $|\mathcal S(Q)|\lesssim \varepsilon^{-2p}=\varepsilon^{-n}$ (Lemma~\ref{lem:sliver-packing}),
...
\item global flat-norm estimate $\mathcal F(\partial T^{\mathrm{raw}})\to 0$ for $p<n/2$ (Corollary~\ref{cor:global-flat-weighted}); in the borderline case $p=n/2$, the current bounds yield only $\mathcal F(\partial T^{\mathrm{raw}})\lesssim \varepsilon^{-1}$ and require an additional closure input (Lemma~\ref{lem:borderline-p-half}),
\item filling with vanishing mass (Proposition~\ref{prop:glue-gap}).
\end{itemize}
The exact-class conclusion is enforced by Proposition~\ref{prop:cohomology-match} together with the per-face matching/transport mechanism (Proposition~\ref{prop:integer-transport}). \editamir{For $p=n/2$, this still requires an additional closure input to guarantee that the filling current $U_\epsilon$ in Proposition~\ref{prop:cohomology-match} can be taken with arbitrarily small mass (Lemma~\ref{lem:borderline-p-half}).}
rather than relying on a decay exponent in $h$.
Thus the hypothesis \textnormal{(H2)} in Theorem~\ref{thm:spine-quantitative} holds \editamir{for $p<n/2$ under the present estimates; the borderline case $p=n/2$ is reduced to supplying one of the additional closure inputs listed in Lemma~\ref{lem:borderline-p-half}}.
\end{proposition}
\end{editjonblock}
...
(ii) the large tensor power $L^{\otimes m}$ in the Bergman/holomorphic inputs (e.g.\ the $m^{-1/2}$ scale).
When both parameters appear in the same discussion, we will temporarily denote the tensor-power parameter by $N$ to avoid collision.
\end{editamirblockNEW}
...
Then there exists a number $N=N(n,p)$ (one may take $N=\dim(\mathcal V_x)+1=\binom{n}{p}^2+1$) such that for every $x\in X$ and every
...
hence using at most $N=D+1$ points in $\mathcal V_x$.
\end{proof}
...
\smallskip
\noindent\textup{(i)} For each point $x\in X$ there exists an integer $m_0(k,x)$ such that for all $m\ge m_0(k,x)$
the natural evaluation map on $k$-jets
\[
H^0(X,L^m)\longrightarrow J^k_x(L^m)
\]
\editamir{Here $J^k_x(L^m):=\mathcal O_X(L^m)_x/\mathfrak m_x^{k+1}\mathcal O_X(L^m)_x$ denotes the $k$-jet space at $x$.}

is surjective.

\smallskip
\noindent\textup{(ii)} More generally, for any \emph{finite} set $S\subset X$ there exists $m_0(k,S)$ such that for all $m\ge m_0(k,S)$
the joint evaluation map
\[
H^0(X,L^m)\longrightarrow \bigoplus_{x\in S} J^k_x(L^m)
\]
...
$H^1(X,L^m\otimes \mathfrak{m}_x^{k+1})=0$ for all $m\ge m_0(k,x)$ (see \cite[III, Thm.~5.2]{Hartshorne77}).
Taking global sections yields the desired surjection
\[
H^0(X,L^m)\twoheadrightarrow H^0\!\left(X,L^m\otimes \mathcal{O}_X/\mathfrak{m}_x^{k+1}\right)\cong J^k_x(L^m).
\]
...
Serre vanishing gives $H^1(X,L^m\otimes \mathcal{I}_Z)=0$ for all $m\ge m_0(k,S)$, hence
\[
H^0(X,L^m)\twoheadrightarrow H^0\!\left(X,L^m\otimes \mathcal{O}_X/\mathcal{I}_Z\right)\cong
\bigoplus_{x\in S} J^k_x(L^m),
\]
as claimed.
\end{proof}
\end{editamirblock}
...
\begin{editamirblock}
\editamir{
\noindent\textbf{Analytic input (Bergman kernel at the $m^{-1/2}$ scale).}
We invoke a \emph{near off-diagonal} Bergman kernel expansion with \emph{derivative control up to order $2$}
on rescaled balls of radius $\asymp m^{-1/2}$ for the positive Hermitian line bundle $(L,h)$ with curvature $\omega$.
...
so that first derivatives of the kernel (hence gradients of the induced peak sections) vary by $O(m^{-1/2})$.
...
We therefore take the $C^2$ near off-diagonal Bergman kernel expansion on the $m^{-1/2}$ scale as a published analytic input
...
Let $P_m(\cdot,\cdot)$ denote the Bergman kernel of $H^0(X,L^m)$ for the $L^2$ inner product induced by $(h^m,\omega)$, written in these coordinates and this frame.
...
$Z=\sqrt m\,z$ and $Z'=\sqrt m\,z'$, we have
...
\medskip
\[
m^{-n}\,P_m\!\left(\tfrac Z{\sqrt m},\tfrac{Z'}{\sqrt m}\right)
=
P_{\mathrm{BF}}(Z,Z') + O(m^{-1/2})
...
\[
s_{a,m}(z)\ :=\ m^{-(n+1)/2}\,\partial_{\overline{Z'_a}}
\Big(P_m(\tfrac Z{\sqrt m},\tfrac{Z'}{\sqrt m})\Big)\Big|_{Z'=0},
\]
viewed as a local holomorphic function via the chosen frame.
Since $P_{\mathrm{BF}}(Z,Z')$ is linear in $\overline{Z'}$ to first order, the $C^2$--control above implies:
\begin{enumerate}
\item $s_{a,m}(0)=0$ for all $a$;
\item the $1$--jets satisfy $ds_{a,m}(0)=dz^a+o(1)$ as $m\to\infty$;
\item moreover, $\sup_{|Z|\le \sigma}\|ds_{a,m}(Z)-ds_{a,m}(0)\|=o(1)$ as $m\to\infty$.
\end{enumerate}
In particular, for $m\ge m_1(\varepsilon)$ the matrix $A_m:=(ds_{a,m}(0))_{a=1}^n$ is invertible and
\[
\sup_{|Z|\le \sigma}\|ds_{a,m}(Z)-dz^a\|\le \varepsilon
\qquad\text{for all }a=1,\dots,n.
\]

Now write each $\lambda_i=\sum_{a=1}^n \lambda_i^a\,dz^a$ in these coordinates and set
\[
s_i \ :=\ \sum_{a=1}^n b_{i,a}\,s_{a,m},
\qquad\text{where } b_i:=A_m^{-1}(\lambda_i).
\]
Then $s_i(0)=0$ and $ds_i(0)=\lambda_i$ \emph{exactly} by construction.
Using the derivative control above and $\|A_m^{-1}\|=O(1)$ for $m$ large, we obtain
\[
\|ds_i(Z)-\lambda_i\|\le \varepsilon\max_{1\le j\le p}\|\lambda_j\|
\qquad\text{for all }|Z|\le \sigma.
\]
Returning to the original variables gives the desired estimate on $B_{c\,m^{-1/2}}(x)$ with $c:=\sigma$.
...
$B_{c\,m^{-1/2}}(x)$, which yields a \emph{single-sheet $C^1$ graph} there.
...
(possibly with singularities away from $B_{c\,m^{-1/2}}(x)$), whose associated integration current is
...
$\{ds_1(y),\ldots,ds_p(y)\}$ linearly independent on $B_{c\,m^{-1/2}}(x)$, hence $Y$ is smooth on that ball.
...
\editamir{For $m$ large enough (equivalently $h$ small enough), there is a holomorphic chart}
...
Fix $k\gg 1$ (to be chosen in Substep~3.5) and let $\eta_k\to 0$ be the graph parameter produced by
Proposition~\ref{prop:cell-scale-linear-model-graph} for $L^k$ on Bergman balls of radius $\asymp k^{-1/2}$.
Choose the translation vectors so that, for each fixed $j$,
\[
|t_{j,a}|\le c\,h
\qquad\text{and}\qquad
\|t_{j,a}-t_{j,a'}\|\ge 20\,\eta_k\,h\quad(a\neq a'),
\]
...
$\dist\bigl(Y_j^a\cap Q,\ \widetilde\Pi_j+t_{j,a}\bigr)\le 2\,\eta_k\,h$ (by Lemma~\ref{lem:global-graph-contraction}
...
$\omega=c_1(L)$ and work with large tensor powers $L^k$.
After shrinking $Q$ (or refining the cube partition), assume $Q$ is contained in a
Bergman ball of radius $O(k^{-1/2})$ for $L^k$.
...
sections $s^{(1)}_{j,a},\ldots,s^{(p)}_{j,a}\in H^0(X,L^k)$ whose restrictions to $Q$,
after trivializing $L^k$ on $Q$, are $C^2$--close to the affine linear functions cutting out
the model planes $\widetilde Y_j^a$.  For $k\gg 1$, the holomorphic implicit function theorem
then gives that
\[
...
(For quantitative transversality/persistence estimates in the large $k$ regime, compare
\cite{Donaldson01}.)
\end{editamirblock}
...
$\sigma^{(1)}_{j,a},\dots,\sigma^{(p)}_{j,a}\in H^0(X,L^k)$ such that
\[
...
$\widetilde\Pi_j+t_{j,a}$ with slope $\le 2\eta_k$.
Mass persistence then follows from Lemma~\ref{lem:sliver-stability}\textnormal{(i)}:
\[
\Mass([Y_j^a]\llcorner Q)=\bigl(1+O(\eta_k^2)\bigr)\Mass([\widetilde\Pi_j+t_{j,a}]\llcorner Q),
\]
so by choosing $k$ large enough that $\eta_k^2\ll \delta$ and taking $m$ large enough in Substep~3.3,
...
Set also $R_{y\to y'}:=H_\#([0,1]\times \partial\Sigma_y)$.
\editamir{(Homotopy formula for currents under a Lipschitz homotopy; see \cite{Fed69,FF60}.)}
Then
\[
\Sigma_{y'}-\Sigma_y\ =\ R_{y\to y'}+\partial Q_{y\to y'},
\qquad
\Mass(Q_{y\to y'})\ \le\ \|v\|\,\Mass(\Sigma_y),
\qquad
\Mass(R_{y\to y'})\ \le\ \|v\|\,\Mass(\partial\Sigma_y).
\]
By Stokes and the comass bounds on $\eta$ and $d\eta$,
\[
|f_\eta(y')-f_\eta(y)|
=|Q_{y\to y'}(d\eta)+R_{y\to y'}(\eta)|
\le \|v\|\Bigl(\Mass(\Sigma_y)+\Mass(\partial\Sigma_y)\Bigr).
\]
Under the small-angle graph hypothesis \textnormal{(a)} and bounded geometry of the chart, each slice has mass
$\Mass(\Sigma_y)\le C\,h^{2n-2p-1}$ and $\Mass(\partial\Sigma_y)\le C\,h^{2n-2p-2}$ with $C=C(n,p,X)$.
Hence
\[
...
$m,h,\varepsilon$ must be synchronized as follows:
\begin{itemize}
\item choose $m$ large and set the cell scale $h$ so that $\sqrt m\,h\sim c\,m^{-1/2}$ (Bergman scale in Lemma~\ref{lem:bergman-control});
\item choose the graph-slope/separation parameter $\varepsilon=\varepsilon(m)\downarrow0$ as required in Proposition~\ref{prop:finite-template};
...
\item take the translation separation in Proposition~\ref{prop:finite-template} with $\delta:=10\,\varepsilon\,\mathrm{diam}(Q)$.
\end{itemize}
...
satisfying \textnormal{(a)} and \textnormal{(b)} is precisely the content of Lemma~\ref{lem:slow-variation-rounding}
together with the $0$--$1$ stability Lemma~\ref{lem:slow-variation-discrepancy}, applied label-by-label.
...
Remark~\ref{rem:weighted-scaling} (in particular, $h\sim c\,m^{-1/2}$ and the corner-exit/graph parameters are synchronized as there).
Then the associated raw current $T^{\mathrm{raw}}$ satisfies
\[
\mathcal F(\partial T^{\mathrm{raw}})=o(m)\qquad\text{as }h\downarrow 0\ \ (\text{equivalently }m\to\infty).
\]
...
\ \lesssim\ m^{\frac{k-1}{k}}\,h^{\,2-\frac{2n}{k}}\;\varepsilon^{-\frac{2p}{k}}.
\]
At the intrinsic Bergman cell size $h\sim m^{-1/2}$, this yields
\[
\frac{\mathcal F(\partial T^{\mathrm{raw}})}{m}\ \lesssim\ m^{-1+\frac{n-1}{k}}\;\varepsilon^{-\frac{2p}{k}}\ \xrightarrow[m\to\infty]{}\ 0
\]
whenever $k>n-1$ (equivalently $p<\tfrac{n+1}{2}$), which includes the Lefschetz-reduced range $p\le n/2$.
This is exactly $\mathcal F(\partial T^{\mathrm{raw}})=o(m)$.
\end{editamirblockNEW}
\end{proof}
...
\editamirNEW{Moreover, the construction in the proof yields the quantitative bound $\varepsilon_m\le C_R\,e^{-c m}$ for some $c>0$ (uniform over $\ell$ with $|a|+|b|\le 1$ and over charts in a fixed finite atlas). In particular, $\varepsilon_m=o(m^{-1/2})$.}
\end{lemma}
...
Solve $\bar\partial u=\bar\partial\tilde s$ using H\"ormander $L^2$ estimates \editamirNEW{(see \cite{Demailly12,MaMarinescu07})} for the positive bundle $(L^m,h^m)$; the weight $e^{-m\phi}$
...
\fi
\editamir{Fix a holomorphic chart identifying a neighborhood of a cell $Q$ with a domain in $\C^{n}=\C^{n-p}\times\C^{p}$ with coordinates $z=(u,w)$, and assume $Q\subset B_{R/\sqrt m}(0)$ for some fixed $R$. Assume moreover that the cell diameter satisfies $h\asymp m^{-1/2}$.}
Let $t\in\C^p$ satisfy $|t|\le c\,h$ (with \editamir{$h\asymp m^{-1/2}$}).
...
$s_j\in H^0(X,L^m)$ with local coefficient $f_j$ on $B_{R/\sqrt m}$ such that
\[
\sup_{B_{R/\sqrt m}}\Bigl(|f_0-1|+\sqrt m\,|\nabla(f_0-1)|\Bigr)\le \varepsilon_m,
\qquad
\sup_{B_{R/\sqrt m}}\Bigl(|f_j-w_j|+\sqrt m\,|\nabla(f_j-w_j)|\Bigr)\le \varepsilon_m\ (1\le j\le p),
\]
for some $\varepsilon_m\to 0$ (as in Lemma~\ref{lem:bergman-affine-approx-hormander}).

For $t=(t_1,\dots,t_p)$ define $\sigma_j:=s_j-t_j s_0$ and write
$\sigma_j=F_j\cdot e^{\otimes m}$ in the chosen local frame, so
\[
F_j(u,w)=f_j(u,w)-t_j f_0(u,w).
\]
Since $\nabla(w_j)=e_j$ and $\nabla(1)=0$ in the Euclidean chart, the above estimates imply
\[
\|\partial_w F-I\|_{L^\infty(B_{R/\sqrt m})}+\|\partial_u F\|_{L^\infty(B_{R/\sqrt m})}
\ \le\ C\,\frac{\varepsilon_m}{\sqrt m},
\]
and at $w=t$ we have
\[
F_j(u,t)=(f_j-w_j)(u,t)-t_j(f_0-1)(u,t),
\qquad\text{hence}\qquad
\sup_{u:(u,t)\in B_{R/\sqrt m}}|F(u,t)|\le C\,\varepsilon_m .
\]
Set $\eta_m:=C\,\varepsilon_m/h$, where $h=\mathrm{diam}(Q)\asymp m^{-1/2}$ and (by Lemma~\ref{lem:bergman-affine-approx-hormander}) $\varepsilon_m=o(m^{-1/2})$; hence $\eta_m\to 0$. Moreover, for $m$ large one has $C\,\varepsilon_m/\sqrt m\le \eta_m$, so
\[
\|\partial_w F-I\|_{L^\infty}+\|\partial_u F\|_{L^\infty}\le \eta_m,
\qquad
\sup_{u:(u,t)\in B_{R/\sqrt m}}|F(u,t)|\le \eta_m\,h .
\]
...
$\sup_{u}|\widetilde F(u,0)|=\sup_{u}|F(u,t)|\le \eta_m h$.

Choose $r\simeq h$ and a product set $U_u\times U_{\widetilde w}\subset B_{R/\sqrt m}$
with $Q\subset U_u\times (t+U_{\widetilde w})$ and $U_{\widetilde w}\subset B_{\widetilde w}(0,r)$.
For $m$ large we have $\eta_m\ll 1$ and $\eta_m h\le r/2$, so Lemma~\ref{lem:global-graph-contraction}
...
\iffalse
\begin{proposition}[{\color{blue}Corner--exit footprint geometry is preserved under small--slope holomorphic graphs}]\label{prop:holomorphic-corner-exit-g1g2}
\begin{editamirblockNEW}
Fix $d\ge 2$ and $1\le k<d$.  Let $Q=[0,h]^d$ and let $v$ be a vertex.
...
Let $\delta:=\min\{\dist(E,F): F\ \text{a codimension--$1$ face of $Q$ with }F\notin\{F_0,\dots,F_k\}\}>0$ (positivity follows from \eqref{H1corner}). Assume $\varepsilon$ is small enough so that $\Phi(E)\subset B(v,(c_0+\tfrac12)h)$ and $\sup_{x\in E}|\Phi(x)-x|<\delta/2$.
...
\iffalse
\begin{editamirblock}
\editamir{This proves the ``$\Rightarrow$'' direction in \textup{(G1)}.  The ``$\Leftarrow$'' direction follows because each facet map is a homeomorphism on its interior (by small-slope graph control), hence the image meets a codimension-one face if and only if the preimage meets that face.}
\end{editamirblock}
\iffalse
This proves the ``$\Rightarrow$'' direction in \eqref{G1faces}.  The ``$\Leftarrow$'' direction follows because each facet
\fi
\fi
\begin{editamirblock}
...
separated by $\editamir{\|t_a-t_b\|\ge 10\,C_{\mathrm{graph}}\,\varepsilon\,\mathrm{diam}(Q)}$.
...
\]
where $c>0$ is the universal constant in Lemma~\ref{lem:bergman-control}.\editamir{ By increasing $m$ if necessary, we may assume in fact $\mathrm{diam}(Q)\le (c/2)\,m^{-1/2}$; then $Q\subset B_{(c/2)\,m^{-1/2}}(x)$ for every $x\in Q$.}
...
$s_{a,1},\dots,s_{a,p}\in H^0(X,L^m)$ whose first-order jets at $x_a$ realize the covectors $\lambda_i$. \editamir{This applies on $Q$ since $Q\subset B_{(c/2)\,m^{-1/2}}(x_a)\subset B_{c\,m^{-1/2}}(x_a)$ (because $x_a\in Q$ and we ensured $\mathrm{diam}(Q)\le (c/2)\,m^{-1/2}$).}
Let $Y^a:=\{s_{a,1}=\cdots=s_{a,p}=0\}$.
\editamir{Let $C_{\mathrm{graph}}$ be the slope constant from Lemma~\ref{lem:graph-from-grad}.} Then Lemma~\ref{lem:graph-from-grad} gives (i) with slope $\le C_{\mathrm{graph}}\varepsilon$, and Lemma~\ref{lem:sliver-stability}(i) gives (iii) with the same slope parameter. \editamir{Applying Lemma~\ref{lem:sliver-stability}(ii) with $\varepsilon$ replaced by $C_{\mathrm{graph}}\varepsilon$ and using the separation assumption $\|t_a-t_b\|\ge 10\,C_{\mathrm{graph}}\varepsilon\,\mathrm{diam}(Q)$ yields (ii).}
\end{proof}
\end{editblock}
...
\qquad
\varepsilon_{\mathrm{glue}}\xrightarrow[\delta,\varepsilon\to 0,\ \mathrm{mesh}\to 0,\ m\to\infty]{}0.
\]
\end{lemma}
...
\]
Apply Proposition~\ref{prop:glue-gap} (with $T^{\mathrm{raw}}=S$) to obtain an integral $k$--current $R_{\mathrm{glue}}$ with
\[
...
linear system $|L^{\otimes m}|$, such that
\[
\mathrm{PD}([Z]) \;=\; c_1(L^{\otimes m})^p \;=\; m^p\,[\omega^p].
\]
In particular, any rational multiple of $[\omega^p]$ is an algebraic class.
\end{lemma}

\begin{proof}
Choose $m\gg 0$ so that $L^{\otimes m}$ is very ample and basepoint free.
Let $D_1,\dots,D_p\in |L^{\otimes m}|$ be generic divisors and set
\[
...
\;=\; c_1(L^{\otimes m})^p \;=\; m^p\,c_1(L)^p \;=\; m^p\,[\omega^p],
\]
as claimed.
\end{proof}
\end{editamirblock}
...
and near/off-diagonal Bergman kernel expansions with controlled derivatives at the $m^{-1/2}$ scale.%
