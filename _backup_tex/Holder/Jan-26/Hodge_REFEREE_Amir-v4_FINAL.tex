...
% ==========================================================
% Define colors
\definecolor{editjonColor}{rgb}{0,0,0}% editjonColor forced to black
...
\newenvironment{editamirblock}{\begingroup\color{black}}{\endgroup}

\newcommand{\editamirNEW}[1]{#1}
\newenvironment{editamirblockNEW}{\begingroup\color{black}}{\endgroup}
...
We prove the Hodge conjecture: every rational Hodge class
\[
\gamma \in H^{2p}(X,\Q)\cap H^{p,p}(X)
\]
is represented by an algebraic cycle.
The proof reduces the problem to a realization statement for smooth closed strongly positive $(p,p)$--forms.
Given $\gamma$, choose $N\gg 1$ so that $\gamma^+:=\gamma+N[\omega^p]$ admits a smooth closed strongly positive representative,
while $\gamma^-:=N[\omega^p]$ is algebraic (as a complete intersection), hence $\gamma=\gamma^+-\gamma^-$.

For such a cone--positive class $\gamma^+$ with representative $\beta$, we construct (for a fixed integer $m$) a sequence of
integral cycles $T_k$ in the class $\mathrm{PD}(m[\gamma^+])$ whose calibration defects tend to $0$ and whose masses converge
to the cohomological lower bound; in particular, the limiting object yields an effective integral cycle realizing $m[\gamma^+]$.
Algebraicity of $\gamma$ then follows by subtracting the complete--intersection contribution and reducing to $p\le n/2$ via Hard
Lefschetz.
The final passage from analytic to algebraic cycles uses Chow's theorem and GAGA. \cite{Hartshorne77,GH78,Serre56}
\end{abstract}
...
Equivalently, if $T$ is represented by a countably $\ell$--rectifiable set $M$
...
{\color{black}$\varepsilon\,h\le s/100$} so the realized sheet inherits the same designated-face footprint
...
Any calibrated limit is a positive combination of complex subvarieties (King/Harvey--Lawson), hence algebraic on projective manifolds (Chow/GAGA).
A signed decomposition reduces the general case to the cone--positive case, and Hard Lefschetz treats the full $p$--range.

We retain the term ``calibration--coercivity'' for historical continuity; the explicit calibration--coercivity estimate is not used in the main realization/SYR chain.
...
recorded in Proposition~\textup{(gluing-gap proposition; proved later)} produces fixed-class integral
cycles $T_k$ with $\Mass(T_k)\to c_0$ (equivalently, $\Mass(T_k)-\langle T_k,\psi\rangle\to 0$).
By Theorem~\textup{(realization-from-almost theorem; proved later)}, a subsequential limit is a $\psi$--calibrated integral current; Harvey--Lawson then identifies it as a positive sum of complex analytic subvarieties, hence algebraic on projective $X$ by Chow/GAGA.
...
By Hard Lefschetz (Remark~\textup{(Lefschetz reduction remark; proved later)}), it suffices to treat the range $p\le n/2$.
...
Section~\textup{(realization block; proved later)} constructs, for a fixed integer $m$, a sequence of integral cycles $T_k$
in the class $\mathrm{PD}(m[\gamma^+])$ such that $\Mass(T_k)-\langle T_k,\psi\rangle\to 0$ (hence $\Mass(T_k)\to m\int_X\beta\wedge\psi$), culminating in the SYR summary theorem (Theorem~\textup{(automatic SYR theorem; proved later)}).
The key technical point is the microstructure/gluing estimate $\mathcal F(\partial T^{\mathrm{raw}})=o(m)$ (Proposition~\textup{(gluing-gap proposition; proved later)}),
...
(see Remark~\textup{(weighted-scaling remark; proved later)}).
...
(Remark~\textup{(Lefschetz reduction remark; proved later)}), and the case $p>n/2$ is recovered by intersecting with hyperplanes.
...
Section~\textup{(realization block; proved later)} is the heart of the manuscript: it proves the projective tangential approximation and the microstructure/gluing theorem needed to realize smooth cone-valued forms by holomorphic pieces with vanishing flat-norm boundary (after correction by integral fillings), culminating in the SYR summary theorem (Theorem~\textup{(automatic SYR theorem; proved later)}).
...
the realization/SYR construction produces almost-calibrated integral cycles and a calibrated limit current (Theorem~\textup{(automatic SYR theorem; proved later)}), which is
...
\small
\textbf{Main closure chain (used for Theorem~\textup{(main Hodge theorem; proved later)}).}
\begin{enumerate}
\item \textbf{Hard Lefschetz reduction} (Remark~\textup{(Lefschetz reduction remark; proved later)}): reduces the Hodge problem to the range $p\le n/2$.
\item \textbf{Signed decomposition} (Lemma~\textup{(signed-decomposition lemma; proved later)}): $\gamma=\gamma^+-\gamma^-$ with $\gamma^- = N[\omega^p]$ and $\gamma^+$ cone--positive.
\item \textbf{Algebraicity of $\gamma^-$} (Lemma~\textup{($\gamma^-$ algebraicity lemma; proved later)}): $[\omega^p]$ is represented by complete intersections, hence $\gamma^-$ is algebraic.
\item \textbf{Microstructure/gluing estimate} (Proposition~\textup{(gluing-gap proposition; proved later)}): $\mathcal F(\partial T^{\mathrm{raw}})=o(m)$ for the constructed sheet-sum on a mesh (in the range $p\le n/2$; see Remark~\textup{(weighted-scaling remark; proved later)}).
\item \textbf{Mass convergence / almost-calibration} (Proposition~\textup{(almost-calibration proposition; proved later)}): for the corrected cycles $T_\epsilon=S-U_\epsilon$ one has
...
\item \textbf{Automatic SYR} (Theorem~\textup{(automatic SYR theorem; proved later)}): starting from a smooth closed cone-valued representative $\beta$ of $\gamma^+$, the construction yields fixed-class integral cycles with vanishing calibration defect (hence $\Mass(T_k)\to c_0$).
\item \textbf{Calibrated limit and algebraicity}:
Theorem~\textup{(realization-from-almost theorem; proved later)} gives a $\psi$--calibrated integral limit current; Harvey--Lawson identifies it with a positive sum of complex analytic subvarieties, which are algebraic on projective $X$ by Remark~\textup{(Chow--GAGA remark; proved later)}.
\end{enumerate}
...
In this section we fix the analytic and geometric setup used throughout.
...
These conventions will be used systematically in the calibration and current formalisms and in the quantitative estimates appearing later.
...
\]
If $T$ is a $k$--current on $X$, its \emph{mass} is
\[
...
We introduce the calibrated Grassmannian of complex $p$--planes and the pointwise cone geometry determined by the K\"ahler calibration.
...
We record standard K\"ahler/Hodge estimates controlling off--type components and the primitive part of a closed form in terms of the energy of a Coulomb potential. These estimates provide analytic background and are not used in the main realization/SYR chain.
...
\begin{proof}
We use standard Hodge-theoretic elliptic estimates; see \cite{Wells}.
...
We construct an explicit $\varepsilon$--net on the fiberwise calibrated Grassmannian and record covering bounds used later to discretize calibrated directions and control angular errors in quantitative estimates.
...
We develop pointwise linear--algebra estimates converting wedge/Pl\"ucker closeness into distance to the calibrated cone, with explicit tracking of constants. These estimates interface with the $\varepsilon$--net from Section~\ref{sec:epsnets} and the calibration--coercivity inequalities proved later.



\noindent\textbf{Optional background (Hermitian/PSD comparison).}
The material in this block is included for context and comparison and is not used in the main realization/SYR chain.
...
explicit SYR/microstructure construction in Section~\textup{(realization block; proved later)}.
\end{remark}
...
\item global flat-norm estimate $\mathcal F(\partial T^{\mathrm{raw}})\to 0$ for $p<n/2$ (Corollary~\ref{cor:global-flat-weighted}); in the borderline case $p=n/2$, the current bounds yield only $\mathcal F(\partial T^{\mathrm{raw}})\lesssim \varepsilon^{-1}$ and require an additional closure input (the middle-dimensional closure lemma (proved later in Section 8.B)),
\item filling with vanishing mass (Proposition~\ref{prop:glue-gap}).
\end{itemize}
The exact-class conclusion is enforced by Proposition~\ref{prop:cohomology-match} together with the per-face matching/transport mechanism (Proposition~\ref{prop:integer-transport}). \editamir{For $p=n/2$, this still requires an additional closure input to guarantee that the filling current $U_\epsilon$ in Proposition~\ref{prop:cohomology-match} can be taken with arbitrarily small mass (the middle-dimensional closure lemma (proved later in Section 8.B)).}
rather than relying on a decay exponent in $h$.
Thus the hypothesis \textnormal{(H2)} in Theorem~\ref{thm:spine-quantitative} holds \editamir{for $p<n/2$ under the present estimates; the borderline case $p=n/2$ is reduced to supplying one of the additional closure inputs listed in the middle-dimensional closure lemma (proved later in Section 8.B)}.
\end{proposition}
...
$\partial T=0$.
\noindent\emph{(See \cite[\S\S 4.1--4.2]{Fed69}.)}
Therefore $T$ is integral and, by the first part, a cycle.
\end{proof}
...
If $X$ is projective, each $V_j$ is algebraic (Chow/GAGA (see \cite{Hartshorne77,GH78,Serre56})), and therefore $[\gamma]\in H^{2p}(X;\Q)$ is an
...
\noindent\emph{(See \cite{HL82,King71}.)}
...
If $X$ is projective, each $V_j$ is algebraic by Chow/GAGA (Chow/GAGA (see \cite{Hartshorne77,GH78,Serre56})).
\end{proof}
...
\noindent\emph{(See \cite[Ch.~XIV]{LangGmT} \cite{Sim83}.)}
...
\begin{proof}
Since $[T_k]=A$ is fixed and $\psi$ is closed, the pairing is constant:
\(
\langle T_k,\psi\rangle=\langle A,[\psi]\rangle=c_0
\)
for all $k$.  The hypothesis $\Def_{\mathrm{cal}}(T_k)\to 0$ therefore gives $\Mass(T_k)\to c_0$, hence $\sup_k\Mass(T_k)<\infty$.
...
By Federer--Fleming compactness for integral currents on a compact manifold (e.g.\ \cite{Fed69}), after passing to a subsequence
we may assume $T_k\to T$ in the flat norm for some integral current $T$.
Because $\partial T_k=0$ for all $k$, the limit is also a cycle, $\partial T=0$ (Lemma~\ref{lem:flat_limit_of_cycles_is_cycle}),
and the homology class is preserved, $[T]=A$.
...
\Mass(T)=\langle T,\psi\rangle=c_0,
\)
so $T$ is $\psi$--calibrated.

\noindent\emph{(See \cite{HL82,King71}.)}
For the K\"ahler calibration, calibrated integral currents are positive and $d$--closed of bidimension $(p,p)$ (see \cite{HL82}),
and King's theorem \cite{King71} implies such a current is a holomorphic chain
\(
T=\sum_j m_j[V_j].
\)
If $X$ is projective, each $V_j$ is algebraic by Chow/GAGA (Chow/GAGA (see \cite{Hartshorne77,GH78,Serre56})).
...
\]
\editamir{where $V_j\subset X$ are irreducible complex analytic subvarieties of codimension $p$.  If, moreover, $X$ is projective, then each $V_j$ is algebraic by Chow/GAGA (Chow/GAGA (see \cite{Hartshorne77,GH78,Serre56})), so $[\gamma]\in H^{2p}(X;\Q)$ is an algebraic class.}
\end{theorem}
...
\(T=\sum_j m_j[V_j]\).
When $X$ is projective, algebraicity of analytic subvarieties follows by Chow/GAGA (see \cite{Hartshorne77,GH78,Serre56}).
\end{proof}
...
\smallskip\noindent\textbf{Remark (what is actually used later).}
...
\smallskip
\noindent\textup{(i)} For each point $x\in X$ there exists an integer ${\color{black}m_{\mathrm{hol},0}(k,x)}$ such that for all {\color{black}$\mhol\ge m_{\mathrm{hol},0}(k,x)$}
the natural evaluation map on $k$-jets
\[
H^0(X,L^{\mhol})\longrightarrow J^k_x(L^{\mhol})
\]
\editamir{Here $J^k_x(L^{\mhol}):=\mathcal O_X(L^{\mhol})_x/\mathfrak m_x^{k+1}\mathcal O_X(L^{\mhol})_x$ denotes the $k$-jet space at $x$.}

is surjective.

\smallskip
\noindent\textup{(ii)} More generally, for any \emph{finite} set $S\subset X$ there exists ${\color{black}m_{\mathrm{hol},0}(k,S)}$ such that for all {\color{black}$\mhol\ge m_{\mathrm{hol},0}(k,S)$}
...
\]
\noindent\emph{(See \cite[Thm.~III.5.2]{Hartshorne77}.)}
Since $\mathfrak{m}_x^{k+1}$ is coherent and $L$ is ample, Serre vanishing (see Hartshorne~\cite[Thm.~III.5.2]{Hartshorne77}) gives
$H^1(X,L^{\mhol}\otimes \mathfrak{m}_x^{k+1})=0$ for all {\color{black}$\mhol\ge m_{\mathrm{hol},0}(k,x)$} (see \cite[III, Thm.~5.2]{Hartshorne77}).
...
Serre vanishing (Hartshorne~\cite[Thm.~III.5.2]{Hartshorne77}) gives $H^1(X,L^{\mhol}\otimes \mathcal{I}_Z)=0$ for all {\color{black}$\mhol\ge m_{\mathrm{hol},0}(k,S)$}, hence
\[
...
Let $P_{\mhol}(z,z')$ be the Bergman kernel of $(L^{\mhol},h^{\mhol})$ written in this frame.
...
\noindent\emph{(See \cite{MaMarinescu13OffDiag,MaMarinescu07}.)}
See e.g.\ \cite{MaMarinescu13OffDiag,MaMarinescu07} for statements implying \eqref{eq:bergman-weighted-rescaled} (near off-diagonal with $C^2$ control).
...
\]
Here $c>0$ is the constant from Lemma~\ref{lem:bergman-control}, and angles are computed after identifying $T_yX$ with $T_xX$ by Levi--Civita parallel transport inside $B_{c\,\mhol^{-1/2}}(x)$.
...
By the $C^1$ implicit function theorem recorded above (applied to $S=(s_1,\dots,s_p)$ in the $(w,u)$-coordinates),
{\color{black}If $\varepsilon>\tfrac14$, replace $\varepsilon$ by $\min\{\varepsilon,\tfrac14\}$; this only strengthens the conclusion. Hence we may assume $\varepsilon\le \tfrac14$.} if $0<\varepsilon\le 1/4$ then $dS$ remains uniformly surjective in the $w$-directions on $B_{c\mhol^{-1/2}}(x)$,
hence $Y$ is a smooth complex submanifold on that ball.
...
\]
{\color{black}(each a smooth complete intersection in $|L^{m_{\mathrm{hol},\alpha,j}}|$ for some sufficiently large integer $m_{\mathrm{hol},\alpha,j}$),}
...
Apply Proposition~\ref{prop:tangent-approx-full}{\color{black} (Since the set of pairs $(x_\alpha,\Pi_{\alpha,j})$ used below is finite, fix one integer $\mhol(\varepsilon)\ge m_{\mathrm{hol},0}(\varepsilon,X)$ so the proposition applies simultaneously to each pair; we use this common $\mhol(\varepsilon)$ throughout.)} to realize each net direction by a smooth $\psi$-calibrated complete
...
\editamir{This step constructs multiple calibrated sheet pieces on each cube $Q$ (multiplicity allowed; disjointness is optional) with prescribed}
...
\]
i.e.\ by $\sum_j(x_j+y_j)\le s$ together with nonnegativity of the $k=2(n-p)$ coordinates $(x_j,y_j)_{j=1}^{n-p}$.
...
The stated $\mathcal{H}^k$ and $\mathcal{H}^{k-1}$ estimates follow from the area formula applied to $\Gamma$ on $E$ and on rectifiable charts for $F$,
...
(from Lemma~\ref{lem:bergman-control}).  Define the \emph{holomorphic scale}
\[
s:=\sigma\,\mhol^{-1/2}
\qquad\text{with a fixed }\sigma\in\bigl(0,\tfrac{c(X,\omega)}{100}\bigr)
\]
and assume $s\le h/100$ (equivalently, $\mhol$ is large compared to $h^{-2}$).
Let $k:=2n-2p$.
...
{\color{black}Let $\mathcal E_j\subset \widetilde\Pi_j$ be a fixed \emph{corner--exit footprint shape} of diameter $\asymp s$.
...
Its mass is controlled by the standard area estimate for $C^1$ graphs:
...
$P_{x,j}$ on $Q$.  {\color{black}Apply Theorem~\ref{thm:local-sheets} to each cube to obtain, for each direction label $(Q,j)$, a $\psi$--calibrated sheet piece $Y_{Q,j}\subset Q$.
...
\]
and, writing $\mathsf{Bar}(S_Q)$ for the (mass--normalized) barycenter of the tangent--plane Young measure of $S_Q$,
\[
...
{\color{black}We identify an oriented complex $(n-p)$--plane $\Pi$ with its associated Hilbert--Schmidt unit tensor $\xi_{\Pi}$
...
\]
A standard Lipschitz estimate on the Grassmannian gives $\|\xi_{\Pi}-\xi_{\Pi'}\|_{\mathrm{HS}}\le C\,\angle(\Pi,\Pi')$,
...
$\beta$ with $\beta(x)\in K_p(x)$ pointwise.  {\color{black}There exists an integer $m\ge 1$ (clearing denominators of $[\gamma]$) such that for every $\varepsilon>0$
...
\[
A_{Q,j}\ \le\ {\color{black}\frac{\delta}{2J(Q)}}\,\max\{M_Q,h^{2n}\}
\qquad\text{for all }j,
\]
...
{\color{black}Apply Theorem~\ref{thm:local-sheets} to realize each direction $(Q,j)$ by a single $\psi$--calibrated sheet piece $Y_{Q,j}\subset Q$
...
{\color{black}The transport/gluing estimates across faces require a precise identification of the transverse
...
\begin{lemma}[Integral optimal couplings for discrete integer measures]\label{lem:w1-integral-optimal}
...
the hypothesis of Corollary holds with $\editamir{\Delta_F=O(\varrho\,h^2)}$.
\end{lemma}
...
\smallskip
\editblue{In the \emph{sliver} regime, the count $N_F$ is not controlled by total mass; see Remark and
...
\editamir{In the global-coherence regime (Proposition), the boundary slices on $F$ are parameterized by the \emph{same} integer-weighted discrete measure}
...
\begin{align*}
\mathcal F(B_F)
&\le \editamir{C\,\varrho\,h^2}\Biggl[
...
\mu_{Q'\to F}=(\Phi_{Q',F})_\#\nu',
\]
where $\nu$ and $\nu'$ are integer-weighted discrete measures supported in $\editamir{B_{C_0\varrho h}(0)}\subset\R^{2p}$ {\color{black}and the face maps satisfy (by Lemma~\ref{lem:face-map-stability})}
...
\begin{aligned}
\mathcal F(B_F)\ \le\ {}&
C\,h^2\Bigl(\Mass(\partial S_Q\llcorner F)+\Mass(\partial S_{Q'}\llcorner F)\Bigr)\\
&+C\,h\,\theta_F\Bigl(\Mass(\partial S_Q\llcorner F)+\Mass(\partial S_{Q'}\llcorner F)\Bigr)
\ +\ C_{\angle}\,\varepsilon\,M_F.
\end{aligned}
\]
...
$\mathbf y=(y_a)_{a\ge1}\subset \editamir{B_{C_0\varrho h}(0)}\cap \delta_{\perp}\mathbb Z^{2p}$.
...
For \textnormal{(ii)}, by Proposition the sheet piece $Y^{Q,a}\cap Q$ is a single
...
\]
and the packing bound $|\mathcal S(Q)|\lesssim \varepsilon^{-2p}=\varepsilon^{-n}$ (Lemma),
...
(Proposition~\ref{prop:holomorphic-corner-exit-g1g2} and Corollary), together with the prefix-tail reduction
in Lemma.
\end{remark}
...
For example, Lemma produces a nested ordered sequence on a sphere (uniform density), and scaling embeds it into $\editamir{B_{C_0\varrho h}(0)}$.
...
If the target counts come from rounding a smooth density, Lemma implies $|N_Q-N_{Q'}|/N_Q=O(h)$ in the “many pieces’’ regime.
...
\item[\textnormal{(iii)}] (\textbf{Local realizability on a fixed template}) for each $Q$ {\color{black}there exist $\psi$--calibrated holomorphic pieces (no disjointness is assumed; repetitions/multiplicity are allowed)}
...
and  / ).
\item \textbf{All labels simultaneously (B1)} The all-direction packaged execution is recorded in Proposition.
\end{itemize}
...
In the Euclidean ball-cell model of Proposition, fix a radius $r\in(0,h)$ so that each affine piece
...
Consequently the tail-heaviness hypothesis \textnormal{(b)} in Lemma~\ref{lem:oh-face-edit-regime} holds with $\kappa=1$ (uniformly in the refinement parameter), and item~\textnormal{(iv)} follows by combining Lemma~\ref{lem:oh-face-edit-regime} with the checkerboard face-edit estimate of Proposition.
...
\noindent\emph{(See \cite[Ch.~6]{Voisin02}.)}
...
Fix $\varepsilon>0$ and choose {\color{black}$\mhol\ge m_{\mathrm{hol},1}(\varepsilon)$} (Lemma~\ref{lem:bergman-control}) with $\mathrm{diam}(Q)\le c\,\mhol^{-1/2}$.
...
With {\color{black}$\mhol\ge m_{\mathrm{hol},1}(\varepsilon)$} and $\mathrm{diam}(Q)\le c\,\mhol^{-1/2}$, the Bergman-scale $C^1$ control and the holomorphic finite-template
...
The global activation/gluing bookkeeping (Theorem~\ref{thm:sliver-mass-matching-on-template} and Proposition)
...
the affine planes $\{P_{v,a}\}_{a\ge 1}$ {\color{black}are realized in $Q$ by $\psi$--calibrated holomorphic pieces (no disjointness is assumed; one may repeat pieces and interpret repetition as multiplicity)}
...
(e.g.\ by Lemma applied to Lipschitz target budgets).
\end{enumerate}
...
one may implement the block-uniform coded interleaving of Definitions--
and then use Proposition for the face-edit estimate.
...
(Alternatively, under the block-uniform coded interleaving, one may invoke Proposition.)
...
$|N_{Q,v,i}-N_{Q',v,i}|\lesssim h\min\{N_{Q,v,i},N_{Q',v,i}\}$ on the region where $M_{Q,i}$ is not negligible (e.g.\ via Lemma
and the $0$--$1$ stability Lemma);
...
(Lemma), one can choose the integer activations so that the \emph{raw} current has the desired periods up to an error $<\tfrac14$
...
\]
\editamir{(Here $S_Q:=\sum_{j=1}^N\sum_{a=1}^{N_{Q,j}}[Y_{Q,j}^a]\llcorner Q$ is the local integral current built from the sheet pieces, and $S_Q(\Theta_\ell):=\int_{S_Q}\Theta_\ell=\sum_{j,a}\int_{Y_{Q,j}^a\cap Q}\Theta_\ell$.)}
...
\]
Set $U_\epsilon:=-R_{\mathrm{glue}}$. Then $\partial U_\epsilon=R=\partial S$ and $\Mass(U_\epsilon)$ satisfies the bound required for Step~4.

Then $T_\epsilon:=S-U_\epsilon$ is a closed integral cycle, hence by Lemma each $\int_{T_\epsilon}\Theta_\ell\in\Z$.
...
By Lemma it follows that $\int_{T_\epsilon}\Theta_\ell=mI_\ell$ for all $\ell$.
...
Using Lemma and Lemma, round the real budgets
...
(Lemma) to achieve item \textnormal{(c)}.
...
satisfying \textnormal{(a)} and \textnormal{(b)} is precisely the content of Lemma
together with the $0$--$1$ stability Lemma, applied label-by-label.
...
Lemma and the class-identification Proposition~\ref{prop:cohomology-match}.
...
(Proposition) with the small-boundary input supplied by the global flat-weighted estimates
...
\]
At the intrinsic Bergman cell size $h\sim \mhol^{-1/2}$, one can record the indicative scaling {\color{black}
\[
\frac{\mathcal F(\partial T^{\mathrm{raw}})}{m}\ \lesssim\ m^{-1/k}\,\mhol^{-\left(1-\frac{n}{k}\right)}\;\varepsilon^{-\frac{2p}{k}}.
\]
}
{\color{black}We emphasize again: $m$ is fixed in the SYR definition; no $m\to\infty$ limit is taken. Smallness is obtained by taking $\delta,\varepsilon\to0$ and refining the mesh (equivalently choosing $\mhol$ large enough for the local realization scale), as quantified in Lemma.}
...
This checkpoint is now achieved by Proposition, which builds holomorphic complete intersections
...
Then for all sufficiently large {\color{black}$\mhol$} there exists a global section
{\color{black}$s_{\ell,\mhol}\in H^0(X,L^{\mhol})$} such that, writing
{\color{black}$s_{\ell,\mhol}=f_{\ell,\mhol}\,e^{\otimes \mhol}$} on $B_{\rho/8}(0)$, one has on the Bergman-scale ball
{\color{black}$B_{R/\sqrt{\mhol}}(0)\subset B_{\rho/8}(0)$}:
\[
\sup_{|z|\le R/\sqrt{\mhol}}
\Bigl(|f_{\ell,\mhol}(z)-\ell(z)|+\sqrt{\mhol}\,|\nabla(f_{\ell,\mhol}-\ell)(z)|\Bigr)
\ \le\ {\color{black}\varepsilon_{\mathrm{hol}}(\mhol)},
\qquad
{\color{black}\varepsilon_{\mathrm{hol}}(\mhol)\xrightarrow[\mhol\to\infty]{}0},
\]
with constants uniform in $x$ (over a fixed finite atlas) and in $\ell$ with $|a|\le 1$.
{\color{black}Moreover one may take $\varepsilon_{\mathrm{hol}}(\mhol)\le C_R e^{-c\mhol}$ for some $c>0$ depending only on the
positivity of $(L,h)$ and the cutoff scale.}
\end{lemma}

\begin{proof}
Choose a cutoff $\chi$ supported in $B_{\rho/2}(0)$ with $\chi\equiv 1$ on $B_{\rho/4}(0)$ and set
{\color{black}$\tilde s:=\chi\,\ell\,e^{\otimes \mhol}$} (extended by $0$ outside $U$).
Then $\bar\partial\tilde s=(\bar\partial\chi)\,\ell\,{\color{black}e^{\otimes \mhol}}$ is supported in the annulus
$\{\rho/4\le |z|\le \rho/2\}$.
\noindent\emph{(See \cite{Hormander65,Demailly12}.)}
Solve $\bar\partial u=\bar\partial\tilde s$ using H\"ormander $L^2$ estimates (see \cite{Hormander65,Demailly12}) for the positive bundle
{\color{black}$(L^{\mhol},h^{\mhol})$}; the weight $e^{-{\color{black}\mhol}\phi}$ forces
$\|u\|_{L^2(h^{\mhol})}\le C\,e^{-c\,{\color{black}\mhol}}$.
...
(e.g.\ mean-value inequality plus Cauchy estimates at scale ${\color{black}\mhol^{-1/2}}$)
give $\|u\|_{C^1({\color{black}B_{R/\sqrt{\mhol}}})}\le C_R\,e^{-c\,{\color{black}\mhol}}$.
Setting {\color{black}$s_{\ell,\mhol}:=\tilde s-u$} yields $s_{\ell,\mhol}$ holomorphic and
$f_{\ell,\mhol}=\ell-\text{(holomorphic error)}$ on ${\color{black}B_{R/\sqrt{\mhol}}}$ with the stated bound.
\end{proof}
...
\editamir{Fix a holomorphic chart identifying a neighborhood of a cell $Q$ with a domain in $\C^{n}=\C^{n-p}\times\C^{p}$ with coordinates $z=(u,w)$, and assume $Q\subset B_{R/\sqrt{\mhol}}(0)$ for some fixed $R$. Assume moreover that the cell diameter satisfies $h\asymp \mhol^{-1/2}$.}
Let $t\in\C^p$ satisfy $|t|\le c\,h$ (with \editamir{$h\asymp \mhol^{-1/2}$}).
Then {\color{black}for all sufficiently large $\mhol$ there exist sections} $\sigma_1,\dots,\sigma_p\in H^0(X,L^{\mhol})$ such that, writing
...
\]
{\color{black}with $\eta_{\mhol}\to 0$ as $\mhol\to\infty$.}
...
\[
\begin{aligned}
\sup_{B_{R/\sqrt{\mhol}}}\Bigl(|f_0-1|+\sqrt{\mhol}\,|\nabla(f_0-1)|\Bigr)&\le \varepsilon_{\mhol},\\
\sup_{B_{R/\sqrt{\mhol}}}\Bigl(|f_j-w_j|+\sqrt{\mhol}\,|\nabla(f_j-w_j)|\Bigr)&\le \varepsilon_{\mhol}\qquad (1\le j\le p).
\end{aligned}
\]
...
\]
{\color{black}Set $\eta_{\mhol}:=C\,\varepsilon_{\mhol}$. Since the ball has radius $R/\sqrt{\mhol}$, the above $C^1$ bounds imply}
\[
\sup_{B_{R/\sqrt{\mhol}}}|f_0-1|+\max_{1\le j\le p}\sup_{B_{R/\sqrt{\mhol}}}|f_j-w_j|
\le C\,\varepsilon_{\mhol}/\sqrt{\mhol}.
\]
{\color{black}Hence the affine error satisfies}
\[
\begin{aligned}
\|\partial_w F-I\|_{L^\infty(B_{R/\sqrt{\mhol}})}+\|\partial_u F\|_{L^\infty(B_{R/\sqrt{\mhol}})}&\le \eta_{\mhol},\\
\sup_{u:(u,t)\in B_{R/\sqrt{\mhol}}}|F(u,t)|&\le \eta_{\mhol}/\sqrt{\mhol}\le \eta_{\mhol}\,h.
\end{aligned}
\]
{\color{black}since $h=\mathrm{diam}(Q)\asymp \mhol^{-1/2}$.}
...
\editamir{This proves the ``$\Rightarrow$'' direction in \textup{(G1)}. The ``$\Leftarrow$'' direction follows because each facet}
...
\]
with an error controlled by {\color{black}$(\delta+\varepsilon+\mathrm{mesh})\cdot m$} (with $m$ fixed).
...
\[
\mathcal F\!\left(\partial T^{\mathrm{raw}}\right)\ \le\ {\color{black}\varepsilon_{\mathrm{glue}}(\delta,\varepsilon,\mathrm{mesh})}\cdot m,
\qquad
{\color{black}\varepsilon_{\mathrm{glue}}\xrightarrow[\delta,\varepsilon\to0,\ \mathrm{mesh}\to0]{}0}.
\]
\end{lemma}
...
{\color{black}$\varepsilon_{\mathrm{glue}}(\delta,\varepsilon,\mathrm{mesh})\to 0$ as $\delta,\varepsilon\to 0$ and $\mathrm{mesh}\to 0$} (with $m$ fixed).
\end{proof}

\begin{remark}[note: this is the quantitative bottleneck]\label{rem:lean-bottleneck-flatnorm}
...
\noindent\emph{(See \cite{FF60,Fed69}.)}
By the Euclidean isoperimetric (filling) inequality for integral currents (Federer--Fleming \cite{FF60}; see also Federer \cite{Fed69}),
...
Federer--Fleming isoperimetric inequality.  They are \emph{not} \editamir{not required} to be $\psi$--calibrated, nor to have any positivity/type property.
...
\editamir{Assume the local-sheet construction of Theorem~\ref{thm:local-sheets} producing a $\psi$--calibrated sheet current $S$, and let $U_\varepsilon$ be the gluing correction current constructed in Proposition~\ref{prop:glue-gap} so that $T_\varepsilon:=S-U_\varepsilon$ is a closed integral current.  Assume Proposition~\ref{prop:cohomology-match} so that $[T_\varepsilon]=\mathrm{PD}(m[\gamma])$ in $H_{2n-2p}(X;\R)$ (hence in $H_{2n-2p}(X;\Q)$ modulo torsion), and Proposition~\ref{prop:almost-calibration} so that $0\le \Mass(T_\varepsilon)-\int_{T_\varepsilon}\psi\le 2\,\Mass(U_\varepsilon)\to 0$.}
...
\medskip\noindent
\noindent\emph{(See \cite{Allard72,Sim83}.)}
\editamir{\textbf{Substep 6.2: Compactness (currents and varifolds) \cite{Allard72,Sim83}.}}

\noindent\emph{(See \cite[\S\S 4.1--4.2]{Fed69}.)}
By Federer--Fleming compactness for integral currents on the compact manifold $X$, using $\sup_k\Mass(T_k)<\infty$ and $\partial T_k=0$, after passing to a subsequence we obtain $T_k\to T$ in the flat norm. Let $V_k$ be the integral varifold associated to $T_k$. Since $\sup_k\Mass(T_k)<\infty$, the varifold measures have uniformly bounded mass, hence by weak-$\ast$ compactness of Radon measures (cf.\ \cite{Allard72,Sim83}) after passing to a further subsequence we obtain $V_k\to V$ as varifolds.
...
\medskip\noindent
\textbf{Substep 6.3: Optional varifold viewpoint (not used for the Hodge conclusion).}
One may package the vanishing calibration deficit $\Def_{\mathrm{cal}}(T_k)\to 0$ as a statement about the associated integral varifolds $V_k$: namely,
\[
\Def_{\mathrm{cal}}(T_k)=\int_{X\times\Gr_{n-p}(TX)}\bigl(1-\psi(P)\bigr)\,dV_k(x,P)\to 0.
\]
In particular, any varifold subsequential limit is supported on the $\psi$--calibrated Grassmannian. We record this only as intuition; the calibration of the current limit will be proved directly in Substep~6.4.
...
\noindent\emph{(See \cite{HL82} \cite{King71}.)}
Therefore $T$ is a positive holomorphic $(n-p)$--cycle.

If $X$ is projective, then each analytic component is algebraic by Chow/GAGA; see \cite{Serre56,GH78,Hartshorne77}. Hence $[\gamma]$ is represented by an algebraic cycle with rational coefficients.
\end{proof}
...
built as a finite sum of local $\psi$--cal{\color{black}\,(invoked with fixed clearing integer $m$, then choose $(h,s)$, then choose $\mhol$ so that Lemma~\ref{lem:bergman-control} applies on the needed $B_{c\mhol^{-1/2}}$ and the graph displacement is $\ll s$).}ibrated sheets (so $\Mass(S_\epsilon)=\langle S_\epsilon,\psi\rangle$ on each cell),
...
{\color{black}(Here $m$ is fixed first; the auxiliary parameters $\delta,h,s,\mhol$ are chosen \emph{after} a target error $\varepsilon>0$ and may vary with $\varepsilon$.)}
...
Applying Theorem~\ref{thm:syr} concludes that $[\gamma]$ is represented by a holomorphic chain and, since $X$ is projective (Chow/GAGA (see \cite{Hartshorne77,GH78,Serre56})),
is algebraic.
\end{proof}
...
Since $X$ is projective, each $V_j$ is algebraic by Chow/GAGA (see \cite{Hartshorne77,GH78,Serre56}), so $m[\gamma^+]$ is algebraic and therefore $\gamma^+$ is algebraic.
...
\noindent\emph{(See \cite{Hartshorne77,GH78,Serre56}.)}
}
\end{remark}
...
{\color{black}(Invoked after fixing $m$ and choosing $\mhol$ so that the SYR construction is closed and the integral filling is at scale $\ll 1$; no new ``choose small'' parameters are introduced here.)}
...
\bibitem{BBS08}
R.~Berman, B.~Berndtsson, and J.~Sjöstrand,
\emph{A direct approach to Bergman kernel asymptotics for positive line bundles},
Arkiv för Matematik \textbf{46} (2008), 197--217.

\bibitem{BaranyGrinberg81}
I.~Bárány and V.~S.~Grinberg,
