...
\title{\textbf{Neutrino Masses and the Deep $\phig$-Ladder}\\[0.25em]
\large Fractional rungs, mass-squared splittings, and falsifiable structural ratios}
...
\begin{abstract}
Neutrino masses are tiny, but their mass splittings and ordering exhibit rigid structure.
This paper extends the single-anchor $\phig$-ladder framework to the deep (low-mass) end by placing neutrinos on \emph{fractional} rungs of the ladder. \HYP
The model yields a normal hierarchy with absolute masses at the level
\(m_1\approx 0.00354~\mathrm{eV}\), \(m_2\approx 0.00926~\mathrm{eV}\), \(m_3\approx 0.0499~\mathrm{eV}\), and hence \(\sum m_\nu \approx 0.0627~\mathrm{eV}\). \VAL

The key structural prediction is an exact $\phig$-power relation among squared masses implied by the deep rung spacing:
\((m_3^{\mathrm{pred}})^2/(m_2^{\mathrm{pred}})^2 = \phig^{7}\). \HYP
The individual splittings \(\Delta m^2_{21}\) and \(\Delta m^2_{31}\) fall within standard NuFIT windows at current precision (validation). \VAL
The predicted mass sum is consistent with present cosmological bounds without introducing per-species fitting knobs. \VAL

All eV-reported values are stated under an explicit, single-scalar calibration seam (a declared reporting convention), and the framework forbids
particle-by-particle tuning. \CERT
We conclude with falsifiers: if future oscillation analyses or absolute-mass probes rule out the $\phig^{7}$ ratio, the normal ordering implied by the deep ladder,
or the predicted mass scale, the framework is refuted. \VAL
\end{abstract}
...
Neutrino oscillations reveal precise structure in \emph{differences} of squared masses, but do not determine absolute masses.
This paper proposes that neutrinos live on the deep end of the same $\phig$-ladder used for charged sectors, with fractional rungs.
We state a conservative claim contract: no per-state fitting, explicit unit seams for eV reporting, and falsifiers that future experiments can settle.}}

\subsection{What oscillations measure (and what they do not)}
In the Standard Model extended to include neutrino masses, oscillation experiments primarily measure:
\begin{itemize}
  \item two independent mass-squared splittings (\(\Delta m^2_{21}\) and \(\Delta m^2_{31}\)), and
  \item mixing parameters (the PMNS angles and phases).
\end{itemize}
These observables establish that neutrinos are massive and that the three mass eigenstates are non-degenerate.
However, oscillations do \emph{not} fix the absolute mass scale: adding a common offset to all three masses leaves \(\Delta m^2\) values unchanged. \PROVED

Independent constraints on the absolute scale come from:
\begin{itemize}
  \item kinematic measurements (e.g.\ beta-decay endpoints, summarized by PDG \cite{PDG2024}), and
  \item cosmological bounds on the sum \(\sum m_\nu\) (survey-dependent; we use only the qualitative requirement that \(\sum m_\nu\) remain below current limits). \VAL
\end{itemize}

\subsection{Framework overview: the deep $\phig$-ladder}
Papers~1--2 develop a discrete-geometry program in which hierarchies are encoded by a $\phig$-ladder coordinate.
This paper extends that ladder into the deep regime where masses are far below the charged sectors. \HYP

The central modeling premise is that the neutrino rungs are \emph{fractional} (quarter-step) positions on the $\phig$-ladder. \HYP
Once those rungs are fixed, the model implies:
\begin{itemize}
  \item a normal ordering of the three neutrino masses, \HYP
  \item concrete values for \(\Delta m^2_{21}\) and \(\Delta m^2_{31}\), \HYP
  \item and a rigid structural relation between them (an exact $\phig$-power ratio). \PROVED
\end{itemize}

\subsection{Claim contract: no per-species fitting and explicit unit seams}
This paper follows the same contract as the earlier papers in the series:
\begin{itemize}
  \item \textbf{No per-eigenstate fitting.} The model does not permit choosing separate coefficients for \(\nu_1,\nu_2,\nu_3\).
  All structure is shared (ladder base \(\phig\), rung rule, and global constants). \PROVED
  \item \textbf{Explicit eV reporting seam.} Converting dimensionless ladder outputs to eV is a declared reporting convention (a single-scalar calibration seam),
  and is not claimed to be derived from the neutrino sector alone. \CERT
  \item \textbf{Validation labeling.} Agreement with NuFIT windows for \(\Delta m^2\) is labeled as validation and never used as an input. \VAL
\end{itemize}

\subsection{What this paper delivers (and what it does not)}
Deliverables:
\begin{itemize}
  \item a precise definition of the deep ladder and the fractional rung convention used for neutrinos, \HYP
  \item predicted absolute masses \(m_1,m_2,m_3\) under an explicit reporting seam, \HYP
  \item predicted splittings \(\Delta m^2_{21}\) and \(\Delta m^2_{31}\) and an exact ratio constraint, \PROVED
  \item cosmological and kinematic consistency checks stated only as validation, \VAL
  \item falsifiers that future oscillation and absolute-mass measurements can confirm or refute. \VAL
\end{itemize}
Non-deliverables:
we do not introduce a new global fit, we do not tune phases to match any particular dataset, and we do not claim that the structural ratio is insensitive
to future revisions of experimental systematics. \HYP

\subsection{Notation and classical correspondences}
Table~\ref{tab:classical_dict_p3} summarizes the key terms used in this paper
...
\begin{table}[h]
  ...
  \begin{tabular}{llll}
    ...
    \midrule
    Deep $\phig$-ladder & log-scale mass coordinate & Bridge & Extends Paper~1 ladder \\
    Fractional rungs & quarter-step positions & Novel & No classical analog \\
    $\kappa_{\mathrm{eV}}$ & eV calibration seam & Cert & Declared convention \\
    $\Delta m^2_{ij}$ & mass-squared splitting & Twin & Standard oscillation observable \\
    $\phig^7$ ratio & seam-free structural ratio & Bridge & Scale cancels \\
    Normal hierarchy & $m_1<m_2<m_3$ ordering & Twin & Standard NuFIT convention \\
    \bottomrule
  \end{tabular}
  \caption{Dictionary of framework terms and their classical correspondences.
    ...
    \emph{Bridge}: corresponds via an explicit mapping.
    \emph{Novel}: no direct classical analog.
    \emph{Cert}: declared calibration convention.}
  \label{tab:classical_dict_p3}
\end{table}
...
\section{Conclusions}
This paper presented a deep-ladder neutrino mass framework in which neutrino masses occupy fractional rungs of the $\phig$-ladder.
The goal was not to ``fit'' oscillation numbers, but to propose a small, auditable set of discrete inputs (a rung triple and a rung lattice)
whose consequences are sharply falsifiable. \HYP

\subsection{What is structural vs what is a seam vs what is hypothesized}
\paragraph{Structural (mathematics).}
Once a ladder base \(\phig>1\) is fixed, ladder coordinates turn ratios into differences, and seam parameters cancel from ratios (Secs.~2,4--5). \PROVED
In particular, the ratio of mass-squared splittings depends only on rung differences and \(\phig\), not on the eV reporting seam. \PROVED

\paragraph{Declared seam (units).}
Absolute masses in eV require an explicit reporting seam (Sec.~3), represented by a single global scalar \(\tau_0\) (or equivalently \(\kappa_{\mathrm{eV}}\)).
This seam is fixed framework-wide and is not a neutrino-sector tuning knob. \CERT

\paragraph{Model hypotheses (physics).}
The physical content of this paper is the deep rung lattice choice \(r\in \tfrac14\mathbb Z\) and the specific rung triple \((r_1,r_2,r_3)\) used for neutrinos (Sec.~2),
together with the deep-ladder mass law \(m_i^{\mathrm{pred}}=\kappa_{\mathrm{eV}}\phig^{r_i}\) (Sec.~3). \HYP
These hypotheses are not asserted as inevitable consequences of the Standard Model; they are proposed structural laws to be tested. \HYP

\subsection{What the validation indicates}
Under the declared seam and rung triple, the predicted \(\Delta m^2_{21}\) and \(\Delta m^2_{31}\) lie within standard NuFIT summary windows at current precision (Sec.~4). \VAL
The predicted mass sum \(\Sigma_\nu^{\mathrm{pred}}\approx 0.0627\,\mathrm{eV}\) is consistent with representative cosmological bounds (Sec.~7), with the caveat that such bounds are model-dependent. \VAL

\subsection{The core falsifiers}
The most robust tests are seam-free:
\begin{itemize}
  \item the splitting ratio \(R_\Delta=\Delta m^2_{31}/\Delta m^2_{21}\) (Eq.~\eqref{eq:Rdelta_pred}), \VAL
  \item the ordering prediction (normal ordering implied by rung ordering; Eq.~\eqref{eq:normal_ordering_pred}), \VAL
  \item and, once absolute-mass information becomes available, the squared-mass ratio implied by the deep rung gap (Eq.~\eqref{eq:nu_sq_mass_ratio_phi7}). \VAL
\end{itemize}
Additional falsifiers test the absolute scale, including future cosmological tightening and direct absolute-mass probes (Sec.~8). \VAL

\subsection{Next steps}
The remaining work for completing Paper~3 is to document the interval/rounding bounds used for the \(\phig\)-power evaluations in Appendix~A,
so that each quoted numerical window can be audited from a single place. \PROVED

\appendix

\section{Interval Bounds (for auditing)}
This appendix records the explicit numerical interval bounds used to produce the certified windows in the main text.
All bounds are stated as outward-rounded intervals (to avoid over-claiming precision) and are treated as certified numerical facts. \CERT

\subsection{Tight bounds for \texorpdfstring{$\phig$}{phi} and quarter steps}
We use the following tight bracket for the golden ratio:
\begin{equation}
  1.61803395 < \phig < 1.6180340. \CERT
  \label{eq:phi_tight_bounds}
\end{equation}
From this one may conservatively bound any fixed integer power \(\phig^n\) by monotonicity. \PROVED

For quarter-step reasoning, we also record a bound on \(\phig^{1/4}\):
\begin{equation}
  1.12783847 < \phig^{1/4} < 1.12783849. \CERT
  \label{eq:phi_quarter_bounds}
\end{equation}

\subsection{Certified bounds for the deep-ladder $\phig$-powers}
The neutrino rungs in Eq.~\eqref{eq:nu_rungs} require the following negative quarter-rung powers:
\begin{align}
  4.594\times 10^{-12} &< \phig^{-217/4} < 4.598\times 10^{-12}, \CERT \label{eq:phi_neg2174_bounds}\\
  8.515\times 10^{-13} &< \phig^{-231/4} < 8.538\times 10^{-13}. \CERT \label{eq:phi_neg2314_bounds}
\end{align}
We also use a coarse bound for \(\phig^{-2}\):
\begin{equation}
  0.3818 < \phig^{-2} < 0.382. \CERT
  \label{eq:phi_neg2_bounds}
\end{equation}
Combining Eqs.~\eqref{eq:phi_neg2314_bounds} and~\eqref{eq:phi_neg2_bounds} yields a conservative interval for \(\phig^{-239/4}=\phig^{-231/4}\phig^{-2}\):
\begin{equation}
  3.251\times 10^{-13} < \phig^{-239/4} < 3.262\times 10^{-13}. \CERT
  \label{eq:phi_neg2394_bounds}
\end{equation}

\subsection{Pinned eV seam used for the reported windows}
To evaluate absolute masses in eV, the paper uses the global seam constant \(\kappa_{\mathrm{eV}}\) introduced in Eq.~\eqref{eq:kappa_ev}. \CERT
For the numerical windows reported in Sec.~3, we pin:
\begin{equation}
  \kappa_{\mathrm{eV}}
  \;:=\;
  \left(2^{-22}\phig^{51}\right)\times 10^{6}\ \mathrm{eV}
  \;\approx\;
  1.0856997758\times 10^{10}\ \mathrm{eV}.
  \CERT
  \label{eq:kappa_ev_pin}
\end{equation}
Equivalently, this corresponds to the single-scalar tick seam
\(\tau_0=\hbar/(\kappa_{\mathrm{eV}}\cdot 1\,\mathrm{eV})\approx 6.0626\times 10^{-26}\,\mathrm{s}\). \CERT

\subsection{Derived certified windows used in the main text}
Combining Eq.~\eqref{eq:kappa_ev_pin} with Eqs.~\eqref{eq:phi_neg2174_bounds}--\eqref{eq:phi_neg2394_bounds} yields the mass windows:
\begin{align}
  0.0035296 &< m_1^{\mathrm{pred}} < 0.0035411\ \mathrm{eV}, \CERT \\
  0.0092447 &< m_2^{\mathrm{pred}} < 0.0092698\ \mathrm{eV}, \CERT \\
  0.0498770 &< m_3^{\mathrm{pred}} < 0.0499206\ \mathrm{eV}. \CERT
\end{align}
From these, conservative propagation gives splitting windows:
\begin{align}
  7.2926\times 10^{-5}
  &< \Delta m^2_{21}{}^{\mathrm{pred}} < 7.3469\times 10^{-5}\ \mathrm{eV}^2, \CERT \\
  2.47518\times 10^{-3}
  &< \Delta m^2_{31}{}^{\mathrm{pred}} < 2.47960\times 10^{-3}\ \mathrm{eV}^2. \CERT
\end{align}

\subsection{Seam-free ratios}
Using the rung differences implied by Eq.~\eqref{eq:nu_rungs}, the splitting ratio has the seam-free closed form
of Eq.~\eqref{eq:dm2_ratio_closed_form}.
Bounding \(\phig\) by Eq.~\eqref{eq:phi_tight_bounds} yields
\begin{equation}
  33.823291 < \frac{\phig^{11}-1}{\phig^{4}-1} < 33.823298. \CERT
  \label{eq:dm2_ratio_interval}
\end{equation}

\paragraph{Reproducibility note.}
For a quick numerical cross-check (not a substitute for interval bounds), the repository includes
\texttt{scripts/neutrino\_mass\_scale\_validate.py}. \PROVED

\section{Reproducibility}
This appendix summarizes the minimal commands needed to (i) re-evaluate the headline numerical values quoted in the main text and Appendix~A, and (ii) rebuild the PDF.
All computations are straightforward evaluations of the closed-form expressions already stated in the paper; there is no per-eigenstate fitting workflow. \PROVED

\subsection{Quick numerical cross-check (floating point)}
From the repository root, run:
\begin{verbatim}
python3 scripts/neutrino_mass_scale_validate.py
\end{verbatim}
This script prints:
\begin{itemize}
  \item the implied absolute masses \(m_1,m_2,m_3\) under the declared eV reporting seam,
  \item the splittings \(\Delta m^2_{21}\), \(\Delta m^2_{31}\), and their ratio,
  \item and basic consistency checks against representative cosmology and direct-mass bounds.
\end{itemize}
This is a \emph{floating-point} check only; the outward-rounded certified intervals used in the paper are the explicit brackets recorded in Appendix~A. \CERT

\subsection{Compiling the PDF}
Compile Paper~3 from \texttt{papers/tex/}:
\begin{verbatim}
cd papers/tex
pdflatex -interaction=nonstopmode -output-directory=../pdf masses_paper3_neutrinos.tex
pdflatex -interaction=nonstopmode -output-directory=../pdf masses_paper3_neutrinos.tex
\end{verbatim}
The resulting PDF is written to \texttt{papers/pdf/masses\_paper3\_neutrinos.pdf}. \PROVED
