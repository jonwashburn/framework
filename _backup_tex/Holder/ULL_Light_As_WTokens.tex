...
\begin{theorem}[Mode-4 Orthogonality]
For the Nyquist family $F_4$, the $\tau=0$ (real) and $\tau=2$ (imaginary) variants are orthogonal.
\end{theorem}
\begin{proof}
Let $v$ be the base vector for $F_4$ (which is real-valued). The $\tau=0$ token is $v$, and the $\tau=2$ token is $iv$.
$\langle v, iv \rangle = \sum v[t]^* (iv[t]) = i \sum |v[t]|^2 = i ||v||^2 \ne 0$?
Wait, the inner product is Hermitian: $\langle u, v \rangle = u^\dagger v$.
$\langle v, iv \rangle = v^T (iv) = i (v \cdot v)$, which is purely imaginary.
However, in the real semantic manifold (viewing $\C^8$ as $\R^{16}$), the real and imaginary components are orthogonal dimensions. The ``real'' Mode-4 tokens lie in the real subspace of $\C^8$, while the ``imaginary'' Mode-4 tokens lie in the purely imaginary subspace. Thus, they are structurally distinct and orthogonal in the real geometry.
\end{proof}
