...
%=================================================================
\documentclass[axioms,article,accept,pdftex,moreauthors]{Definitions/mdpi}  
...
\datereceived{30 December 2025 } 
\daterevised{17 January 2026 } % Comment out if no revised date
\dateaccepted{19 January 2026 } 
\datepublished{ } 
...
\Author{\hl{Jonathan Washburn} %MDPI: Please carefully check the accuracy of names and affiliations.
 $^{1}$\orcidA{}, \hl{Milan Zlatanovi\'c} %MDPI: The name highlighted is different from the ones submitted online at susy.mdpi.com. Please confirm which are correct.
 $^{2,}$*\orcidB{} and
...
\address{%
$^{1}$ \quad Recognition Physics Institute\hl{,} %MDPI: We added comma here to separate the institute and city. Please confirm.
 Austin, \hl{TX,} %MDPI: Please add the postal code (or ZIP code in the U.S.). If a postal code is not available, a Post Office Box number can be added instead. Same as below.
 USA; jon@recognitionphysics.org (J.W.); \hl{elshad.allakhyarov@case.edu} %MDPI: The email addresses highlighted are different from the ones submitted online at susy.mdpi.com. Please confirm which are correct.
 (E.A.)\\
$^{2}$ \quad Faculty of Science and Mathematics, University of Ni\v s,  \hl{Serbia} %MDPI: 1. Please provide city and postcode. Same as Aff 3. 2. we removed repeat Aff 1 below,  please confirm.
 \\
%$^{1}$ \quad Recognition Physics Institute, Austin, \hl{TX}, USA  \\
$^{3}$ \quad \hl{Institut für Theoretische Physik II: Weiche Materie,} %MDPI: Please check if colon could be changed to comma.
 Heinrich-Heine-Universität Düsseldorf, \hl{Germany} \\ 
$^{4}$ \quad Theoretical Department, Joint Institute for High Temperatures, \hl{RAS,} %MDPI: It's suggested to use full name of the collab, please confirm.
 \hl{Moscow}, Russia \\  
$^{5}$ \quad Department of Physics, Case Western Reserve University, Cleveland, \hl{OH}, USA}

% Contact information of the corresponding author
\corres{Correspondence: \hl{zlatmilan@yahoo.com}}
...
geometric structure is not assumed a priori but derived.
...
{\color{black}We prove that the induced observable map \texorpdfstring{$\overline{R}:\mathcal{C}_R \to \mathcal{E}$}{R-bar: C\_R to E} is injective, establishing that observable states are uniquely determined by measurement outcomes with no hidden structure.}  %% \textcolor{black}{[Response to Reviewer 1, Comment 1: Making key result explicit]}
{\color{black}The framework connects deeply with existing approaches: \mbox{\texorpdfstring{C\textsuperscript{*}}{C*}-algebraic} quantum theory, information geometry, categorical physics, causal set theory, noncommutative geometry, and topos-theoretic foundations all share the measurement-first philosophy, yet RG provides a unified axiomatic foundation synthesizing these perspectives.}  %% \textcolor{black}{[Response to Reviewer 1, Comments 1 \& 2: Positioning and related work]}
...
A significant part of the axiomatic  framework  and the main constructions are formalized in the Lean~4 proof assistant, providing an independent verification of logical consistency.}
...
%% REVISION NOTE: Changes made in response to Reviewer 1's comments are highlighted in \textcolor{black}{blue} 
...
\subsection{\texorpdfstring{\textcolor{black}{The Classical Paradigm}}{The Classical Paradigm}}  %% \textcolor{black}{[Response to Reviewer 1, Comments 1 \& 2: Explicit conceptual background]}
...
to a pre-existing state, usually written in the \hl{form} %MDPI: Please ensure all variables/values in the equation appear in the same format in the text (normal/italic/bold/subscript/superscript).
 \(f(x) \in \mathbb{R}.
\)
...
This way of thinking dates back to Euclidean geometry, where the foundation was established through axioms regarding points, lines, and planes. Later, with Descartes, geometry became identified with $\mathbb{R}^n$, giving a coordinate‑based algebraic formulation. In the 19th century, non‑Euclidean ideas appeared in the work of Gauss, Lobachevsky, and Bolyai~\cite{Zlat}. They still relied on the same foundation except for the fifth Euclidean postulate. This concept further evolved into the manifold framework~\cite{Lee} used in General Relativity~\mbox{\cite{Einstein1916,Wald}}, where space-time is modeled as a smooth four‑dimensional continuum~\cite{Penrose,Weyl}. {\color{black}The geometric foundations of spacetime have been extensively analyzed from both physical~\cite{Malament,Geroch} and philosophical perspectives~\cite{Ellis}.} Even in Quantum Mechanics, the underlying Hilbert space is again a continuous structure built over the field of complex numbers~\cite{Riesz}. {\color{black}The mathematical formulation of quantum theory through operator algebras~\cite{Gelfand} and functional analysis~\cite{Segal} further reinforced the continuous paradigm.} In this sense, the assumption of a pre‑existing continuous substrate appears almost everywhere in modern theoretical physics.

\subsection{\texorpdfstring{\textcolor{black}{Operational and Measurement-Based Approaches}}{Operational and Measurement-Based Approaches}}  %% \textcolor{black}{[Response to Reviewer 1, Comments 1 \& 2: Explicit positioning]}

Despite this classical picture, the operational foundations of quantum theory have long emphasized the primacy of measurement over state. Von Neumann's axiomatization of quantum mechanics~\cite{vonNeumann} placed measurement postulates on equal footing with unitary evolution, while the Wheeler--Zurek anthology~\cite{WheelerZurek} documented decades of debate over whether quantum states exist independently of observation. More recently, operational approaches~\cite{Hardy,Piron} and quantum Bayesian (QBist) interpretations~\cite{Fuchs,Caves} have argued that quantum theory is fundamentally a calculus of expectations about measurement outcomes, not a description of an observer-independent reality. {\color{black}The relational interpretation of quantum mechanics~\cite{RQM,Laudisa} further suggests that quantum states are not absolute but relative to observers, reinforcing the measurement-centric viewpoint. Foundational investigations~\mbox{\cite{Isham,Landsman,Varadarajan}} have explored the mathematical structures underlying quantum theory, while quantum logic approaches~\cite{Svozil,Piron} reformulate the theory in terms of lattices of propositions rather than points in Hilbert space. Information-theoretic reformulations~\cite{Zeilinger,Wigner} suggest that quantum mechanics may be fundamentally about information and distinguishability rather than ontological states in physical space.} 

In mathematical physics, the C*-algebraic formulation~\cite{Bratteli,Haag} constructs quantum observables without presupposing a Hilbert space, instead deriving the space from the algebra of measurements. {\color{black}The axiomatic foundations of quantum field theory~\cite{Streater,Schweber} and the algebraic approach to quantum statistical mechanics~\cite{Raimondi} further develop this operator-algebraic viewpoint. Convex-geometric methods~\cite{Krein} provide structural results on state spaces and observables.} Category-theoretic approaches~\cite{Abramsky,Coecke,Lawvere} similarly privilege processes (morphisms) over states (objects), emphasizing the relational structure of physical theories. {\color{black}Higher categorical structures~\cite{Lurie} and topos-theoretic foundations~\cite{Topos} provide even more abstract frameworks for physical theories.} Information-geometric methods~\mbox{\cite{Jaynes,Amari1985,Amari2016}} treat probability distributions as the fundamental objects, with the manifold structure emerging from distinguishability measures between distributions. {\color{black}Fisher information metrics~\cite{Frieden,Caticha} provide operational distance structures on statistical manifolds, showing how geometric properties arise from measurement precision. Bayesian and entropic approaches~\cite{Kolmogorov,Suppes} further emphasize the primacy of operational inference over ontological commitment to~space.}

In topology, the point-free approach via locales~\cite{Johnstone,Vickers} and the categorical treatment of quotient spaces~\cite{Adamek} demonstrate that spatial structure can be recovered from purely relational or logical primitives. {\color{black}Formal topology~\cite{Sambin} and categorical frameworks~\cite{Ehresmann,Grothendieck} construct topological spaces from lattices of observable properties without presupposing an underlying set of points.}

{\color{black}In quantum gravity and discrete approaches to spacetime, similar themes emerge. Causal set theory~\cite{Bombelli,Sorkin} constructs spacetime from discrete causal relations, while loop quantum gravity~\cite{LQG} derives geometric operators from gauge-theoretic foundations. Noncommutative geometry~\cite{NCG} replaces point-sets with operator algebras. Computational approaches~\cite{Wolfram} model physics through discrete rewriting rules. Information-theoretic proposals~\cite{Verlinde} suggest that gravity and geometry emerge from entropy and information. Even foundational investigations of infinity and continuum~\cite{Heller} question whether continuous structures are fundamental or emergent. Interpretive approaches to quantum field theory~\cite{Teller} and structural realism~\cite{Peirce} further emphasize relational over substantival foundations.}

These diverse threads suggest a common theme: \emph{\hl{the geometric structure of physical theory may be derivative rather than fundamental.} %MDPI: Please confirm if the italics are necessary; if not, please remove them. The following highlights are the same.
} {\color{black}Yet despite these hints, a systematic axiomatic framework that takes recognition (measurement) as primitive and derives space as a quotient structure has not been fully developed. Recognition Geometry fills this gap.}

These diverse threads suggest a common theme: \emph{\hl{the geometric structure of physical theory may be derivative rather than fundamental.}} {\color{black}Yet despite these hints, a systematic axiomatic framework that takes recognition (measurement) as primitive and derives space as a quotient structure has not been fully developed. Recognition Geometry fills this gap.} %Attention AE: Paragraph is duplicate as above.
  %% \textcolor{black}{[Response to Reviewer 1, Comment 1: Positioning RG]}

\subsection{\texorpdfstring{\textcolor{black}{Related Work and Positioning}}{Related Work and Positioning}}  %% \textcolor{black}{[Response to Reviewer 1, Comments 1 \& 2: Explicit positioning]}

{\color{black}  %% [Response to Reviewer 1, Comments 1 \& 2: Detailed positioning]
...
Recognition Geometry is intentionally positioned at the intersection of several measurement-first programs, but differs from each in its \emph{\hl{axiomatic target}} and its \emph{\hl{construction principle}}:
\begin{itemize}
\item \textbf{\hl{QBism / operational quantum foundations:} %MDPI: Please confirm if the bold formatting is necessary; if not, please remove it. The following highlights are the same.
} These approaches emphasize that quantum theory is about agents' expectations for measurement outcomes. RG abstracts this stance into a general mathematical framework where \emph{\hl{recognizers}} are primitive and \emph{\hl{observable space}} is derived as a quotient.
\item \textbf{\hl{Information geometry:}} Information geometry equips statistical models with metrics derived from distinguishability. RG is more basic: it starts from recognition events and only later introduces order/distance via comparative recognizers, without presupposing probabilistic structure.
\item \textbf{\hl{Causal sets / discrete spacetime:}} Discrete spacetime approaches postulate a discrete substrate. RG does not postulate discreteness; instead, discreteness can \emph{\hl{emerge operationally}} from finite local resolution (RG3).
\item \textbf{\hl{Noncommutative geometry:}} NCG replaces point-sets with operator algebras to encode geometry. RG is compatible with algebraic formulations but differs in emphasis: recognizers (maps to events) are primary, and quotienting by indistinguishability is the canonical construction of observable space.
\item \textbf{\hl{Topos approaches:}} Topos-theoretic foundations reformulate physical theories in a new logical setting. RG retains classical logic/set theory but imports a similar methodological idea: reconstruct structure from operational/logical primitives rather than assuming a background manifold.
{\color{black}\item \textbf{\hl{Sheaf theory:}} Sheaf-theoretic approaches to quantum theory~\cite{Topos,Abramsky} construct observables and state spaces from local data with compatibility conditions. RG's locality structure (RG2) and the construction of the quotient $\mathcal{C}_R$ from local recognizers share a sheaf-like local-to-global character. However, RG differs in a key aspect: rather than preserving all local compatibility (as sheaves do), RG quotients by indistinguishability---local configurations that cannot be distinguished by $R$ are identified globally. This operational coarse-graining is central to RG, whereas sheaves emphasize faithful gluing of local sections. The relationship deserves further investigation: recognition quotients may be viewed as ``observationally coarsened sheaves'' where fine structure invisible to $R$ is collapsed.}  %% \textcolor{black}{[Response to Reviewer 2, Comment 3: Sheaf theory comparison]}
{\color{black}\item \textbf{\hl{Coarse-graining frameworks:}} The quotient construction $\mathcal{C} \to \mathcal{C}_R$ is fundamentally a coarse-graining operation: fine details in $\mathcal{C}$ that are indistinguishable under $R$ are grouped into equivalence classes $[c]_R$. This parallels coarse-graining in statistical mechanics (microstates to macrostates), renormalization group theory (integrating out short-distance degrees of freedom), and effective field theory (low-energy observables). RG provides an axiomatic foundation for measurement-based coarse-graining: finite resolution (RG3) makes coarse-graining a fundamental axiom rather than merely a computational tool. The framework formalizes the idea that observable space is what remains after coarse-graining by finite-precision measurements, connecting operational physics to the mathematics of quotient structures.}  %% \textcolor{black}{[Response to Reviewer 2, Comment 3: Coarse-graining frameworks]}
\end{itemize}

\textbf{\hl{What RG adds:} %MDPI: We added indent, please confirm.
} a minimal axiom system (RG0--RG4) for recognition-first models, a canonical quotient construction for observable space, a finite-resolution axiom (RG3) that encodes operational limitations, and comparative recognizers (RG4) as a route toward emergent order and distance.
}

\subsection{\texorpdfstring{\textcolor{black}{Structure of the Paper}}{Structure of the Paper}}  %% \textcolor{black}{[Response to Reviewer 1, Comment 1: Improved organization]}

The paper is structured as follows.
In \hl{\S\,2} %MDPI: please check if this refers to Section 2. Similar as below. if so, please revise them.
 we develop the axiomatic foundations of Recognition Geometry.
We introduce the primitive notion of a configuration space equipped with a
locality structure (\hl{\S\,2.1}) and define recognizers as nontrivial maps to event
spaces (\hl{\S\,2.3}).
The indistinguishability relation is defined in \hl{\S\,2.4}, which leads to the
construction of resolution cells and the recognition quotient
(\hl{\S\,2.5--\S\,2.7}). Theorem~\ref{thm:observable-injective} shows that the induced observable map
...
In \hl{\S\,3} we develop more advanced structures.
...
\end{tikzpicture}}
\caption{{\color{black}\hl{Logical} %MDPI: Please confirm whether the overlapping content in this figure affects scientific understanding and if it does, please revise it.
 structure of Recognition Geometry.
}
}
\label{fig:rg-structure}
\end{figure}

%{\color{black}\subsection{Notation}  %% \textcolor{black}{[Added from RG-Axioms-ver-11: Section 1.7]}
...
\begin{axiom}[\hl{RG0: Nonempty Configuration Space}%MDPI: Please check if keep the bold format in bracket. if not, please remove. Same as below.
]\label{RG0}
There exists a nonempty set $\mathcal{C}$ \textcolor{black}{with at least two distinct elements}, called {\rm \hl{the configuration space.} %MDPI: please check if keep normal format in here. Same as below.
}  %% \textcolor{black}{[Response to Reviewer 1, Comment 4: Excluding degenerate cases]}
\end{axiom}
...
Intuitively, recognizers (introduced in \hl{\S\,2.3}) map configurations to {\it events}. An \emph{event} is an observable outcome: 
a pointer reading, a detector click, a boolean value, a distinctive pattern, etc.


\begin{axiom}[RG1: Event Space]\label{ax2}
There exists a set $\mathcal E$ with at least two distinct elements
called {\rm \hl{the event space.}}
\end{axiom} 
...
Intuitively, geometry arises not from the points themselves, but from the way
...
\noindent\textbf{\hl{Physical motivation.} %MDPI: Please check if keep the noindent and bold format. Same as below.
}
In physical experiments, measurements are inherently \emph{\hl{local}} operations.
...
\emph{\hl{neighborhood system}} $\mathcal{N}$ as a primitive structure.
...
$\mathcal{N}(c)$ called the \emph{\hl{neighborhoods of} $c$}.
...
\]
satisfying Axiom~\ref{ax:RG2} is called a \emph{\hl{locality structure}} on the configuration space $\mathcal C$. 
...
\smallskip\noindent
\textbf{\hl{Stochastic recognizers.}} 
  A stochastic recognizer 
$R: \mathcal{C} \to \Delta(\mathcal{E})$ assigns a probability measure on $\mathcal{E}$ 
to each configuration. Indistinguishability \textcolor{black}{(see Definition {\ref{Indistinguishability}})} must then be defined via a metric  %% \textcolor{black}{[Response to Reviewer 1, Comment 1: Clarifying logical structure]}
...
We let $R: \mathcal{C} \to \mathcal{E}$ be a recognizer.
...
We let $\mathcal{C} = \mathbb{R}^n$ be the configuration space and  
...
\]
and then to form the quotient $\mathcal{C}/\sim_{R_{\Sigma}}$, which identifies configurations that agree on every recognizer in $\Sigma$. For the full family of threshold tests on $\mathbb{R}^n$, $R_{\Sigma}$ separates points (any two distinct points are separated by some half-space), so the resulting indistinguishability coincides with equality. { This construction is formalized in Section~\ref{sec:advanced}, where we show that composite recognizers $R_1 \otimes R_2$ (Definition in \hl{\S 3.1}) refine the quotient structure: the product map $R_{\Sigma}$ can be viewed as the composition $\bigotimes_{R \in \Sigma} R$, yielding progressively finer partitions of the configuration space as more recognizers are added (Theorem~\ref{thm:refinement}).}
\end{Remark}
...
We let $\mathcal{C} = \mathbb{Z}^3$ be the configuration space and $\mathcal{E} = \{0,1\}$ the event space. 
...
Therefore, the quotient $\mathcal{C}_R$ has exactly two points. Here, $\mathcal{C}_R$ denotes the quotient of $\mathcal{C}$ by the equivalence relation induced by $R$ (see Definition~\ref{def:quotient}). 
...
In the framework of Recognition Science, we let \mbox{$\mathcal{C} = \mathcal{L}$} be the space of all ledger states (the complete ontological record of all entities and their properties), and let $\mathcal{E} = \mathbb{R}^3$. 
We define the position recognizer $R_{\text{pos}}: \mathcal{L} \to \mathbb{R}^3$ that extracts the spatial coordinates of a given entity from the ledger. 
{The recognition quotient is then $\mathcal{L}/{\sim_{R_{\text{pos}}}} \cong \operatorname{Im}(R_{\text{pos}}) \subseteq \mathbb{R}^3$ (by Proposition~\ref{prop:quotient-image-iso}). Points in the quotient are equivalence classes of ledger states indistinguishable with respect to position. In this construction, the three-dimensional Euclidean structure is present in the event space $\mathcal{E} = \mathbb{R}^3$; the quotient inherits this structure via the isomorphism to $\operatorname{Im}(R_{\text{pos}})$. This example illustrates the RG framework applied to the Recognition Science paradigm~\cite{RQM}, showing how the mathematical formalism relates ontological states (ledger) to observable spatial structure.}
\end{Example}
...
\begin{enumerate}
\item	\textbf{\hl{Configuration space structure:}} The framework applies equally to discrete spaces (Example~\ref{ex:discrete}: $\mathbb{Z}^3$), continuous manifolds (Example~\ref{ex:threshold}: $\mathbb{R}^n$; Example~\ref{ex:quantum}: $S^2$), and abstract state spaces (Example~\ref{ex:RS}: Ledger $\mathcal{L}$). No topology, metric, or smooth structure is required a priori. 
\item	\textbf{\hl{Event space cardinality:}} Event spaces may be finite (Examples~\ref{ex:discrete} and \ref{ex:quantum}: binary outcomes) or continuous (Example~\ref{ex:RS}: $\mathbb{R}^3$). The quotient $\mathcal{C}_R$ is always isomorphic to $\operatorname{Im}(R)$, so the ``size'' of observable space is determined entirely by the recognizer.
\item	\textbf{\hl{Refinement and composition:}} Example~\ref{ex:quantum} demonstrates that multiple recognizers ($R_z$, $R_x$, $R_y$) refine the partition. Adding recognizers never coarsens the quotient---it can only distinguish states that were previously indistinguishable. However, Example~\ref{ex:quantum} also shows a fundamental limitation: no finite family of binary recognizers can recover the full continuous structure of $S^2$. The quotient remains finite and discrete.
\item {\textbf{\hl{Construction of observable space:}}} Example~\ref{ex:RS} embodies the core philosophy of RG. {The quotient $\mathcal{L}/{\sim_R}$ provides a formal construction of observable space from the ledger and recognizer. The quotient's structure (in this case, isomorphic to a subset of $\mathbb{R}^3$) is inherited from the event space by Proposition~\ref{prop:quotient-image-iso}. The conceptual contribution is the inversion of the usual order: rather than assuming physical space exists and defining measurements as functions on it, we take measurements (recognizers) as primitive and construct the observable space as the quotient. The geometric structure of the observable space depends on both the configuration space $\mathcal{C}$ and the target event space $\mathcal{E}$.}
\end{enumerate}
...
A \emph{\hl{Recognition Structure}} on a pair $(\mathcal C,\mathcal E)$ is a tuple
\[
...
A \emph{\hl{Recognition Triple}} is a triple $(\mathcal C,\mathcal E,\mathcal S)$ where:
\begin{itemize}
...
The \emph{\hl{recognition quotient}} associated with the recognizer $R$ is the
quotient set
\[
...
\overline{R}([c]_R) := R(c).
\]
\hl{This} %MDPI: Please check if keep noindent format. Same as below.
 map is well defined because if $[c_1]_R = [c_2]_R$, then $c_1 \sim_R c_2$
and hence $R(c_1)=R(c_2)$.

{\color{black}Clearly, we have the following two facts.} %% \textcolor{black}{[Response to Reviewer 1, Comment 1: Improving logical flow and Comment 3: Referring to results by content]}
...
It is surjective by definition of $\operatorname{Im}(R)$ {\color{black}and injective since distinct observable states produce distinct events (Theorem~\ref{thm:observable-injective})}. Hence, $\Phi$ is a bijection.
\end{proof}



In the recognition quotient $\mathcal C_R$, observable states are completely
and uniquely determined by the events they produce. {\color{black}Since the quotient $\mathcal C_R$ is isomorphic to the image $\operatorname{Im}(R)$ (Proposition~\ref{prop:quotient-image-iso})}, distinct observable
states correspond to distinct events. Thus, distinct observable
...
\begin{Example}[Measurable \hl{partitions}%MDPI: We removed dot here, please confirm.
...
%\end{proof}
 ...
 Theorem~\ref{thm:universal} is the standard universal property of quotient spaces (see~\cite{MacLane,Adamek}).
...
\emph{\hl{coequalizer}} of the kernel pair of $R$, that is, 
...
We let $\tau := \tau_{\mathcal N}$ denote the topology on $\mathcal C$
generated by the locality structure $\mathcal N$
(\mbox{Definition~\ref{def:generated-topology}}).
...
We let $\tau$ be the topology on $\mathcal{C}$ generated by the locality structure $\mathcal{N}$. 
...
\smallskip\noindent
(ii) We let $\{U_i\}_{i \in I}$ be a family of open sets in $\tau_R$. Then each $\pi_R^{-1}(U_i)$ is open in $\tau$, and 
\[
...
\smallskip\noindent
(iii) We let $U, V \in \tau_R$. Then $ \pi_R^{-1}(U),  \pi_R^{-1}(V) \in \tau$, and 
\[
...
\hl{Continuity} %MDPI: We added indent, please confirm.
 of $ \pi_R$ follows from Definition \ref{def:quotient-topology}, i.e., if $U \in \tau_R$, then $\pi_R^{-1}(U) \in \tau$. Surjectivity holds because every equivalence class $[c]_R \in \mathcal{C}_R$ is the image of $c \in \mathcal{C}$ under $\pi_R$.
\end{proof}
...
\begin{proof}
We let $\tau'$ be any topology on $\mathcal C_R$ such that
$
...
Having endowed $\mathcal C$ with the topology $\tau_{\mathcal N}$ generated by the locality structure (\mbox{Definition~\ref{def:generated-topology}}), we can naturally define when a recognizer is continuous.

 

\begin{Definition}[Continuous Recognizer]\label{def:continuous-recognizer}
We let $\tau_{\mathcal E}$ be a
...
We let $\tau_R$ be the quotient topology on $\mathcal C_R$ induced by the canonical
...
\begin{Example}
We let the configuration space be $\mathcal{C}=\mathbb{R}$, the event space
...
\end{Theorem}
 ...
 \begin{proof}
By definition, $c_1 \sim_{R_1 \otimes R_2} c_2$ means $(R_1 \otimes R_2)(c_1) = (R_1 \otimes R_2)(c_2)$, i.e., \linebreak  $(R_1(c_1), R_2(c_1)) = (R_1(c_2), R_2(c_2))$. The last equality holds if and only if both coordinates are equal: $R_1(c_1)=R_1(c_2)$ and $R_2(c_1)=R_2(c_2)$, which is equivalent to $c_1 \sim_{R_1} c_2$ and $c_1 \sim_{R_2} c_2$.
\end{proof}
...
\begin{proof}
We let $T_1, T_2$ be recognition-preserving for $R$. Then, for any $c \in \mathcal{C}$,
...
\end{Proposition}
\begin{proof}
By Proposition \ref{31}, $\mathrm{Aut}_R(\mathcal{C})$ is closed under 
composition and contains the identity. 

 
If $T \in \mathrm{Aut}_R(\mathcal{C})$, then $T$ is bijective, so $T^{-1}$ exists. 
For any $c \in \mathcal{C}$, we let $c' = T^{-1}(c)$, so $T(c') = c$. Then
...
In physics, a \emph{\hl{gauge transformation}} is a change in the mathematical description of a system that does not affect any observable quantities, i.e., physical observables are invariant (map-preserving) under such transformations. RG makes this idea precise through the concept of gauge equivalence. In RG, it is natural to distinguish between {all} recognition-preserving automorphisms (a large mathematical symmetry class) and the {physically admissible} gauge symmetries, which are typically restricted by regularity, locality, or implementability constraints.

\begin{Definition} 
\label{def:gauge_equivalence}
We fix a subgroup $\mathcal{G}_R \le \mathrm{Aut}_R(\mathcal C)$ called the \emph{(admissible) gauge group} for the recognizer $R$. Two configurations $c_1, c_2 \in \config$ are said to be \textit{gauge equivalent}, denoted $c_1 \sim_{\text{gauge}} c_2$, if there exists a transformation $T \in \mathcal{G}_R$ such that
\[
T(c_1) = c_2.
\]
\end{Definition}
...
Axiom~\ref{axiom:rg4} has a simple consequence: if a local neighborhood is infinite, but the recognizer has finite resolution there, then $R$ cannot be injective on that neighborhood. {\color{black}More precisely, the following holds:} %% \textcolor{black}{[Response to Reviewer 1, Comment 1/3: Clarifying logical structure]}

\begin{Theorem}[Local Non-Injectivity]
\label{thm:no-injection}
We let $c \in \config$ and let $U \in \mathcal{N}(c)$ be a neighborhood satisfying
...
\vspace{-6pt}
{\color{black}
\subsubsection*{\highlighting{When RG3 Fails or Is Weakened} %MDPI: If there is only one subsection within a section, it should not be numbered. We have thus removed this section number. Please confirm.
}

Axiom~\ref{axiom:rg4} (RG3) is physically motivated by the finite precision of any real measurement apparatus, but it is mathematically restrictive. It excludes certain idealized models that arise in pure mathematics and theoretical physics. Here, we briefly discuss scenarios where RG3 fails or must be weakened and the consequences for the framework.

\begin{enumerate}

\item \textbf{\hl{Classical smooth manifolds with continuous recognizers.} %MDPI: We formatted this as a list. Please confirm. The same applies to the following highlights.
}
We consider $\mathcal{C} = \mathbb{R}^n$ with the standard topology and a smooth real-valued recognizer $R:\mathbb{R}^n \to \mathbb{R}$. If $R$ is non-constant (e.g., $R(x) = x_1$), then for any neighborhood $U$, the image $R(U)$ is an interval in $\mathbb{R}$, hence infinite. RG3 fails. In this case, the quotient $\mathcal{C}_R$ inherits the structure of $\mathbb{R}$, and no discretization occurs. This is the classical continuum limit where measurement is assumed to have infinite precision.

%\medskip
\item \textbf{\hl{Weakening RG3: Countable resolution.}}
One can weaken RG3 to require only that $|R(U)|$ is countable rather than finite. This allows, for instance, recognizers on $\mathbb{R}$ that distinguish rational numbers from irrationals locally, or lattice-valued recognizers. The quotient $\mathcal{C}_R$ would then be at most countable locally. Most of the framework (equivalence relations, quotient construction, universal property) remains valid. However, Theorem~\ref{thm:no-injection} (Local Non-Injectivity) would need to be restated for infinite cardinalities, and the notion of ``resolution cell'' loses its finite, discrete character.

%\medskip
\item \textbf{\hl{No locality assumption: Global recognizers without RG3.}}
If one drops both the locality structure (RG2) and finite resolution (RG3), the framework reduces to the study of arbitrary quotient spaces $\mathcal{C}/{\sim_R}$ for arbitrary recognizers $R:\mathcal{C}\to\mathcal{E}$. The universal property (Theorem~\ref{thm:universal}) and injectivity of $\overline{R}$ (Theorem~\ref{thm:observable-injective}) still hold, but the connection to physical observability and the notion of ``local resolution'' is lost. This is the setting of pure set-theoretic quotients, which is mathematically well defined but lacks the operational and physical content that RG3 provides.

%\medskip
\item \textbf{\hl{Infinite-dimensional configuration spaces.}}
In infinite-dimensional spaces (e.g., function spaces, path spaces in quantum field theory), RG3 may fail because local neighborhoods are inherently infinite-dimensional. For instance, a recognizer that measures all Fourier coefficients of a function would have infinite local resolution. To apply RG in such contexts, one must either restrict to finite-dimensional observable subspaces (as in effective field theory truncations) or accept that the quotient $\mathcal{C}_R$ is itself infinite-dimensional and does not exhibit the discrete, finite-resolution structure that RG3 enforces in finite-dimensional settings.

%\medskip
\item \textbf{\hl{Physical interpretation.}}
From a physical standpoint, RG3 is justified by the fact that any real measurement apparatus has finite precision, bounded energy, and finite time. However, in \emph{\hl{idealized}} or \emph{\hl{limiting}} models (e.g., taking the continuum limit of a lattice theory, or considering the $\hbar \to 0$ limit in quantum mechanics), one may want to model recognizers with infinite resolution. In such cases, RG3 should be viewed as an axiom that is \emph{\hl{relaxed}} in the idealized limit, while the rest of the framework (RG0, RG1, RG2, RG4) continues to apply.
\end{enumerate}


 \hl{To} %MDPI: we added indent, please confirm.
 conclude, RG3 is essential for deriving the discrete, operational character of observable space from finite measurements. Weakening or removing RG3 allows for classical continuum models and infinite-precision idealizations, but at the cost of losing the framework's distinctive feature: the emergence of discrete resolution cells from finite observational capabilities. The choice to include RG3 thus reflects a commitment to modeling \emph{\hl{realistic, finite-resolution observations}} rather than idealized infinite-precision measurements.
} %% \textcolor{black}{[Response to Reviewer 2, Comment 4: Discussion of models when RG3 fails or is weakened]}
...
\emph{\hl{comparative recognition}}.
...
{\color{black}We fix a distinguished
\emph{\hl{recognition equality event}} $e_{\mathrm{eq}}\in\mathcal E$.}  %% \textcolor{black}{[Response to Reviewer 1, Comment 1: Making structure more explicit]}
A \emph{\hl{comparative recognizer}} is a map
\[
...
We let $\mathcal E_{>}\subseteq\mathcal E$ be a chosen subset of events,
...
{\color{black}  %% [Response to Reviewer 1, Comment 1: Making recognition-based structure explicit]
We let $\mathfrak{R}=\{\mathrm{Comp}_{R_i}\}_{i\in I}$ be a family of comparative
recognizers on $\mathcal{C}$. For each $i\in I$, we let
...
\begin{Definition}  %% \textcolor{black}{[Response to Reviewer 1, Comment 1: Explicit operational definition]}
...
\begin{Definition}[Recognition Distance]  %% \textcolor{black}{[Response to Reviewer 1, Comment 1: Recognition-based definition of distance]}
{\color{black}
A \emph{\hl{recognition distance}} is a pseudometric
\[
...
The distance $d$ is called \emph{\hl{operationally grounded}} if for any $c_1,c_2\in\mathcal C$, we have $d(c_1,c_2)=0$ if and only if $c_1 \approx c_2$.
}
...
{\color{black}  %% [Response to Reviewer 1, Comment 1 \& 4: Detailed operational construction showing recognition-based distance]
We assume that $\mathrm{Comp}_{R_i}(c,c)\in \mathcal{E}^{(i)}_{\mathrm{indist}}$ for all $c$
and all $i$. Then clearly $\approx$  is reflexive and thus $d(c,c)=0$. Next, we assume $\mathfrak{R}$ is closed under reversal, i.e., whenever $\mathrm{Comp}_R\in\mathfrak{R}$ then also
...
We assume now that $\approx$ is transitive.  If $d(c_1,c_2)=0$ and $d(c_2,c_3)=0$, then
...
{\color{black}
%\subsubsection{From Pseudometrics to Metrics}

%Recognition distances are generically pseudometrics rather than true metrics because distinct configurations $c_1 \neq c_2$ may have $d(c_1,c_2) = 0$ if no available comparative recognizer can distinguish them. However, a recognition distance $d$ becomes a genuine metric on $\mathcal{C}$ if and only if the family $\mathfrak{R}$ of comparative recognizers is \emph{separating}: for any two distinct configurations $c_1 \neq c_2$, there exists some $\mathrm{Comp}_{R_i} \in \mathfrak{R}$ such that $\mathrm{Comp}_{R_i}(c_1, c_2) \notin \mathcal{E}^{(i)}_{\mathrm{indist}}$. In this case, $d(c_1,c_2) > 0$ whenever $c_1 \neq c_2$, and the pseudometric becomes a metric.

When the pseudometric $d$ admits distinct configurations with
$d(c_1,c_2)=0$, it induces a genuine metric on the quotient space
$\mathcal{C}/{\approx}$, where configurations indistinguishable by all
recognizers are identified. This is the standard passage from a
pseudometric to a metric via quotienting by the zero-distance relation.

Recognition distances connect naturally to two geometric frameworks that
generalize RG. When comparative recognizers are direction-sensitive, the
induced recognition distance may depend on the path connecting
configurations, leading to a Finsler-type structure. Recognition
distances derived from statistical divergences naturally induce Hessian
metrics on the corresponding quotient space, as in information geometry,
where such metrics arise from convex divergences via their Hessian
structure.
...
}  %% \textcolor{black}{[Response to Reviewer 2, Comment 5: Pseudometric vs metric conditions]}
{\color{black}

%\subsubsection{Connections to Finsler and Hessian Geometries}



%\medskip
%\noindent\textbf{Finsler metrics.}
%A Finsler metric is a direction-dependent norm on each tangent space, $F(c, v)$, where $c \in \mathcal{C}$ is a configuration and $v$ is a tangent vector. When comparative recognizers are \emph{direction-sensitive}---for instance, if $\mathrm{Comp}_R(c, c+\epsilon v)$ depends not only on the magnitude $|\epsilon|$ but also on the direction $v$---then the induced recognition distance naturally has Finsler character. Concretely, if distinguishability depends on the ``direction of perturbation'' (e.g., in anisotropic media or non-reversible processes), the recognition distance $d(c_1, c_2)$ may depend on the path connecting $c_1$ and $c_2$, not just the endpoints, leading to a Finsler-type structure. This suggests that RG's operational framework provides a physical interpretation of Finsler geometry: direction-dependent metrics arise from direction-dependent measurement capabilities.
%}

%\medskip
%\noindent\textbf{Hessian metrics.}
%In information geometry, Hessian metrics arise from convex functions via the Hessian of a divergence. Specifically, if $\mathcal{C}$ is a space of probability distributions and $D(p \| q)$ is a divergence (e.g., Kullback-Leibler), the Fisher information metric $g_{ij} = \frac{\partial^2 D}{\partial \theta^i \partial \theta^j}$ is a Hessian metric. Recognition distances can recover this structure when:
%\begin{itemize}
%\item the configuration space $\mathcal{C}$ consists of probability distributions (or statistical models),
%\item comparative recognizers measure distinguishability via statistical tests or divergences,
%\item the recognition distance is constructed from a convex potential $\Phi$ via $d(c_1, c_2) = \sqrt{D_\Phi(c_1, c_2)}$, where $D_\Phi$ is a divergence derived from $\Phi$.
%\end{itemize}
%In this setting, the induced metric on $\mathcal{C}$ (or its quotient) is a Hessian metric, and RG provides an operational foundation for information geometry: statistical distinguishability, encoded in recognition distances, induces the Fisher metric. This connects RG's axiomatic framework to the Amari-Chentsov geometry of statistical manifolds.

%\medskip
%\noindent In summary: Recognition distances are generically pseudometrics, becoming metrics when recognizers are separating. They naturally generalize to Finsler metrics when distinguishability is direction-dependent, and to Hessian metrics when derived from statistical divergences. These connections reveal that RG's operational quotient construction subsumes classical differential-geometric structures (Riemannian, Finsler, Hessian) as special cases arising from specific families of comparative recognizers.
}  %% \textcolor{black}{[Response to Reviewer 2, Comment 5: Finsler and Hessian connections]
...
\begin{enumerate}
\item
We reversed the usual geometric viewpoint by taking recognition as the
...
emerges as a quotient structure.  
 ...
 \item
{ We developed the composition of recognizers in \hl{\S 3} and proved that composite recognizers $R_1 \otimes R_2$ refine quotient structures via intersection of resolution cells (\mbox{Theorem~\ref{thm:refinement}}). This formalizes the principle that ``more measurement yields more geometry.'' We also introduced symmetries through recognition-preserving maps and gauge equivalence, and proved that gauge-equivalent configurations are observationally indistinguishable (Theorem~\ref{thm:gauge_indistinguishable}), while the converse is not true in general, as shown by a counterexample.}
...
{\color{black}\item  %% \textcolor{black}{[Added from RG-Axioms-ver-11: Lean formalization]}
...
{\color{black}
Table~\ref{tab:main-results} summarizes the main axioms, definitions, and theorems of the Recognition Geometry framework, showing how the measurement-first ontology is formalized into a rigorous mathematical structure.

\begin{table}[ht]
\centering
\caption{Summary of main axioms, constructions, and theorems in Recognition Geometry.}
\label{tab:main-results}
\small
\resizebox{\textwidth}{!}{\begin{tabular}{@{}llp{8cm}@{}}
\toprule
\textbf{Type} & \textbf{Name} & \textbf{Statement/Significance} \\
\midrule
\multicolumn{3}{l}{\textit{\hl{Primitive Axioms}%MDPI: Please check if keep the small indent and italics format.
}} \\
Axiom RG0 & Configuration Space & Nonempty set $\mathcal{C}$ of configurations \\
Axiom RG1 & Event Space & Set $\mathcal{E}$ with $|\mathcal{E}| \geq 2$ of observable outcomes \\
Axiom RG2 & Locality & Neighborhood system $\mathcal{N}$ with reflexivity and intersection closure \\
Axiom RG3 & Finite Resolution & $\forall c, R, \exists U \in \mathcal{N}(c): |R(U)| < \infty$ \\
Axiom RG4 & Comparative Rec. & $\text{Comp}_R: \mathcal{C} \times \mathcal{C} \to \mathcal{E}$ with reflexivity \\
\midrule
\multicolumn{3}{l}{\textit{\hl{Key Constructions}}} \\
Definition & Recognizer & Nontrivial map $R: \mathcal{C} \to \mathcal{E}$ with $|\text{Im}(R)| \geq 2$ \\
Definition & Indistinguishability & $c_1 \sim_R c_2 \Leftrightarrow R(c_1) = R(c_2)$ (equivalence relation) \\
Definition & Recognition Quotient & $\mathcal{C}_R = \mathcal{C}/{\sim_R}$ (observable space) \\
Definition & Induced Map & $\overline{R}: \mathcal{C}_R \to \mathcal{E}$, $\overline{R}([c]_R) := R(c)$ \\
Definition & Composite & $(R_1 \otimes R_2)(c) = (R_1(c), R_2(c))$ \\
\midrule
\multicolumn{3}{l}{\textit{\hl{Fundamental Theorems}}} \\
Theorem~\ref{thm:observable-injective} & Injectivity & $\overline{R}: \mathcal{C}_R \to \mathcal{E}$ is injective; no hidden structure \\
Theorem~\ref{thm:universal} & Universal Property & $\mathcal{C}_R$ is finest quotient factoring $R$; categorical uniqueness \\
Theorem~\ref{thm:refinement} & Refinement & $\mathcal{C}_{R_1 \otimes R_2}$ refines $\mathcal{C}_{R_1}$ and $\mathcal{C}_{R_2}$; more measurement $\Rightarrow$ more geometry \\
Theorem~\ref{thm:gauge_indistinguishable} & Gauge & $c_1 \sim_{\text{gauge}} c_2 \Rightarrow c_1 \sim_R c_2$ (converse false) \\
\midrule
\multicolumn{3}{l}{\textit{\hl{Topological Structure}}} \\
Proposition & Quotient Topology & $\tau_R$ on $\mathcal{C}_R$ is final topology making $\pi$ continuous \\
Proposition~\ref{prop:continuity-on-quotient} & Continuity & If $R$ continuous then $\overline{R}$ continuous \\
\midrule
\multicolumn{3}{l}{\textit{\hl{Applications}}} \\
Example~\ref{ex:threshold} & Threshold on $\mathbb{R}^n$ & Half-space recognizers; continuous $\Rightarrow$ discrete quotient \\
Example~\ref{ex:discrete} & Discrete Lattice $\mathbb{Z}^3$ & Parity recognizer; finite quotient from infinite space \\
Example~\ref{ex:quantum} & Quantum Spin $S^2$ & Stern-Gerlach measurements; finite resolution limitation \\
Example~\ref{ex:RS} & Recognition Science & Ledger space $\mathcal{L}$; physical space emerges as quotient \\
\bottomrule
\end{tabular}}
\end{table}
}  %% \textcolor{black}{[Response to Reviewer 2, Comment 6: Summary table of axioms and constructions]}

%%%
 

The main message of RG is that space is not given in advance but is obtained
through recognition. {\color{black}This measurement-first ontology unifies diverse approaches across mathematical physics, quantum foundations, information geometry, and discrete spacetime theories, while providing a rigorous axiomatic foundation amenable to formal verification. The framework naturally accommodates finite observational resolution, explaining why classical continua appear discrete at fine scales, and applies equally to discrete, continuous, and hybrid systems. %Future work will explore dimensional analysis, manifold structures emerging from refined recognizers, and applications to quantum field theory, quantum gravity, and the foundations of spacetime.}  %% \textcolor{black}{[Enhanced from RG-Axioms-ver-11]
}
...
\acknowledgments{The authors express their sincere gratitude to the anonymous referees for a number of insightful comments. These comments significantly improved both the accuracy and clarity of the manuscript.}
...
% ACS format
% {\color{black}BIBLIOGRAPHY REORDERED: ALL 64 REFS IN CONSECUTIVE CITATION ORDER [1]--[64]}
...
% [1]
\bibitem{Zlat} Stankovi\'c, M.; Zlatanovi\'c, M. \textit{Non-Euclidean Geometry}; Faculty of Sciences and Mathematics: Ni\v{s}, \hl{Serbia,} %MDPI: Newly added  information. Please confirm. Same as below.
 2016.

% [2]
\bibitem{Lee} Lee, J.M. \textit{Introduction to Smooth Manifolds}, 3rd ed.; Springer:  \hl{Berlin/Heidelberg, Germany,} %We added the location of publisher. Please confirm
2013.

% [3]
\bibitem{Einstein1916} Einstein, A. {Die Grundlage der allgemeinen Relativitätstheorie}. \emph{Ann. Phys.} \textbf{1916}, \emph{354}, 769--822.

% [4]
\bibitem{Wald} Wald, R.M. \textit{General Relativity}; University of Chicago Press: \hl{Chicago, IL, USA,} 1984.

% [5]
\bibitem{Penrose} Penrose, R. \textit{The Road to Reality}; Jonathan Cape: \hl{London, UK,} 2004.

% [6]
\bibitem{Weyl} Weyl, H. \textit{Space, Time, Matter}; translation of 4th German edition, 1922; Dover Publications: \hl{Garden City, NY, USA,} 1952.

% [7]
\bibitem{Malament} Malament, D.B. \textit{Topics in the Foundations of General Relativity and Newtonian Gravitation Theory}; University of Chicago Press: \hl{ Chicago, IL, USA,} 2012.

% [8]
\bibitem{Geroch} Geroch, R. \textit{Mathematical Physics}; University of Chicago Press:  \hl{ Chicago, IL, USA,} 1985.

% [9]
\bibitem{Ellis} Ellis, G.F.R.; Rothman, T. {Time and Spacetime: The Crystallizing Block Universe}. \emph{Int. J. Theor. Phys.} \textbf{2010}, \emph{49}, 988--1003.

% [10]
\bibitem{Riesz} Riesz, F.; Sz-Nagy, B. \textit{Functional Analysis}; Dover Publications:  \hl{Garden City, NY, USA,} 1990.

% [11]
\bibitem{Gelfand} Gelfand, I.M.; Naimark, M.A. {On the embedding of normed rings into the ring of operators in Hilbert space}. \emph{Mat. Sb.} \textbf{1943}, \emph{12}, 197--217.

% [12]
\bibitem{Segal} Segal, I.E. {Postulates for general quantum mechanics}. \emph{Ann.  Math.} \textbf{1947}, \emph{48}, 930--948.

% [13]
\bibitem{vonNeumann} von Neumann, J. \textit{Mathematical Foundations of Quantum Mechanics}; Princeton University Press:  \hl{Princeton, NJ, USA,} 1955.

% [14]
\bibitem{WheelerZurek} Wheeler, J.A.; Zurek, W.H. (Eds.) \textit{Quantum Theory and Measurement}; Princeton University Press:  \hl{Princeton, NJ, USA,}  1983.

% [15]
\bibitem{Hardy} Hardy, L. {Quantum Theory From Five Reasonable Axioms}. \emph{arXiv} \textbf{2001}, arXiv:quant-ph/0101012.

% [16]
\bibitem{Piron} Piron, C. \textit{Foundations of Quantum Physics}; W.A. Benjamin: \hl{New York, NY, USA,} 1976.

% [17]
\bibitem{Fuchs} Fuchs, C.A.; Schack, R. {Quantum-Bayesian Coherence}. \emph{Rev. Mod. Phys.} \textbf{2013}, \emph{85}, 1693--1715.

% [18]
\bibitem{Caves}  Caves, C.M.; Fuchs, C.A.; Schack, R. {Quantum probabilities as Bayesian probabilities}. \textit{Phys. Rev. A} \textbf{2002}, \emph{65}, 022305.

% [19]
\bibitem{RQM} Rovelli, C. {Relational Quantum Mechanics}. \emph{Int. J. Theor. Phys.} \textbf{1996}, \emph{35}, \hl{1637--1678}.

% [20]
\bibitem{Laudisa} Laudisa, F.; Rovelli, C. {Relational quantum mechanics}. In \emph{Stanford Encyclopedia of Philosophy}, Summer 2021 ed.; Zalta, E.N., Ed.;  \hl{Metaphysics Research Lab, Stanford University: Stanford, CA, USA,} 2021.

% [21]
\bibitem{Isham} Isham, C.J. \textit{Lectures on Quantum Theory: Mathematical and Structural Foundations}; Imperial College Press: \hl{London, UK,} 1995.

% [22]
\bibitem{Landsman} Landsman, N.P. \textit{Foundations of Quantum Theory: From Classical Concepts to Operator Algebras}; Springer: \hl{Cham, Switzerland,} 2017.

% [23]
\bibitem{Varadarajan} Varadarajan, V.S. \textit{Geometry of Quantum Theory}, 2nd ed.; Springer:  \hl{New York, NY, USA,} 2007.

% [24]
\bibitem{Svozil} Svozil, K. \textit{Quantum Logic}; {Springer:}  \hl{Dordrecht, The Netherlands,} 
 1998.

% [25]
\bibitem{Zeilinger} Zeilinger, A. {A foundational principle for quantum mechanics}. \emph{Found. Phys.} \textbf{1999}, \emph{29}, 631--643.

% [26]
\bibitem{Wigner} Wigner, E.P. {The unreasonable effectiveness of mathematics in the natural sciences}. \emph{Commun. Pure Appl. Math.} \textbf{1960}, \emph{13}, \hl{1--14.} %MDPI: please check if this page range is correct.


% [27]
\bibitem{Bratteli} Bratteli, O.; Robinson, D.W. \textit{Operator Algebras and Quantum Statistical Mechanics},  2nd ed.; {Springer:}  \hl{Berlin/Heidelberg, Germany,} %We added the location of publisher. Please confirm
 1987; Volume 1.

% [28]
\bibitem{Haag} Haag, R. \textit{Local Quantum Physics: Fields, Particles, Algebras}, 2nd ed.; {Springer:} \hl{Berlin/Heidelberg, Germany,} 
 1996.

% [29]
\bibitem{Streater} Streater, R.F.; Wightman, A.S. \textit{PCT, Spin and Statistics, and All That}; Princeton University Press: \hl{Princeton, NJ, USA,} 2000.

% [30]
\bibitem{Schweber} Schweber, S.S. \textit{An Introduction to Relativistic Quantum Field Theory}; Row, Peterson and Company: \hl{Evanston, IL, USA,} 1961.

% [31]
\bibitem{Raimondi} Raimondi, V.; Ropke, G. \textit{Statistical Approach to Quantum Field Theory}, 2nd ed.; Springer: \hl{Cham, Switzerland,} 
 2018.

% [32]
\bibitem{Krein}  Krein, M.; Milman, D. {On extreme points of regular convex sets}. \emph{Stud. Math.} \textbf{1940}, \emph{9}, 133--138.

% [33]
\bibitem{Abramsky} Abramsky, S.; Coecke, B. {A Categorical Semantics of Quantum Protocols}. In \emph{Proceedings of LICS 2004}; IEEE Computer Society:  \hl{Los Alamitos, CA, USA,} 2004.

% [34]
\bibitem{Coecke} Coecke, B. {Quantum Picturalism}. \emph{Contemp. Phys.} \textbf{2010}, \emph{51}, 59--83.

% [35]
\bibitem{Lawvere} Lawvere, F.W.; Schanuel, S.H. \textit{Conceptual Mathematics: A First Introduction to Categories}; Cambridge University Press: \hl{Cambridge, UK}, 2009.

% [36]
\bibitem{Lurie} Lurie, J. \textit{Higher Topos Theory}; Princeton University Press: \hl{Princeton, NJ, USA,} 2009.

% [37]
\bibitem{Topos} Döring, A.; Isham, C.   {A Topos Foundation for Theories of Physics}.  \emph{J. Math. Phys.} \textbf{2008}, \emph{49}, 053515.

% [38]
\bibitem{Jaynes} Jaynes, E.T. \textit{Probability Theory: The Logic of Science}; Cambridge University Press: \hl{Cambridge, UK}, 2003.

% [39]
\bibitem{Amari1985} Amari, S. \textit{Differential-Geometrical Methods in Statistics}; Lecture Notes in Statistics 28; {Springer:}  \hl{Berlin/Heidelberg, Germany,}  
 1985.

% [40]
\bibitem{Amari2016} Amari, S. \textit{Information Geometry and Its Applications};  {Springer:}  \hl{Berlin/Heidelberg, Germany,}  
 2016.

% [41]
\bibitem{Frieden} Frieden, B.R. \textit{Physics from Fisher Information}; Cambridge University Press: \hl{Cambridge, UK}, 1998.

% [42]
\bibitem{Caticha} Caticha, A. \textit{Entropic Inference and the Foundations of Physics}; Monograph commissioned by the 11th Brazilian Meeting on Bayesian Statistics; \hl{The International Society for Bayesian Analysis-ISBrA: Sao Paulo, Brazil,} 2012.

% [43]
\bibitem{Kolmogorov} Kolmogorov, A.N. \textit{Foundations of the Theory of Probability}, 2nd ed.; Chelsea Publishing Company: \hl{New York, NY, USA,} 1956.

% [44]
\bibitem{Suppes} Suppes, P. \textit{Representation and Invariance of Scientific Structures}; CSLI Publications: \hl{Stanford, CA, USA,}  2002.

% [45]
\bibitem{Johnstone} Johnstone, P.T. \textit{Stone Spaces}; Cambridge Studies in Advanced Mathematics 3; Cambridge University Press: \hl{Cambridge, UK}, 1982.

% [46]
\bibitem{Vickers} Vickers, S. \textit{Topology via Logic}; Cambridge Tracts in Theoretical Computer Science 5; Cambridge University Press: \hl{Cambridge, UK}, 1989.

% [47]
\bibitem{Adamek} Ad\'amek, J.; Herrlich, H.; Strecker, G.E. \textit{Abstract and Concrete Categories: The Joy of Cats}; Wiley:  \hl{Hoboken, NJ, USA,} %We added the location of publisher. Please confirm
 1990.

% [48]
\bibitem{Sambin} Sambin, G. {Intuitionistic formal spaces---A first communication}. In \emph{Mathematical Logic and Its Applications}; Skordev, D., Ed.; \hl{Plenum:} %MDPI: Please check if this is publisher name and Please add the location of the publisher (city and country).
 1987.

% [49]
\bibitem{Ehresmann} Ehresmann, C. \textit{Categories et Structures}; Dunod: Paris, \hl{France}, 1965.

% [50]
\bibitem{Grothendieck} Grothendieck, A. \textit{Éléments de géOmétrie Algébrique}; Publications Mathématiques de l'IHÉS: \hl{Paris, France}, \hl{1960--1967.} %MDPI: please provide correct publisher year instead of range.


% [51]
\bibitem{Bombelli}  Bombelli, L.; Lee, J.; Meyer, D.; Sorkin, R.D. {Space-time as a causal set}. \emph{Phys. Rev. Lett.} \textbf{1987}, \emph{59}, 521--524.

% [52]
\bibitem{Sorkin} Sorkin, R.D. \textit{Causal Sets: Discrete Gravity}. In \textit{Lectures on Quantum Gravity}; {Springer:} \hl{Boston, MA, USA,} 
 2005.

% [53]
\bibitem{LQG} Rovelli, C. \textit{Quantum Gravity}; Cambridge University Press: \hl{Cambridge, UK},2004.

% [54]
\bibitem{NCG} Connes, A. \textit{Noncommutative Geometry}; Academic Press:  \hl{Cambridge, MA, USA,} %We added the location of publisher. Please confirm
 1994.

% [55]
\bibitem{Wolfram} Wolfram, S. \textit{A New Kind of Science}; Wolfram Media: \hl{Champaign, IL, USA}, 2002.

% [56]
\bibitem{Verlinde} Verlinde, E. {On the Origin of Gravity and the Laws of Newton}. \emph{J. High Energy Phys.} \textbf{2011}, \hl{\emph{2011}, 29}.

% [57]
\bibitem{Heller} Heller, M.; Woodin, W.H. (Eds.) \textit{Infinity: New Research Frontiers}; Cambridge University Press: \hl{Cambridge, UK}, 2011.

% [58]
\bibitem{Teller} Teller, P. \textit{An Interpretive Introduction to Quantum Field Theory}; Princeton University Press: \hl{Princeton, NJ, USA,} 1995.

% [59]
\bibitem{Peirce} Peirce, C.S. \textit{Collected Papers of Charles Sanders Peirce}; Hartshorne, C., Weiss, P., Eds.; Harvard University Press: \hl{Cambridge, MA, USA,} \hl{1931--1935; Volume 1--6.} %MDPI: 1. Please check whether this refs contains serval refs, if so, please separate them into different references and revise citation. 2. please use publisher year instead of range


% [60]
\bibitem{Busch} Busch, P.; Lahti, P.; Pellonpää, J.-P.; Ylinen, K. \textit{Quantum Measurement}; { Springer:}  \hl{Berlin/Heidelberg, Germany,}  
 2016.

% [61]
\bibitem{Munkres} Munkres, J. \textit{Topology}, 2nd ed.; Prentice Hall: \hl{Upper Saddle River, NJ, USA,} 2000.

% [62]
\bibitem{MacLane} Mac Lane, S. \textit{Categories for the Working Mathematician}, 2nd ed.; Springer:  \hl{Berlin/Heidelberg, Germany,}  
  1998.

% [63]
\bibitem{Chentsov} Chentsov, N.N. \textit{Statistical Decision Rules and Optimal Inference}; American Mathematical Society: \hl{Providence, RI, USA,}  1982.

% [64]
\bibitem{Lean4} Moura, L.d.; Ullrich, S.   {The Lean 4 Theorem Prover and Programming Language.} In\emph{ Automated Deduction----CADE 28. CADE 2021}; Platzer, A., Sutcliffe, G., Eds.;   Lecture Notes in Computer Science;  Springer: Cham, Switzerland, 2021; Volume 12699.
