...
% \usepackage{booktabs}  % removed: table removed
% \usepackage{float}     % removed: no floats
...
% \newtheorem{hypothesis}[theorem]{Hypothesis}  % unused
...
% RS-informed additions (violet) — added 2026-02-06
\newcommand{\rsadd}[1]{{\color{violet}#1}}
...
This paper proves the Riemann Hypothesis.
\begin{theorem}[Riemann Hypothesis]\label{thm:farfield}
The Riemann zeta function has no zeros in the open half-plane $\{\,s\in\C:\ \Re s> \tfrac12\,\}$.
Equivalently: for every $\varepsilon>0$, $\zeta(s)\neq 0$ for $\Re s\ge \tfrac12+\varepsilon$.

\noindent\textit{Proof.} See \S\ref{sec:proof-farfield}.
\end{theorem}

\noindent\textbf{Scope and supplementary computation.}
The proof of Theorem~\ref{thm:farfield} proceeds by direct contradiction (\S\ref{sec:proof-farfield}),
using a neutralized CR--Green energy estimate developed in Appendix~\ref{app:pplus-proof}.
% (The (P+) boundary wedge route has been removed from this version;
%  the primary proof uses a direct contradiction.)
The proof is purely analytic.
The small-height case ($|\gamma_0|\le 2$) is vacuous because the first nontrivial zero
of $\zeta$ has $|\gamma|\approx 14.13$.

The Appendix collects the supporting analytic lemmas
(outer normalization, Carleson energy, inner reciprocal, energy bound, and CR--Green pairing).

\subsection*{Strategy: direct contradiction via the inner reciprocal}
We work on the right half-plane $\Omega=\{\,\Re s>\tfrac12\,\}$.
In Section~\ref{sec:defs} we define an arithmetic ratio $\mathcal J_{\rm out}$
built from $\dettwo(I-A(s))$ (a regularized determinant over primes),
$\zeta(s)$, and an outer normalizing function $\mathcal O_\zeta$.
Its poles in $\Omega$ correspond exactly to zeros of $\zeta$ in $\Omega$.

The \textbf{inner reciprocal} $\mathcal I:=B^2/\mathcal J_{\rm out}$
(where $B(s)=(s-1)/s$) is holomorphic on $\Omega$ with
$|\mathcal I|\le 1$ (Phragm\'en--Lindel\"of) and $|\mathcal I^*|=1$ a.e.
Zeros of $\zeta$ in $\Omega$ become \textbf{zeros} (not poles) of $\mathcal I$.

The proof proceeds by contradiction: assuming a zero $\rho_0$ with
$\Re\rho_0>\tfrac12$, the neutralized inner reciprocal
$\mathcal I_{\rm neut}:=\mathcal I/B_{\rm box}$ (with near-box zeros divided out)
provides both a \textbf{positive lower bound} on the windowed boundary phase derivative
(from $\rho_0$'s Poisson contribution) and a \textbf{Cauchy--Schwarz upper bound}
(from the Whitney-box energy of $-\log|\mathcal I_{\rm neut}|$).
The height-dependent Whitney parameter $c=c_0/\log\angles{\gamma_0}$
cancels the $\log^2$ energy growth, yielding a height-independent contradiction.
...
% [Schur/Herglotz terminology and "Outline" subsections removed:
%  not used in the primary proof (direct contradiction via inner reciprocal).]

% [§3 Schur/Herglotz pinch mechanism removed: not used in the primary proof.
%  The direct contradiction in §4.1 does not invoke the Schur bound or Corollary no-poles.]
\iffalse
...
Set $c_\varepsilon:=4\pi/(\varepsilon+1)$, $c_0:=\min\bigl\{(c_\varepsilon/(2A))^2,\;1/2\bigr\}$, and
\[
...
\begin{equation}\label{eq:lower-zero}
  ...
  \;\ge\; c_\varepsilon\,L.
\end{equation}
(Using \(\delta_0\ge\varepsilon\) and \(L\le 1\):
\(4\pi L/(\delta_0+L)\ge 4\pi L/(\varepsilon+1)=:c_\varepsilon\,L>0\).)
...
\textbf{Contradiction.}
(Here \(c_\varepsilon=4\pi/(\varepsilon+1)>0\) depends only on~\(\varepsilon\),
...
\end{proof}
% [Remark Pplus-route removed: (P+) route is not used.]
% [Supplementary computational artifacts removed; not logically required for the proof.]
