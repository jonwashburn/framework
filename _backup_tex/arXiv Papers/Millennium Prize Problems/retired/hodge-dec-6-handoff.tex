...
cone generated by simple $\psi$-saturating $(p,p)$-forms $\xi_P$ associated to
complex $(n-p)$-planes $P\subset T_xX$.  By Carath\'eodory's theorem in
$\R^{D}$, every $\beta(x)\in K_p(x)$ can be written as a convex combination
of at most $D+1$ extremal generators, where $D=\dim(\mathcal{V}_x)=\binom{n}{p}^2$
is independent of $x$.
...
$\varepsilon$ of $\{\Pi_{Q,j}\}$ with weights proportional to $N_{Q,j}$.
...
(in Hilbert--Schmidt norm) of $\beta$ on $Q$, and
\[
\Mass(S_Q) \to \int_Q \beta\wedge \psi
\quad \text{as }\delta\to 0.
\]
\end{lemma}

\begin{proof}
By Substep~3.3, the integer multiplicities $N_{Q,j}$ can be chosen so that
$N_{Q,j}/(\sum_i N_{Q,i})$ approximates $\theta_{x,j}$ (nearly constant on
$Q$) to within $O(1/m)$.  Then
$\sum_j(N_{Q,j}/\sum_i N_{Q,i})\,\xi_{\Pi_{Q,j}}$ is within $\delta$ of
$\beta$ on $Q$.  Because the tangent angles are $<\varepsilon$ and
$\varepsilon\ll\delta$, the Hilbert--Schmidt distance of barycenters is
$\le C(\varepsilon+\delta)$.  Calibratedness gives
$\Mass([Y_{Q,j}^a]\llcorner Q)=\int_Q\psi\llcorner[Y_{Q,j}^a]$, yielding
...
partition of $X$.  For every $\varepsilon>0$, there exist:
\begin{itemize}
...
By Lemma~\ref{lem:caratheodory-general}, decompose
\[
\beta(x_Q)=\sum_{j=1}^{J(Q)}\theta_{Q,j}\,\xi_{Q,j},
\]
with $\xi_{Q,j}$ calibrated planes in $K_p(x_Q)$, $\theta_{Q,j}\ge 0$,
$\sum_j\theta_{Q,j}=1$, and $J(Q)\le N=N(n,p)$ uniformly bounded.

Since $[\gamma]$ is rational, all its periods lie in $(1/M)\Z$ for some
fixed $M$.  Choose $m\gg 1$ divisible by $M$ and large enough that for
all $(Q,j)$ we can pick integers $N_{Q,j}$ with
\[
\Bigl|\frac{N_{Q,j}}{m}-\theta_{Q,j}\,\mathrm{Vol}(Q)\Bigr|
\le\delta\,\mathrm{Vol}(Q).
\]
Apply Theorem~\ref{thm:local-sheets} to realize each $\xi_{Q,j}$ as
calibrated sheets $Y_{Q,j}^a\subset Q$ ($a=1,\ldots,N_{Q,j}$) with
tangent planes close to $\xi_{Q,j}$ and boundary on $\partial Q$.
...
\textbf{Key point:} Because $\beta$ is closed and we matched $\beta$'s
fluxes to the sheet stacks to accuracy $O(\delta+1/m)$, the mass of
each $B_F$ is small.  By the Federer--Fleming isoperimetric inequality
on compact manifolds, there exists a correction current
$R_{\mathrm{glue}}$ with
\begin{itemize}
...
\]
Since $[\gamma]$ is rational, there exists $M$ such that for any closed
$(n-p,n-p)$-form $\eta$, the period $\int_X\beta\wedge\eta\in(1/M)\Z$.
We chose $m$ divisible by $M$.  By careful choice of $N_{Q,j}$:
\[
\Bigl|\frac{1}{m}\int_{T^{(1)}}\eta_\ell-\int_X\beta\wedge\eta_\ell\Bigr|
\le\frac{1}{2M}.
\]
Since both terms lie in $(1/M)\Z$ and differ by less than $1/2$, they
are equal.  Hence
\[
...
\begin{proof}
This is a finite system of linear constraints with integer unknowns
$\{N_{Q,j}\}$.  The map $(N_{Q,j})\mapsto\int_{T^{\mathrm{raw}}}\eta_\ell$
is linear in $N_{Q,j}$.  We know from the continuous $\beta$ decomposition
that there exists a real solution $(\theta_{Q,j})$ giving exact equality
in the limit $\delta\to 0$.  By making the partition sufficiently fine,
the contribution of each $(Q,j)$ to the vector $(\Theta_\ell)_{\ell=1}^b$
can be made arbitrarily small and the feasible set becomes $\epsilon$-dense.
Standard Diophantine approximation (multidimensional continued fractions
or LLL lattice basis reduction) yields simultaneous approximation within
$<\tfrac12$ for all $\ell$.  Since the left side is an integer by
construction, equality follows by integrality of the lattice $H_{2(n-p)}(X,\Z)$.
\end{proof}
...
Riemannian manifolds, there exist integral $(2n-2p+1)$-currents
$U_\epsilon$ with
\[
...
\[
T_\epsilon := S - \partial U_\epsilon,
...
\[
\Mass(T_\epsilon)
\le \Mass(S) + \Mass(\partial U_\epsilon)
\to \int_X \beta\wedge \psi,
\]
since $\Mass(U_\epsilon)\to 0$ implies
$\Mass(\partial U_\epsilon)\to 0$ by the boundary-mass estimate.
...
\begin{enumerate}
\item[\textnormal{(i)}] $\Mass(T_\varepsilon)\to\int_X\beta\wedge\psi$;
...
=m\int_X\gamma\wedge\varphi=\int_X\beta\wedge\psi.
\]
Thus $\Mass(T_k)\ge\int_X\beta\wedge\psi-o(1)$ as well.  We conclude:
\begin{itemize}
...
\Mass(T)\le\liminf_{k\to\infty}\Mass(T_k)\le\int_X\beta\wedge\psi.
\end{equation}

\medskip\noindent
\textbf{Substep 6.3: Tangent-plane Young measures.}
For each $k$, the tangent planes of $T_k$ around $x$ induce a probability
measure $\nu_x^{(k)}$ on $\Gr_{n-p}(T_xX)$.  The cone-defect estimate from
calibration coercivity gives:
\[
\int_X\int\mathrm{dist}^2\bigl(\xi_P,K_p(x)\bigr)\,d\nu_x^{(k)}(P)\,d\mu_k(x)
\xrightarrow{k\to\infty}0.
\]
Also, by Lemma~\ref{lem:local-bary}:
\[
...
\int_X\int\mathrm{dist}^2\bigl(\xi_P,K_p(x)\bigr)\,d\nu_x(P)\,d\mu(x)=0,
\]
so for $\mu$-a.e.\ $x$, $\mathrm{supp}\,\nu_x\subset K_p(x)$.  In particular,
every $P$ in the support of $\nu_x$ is $\psi$-calibrated
($\psi(\xi_P)=1$).

Passing to the limit in the barycenter identity gives:
\[
\int\xi_P\,d\nu_x(P)=\beta(x)\quad\text{for }\mu\text{-a.e.\ }x.
\]
...
\begin{enumerate}
\item Mass convergence: $\Mass(T_k)\to\int_X\beta\wedge\psi$;
...
$\int\xi_P\,d\nu_x(P)=\beta(x)$ holds for $\mu$-almost every $x$,
...
Conjecture.
\end{itemize}
The key insight is that $\beta$ prescribes the \emph{local tangent-plane
distribution} and \emph{mass density} of the algebraic cycle $T$, not
its support.  The support of $T$ is a positive combination of complex
subvarieties whose combined homology class is $\mathrm{PD}(m[\gamma])$.
\end{remark}
...
key estimate is:
\[
\Mass(R_{\mathrm{glue}})\le C\sum_F\Mass(B_F),
\]
where $B_F$ is the boundary mismatch on face $F$.  Because $\beta$ is
\emph{smooth}, the Carath\'eodory decomposition varies slowly (Lipschitz
in the cube size $\varepsilon$), so:
\[
\Mass(B_F)=O(\varepsilon)\cdot(\text{mass of sheets crossing }F).
\]
Summing over all faces:
\[
\Mass(R_{\mathrm{glue}})=O(\varepsilon)\cdot\Mass(T^{\mathrm{raw}})
=O(\varepsilon)\to 0\quad\text{as }\varepsilon\to 0.
\]
...
\begin{enumerate}
\item[\textnormal{(i)}]
$\Mass(T_k)\to \displaystyle\int_X \beta\wedge\psi$,
\item[\textnormal{(ii)}]
