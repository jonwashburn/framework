...
\subsection*{Level of prediction and anchors}
Our architecture is parameter--free in the sense that no sector--specific fit parameters or weights are introduced. Dimensionless predictions (ratios, rung assignments, mixings) follow from the fixed invariants and the $\varphi$--sheet fixed point. Absolute values require one experimental anchor to set units: for neutrinos we use $\Delta m^2_{31}$ (fixing a global scale $s$ subsequently applied to all charged leptons), and for bosons we report $M_Z,M_H$ from rung ratios anchored to $M_W$. This is analogous to the SM use of $G_F$ to set $v$. Results never depend on inserting any particle’s mass on the right--hand side of the fixed--point map.

\paragraph{Glossary (to standard FT language).}
\textbf{Rung $r\in\mathbb{Z}$:} discrete index that encodes the coarse hierarchy; it labels the multiplicative step $m\propto\varphi^r$.\;\;
\textbf{Ledger/LNAL:} a fixed set of representation invariants used in the logarithmic normal--additive ledger (LNAL) that couples the short RG window to group data (hypercharge $Y_R$ and SU(2) Casimir), producing a parameter--free gap series $\sum_m g_m I_m(i)$.\;\;
\textbf{Gap series:} the additive correction in $f_i$ with coefficients $g_m=(-1)^{m+1}/(m\varphi^m)$ coming from the expansion of $F(z)=\ln(1+z/\varphi)$; $I_m(i)$ are fixed invariants tied to the species’ representation.\;\;
\textbf{$\varphi$--sheet:} a probe--free average of the same short RG window over the geometric stack $\{\mu,\varphi\mu,\varphi^2\mu,\ldots\}$ with signed weights $w_k\propto g_{k+1}$, normalized in $\ell^1$; it removes arbitrary scale choice while adding no parameters.\;\;
\textbf{Occupancy (8--beat):} a rung--class function $\Delta f_\chi(r)$ that encodes right--chiral occupancy modulo 8; we use the unique mean--zero, period--8 linear form satisfying the ledger symmetry constraints, see Eq.~\eqref{eq:I1I2}.
...
\begin{table}[h]
\centering
\caption{Charged leptons (parameter--free, $\varphi$--sheet fixed points; v22b snapshot).}
\setlength{\tabcolsep}{6pt}
\begin{tabular}{lrrrrrr}
\hline
Species & $r$ & $f_i$ & $B$ & $m_{\exp}$ [GeV] & $m_{\rm calc}$ [GeV] & $\delta$ [\%]\\
\hline
$e$   & 0  & $-0.012891$ & 1 & 0.000\,510\,998\,95 & 0.000\,510\,998\,90 & $-9.8\times10^{-6}$\\
$\mu$ & 11 & $-0.022716$ & 1 & 0.105\,658\,3755    & 0.105\,658\,4000    & $+2.3\times10^{-5}$\\
$\tau$& 17 & $-0.029878$ & 1 & 1.776\,86           & 1.776\,86           & $+0.0$\\
\hline
\end{tabular}
\end{table}
...
\paragraph{Why the $\varphi$--sheet.}
Among all $\ell^1$ sequences $\{w_k\}$ the choice $w_k\propto g_{k+1}$ (with $g_m$ the ledger gap coefficients) is the unique one that (i) keeps the same analytic structure that defines the gap series, (ii) is shift--invariant under $\mu\!\to\!\varphi^j\mu$ (reindex $k\mapsto k{-}j$), and (iii) introduces no new scale or depth parameter (absolute summability $\Rightarrow$ adaptive truncation). Hence the probe scale drops out without adding freedom.
...
\begin{itemize}
  ...
  \]
with $R(s)$ modeled as narrow resonances $+$ continuum plateaus and an Adler--function method above $s_0\simeq(2.5\,{\rm GeV})^2$ for the high--$Q^2$ tail.\;This backend is modular (the $R(s)$ table can be swapped without changing callers) and is the same object consumed by the lepton solver \cite{EidelmanJegerlehner1995,Jegerlehner2003,Keshavarzi2020,Davier2017}.
...
\paragraph{Mixing from rung geometry (operational map).}
Given up--type rungs $\{r^u_i\}$ and down--type rungs $\{r^d_j\}$ define a raw overlap
\[
M_{ij}=\sigma_{ij}\,\varphi^{-|r^u_i-r^d_j|}\,,\qquad \sigma_{ij}\in\{\pm1\}\text{ fixed by the 8--beat parity of }(r^u_i-r^d_j)\bmod 8.
\]
Normalize rows and columns to unit $\ell^2$ norm and project to the nearest unitary by polar decomposition, $V = M (M^\dagger M)^{-1/2}$ (or Gram--Schmidt on columns). Apply the same construction to leptons with $(r_e,r_\mu,r_\tau)=(0,11,17)$ and $(r_{\nu_1},r_{\nu_2},r_{\nu_3})=(7,9,12)$.
...
\begin{itemize}
  ...
  \item Provenance (this repository snapshot): commit \texttt{7d1e5aec7e91fb408f7a9bf613990359e294271e}, tree \texttt{70d25e15b765927d3694380fb6a4e55323a3a9b0}; script blob for \texttt{ledger\_snapshot\_v22c.py} is \texttt{fe6d7bba9822cb575254a76b5c19e2fa0952dbc9}. Source SHA--256: \texttt{7b0b02449e5c054a40eff15af7a0c594977c17179b5950601cf97db5e1934c2f}; compiled pyc SHA--256: \texttt{e868d7765a532b9f190b2ea1a6971b71d20628721cfe6ceb29f4bd4a121691e2}. (These IDs are what the v22c script prints in its provenance block.)
\end{itemize}
...
\paragraph{Scope.} The present construction applies to elementary SM fields (charged leptons, neutrinos in a Dirac realization, quarks, and gauge/Higgs bosons). It is not intended to model composite resonances or bound--state masses.

\paragraph{Baseline factor $B$.} For charged leptons we fix $B_i\equiv 1$ (single ledger channel), exactly as implemented in the solver. No coherent multi--channel sum is used in this sector. If a sector requires $B>1$ in future, we will enumerate the channels explicitly in the table for that sector.
...
\section*{Appendix A: Derivations of $w_L=3/19$, $I_2=9/76$, and $\Delta f_\chi(r)$}
For $T=\tfrac12$, $C_2(SU2)=T(T{+}1)=\tfrac34$. With $Y_R^2=4$, define $w_L=C_2/(Y_R^2+C_2)=\tfrac{3/4}{4+3/4}=\tfrac{3}{19}$, so $I_2=w_L\,T(T{+}1)=\tfrac{9}{76}$. The unique period--8, mean--zero, odd (about $r\mapsto r{+}4$) affine form with minimal step is $\Delta f_\chi(r)=\big((r\bmod 8)-4\big)/8$.

\section*{Appendix B: $\varphi$--sheet invariance}
With $w_k\propto g_{k+1}$ and absolute summability, shifting the probe $\mu\to\varphi^j\mu$ reindexes $k\mapsto k{-}j$ and leaves Eq.~\eqref{eq:sheet-average} invariant up to the truncated tail. The weights share the same analytic structure as the ledger gap coefficients, introducing no extra depth/scale.

\section*{Appendix C: CKM/PMNS operational map}
Construct $M_{ij}=\sigma_{ij}\,\varphi^{-|r^u_i-r^d_j|}$ with parity signs from $(r^u_i-r^d_j)\bmod 8$, normalize, and project to unitary $V=M(M^\dagger M)^{-1/2}$. Apply analogously to leptons.

\section*{Appendix D: Worked example (muon)}
Inputs: $r_\mu=11$, $B_\mu=1$, $I_1=4-\tfrac18$, $I_2=\tfrac{9}{76}$. Define $f_\mu$ by Eq.~\eqref{eq:invariants} with $w_k\propto g_{k+1}$ and solve Eq.~\eqref{eq:fixed-point}. From v22b, $f_\mu\approx-0.022716$, matching the table; absolutes follow from the single global scale $s$ fixed by $\Delta m^2_{31}$.
\end{document}