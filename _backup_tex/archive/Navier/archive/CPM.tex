...
\[
  c\ =\ (\Cnet\,\Cproj\,\Ceng)^{-1}.
\]
In the intrinsic cone-projection route (\(\Cnet=1\)), one may take \(\Cproj=2\) (rank-one Hermitian control) and \(\Ceng=1+d\,C_\Lambda^2\).
With the normalized trace choice \(C_\Lambda=d^{-1/2}\) (so that \(d\,C_\Lambda^2=1\)), this gives \(\Ceng=2\) and hence \(c=(\Cnet\Cproj\Ceng)^{-1}=1/4\).
Minimizing sequences have vanishing defect and converge to positive calibrated currents; on projective manifolds these are algebraic cycles.
...
\paragraph{Setup.} Let \(\omega=\nabla\times u\). Define the scale-critical vorticity functional
\[
 \mathcal W(x,t;r)\ :=\ r^{-2}\iint_{Q_r(x,t)} |\omega|^{3/2},
\]
and its supremum profile. (The normalization \(r^{-2}\) is the 3D Navier--Stokes scale-invariant choice since \(|Q_r|\sim r^5\) and \(\omega\) scales like \(r^{-2}\).)
Let the defect aggregate these critical quantities on a final time window. Energy control stems from heat-flow estimates and Calder\'on--Zygmund bounds. The structured set corresponds to small-data regimes characterized by a \(\mathrm{BMO}^{-1}\) time slice.
...
\subsection*{Hodge}
\begin{itemize}
\item \(\Cnet=1\) (intrinsic cone projection); \(\Cproj=2\); \(\Ceng=1+d\,C_\Lambda^2\) with \(d=\binom{n}{p}\), \(C_\Lambda=d^{-1/2}\).
\item Resulting coercivity constant: \(c=(\Cnet\Cproj\Ceng)^{-1}\); under \(C_\Lambda=d^{-1/2}\) (so \(d\,C_\Lambda^2=1\)) this gives \(c=1/4\).
\end{itemize}
