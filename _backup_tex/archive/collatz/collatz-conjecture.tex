...
All certificate data, validation scripts, and formal verification stubs are available at:
\begin{center}
\url{https://github.com/jonwashburn/collatz}
\end{center}
...
Then for any odd $n>N_0^{\ast}$, the orbit strictly decreases within at most $J^{\ast}=j_{\max}+L$ odd steps.
Iterating, the odd values cannot stay $>N_0^{\ast}$ forever; once $\le N_0^{\ast}$, finitely many cases remain and are checked directly.
...
\item Check $N_0^{\ast}$ exceeds all $2^{F(R)}N_{0,R}$; then verify Collatz for all $n\le N_0^{\ast}$ (finite).
\end{enumerate}
...
\item Explicit global constants $J^{\ast}=26$ and $N_0^{\ast}=24{,}989{,}664$ that bound the step count and height range needed for the proof.
...
\subsection{Global constants \texorpdfstring{$J^{\ast}$}{J*} and \texorpdfstring{$N_0^{\ast}$}{N0*}}
...
\[
J^{\ast} := j_{\max} + L \le 26,
\]
so that every sufficiently large $n$ sees a strict decrease in $E$ within at most $J^{\ast}$ odd steps.
...
\[
N_0^{\ast} = \left\lceil 2^{16} \max N_0 \right\rceil = 24{,}989{,}664.
\]
Thus any odd $n>N_0^{\ast}$ decreases within at most $J^{\ast}$ odd steps, and the finitely many $n\le N_0^{\ast}$ are verified directly (Section~\ref{sec:verification}).

\subsection{Per-residue refinements}

The bound $N_0^{\ast}$ is deliberately conservative, as it uses the worst-case funnel length $L$ and the maximum window threshold across all residues. The certificate contains finer information:
...
These refinements are not needed for the proof---the existence of any finite global $N_0^{\ast}$ suffices---but they may be of independent interest for analyzing stopping times, measuring the ``difficulty'' of particular residues, or optimizing the finite verification phase.
...
\subsection{Finite check below \texorpdfstring{$N_0^{\ast}$}{N0*}}

With $J^{\ast}$ and $N_0^{\ast}$ fixed, \texttt{finite\_check.py} simulates every Collatz orbit with $n\le 24{,}989{,}664$. The resulting log reports the maximal stopping time (704 steps at $n=15{,}733{,}191$). This establishes convergence for the entire range below $N_0^{\ast}$.
...
\[
J^{\ast} = j_{\max}+L,\qquad
N_0^{\ast} = \left\lceil 2^{L}\,\max N_0 \right\rceil.
\]
Then every odd integer $n>N_0^{\ast}$ is guaranteed to satisfy
\[
E\bigl(T^{\,k}(n)\bigr) < E(n)
\]
for some $k\le J^{\ast}$, and therefore cannot grow without bound. The finitely many integers $n\le N_0^{\ast}$ are verified directly as part of the finite check.
...
\end{itemize}
Together these files, plus the validator, reconstruct the constants $J^{\ast}$ and $N_0^{\ast}$ and certify all necessary congruences and transitions; their hashes appear in \texttt{summary.json}.

\subsection{Sensitivity to modulus and window depth}

The choice $M=18$, $j_{\max}=10$, and $L=16$ reflects a balance between search complexity and certificate size. In principle, increasing $M$ (working modulo a higher power of two) or allowing slightly deeper windows (larger $j_{\max}$) would enlarge the search space, but they also tend to produce more and stronger contracting patterns, which could reduce both $J^{\ast}$ and $N_0^{\ast}$:
\begin{itemize}
\item A larger modulus provides finer control over residue classes and may allow shorter funnels (smaller $L$) because residues see more distinct window types. This directly decreases $J^{\ast}=j_{\max}+L$.
\item Deeper windows with larger $K$ can yield smaller contraction ratios $A=3^j/2^{K}$ and therefore smaller thresholds $N_0$. If enough such windows appear at a given modulus, the overall $N_0^{\ast}$ can be lowered.
\end{itemize}
...
\end{itemize}
Once these lemmas are available, the abstract energy argument (Section~5) can be formalized in the logic of the proof assistant, concluding that every trajectory decreases the logarithmic height $E(n)$ in bounded time above $N_0^{\ast}$, and that all smaller inputs have been checked by finite computation.

\subsection{Data availability and integrity}

All artifacts reside in the GitHub repository at \url{https://github.com/jonwashburn/collatz}, including:
\begin{itemize}
\item \texttt{collatz-conjecture.tex} and \texttt{collatz-conjecture.pdf}
...
\item \emph{Global bounds.} Compute $j_{\max}$, $L$, and $N_0^{\ast}$ from the validated data and output the resulting constants $J^{\ast}=j_{\max}+L$ and $N_0^{\ast}$.
\item \emph{Finite Collatz check.} Run a Collatz simulator on all $n\le N_0^{\ast}$, confirming that each trajectory reaches $1$.
\end{enumerate}
...
Nothing in the argument fundamentally restricts us to $2^{18}$. It is plausible that smaller moduli could suffice if deeper windows are allowed, or that larger moduli could yield smaller global constants $J^{\ast}$ and $N_0^{\ast}$, but those optimizations are orthogonal to the central point: there exists at least one finite modulus at which a complete window--funnel certificate can be constructed.
...
\item Increasing $j_{\max}$ allows more aggressive contractions ($A$ significantly below $1$), which can reduce the thresholds $N_0$ and therefore the global $N_0^{\ast}$, at the cost of more complex windows.
\item The uniform step bound $J^{\ast} = j_{\max}+L$ suggests that both $j_{\max}$ and $L$ should be kept modest; too large a value in either dimension increases the worst-case time to see a strict decrease in $E$.
\end{itemize}
