...
\subsection{Amplified Dispersion Strategy (Kuznetsov + \(k{=}3\) Heath--Brown)}
\label{subsec:amplified-dispersion}
\paragraph{Goal.} Prove the Type~II multiplicative energy certificate (Theorem~\ref{thm:typeII-energy-target}). We record a concrete route that isolates the true off-diagonal and reduces it to a spectral inequality amenable to Kuznetsov/large-sieve technology.

\paragraph{Step 1: \(k{=}3\) Heath--Brown identity and balanced dyadic block.}
Replace Vaughan by the \(k{=}3\) Heath--Brown identity so that Type~II coefficients satisfy
\[
  \sum_{m\sim M} |a_m|^2 \ll M(\log N)^{A_1},\qquad
  \sum_{n\sim N/M} |b_n|^2 \ll (N/M)(\log N)^{A_2},
\]
with \(A_1{+}A_2\) minimal (three-fold convolutions suppress peak divisor spikes). Choose \(M \in [N^{1/2-\delta},\,N^{1/2+\delta}]\) (dyadic window) to maximize bilinear symmetry and minimize the large-sieve loss.

\paragraph{Step 2: Amplification and completion on medium arcs.}
On \(\alpha=\frac{a}{q}{+}\beta\) with \(Q<q\le Q'\), complete the inner sum and insert an amplifier supported on the balanced dyadic block \(M\). This yields
\[
  B_M\!\Big(\tfrac{a}{q}{+}\beta\Big)\;=\; \sum_{t\sim N} c_t(a,q)\,e(\beta t),\qquad
  \sum_t |c_t(a,q)|^2 \asymp \sum_t \Big|\sum_{mn=t} a_m b_n\Big|^2,
\]
uniformly in \((a,q)\). The medium-arc \(L^4\) bound reduces to controlling
\(\sum_{Q<q\le Q'}\sum_{(a,q)=1} |B_M(\frac{a}{q})|^2\) in terms of \(\sum_t |c_t|^2\).

\paragraph{Step 3: Kuznetsov transform of the off-diagonal.}
The quantity \(\sum_{q,a} |B_M(\frac{a}{q})|^2\) expands into diagonal and off-diagonal parts. The diagonal contributes the natural size \(N(\log N)^{A_1{+}A_2}\). The off-diagonal is transformed via Kuznetsov to a spectral sum of Kloosterman bilinear forms at moduli \(c\asymp q\) with test functions adapted to the medium-arc window. Schematically,
\[
  \sum_{Q<q\le Q'}\!\!\sum_{(a,q)=1} \Big|\sum_{m\sim M}\sum_{n\sim N/M} a_m b_n\,e\!\Big(\tfrac{amn}{q}\Big)\Big|^2
  \;\leadsto\;
  \sum_{c\asymp [Q,Q']} \frac{1}{c}\, \Big|\sum_{m\sim M}\sum_{n\sim N/M} a_m b_n\,S(mn,1;c)\,W_{c}(m,n)\Big|^2
\]
plus Maass/holomorphic spectral contributions with Bessel weights \(W_c\).

\paragraph{Step 4: Spectral large sieve with bilinear structure.}
Assume the following off-diagonal spectral bound.
\begin{theorem}[Spectral Off-Diagonal Bound (SODB)]\label{thm:SODB}
Let \(M\in[N^{1/2-\delta},N^{1/2+\delta}]\), \(c\in[Q,Q']\), and \(a_m,b_n\) be Type~II coefficients with the \(L^2\) sizes above. For any admissible Bessel weights \(W_c\) supported on \(m\sim M\), \(n\sim N/M\),
\[
  \sum_{c\asymp [Q,Q']} \frac{1}{c}\,\Big|\sum_{m\sim M}\sum_{n\sim N/M} a_m b_n\,S(mn,1;c)\,W_{c}(m,n)\Big|^2
  \ \ll\ N\,(\log N)^{1+\varepsilon},
\]
uniformly in \(M\) and \(\delta>0\) fixed. (The implied constant depends on \(\varepsilon,\delta\).)
\end{theorem}
This is a bilinear spectral large sieve at the natural scale; its truth would sharpen the classical \((\log N)^3\) to \((\log N)^{1+\varepsilon}\) by exploiting cancellation from the Möbius/log structure of the Type~II coefficients.

\paragraph{Step 5: From SODB to the multiplicative energy certificate.}
\begin{proposition}[SODB \(\Rightarrow\) Type~II energy]\label{prop:SODB-to-energy}
Assuming Theorem~\ref{thm:SODB}, the Type~II multiplicative energy certificate (Theorem~\ref{thm:typeII-energy-target}) holds with \(\sum_t|\sum_{mn=t}a_mb_n|^2 \ll N(\log N)^{1+\varepsilon}\), uniformly in \(M\in[N^{1/3},N^{2/3}]\).
\end{proposition}
\begin{proof}[Proof sketch]
Kuznetsov identifies the off-diagonal of \(\sum_{q,a}|B_M(\frac{a}{q})|^2\) with the spectral sum in Theorem~\ref{thm:SODB}. The diagonal is \(\ll N(\log N)^{A_1{+}A_2}\). The contribution from the spectral side is \(\ll N(\log N)^{1+\varepsilon}\). Balancing and summing over \(c\asymp[Q,Q']\) yields \(\sum_{q,a}|B_M(\frac{a}{q})|^2 \ll Q'^2 \cdot N(\log N)^{1+\varepsilon}\). The local \(L^4\) lemma and the medium-arc measure then give \(\int_{\mathfrak M_{\mathrm{med}}}|B_M|^4 \ll N^{5/3}(\log N)^{6+\varepsilon}\). Undoing completion returns \(\sum_t|\sum_{mn=t}a_mb_n|^2 \ll N(\log N)^{1+\varepsilon}\).
\end{proof}

\paragraph{Outcome.} Establishing Theorem~\ref{thm:SODB} (with \(k{=}3\) HB decomposition and balanced \(M\)) would prove Theorem~\ref{thm:typeII-energy-target}, hence Theorem~\ref{thm:medL4-target}, and therefore close Goldbach via Theorem~\ref{thm:gb-closure-cpm}.
...
\subsection{Closure Theorem via CPM (Goldbach)}\label{subsec:gb-closure}
\paragraph{Dispersion certificates (equivalent CPM targets).}
We record two equivalent dispersion certificates that suffice to close the binary Goldbach proof under the CPM aggregator:
\begin{enumerate}
  \item[(A)] \textbf{Medium-arc \(L^4\) certificate (power saving).} For some absolute \(C_0>0\) and \(N_1\),
  \[
    \mathcal D_{\mathrm{med}}(N)\ :=\ \int_{\mathfrak M_{\mathrm{med}}}\!\big(|S(\alpha)|^4+|S_{\chi_8}(\alpha)|^4\big)\,d\alpha\ \le\ C_0\,N^{5/3}\,(\log N)^{6},\qquad \forall N\ge N_1.
  \]
  \item[(B)] \textbf{Type~II multiplicative energy certificate.} For each dyadic \(M\in[N^{1/3},N^{2/3}]\) and for Vaughan/Heath--Brown Type~II coefficients \(a_m,b_n\) supported on \(m\sim M\), \(n\sim N/M\) with the standard \(L^2\) sizes,
  \[
    \sum_{t\sim N}\ \Big|\sum_{\substack{mn=t\\ m\sim M,\,n\sim N/M}} a_m b_n\Big|^2\ \le\ C_E\,N\,(\log N)^{1+\varepsilon}
  \]
  uniformly in \(M\) (fixed \(\varepsilon>0\)). 
\end{enumerate}
Standard circle-method reductions (completion modulo \(q\), dispersion, and the additive large sieve with constant \(1\)) imply (B)\(\Rightarrow\)(A).

\begin{theorem}[CPM Goldbach Closure]\label{thm:gb-closure-cpm}
Assume the Goldbach schedule \(Q=N^{1/2}(\log N)^{-4}\), \(Q'=N^{2/3}(\log N)^{-6}\), \(U=V=N^{1/3}\), and smoothing \(\eta\) with \(\Delta(\eta)\le C_{\eta}(\log N)^{-10}\). Suppose either dispersion certificate (A) or (B) holds with explicit constants. Then there exists an explicit \(N_0\) such that, for all \(N\ge N_0\) and every even \(2m\le 2N\),
\[
  R_8(2m;N)\ >\ 0.
\]
Consequently, binary Goldbach holds for all even integers \(2m\ge 2N_0\).
\end{theorem}
\begin{proof}[Proof sketch]
By Theorem~\ref{thm:goldbach-coercivity}, 
\[
R_8(2m;N)\ \ge\ \mathrm{main}(2m;N)\ -\ C\,\Defect_{\mathrm{med}}^{1/2}\ -\ \epsilon_{\mathrm{deep}}(N),
\]
with \(\mathrm{main}\ge c_8(2m)c_0\,N/\log^2N\), \(c_8(2m)\in\{1,\tfrac12\}\), \(\epsilon_{\mathrm{deep}}(N)\ll N/(\log N)^A\) for \(A\ge 6\). Under (A),
\[
\Defect_{\mathrm{med}}^{1/2}\ \le\ C_0^{1/2}\,N^{5/6}\,(\log N)^{3},
\]
and the medium-arc Cauchy factor contributes an additional \(C_{\mathrm{meas}}(Q,Q';N)^{1/2}\asymp N^{-1/6}(\log N)^{-5/2}\sqrt{\tfrac16\log N-2\log\log N}\). Altogether,
\[
|E_{\mathrm{med}}|\ \le\ \kappa\,N^{2/3}\,(\log N)^{1/2}\sqrt{\tfrac16\log N-2\log\log N},
\]
for an explicit \(\kappa=\kappa(C_0)\). Solving
\[
\kappa\,N^{2/3}\,(\log N)^{1/2}\sqrt{\tfrac16\log N-2\log\log N}\ \le\ \tfrac12\,(c_0/2)\,\frac{N}{\log^2N}
\]
yields an explicit \(N_0\) beyond which \(|E_{\mathrm{med}}|+\epsilon_{\mathrm{deep}}\le \tfrac12\,\mathrm{main}\), hence \(R_8(2m;N)>0\). Under (B), the implication (B)\(\Rightarrow\)(A) via completion+large sieve gives the same conclusion.
\end{proof}

\begin{remark}[Thresholds and feasibility]
With conservative constants, \(N_0\) may be large; however, any reduction in the log exponent within (A) (equivalently, any improvement in the energy exponent of (B)) decreases \(N_0\) monotonically. In particular, a bound of the form \(\mathcal D_{\mathrm{med}}\ll N^{5/3}(\log N)^6\) (or \(C\le 6\) in the fourth-moment log exponent) drives \(N_0\) into computational range.
\end{remark}

\begin{theorem}[Gap Theorem: Goldbach Dispersion Certificate]\label{thm:gb-gap}
Binary Goldbach reduces to either dispersion certificate (A) or (B). More precisely:
\begin{enumerate}
  \item (A)\(\Rightarrow\) Theorem~\ref{thm:gb-closure-cpm} unconditionally with explicit \(N_0\).
  \item (B)\(\Rightarrow\)(A) via completion and the additive large sieve (constant \(1\)), hence \(\Rightarrow\) Theorem~\ref{thm:gb-closure-cpm}.
\end{enumerate}
Thus, a proof of (A) or (B) with explicit constants closes the Goldbach proof within the CPM framework.
\end{theorem}

\begin{corollary}[Finite computational closure]\label{cor:finite-computational-closure}
If \(N_0\) produced by Theorem~\ref{thm:gb-closure-cpm} satisfies \(N_0\le X_{\mathrm{ver}}\) (the verified bound for Goldbach, e.g. \(4\times 10^{18}\)), then binary Goldbach holds for all even integers. If \(N_0\) is modestly larger (e.g. \(\le 2\times 10^{20}\)), a deterministic distributed verification protocol (segmented sieve + mod‑8 gate + wheel‑840) closes the residual finite range.
\end{corollary}

\subsection{Dispersion Inequality Needed to Close Goldbach}
\label{subsec:gb-dispersion-inequality}
\paragraph{Target inequalities.}
Either of the following two statements (equivalent in the CPM pipeline) suffices to close the proof unconditionally:
\begin{theorem}[MED--\(L^4\) dispersion inequality (target)]
\label{thm:medL4-target}
Fix the Goldbach schedule \(Q=N^{1/2}(\log N)^{-4}\), \(Q'=N^{2/3}(\log N)^{-6}\), \(U=V=N^{1/3}\), and a Vaaler window \(\eta\) with \(\Delta(\eta)\le C_{\eta}(\log N)^{-10}\). There exist absolute constants \(C_0>0\) and \(N_1\ge 3\) such that for all \(N\ge N_1\),
\[
  \int_{\mathfrak M_{\mathrm{med}}}\!\Big(|S(\alpha)|^4+|S_{\chi_8}(\alpha)|^4\Big)\,d\alpha\ \le\ C_0\,N^{5/3}\,(\log N)^{6}.
\]
\end{theorem}
\begin{theorem}[Type~II multiplicative energy inequality (target)]
\label{thm:typeII-energy-target}
Let \(M\) be dyadic with \(N^{1/3}\le M\le N^{2/3}\). Let \(a_m,b_n\) be Vaughan/Heath--Brown Type~II coefficients supported on \(m\sim M\), \(n\sim N/M\), with standard \(L^2\) sizes \(\sum_{m\sim M}|a_m|^2\ll M(\log N)^A\), \(\sum_{n\sim N/M}|b_n|^2\ll (N/M)(\log N)^B\) for fixed \(A,B\). Then for some absolute \(C_E>0\) and all sufficiently large \(N\),
\[
  \sum_{t\sim N}\ \Big|\sum_{\substack{mn=t\\ m\sim M,\,n\sim N/M}} a_m\,b_n\Big|^2\ \le\ C_E\,N\,(\log N)^{1+\varepsilon},
\]
uniformly in \(M\) (for fixed \(\varepsilon>0\)).
\end{theorem}
\begin{proposition}[Type~II \(\Rightarrow\) MED--\(L^4\)]
\label{prop:typeII-to-medL4}
Under the Goldbach schedule and smoothing above, Theorem~\ref{thm:typeII-energy-target} implies Theorem~\ref{thm:medL4-target} via Dirichlet completion on \(\alpha=a/q+\beta\) with \(Q<q\le Q'\) and the additive large sieve with constant \(1\). In particular, \(\)Theorem~\ref{thm:typeII-energy-target}\(\) suffices for Theorem~\ref{thm:gb-closure-cpm}.
\end{proposition}
\begin{remark}
Combining Theorem~\ref{thm:medL4-target} (or Theorem~\ref{thm:typeII-energy-target} through Proposition~\ref{prop:typeII-to-medL4}) with Theorem~\ref{thm:gb-closure-cpm} yields \(R_8(2m;N)>0\) for all even \(2m\le 2N\) beyond an explicit \(N_0\), thereby closing binary Goldbach after finite verification below \(N_0\).
\end{remark}
