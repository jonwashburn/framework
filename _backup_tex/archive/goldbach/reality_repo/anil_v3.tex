...
\begin{equation}
    \alpha^{-1} = 4\pi \cdot 11 - \ln \phi - \frac{103}{102\pi^5} \approx 137.0359991185
\end{equation}
This value is derived purely from the geometry of the ledger (seed $4\pi \cdot 11$, gap $\ln \phi$, curvature correction). It has no scaling freedom.

\AT{The proposed expression
\[
  \alpha^{-1} = 4\pi\cdot 11 - \ln\phi - \frac{103}{102\pi^5}
\]
is a pure number. That is, it is a priori just numerology unless three things are demonstrated:
\begin{enumerate}
  \item Each ingredient (\(4\pi\cdot 11\), \(\ln\phi\), \(103/(102\pi^5)\)) is uniquely and rigorously
