...
\[
  \|R(z)\,D\,R_a(z)\,I_{a,R}^*\|
  \ \le\ \|R(z)\|\,\|D(H_{a,R}+1)^{-1/2}\|\,\|(H_{a,R}+1)^{1/2}R_a(z)\|
  \ \le\ \frac{C_R\,C(z)}{|\Im z|}\,a.
\]
Combining the bounds yields the claim with $C(z,\Lambda;R):=|\Im z|^{-1}\big(C_{R,\Lambda}+C_R C(z)\big)$.
\end{proof}
...
\begin{theorem}[OS$\to$Wightman export (conditional)]\label{thm:os-wightman-export}
...
\subsection{Explicit AF-Style Scaling $\beta(a)$ and Tightness (optional; convergence conditional)}\label{subsec:beta-schedule}
...
\emph{Quantitative defect/projector inputs.} Assumption~\ref{assump:U2b} supplies the graph-defect bound $\|D_{a,L}(H_{a,L}+1)^{-1/2}\|\le C_{\rm gd}a$ and the low-energy projection control $\|(I-P_{a,L})E_H([0,\Lambda])\|\le C_\Lambda a$ needed for operator-norm NRC on fixed regions.
...
\[
  (H-z)^{-1} - I_{a,L}(H_{a,L}-z)^{-1} I_{a,L}^* 
   = (H-z)^{-1}(I-P_{a,L}) - (H-z)^{-1} D_{a,L} (H_{a,L}-z)^{-1} I_{a,L}^*.
\]
Moreover, assume $D_{a,L}(H_{a,L}+1)^{-1/2}$ extends by density to a bounded operator with $\|D_{a,L}(H_{a,L}+1)^{-1/2}\|\le C_{\rm gd}\,a$ (graph-defect input, uniform in $L$).
\end{proposition}
...
\begin{itemize}
  ...
  \[
    \|D_{a,L}(H_{a,L}+1)^{-1/2}\|\ \le\ C_{\rm gd}\,a.
  \]
...
Fix a bounded region $R\Subset\mathbb R^4$ and a cutoff $\Lambda\ge 1$. Let $(\mathcal H_a,H_a,J_a)$ denote a concrete fixed-slab discretization/comparison model on $R$ (cell-averaging injection $J_a$) for which one has proved quantitative defect/projector bounds, e.g. Proposition~\ref{lem:graph-defect-Oa} and Lemma~\ref{lem:low-energy-proj}, together with the low-energy approximation property of $J_a^*J_a$ (Lemma~\ref{lem:cell-avg-lowenergy}).

Assume there exist unitary identifications (on the time-zero local core supported in $R$)
\[
  V_{a,L}:\mathcal H_{a,L}\ \to\ \mathcal H_a
\]
such that the OS/GNS embedding framework $(I_{a,L},H_{a,L})$ is \emph{quantitatively quasi-unitarily equivalent} to the comparison model $(J_a^*,H_a)$ on $R$, with error $O(a)$ uniformly in $L$ (in energy/graph norms). Concretely, assume:
\begin{itemize}
  ...
  \]
  \item[(B2)] \textbf{Generator match (graph weighted).} There exists $C_{\rm br,H}>0$ such that for all sufficiently small $a$ and all $L$,
  \[
    \big\|\,(V_{a,L}H_{a,L}-H_aV_{a,L})\,(H_{a,L}+1)^{-1/2}\,\big\|\ \le\ C_{\rm br,H}\,a.
  \]
\end{itemize}
...
\]
as the sum of the comparison defect $(H J_a^*-J_a^*H_a)V_{a,L}$ and bridge remainders involving $(I_{a,L}-J_a^*V_{a,L})$ and $(V_{a,L}H_{a,L}-H_aV_{a,L})$. Combining the adjoint of Proposition~\ref{lem:graph-defect-Oa} with (B1)--(B2) yields the graph-defect bound $\|D_{a,L}(H_{a,L}+1)^{-1/2}\|\le C_{\rm gd}a$.
...
\]
The first term is controlled by Lemma~\ref{lem:cell-avg-lowenergy}. The second term is controlled by (B1) and the identity $(J_a^*V_{a,L})(J_a^*V_{a,L})^*=J_a^*J_a$ (since $V_{a,L}$ is unitary), so $P_{a,L}=I_{a,L}I_{a,L}^*$ is $O(a)$-close to $J_a^*J_a$ on the low-energy subspace. This yields $\|(I-P_{a,L})E_H([0,\Lambda])\|\le C_\Lambda a$.
\end{proof}
\noindent\emph{Remark (how one would verify (B1)--(B2)).} Conditions (B1)--(B2) are a quantitative quasi-unitary equivalence hypothesis between the OS/GNS embedding framework and a concrete comparison discretization on a fixed region. A standard route to proving such bounds is via a Kato-style \emph{form approximation} inequality on a common core after identifying the Hilbert spaces (cf.\ Section~\ref{NRC:form} and Theorem~\ref{NRC:form-thm}); we isolate (B1)--(B2) as the precise RG-grade input needed here.
...
\begin{itemize}
  ...
  \item U2b graph-defect bound $\|D_{a,L}(H_{a,L}+1)^{-1/2}\|\le C_{\rm gd}a$ and low-energy projection control $\|(I-P_{a,L})E_H([0,\Lambda])\|\le C_\Lambda a$: Assumption~\ref{assump:U2b} (U2b; currently conditional/target in the OS/GNS embedding framework).
...
Let $U\Subset\mathbb R^4$ be a bounded Lipschitz region and let $A$ be a smooth $\mathfrak{su}(N)$ connection on an open neighborhood of $\overline U$, with curvature $F$ and bounded covariant derivatives up to order $2$ on $U$. Assume the usual coupling normalization $\beta=\tfrac{2N}{g_0^2}$ and (to avoid boundary-layer discretization effects) that the curvature is supported away from the boundary: there exists $\delta>0$ such that $\mathrm{supp}(F)\Subset U_\delta:=\{x\in U:\mathrm{dist}(x,\partial U)>\delta\}$.

For lattice spacing $a\in(0,\delta]$, let $S^{(a)}_U(A)=\tfrac{\beta}{N}\sum_{p\subset U}\Re\,\Tr(I-U_p)$ be the Wilson plaquette action over plaquettes entirely contained in $U$ (with $U_p$ the plaquette holonomy induced by $A$), and let $S_U(A)=\tfrac{1}{2g_0^2}\int_U \Tr(F_{\mu\nu}F_{\mu\nu})\,dx$. Then for all sufficiently small $a>0$,
\[
