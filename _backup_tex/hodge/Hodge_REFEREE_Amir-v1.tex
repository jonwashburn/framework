...
% AI assistant layer (RED; Jan 2026 — distinct from prior blue edits)
\definecolor{editaiColor}{rgb}{0.80,0.00,0.00}
...
\item \editai{$N_{\mathrm{Car}}$: Carath\'eodory bound $N_{\mathrm{Car}}(n,p)$ on the number of calibrated directions needed to express a strongly positive form at a point (Lemma~\ref{lem:caratheodory-general}).}
...
\item \editai{$\varrho=\varrho(h)\in(0,1]$: \emph{transverse parameter radius factor}; template translation parameters are chosen in a ball of radius $\varrho\,h$ (hence per-face displacement improves to $\Delta_F\lesssim \varrho\,h^{2}$). In the borderline case $p=n/2$ we impose $\varrho=o(\varepsilon)$.}
\item \editai{$\delta$: transverse grid spacing / separation scale used to keep different sheets/templates disjoint; in the refined schedule we take $\delta\asymp \varrho\,\varepsilon\,h$ so that $B_{\,\varrho h}(0)$ still contains $\asymp \varepsilon^{-2p}$ available lattice sites.}
...
\]
\editai{Fix any sequence of mesh scales $h_j\to 0$. For each $j$, apply the constructions assembled in the H1/H2 packages (Propositions~\ref{prop:h1-package} and \ref{prop:transport-flat-glue-weighted}) together with global coherence (Proposition~\ref{prop:global-coherence-all-labels}) to obtain, at scale $h_j$:}
\begin{enumerate}
...
\editai{Choose also the transverse parameter radius factor $\varrho_j$ (defining the corner-exit template scale $s_j\asymp \varrho_j h_j$) such that $\varrho_j=o(\varepsilon_j)$ (required for the borderline $p=n/2$ case; see Lemma~\ref{lem:borderline-p-half}).}
\editai{In the corner-exit templates, the relevant footprint diameter satisfies $D_Q\asymp s_j$, so the within-label translation separation needed for disjointness is at the footprint scale $\delta_{\mathrm{sep},j}\asymp \varepsilon_j\,s_j$ (cf.\ Lemma~\ref{lem:sliver-stability} and Proposition~\ref{prop:finite-template}).}
\editai{Fix, for each interior face $F=Q\cap Q'$ in the mesh-$h_j$ cubulation, a tubular/flat face chart $\Omega_F\cong B^{2p}(0,c h_j)$ and linear face-parameter maps
...
\]
\editai{(In the borderline case $p=n/2$, this is ensured by imposing the refined displacement schedule $\varrho_j=o(\varepsilon_j)$ so that Lemma~\ref{lem:borderline-p-half} gives $\delta_j\to 0$.)}
...
\[
\mathcal F(\partial T^{\mathrm{raw}})
\ \editai{\lesssim\ \varrho\,h^2} \sum_Q \sum_{a\in\mathcal S(Q)} m_{Q,a}^{\frac{n-1}{n}}.
\]
...
with quantitative \editai{within-direction} disjointness, slope, and budget control.  Thus the hypothesis \textnormal{(H1)} in Theorem~\ref{thm:spine-quantitative} holds in this manuscript.
...
\editai{In the borderline case $p=n/2$, Lemma~\ref{lem:borderline-p-half} supplies $\mathcal F(\partial T^{\mathrm{raw}}_j)\to 0$ under the refined schedule $\varrho_j=o(\varepsilon_j)$, so Proposition~\ref{prop:glue-gap} still produces fillings $U_\epsilon$ of arbitrarily small mass for use in Proposition~\ref{prop:cohomology-match}.}
Thus the hypothesis \textnormal{(H2)} in Theorem~\ref{thm:spine-quantitative} holds \editai{for $p<n/2$ under the present estimates, and also for $p=n/2$ provided the refined displacement schedule $\varrho=o(\varepsilon)$ from Lemma~\ref{lem:borderline-p-half} is enforced.}
\end{proposition}
\end{editjonblock}
...
$\partial T_k=0$ and \editai{$[T_k]=A$ in $H_{2n-2p}(X;\Z)/\mathrm{Tor}$} such that the calibration defect tends to zero:
\[
...
Then there exists an integral $(2n-2p)$--cycle $T$ with $\partial T=0$ and \editai{$[T]=A$ in $H_{2n-2p}(X;\Z)/\mathrm{Tor}$} such that
\[
...
\begin{editaiblock}
\begin{proof}
Since \editai{$[T_k]=A$ in $H_{2n-2p}(X;\Z)/\mathrm{Tor}$} is fixed and $\psi$ is closed, the pairing is constant:
\(
...
If the calibration deficits \editai{$\Def_{\mathrm{cal}}(T_k):=\Mass(T_k)-\langle T_k,\psi\rangle$} satisfy $\Def_{\mathrm{cal}}(T_k)\to 0$,
...
=\langle T,\psi\rangle$.
\editai{On the other hand, $\mathrm{comass}(\psi)\le 1$ implies $|\langle T,\psi\rangle|\le \Mass(T)$ and hence $\langle T,\psi\rangle\le \Mass(T)$ (see e.g. \cite[Lemma~3.5]{HL82}).}
...
\begin{enumerate}
\item $\partial T_k=0$ and \editai{$[T_k]=\mathrm{PD}(m[\gamma])$ in $H_{2n-2p}(X;\Z)/\mathrm{Tor}$ (equivalently in $H_{2n-2p}(X;\Q)$)} for some fixed integer $m\ge 1$ (independent of $k$), and
\item the \emph{calibration defect} satisfies
\[
\Def_{\mathrm{cal}}(T_k)\ :=\ \Mass(T_k)-\langle T_k,\psi\rangle\ \longrightarrow\ 0.
\]
\end{enumerate}
Equivalently, since $\psi$ is closed and \editai{$[T_k]=\mathrm{PD}(m[\gamma])$ in $H_{2n-2p}(X;\Z)/\mathrm{Tor}$}, one has the exact pairing identity
\[
...
\editai{Throughout the referee-layer patches we reserve $m$ for the cohomology multiplier and use $N$ for the holomorphic tensor power, so the Bergman scale is always $N^{-1/2}$.}
\end{editaiblock}
...
Then there exists an integral $(2n-2p)$--cycle $T$ with $\partial T=0$ and \editai{$[T]=\mathrm{PD}(m[\gamma])$ in $H_{2n-2p}(X;\Z)/\mathrm{Tor}$} such that
\[
\Mass(T)=\langle T,\psi\rangle=\bigl\langle \mathrm{PD}(m[\gamma]),[\psi]\bigr\rangle.
\]
\editai{In particular, $T$ is $\psi$--calibrated.  For the K\"ahler calibration $\psi=\omega^{n-p}/(n-p)!$, calibrated integral currents are $d$--closed and positive of bidimension $(p,p)$ (see \cite{HL82}).  By King's theorem on positive closed locally integral currents \cite{King71}, $T$ is a holomorphic chain}
\[
T=\sum_j m_j[V_j],
\qquad m_j\in\mathbb{N},
\]
\editai{where $V_j\subset X$ are irreducible complex analytic subvarieties of codimension $p$.  If, moreover, $X$ is projective, then each $V_j$ is algebraic by Chow/GAGA (Remark~\ref{rem:chow-gaga}), so $[\gamma]\in H^{2p}(X;\Q)$ is an algebraic class.}
\end{theorem}
...
Applying Theorem~\ref{thm:realization-from-almost} to the fixed--class sequence $\{T_k\}$ yields an integral cycle $T$ with \editai{$[T]=\mathrm{PD}(m[\gamma])$ in $H_{2n-2p}(X;\Z)/\mathrm{Tor}$}
...
\begin{proof}[Proof (classical construction in charts)]
	...
	\editai{While $\Mass(\partial S^{\mathrm{raw}}_\varepsilon)$ may be large, the robust quantity for gluing is the \emph{flat norm} $\mathcal F(\partial S^{\mathrm{raw}}_\varepsilon)$.
	...
	\editai{To enforce the exact homology class $\mathrm{PD}(m[\gamma])$ (modulo torsion), one may refine the mesh so that the period contributions of individual plaques are uniformly tiny and then apply the same fixed-dimensional rounding / lattice-discreteness argument used later in Proposition~\ref{prop:cohomology-match}.}
...
Then there exists a number \editai{$N_{\mathrm{Car}}=N_{\mathrm{Car}}(n,p)$} (one may take $N_{\mathrm{Car}}=\dim(\mathcal V_x)+1=\binom{n}{p}^2+1$) such that for every $x\in X$ and every
...
Then there exists a number \editai{$N_{\mathrm{Car}}=N_{\mathrm{Car}}(n,p)$} (one may take $N_{\mathrm{Car}}=\dim_{\R}(\mathcal V_x)+1=\binom{n}{p}^2+1$) such that for every $x\in X$ and every
...
system $|L^{\otimes N}|$.
\editai{(Parameter convention: throughout the holomorphic/Bergman manufacturing block, $N$ denotes the tensor power of $L$ controlling the Bergman scale $N^{-1/2}$; the cohomology multiplier remains $m$ as in Definition~\ref{def:syr}.)}
...
\item[\textnormal{(ii)}] on the geodesic ball $B_{c\,\editai{N^{-1/2}}}(x)$
...
\quad\text{for all } y\in B_{c\,\editai{N^{-1/2}}}(x).
\]
\end{enumerate}
...
\]
Returning to the original variables gives the desired estimate on $B_{c\,\editai{N^{-1/2}}}(x)$ with $c:=\sigma$.
...
to $\Pi_{\alpha,j}$ on a ball of radius $c\,N^{-1/2}$. \editai{(Since there are only finitely many directions in the net, we may take a single $N$ large enough so that all of the resulting local graph estimates hold on the same Bergman radius $c\,N^{-1/2}$.)}
...
$\Pi_1,\ldots,\Pi_J\in \Gr_{n-p}(TQ)$ be constant $(n-p)$-planes \editai{(assumed complex/$\psi$-calibrated in the intended application)}, and let
...
\par\smallskip
\editai{Referee note: claiming pairwise disjointness for sheets built from \emph{different} directions is generally false (e.g. for $p=1$, non-parallel complex hypersurfaces intersect locally).  Disjointness is only enforced within each fixed-direction family $\{Y_j^a\}_a$.}
\end{editaiblock}
...
\begin{proof}
Let $k:=2n-2p$.
\editai{Choose a corner-exit scale $s=s(Q,\delta)\ll \mathrm{side}(Q)$ and, for each direction label $(Q,j)$, use the corner-exit translation template mechanism
...
\]
\editai{By shrinking the corner-exit scale $s$ (hence shrinking all $A_{Q,j}\asymp s^k$ uniformly) one increases the available total sheet count
...
$J(Q)\le \editai{N_{\mathrm{Car}}=N_{\mathrm{Car}}(n,p)}$ uniformly bounded.
...
\editai{Fix also a corner-exit footprint scale $s=s(h,\delta)\ll h:=\mathrm{side}(Q)$ and write $k:=2n-2p=2(n-p)$.  Using the corner-exit translation templates
...
\editai{(This is enforced in the vertex-template/corner-exit regime by the face-edit localization
...
Therefore, by Kantorovich--Rubinstein duality for $W_1$, \editai{(e.g.\ \cite{Villani03})}
\[
|B_F(\eta)|
...
\editai{Let $\mu$ be a finite Borel measure on $\R^{2p}$ supported in a ball of radius $O(\varrho h)$ and with total mass $\mu(\R^{2p})=N$.}
Let $\Phi,\Phi':\R^{2p}\to\R^{2p}$ be linear maps with $\|\Phi-\Phi'\|_{\mathrm{op}}\le C\,h$.
Then
\[
\editai{W_1(\Phi_\#\mu,\Phi'_\#\mu)\ \le\ C\,h\int_{\R^{2p}}\|y\|\,d\mu(y)\ \le\ C'\,\varrho\,h^2\,N.}
\]
\end{lemma}
...
\]
\editai{If $\operatorname{supp}\mu\subset B(0,C_0 \varrho h)$, then $\int\|y\|\,d\mu\le C_0 \varrho h\,\mu(\R^{2p})=C_0 \varrho h\,N$.}
...
Let $y_1,\dots,y_N\in\R^{2p}$ satisfy $\|y_a\|\le \editai{C_0\,\varrho\,h}$ and let $\Phi,\Phi':\R^{2p}\to\R^{2p}$ be linear maps with
$\|\Phi-\Phi'\|_{\mathrm{op}}\le C_1\,h$.
Define two multisets $u_a:=\Phi y_a$ and $u'_a:=\Phi' y_a$.
Then the index-wise matching satisfies
\[
\|u_a-u'_a\|\ \le\ \editai{C_0C_1\,\varrho\,h^2}\qquad\text{for all }a.
\]
...
the hypothesis of Corollary~\ref{cor:global-flat-weighted} holds with $\editai{\Delta_F=O(\varrho\,h^2)}$.
\end{lemma}
...
\editai{Let $\Omega\subset\R^{2p}$ be a bounded domain of diameter $\mathrm{diam}(\Omega)\le C\,\varrho h$.}
...
\[
\editai{W_1(\mu,\mu')\ \le\ r\cdot \mathrm{diam}(\Omega)\ \le\ C\,r\,\varrho\,h.}
\]
\end{lemma}
...
\[
\editai{W_1(\mu_{Q\to F},\mu_{Q'\to F})\ \lesssim\ \varrho\,h^2\,N_F,}
\]
...
\[
\editai{\mathcal F(\partial T^{\mathrm{raw}})\ \lesssim\ m\,\varrho\,h \;+\; O(\varepsilon\,m),}
\]
\begin{editaiblock}
...
$\mathbf y=(y_a)_{a=1}^{N_*}$ with $y_a\in \editai{B_{C_0\varrho h}(0)}\cap \delta_{\perp}\mathbb Z^{2p}$.
For $1\le N\le N_*$ write $\nu^{(N)}:=\sum_{a=1}^N\delta_{y_a}$ (cf.\ Proposition~\ref{prop:prefix-template-coherence}).

Let $\Phi_{Q,F},\Phi_{Q',F}:\editai{B_{C_0\varrho h}(0)}\to \Omega_F$ be the face maps from
...
\ \le\ \editai{C_\Phi\,C_0\,\varrho\,h^2}\,N_F.
\]
In particular, hypothesis \textnormal{(c)} in Proposition~\ref{prop:transport-flat-glue-weighted}
holds with
\[
\tau_F\ :=\ \editai{C_\Phi\,C_0\,\varrho\,h^2}\,N_F.
\]
\editai{Moreover, the same identity pairing yields the \emph{uniform pointwise} displacement bound
...
\]
Using Lemma~\ref{lem:template-displacement-edits} and $\editai{|y_a|\le C_0\varrho h}$ we get
\[
\big|\Phi_{Q,F}(y_a)-\Phi_{Q',F}(y_a)\big|
\ \le\ \|\Phi_{Q,F}-\Phi_{Q',F}\|_{\mathrm{op}}\,|y_a|
\ \le\ \editai{C_\Phi h\cdot C_0\varrho h\ =\ C_\Phi C_0 \varrho h^2}.
\]
...
\editai{Choose a nearest boundary point $t_0\in\partial D$ to $t$ and set $s:=\|t_0-t\|$ and $u:=(t_0-t)/\|t_0-t\|\in P^\perp$, so $t=t_0-s u$.
...
\[
\Mass(\Sigma_F(u_a))\ \le\ \editai{C\,m_{Q,a}^{\frac{k-1}{k}}},
\qquad
...
$\partial\Sigma_F(u_a)=0$ and $\Mass(\Sigma_F(u_a))\le \editai{C\, m_{Q,a}^{\frac{k-1}{k}}}$ (with $k:=2n-2p$)
for every slice meeting a cell $Q$.
Assuming in addition the schedule/face parameterization control of Lemma~\ref{lem:face-displacement} (so that $\editai{\Delta_F=O(\varrho\,h^2)}$ on all interior faces),
we obtain
\[
\mathcal F(\partial T^{\mathrm{raw}})\ \le\ \editai{C\,\varrho\, h^2} \sum_Q \sum_{a\in \mathcal S(Q)} m_{Q,a}^{\frac{k-1}{k}},
\]
...
\begin{proof}
\editai{Apply Proposition~\ref{prop:transport-flat-glue-weighted} facewise, then sum over interfaces and use the triangle inequality for $\mathcal F$.}
...
Under the additional assumptions $\editai{\Delta_F\le C\,\varrho\,h^2}$ and
$\Mass(\Sigma_F(u_a))+\Mass(\partial\Sigma_F(u_a))\lesssim m_a^{\frac{k-1}{k}}$ (with $k=2n-2p$),
we obtain
\[
\mathcal F(B_F)\ \editai{\lesssim\ \varrho\,h^2}\sum_{a\in\mathcal S(F)} m_{F,a}^{\frac{k-1}{k}}.
\]
...
\[
\mathcal F\!\left(\partial T^{\mathrm{raw}}\right)
\ \editai{\lesssim\ \varrho\,h^2}\sum_Q\ \sum_{a\in\mathcal S(Q)} m_{Q,a}^{\frac{k-1}{k}},
\]
as claimed.
\end{proof}
...
The corollary then yields $\mathcal F(\partial T^{\mathrm{raw}})\lesssim \editai{\varrho\,h^2}\sum_Q(M_Q/h)=\editai{\varrho\,h}\,\sum_Q M_Q\editai{\asymp m\,\varrho\,h}$,
...
\editai{if $D_Q:=\max_a\mathrm{diam}((P+t_a)\cap Q)$, then require $\delta:=10\,\varepsilon\,D_Q$ (in the corner-exit regime $D_Q\asymp s\asymp \varrho h$).}
\end{itemize}
\editai{Note: read this scaling remark only after the global coherence construction is established (Proposition~\ref{prop:global-coherence-all-labels}), to avoid any circular use of the conclusion.}
\end{editaiblock}
...
so Lemma~\ref{lem:face-displacement} gives \editai{$\Delta_F\lesssim \varrho\,h^2$}.
...
\[
\mathcal F(\partial T^{\mathrm{raw}})\ \editai{\lesssim\ \varrho\,h^2}\sum_Q\sum_{a\in\mathcal S(Q)} m_{Q,a}^{\frac{k-1}{k}}
\]
...
\]
If, across a shared interface, the corresponding face slices are displaced by $\|v\|=O(h^2)$ (as in the template/face-map variation \editai{estimate (cf.\ Lemma~\ref{lem:template-displacement})}),
...
\[
\mathcal F(B_F)\ \editai{\lesssim\ \varrho\,h^2}\,\mathrm{Bdry}(Q)\ \editai{\asymp\ \varrho\,h^2}\,M_Q^{\frac{k-1}{k}}\,N_Q^{\frac1k}.
\]
...
\[
\mathcal F(B_F)\ \le\ \editai{C\,\varrho\,h^2}\,\Biggl(\sum_{a=1}^{N_F} w_a\Bigl(\Mass(\Sigma_{\Phi_{Q,F}y_a})+\Mass(\partial\Sigma_{\Phi_{Q,F}y_a})\Bigr)
...
\editai{Since $\|y_a\|\le C_0\varrho h$ and $\|\Phi_{Q,F}-\Phi_{Q',F}\|_{\mathrm{op}}\le C_\Phi h$, we have $\|v_a\|\le C\,\varrho\,h^2$.}
...
\le \editai{C\,\varrho\,h^2}\Bigl(\Mass(\Sigma_{\Phi_{Q,F}y_a})+\Mass(\partial\Sigma_{\Phi_{Q,F}y_a})\Bigr).
\]
By subadditivity of $\mathcal F$ and summing over $a$ (with weights $w_a$),
\[
\mathcal F(B_F)\le \editai{C\,\varrho\,h^2}\sum_{a=1}^{N_F} w_a\,\Bigl(\Mass(\Sigma_{\Phi_{Q,F}y_a})+\Mass(\partial\Sigma_{\Phi_{Q,F}y_a})\Bigr).
\]
...
\]
\editai{Summing over the integer-weighted family of slices meeting $F$ it therefore suffices to bound
...
\mu_{Q'\to F}=(\Phi_{Q',F})_\#\nu',
\]
where $\nu$ and $\nu'$ are integer-weighted discrete measures supported in $\editai{B_{C_0\varrho h}(0)}\subset\R^{2p}$ and the face maps satisfy
...
Then
\[
\mathcal F(B_F^{\wedge})\ \le\ \editai{C\,\varrho\,h^2}\Bigl(\Mass(\partial S_Q\llcorner F)+\Mass(\partial S_{Q'}\llcorner F)\Bigr)\ +\ C_{\angle}\,\varepsilon\,M_F,
\]
and, moreover,
\[
...
Fix, for each direction label, an \emph{ordered} master template of transverse atoms $(y_a)_{a\ge 1}\subset \editai{B_{C_0\varrho h}(0)}\subset\R^{2p}$.
For example, Lemma~\ref{lem:sphere-quantize-nested} produces a nested ordered sequence on a sphere (uniform density), and scaling embeds it into $\editai{B_{C_0\varrho h}(0)}$.
...
Fix an ordered template of transverse atoms $(y_a)_{a\ge 1}\subset \editai{B_{C_0\varrho h}(0)}\subset\R^{2p}$ and define prefixes
\[
...
Fix an \emph{ordered} master template of transverse atoms $(y_a)_{a\ge 1}\subset \editai{B_{C_0\varrho h}(0)}\subset\R^{2p}$.
...
\[
\mathcal F(\partial T^{\mathrm{raw}})\ \editai{\lesssim\ \varrho\,h^2}\sum_Q\sum_{a\in\mathcal S(Q)} m_{Q,a}^{\frac{k-1}{k}}\ +\ O(\varepsilon\,m),
\qquad k:=2n-2p,
\]
...
\editai{(In the vertex-template activation route, Proposition~\ref{prop:vertex-template-face-edits} provides the same $O(h)$ face-edit conclusion without invoking checkerboarding.)}
\end{editaiblock}
...
Fix $\varepsilon>0$ and choose a holomorphic tensor power \editai{$N_{\mathrm{hol}}\ge N_1(\varepsilon)$} (Lemma~\ref{lem:bergman-control}) with
$\mathrm{diam}(Q)\le c\,\editai{N_{\mathrm{hol}}^{-1/2}}$.
...
With \editai{$N_{\mathrm{hol}}\ge N_1(\varepsilon)$} and $\mathrm{diam}(Q)\le c\,\editai{N_{\mathrm{hol}}^{-1/2}}$, the Bergman-scale $C^1$ control and the holomorphic finite-template
...
\editai{Given the per-cell mass budgets $M_{Q,i}$, choose any vertex split $M_{Q,v,i}\ge 0$ with $\sum_{v\in\mathrm{Vert}(Q)}M_{Q,v,i}=M_{Q,i}$ (e.g.\ equal split).}
\editai{For each $(Q,v,i)$ let $\mu_{Q,v,i}>0$ denote the common per-piece mass scale in $Q$ for the $v$-anchored pieces of label $i$ (as in Proposition~\ref{prop:vertex-template-mass-matching}), and define}
\[
\editai{N_{Q,v,i}\ :=\ \Bigl\lfloor \frac{M_{Q,v,i}}{\mu_{Q,v,i}}\Bigr\rceil.}
\]
\editamir{Then item \textnormal{(a)} is exactly Proposition~\ref{prop:vertex-template-mass-matching} applied label-by-label.}
\editai{Item \textnormal{(b)} follows by applying Lemma~\ref{lem:slow-variation-rounding} (and the $0$--$1$ stability Lemma~\ref{lem:slow-variation-discrepancy})
...
\]
\begin{editaiblock}
\editai{In particular, since $F$ is $C^1$ and $\partial_wF(u,g(u))$ is invertible for all $u\in U_u$, the implicit function theorem implies $g\in C^1(U_u)$; hence the displayed bound holds for every $u\in U_u$.}
\end{editaiblock}
...
\editai{Moreover, the construction in the proof yields the quantitative bound $\varepsilon_N\le C_R\,e^{-c N}$ for some $c>0$ (uniform over $\ell$ with $|a|+|b|\le 1$ and over charts in a fixed finite atlas). In particular, $\varepsilon_N=o(N^{-1/2})$.}
\end{lemma}
...
\begin{proposition}[\editai{Cell-scale linear-model complete intersections are single-sheet graphs}]\label{prop:cell-scale-linear-model-graph}
\iffalse
...
\fi
\editai{Fix a holomorphic chart identifying a neighborhood of a cell $Q$ with a domain in $\C^{n}=\C^{n-p}\times\C^{p}$ with coordinates $z=(u,w)$, and assume $Q\subset B_{R/\sqrt N}(0)$ for some fixed $R$. Assume moreover that the cell diameter satisfies $h\asymp N^{-1/2}$.}
Let $t\in\C^p$ satisfy $|t|\le c\,h$ (with \editai{$h\asymp N^{-1/2}$}).
...
\end{enumerate}
\editai{Assume moreover that $\varepsilon>0$ is chosen small enough (depending only on $k$ and $\Lambda$) so that Proposition~\ref{prop:holomorphic-corner-exit-g1g2} applies to every pair $(E_a,Y^{(a)})$.}
...
\editai{This is legitimate because the smallness threshold in Proposition~\ref{prop:holomorphic-corner-exit-g1g2} depends only on $(k,\Lambda)$ (and the slope bound), not on $a$.}
\end{editaiblock}
\end{proof}
...
is stable), and the natural analytic resolution scale in K\"ahler quantization is the Bergman scale \editai{$N^{-1/2}$}.
...
\]
\editai{If $\|t_1-t_2\|\ge 10\,\varepsilon\,\max\{D_1,D_2\}$, then the tubular neighborhoods $\mathcal N_{\varepsilon D_1}(P+t_1)$ and $\mathcal N_{\varepsilon D_2}(P+t_2)$ are disjoint, hence so are $Y_1\cap Q$ and $Y_2\cap Q$.}
\end{editaiblock}
\end{proof}
...
Fix $\varepsilon>0$ and choose a holomorphic tensor power \editai{$N_{\mathrm{hol}}\ge N_1(\varepsilon)$} as in Lemma~\ref{lem:bergman-control},
with $N_{\mathrm{hol}}$ large enough that
\[
\mathrm{diam}(Q)\ \le\ c\,\editai{N_{\mathrm{hol}}^{-1/2}},
\]
where $c>0$ is the universal constant in Lemma~\ref{lem:bergman-control}.\editai{ By increasing $N_{\mathrm{hol}}$ if necessary, we may assume in fact $\mathrm{diam}(Q)\le (c/2)\,N_{\mathrm{hol}}^{-1/2}$; then $Q\subset B_{(c/2)\,N_{\mathrm{hol}}^{-1/2}}(x)$ for every $x\in Q$.}
...
\begin{enumerate}
\item[\textnormal{(i)}] $Y^a$ is a $C^1$ graph over $P+t_a$ with slope \editai{$\le C_{\mathrm{graph}}\varepsilon$} (hence $\angle(T_yY^a,P)\le C\varepsilon$);
...
$s_{a,1},\dots,s_{a,p}\in H^0(X,L^{N_{\mathrm{hol}}})$ whose first-order jets at $x_a$ realize the covectors $\lambda_i$. \editai{This applies on $Q$ since $Q\subset B_{(c/2)\,N_{\mathrm{hol}}^{-1/2}}(x_a)\subset B_{c\,N_{\mathrm{hol}}^{-1/2}}(x_a)$ (because $x_a\in Q$ and we ensured $\mathrm{diam}(Q)\le (c/2)\,N_{\mathrm{hol}}^{-1/2}$).}
Let $Y^a:=\{s_{a,1}=\cdots=s_{a,p}=0\}$.
\editai{Let $C_{\mathrm{graph}}$ be the slope constant from Lemma~\ref{lem:graph-from-grad}.} Then Lemma~\ref{lem:graph-from-grad} gives (i) with slope $\le C_{\mathrm{graph}}\varepsilon$, and Lemma~\ref{lem:sliver-stability}(i) gives (iii) with the same slope parameter. \editai{Applying Lemma~\ref{lem:sliver-stability}(ii) with $\varepsilon$ replaced by $C_{\mathrm{graph}}\varepsilon$ and using the separation assumption $\|t_a-t_b\|\ge 10\,C_{\mathrm{graph}}\varepsilon\,D_Q$ yields (ii).}
\end{proof}
\end{editblock}
...
\editai{Suppose that} each $E_a$ is a $\Lambda$--fat corner--exit simplex in the sense of Proposition~\ref{prop:holomorphic-corner-exit-g1g2} (so it comes with designated exit faces
...
\lesssim_{k,\Lambda,\varepsilon} v_{E_a}^{(k-1)/k},
\]
and summing over $a$ gives the stated $L^1$ bound.\end{editaiblock}
\end{proof}
\fi
...
\editai{Suppose that} each $E_a$ is a $\Lambda$--fat corner--exit simplex footprint in the sense of
...
If the mesh satisfies $h\lesssim \editai{N_{\mathrm{hol}}^{-1/2}}$ with a small enough constant, then the Bergman ball $B_{c\,\editai{N_{\mathrm{hol}}^{-1/2}}}(x_a)$ contains the entire
...
\editai{The relative slow-variation form $|N_Q-N_{Q'}|\le C\,h\,N_Q$ is automatic once $N_Q\gtrsim h^{-1}$, i.e.\ once the additive rounding error is negligible compared to the local count.}
\editai{In the sliver/corner-exit regime at fixed cohomology multiplier $m$, this lower bound is achieved by shrinking the per-piece mass scale $A\asymp s^{k}$ (with $s\ll h$), so that $N_Q\sim M_Q/A$ is large even though the total budget $M_Q\asymp m\,h^{2n}$ is $O(m)$.}
\end{remark}
\begin{editblock}
...
\]
\editai{(Here $S_Q:=\sum_{j=1}^{N_{\mathrm{Car}}}\sum_{a=1}^{N_{Q,j}}[Y_{Q,j}^a]\llcorner Q$ is the local integral current built from the sheet pieces, and $S_Q(\Theta_\ell):=\int_{S_Q}\Theta_\ell=\sum_{j,a}\int_{Y_{Q,j}^a\cap Q}\Theta_\ell$.)}
...
\]
\editai{Taking boundaries gives $\partial R=\partial^2S-\partial^2Q=0$, so $R$ is an integral cycle.  Moreover $R=\partial(S-Q)$, hence $R$ bounds in $X$ by an \emph{integral} $k$--current.}
...
Federer--Fleming isoperimetric inequality.  They are \editai{not required} to be $\psi$--calibrated, nor to have any positivity/type property.
...
and represents $\mathrm{PD}(m[\gamma])$\editai{ in $H_{2n-2p}(X;\R)$ (equivalently in $H_{2n-2p}(X;\Z)/\mathrm{Tor}$).}
\end{enumerate}
...
\editai{Set $T_k:=T_{1/k}$ and write $U_{1/k}$ for the gluing correction from Proposition~\ref{prop:glue-gap}.  By Proposition~\ref{prop:almost-calibration} we have}
\begin{editaiblock}
\[
...
\]
\end{editaiblock}
\editai{and $\Mass(U_{1/k})\to 0$ as $k\to\infty$.}
\editai{By construction, $T_k$ is obtained from the raw current $T^{\mathrm{raw}}_{1/k}$ by the gluing correction from Proposition~\ref{prop:glue-gap}:
...
\item \editai{By Lemma~\ref{lem:flat_limit_of_cycles_is_cycle}, since $T_k\to T$ in the flat norm (hence weakly) and $\partial T_k=0$, we have $\partial T=0$; thus $T$ is an integral $(2n-2p)$-cycle;}
...
\editai{One can interpret $\Def_{\mathrm{cal}}(T_k)$ as measuring, in an averaged sense, how far the approximate tangent planes of $T_k$ are from the calibrated Grassmannian determined by $\psi$.  This can be formalized using standard tangent-measure/varifold language, but it is not used in the argument below.}
\iffalse
...
\]
\editai{By Proposition~\ref{prop:almost-calibration}, the calibration defect satisfies $\Def_{\mathrm{cal}}(T_k)=\Mass(T_k)-\int_{T_k}\psi\le 2\,\Mass(U_{1/k})\to 0$, hence $\Mass(T_k)\to m\int_X\beta\wedge\psi$.}
Combining with \eqref{eq:mass-lsc} and the calibration inequality $\langle T,\psi\rangle\le \Mass(T)$ yields $\Mass(T)=\langle T,\psi\rangle$, so $T$ is $\psi$-calibrated.\editai{ (This is exactly Lemma~\ref{lem:limit_is_calibrated} in this setting.)}
...
\editai{$[T_k]=\mathrm{PD}(m[\gamma])$ in $H_{2n-2p}(X;\Z)/\mathrm{Tor}$ (equivalently in $H_{2n-2p}(X;\Q)$)} for some fixed $m\in\N$ independent of $k$, such that
\[
\Def_{\mathrm{cal}}(T_k)=\Mass(T_k)-\langle T_k,\psi\rangle\ \longrightarrow\ 0.
\]
...
\(
\editai{[T_\epsilon]=\mathrm{PD}(m[\gamma])\in H_{2n-2p}(X;\Z)/\mathrm{Tor}\quad\text{(equivalently in }H_{2n-2p}(X;\Q)\text{)}.}
\)
...
\editai{$[T_k]=\mathrm{PD}(m[\gamma^+])$ in $H_{2n-2p}(X;\Z)/\mathrm{Tor}$ (equivalently in $H_{2n-2p}(X;\Q)$)} and $\Def_{\mathrm{cal}}(T_k)\to 0$.
Applying Theorem~\ref{thm:syr} to this SYR data produces a $\psi$--calibrated integral cycle
\(T=\sum_j m_j[V_j]\)
with \editai{$[T]=\mathrm{PD}(m[\gamma^+])$ in $H_{2n-2p}(X;\Z)/\mathrm{Tor}$ (equivalently in $H_{2n-2p}(X;\Q)$)}, hence an analytic cycle representative of $m[\gamma^+]$.
...
\editai{This is a standard consequence of Chow's theorem (for projective space) together with Serre's GAGA.
...
\editai{By Lemma~\ref{lem:gamma-minus-alg}, $\gamma^-$ is algebraic: it is represented (in $H^{2p}(X,\Q)$) by a rational multiple of a complete intersection $Z^-\subset X$.}
...
and near/off-diagonal Bergman kernel expansions with controlled derivatives at the \editai{$N^{-1/2}$} scale.%
