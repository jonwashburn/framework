...
The core idea is simple: many domain models can be expressed as an \emph{8-phase} dynamical system with (i) a state, (ii) a discrete phase in $\Fin{8}$, (iii) a one-tick step function, and (iv) optional cost and admissibility constraints. We then connect models across domains using \emph{bridges}: structure-preserving maps that keep phases aligned and commute with the step function. From these ingredients we obtain (1)~a general synchronization theorem: any collection of 8-phase systems that advance phase by one tick remains phase-aligned under iteration; (2)~a compositional bridge calculus that supports reusable cross-domain projections; (3)~worked case studies including an 8-tick conservation invariant in a semantic virtual machine and a structured 20-token semantic alphabet with a proved bijection to the 20 amino acids; and (4)~a built-in falsifiability discipline in which every empirical hypothesis ships with a precise falsifier and a proved incompatibility (so the hypothesis and its falsifier cannot both hold).
...
There are $2^3 = 8$ distinct binary patterns of length 3. A cyclic sequence of length 8 (a de Bruijn cycle of order 3) contains each of these 3-bit patterns exactly once as a sliding window; no shorter cycle can do so. This is why $\Fin{8}$ is the canonical shared phase space in the framework.
...
\noindent\textbf{Status note (this repo snapshot).} In our RSFold V2 benchmark snapshots, the preregistered dataset \texttt{obs\_1VII} \emph{does} trigger the falsifier under the default preregistration budget (see \S\ref{sec:example-falsifier}). This is a concrete example of the discipline working: a specific instantiation of a cross-domain claim can be cleanly refuted without moving goalposts.

This design requirement forces intellectual honesty: you cannot state a hypothesis without specifying how it could fail. The falsifier registry (\S\ref{sec:falsifiability}) catalogs all hypothesis--falsifier pairs and their intended tests.
...
\midrule
\textbf{Formal modules} & 7,500+ \\
\textbf{Proof holes (placeholders)} & 0 \\
Explicit axioms & 225 declarations \\
Implementation milestones & 18 completed \\
\textbf{Build status} & passes (see build appendix) \\
\bottomrule
\end{tabular}
...
The key takeaway is transparency: assumptions are explicit, and the implementation has no hidden ``to-do'' proof gaps. The axiom count includes standard mathematical foundations and explicitly tagged domain assumptions.
...
\begin{enumerate}
  ...
  \item \textbf{Key results are proved within the framework}: The WToken--AminoAcid bijection (as a statement about the token system), the LNAL neutrality invariant, and the universal synchronization theorem are proved as formal theorems in the reference implementation.
...
If multiple 8-phase systems advance phase by one tick per step, then any initial phase alignment between them is preserved under iteration.
\end{theorem}
...
We model ``context'' as a sliding window of length $k$ over an alphabet $\Sigma$. A cyclic sequence of period $p$ induces exactly $p$ such windows (wrapping around). The question is:
\emph{what is the smallest period $p$ that covers all $|\Sigma|^k$ possible windows?}

For the Octave System we take $\Sigma=\{0,1\}$ and $k=3$ (three binary features). The number $8$ then falls out of the count of possible contexts: $|\Sigma|^k = 2^3 = 8$.
...
\textbf{Lower bound} ($p \geq 8$): A period-$p$ cyclic sequence has exactly $p$ length-3 windows (with wrap-around). To cover all 8 distinct 3-bit patterns, we need $p \ge 8$.

\textbf{Upper bound} ($p \leq 8$): A period-8 de Bruijn cycle of order 3 covers all 3-bit patterns exactly once. One explicit example is:
\[
  \texttt{00010111}
\]
Its wrap-around length-3 windows are:
\[
  \{000,001,010,101,011,111,110,100\},
\]
which exhaust the 8 possible 3-bit patterns. \qedhere
\end{proof}

\subsubsection{Reference implementation (formal check)}

A mechanically checked version of this argument exists in the reference implementation (including the explicit witness cycle and a finite verification of coverage). In this concept-first paper we treat that as supporting reproducibility detail; see the repository / appendices for the formal statement.
...
This layer is the most direct bridge between the abstract interface and its combinatorial origin. It makes the ``Why 8?'' argument concrete: the phase index is interpreted as a position in a period-8 pattern-cover cycle (a de Bruijn cycle for 3-bit contexts).

\subsubsection{State Space}

Conceptually, the state records:
\begin{itemize}
  \item a phase index in $\Fin{8}$ (which 3-bit context we are visiting in the cycle), and
  \item an unbounded tick counter (how many steps have elapsed).
\end{itemize}
We omit the concrete record definition here; it is straightforward and available in the reference implementation.
...
\begin{itemize}
  ...
  \item \textbf{step} increments both the pattern index (mod 8) and the tick counter,
...
This layer is the most explicit ``combinatorial clock'' in the system: it provides a concrete interpretation of the abstract phase index as a traversal of all 3-bit contexts in a minimal cycle. Other layers may adopt the same 8-phase interface for different reasons, but this layer shows the interface is not arbitrary.
...
The VM carries a bundled admissibility predicate (\texttt{VMInvariant}) that enforces simple boundedness constraints (e.g., small discrete ranges for certain registers and correct 8-window indexing). The exact predicate is defined in the reference implementation; for the concept-first narrative, the key point is that admissibility is explicit and intended to be preserved by stepping.
...
\begin{itemize}
  ...
  \item \textbf{cost}: a per-instruction cost model (used as a strain proxy).
  \item \textbf{admissible}: the VM invariant predicate.
\end{itemize}
Different programs yield different instances but share the same 8-phase interface.

\subsubsection{Properties}

For the reference implementation, the key interface properties are established: stepping advances phase by one tick (via the breath counter), and the admissibility predicate is preserved by stepping (so ``valid'' executions remain valid).
...
\begin{itemize}
  ...
  \item \textbf{admissible}: the position remains within the trajectory bounds.
\end{itemize}
...
\subsubsection{Empirical Status (Current)}

\noindent\textbf{Important scope note.} The BiologyQualiaLayer is presented here as a \emph{modeling choice} (a discrete proxy state space plus a strain-like cost), not as an established biochemical mechanism.

\noindent In the current repository snapshot we have implemented an empirical harness for codon-derived ``qualia strain'' features and tested them against curated folding-rate datasets. The interaction feature \(\text{strain} \times \text{contact order}\) can be predictive out-of-sample, but randomization/null controls currently match or exceed the observed performance; this means the current \(Q_6\) mapping is \emph{not} yet validated as a unique mechanism by those data. In addition, most classic folding-rate datasets are \emph{in vitro refolding} experiments where codons are not present; codon-level causality must therefore be tested in translation-coupled contexts. For that reason, our highest-signal next tests are translation-coupled falsifiers (synonymous-variant outcomes and codon IR spectroscopy) rather than additional in-vitro refolding correlations.
...
\bottomrule
\end{tabular}
\caption{Complete bridge inventory. Each listed bridge is defined and checked in the reference implementation.}
...
At the completion of a window (when \texttt{winIdx8 = 7} and execution continues), the scheduler resets the window accumulator:
\[
...
From a suitably aligned start state, neutrality holds at every 8th tick:
\[
  \forall n\in\mathbb{N},\quad ((\texttt{lStep}\;P)^{8n+7}(s)).\texttt{winSum8}=0.
\]
...
In the reference implementation, this is expressed as a checkable specification over data structures, with the incompatibility verified once and then reused across experiments.
...
Incompatible & $H \land F$ is provably false (checked in the reference implementation) \\
...
\paragraph{Empirical note.} As implemented in the current RSFold sonification pipeline (roughness-based consonance), this specific claim is \emph{refuted} by the preregistered three-seed snapshot \texttt{obs\_1VII} (see Table~\ref{tab:sonify_examples}). We retain it here as a worked example because it demonstrates the intended behavior of the hypothesis--falsifier mechanism: the framework can produce decisive negative results and force revisions to the mapping or the claim.
...
The incompatibility argument is immediate: if \(H\) asserts ``preregistered implies \(V(\mathrm{obs}) \le k\)'' while \(F\) asserts ``preregistered and \(V(\mathrm{obs}) > k\)'', they cannot both hold for the same \(\mathrm{obs}\). This incompatibility is checked once in the reference implementation.
...
\begin{tikzpicture}[scale=0.8,
  ...
  \node[draw, dashed, rounded corners=4pt, fill=octPurple!10, font=\footnotesize, align=center, thick, minimum width=2.8cm] (proof) at (0, 4) {
    \textbf{Checked:}\\$H \land F \Rightarrow \bot$\\{\tiny (incompatibility)}
  };
  ...
  \node[draw, rounded corners=4pt, fill=octBlue!6, font=\footnotesize, align=center] at (0, -3.8) {
    \textbf{12 hypothesis--falsifier pairs} across 6 domains, all checked
  };
...
\end{tikzpicture}
\caption{\textbf{Figure 6: The Falsifiability Framework.} Our approach to empirical claims: every hypothesis $H$ is paired with a falsifier $F$, and the incompatibility $H \land F \Rightarrow \bot$ is checked in the reference implementation \emph{before} experiments run. When data arrives, either $F$ holds (the hypothesis is refuted under its preregistered criterion) or $\neg F$ holds (the hypothesis survives for now).}
...
To sanity-check that the falsifier can trigger and can fail to trigger, the reference implementation includes:
\begin{itemize}
  \item a small ``passes'' example dataset (\texttt{obs\_1PGB}),
  \item a synthetic negative control (\texttt{obs\_negControl}), and
  \item a real benchmark snapshot that triggers the falsifier (\texttt{obs\_1VII}, from RSFold V2).
\end{itemize}
...
\midrule
obs\_1PGB (included example; 3-seed snapshot) & 3 & 0 & Passes (0 $\leq$ 0) \\
obs\_negControl (synthetic negative control) & 3 & 3 & Fails (3 $>$ 0) \\
obs\_1VII (RSFold V2 snapshot; real benchmark) & 3 & 3 & Fails (3 $>$ 0) \\
\bottomrule
\end{tabular}
\caption{Illustrative verification artifacts (included in the reference implementation).}
\label{tab:sonify_examples}
\end{table}
...
\begin{tabular}{llllc}
\toprule
\textbf{Domain} & \textbf{Hypothesis} & \textbf{Falsifier} & \textbf{Test Type} & \textbf{Incompat.} \\
\midrule
...
\bottomrule
\end{tabular}
\caption{Complete falsifier registry. For each pair, incompatibility (\(H \land F = \bot\)) is checked in the reference implementation.}
...
Our contribution: the incompatibility check \(H \land F = \bot\) is \textbf{checked in the reference implementation}, preventing moving goalposts.
...
\midrule
Formal verification & Flyspeck, Feit-Thompson & Methods & + Explicit H/F discipline for empirical claims \\
...
Sonification & Auditory display & Perception & + Preregistered mapping + negative controls \\
\bottomrule
\end{tabular}
...
\begin{enumerate}
  ...
  \item \textbf{A falsifiability discipline} for empirical hypotheses: each hypothesis comes with a preregistered refutation condition (a falsifier) and a checked incompatibility \(H \land F = \bot\), so the target cannot be moved after seeing data.
\end{enumerate}
...
\textbf{Our advance}: We emphasize a preregistered, falsifiable evaluation pattern: define a mapping from observations to an audio representation, and then test an explicit cross-domain ordering claim with a paired falsifier (Section~\ref{sec:falsifier-registry}). In our case study the hypothesis is that lower RMSD should tend to co-occur with higher consonance (under a specified sonification and scoring rule); the falsifier is that too many pairwise order violations occur. In the current repo snapshot this falsifier \emph{does} trigger on \texttt{obs\_1VII}, demonstrating that the roughness-based consonance metric is not a reliable quality signal without further refinement.
...
\begin{itemize}
  ...
  \item \textbf{Checked incompatibility} (\(H \land F = \bot\)) in the reference implementation
  \item \textbf{Positive and negative controls} included as reference artifacts (example datasets / negative controls)
\end{itemize}
...
\textbf{Metric} & \textbf{Value} \\
\midrule
Reference modules & 7,500+ \\
Proof holes (in reference) & 0 \\
Axioms (explicit) & 225 \\
Bridge instances & 15+ \\
Proof LOC (approx.) & $\sim$50,000 \\
\bottomrule
\end{tabular}
...
structure WaterClockState where
  ...
  phaseAngle : Real           -- libration phase (0 to 2pi)
\end{lstlisting}
...
\subsubsection{Admissibility}
\begin{lstlisting}
admissible := fun s => s.intensity > 0 /\ s.phaseAngle < 2pi
\end{lstlisting}
...
structure ConsciousnessState where
  ...
  globalTheta : Real          -- global phase field (0 to 2pi)
...
admissible := fun s => 
  ...
  0 <= s.globalTheta /\ s.globalTheta < 2pi
\end{lstlisting}
...
structure PhysicsScaleState where
  rung : Int                  -- current rung on phi-ladder (can be negative)
  scaleValue : Real           -- physical scale = phi^rung * baseScale
...
\midrule
Biology $\to$ Consciousness & via Phase & \texttt{biologyToPhase} $\circ$ \texttt{phaseToConsciousness} \\
Water $\to$ Physics & via Phase & \texttt{waterToPhase} $\circ$ \texttt{phaseToPhysics} \\
LNAL $\to$ Biology & via Meaning & \texttt{lnalToMeaning} $\circ$ \texttt{meaningToBiology} \\
Pattern $\to$ Water & via Phase & \texttt{patternToPhase} $\circ$ \texttt{phaseToWater} \\
\bottomrule
\end{tabular}
...
theorem wtoken_amino_bijection : Function.Bijective wtoken_to_amino := by
  ...
  . -- Surjectivity: forall a, exists w, wtoken_to_amino w = a
    intro a
...
theorem wtoken_amino_leftInverse : 
    forall w, amino_to_wtoken (wtoken_to_amino w) = w := by
  intro w; cases w; simp [wtoken_to_amino, amino_to_wtoken]

theorem amino_wtoken_rightInverse :
    forall a, wtoken_to_amino (amino_to_wtoken a) = a := by
  intro a; cases a; rfl
\end{lstlisting}
\end{proof}
...
theorem neutral_every_8th_from0 (P : LProgram) (s : LState) (hInv : EightTickInvariant s) 
    (n : Nat) : (lStep P)^[8*n + 7] s |>.winSum8 = 0
