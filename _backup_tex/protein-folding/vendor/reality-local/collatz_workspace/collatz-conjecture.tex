...
We attach two finite CSVs:
\begin{itemize}
\item \emph{Anchor windows (targets):} a list of residues in $W$ together with witnesses $(j,K,s,A,B,N_0)$ (file: \texttt{routeA\_2\^{}18\_target\_windows.csv});
\item \emph{Funnel:} for every odd residue $R\bmod 2^{18}$ the minimal $F(R)\le 16$ with $T^{\,F(R)}(R)\in W$ (file: \texttt{routeA\_2\^{}18\_funnel\_L16\_min\_lengths.csv}).
\end{itemize}
For illustration: $j_{\max}=10$, $L=16$. We set
\[
J^\*:=j_{\max}+L,\qquad
N_0^\*:=\left\lceil 2^{L}\cdot \max_{(j,s,r)} N_0\right\rceil.
\]
...
\end{cases}
\]
eventually reaches $1$ for every positive integer $n$. This paper presents a finite-certificate framework and a conditional theorem: given explicit finite CSV datasets (a window catalog and a funnel map) satisfying checkable arithmetic conditions, the conjecture follows.

The method has three components. First, the dynamics are reduced to the accelerated odd-step map $T$, and for this map we use a finite collection of exact $2$-adic \emph{window witnesses}: valuation patterns that force a uniform $j$-step affine relation
\[
T^{\,j}(n) = A n + B,\qquad A < 1,
\]
on entire congruence classes. Second, at a fixed modulus $2^M$ (e.g.\ $M=18$) one seeks, for every odd residue, either immediate membership in these windows or a short \emph{funnel} of bounded length that reaches one. Third, using the logarithmic height $E(n)=\ln n$ as an energy, one obtains a uniform descent: above a finite threshold, the orbit strictly decreases within a bounded number of odd steps.

All of the information needed for the argument is encoded in finite CSV files: a list of window witnesses and a list of minimal funnel lengths for each residue modulo $2^M$. From these finite data one computes explicit global constants $J^{\ast}=j_{\max}+L$ and
\[
N_0^{\ast} \;=\; \left\lceil 2^{L}\,\max N_0 \right\rceil,
\]
where $j_{\max}$ is the maximum window length, $L$ is the maximum funnel length, and $N_0$ ranges over window thresholds. For illustration, one may target $M=18$ with windows of length $\le 10$ and funnels of length $\le 16$; once a verified certificate achieving such parameters is supplied, the descent bound $J^{\ast}$ and the finite threshold $N_0^{\ast}$ follow mechanically, and the small cases $n\le N_0^{\ast}$ are checked by finite computation. The entire argument is finitely checkable once the datasets are supplied.
...
In a prospective certificate at modulus $2^{18}$ one can target windows with $j\le 10$ and funnels with $L\le 16$. For any verified certificate achieving these parameters, one may take
\[
J^{\ast} \;=\; j_{\max}+L,\qquad N_0^{\ast} \;=\; \left\lceil 2^{L}\,\max N_0 \right\rceil,
\]
so that every odd $n>N_0^{\ast}$ decreases in at most $J^{\ast}$ odd steps. The finitely many integers $n\le N_0^{\ast}$ are then checked exhaustively as part of the finite verification, and the global conclusion follows from the conditional main theorem developed below.
...
\item \verb|j|: window length,
\item \verb|K|: total valuation sum,
\item \verb|s_vec|: the valuation pattern $s=(s_0,\dots,s_{j-1})$ (encoded as a finite list),
\item \verb|A|: the contraction ratio $3^j/2^{K}$,
\item \verb|B|: the offset $c_j/2^{K}$,
\item \verb|N0|: the window threshold $B/(1-A)$.
\end{itemize}
In the certificate at modulus $2^{18}$ used in this paper, the anchor window catalog contains on the order of $10^3$ residues (e.g.\ \verb|anchors_count| $\approx 600$), covering a small but non-negligible fraction of all odd residues modulo $2^{18}$. The remaining residues are handled by short funnels of length at most $16$, which are cataloged in a separate CSV. Together, the window and funnel files form a complete finite certificate from which the global constants $J^{\ast}$ and $N_0^{\ast}$ are derived in later sections.
...
The proof is immediate from $A<1$ and the definition of $N_0$.
...
\begin{lemma}[Funnel plus window implies bounded-time drop]
...
In the concrete certificate at modulus $2^{18}$, the anchored windows satisfy $j\le j_{\max}=10$ and the funnel lengths satisfy $d_R\le L=16$ for all residues. A uniform step bound is therefore
\[
J^\* := j_{\max} + L \le 26,
\]
so that every sufficiently large $n$ sees a strict decrease in $E$ within at most $J^\*$ odd steps.

For a global threshold it is enough to bound
\[
n > 2^{d_R} N_{0,R}
\]
uniformly in $R$. A simple worst-case choice is
\[
N_0^\* := \left\lceil 2^{L} \max_{(j,s,r)} N_0 \right\rceil,
\]
where the maximum is taken over all anchored windows in the certificate. In the present computation this yields
\[
N_0^\* \le 1{,}608{,}153.
\]
Thus any odd $n>N_0^\*$ decreases within at most $J^\*$ odd steps, and the finitely many $n\le N_0^\*$ can be checked exhaustively.
...
In this section I describe how to verify the finite certificate by straightforward arithmetic computations. The goal is to make the argument reproducible and suitable for independent implementation, including in a formal proof assistant.

\subsection{Window checks}

Each row of the window CSV encodes a purported window witness by specifying:
\[
R,\; j,\; K,\; s=(s_0,\dots,s_{j-1}),\; A,\; B,\; N_0.
\]
To verify such a row, one performs the following checks:
\begin{enumerate}
\item \emph{Reconstruct $K$ and $c_j$.} Starting from $K_0=0$ and $c_0=0$, define
\[
K_{t+1}=K_t + s_t,\qquad c_{t+1}=3c_t+2^{K_t},
\]
and confirm that $K_j=K$ and that the cumulative $c_j$ matches the value used to form $B$.

\item \emph{Check the congruences.} For each $t=0,\dots,j-1$ compute
\[
3^{t+1}R + 3c_t + 2^{K_t} \;\; \text{mod}\; 2^{K_{t+1}+1}
\]
and verify that the result is exactly $2^{K_{t+1}}$. This confirms that $\nu_2(3n_t+1)=s_t$ holds whenever $n_0\equiv R \pmod{2^{K+1}}$.

\item \emph{Check the affine form and contraction.} Compute
\[
A' = \frac{3^j}{2^{K}},\qquad B' = \frac{c_j}{2^{K}},
\]
and verify that $A=A'$, $B=B'$, and $A<1$.

\item \emph{Check the threshold.} Compute
\[
N_0' = \frac{B}{1-A},
\]
and verify that it matches the recorded $N_0$ within the chosen numeric precision.
\end{enumerate}
These checks are purely arithmetic and can be performed in arbitrary precision rational or integer arithmetic. In a formal system such as Lean or Isabelle, all computations can be done on exact integers and rationals, avoiding floating-point rounding issues.

\subsection{Funnel checks}

The funnel CSV associates to each odd residue $R\bmod 2^{18}$ a minimal funnel length $d_R\in\{0,1,\dots,16\}$.
To validate the funnel:
\begin{enumerate}
\item \emph{Check that $d_R=0$ implies $R\in W$.} For all rows with $d_R=0$, verify that $R$ appears in the window CSV.

\item \emph{Check minimality for $d_R>0$.} For each residue $R$ with $d_R>0$, iterate the residue map
\[
R_0=R,\qquad R_{k+1} \equiv \frac{3R_k+1}{2^{\nu_2(3R_k+1)}} \pmod{2^{18}}
\]
for $k=0,1,\dots,d_R-1$ and check that:
\begin{itemize}
\item $R_k\notin W$ for $k<d_R$, and
\item $R_{d_R}\in W$.
\end{itemize}
This confirms that $d_R$ is indeed the minimal funnel length from $R$ into the window set $W$.
\end{enumerate}
Again, these checks are finite and purely modular. They involve no growth beyond the fixed modulus $2^{18}$.

\subsection{Finite check below \texorpdfstring{$N_0^\*$}{N0*}}

Once the global constants $J^\*$ and $N_0^\*$ are fixed from the window and funnel data, the remaining task is to verify Collatz for all $n\le N_0^\*$. This can be done in several ways:
\begin{itemize}
\item By embedding a simple Collatz simulator in the verification script, iterating the original Collatz map on integers and checking that every trajectory reaches $1$ before leaving the range or exceeding a conservative bound on its odd height.
\item By relying on an external certificate: for example, an existing verified computation that already covers a range beyond $N_0^\*$, whose validity has been independently checked. In this case the verification protocol simply confirms that $N_0^\*$ lies below the certified bound.
\end{itemize}
Either approach is finite. The first is entirely self-contained; the second may be preferable when integrating with large existing computational verifications.

\subsection{Complexity, reproducibility, and integrity}

The verification steps scale linearly in the size of the certificate:
\begin{itemize}
\item Window checking is proportional to the number of window rows times the typical window length $j$, with a small overhead for computing powers of $3$ and $2$.
\item Funnel checking is proportional to the number of residue rows (here $2^{17}=131{,}072$) times the funnel length $L=16$.
\end{itemize}
In practice, these checks can be carried out in seconds to minutes on a modern machine. Because all computations are explicit and discrete, they are straightforward to port to multiple languages and proof assistants.

To ensure integrity, the certificate files can be distributed together with cryptographic hashes (e.g.\ SHA-256) and a simple validator script. A formalization can treat the CSVs as external constants and prove, inside the logical system, that the arithmetic relations encoded in those constants imply the Collatz conjecture.
...
\begin{itemize}
\item The anchor window CSV contains on the order of $10^3$ rows (for example, \texttt{anchors\_count} $\approx 600$), each describing a contracted congruence class together with its parameters $(j,K,s,A,B,N_0)$.
\item The funnel CSV contains exactly $2^{17}=131{,}072$ rows, one for each odd residue, recording the minimal funnel length $d_R\in\{0,\dots,16\}$ into the window set.
\end{itemize}
Together these files, plus a short verification script, suffice to reconstruct the constants $J^\*$ and $N_0^\*$ and to check all necessary congruences and transitions.
...
For reproducibility, the window and funnel catalogs should be made available as part of the paper's supplementary material. Each CSV file can be accompanied by:
\begin{itemize}
\item a complete schema description (column names and types),
\item a cryptographic hash (e.g.\ SHA-256) to ensure integrity, and
\item a short text description of how the data were generated.
\end{itemize}
The data can be archived in a persistent repository with a stable identifier, allowing independent teams to download the files and run their own verifiers. In the context of a formal proof assistant, the files can be imported as external constants, and the hash can be checked as part of the build process.
...
\begin{itemize}
\item \emph{Tightening $J^\*$ and $N_0^\*$.} More refined searches, higher moduli, or adaptive strategies for choosing windows could reduce the global bounds and yield a more efficient certificate.
...
\begin{verbatim}
target_residue_mod_262144,j,K,s_vec,A,B,N0
12345,4,7,"[2,1,2,2]",0.421875,5.25,9.0869565217
\end{verbatim}
...
\end{itemize}
In the present certificate, most windows have relatively small $j$ (with a sharp cutoff at $j=10$) and thresholds $N_0$ bounded by a few tens before funnel amplification.
...
\end{itemize}
Empirically, a large fraction of residues funnel into $W$ within $L\le 7$, and full coverage is achieved by $L=16$.
