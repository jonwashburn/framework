...
\iftrackchanges
  ...
  \newcommand{\RSdel}[1]{\textcolor{RSDel}{#1}}
...
Despite these successes, CDM faces persistent small-scale tensions: the Cusp-core problem \cite{deBlok2010,Oh2015}, Missing satellites \cite{Klypin1999,Moore1999}, Too-big-to-fail \cite{Boylan2011}, and the Diversity problem \cite{Bullock2017,Oman2015}. Furthermore, the tight correlation between observed and baryonic acceleration (Radial Acceleration Relation, RAR) \cite{McGaugh2016} suggests a direct link between baryons and dynamics, unexpected if dark matter dominates. Finally, despite decades of searches, no dark matter particle has been detected \cite{Bertone2005,Aprile2018}.
...
An external falsification opportunity comes from galaxy-cluster weak gravitational lensing, which probes long-dynamical-time environments distinct from galactic disks. Extrapolating the same dynamical-time dependence to cluster cores, the model predicts an enhanced convergence ratio $\kappa/\kappa_{\rm GR} \approx 1.8 \pm 0.3$ at projected radii $R=20$--50 kpc (Sec.~\ref{sec:cluster_lensing}).\RSadd{This cluster lensing number is a phenomenological extrapolation (it assumes the same weight $w(r)$ rescales the quasi-static potential for lensing); the current RS/ILG relativistic lensing stack in Lean instead bounds only small deviations from GR for RS-small couplings, so an RS-derived cluster lensing prediction is deferred.} This prediction is distinct from baryons-only GR ($\kappa/\kappa_{\rm GR}\approx 1$) and from standard MOND expectations in clusters without additional dark components. Upcoming survey data (including Euclid-era cluster lensing measurements) can therefore provide a sharp empirical check of the framework.
...
\begin{equation}
 H(i\omega,r) = 1 + \int_{0}^\infty \Gamma(\tau,r) e^{-i\omega\tau} d\tau = 1 + \frac{w(r) - 1}{1 + i\omega\tau_\star} 
...
C(\omega,r) \to 1
\]
The memory term vanishes and the response becomes instantaneous Newtonian. This ensures consistency with Solar System tests and laboratory gravity experiments where $T_{\rm dyn} \ll \tau_\star$.
...
\begin{equation}
w(r) \propto \left(\frac{a_0}{a_{\rm baryon}(r)}\right)^{\alpha/2} 
\label{eq:weight_scaling}
\end{equation}
where $a_0$ is a characteristic acceleration scale and $0 < \alpha < 1$ is the power-law exponent in \emph{dynamical-time space}. Since $a_{\rm baryon}(r)=v_{\rm baryon}^2(r)/r$ and $T_{\rm dyn}(r)=2\pi r/v_{\rm baryon}(r)$ for circular orbits, one has the exact identity $\big(T_{\rm dyn}(r)/T_0\big)^2 = \big(a_0/a_{\rm baryon}(r)\big)\,\big(r/r_0\big)$ with $T_0 = 2\pi\sqrt{r_0/a_0}$. Therefore
\[
\left(\frac{T_{\rm dyn}}{T_0}\right)^{\alpha}
  = \left(\frac{a_0}{a_{\rm baryon}}\right)^{\alpha/2}\left(\frac{r}{r_0}\right)^{\alpha/2},
\]
and at $r=r_0$ one has the clean bridge \(\left(T_{\rm dyn}/T_0\right)^{\alpha}=\left(a_0/a\right)^{\alpha/2}\).\RScom{GP-2026-01-02-010: Exact time↔acceleration exponent bridge proven in Lean (`IndisputableMonolith/Gravity/ParameterizationBridge.lean`: `time_power_eq_accel_power_at_r_eq_r0`).}
\begin{equation}
w(r) \propto \left(\frac{T_{\rm dyn}(r)}{T_0}\right)^{\alpha}, \quad T_0 = 2\pi\sqrt{r_0/a_0}
...
\begin{equation}
w(r) = \xi \cdot n(r) \cdot \left(\frac{a_0}{a_{\rm baryon}(r)}\right)^{\alpha/2} \cdot \zeta(r)
\label{eq:weight}
\end{equation}
This multiplicative form ensures that each physical effect contributes independently (in log space, the effects add). We now explain the role of each factor:
...
The term $(a_0/a_{\rm baryon}(r))^{\alpha/2}$ captures the core dynamical-time dependence and is motivated by the scale-free latency discussion in Sec.~\ref{sec:power_law_motivation}. Regions with low baryonic acceleration $a_{\rm baryon}(r)$ typically correspond (at fixed or comparable radii) to longer dynamical times $T_{\rm dyn}(r)=2\pi r/v(r)$ and exhibit stronger gravitational enhancement. Using $a(r)=v^2(r)/r$ gives $T_{\rm dyn}(r) \propto \sqrt{r/a(r)}$, so at fixed $r$ one has $T_{\rm dyn}\propto 1/\sqrt{a}$. The characteristic acceleration $a_0$ sets the scale at which the modification becomes significant. The exponent $0 < \alpha < 1$ controls the strength of the power-law dependence in \emph{time space} (Sec.~2.2), while the corresponding acceleration-space exponent is $\alpha/2$ at $r \approx r_0$. Both $a_0$ and $\alpha$ are global parameters fitted to the entire galaxy sample. For circular orbits, $a_{\rm baryon}(r) = v_{\rm baryon}^2(r)/r$, so this term is directly computable from the baryonic mass model and radius.
...
\begin{equation}
a_{\rm eff}(r) = \xi \cdot n(r) \cdot \left(\frac{a_0}{a_{\rm baryon}(r)}\right)^{\alpha/2} \cdot \zeta(r) \cdot a_{\rm baryon}(r)
\label{eq:a_eff_full}
\end{equation}
For circular orbits in the steady-state regime, this yields the rotation velocity:
\begin{equation}
v_{\rm eff}^2(r) = w(r) \, v_{\rm baryon}^2(r) = \xi \cdot n(r) \cdot \left(\frac{a_0}{a_{\rm baryon}(r)}\right)^{\alpha/2} \cdot \zeta(r) \cdot v_{\rm baryon}^2(r)
\label{eq:v_eff_full}
\end{equation}
...
Note that $\zeta(r)$ uses the per-galaxy disk scale length $R_d$ (a catalog observable, not a fitted parameter), but the overall normalization factor (0.2 in the numerator) could in principle be treated as an additional global fit parameter. In this work, we fix it to 0.2 based on physical estimates to keep the model at seven global fit parameters. Sensitivity tests (Appendix~\ref{app:sensitivity}) show that varying this factor by $\pm 50\%$ changes the median $\chi^2/N$ by less than 0.05.
...
Sets the global stellar mass normalization used to compute $v_{\rm baryon}(r)$ and thus $a_{\rm baryon}(r)$ from the SPARC photometry. We fit a \emph{single} global value $\Upsilon_\star \equiv M_\star/L_{3.6}$ for the entire catalog (Table~\ref{crg:parameters}), with zero per-galaxy adjustments. This is the seventh global degree of freedom in the end-to-end pipeline; it does not enter Eq.~(\ref{eq:weight}) explicitly, but it rescales the baryonic field to which the weight is applied.
\RSadd{Policy note: in this paper we retain $\Upsilon_\star=1.0$ as a conservative SPARC-calibration choice under the strict global-only protocol; RS derivations predict a characteristic stellar $M/L$ on the $\varphi$-ladder, so RS-aligned reruns of the SPARC pipeline will fix $\Upsilon_\star$ accordingly.}

\paragraph{Disk thickness ratio $h_z/R_d$ (fixed).}
A fixed hyperparameter (not fitted) that controls the geometric correction $\zeta(r) = 1 - 0.2 \tanh(r/R_d)$ [Eq.~(\ref{eq:zeta})]. We adopt the canonical value $h_z/R_d = 0.25$, consistent with observed vertical scale heights in disk galaxies.
...
\begin{itemize}
\item A single global stellar mass-to-light ratio $\Upsilon_\star \equiv M_\star/L_{3.6} = 1.0$ (in solar units) applied to the entire catalog, with no per-galaxy adjustments.\RScom{GP-2026-01-02-003}
...
\end{equation}
achieving median $\chi^2/N = 1.19$ on the Q=1 subset (Table~\ref{crg:global_bench}). Due to the stochastic nature of Differential Evolution, independent optimization runs can converge to slightly different local minima; all results in this paper use the fixed parameter set from Eq.~(\ref{eq:fitted_parameters}) and Table~\ref{crg:parameters} for consistency.
...
\RSadd{Important: Eq.~(\ref{eq:kappa_prediction}) is a phenomenological extension of Eq.~(\ref{eq:ansatz}) that assumes the same $w(r)$ rescales the quasi-static potential for lensing. In an RS/ILG relativistic completion, lensing is controlled by the PPN lensing strength $(1+\gamma)/2$; the current Lean bound `lensing\_strength\_bound` implies only small deviations from GR for RS-small couplings. Thus $\kappa/\kappa_{\rm GR}\approx 1$ would falsify this phenomenological lensing extrapolation, but would be consistent with the present RS/ILG lensing bounds.}

%
This phenomenological estimate is sharply distinct from competing frameworks: (i) $\Lambda$CDM with baryons only predicts $\kappa/\kappa_{\rm GR} \approx 1.0$ (no enhancement without dark matter); (ii) MOND typically predicts $\kappa/\kappa_{\rm GR} \approx 1.0$ in clusters unless supplemented by additional dark components (e.g., sterile neutrinos) \cite{Angus2007,Sanders2007}; (iii) superfluid dark matter models \cite{Berezhiani2015} predict smaller enhancements (e.g., $\kappa/\kappa_{\rm GR} \approx 1.2$) in hot cluster environments. The value $\kappa/\kappa_{\rm GR} \approx 1.8$ therefore provides a high-leverage empirical diagnostic of whether the causal-response weight $w(r)$ applies to lensing in the quasi-static regime.

%
The Euclid Space Telescope (launched 2023, data releases 2025--2030) will measure cluster weak lensing profiles with $\lesssim 10\%$ precision through multi-band imaging of $\sim 10^9$ galaxies \cite{EuclidCollaboration2024}. A measurement consistent with $\kappa/\kappa_{\rm GR} \approx 1.8$ would support the phenomenological extrapolation of the same global $w(r)$ scaling to cluster cores; a result consistent with $\kappa/\kappa_{\rm GR} \approx 1$ would strongly disfavor this lensing extrapolation (while remaining compatible with the current RS/ILG lensing bounds). Intermediate values would motivate more detailed modeling (e.g., environmental dependence, profile systematics, or additional physics beyond the present phenomenology). Cluster lensing is therefore one of the most stringent near-term tests of the model.
...
\begin{table}[t]
\centering
\caption{Global parameters and analysis settings (fixed across the SPARC catalog).}
...
$\alpha$ & 0.389 & \RSrep{Dynamical-time exponent}{Power-law exponent in dynamical-time scaling (acceleration exponent is $\alpha/2$ via Sec.~2.2 bridge)} \\
$\Upsilon_\star$ & 1.0 & Global stellar $M_\star/L_{3.6}$ (single value for entire catalog)\RScom{GP-2026-01-02-003: RS Lean derives characteristic stellar M/L ≈ $\varphi$ (`IndisputableMonolith/Astrophysics/MassToLight.lean`). Paper uses 1.0 for SPARC global-only calibration; reconcile policy in revision.} \\
...
The metrics reported in Table~\ref{crg:global_bench} demonstrate consistent improvement across different statistical measures. The median $\chi^2/N$ is the primary metric (Sec.~\ref{sec:goodness_of_fit}), showing a 33\% reduction. For an absolute (km/s) diagnostic, we define the per-galaxy RMS velocity residual as
\begin{equation}
  \mathrm{RMS}_j \equiv \sqrt{\frac{1}{N_j}\sum_{i=1}^{N_j}\big(v_{\rm obs,i,j}-v_{\rm model,i,j}\big)^2}
\end{equation}
and report the median of $\mathrm{RMS}_j$ over the Q=1 sample (galaxy-weighted). In this pinned reproduction the causal-response model achieves 17.3 km/s compared to MOND's 21.5 km/s, a 19.5\% improvement. The outlier count ($\chi^2/N > 5$) is comparable between the two models (26 vs. 23), indicating that the improved median performance is not achieved by systematically sacrificing a subset of galaxies.
...
The small negative mean ($\mu \approx -0.15$) suggests a global bias toward over-prediction. This could arise from systematic uncertainties in the stellar mass-to-light ratio $M_\star/L_{3.6}$ or from unmodeled baryonic contributions (e.g., molecular gas in massive spirals). Overall, the residual analysis provides a diagnostic of error-model calibration and global bias under the strict global-only protocol.

\begin{figure}[t]
\centering
\includegraphics[width=0.48\textwidth]{frg_residuals_a}
\includegraphics[width=0.48\textwidth]{frg_residuals_b}
\caption{Residual analysis for the causal-response model fits. Normalized residuals are defined as $\delta = (v_{\rm obs} - v_{\rm model})/\sigma_{\rm eff}$. Left: Histogram of normalized residuals for all $N_{\rm total} \approx 1900$ data points across the Q=1 sample, overlaid with best-fit Gaussian distribution (red curve). In this pinned reproduction, the fitted mean is $\mu \approx -0.15$ and the standard deviation is $\sigma \approx 1.12$. Right: Quantile-quantile (Q-Q) plot comparing empirical residual quantiles (vertical axis) to theoretical Gaussian quantiles (horizontal axis). Points follow the 1:1 line (dashed) closely in the central range.}
\label{crg:residuals}
\end{figure}
...
The causal-response model identifies 26 galaxies with $\chi^2/N > 5$ (compared to 23 for MOND), indicating a comparable outlier rate in this pinned reproduction. While the model performs well on the majority of galaxies (median $\chi^2/N = 1.19$), examining the outliers provides valuable insight into specific physical regimes where the model assumptions may be violated or where extensions are needed. Table~\ref{tab:outliers} summarizes the six most extreme outliers ($\chi^2/N > 5$), listing the galaxy name, the goodness-of-fit metric, the kinematic type, and a preliminary suspected issue/remedy.
...
\toprule
Galaxy & $\chi^2/N$ & Type & \RSrep{Primary Issue}{Suspected Issue} & \RSrep{Proposed Remedy}{Next step} \\
\midrule
NGC 5985 & \RSadd{10.18} & Massive & \RSadd{Mass model / geometry systematics} & \RSadd{Check $M/L$, bulge, inclination} \\
UGC 07399 & \RSadd{6.97} & Spiral & \RSadd{Non-axisymmetry / profile mismatch} & \RSadd{Inspect residuals; 2D kinematics} \\
NGC 4088 & \RSadd{6.78} & Spiral & \RSadd{Warp / non-circular motions} & \RSadd{2D velocity field / warp model} \\
UGC 06667 & \RSadd{6.53} & Spiral & \RSadd{Geometry / inclination sensitivity} & \RSadd{Recheck catalog geometry} \\
UGC 02487 & \RSadd{6.40} & Massive & \RSadd{Inner baryon model mismatch} & \RSadd{Bulge/disk decomposition review} \\
NGC 2841 & \RSadd{5.61} & Massive & \RSadd{Stellar mass normalization tension} & \RSadd{Test global $\Upsilon_\star$ sensitivity} \\
\bottomrule
\end{tabular}
\end{table}


\paragraph{Model/systematics-driven outliers.}
In this pinned reproduction, the most extreme outliers (Table~\ref{tab:outliers}) are dominated by high-velocity massive systems and a few intermediate-mass spirals. This suggests sensitivity to (i) the strict global $\Upsilon_\star$ policy, and (ii) inner-disk geometry / bulge--disk decomposition systematics, where small photometric or inclination errors can propagate into large velocity residuals.

\paragraph{Non-axisymmetry and kinematic complexity.}
Several spiral outliers are plausible candidates for non-axisymmetric structure (bars/spiral arms), warps, or streaming motions that violate the steady, axisymmetric circular-orbit assumption implicit in Eq.~(\ref{eq:v_model}). Future work using 2D velocity fields and explicit warp/bar modeling is expected to reduce these residuals without introducing per-galaxy tuning.

\paragraph{Reproducibility note.}
The outlier set is pipeline-defined by the fixed masking and error model of Sec.~\ref{sec:methods} and the pinned verifier implementation.


\paragraph{Summary and implications.}
The six outliers with $\chi^2/N > 5$ (Table~\ref{tab:outliers} lists the six most extreme) represent approximately 6\% of the Q=1 sample and point to physical regimes where the current model assumptions (axisymmetry, equilibrium, planar geometry) may break down. Importantly, these failure modes are amenable to future extensions (e.g., 2D kinematics and non-axisymmetric modeling). The comparable outlier rate between the causal-response model (26 galaxies with $\chi^2/N > 5$) and MOND (23 galaxies) indicates that the present global-only causal-response fit is not improving the median metric by shifting error into a larger tail.
...
\textbf{Cluster Lensing} & Underpredicts (requires $\nu$/DM) & \RSrep{Predicted $\kappa/\kappa_{\rm GR} = 1.8 \pm 0.3$}{(Phenomenological) $\kappa/\kappa_{\rm GR} \approx 1.8 \pm 0.3$} \\
\bottomrule
\end{tabular}
\end{table}
...
The current model assumes axisymmetric, steady-state rotation with circular orbits. Bars, strong warps, and non-circular motions (radial flows, turbulence) are not explicitly modeled. \RSrep{Outliers such as UGC 2885 (massive barred spiral)}{The reproduced outlier set (Sec.~\ref{sec:outlier_analysis})} shows larger residuals, suggesting the need for 2D or 3D extensions incorporating non-axisymmetric potentials and time-dependent perturbations. Such extensions would increase model complexity but may be necessary for precision fits to individual galaxies with complex morphologies.

\RSadd{\paragraph{Parameter classification and theoretical derivation.}
While the present work treats all seven parameters as empirically fitted, we note that the power-law exponent $\alpha$ is consistent with the theoretical prediction $\alpha = 1 - 1/\varphi \approx 0.382$ (where $\varphi = (1+\sqrt{5})/2$ is the golden ratio), derived from first-principles considerations in the Recognition Science framework. The fitted value $\alpha = 0.389 \pm 0.015$ lies within $1\sigma$ of this prediction. The remaining parameters---morphology coefficient $C_\xi$, spatial profile $(A, r_0, p)$, and stellar mass-to-light ratio $\Upsilon_\star$---are treated here as phenomenological, though ongoing theoretical work aims to derive them from the same foundational principles. The characteristic timescale $\tau_\star \approx 133$ Myr emerges as the galactic-scale rung ($N \approx 143$) on a $\varphi$-scaling ladder from the fundamental tick $\tau_0 \sim 10^{-15}$ s. Full derivation of these connections is beyond the scope of this paper but is documented in the machine-verified Lean repository accompanying this work.}\RScom{GP-2026-01-02-013}
...
Weak lensing $\kappa/\kappa_{\rm GR}$ at 20--50 kpc & $1.8 \pm 0.3$\RSadd{ (phenom.)} & TBD (Euclid 2025+) & $< 1.2$ or $> 2.5$ \\
...
With all seven parameters fixed globally across the entire 99-galaxy sample, the model is rigid: it cannot adjust halo masses, concentrations, or individual galaxy properties to improve fits for outliers. Every galaxy constitutes an independent test of the same universal functional form. The distribution of residuals (median $\chi^2/N = 1.19$, interquartile range 0.7--1.8, Sec.~\ref{sec:global_performance}) provides non-trivial evidence for the physical validity of the weight function $w(r)$.
...
If the cluster weak lensing prediction $\kappa/\kappa_{\rm GR} = 1.8 \pm 0.3$ is confirmed by Euclid observations and a covariant relativistic completion of the framework is achieved, the causal-response model would provide a viable alternative to particle dark matter at galactic and (phenomenologically) cluster scales. In this picture, the MOND acceleration scale $a_0 \approx 1.2 \times 10^{-10}$ m/s$^2$ would emerge as a derived quantity from the fundamental timescale $\tau_\star \approx 133$ Myr and typical galactic sizes, rather than being postulated as a fundamental constant of nature. The apparent "missing mass" in galaxies and clusters would arise not from undetected particles but from enhanced gravitational response $w(r) > 1$ at long dynamical times, a frequency-dependent modification that mimics extra mass while maintaining tight correlations with baryonic structure (as observed in the RAR and BTFR).

Conversely, if Euclid measures $\kappa/\kappa_{\rm GR} = 1.0 \pm 0.1$ (the GR baryons-only prediction), the cluster-scale \emph{lensing} extrapolation of the present causal-response parameterization would be strongly disfavored, indicating either a scale/environment-dependent cutoff that suppresses the enhancement in clusters or that the assumption “$w$ rescales lensing” is not correct.\RSadd{ Such a null result would be compatible with RS/ILG-style small-deviation lensing (per current Lean bounds),} and would strengthen the case for dark-matter-based explanations on cluster scales and require a revised causal-response completion (if any) to remain viable across environments.
