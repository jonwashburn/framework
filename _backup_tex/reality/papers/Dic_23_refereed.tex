...
% Units (robust to minimal TeX installs)
\IfFileExists{siunitx.sty}{%
  ...
  % Fallback: keep compilation working even if `siunitx` is unavailable.
  % NOTE: inside \IfFileExists branches, parameter tokens must be doubled.
  \newcommand{\SI}[3][]{##2\,##3}
  \newcommand{\si}[1]{##1}
...
% highlight (robust to minimal TeX installs)
\IfFileExists{soul.sty}{%
  \usepackage{soul}
}{%
  % Fallback: make highlight/strike macros no-ops if `soul` is unavailable.
  \newcommand{\hl}[1]{##1}
  \newcommand{\st}[1]{##1}
  \newcommand{\ul}[1]{##1}
}
...
\usepackage{tikz}
\IfFileExists{tikz-cd.sty}{\usepackage{tikz-cd}}{} % optional
\usetikzlibrary{positioning}

% Appendix helper (robust to minimal TeX installs)
\IfFileExists{appendix.sty}{%
  \usepackage[title]{appendix}
}{%
  % Fallback: allow `\begin{appendix}...\end{appendix}` using LaTeX's built-in `\appendix`.
  % `\begin{appendix}` calls `\appendix` (already defined); we only need an `\endappendix`.
  \ifcsname endappendix\endcsname\else\def\endappendix{}\fi
}
...
\newcommand{\Recognition}{\Rec} % manuscript alias (was used but not previously defined)
\newcommand{\ed}[1]{\textcolor{magenta}{#1}} % referee edits in magenta
...
\ed{(Lean: \texttt{IndisputableMonolith.Recognition.Ledger} with fields \texttt{debit, credit})}
that records recognition in indivisible atomic ticks \ed{(Lean: \texttt{IndisputableMonolith.Recognition.AtomicTick})}. This ledger enforces 
sequential updates, closed--chain flux cancellation \ed{(Lean: \texttt{IndisputableMonolith.Recognition.Conserves})}, 
\ed{potential uniqueness up to a gauge constant on reach components (Lean: \texttt{IndisputableMonolith.Potential.T4\_unique\_up\_to\_const\_on\_component})},
and a \(2^{d}\)-tick cycle \ed{(minimal cover of \(d\)-bit patterns; for \(d=3\) an explicit 8-phase Gray cycle exists in Lean: \texttt{IndisputableMonolith.Patterns.grayCycle3})} that links 
...
\(\Recognition(\mathtt{Null}, \mathtt{Null}) = \emptyset\), where \(\mathtt{Null}\) 
represents the absence of any recognizer or recognized object, expressing that no 
recognition event can originate from or target nothingness.
...
\begin{itemize}
    ...
    \item \textit{RS Meta-Principle:} A categorical constraint ruling out recognition when 
    there is no recognizer or no recognized object (encoded via \(\mathtt{Null}\)) and 
    enforcing that all recognition processes occur over non-empty substrates.
\end{itemize}
...
\midrule
\textbf{Axioms} & 
T1 (Meta-Principle): $\Recognition(\mathtt{Null}, \mathtt{Null}) = \emptyset$ \\
\midrule
...
\midrule
\textbf{Definitions} & 
Recognition function: $\Recognition: (A \cup \{\mathtt{Null}\}) \times (B \cup \{\mathtt{Null}\}) \to \mathcal{P}(E)$ \\
& Possible recognition event: $e \in E$ (static possibility space) \\
& Actual recognition event: $e_t \in E$ actualized at tick $t$ (dynamic occurrence) \\
...
\ed{Lean note: the certified dependency chain is encoded as \texttt{IndisputableMonolith.Meta.MPChain.proven\_derivation}. Some steps are packaged differently than the present narrative (e.g.\ atomicity via the \texttt{AtomicTick} structure; closed-chain conservation via \texttt{Recognition.Conserves} and \texttt{chainFlux}; potential uniqueness via reach-component lemmas in \texttt{Potential}).}
...
\begin{definition}
Let \(A\) be the set of possible recognizers, \(B\) the set of possible recognized objects, 
and \(E\) the set of all \emph{possible} recognition events (the static possibility space). 
Let \(\mathtt{Null}\) be a symbol not in \(A\) or \(B\), representing the absence of a 
recognizer or recognized object.

We define the recognition function on the extended domain:
\[
\Recognition: (A \cup \{\mathtt{Null}\}) \times (B \cup \{\mathtt{Null}\}) \to \mathcal{P}(E),
\]
where for \((a,b) \in A \times B\):
\[
\Recognition(a,b) = \{ e \in E \mid e \text{ is a possible event of } a \text{ recognizing } b \}.
\]
A \emph{possible recognition event} is an element \(e \in E\) that belongs to \(\Recognition(a,b)\) 
for some \((a,b) \in A \times B\). This represents the minimal relational structure 
assumed between recognizer and recognized in the static possibility space.

An \emph{actual recognition event} (or simply \emph{recognition event}) is a possible event 
\(e \in E\) that is \emph{actualized}---that is, it occurs at a specific tick \(t\) in the 
ledger's temporal sequence. The ledger records these actual occurrences sequentially, not 
all possibilities simultaneously. The distinction between the static possibility space \(E\) 
and dynamic actual occurrences is fundamental: \(E\) represents what \emph{could} happen, 
while the ledger records what \emph{does} happen at each tick.
\end{definition}

\textbf{Axiom 1 (T1).} 
\emph{Recognition cannot occur in absolute nothingness:}
\[
\Recognition(\mathtt{Null}, \mathtt{Null}) = \emptyset.
\]
Additionally, recognition cannot occur when either the recognizer or the recognized object is absent:
\[
\Recognition(\mathtt{Null}, b) = \emptyset \quad \text{for all } b \in B, \qquad
\Recognition(a, \mathtt{Null}) = \emptyset \quad \text{for all } a \in A.
\]

\begin{remark}[Philosophical Distinction]
Interpretively, Axiom 1 forbids the possibility of recognition events when there is no recognizer, 
no recognized object, or both. The symbol \(\mathtt{Null}\) represents ontological nothingness---the 
absence of any recognizer or recognized object---distinct from the empty set \(\emptyset\), which 
is a mathematical object denoting the set with no elements. This distinction is crucial: \(\emptyset\) 
is something (a well-defined mathematical object), while \(\mathtt{Null}\) represents the absence 
of existence itself. The axiom \(\Recognition(\mathtt{Null}, \mathtt{Null}) = \emptyset\) is 
non-trivial because it constrains the recognition function on arguments outside the domain of 
actual recognizers and recognized objects, expressing a substantive principle about the 
preconditions for recognition rather than a definitional consequence.
\end{remark}

\subsection{Atomic Tick Cycle}

The Atomic Tick Principle emerges from the Meta-Principle (T1) by specifying the minimal machinery required to record recognition events. Recognition Science models this through a \emph{ledger}---a sequential, unambiguous record of each recognition act. The ledger's global shape is \ed{taken} to be a $\mathbb{Z}^3$ cubic lattice, \ed{motivated} by the Meta-Principle and conservation.
\ed{(Lean hygiene note: the Lean core does not assume a global $\mathbb{Z}^3$ lattice at this stage; it treats the ledger carrier abstractly as a type \(U\), and later specializes to concrete \(D=3\) cube/octave structures when deriving \(2^D\)-periodicity and Gray adjacency.)}
...
The ledger state $S_t$ at tick $t$ evolves deterministically according to a function $F: \mathcal{S} \times E \to \mathcal{S}$, where $\mathcal{S}$ is the state space and $E$ is the set of all possible recognition events (as defined above). The state update rule is:
\begin{equation}
    S_{t+1} = F(S_t, e_t),
\end{equation}
where $e_t \in E$ is a \emph{single actual} recognition event that occurs at tick $t$---that is, a possible event from $E$ that is actualized at tick $t$. The function $F$ has domain $(\text{state}, \text{single actual event})$, not $(\text{state}, \text{set of events})$ or $(\text{state}, \text{sequence of events})$. This ensures that at each tick, exactly one possible event from $E$ is actualized and recorded in the ledger.
...
At most one \ed{posted carrier element} per tick. There are no concurrent recognitions (within the model).
\end{theorem}
...
\ed{In Lean, T2 is packaged as a short lemma \texttt{IndisputableMonolith.Recognition.T2\_atomicity} derived from the definition of an \texttt{AtomicTick} schedule (\texttt{postedAt} together with a uniqueness proof \texttt{unique\_post}). Existence of such schedules from countability/algorithmic specification is proved in \texttt{IndisputableMonolith.Verification.Necessity.AtomicTickNecessity}.}
...
By the double-entry structure, each recognition event records balanced pairs $\pm\delta$. When we sum postings around a closed cycle, every debit on one edge is matched by a credit on another edge in the cycle (or vice versa). Since the cycle is closed (returns to its starting node), the net balance change around the cycle must be zero: $\sum_{i=1}^n k_i = 0$. Therefore, $\Phi(\gamma, t) = \delta \cdot 0 = 0$.
 ...
 \ed{\textbf{Lean Reference:} \texttt{IndisputableMonolith.Recognition.chainFlux\_zero\_of\_loop} (under \texttt{IndisputableMonolith.Recognition.Conserves}) and \texttt{IndisputableMonolith.Recognition.chainFlux\_zero\_of\_balanced} (from pointwise balance \texttt{debit=credit}).}
\end{proof}
...
\ed{\begin{remark}[Lean status of T4 (existence vs.\ uniqueness)]
The current Lean core formalizes the \emph{uniqueness} half as a reach-component statement: if two potentials \(p,q : U\to \mathbb{Z}\) satisfy the same discrete edge rule \(R\,a\,b \Rightarrow p(b)-p(a)=\delta\) (and similarly for \(q\)), then on each reach component they differ by a constant (Lean: \texttt{IndisputableMonolith.Potential.T4\_unique\_up\_to\_const\_on\_component}, and the basepoint-agreement form \texttt{IndisputableMonolith.Potential.T4\_unique\_on\_component}). A fully constructive existence proof matching the discrete Poincar\'e lemma is treated here classically and can be added to the Lean development as a separate layer.%
\end{remark}}
...
\ed{\begin{remark}[Lean formulation of T5]
The Lean development proves a closely related (and more explicit) uniqueness theorem as \texttt{IndisputableMonolith.CostUniqueness.unique\_cost\_on\_pos}: any \(F:\mathbb{R}\to\mathbb{R}\) satisfying a bundled hypothesis set (reciprocal symmetry, unit normalization, strict convexity on \((0,\infty)\), calibration of \((F\circ\exp)''(0)\), continuity on \((0,\infty)\), and the cosh-add functional identity together with stated ODE-regularity hypotheses) agrees with \texttt{Jcost} on all \(x>0\). This separates the algebraic characterization of \(J\) from the analytic/functional-equation ingredients used to bootstrap uniqueness on the full positive reals.%
\end{remark}}
...
\ed{In the current Lean development, the ``\(D=3\)'' pinning used by the octave/cover arguments is expressed as an \emph{arithmetic rigidity} predicate rather than a topological knot argument: one assumes (i) a complete cover of \(D\)-bit patterns of period \(2^D\) and (ii) a synchronization identity \(\mathrm{lcm}(2^D,45)=360\); from these hypotheses Lean proves \(D=3\) (Theorem \texttt{IndisputableMonolith.Verification.Dimension.onlyD3\_satisfies\_RSCounting\_Gap45\_Absolute}). Topological ``linking'' intuition can motivate why three dimensions are special, but it is not required for this specific formal implication.}
...
\ed{\begin{remark}[Lean status of $w_8$ (definition vs.\ certified numerics)]
In the Lean repository, the value used in the fine-structure pipeline is exposed as \texttt{IndisputableMonolith.Constants.w8\_from\_eight\_tick} and is currently carried as an explicit external/computational certificate value, quarantined in \texttt{IndisputableMonolith.Constants.GapWeightNumericsScaffold} to keep it out of the certified surface. A DFT-8-based definitional derivation is being developed in \texttt{IndisputableMonolith.Constants.GapWeightDerivation}; connecting the symbolic DFT definition to the certified numeric value is an explicit, tracked formalization item (not a fitted parameter).%
\end{remark}}
...
\begin{theorem}[Unweighted DFT energy ratio on the $\phi$-pattern (diagnostic)]
Define the \emph{unweighted neutral/DC energy ratio} of the $\phi$-pattern by:
\begin{equation}
w_8^{\mathrm{ratio}} := \sum_{k=1}^{7} \frac{|\hat{s}_k|^2}{|\hat{s}_0|^2}.
...
We compute $w_8^{\mathrm{ratio}}$ explicitly from the DFT-8 decomposition of the $\phi$-lattice sequence.
...
\textbf{Step 3: Compute the ratio.} Substituting into equation (\ref{eq:w8-definition}) and using
$e^{-2\pi i k}=1$ (for integer $k$), the numerator simplifies and we obtain:
\begin{equation}
w_8^{\mathrm{ratio}}
= \sum_{k=1}^{7} \frac{|\hat{s}_k|^2}{|\hat{s}_0|^2}
= (\phi-1)^2 \sum_{k=1}^{7} \frac{1}{\left|1 - \phi e^{-2\pi i k/8}\right|^2}.
\end{equation}

\textbf{Step 4: Numerical evaluation.} Evaluating this expression numerically yields:
\begin{equation}
w_8^{\mathrm{ratio}} = 0.970692434668\ldots
\end{equation}
\end{proof}

\begin{remark}[RS gap weight used in the $\alpha$ pipeline]
The \emph{RS gap weight} used in the fine-structure derivation is a different quantity, denoted $w_8$, and is currently taken as the certified value
$w_8 = 2.488254397846\ldots$ (see Lean: \texttt{IndisputableMonolith.Constants.GapWeight.w8\_from\_eight\_tick} and \texttt{data/certificates/w8.json}).
Closing a fully internal derivation that links this certified value to a DFT-8 expression is tracked as an open item.
\end{remark}
...
\begin{proposition}[Physical Interpretation of $w_8^{\mathrm{ratio}}$]
The diagnostic ratio $w_8^{\mathrm{ratio}}$ quantifies, for the raw $\phi$-pattern on one 8-tick window, how much DFT energy sits in the neutral modes relative to the DC mode.
\end{proposition}
...
\textbf{Combining all three terms:} Each term is derived from the ledger-to-QED mapping, not assembled from chosen geometric factors. The formula $\alpha^{-1} = 44\pi - 1.197377... + 0.003299762... = 137.0359991185...$ matches CODATA 2022 value $137.035999177(21)$ to within $2.1 \times 10^{-8}$---a prediction to eight significant figures from a framework with no free parameters, where all terms arise from the explicit mapping between ledger structures and QED quantities. \ed{Lean note: the symbolic formula is defined in \texttt{IndisputableMonolith.Constants.Alpha} (via \texttt{alphaInv := alpha\_seed - (f\_gap + delta\_kappa)}); match-to-CODATA checks are quarantined in \texttt{IndisputableMonolith.Constants.AlphaNumericsScaffold} (and interval-bound modules), not in the certified proof surface.}
\end{proof}
...
\ed{\begin{remark}[Lean status: anchors vs.\ absolute unit calibration]
In the Lean development, the electron-mass pipeline is carried in dimensionless RS units and compared to MeV values only via an explicit reference constant \texttt{IndisputableMonolith.Physics.ElectronMass.mass\_ref\_MeV} (used for verification, not as an input to the predicted value). A fully physical ``RS-units \(\to\) SI/MeV'' calibration is treated as an explicit bridge layer and is not required for the internal derivation of the dimensionless structural mass \(2^{-22}\varphi^{51}\) and the residue/shift terms.%
\end{remark}}
...
\ed{Lean note: in the current Lean formalization, the exponent \(51\) is \emph{derived mechanically} from a small set of discrete sector integers (e.g.\ \(B=-22\), \(R_0=62\), \(r=2\), together with \(E_{\mathrm{coh}}=\varphi^{-5}\)); see \texttt{IndisputableMonolith.Physics.ElectronMass.electron\_structural\_mass\_forced}. The open ``model selection'' question is therefore pushed back one level: what uniquely fixes those sector integers from earlier RS constraints.}
...
\begin{equation}
    {\frac{H_0^{\mathrm{late}}}{H_0^{\mathrm{early}}} = \frac{13}{12} = 1.08333\ldots}
\label{eq:hubble-ratio}
\end{equation}
where the observed ratio $73.04/67.4 \approx 1.0837$ matches this value to within $0.04\%$.
\end{observation}

\ed{\begin{remark}[Lean status of the Hubble ratio check]
The geometric ratio \(13/12\) and the corresponding ``match-to-observation'' inequality (using the quoted experimental numbers as explicit constants) are formalized in Lean in \texttt{IndisputableMonolith.Cosmology.HubbleTension} and packaged as \texttt{IndisputableMonolith.Verification.HubbleTensionCert}. This is a verified \emph{consistency check} of the correspondence, not (yet) a full dynamical ledger\(\to\)cosmology derivation.%
\end{remark}}
...
\textbf{Lean Reference:} \ed{\texttt{IndisputableMonolith.Recognition.T2\_atomicity}} \\
\textbf{(Existence-from-countability reference):} \ed{\texttt{IndisputableMonolith.Verification.Necessity.AtomicTickNecessity.atomicTickFromZeroParams}} \\
\textbf{Status:} Proved \\
\textbf{Strategy:} Package ``one update per tick'' as an \texttt{AtomicTick} schedule and discharge atomicity from uniqueness; then show such schedules exist under the zero-parameter/algorithmic-spec hypothesis.

In Lean, an \texttt{AtomicTick} on a carrier \(U\) is a predicate \texttt{postedAt : Nat → U → Prop} together with a proof that for each tick \(t\) there exists a \emph{unique} posted element: \(\exists! u, \texttt{postedAt}\ t\ u\). Theorem T2 (\texttt{Recognition.T2\_atomicity}) is then immediate: if two elements are both posted at the same tick, uniqueness forces them equal.

The ``necessity'' step (existence of an \texttt{AtomicTick} schedule) is separated cleanly: from an explicit surjection \(\mathbb{N}\to U\) one defines \texttt{postedAt t u := (u = f t)}; from the zero-parameter/algorithmic-spec hypothesis \texttt{HasAlgorithmicSpec} plus non-emptiness, Lean constructs a countable discrete skeleton and hence such a surjection (\texttt{AtomicTickNecessity.atomicTickFromZeroParams}). The Axiom~2/Axiom~3 discussion in the main text can be read as a modeling motivation for why an \texttt{AtomicTick} schedule is the minimal bookkeeping needed for unambiguous state evolution.
\end{proof}

\subsection{Proof Sketch of Theorem T8: Ledger Units}

\begin{proof}[Proof Sketch of Theorem T8]
\textbf{Lean Reference:} \ed{\texttt{IndisputableMonolith.LedgerUnits.equiv\_delta\_one}}, \ed{\texttt{IndisputableMonolith.LedgerUnits.quantization}} \\
\textbf{Status:} Proved \\
...
\textbf{Lean Reference:} \ed{\texttt{IndisputableMonolith.Recognition.chainFlux\_zero\_of\_loop}} (under \texttt{Conserves}) \\
\textbf{Balance-implies-flux-zero reference:} \ed{\texttt{IndisputableMonolith.Recognition.chainFlux\_zero\_of\_balanced}} \\
\textbf{Status:} Proved \\
\textbf{Strategy:} Define a node ``potential'' \(\phi=\texttt{debit}-\texttt{credit}\) and a chain flux \(\texttt{chainFlux}=\phi(\text{last})-\phi(\text{head})\); closed chains have zero flux under the conservation law.

In Lean, a ledger state on \(U\) is represented by two integer fields \texttt{debit, credit : U → }\(\mathbb{Z}\) and the derived quantity \(\phi(u)=\texttt{debit}(u)-\texttt{credit}(u)\). For a finite chain \(ch\) in a recognition structure, Lean defines
\[
\texttt{chainFlux}(L,ch)=\phi(\texttt{last}(ch))-\phi(\texttt{head}(ch)).
\]
The conservation law is packaged as a typeclass \texttt{Conserves L} asserting that if a chain is closed (\(\texttt{head}=\texttt{last}\)) then \texttt{chainFlux} is zero; the lemma \texttt{chainFlux\_zero\_of\_loop} is just that axiom unpacked. A useful sufficient condition is pointwise balance \(\forall u,\ \texttt{debit}(u)=\texttt{credit}(u)\), which implies \(\phi\equiv 0\) and hence \texttt{chainFlux} vanishes for all chains (\texttt{chainFlux\_zero\_of\_balanced}). 

The edge-sum flux \(\Phi(\gamma,t)=\sum_{e\in\gamma}\Delta(e,t)\) used in the manuscript can be recovered once one assumes postings arise as differences of a potential (a discrete gradient); in that case the loop-sum is a telescoping sum and reduces to the same closed-chain condition.
\end{proof}

\subsection{Proof Sketch of Theorem T4: Potential Uniqueness}

\begin{proof}[Proof Sketch of Theorem T4]
\textbf{Lean Reference:} \ed{\texttt{IndisputableMonolith.Potential.T4\_unique\_on\_component}} \\
\textbf{(Gauge form):} \ed{\texttt{IndisputableMonolith.Potential.T4\_unique\_up\_to\_const\_on\_component}} \\
\textbf{Status:} Proved \\
\textbf{Strategy:} Assume a discrete edge rule (constant increment \(\delta\) along each recognized edge) and prove that any two potentials satisfying the same rule differ by a constant on each reach component.

Lean formalizes a potential as a function \(p:U\to\mathbb{Z}\) together with a discrete edge rule \(\texttt{DE}\ \delta\ p\), meaning \(R\,a\,b \Rightarrow p(b)-p(a)=\delta\). If \(p\) and \(q\) satisfy \(\texttt{DE}\ \delta\) and agree at a basepoint \(x_0\), then their difference \(p-q\) is invariant along every edge and hence along every finite reach from \(x_0\); therefore \(p(y)=q(y)\) for all \(y\) reachable from \(x_0\) (\texttt{Potential.T4\_unique\_on\_component}). Equivalently, on each reach component there exists a unique constant \(c\) such that \(p(y)=q(y)+c\) for all reachable \(y\) (\texttt{Potential.T4\_unique\_up\_to\_const\_on\_component}).

The discrete Poincar\'{e} lemma presented below is a classical existence result for potentials from vanishing cycle sums; it is compatible with the Lean formulation but is logically separate from the uniqueness lemma currently used in the certified core.
\end{proof}
...
\textbf{Lean Reference (explicit Gray-8 cycle):} \ed{\texttt{IndisputableMonolith.Patterns.grayCycle3}} \\
\textbf{Lean Reference (coverage lower bound):} \ed{\texttt{IndisputableMonolith.Patterns.eight\_tick\_min}} and \ed{\texttt{IndisputableMonolith.Patterns.T7\_nyquist\_obstruction}} \\
\textbf{Status:} Proved (100\% complete as of 2025-09-30) \\
\textbf{Certificate packaging (optional):} \ed{\texttt{IndisputableMonolith.Verification.NyquistObstruction.NyquistObstructionCert}}.

This proof is verified in Lean 4. The proof establishes:
\begin{itemize}
    \item Existence of a complete 3-bit Gray cycle of period $8$ (adjacent one-bit steps and full coverage)
    \item Minimality: any complete cover of 3-bit patterns requires at least $8$ ticks (pigeonhole/Nyquist obstruction)
\end{itemize}

\textbf{Sufficiency:} Lean constructs an explicit Gray cycle on 3-bit patterns (\texttt{Patterns.grayCycle3}), whose phase order corresponds to the usual $000 \to 001 \to 011 \to 010 \to 110 \to 111 \to 101 \to 100 \to 000$ cycle and proves one-bit adjacency at each step.

\textbf{Necessity:} The general pigeonhole/Nyquist obstruction theorem (\texttt{Patterns.T7\_nyquist\_obstruction}) shows that if $T < 2^d$ then there is no surjection \(\mathrm{Fin}\ T \to \mathrm{Pattern}\ d\); in particular, for \(d=3\) any complete cover has length at least \(8\) (\texttt{Patterns.eight\_tick\_min}).
\end{proof}

\subsection{Proof of Theorem T7: Coverage Lower Bound}

\begin{proof}[Proof of Theorem T7]
\textbf{Lean Reference:} \ed{\texttt{IndisputableMonolith.Patterns.T7\_nyquist\_obstruction}}, \ed{\texttt{IndisputableMonolith.Patterns.T7\_threshold\_bijection}} \\
\textbf{Status:} Proved
...
\begin{itemize}
    \item \textbf{T1:} \ed{\texttt{IndisputableMonolith.Recognition.mp\_holds}}
    \item \textbf{T2:} \ed{\texttt{IndisputableMonolith.Recognition.T2\_atomicity}} (schedule existence: \ed{\texttt{IndisputableMonolith.Verification.Necessity.AtomicTickNecessity}})
    \item \textbf{T3:} \ed{\texttt{IndisputableMonolith.Recognition.chainFlux\_zero\_of\_loop}} / \ed{\texttt{chainFlux\_zero\_of\_balanced}}
    \item \textbf{T4:} \ed{\texttt{IndisputableMonolith.Potential.T4\_unique\_on\_component}} / \ed{\texttt{T4\_unique\_up\_to\_const\_on\_component}}
    \item \textbf{T5:} \ed{\texttt{IndisputableMonolith.CostUniqueness.unique\_cost\_on\_pos}} (extension lemma: \ed{\texttt{IndisputableMonolith.Cost.T5\_cost\_uniqueness\_on\_pos}})
    \item \textbf{T6:} \ed{\texttt{IndisputableMonolith.Patterns.grayCycle3}} (and lower bound via \ed{\texttt{IndisputableMonolith.Patterns.eight\_tick\_min}})
    \item \textbf{T7:} \ed{\texttt{IndisputableMonolith.Patterns.T7\_nyquist\_obstruction}}, \ed{\texttt{IndisputableMonolith.Patterns.T7\_threshold\_bijection}}
    \item \textbf{T8:} \ed{\texttt{IndisputableMonolith.LedgerUnits.equiv\_delta\_one}}, \ed{\texttt{IndisputableMonolith.LedgerUnits.quantization}}
\end{itemize}
