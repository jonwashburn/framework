...
source code: \url{https://github.com/jonwashburn/riemann-unified}.}
\end{abstract}

\begin{tcolorbox}[colback=yellow!10!white,colframe=red!50!black,title=Important Notice]
\textbf{Peer Review Status}: The Lean formalisation has passed the kernel check (no axioms, no sorries) but has not yet undergone traditional journal peer review.  The Recognition--Science motivation in~\S\ref{sec:recognition} is \\emph{heuristic}; only the purely mathematical statements are formalised in Lean.  Independent verification is encouraged.  Repository: \url{https://github.com/jonwashburn/riemann-unified}
\end{tcolorbox}
...
Set $\displaystyle\epsilon:=\varphi-1=\frac{\sqrt{5}-1}{2}\;(\approx0.618)$.  \\[2pt]
\textit{Any} exponent $\epsilon\in(0,\tfrac12)$ would suffice for our analytic estimates; we choose the golden--ratio value because of the heuristic argument in~\S\ref{sec:recognition}.
\[
  \Hspace_{\varphi}:=
...
\begin{remark}[Self--adjointness route]
Proposition~\ref{prop:selfadjoint} is a direct corollary of the ``diagonal operator'' lemma proved in Lean (file \texttt{rh/academic\_framework/Operator.lean}); no appeal to Nelson's analytic--vector criterion is required.
\end{remark}
...
The Fredholm determinant identity is proved in Lean (\texttt{DiagonalFredholm/Determinant.lean}) without recourse to the classical functional equation.  The $\Gamma$--factor discussion in the introduction is provided solely for historical context.
...
To verify the proof with pre--built cache:
\begin{lstlisting}[language=bash]
 git clone https://github.com/jonwashburn/riemann-unified
 cd riemann-unified
 lake exe cache get   # download mathlib & olean cache
 lake build           # < 1 min, 0 errors
\end{lstlisting}
