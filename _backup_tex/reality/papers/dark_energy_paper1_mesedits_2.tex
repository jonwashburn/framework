...
% Highlighted edits for the January 2026 referee update pass
...
\end{equation}
with constants completely fixed: $C = \varphi^{-3/2} \approx 0.486$, $\alpha = \frac{1}{2}(1-\varphi^{-1}) \approx 0.191$, and $\tau_0 \approx H_0^{-1}$, where $\varphi = (1+\sqrt{5})/2$ is the golden ratio. \EDIT{These values are outputs of the cost-first Recognition Science derivation chain (with an audited Lean certificate closure for the parameter identities and related constants), not motivated candidates.} Critically, the derived $\alpha \approx 0.191$ independently satisfies the mathematical well-posedness constraint $\alpha \in (0,1/2)$ required for infrared regularity. \EDIT{The ``zero-free-parameter'' designation is therefore upgraded from ``phenomenologically fixed'' to ``derivation-fixed'' (i.e.\ not fit to cosmological data).} In the linear/quasi-static window, the kernel produces mild (order-10\%) modifications at commonly probed linear scales, while remaining bounded and automatically recovering GR as $X\to\infty$. The complete model specification, including motivating context and parameter origin, is presented in Sec.~\ref{sec:model}.
...
\end{equation}
This is the most conservative physical assumption: all matter (baryons + dark matter) contributes information and sources the potential. The specific functional form \eqref{eq:kernel_form} and parameter values are fixed and specified independently of cosmological data. No free functions or fitting procedures are employed; all analysis proceeds independently with the fixed kernel as input.
\EDIT{(Update: in the current theory, the kernel form and the numerical parameter values are treated as derived from the cost-first Recognition Science foundation, with the specific identities packaged in an audited Lean certificate closure.)}
...
\textbf{Parameter origin and mathematical consistency.} \EDIT{The kernel form and the values $(C,\alpha,\tau_0)$ are treated as outputs of the cost-first Recognition Science derivation chain (rather than phenomenological ``inputs''), and the key identities are packaged in an audited Lean certificate closure.} In this paper we nevertheless keep the presentation modular: we take $(C,\alpha,\tau_0)$ as fixed (not fitted to cosmological data) and focus on the consequences of the fixed kernel in the linear/quasi-static regime. Critically, the constraint $\alpha\in(0,1/2)$ is \emph{independently required} for mathematical well-posedness: for $\alpha\ge 1/2$, the modified Poisson operator has infrared divergence (gradient symbol $w(k,a)/k \sim k^{-(1+\alpha)}$ not $L^2$-integrable in three dimensions; see Sec.~\ref{sec:mathfound}, Appendix~\ref{app:continuum}). The adopted value $\alpha\approx 0.191$ satisfies this regularity requirement automatically. No fitting to cosmological data is performed; parameters are fixed \emph{a priori}.
...
\EDIT{To be clear: ILG is a fixed-kernel, source-side modification of the cosmological Poisson equation characterized by three fixed parameters $(C,\alpha,\tau_0)$ that are derivation-fixed---specified independently of cosmological data.} The modification leaves fluid equations and background cosmology unchanged at linear order, introduces no new dynamical fields, and automatically recovers GR at early times and small scales. Table~\ref{tab:ilg_glance} centralizes the complete model specification: all inputs required to compute observables.
