...
% ============================================================================
\begin{abstract}
We present the Recognition Calculus (RC) quantum gravity construction as a \emph{proved, zero-parameter} framework: all fundamental constants $\{c,\hbar,G,\alpha^{-1},\EDIT{\varphi},E_{\mathrm{coh}},\tau_0,\lambda_{\mathrm{rec}}\}$ and the mass-to-light ratio $M/L\simeq\EDIT{\varphi}$ are derived from the Meta-Principle (MP) with no free parameters. \EDIT{The RC exclusivity and completeness theorems referenced here are machine-verified in Lean within an audited certificate closure (the broader monolith contains additional modules still under completion).} The Planck/recognition identity $c^3\lambda_{\mathrm{rec}}^2/(\hbar G)=1/\pi$ is \emph{proved}, not assumed. Building on this foundation, we construct a background-field, de~Donder gauge, BRST-consistent path integral, outline a nonperturbative discrete-exterior-calculus (DEC) realization whose refinement properties are certified, and connect the RC admissible-units quotient to counterterm relations. We confirm Standard-Model anomaly cancellations, reproduce semiclassical black-hole thermodynamics, and translate RC-gated higher-curvature effects into gravitational-wave ``audit bands.'' The resulting theory is unique up to global unit rescalings; bi-interpretable with any zero-parameter competitor within the stated hypothesis set; and fully falsifiable via the declared audit identities and envelopes.
\end{abstract}
...
\EDIT{We write the golden ratio as $\varphi := (1+\sqrt{5})/2$; the symbol $\phi$ is reserved for fields/potentials (e.g.\ discrete cochains) and is not used for the golden ratio in this manuscript.}
...
\begin{enumerate}[(T1)]
    ...
    \item \textit{Zero-parameter derivations:} derive $\{c,\hbar,G,\alpha^{-1},\EDIT{\varphi},E_{\mathrm{coh}},\tau_0,\lambda_{\mathrm{rec}}\}$ and $M/L=\EDIT{\varphi}$ from MP with zero free parameters (ConstantsCert and MassToLightCert bundles).
...
\EDIT{Lean sources live in the main repository under \texttt{IndisputableMonolith/}, with certificate entrypoints under \texttt{IndisputableMonolith/URCGenerators/} and \texttt{IndisputableMonolith/Verification/}. The audit outputs used by this manuscript are recorded under \texttt{artifacts/certificates/} (e.g.\ \texttt{audit\_minimal.json}, \texttt{core\_audit.log}, and the dashboard JSON). Reproducing the certificates corresponds to building the repository with Lean (\texttt{lake build}) and re-running the recorded audit harness; the key requirement for claims in this paper is that the referenced certificate closure executes with no `sorry` placeholders.}
...
\begin{itemize}
    ...
    \item all dimensional constants fixed via the MP $\to$ ledger $\to \EDIT{\varphi}$ chain with zero free parameters and no hidden spurions;
...
\EDIT{To maintain auditability we offer a concise digest of the main Lean certificates referenced throughout the paper. The referenced certificate closure is executed with zero `sorry` placeholders (see the audit outputs in \texttt{artifacts/certificates/}); the broader repository contains additional modules still under completion and is tracked by a separate `sorry` audit.}
\begin{itemize}
...
From the RC derivation chain MP $\to$ ledger $\to \EDIT{\varphi}$ with cost uniqueness and eight-tick minimality, the following hold (certified in the RC corpus): $c=\ell_0/\tau_0$, $\hbar=E_{\mathrm{coh}}\tau_0$, $(c^3\lambda_{\mathrm{rec}}^2)/(\hbar G)=1/\pi$, $\alpha^{-1}=4\pi\cdot 11-\ln\EDIT{\varphi}-103/(102\pi^5)$, and $M/L\in\{1,\EDIT{\varphi},\EDIT{\varphi^2},\EDIT{\varphi^3}\}$ with characteristic value $M/L=\EDIT{\varphi}\simeq1.618$ (MassToLightCert). No external inputs or fitted parameters enter these derivations; units are fixed by the AbsoluteLayerCert.
...
Rotation-curve predictions & Derived kernel with $M/L=\EDIT{\varphi}$; audits ongoing \\
...
lemma brst_nilpotent (X : GhostFields) (h : SymTensor2) :
  ...
  -- proof omitted here; see BRSTConsistencyCert in the audited certificate closure
\end{lstlisting}
...
\begin{equation}
    \alpha^{-1}=4\pi\cdot 11-\ln\EDIT{\varphi}-103/(102\pi^5),
\end{equation}
...
\begin{enumerate}
    ...
    \item \textbf{Information-Theoretic Cost Term ($-\ln\EDIT{\varphi}$):} The stability of the ledger's eight-tick cycle is governed by a recursive, self-similar logic. The certified \texttt{CostUniqueness} theorem proves that the universal constant governing the growth and scaling of information in this process is the golden ratio, \EDIT{$\varphi$}. The term $-\ln\EDIT{\varphi}$ represents a fundamental information-theoretic ``cost'' or entropy associated with the self-stabilization of the interaction. It is subtracted from the base capacity, analogous to a renormalization effect but derived from the information content of the system's logic.
