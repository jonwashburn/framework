...
We develop a systematic analytic approach toward the Hodge conjecture, aiming to represent each rational Hodge class
\[
\gamma \in H^{2p}(X,\Q)\cap H^{p,p}(X)
\]
by an algebraic cycle.
The method reduces the problem to a realization statement for smooth closed strongly positive $(p,p)$--forms.
...
The algebraic realization of $\gamma$ is then obtained by subtracting the complete--intersection contribution and reducing to
$p\le n/2$ via Hard Lefschetz.
...
Preview only (not used as an input): the rigorous statement and proof are given in Theorem~\ref{thm:cal-coercivity}.
...
% ------------------------------------------------------------
\subsection*{Remark: a penalized route (not used in this paper)}
...
\end{itemize}
The exact-class conclusion is enforced by Proposition~\ref{prop:cohomology-match} together with the per-face matching/transport mechanism (Proposition~\ref{prop:integer-transport}). For $p=n/2$, this still requires an additional closure input to guarantee that the filling current $U_\epsilon$ in Proposition~\ref{prop:cohomology-match} can be taken with arbitrarily small mass (the middle-dimensional closure lemma (proved later in Section 8.B)).
rather than relying on a decay exponent in $h$.
Thus the hypothesis \textnormal{(H2)} in Theorem~\ref{thm:spine-quantitative} holds for $p<n/2$ under the present estimates; the borderline case $p=n/2$ is reduced to supplying one of the additional closure inputs listed in the middle-dimensional closure lemma (proved later in Section 8.B).
\end{proposition}
...
This is standard; see \cite{Fed69}. By Federer--Fleming
...
This is standard; see \cite{HL82,King71}. For the K\"ahler/Wirtinger calibration, $\psi$--calibration forces the approximate tangent planes of $T$ to be complex $(n-p)$--planes with the standard complex orientation (Harvey--Lawson
...
\emph{Chow/GAGA require $X$ projective (or more generally Moishezon); then complex analytic subvarieties of $X$ are algebraic.}
 (Chow/GAGA (see \cite{Hartshorne77,GH78,Serre56})).
\end{proof}
...
\]
where $V_j\subset X$ are irreducible complex analytic subvarieties of codimension $p$.  If, moreover, $X$ is projective, then each $V_j$ is algebraic by Chow/GAGA (Chow/GAGA (see \cite{Hartshorne77,GH78,Serre56})), so $[\gamma]\in H^{2p}(X;\Q)$ is an algebraic class.
\end{theorem}
...
\emph{Chow/GAGA require $X$ projective (or more generally Moishezon); then complex analytic subvarieties of $X$ are algebraic.}
 (see \cite{Hartshorne77,GH78,Serre56}).
\end{proof}
...
By Harvey--Lawson
 structure theory, a $\psi$-calibrated integral cycle in a K\"ahler manifold is a positive sum of currents of integration
over irreducible complex analytic subvarieties of codimension $p$.
Since $X$ is projective, Chow's theorem
...
Let $P_{\mhol}(z,z')$ be the Bergman kernel
...
This is standard; see \cite{MaMarinescu13OffDiag,MaMarinescu07}. See e.g.\ \cite{MaMarinescu13OffDiag,MaMarinescu07} for statements implying \eqref{eq:bergman-weighted-rescaled} (near off-diagonal with $C^2$ control).
...
This is standard; see \cite{HL82}. $\psi$--calibrated in the sense of Wirtinger/Harvey--Lawson \cite{HL82}.
...
This is standard; see \cite{HL82,King71}. the associated integration current $[Y]$ is $\psi$--calibrated (Wirtinger), hence strongly positive and closed; see \cite{HL82,King71}.
...
In the global-coherence regime (Proposition), the boundary slices on $F$ are parameterized by the \emph{same} integer-weighted discrete measure
...
For example, Lemma produces a nested ordered sequence on a sphere (uniform density), and scaling embeds it into $B_{C_0\varrho h}(0)$.
...
{We emphasize again: $m$ is fixed in the SYR definition; no $m\to\infty$ limit is taken. Smallness is obtained by taking $\delta,\varepsilon\to0$ and refining the mesh (equivalently choosing $\mhol$ large enough for the local realization scale), as quantified in Lemma.}
...
In particular, since $F$ is $C^1$ and $\partial_wF(u,g(u))$ is invertible for all $u\in U_u$, the implicit function theorem implies $g\in C^1(U_u)$; hence the displayed bound holds for every $u\in U_u$.
...
This step is now achieved by Proposition, which builds holomorphic complete intersections
...
$\{\rho/4\le |z|\le \rho/2\}$.
This is standard; see \cite{Hormander65,Demailly12}. \emph{H\"ormander $L^2$ $\bar\partial$: $(X,\omega)$ complete K\"ahler (or use a complete exhaustion), $(L^m,h^m)$ semipositive/positive curvature as required, and RHS is $\bar\partial$-closed with finite weighted $L^2$ norm.}
...
\qquad
\sup_{B_{R/\sqrt{\mhol}}}\Bigl(|f_j-w_j|+\sqrt{\mhol}\,|\nabla(f_j-w_j)|\Bigr)\le \varepsilon_{\mhol}\ (1\le j\le p),
\]
...
By the Federer--Fleming
 dual characterization of the flat norm, it suffices to test $\partial T^{\mathrm{raw}}$
...
This is standard; see \cite{FF60,Fed69}. By the Euclidean isoperimetric
 (filling) inequality for integral currents (Federer--Fleming \cite{FF60}; see also Federer \cite{Fed69}),
there exists an integral $k$--current $Q$ in $\R^N$ with $\partial Q=\iota_\# R$ and
\[
\Mass(Q)\ \le\ C_{N,k}\,\Mass(\iota_\# R)^{\frac{k}{k-1}}.
\]
This is standard; see \cite{Fed69}. Define $Q_R:=\pi_\# Q$, which is an integral $k$--current in $X$ (pushforward under a Lipschitz map preserves integrality; \cite{Fed69}).
Then
\[
...
By Federer--Fleming compactness for integral currents on the compact manifold $X$, using $\sup_k\Mass(T_k)<\infty$ and $\partial T_k=0$, after passing to a subsequence we obtain $T_k\to T$ in the flat norm. \begin{itemize}
...
No stationarity/first-variation hypotheses are required for this step.  (One may also consider the associated integral varifolds, but this is optional
and not used in the Hodge conclusion.)
\end{remark}
...
This is standard; see \cite{HL82, King71}. \emph{$X$ is K\"ahler and $\psi$ is the Wirtinger calibration (a closed $(n-p,n-p)$ form). Since $T$ is an \emph{integral} $\psi$--calibrated cycle, Harvey--Lawson's structure theorem (and King for holomorphic chains) implies $T$ is a positive holomorphic $(n-p)$--cycle.}
Therefore $T$ is a positive holomorphic $(n-p)$--cycle.

If $X$ is projective, then each analytic component is algebraic by Chow/GAGA
; see \cite{Serre56,GH78,Hartshorne77}. Hence $[\gamma]$ is represented by an algebraic cycle with rational coefficients.
\end{proof}
...
Under uniform bounds on $\Mass(T_j)$ and $\Mass(\partial T_j)$, Federer--Fleming compactness implies that any flat limit of integral
...
the signed decomposition is finite-dimensional linear algebra inside $H^{2p}(X,\Q)\cap H^{p,p}(X)$; it uses only rationality and the existence of a K\"ahler class (after clearing denominators), with no geometric measure input.
\end{remark}
...
Since $X$ is projective, each $V_j$ is algebraic by Chow/GAGA
 (see \cite{Hartshorne77,GH78,Serre56}), so $m[\gamma^+]$ is algebraic and therefore $\gamma^+$ is algebraic.
...
This is standard; see \cite{Hartshorne77,GH78,Serre56}. \end{remark}
...
Chow's theorem identifies analytic cycles with algebraic cycles, and GAGA transfers the resulting cycle class
to the algebraic category.
...
\bibitem{Wells}R.~O.~Wells, Jr.\ \emph{Differential Analysis on Complex Manifolds}. Graduate Texts in Mathematics, vol.~65. Springer, New York, 3rd ed., 2008.
\bibitem{BBS08}
