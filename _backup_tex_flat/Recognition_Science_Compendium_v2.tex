...
\textbf{Status:} The \emph{symbolic} formula is derived in the RC corpus (seed--gap--curvature decomposition). A rigorous interval bound is machine-verified in the Lean numerics layer (e.g.\ \(137.030 < \alpha^{-1} < 137.039\)); full CODATA-level precision remains an empirical/higher-precision numerics task.

The heuristic argument proceeds as follows:
\begin{enumerate}
\item The 8-tick neutral window (T7) suggests $2^3 = 8$ fundamental phases
\item The seed term is fixed by 3D isotropy and ledger closure: \(4\pi \cdot 11\)
\item The gap term is fixed by the eight-tick projection weight: \(f_{\mathrm{gap}} = w_8 \ln\phival\)
\item The curvature/tiling correction contributes \(+\frac{103}{102\pi^5}\) (equivalently \(\delta_\kappa = -\frac{103}{102\pi^5}\))
\end{enumerate}

Equivalent rearrangement:
\[
\alpha^{-1} = 4\pi \cdot 11 - \left(f_{\mathrm{gap}} + \delta_\kappa\right),\qquad
f_{\mathrm{gap}} = w_8\ln\phival,\quad \delta_\kappa = -\frac{103}{102\pi^5}.
\]
