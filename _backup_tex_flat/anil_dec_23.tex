...
\paragraph{Two different things are being called ``\(f^{\mathrm{Rec}}\)'' (this is the source of the disagreement).}
There are \emph{two} distinct quantities in play:
\begin{itemize}
  \item \textbf{Band value (closed form).} The single-anchor phenomenology paper compares an RG-side residue \(f_i(\muStar,m_i)\)
  to the closed-form band value \(\Fgap(Z_i)\) from \eqref{eq:Fgap}. Some drafts casually call this band value a ``recognition residue''.

  \item \textbf{Extracted leftover from a mass factorization.} In Anil's Section~4, \(f^{\mathrm{Rec}}_i\) is defined operationally as
  ``whatever is left'' after choosing a structural prefactor \(\mStruct_i\) and subtracting SM transport. This extracted quantity is
  \emph{definition-dependent}: if \(\mStruct_i\) is chosen so that it already matches the observed masses (up to small transport),
  then the extracted leftover will be small and will \emph{not} numerically match \(\Fgap(Z_i)\).
\end{itemize}
This memo therefore avoids using the same symbol for both objects: \(\Fgap(Z)\) is the band comparator; any extracted leftover should be
written explicitly as ``extracted residual'' (defined below).
...
If one wishes to connect a \emph{structural} prefactor to measured masses, a generic factorization is:
\begin{equation}
  \mPole_i
  \;=\;
  \mStruct_i\ \phiG^{\,f^{\mathrm{(extracted)}}_i}\ \phiG^{\,\fRG_i},
  \label{eq:mass-factorization}
\end{equation}
where the SM part is only \(\fRG_i\) (computed from declared RG kernels/policies),
and \(f^{\mathrm{(extracted)}}_i\) is the leftover exponent implied by the chosen \(\mStruct_i\).
...
\paragraph{What Anil's Eq.\ ``\(f^{\mathrm{Rec}}=\Delta_{\mathrm{obs}}-\fRG\)'' means in plain terms.}
Rearranging \eqref{eq:mass-factorization} at the anchor gives an \emph{extracted residual}
\[
  f^{\mathrm{(extracted)}}_i
  \;=\;
  \frac{1}{\ln\phiG}\ln\!\Bigl(\frac{m_i(\muStar)}{\mStruct_i}\Bigr),
  \qquad
  \text{and the test is}\quad
  f^{\mathrm{(extracted)}}_i \stackrel{?}{\approx} \Fgap(Z_i).
\]
This comparison is meaningful \emph{only if} \(\mStruct_i\) is fixed independently of \(m_i\) (except via a declared calibration protocol)
\emph{and} the definition of \(\mStruct_i\) has not already absorbed the same constant band factor one is trying to test.
In the SM charged-fermion sectors, \(Z\) (hence \(\Fgap(Z)\)) is \emph{constant within each sector}, so any sector-wide constant factor can be
moved back and forth between \(\mStruct_i\) (via a sector offset) and the extracted residual. Without an \emph{independent} definition of the sector
yardstick, the statement ``\(f^{\mathrm{(extracted)}}=\Fgap(Z)\)'' is not identifiable from mass data alone.
...
\paragraph{Resolution (what changes in the manuscript).}
We should \emph{not} write (or imply) ``the extracted leftover from a structural mass factorization equals \(\Fgap(Z)\)''
unless we also provide (i) an independent, non-mass-fitted definition of the sector yardsticks and (ii) a clear convention that prevents absorbing
sector-constant factors into the yardstick.
Otherwise, Anil's numerical check will always show a mismatch, because the yardstick choice can soak up the band factor.

\paragraph{So what is a defensible statement?}
Two clean options:
\begin{itemize}
  \item \textbf{Option A (RG-only paper).} Keep \(\Fgap(Z)\) strictly as the \emph{RG-side comparator} in the single-anchor phenomenology paper:
  compare \(f_i(\muStar,m_i)\) (as defined by the RG integral) directly to \(\Fgap(Z_i)\). Do not introduce a mass-factorization \(f^{\mathrm{Rec}}\) at all.

  \item \textbf{Option B (mass-factorization paper).} If we want a mass-factorization interpretation, we must (i) freeze the sector yardsticks from
  independent theory-side derivations/anchors (not from the same masses) and (ii) define the split so that a sector-constant factor cannot be silently moved
  between the yardstick and the leftover. Only then does comparing an extracted residual to \(\Fgap(Z)\) become a nontrivial test.
\end{itemize}

\paragraph{Clarifying \(\mStruct\) (why his numbers ``don’t match'').}
If \(\mStruct_i\) is taken to be \(B_{B(i)}\,\Ecoh\,\phiG^{r_i+r_0(B(i))}\) with \(r_0\) chosen to make the sector match masses,
then the extracted leftover \(f^{\mathrm{(extracted)}}_i\) will be small by construction and will not equal \(\Fgap(Z)\).
This does not falsify \(\Fgap(Z)\); it shows the manuscript must not claim an equality between \(\Fgap(Z)\) and an extracted leftover
without pinning down the yardstick convention independently.

\paragraph{What we mean by \(\mStruct\) (what would make the test meaningful).}
For a meaningful test, \(\mStruct_i\) must be specified so it does \emph{not} import \(m_i\) species-by-species.
One admissible structural form is sector-global, e.g.
\[
  \mStruct_i := B_{B(i)}\,\Ecoh\,\phiG^{r_i+r_0(B(i))},
\]
...
\section{Non-circular structural hold-out (run and reported)}
\label{sec:holdout-include}

The note's Section~4 critique highlights the risk of \emph{self-use} (tautology) when defining any
``structural mass'' and then extracting a residue from the same measured mass being tested.
To make the methodology explicit, we include a strict hold-out protocol and its results here:
\input{docs/structural_holdout_protocol.tex}
