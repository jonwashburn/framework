...
% ==========================================================
%
% SLIVERPATH VARIANT (Dec 2025)
% - This file is a copy of `hodge-dec-6-handoff.tex` with Step 4 restructured to
%   make the Bergman-scale “sliver” microstructure route the primary path.
% - The original file is intentionally left untouched (you are editing it elsewhere).
%
...
% (Used throughout: \Mass(T) is the mass of a current T.)
...
\begin{abstract}
	...
	\emph{conditional} on the remaining microstructure/gluing step that upgrades
	local calibrated sheet stacks to global cycles with negligible correction
	(see Remark~\ref{rem:glue-gap} and the target Microstructure Theorem
	\ref{thm:microstructure-flatglue}).
\end{abstract}
...
step that reduces the full Hodge conjecture to the effective-class case (and
would make the proof unconditional once the effective-class realization step is
fully established).
...
The proof strategy has three main components (with one remaining conditional input):
\begin{enumerate}
...
algebraic by Harvey--Lawson and Chow.  The remaining nontrivial input here is
the microstructure/gluing estimate needed to pass from local calibrated pieces
to global cycles with negligible correction (Remark~\ref{rem:glue-gap}).
\item \textbf{Conclusion:} $\gamma = [Z^{+}] - [Z^{-}]$ is algebraic.
\end{enumerate}

\begin{remark}[Status and remaining blockers (Dec 2025)]\label{rem:blockers}
At present, all steps of the manuscript are classical and quantitative \emph{except} for the final microstructure/gluing step.
This file records an explicit ``sliver-first'' route (Bergman-scale cells) designed to make unconditional closure plausibly feasible.
The remaining blockers are:
\begin{enumerate}
\item[\textnormal{B1}] \textbf{Microstructure theorem (single target).}
Prove Theorem~\ref{thm:microstructure-flatglue}, i.e.\ construct calibrated local pieces whose global raw current has
$\mathcal F(\partial T^{\mathrm{raw}})=o(m)$.
Concretely, after the reductions in Substep~4.2 this amounts to enforcing \emph{simultaneous} face matching across the entire adjacency graph:
for every interior interface $F=Q\cap Q'$, the induced transverse parameterizations must be template-coherent
(Lemma~\ref{lem:template-displacement} or Lemma~\ref{lem:template-displacement-edits}) with only an $O(h)$ edit fraction
(Lemma~\ref{lem:template-edits-oh} / Corollary~\ref{cor:global-prefix-template-coherence}),
so that each per-face mismatch contributes only $O(h^2)\times$(boundary mass) in flat norm.

\smallskip\noindent
\textbf{Internal structure of \textnormal{B1}.}
For clarity, the remaining microstructure/gluing input splits into three (coupled) subproblems:
\begin{enumerate}
\item[\textnormal{(B1a)}] \emph{Globally labeled directions with slow variation.}
Avoid combinatorial instability of moving Carath\'eodory supports by fixing a finite direction dictionary (whose size may grow as $h\downarrow 0$)
and choosing weights by a strongly convex projection.
See Remark~\ref{rem:stable-direction-dictionary} and Lemma~\ref{lem:regularized-simplex-lipschitz}.
This reduces neighbor direction pairing to a fixed label matching (up to the net approximation error), so the direction-label part of \textnormal{B1}
is treated as standard.
\item[\textnormal{(B1b)}] \emph{Graph-Lipschitz integer multiplicities under global period constraints.}
Once the target weights vary Lipschitzly, rounding alone gives $O(h)$ relative count variation across neighbors
(Lemma~\ref{lem:lipschitz-rounding-counts}), hence only $O(h)$ prefix edits in the nested-template scheme
(Remark~\ref{rem:nested-template-scheme} and Proposition~\ref{prop:prefix-template-coherence}, or globally by
Corollary~\ref{cor:global-prefix-template-coherence}).
The global cohomology/period constraints can be enforced by fixed-dimensional discrepancy rounding
(Lemma~\ref{lem:barany-grinberg} / Proposition~\ref{prop:cohomology-match}), which changes each target count by at most $1$.
Such bounded corrections do not spoil the $O(h)$ edit regime once the per-cell counts satisfy $N_Q\gtrsim h^{-1}$ (Remark~\ref{rem:bounded-corrections}).
Thus, at the level of template bookkeeping, the remaining obstruction is not integer rounding but geometric boundary control.
\item[\textnormal{(B1c)}] \emph{Boundary-budget geometry for slivers on an actual cell decomposition.}
In the Bergman-scale sliver route, the global estimate uses a per-cell bound on $\Mass(\partial S_Q)$ (Hypothesis~\ref{hyp:BB}),
which is proved in a smooth uniformly convex \emph{model} cell by Proposition~\ref{prop:sliver-boundary-budget}.
Implementing an analogue for the actual $C^2$ cell decomposition (Lemma~\ref{lem:rounded-cells}) is a geometric input bundled into the target theorem
Theorem~\ref{thm:microstructure-flatglue}.
\end{enumerate}
Everything else in the chain is standard once such coherent local data exist (Remark~\ref{rem:microstructure-reduction}).
\item[\textnormal{B2}] \textbf{Local sliver microstructure (holomorphic) --- resolved.}
The \emph{flat} transverse-measure approximation is now explicit (Proposition~\ref{prop:flat-sliver-local} for spherical targets and
Proposition~\ref{prop:flat-sliver-local-ball} for volumetric targets, via radial shell discretization).
The corresponding \emph{holomorphic} upgrade on Bergman-scale ball-like cells is packaged as Proposition~\ref{prop:sliver-local}
(built from Lemma~\ref{lem:bergman-control} and Proposition~\ref{prop:finite-template}).
Thus the remaining obstruction is no longer local realizability on a single cell but the global compatibility/gluing problem (\textnormal{B1}).
\item[\textnormal{B3}] \textbf{Template displacement / face-slice stability at scale $h^2$ --- resolved.}
This is now reduced to standard geometry in the fixed-template setting.
Lemma~\ref{lem:face-map-variation} records why the face-slice maps vary by $O(h)$ across adjacent cells (in operator norm) once
plane directions are paired and $\beta$ is smooth, and Lemma~\ref{lem:template-displacement} converts this $O(h)$ variation (together with
support radius $O(h)$) into the desired \emph{per-face} flat-norm mismatch bound at scale $h^2$.
The remaining issue is \emph{not} the $h^2$ scale but enforcing template coherence/pairing across all faces simultaneously (Blocker \textnormal{B1}).
\item[\textnormal{B4}] \textbf{Cell geometry at Bergman scale --- reduced to standard smoothing.}
This is largely geometric/technical rather than conceptual.
Lemma~\ref{lem:normal-distortion} records the standard $(1+O(h^2))$ distortion control in geodesic normal coordinates.
Lemma~\ref{lem:rounded-cells} records that one can work with a $C^2$ cell decomposition at scale $h$ with curvature bounded by $O(1/h)$.
The remaining geometric content needed for the Bergman-scale sliver route is encapsulated by the boundary-budget hypothesis
Hypothesis~\ref{hyp:BB} (cf.\ Remark~\ref{rem:uniformly-convex-partition-caution}).
\end{enumerate}
Everything downstream (Federer--Fleming filling, cohomology rounding/discrepancy, calibrated compactness, Harvey--Lawson/Chow)
is standard once \textnormal{B1} is established.
\end{remark}

\begin{remark}[Hard Lefschetz reduction: it suffices to treat $p\le n/2$]\label{rem:lefschetz-reduction}
For the full Hodge conjecture on a smooth projective $n$--fold, it is enough to prove algebraicity of rational Hodge classes in degrees
$2p$ with $p\le n/2$.
Indeed, if $p>n/2$, Hard Lefschetz implies that cup product
\(
L^{2p-n}:H^{2(n-p)}(X,\Q)\to H^{2p}(X,\Q)
,\ \alpha\mapsto \alpha\smile[\omega]^{2p-n}
\)
is an isomorphism, so every Hodge class $[\gamma]\in H^{2p}(X,\Q)\cap H^{p,p}(X)$ can be written uniquely as
\[
[\gamma]\ =\ [\eta]\smile[\omega]^{2p-n}
\qquad\text{for some }\ [\eta]\in H^{2(n-p)}(X,\Q)\cap H^{n-p,n-p}(X).
\]
Since $[\omega]$ is the hyperplane class of a very ample line bundle, it is algebraic, and multiplying by $[\omega]^{2p-n}$ corresponds to intersecting
cycles with $(2p-n)$ generic hyperplane sections.  Thus algebraicity of $[\eta]$ implies algebraicity of $[\gamma]$.

\smallskip
\noindent
This reduction is also consistent with the Bergman-scale weighted gluing numerology: in the reduced range $p\le n/2$ one has
$k:=2n-2p\ge n$, hence $k>n-1$, which is exactly the inequality that appears in the exponent bookkeeping in
Proposition~\ref{prop:weighted-sliver-glue}.
\end{remark}

\begin{remark}[Caution: finite-direction laminates are too rigid in flat models]\label{rem:finite-direction-nogo}
The remaining Blocker~\textnormal{B1} is genuinely \emph{global} and cannot be dismissed by a ``fixed finite direction'' laminate heuristic.
In the flat torus model $T^6=\C^3/\Lambda$ and $p=2$, one can prove a sharp rigidity statement: any closed $\varphi_0$--calibrated integral current
whose tangent planes take values in a fixed finite set of calibrated directions forces the associated macroscopic cone coefficients to be constant,
so nonconstant cone-valued closed forms cannot be realized by such finite-direction stacks.
See \texttt{hodge-blocker.tex} (Theorem ``finite-direction no-go'') for a clean statement and proof.

\smallskip
\noindent
Thus any successful microstructure/gluing theorem in middle codimension must either use \emph{infinitely many} calibrated directions in an essential way,
or exploit genuinely curved holomorphic pieces (not just parallel plane stacks).
\end{remark}

\begin{theorem}[Microstructure / flat-norm gluing (target theorem)]\label{thm:microstructure-flatglue}
Let $(X,\omega)$ be a smooth projective K\"ahler manifold of complex dimension $n$ and fix $1\le p\le n$.
Let $[\gamma]\in H^{2p}(X,\Q)\cap H^{p,p}(X)$ be an \emph{effective} class with a smooth closed cone-valued representative
$\beta$ (so $\beta(x)\in K_p(x)$ for all $x\in X$).
Let $\psi:=\omega^{n-p}/(n-p)!$.
Then there exist a sequence of integers $m\to\infty$ together with auxiliary parameters
\[
h=h(m)\downarrow 0,\qquad \delta=\delta(m)\downarrow 0,\qquad \varepsilon=\varepsilon(m)\downarrow 0
\qquad (m\to\infty),
\]
where $h$ is the cell scale, $\delta$ is the local barycenter/quantization tolerance, and $\varepsilon$ is the small-angle/graph parameter
for the holomorphic sheets (typically one targets the Bergman-scale choice $h\asymp m^{-1/2}$),
and for each such $m$ a $\psi$--calibrated integral $(2n-2p)$--current
$T^{\mathrm{raw}}_m$ representing $\mathrm{PD}(m[\gamma])$ up to a boundary correction, such that:
\begin{enumerate}
\item[\textnormal{(i)}] (\textbf{Correct mass scale}) $\Mass(T^{\mathrm{raw}}_m)=m\int_X \beta\wedge\psi+o(m)$;
\item[\textnormal{(ii)}] (\textbf{Flat-norm small boundary}) there exists an error fraction $\varepsilon_{\mathrm{glue}}(m;h,\delta,\varepsilon)\ge 0$
with $\varepsilon_{\mathrm{glue}}(m;h,\delta,\varepsilon)\to 0$ as $m\to\infty$ (under $h,\delta,\varepsilon\to 0$) such that
\[
\mathcal F(\partial T^{\mathrm{raw}}_m)\ \le\ \varepsilon_{\mathrm{glue}}(m;h,\delta,\varepsilon)\cdot m.
\]
\end{enumerate}
Consequently there exist integral correction currents $R_{\mathrm{glue},m}$ with
$\partial R_{\mathrm{glue},m}=-\partial T^{\mathrm{raw}}_m$ and $\Mass(R_{\mathrm{glue},m})=o(m)$,
so that $T_m:=T^{\mathrm{raw}}_m+R_{\mathrm{glue},m}$ is a closed $\psi$--calibrated integral current with
$[T_m]=\mathrm{PD}(m[\gamma])$ and $\Mass(T_m)=m\int_X\beta\wedge\psi+o(m)$.
In particular, $\beta$ is SYR-realizable (Definition~\ref{def:syr}) and $[\gamma]$ is algebraic by Harvey--Lawson and Chow.
\end{theorem}

\begin{proof}[Proof (conditional; reduction to explicit local inputs)]
This theorem is the “microstructure/gluing’’ step.  The paper proves that once one can build local calibrated pieces with a \emph{sublinear}
flat-norm boundary defect, the rest of the SYR$\Rightarrow$algebraicity chain is standard.

\smallskip\noindent
\textbf{Step 1 (local sheet pieces; mass scale).}
Fix $m$ and choose a sufficiently fine cell decomposition at mesh $h=h(m)$ so that $\beta$ and the K\"ahler geometry are nearly constant on each cell.
Implement the local construction of Substep~4.1: on each cell $Q$, express $\beta(x_Q)$ as a convex combination of calibrated extremals and build
families of $\psi$--calibrated holomorphic sheets whose restrictions to $Q$ form an integral current $S_Q$ with
\[
\Mass(S_Q)=m\int_Q \beta\wedge\psi+o(m\,\mathrm{vol}(Q)).
\]
For later global matching, it is convenient to implement the convex decomposition using a \emph{fixed} local direction dictionary with Lipschitz weights,
as in Remark~\ref{rem:stable-direction-dictionary} and Lemma~\ref{lem:regularized-simplex-lipschitz}, rather than an unstable moving Carath\'eodory support.
Summing gives a global raw current $T^{\mathrm{raw}}_m:=\sum_Q S_Q$ with the claimed mass scale
$\Mass(T^{\mathrm{raw}}_m)=m\int_X\beta\wedge\psi+o(m)$.
This part of the argument is quantitative but classical once the local holomorphic sheet pieces exist with uniform $C^1$ control.

\smallskip\noindent
\textbf{Step 2 (flat-norm boundary estimate: two explicit routes).}
The remaining issue is to show $\mathcal F(\partial T^{\mathrm{raw}}_m)=o(m)$.
The manuscript isolates two routes:
\begin{enumerate}
\item[\textnormal{(I)}] \emph{Dense-sheet / fixed-template route.}  If the sheet families satisfy the template coherence hypotheses
\textnormal{(T1)}--\textnormal{(T2)} on every interior interface $F$ (Proposition~\ref{prop:template-flat-glue}),
then Proposition~\ref{prop:template-flat-glue} gives
\[
\mathcal F(\partial T^{\mathrm{raw}}_m)\ \le\ C\,(h+\varepsilon)\,m\ =\ o(m)
\]
along any schedule with $h+\varepsilon\to 0$.
\item[\textnormal{(II)}] \emph{Bergman-scale sliver route.}  If each cell admits a sliver microstructure obeying the boundary-budget estimate
(Hypothesis~\ref{hyp:BB}) and the interface slices are template-coherent up to displacement (Lemma~\ref{lem:template-displacement},
or with small edits as in Lemma~\ref{lem:template-displacement-edits}), then Proposition~\ref{prop:weighted-sliver-glue} gives an explicit sublinear bound
$\mathcal F(\partial T^{\mathrm{raw}}_m)=o(m)$ at $h\asymp m^{-1/2}$ under the polynomial-growth condition on the number of slivers per cell.
\end{enumerate}
Either route yields the flat-norm small-boundary conclusion \textnormal{(ii)}.

\smallskip\noindent
\textbf{Step 3 (flat norm $\Rightarrow$ small-mass filling).}
Let $k:=2n-2p$ and set $\eta_m:=\mathcal F(\partial T^{\mathrm{raw}}_m)=o(m)$.
By definition of $\mathcal F$, write $\partial T^{\mathrm{raw}}_m=R_m+\partial Q_m$ with
$\Mass(R_m)+\Mass(Q_m)\le 2\eta_m$.
Since $\partial T^{\mathrm{raw}}_m$ is a boundary, $R_m$ is null-homologous, so by the Federer--Fleming isoperimetric inequality there is an
integral $k$--current $Q_{R,m}$ with $\partial Q_{R,m}=R_m$ and
$\Mass(Q_{R,m})\le C\,\Mass(R_m)^{k/(k-1)}\le C\,(2\eta_m)^{k/(k-1)}$.
Define the correction current
\[
R_{\mathrm{glue},m}:=-(Q_m+Q_{R,m}).
\]
Then $\partial R_{\mathrm{glue},m}=-\partial T^{\mathrm{raw}}_m$ and
\[
\Mass(R_{\mathrm{glue},m})\ \le\ 2\eta_m + C\,(2\eta_m)^{k/(k-1)}\ =\ o(m).
\]
Setting $T_m:=T^{\mathrm{raw}}_m+R_{\mathrm{glue},m}$ gives a closed integral current with the stated mass asymptotics.
Finally, since each local piece is $\psi$--calibrated, the same holds for $T_m$.
\end{proof}

\begin{remark}[How later estimates reduce Theorem~\ref{thm:microstructure-flatglue}]\label{rem:microstructure-reduction}
Theorem~\ref{thm:microstructure-flatglue} is \emph{exactly} the remaining microstructure/gluing input.
The manuscript isolates two explicit quantitative routes to its conclusion once suitable coherent local sheet data are available:
\begin{enumerate}
\item[\textnormal{(I)}] (\textbf{Constant-mass / dense sheets}) If one can implement the local sheet construction on mesh-$h$ cells
with the template coherence hypotheses \textnormal{(T1)}--\textnormal{(T2)} in Proposition~\ref{prop:template-flat-glue}, then
$\mathcal F(\partial T^{\mathrm{raw}})\lesssim (h+\varepsilon)\,m$ and hence $\mathcal F(\partial T^{\mathrm{raw}})=o(m)$ as $h+\varepsilon\to 0$.
\item[\textnormal{(II)}] (\textbf{Bergman-scale / slivers}) If one can implement a sliver microstructure on ball-like cells and enforce
face-by-face template coherence in the sense of Lemma~\ref{lem:template-displacement}, then Proposition~\ref{prop:weighted-sliver-glue}
gives an explicit sublinear bound $\mathcal F(\partial T^{\mathrm{raw}})=o(m)$ at the Bergman scale $h\asymp m^{-1/2}$ provided the number of slivers per cell
grows at most polynomially with exponent $<k-(n-1)$ (with $k:=2n-2p$) and $\varepsilon=o(1)$.
\end{enumerate}
Once any such flat-norm bound is available, the existence of a correction current $R_{\mathrm{glue}}$ with $\Mass(R_{\mathrm{glue}})=o(m)$ is standard
by the flat-norm decomposition and the Federer--Fleming isoperimetric inequality (see the argument in Remark~\ref{rem:glue-gap}).
\end{remark}
...
\begin{proof}
	...
	|\alpha^{(p+1,p-1)}_x|^2
	+ |\alpha^{(p-1,p+1)}_x|^2
	+ \|(\alpha^{(p,p)}_x-(\gharm)_x)_{\prim}\|^2
	+ d\,\mu(x)^2.
	\]
...
\begin{remark}[A stable alternative: fixed direction dictionaries and Lipschitz weights]\label{rem:stable-direction-dictionary}
Lemma~\ref{lem:caratheodory-general} is sufficient for many qualitative steps, but its moving Carath\'eodory support
can change combinatorially from cell to cell.  For the global face-matching problem (Blocker~\textnormal{B1}), it is
often preferable to use a \emph{fixed} finite direction dictionary (whose size is allowed to grow as $h\downarrow 0$)
and solve for the coefficients by a strongly convex projection.

Concretely, in a normal-coordinate chart at scale $h$, choose a finite set of normalized calibrated extremals
$\{\xi_r\}_{r=1}^R\subset G_p$ (e.g.\ an $\varepsilon_h$--net with $\varepsilon_h\ll h$ in the fiber metric).
Then represent $\widehat\beta(x):=\beta(x)/\langle\beta(x),\psi_x\rangle$ by weights $w(x)\in\Delta^R$ obtained as the unique minimizer of a
strictly convex quadratic functional.  This produces globally labeled directions $r\in\{1,\dots,R\}$ and, by standard
sensitivity theory for strongly convex programs, yields Lipschitz (hence $O(h)$ neighbor-stable) weights on the mesh.

The next lemma records the abstract analytic statement (fiberwise, in a fixed inner-product space) used in this reduction.
\end{remark}

\begin{lemma}[Lipschitz dependence of regularized simplex weights]\label{lem:regularized-simplex-lipschitz}
Let $(V,\langle\cdot,\cdot\rangle)$ be a finite-dimensional real inner-product space and fix vectors
$\xi_1,\dots,\xi_R\in V$ and a parameter $\lambda>0$.
Let $\Delta^R:=\{w\in\R^R:\ w_r\ge 0,\ \sum_{r=1}^R w_r=1\}$.
For $u\in V$ define
\[
w(u)\ :=\ \arg\min_{w\in\Delta^R}\ \Bigl\|u-\sum_{r=1}^R w_r\,\xi_r\Bigr\|^2\ +\ \lambda\,\|w\|_{\R^R}^2.
\]
Then $w(u)$ exists, is unique, and the map $u\mapsto w(u)$ is Lipschitz:
there is a constant $C=C(\xi_1,\dots,\xi_R,\lambda)$ such that
\[
\|w(u)-w(u')\|_{\R^R}\ \le\ C\,\|u-u'\|_V
\qquad\text{for all }u,u'\in V.
\]
\end{lemma}

\begin{proof}[Sketch]
The objective is continuous and $\Delta^R$ is compact, so a minimizer exists.
Since $w\mapsto \lambda\|w\|^2$ is $\lambda$--strongly convex on $\R^R$, the full objective is strongly convex on the convex set $\Delta^R$,
hence the minimizer is unique.

Write $A:\R^R\to V$ for the linear map $A w=\sum_{r=1}^R w_r\xi_r$ and set
$F_u(w):=\|u-Aw\|^2+\lambda\|w\|^2$.
Strong convexity gives the standard variational inequality
\[
\langle \nabla F_u(w(u))-\nabla F_u(w(u')),\,w(u)-w(u')\rangle\ \ge\ \lambda\,\|w(u)-w(u')\|^2.
\]
Since $\nabla F_u(w)=2A^{*}(Aw-u)+2\lambda w$, subtracting the gradients for $u$ and $u'$ yields
\[
\nabla F_u(w(u'))-\nabla F_{u'}(w(u'))\ =\ 2A^{*}(u'-u),
\]
and therefore, by Cauchy--Schwarz,
\[
\lambda\,\|w(u)-w(u')\|^2
\ \le\ 2\|A^{*}\|_{\mathrm{op}}\,\|u-u'\|\,\|w(u)-w(u')\|.
\]
Cancel $\|w(u)-w(u')\|$ to obtain the Lipschitz bound with $C=2\|A^{*}\|_{\mathrm{op}}/\lambda$.
\end{proof}
...
\begin{remark}[Many local pieces: jet separation]\label{rem:jet-separation}
The above “upgrade’’ can be carried out simultaneously for \emph{many} local target planes and translations, because high tensor powers
$L^k$ separate jets at finitely many points: for $k\gg 1$ the evaluation map to finite collections of $1$-jets (or $2$-jets) is surjective.
Equivalently, one can prescribe the linear part and constant term of the local defining functions at finitely many chosen centers and then
use standard transversality/Bertini arguments to ensure smooth complete intersections.
This flexibility is particularly relevant for the “sliver’’ regime (Remark~\ref{rem:sliver}), where one may need many small-mass local pieces.
\end{remark}
\begin{lemma}[Jet separation for high tensor powers (standard)]\label{lem:jet-sep}
Let $L\to X$ be an ample line bundle on a smooth projective variety.
Fix an integer $r\ge 1$ and a finite set of points $\{x_1,\dots,x_M\}\subset X$.
Then for $k\gg 1$ (depending on $r$ and the configuration of points), the evaluation map to $r$-jets
\[
H^0(X,L^k)\ \longrightarrow\ \bigoplus_{i=1}^M J^r(L^k)_{x_i}
\]
is surjective. Equivalently, one can prescribe the $r$-jet of a section of $L^k$ at finitely many points, independently, for $k$ large.
\end{lemma}

\begin{remark}[References]
Lemma~\ref{lem:jet-sep} is standard in algebraic geometry: high tensor powers of an ample line bundle are $r$-jet ample / separate $r$-jets.
One can prove it via Castelnuovo--Mumford regularity and Serre vanishing, or via analytic peak-section constructions using H\"ormander $L^2$ estimates.
See e.g.\ Lazarsfeld, \emph{Positivity in Algebraic Geometry} (Vol.~I), and standard treatments of Bergman kernel asymptotics.
\end{remark}
\end{proof}
...
\begin{remark}[On using smooth convex cells instead of sharp cubes]\label{rem:smooth-cells}
The cubical decomposition is convenient for bookkeeping and for standard deformation theorems.
However, the “sliver’’ mechanism (Remark~\ref{rem:sliver} / Proposition~\ref{prop:sliver-local}) benefits from working in cells with \emph{smooth convex}
boundary, because then small slice volume forces small boundary slices in a quantitative way (Remark~\ref{rem:sliver-cell-shape}).
One can replace sharp cubes by “rounded cubes’’ (a small smoothing of corners/edges inside each chart) while preserving the same adjacency graph.
All flat-norm estimates are stable under such uniformly bilipschitz modifications, up to changing constants.
\end{remark}

\begin{lemma}[Normal-coordinate distortion on small balls]\label{lem:normal-distortion}
Let $(X,g)$ be a compact Riemannian manifold.  There exists $h_0>0$ and $C\ge 1$ such that for every $x\in X$ and every $0<h\le h_0$,
the exponential chart $\exp_x:B_h(0)\subset T_xX\to X$ is $C^2$ and $(1+Ch^2)$--bilipschitz onto its image.
In particular, any $k$--dimensional $C^1$ graph in $B_h(0)$ has $k$--mass and $(k-1)$--boundary mass distorted by a factor $1+O(h^2)$ under $\exp_x$,
with constants uniform in $x$.
\end{lemma}

\begin{proof}[Sketch]
In geodesic normal coordinates, one has the standard expansion $g_{ij}(y)=\delta_{ij}+O(|y|^2)$ and $\partial g_{ij}(y)=O(|y|)$ with constants
controlled by curvature bounds; compactness makes these bounds uniform in $x$.
Integrating the metric distortion along line segments in $B_h(0)$ yields the $(1+Ch^2)$ bilipschitz estimate for $\exp_x$ on $B_h(0)$.
Mass distortion for $C^1$ graphs follows from the area formula and the uniform Jacobian bounds.
\end{proof}

\begin{lemma}[Existence of a $C^2$ cell decomposition at scale $h$]\label{lem:rounded-cells}
Let $(X,g)$ be a compact Riemannian manifold of dimension $2n$.
For all sufficiently small $h>0$ there exists a finite partition of $X$ (up to a null set) into sets $\{Q\}$ with disjoint interiors such that:
\begin{enumerate}
\item[\textnormal{(i)}] (\textbf{Scale}) $\mathrm{diam}(Q)\asymp h$ uniformly in $Q$;
\item[\textnormal{(ii)}] (\textbf{Chart control}) each $Q$ is contained in a normal-coordinate chart of radius $\asymp h$ on which the metric is $(1+O(h^2))$--bilipschitz
to the Euclidean metric (as in Lemma~\ref{lem:normal-distortion});
\item[\textnormal{(iii)}] (\textbf{$C^2$ boundary with controlled curvature}) each $Q$ has $C^2$ boundary and its principal curvatures satisfy the \emph{absolute} bound
\[
\bigl|\kappa_i\bigr|\ \le\ \frac{C}{h}
\qquad\text{everywhere on }\partial Q,
\]
with a constant $C$ depending only on $(X,g)$.
\end{enumerate}
\end{lemma}

\begin{proof}[Sketch]
Fix $h\ll 1$ and choose a finite normal-coordinate atlas by geodesic balls of radius $\asymp h$.
Inside each chart, start from a standard cubical grid at mesh $\asymp h$ (a polyhedral cell decomposition).
One may smooth the resulting codimension-$1$ cell complex (the union of interfaces) to a $C^2$ hypersurface network at scale $\asymp h$,
producing a genuine partition whose interfaces are shared by adjacent cells and have principal curvatures bounded by $C/h$.
Pushing forward/back by the chart maps preserves these bounds up to uniform constants by Lemma~\ref{lem:normal-distortion}.
\end{proof}
...
\noindent
\textbf{Sliverpath (restructured) strategy.}
There are two conceptually different regimes for the ``microstructure'' part of Step~4:
\begin{enumerate}
\item[\textnormal{(I)}] (\textbf{Dense-sheet / constant-mass regime}) One uses families of calibrated sheets whose cell-restricted masses are
comparable to $h^{2(n-p)}$ and whose multiplicities are large ($N_Q\gg 1$).  In this regime the template route
(Lemma~\ref{lem:w1-auto} / Proposition~\ref{prop:template-flat-glue}) yields a clean bound
$\mathcal F(\partial T^{\mathrm{raw}})\lesssim (h+\varepsilon)\,m$.
\item[\textnormal{(II)}] (\textbf{Bergman-scale / sliver regime}) In middle codimension ($1<p<n-1$), the holomorphic $C^1$ control scale
from Bergman kernels is naturally $h\asymp m^{-1/2}$; then the constant-mass model predicts $N_Q\sim m\,h^{2p}\asymp m^{1-p}$,
which is $<1$ for $p>1$ (Remark~\ref{rem:param-tension} and the ``Bergman-scale amplification'' discussion).
To keep enough degrees of freedom per cell at this intrinsic $h$, one must split the target mass into many tiny calibrated
pieces (``slivers'') while controlling their induced face slices.
\end{enumerate}
In this file we treat \textnormal{(II)} as the main unconditional target and view \textnormal{(I)} as a simplified special case.
...
$\beta$ with $\beta(x)\in K_p(x)$ pointwise.  Let $\{Q\}$ be a partition of $X$ into
coordinate cells of diameter $h$ (e.g.\ cubes, or uniformly rounded/smooth convex cells as in Remark~\ref{rem:smooth-cells}).
Assume that on each cell $Q$ one can realize the required local tangent-plane data by calibrated holomorphic pieces with either:
\begin{enumerate}
\item[\textnormal{(I)}] the constant-mass sheet model of Substep~4.1 (many sheets per cell), or
\item[\textnormal{(II)}] the sliver microstructure mechanism of Proposition~\ref{prop:sliver-local} (many tiny pieces per cell).
\end{enumerate}
Then there exists an integer $m\ge 1$ (clearing denominators of
...
\smallskip\noindent
\textbf{Sliver variant (regime \textnormal{(II)}).}
At the intrinsic Bergman scale $h\asymp m^{-1/2}$ and in middle codimension $p>1$, the constant-mass model
$A_{Q,j}\asymp h^{2(n-p)}$ typically forces $N_{Q,j}=O(1)$ (or $0$) and cannot supply enough degrees of freedom
to satisfy both transport/gluing and cohomology constraints.
Instead, one replaces the constant-mass sheets in family $(Q,j)$ by \emph{many} calibrated \emph{sliver pieces} whose individual
masses are much smaller, so that the total mass in family $j$ still matches $\theta_{Q,j}\,M_Q$ while the number of pieces is large.
The flat ball model shows this is compatible with transverse $W_1$ approximation (Proposition~\ref{prop:flat-sliver-local}),
and the holomorphic upgrade can realize finite translation templates with uniform $C^1$ control on Bergman-scale cells
(Corollary~\ref{cor:holomorphic-flat-sliver-local} / Proposition~\ref{prop:finite-template}).
The remaining global gluing bookkeeping in the sliver regime is packaged as the weighted reduction
Proposition~\ref{prop:weighted-sliver-glue}.
...
\begin{lemma}[Flat-norm boundary control suffices for gluing]\label{lem:flat-glue-criterion}
...
Assume $\mathcal F(S)\le \eta$ for some $\eta>0$.
Then there exists an integral $k$--current $R_{\mathrm{glue}}$ with $\partial R_{\mathrm{glue}}=-S$ and
\[
\Mass(R_{\mathrm{glue}})\ \le\ C(X,k)\,\Bigl(\eta+\eta^{\frac{k}{k-1}}\Bigr).
\]
In particular, $T^{\mathrm{raw}}+R_{\mathrm{glue}}$ is a closed integral $k$--cycle and
\[
\Mass(T^{\mathrm{raw}}+R_{\mathrm{glue}})\ \le\ \Mass(T^{\mathrm{raw}})+C(X,k)\,\Bigl(\eta+\eta^{\frac{k}{k-1}}\Bigr).
\]
\end{lemma}

\begin{proof}[Sketch]
Write $S=R+\partial Q$ with $\Mass(R)+\Mass(Q)\le 2\mathcal F(S)\le 2\eta$.
Since $S$ is a boundary, $R=S-\partial Q$ is also a boundary and hence null-homologous.
By the Federer--Fleming isoperimetric inequality there exists an integral $k$--current $Q_R$ with $\partial Q_R=R$ and
$\Mass(Q_R)\le C\,\Mass(R)^{k/(k-1)}\le C\,(2\eta)^{k/(k-1)}$.
Setting $R_{\mathrm{glue}}:=-(Q+Q_R)$ gives $\partial R_{\mathrm{glue}}=-S$ and the stated mass bound.
\end{proof}
...
\begin{definition}[Toy marginal realization problem]\label{def:toy-marginals}
Fix integers $d\ge 1$ and $M\ge 1$ and write $\Omega:=[M]^d$ for a $d$--dimensional grid.
For each coordinate $\ell\in\{1,\dots,d\}$ let $m_\ell\in\Z_{\ge 0}^{[M]}$ be an integer histogram with common total mass
$\sum_{a=1}^M m_\ell(a)=N$ (independent of $\ell$).
The \emph{marginal realization problem} asks for a multiset of grid points $\{x_1,\dots,x_N\}\subset \Omega$ such that for each $\ell$ and $a\in[M]$,
the number of points with $(x_i)_\ell=a$ equals $m_\ell(a)$.
\end{definition}

\begin{lemma}[Existence of a multiset with prescribed 1D marginals]\label{lem:toy-marginals}
The marginal realization problem in Definition~\ref{def:toy-marginals} always has a solution.
\end{lemma}

\begin{proof}[Sketch]
For $d=2$ this is the existence of a nonnegative integer matrix with prescribed row and column sums,
equivalently a bipartite multigraph with given degrees; it can be constructed greedily or via max-flow.
For $d>2$, realize the first $d-1$ marginals by induction to obtain a multiset in $[M]^{d-1}$, then append the $d$th coordinate by a bipartite matching
between the realized $(d-1)$--tuples and the $d$th-coordinate values with multiplicities $m_d$.
\end{proof}

\begin{remark}[Interpretation for the microstructure/gluing problem]\label{rem:toy-marginals-meaning}
Lemma~\ref{lem:toy-marginals} records that, in an idealized setting where different faces ``see'' disjoint coordinate projections of a translation parameter,
the simultaneous matching constraints are \emph{purely combinatorial} and always solvable.
In the calibrated-sheet setting, however, each face measure is obtained from the same translation multiset by a \emph{nontrivial linear face map}
depending on the plane direction and the local face chart (Remark~\ref{rem:transport-hypotheses}).
This turns the global matching requirement into a discrete tomography / simultaneous pushforward problem for a family of linear maps,
which is the conceptual core of Blocker~\textnormal{B1}.
\end{remark}

\begin{lemma}[Rounding a Lipschitz density varies by $O(h)$ across neighbors]\label{lem:lipschitz-rounding-counts}
Let $\{Q\}$ be a mesh-$h$ cell decomposition and choose a representative point $x_Q\in Q$ for each cell.
Let $f:X\to\R$ be $L$--Lipschitz and bounded.
Fix $m\ge 1$ and define
\[
N_Q\ :=\ \Bigl\lfloor m\,h^{2p}\,f(x_Q)\Bigr\rceil\ \in\ \Z,
\]
where $\lfloor\cdot\rceil$ denotes rounding to the nearest integer.
Then for any adjacent cells $Q\sim Q'$ (sharing an interface) one has
\[
|N_Q-N_{Q'}|\ \le\ C\,L\,m\,h^{2p+1}+2,
\]
with $C$ depending only on the uniform geometry of the mesh.
In particular, if $f\ge f_0>0$ and $m h^{2p}\to\infty$, then
\(
|N_Q-N_{Q'}|/N_Q = O(h)
\)
uniformly over adjacent pairs.
\end{lemma}

\begin{proof}[Sketch]
Since $d(x_Q,x_{Q'})\lesssim h$ for adjacent cells and $f$ is $L$--Lipschitz,
\[
|m h^{2p} f(x_Q)-m h^{2p} f(x_{Q'})|\ \le\ L\,m\,h^{2p}\,d(x_Q,x_{Q'})\ \lesssim\ L\,m\,h^{2p+1}.
\]
Rounding to the nearest integer changes each term by at most $1$, giving the stated bound.
\end{proof}

\begin{remark}[Bounded global corrections do not spoil the $O(h)$ edit regime]\label{rem:bounded-corrections}
In applications, one often needs to adjust the rounded counts $N_Q$ by a bounded amount (e.g.\ to enforce finitely many global period constraints).
Lemma~\ref{lem:lipschitz-rounding-counts} shows that the \emph{neighbor variation} is $O(h)$ in a \emph{relative} sense once $N_Q\to\infty$.
In particular, if $N_Q\gtrsim h^{-1}$ uniformly and $\widetilde N_Q:=N_Q+\Delta_Q$ with $|\Delta_Q|\le C_0$,
then $\widetilde N_Q/N_Q=1+O(h)$ and hence the induced insertions/deletions are an $O(h)$ fraction:
\[
\frac{|\widetilde N_Q-N_Q|}{\widetilde N_Q}\ \le\ \frac{C_0}{\widetilde N_Q}\ \lesssim\ C_0\,h.
\]
Thus any such bounded correction is harmless for the nested-template scheme (Remark~\ref{rem:nested-template-scheme}) and
can be absorbed by Lemma~\ref{lem:template-edits-oh} provided $h$ is sufficiently small.
\end{remark}

\begin{remark}[A nested-template scheme reduces the edit part of Blocker \textnormal{B1}]\label{rem:nested-template-scheme}
Fix, for each local direction family, an \emph{ordered} template of transverse atoms $(y_a)_{a\ge 1}\subset B_{C_0h}(0)\subset\R^{2p}$.
For example, Lemma~\ref{lem:sphere-quantize-nested} produces such a nested sequence on a sphere (uniform density), and scaling embeds it into
$B_{C_0h}(0)$.
If the two neighboring cells choose their transverse discrete measures by taking the \emph{first} $N_Q$ and $N_{Q'}$ atoms
(with comparable slice weights per atom), then the unmatched part across the interface is exactly a \emph{prefix edit} of size $|N_Q-N_{Q'}|$.
When $N_Q$ is obtained by rounding a smooth target density, Lemma~\ref{lem:lipschitz-rounding-counts} gives that the edit fraction is $O(h)$.
Lemma~\ref{lem:template-edits-oh} then shows that this unmatched part is automatically $O(h^2)$ in flat norm and can be absorbed into the matched
displacement bookkeeping.

Thus, the remaining conceptual difficulty in Blocker~\textnormal{B1} is not that counts must match \emph{exactly}, but arranging a globally coherent choice
of ordered templates (and direction pairings) so that across every face there is a large matched subtemplate and only an $O(h)$ fraction of edits,
while simultaneously meeting the local mass/barycenter constraints.
\end{remark}

\begin{proposition}[Prefix templates $\Rightarrow$ interface coherence up to $O(h)$ edits]\label{prop:prefix-template-coherence}
Work in the setting of Proposition~\ref{prop:weighted-sliver-glue} on a mesh-$h$ cell decomposition and fix an interior interface $F=Q\cap Q'$.
Assume that for some paired direction label $j$ the two sides admit tubular-face parameterizations with linear face maps
$\Phi_{Q,F}$ and $\Phi_{Q',F}$ as in Lemma~\ref{lem:template-displacement-edits}.
Fix an ordered template of transverse atoms $(y_a)_{a\ge 1}\subset B_{C_0h}(0)\subset\R^{2p}$ and define, for integers $N\ge 0$,
\[
\nu^{(N)}\ :=\ \sum_{a=1}^{N}\delta_{y_a}.
\]
Suppose the two cells choose their face measures from prefixes:
\[
\mu_{Q\to F}=(\Phi_{Q,F})_\#\nu^{(N_Q)},\qquad
\mu_{Q'\to F}=(\Phi_{Q',F})_\#\nu^{(N_{Q'})},
\]
for some integers $N_Q,N_{Q'}\ge 0$.
Assume the \emph{slow-variation} bound
\[
|N_Q-N_{Q'}|\ \le\ \theta_F\,\min\{N_Q,N_{Q'}\}
\qquad\text{with}\qquad \theta_F\ \lesssim\ h.
\]
Then writing $B_F$ for the mismatch current on $F$ coming from these paired families,
one has the per-face flat-norm estimate
\[
\mathcal F(B_F)\ \le\ C\,h^2\Bigl(\Mass(\partial S_Q\llcorner F)+\Mass(\partial S_{Q'}\llcorner F)\Bigr)\ +\ O(\varepsilon\,M_F),
\]
with $C$ depending only on $(n,p,X)$ and the uniform tubular-face charts (and $M_F\lesssim M_Q+M_{Q'}$ as in Lemma~\ref{lem:template-displacement}).
\end{proposition}
...
\nu^{(N_{Q'})}=\nu^{(N_{\min})}+\nu^{-},
\]
where $\nu^{\pm}$ are the (possibly empty) unmatched tails of total mass $|N_Q-N_{Q'}|$.
Then Lemma~\ref{lem:template-displacement-edits} applies with matched part $\nu^{(N_{\min})}$ and unmatched part $\nu^{\pm}$.
The matched part contributes the $C h^2(\cdots)+O(\varepsilon M_F)$ term.

For the unmatched part, the hypothesis implies the edit fraction is $\theta_F\lesssim h$.
Thus Lemma~\ref{lem:template-edits-oh} shows the unmatched contribution is also $O(h^2)$ times the total boundary mass on the face.
Combining the two bounds yields the stated estimate.
\end{proof}

\begin{corollary}[Global prefix-template coherence from Lipschitz targets]\label{cor:global-prefix-template-coherence}
In the setting of Proposition~\ref{prop:prefix-template-coherence}, suppose the prefix lengths $N_Q$ arise by rounding a Lipschitz target as in
Lemma~\ref{lem:lipschitz-rounding-counts}, with $f\ge f_0>0$ and $m h^{2p}\to\infty$ so that $|N_Q-N_{Q'}|/N_Q=O(h)$ uniformly over adjacent cells.
Then the slow-variation hypothesis holds on every interior face with $\theta_F\lesssim h$, so Proposition~\ref{prop:prefix-template-coherence} applies
uniformly over all interfaces.

Moreover, if one subsequently adjusts the counts by bounded amounts (e.g.\ to enforce finitely many global cohomology constraints as in
Proposition~\ref{prop:cohomology-match}), the $O(h)$ edit-fraction conclusion persists provided $N_Q\gtrsim h^{-1}$; see
Remark~\ref{rem:bounded-corrections} and Remark~\ref{rem:cohomology-vs-prefix}.
\end{corollary}
...
\begin{lemma}[Why $\|\Phi_{Q,F}-\Phi_{Q',F}\|_{\mathrm{op}}=O(h)$ is geometric]\label{lem:face-map-variation}
Fix a mesh-$h$ cell decomposition and an interior interface $F=Q\cap Q'$.
Assume $h$ is small enough that $Q\cup Q'$ lies in a single holomorphic coordinate chart and that $F$ admits a tubular product chart of radius $\asymp h$
(both are automatic on compact $X$ for $h\ll1$).
Fix a paired direction index $j$ so that the calibrated reference planes used in Substep~4.2 satisfy
\[
\angle(\Pi_{Q,j},\Pi_{Q',j})\ \le\ C\,h,
\]
as ensured by the local stabilization in Lemma~\ref{lem:caratheodory-general} and smoothness of $\beta$.
Then one may choose the face-slice linear maps $\Phi_{Q,F}$ and $\Phi_{Q',F}$ in Proposition~\ref{prop:template-flat-glue}\textnormal{(T1)} so that
\[
\|\Phi_{Q,F}-\Phi_{Q',F}\|_{\mathrm{op}}\ \le\ C'\,h,
\]
with $C'$ depending only on $(X,\omega)$ and $(n,p)$ (and not on $Q,Q',F$ once $h$ is sufficiently small).
\end{lemma}

\begin{proof}[Sketch]
In the flat model, for a fixed face hyperplane $F$ and a fixed plane direction $\Pi$, the dependence of the induced face parameter on the translation
vector $t\in\Pi^\perp$ is linear (it is obtained by restricting an affine translate and then projecting to the fixed transverse parameter space on $F$).
Thus the corresponding map $\Phi_{\Pi,F}:\Pi^\perp\to\R^{2p}$ is represented by a matrix whose entries are smooth functions of the angle data
between $\Pi$ and $TF$ (equivalently, smooth functions on the Grassmannian away from degeneracy).
On compact subsets of the calibrated Grassmannian and for uniformly transverse faces, this dependence is Lipschitz:
$\|\Phi_{\Pi,F}-\Phi_{\Pi',F}\|_{\mathrm{op}}\le C\,\angle(\Pi,\Pi')$.

In the curved setting, working in a fixed holomorphic chart and a fixed tubular-face chart of radius $\asymp h$, the metric and the identification of
normal/transverse directions are $C^1$-close to the flat model with $O(h)$ error.
Combining the $O(h)$ variation of $\Pi_{Q,j}$ across adjacent cells with the $C^1$ control of the coordinate/tubular charts yields the stated
$O(h)$ bound for $\|\Phi_{Q,F}-\Phi_{Q',F}\|_{\mathrm{op}}$.
\end{proof}
...
\begin{proposition}[Template coherence $\Rightarrow$ quantitative flat-norm gluing]\label{prop:template-flat-glue}
Fix a cubulation of $X$ by coordinate cubes of side length $h$ and write $T^{\mathrm{raw}}=\sum_Q S_Q$ as in Substep~4.2.
Assume the small-angle and transverse-parameter hypotheses \textnormal{(a)}--\textnormal{(b)} of
Proposition~\ref{prop:transport-flat-glue} hold with the same $\varepsilon$.
Assume moreover the following \emph{template coherence} holds on each interior face $F=Q\cap Q'$:
\begin{enumerate}
\item[\textnormal{(T1)}] (\textbf{Same template up to $O(h)$ face-map variation}) For each face $F$ there are linear face-slice maps
$\Phi_{Q,F},\Phi_{Q',F}:\R^{2p}\to\R^{2p}$ with $\|\Phi_{Q,F}\|_{\mathrm{op}}+\|\Phi_{Q',F}\|_{\mathrm{op}}\le C_{\Phi,0}$ and
$\|\Phi_{Q,F}-\Phi_{Q',F}\|_{\mathrm{op}}\le C_{\Phi}\,h$ such that
$\mu_{Q\to F}=(\Phi_{Q,F})_\#\nu_{Q,F}$ and $\mu_{Q'\to F}=(\Phi_{Q',F})_\#\nu_{Q',F}$ for some integer-weighted measures
$\nu_{Q,F},\nu_{Q',F}$ supported in a ball of radius $C_0h$.
\item[\textnormal{(T2)}] (\textbf{Slowly varying multiplicities}) The measures $\nu_{Q,F}$ and $\nu_{Q',F}$ differ by at most $r_F$ insertions/deletions
and $r_F\le C_{\mathrm{sv}}\,h\,N_F$, where $N_F:=\mu_{Q\to F}(\R^{2p})=\mu_{Q'\to F}(\R^{2p})$.
\end{enumerate}
Then for each interior face $F$ one has the quantitative face matching bound
\[
W_1(\mu_{Q\to F},\mu_{Q'\to F})\ \le\ C_1\,h^2\,N_F,
\]
and consequently there exists a constant $C_2=C_2(n,p,X)$ such that
\[
\mathcal F\!\left(\partial T^{\mathrm{raw}}\right)\ \le\ C_2\,\bigl(h+\varepsilon\bigr)\,\Mass(T^{\mathrm{raw}}).
\]
In particular, if $\Mass(T^{\mathrm{raw}})\asymp m$ (as in Substep~4.1), then
\[
\mathcal F\!\left(\partial T^{\mathrm{raw}}\right)\ \le\ C_3\,\bigl(h+\varepsilon\bigr)\,m,
\]
so one may take $\varepsilon_{\mathrm{glue}}(m,\varepsilon,h)=C_3(h+\varepsilon)$ in the flat-norm estimate sought in Remark~\ref{rem:glue-gap}.
\end{proposition}

\begin{proof}
Fix an interior face $F=Q\cap Q'$.
By Lemma~\ref{lem:w1-template-edit} and the insertion/deletion hypothesis \textnormal{(T2)},
there exist a coupling of $\nu_{Q,F}$ and $\nu_{Q',F}$ with transport cost $\le C\,r_F\,h\le C\,C_{\mathrm{sv}}\,h^2N_F$.
Applying the face-map stability Lemma~\ref{lem:w1-auto} to $\nu_{Q,F}$ and the maps $\Phi_{Q,F},\Phi_{Q',F}$ from \textnormal{(T1)} gives
\[
W_1\bigl((\Phi_{Q,F})_\#\nu_{Q,F},(\Phi_{Q',F})_\#\nu_{Q,F}\bigr)\ \le\ C\,h^2\,\nu_{Q,F}(\R^{2p})\ =\ C\,h^2\,N_F.
\]
Moreover, $\Phi_{Q',F}$ is $C_{\Phi,0}$-Lipschitz by \textnormal{(T1)}, hence
\[
W_1\bigl((\Phi_{Q',F})_\#\nu_{Q,F},(\Phi_{Q',F})_\#\nu_{Q',F}\bigr)
\le C_{\Phi,0}\,W_1(\nu_{Q,F},\nu_{Q',F})
\le C_{\Phi,0}\,C\,r_F\,h
\le C\,h^2\,N_F.
\]
Triangle inequality for $W_1$ therefore yields
\[
W_1(\mu_{Q\to F},\mu_{Q'\to F})
\le W_1\bigl((\Phi_{Q,F})_\#\nu_{Q,F},(\Phi_{Q',F})_\#\nu_{Q,F}\bigr)
 + W_1\bigl((\Phi_{Q',F})_\#\nu_{Q,F},(\Phi_{Q',F})_\#\nu_{Q',F}\bigr)
\le C_1\,h^2\,N_F,
\]
after absorbing the insertion/deletion term into the same $h^2N_F$ scale.

Now insert $\tau_F:=C_1h^2N_F$ into Proposition~\ref{prop:transport-flat-glue} and sum over interior faces:
\[
\mathcal F(\partial T^{\mathrm{raw}})
\le C\,h^{2n-2p-1}\sum_F \tau_F \;+\; C\,\varepsilon\,h^{2n-2p}\sum_F N_F.
\]
Since $\tau_F=C_1h^2N_F$, the first term is $C\,h^{2n-2p+1}\sum_F N_F$.
Each sheet piece contributing to $T^{\mathrm{raw}}$ meets at most $O(1)$ faces, hence $\sum_F N_F\le C'\,(\text{number of sheet pieces})$.
Under the local model in Substep~4.1 (constant-mass sheet pieces), each sheet piece has $\psi$--mass $\asymp h^{2(n-p)}$, so
$(\text{number of sheet pieces})\cdot h^{2(n-p)}\lesssim \Mass(T^{\mathrm{raw}})$ and therefore
\[
\sum_F N_F\ \le\ C''\,\Mass(T^{\mathrm{raw}})\,h^{-2(n-p)}.
\]
Plugging this into the two global terms gives
\[
h^{2n-2p+1}\sum_F N_F\ \le\ C''\,h^{2n-2p+1}\,\Mass(T^{\mathrm{raw}})\,h^{-2n+2p}
= C''\,h\,\Mass(T^{\mathrm{raw}}),
\]
and similarly
\[
\varepsilon\,h^{2n-2p}\sum_F N_F\ \le\ C''\,\varepsilon\,h^{2n-2p}\,\Mass(T^{\mathrm{raw}})\,h^{-2n+2p}
= C''\,\varepsilon\,\Mass(T^{\mathrm{raw}}).
\]
Absorbing constants yields $\mathcal F(\partial T^{\mathrm{raw}})\le C_2(h+\varepsilon)\Mass(T^{\mathrm{raw}})$, as claimed.
\end{proof}

\begin{remark}[Sliver regime: what changes in the global counting estimate]\label{rem:sliver-vs-template}
Proposition~\ref{prop:template-flat-glue} isolates the \emph{face-by-face transport} input needed for gluing in flat norm.
The final global bound in the proof used an additional bookkeeping step:
\[
\sum_F N_F\ \lesssim\ \Mass(T^{\mathrm{raw}})\,h^{-2(n-p)},
\]
which is valid in the \emph{constant-mass-per-sheet} model where each sheet piece contributing to $T^{\mathrm{raw}}$ has
$\psi$--mass $\asymp h^{2(n-p)}$ inside a cube.

\smallskip\noindent
In the \emph{sliver} regime (Remark~\ref{rem:sliver} / Proposition~\ref{prop:flat-sliver-local}), one deliberately allows many pieces of
very small mass per cube.  Then $\sum_F N_F$ can be arbitrarily large at fixed total mass, so the above reduction is no longer available.
To extend Proposition~\ref{prop:template-flat-glue} to this regime, one needs to replace the crude counting bound by a \emph{weighted} estimate
that tracks the actual size of each face slice.

\smallskip\noindent
\textbf{Model heuristic.} In a smooth convex cell (e.g.\ a ball), a $k$-dimensional affine slice of volume $v$ has boundary size on the cell boundary
of order $v^{(k-1)/k}$ (cf.\ the ball scaling in the flat model).  Thus splitting a fixed total mass $M$ into $N$ equal slivers increases total boundary
size only like $N^{1/k}$ (sublinear).  This suggests that a sharpened version of the transport-flat-norm bound should control $\mathcal F(\partial T^{\mathrm{raw}})$
in terms of a boundary-size functional (or first moment) rather than the raw count $\sum_F N_F$.

\smallskip\noindent
\textbf{Concrete target.} Replace $\sum_F N_F$ in the global estimate by a quantity of the form
\[
\sum_F \sum_{a\in\mathcal S(F)} \Mass\!\big(\partial([Y^a]\llcorner Q)\llcorner F\big),
\]
or an equivalent transverse-parameter integral, and show it is controlled (sublinearly in the number of slivers) by the total mass in $Q$ for the specific
flat/graph models used here.
In the flat ball model this bookkeeping is made explicit in Proposition~\ref{prop:sliver-boundary-budget}, and its global implication for gluing is summarized in
Proposition~\ref{prop:weighted-sliver-glue}.
For rounded cubes / smooth uniformly convex cells with curvature pinched at scale $h$ (Remark~\ref{rem:smooth-cells}),
Lemma~\ref{lem:uniformly-convex-slice-boundary} provides the same boundary-shrinkage input
$\Mass(\partial([P+t]\llcorner Q))\lesssim \Mass([P+t]\llcorner Q)^{(k-1)/k}$ up to uniform constants.
Thus the remaining analytic input in regime \textnormal{(II)} is primarily the \emph{template displacement} estimate for holomorphic slivers
(Lemma~\ref{lem:template-displacement}) together with ensuring the chosen rounded cells satisfy the curvature hypotheses.
\end{remark}

\begin{lemma}[Ball-section boundary scaling in the flat model]\label{lem:ball-section-scaling}
Let $k\ge 2$ and let $Q:=B_h(0)\subset\R^{k}\times\R^{d-k}$ be the Euclidean ball of radius $h$.
Let $P:=\R^{k}\times\{0\}$ and let $t\in P^\perp$ with $r:=\|t\|\in(0,h)$.
Then the section $(P+t)\cap Q$ is a $k$--ball of radius $\sqrt{h^2-r^2}$, hence
\[
\Mass\bigl([P+t]\llcorner Q\bigr)\ =\ c_k\,(h^2-r^2)^{k/2},
\qquad
\Mass\bigl(\partial([P+t]\llcorner Q)\bigr)\ =\ c_{k-1}\,(h^2-r^2)^{(k-1)/2},
\]
for dimensional constants $c_k,c_{k-1}>0$.
In particular,
\[
\Mass\bigl(\partial([P+t]\llcorner Q)\bigr)\ \asymp\ \Mass\bigl([P+t]\llcorner Q\bigr)^{\frac{k-1}{k}}
\]
uniformly in $r\in(0,h)$.
\end{lemma}

\begin{proof}[Sketch]
The intersection of a Euclidean ball with an affine $k$--plane at distance $r$ from the center is a Euclidean $k$--ball of radius $\sqrt{h^2-r^2}$.
The formulas follow from the standard volume/area of balls and spheres.
\end{proof}
...
The curvature pinching $\kappa_i\asymp h^{-1}$ implies interior/exterior tangent balls of radius $\asymp h$ (the classical \emph{rolling ball} principle).
Uniqueness of supporting points for a given outward normal follows from strict convexity and $C^2$ regularity of $\partial Q$.
See, e.g., Schneider, \emph{Convex Bodies: The Brunn--Minkowski Theory}.
\end{remark}

\begin{remark}[Caution: uniformly convex cells are a \emph{model}, not a literal partition]\label{rem:uniformly-convex-partition-caution}
Lemma~\ref{lem:uniformly-convex-slice-boundary} and Proposition~\ref{prop:sliver-boundary-budget} are stated for a single smooth uniformly convex domain $Q$,
because that hypothesis forces the clean implication ``small slice mass $\Rightarrow$ small boundary slice''.
In an actual cell decomposition (a partition of $X$ into sets with \emph{shared} codimension-$1$ interfaces), one should not read this as asserting that
\emph{every} cell can be uniformly convex with respect to its outward normal:
if a $C^2$ hypersurface is shared by two adjacent cells, reversing the normal reverses the principal curvatures, so strict convexity on both sides would force
those curvatures to vanish (flat interface).

Accordingly, the sliver route should be interpreted as requiring an explicit \emph{boundary-budget mechanism} (Blocker~\textnormal{B1c}) ensuring
that the particular sliver pieces used in the construction have small boundary slices on the chosen interfaces, even though the global partition is not
uniformly convex in the classical sense.
\end{remark}

\begin{proposition}[Sliver boundary budget on a smooth uniformly convex cell (flat model)]\label{prop:sliver-boundary-budget}
Let $Q\subset\R^{2n}$ be a bounded $C^2$ uniformly convex domain of diameter $\asymp h$ with principal curvatures pinched at scale $h$
(so Lemma~\ref{lem:uniformly-convex-slice-boundary} applies) and let $k:=2n-2p$.
Suppose we write a target mass $M>0$ as a sum of $N$ \emph{equal-mass} affine calibrated pieces
\[
T:=\sum_{a=1}^N [P+t_a]\llcorner Q,
\qquad
\Mass([P+t_a]\llcorner Q)=M/N,
\]
where each $P+t_a$ is an affine calibrated $k$--plane.
Then
\[
\Mass(\partial T)\ \le\ C(n,p,c,C)\,M^{\frac{k-1}{k}}\,N^{\frac1k}.
\]
Moreover, if $Y^a$ are $C^1$ graphs over $P+t_a$ on $Q$ with slope $\le\varepsilon$, then
\[
\Mass\bigl(\partial([Y^a]\llcorner Q)\bigr)
=\bigl(1+O(\varepsilon^2)\bigr)\,\Mass\bigl(\partial([P+t_a]\llcorner Q)\bigr),
\]
so the same bound holds up to a $(1+O(\varepsilon^2))$ factor.
\end{proposition}

\begin{proof}
For each $a$, the boundary current $\partial([P+t_a]\llcorner Q)$ is supported on $\partial Q$ and
\[
\Mass\bigl(\partial([P+t_a]\llcorner Q)\bigr)=\mathcal H^{k-1}\bigl((P+t_a)\cap\partial Q\bigr).
\]
Since $\Mass([P+t_a]\llcorner Q)=\mathcal H^{k}\bigl((P+t_a)\cap Q\bigr)=M/N$, Lemma~\ref{lem:uniformly-convex-slice-boundary} (with $d=2n$) gives
\[
\Mass\bigl(\partial([P+t_a]\llcorner Q)\bigr)\ \le\ C_*\,(M/N)^{\frac{k-1}{k}}.
\]
Summing over $a$ and using $\Mass(\partial T)\le \sum_a \Mass(\partial([P+t_a]\llcorner Q))$ yields
\[
\Mass(\partial T)\ \le\ N\cdot C_*\,(M/N)^{\frac{k-1}{k}}
= C_*\,M^{\frac{k-1}{k}}\,N^{\frac1k}.
\]
(In the ball case $Q=B_h$, one may recover the same scaling with an explicit sharp constant from Lemma~\ref{lem:ball-section-scaling}.)

The $C^1$ graph perturbation statement follows by the area formula and uniform bilipschitz control of the graph map on $Q$ (as in Lemma~\ref{lem:sliver-stability}).
\end{proof}

\begin{hypothesis}[Boundary-budget input for Bergman-scale slivers]\label{hyp:BB}
Fix a mesh-$h$ cell decomposition and set $k:=2n-2p$.
For each cell $Q$ and each $m$ in the sliver regime, suppose the local holomorphic construction produces a decomposition
\[
S_Q\ =\ \sum_{a=1}^{N_Q} [Y^a]\llcorner Q,
\]
where each $Y^a$ is $\psi$--calibrated and, on $Q$, is a $C^1$ graph with slope $\le\varepsilon$ over an affine calibrated $k$--plane slice.
Assume moreover that the pieces are \emph{equal-mass} in $Q$,
\(
\Mass([Y^a]\llcorner Q)=M_Q/N_Q
\)
with $M_Q\sim m\int_Q\beta\wedge\psi$, and that their total boundary satisfies the bound
\[
\Mass(\partial S_Q)\ \le\ C\,M_Q^{\frac{k-1}{k}}\,N_Q^{\frac1k},
\]
with $C$ depending only on $(n,p,X)$ (and absorbing the harmless $(1+O(\varepsilon^2))$ graph factor).

In a smooth uniformly convex \emph{model} cell, this hypothesis follows from Lemma~\ref{lem:uniformly-convex-slice-boundary} and
Proposition~\ref{prop:sliver-boundary-budget}; for a general $C^2$ partition it is the remaining geometric input in Blocker~\textnormal{B1c}.
\end{hypothesis}

\begin{remark}[Why Hypothesis~\ref{hyp:BB} is genuinely geometric (a cylinder counterexample)]\label{rem:bb-not-automatic}
Hypothesis~\ref{hyp:BB} is \emph{not} a formal consequence of smoothness and bounded curvature of the cell boundary.
Even for smooth \emph{convex} domains of diameter $\asymp h$, boundary shrinkage can fail if the boundary has flat directions.

For example, in $\R^{k}$ (with $k\ge 2$) consider a rounded cylinder
\(
Q:=B^{k-1}(h)\times[-h,h]
\)
(smoothly rounded near the rims so that $|\kappa|\lesssim 1/h$).
Let $P\subset\R^k$ be a $k$--plane containing the cylinder axis and translate it so that
$(P+t)\cap Q$ is a thin product region of the form $B^{k-1}(r)\times[-h,h]$ with $r\downarrow 0$.
Then
\[
v:=\mathcal H^{k}\bigl((P+t)\cap Q\bigr)\ \asymp\ r^{k-1}\,h,
\qquad
a:=\mathcal H^{k-1}\bigl((P+t)\cap\partial Q\bigr)\ \gtrsim\ r^{k-2}\,h,
\]
so $a/v^{(k-1)/k}\to\infty$ as $r\downarrow 0$.
Geometrically, one can have arbitrarily small slice volume while retaining a long boundary component inherited from the flat cylinder direction.

Thus any proof of Hypothesis~\ref{hyp:BB} must exploit a \emph{shape mechanism} (e.g.\ uniform convexity in the relevant slice directions, or an explicit
construction forcing sliver pieces to exit through rounded regions), not merely $C^2$ regularity.
\end{remark}

\begin{remark}[A plausible cube-based substitute: vertex slivers]\label{rem:bb-vertex-slivers}
The failure mode in Remark~\ref{rem:bb-not-automatic} comes from slices that are thin in \emph{one} direction but long in flat directions.
In polyhedral cells one can instead force small pieces to ``live near corners'', where the cell provides curvature in many directions \emph{combinatorially}
(many supporting halfspaces meet).
The next lemma records a clean flat-model estimate in this direction.

\begin{lemma}[Boundary shrinkage for transverse plane slices near a cube vertex (flat model)]\label{lem:cube-vertex-slice-boundary}
Let $Q:=[0,h]^d\subset\R^d$ and fix integers $1\le k<d$.
Let $P\subset\R^d$ be an oriented affine $k$--plane.
Assume $P$ is \emph{uniformly transverse} to the coordinate hyperplanes through the vertex $0\in Q$ in the following sense:
there exist $k$ coordinate indices $i_1,\dots,i_k$ such that the restrictions
$x\mapsto x_{i_r}$ form a basis of $P^*$ with condition number $\le \Lambda$.
Then there exists $c=c(d,k,\Lambda)>0$ and $C=C(d,k,\Lambda)>0$ such that for every translate $P+t$ with
$(P+t)\cap Q\subset B_{c h}(0)$ (so the slice lies in a small neighborhood of the vertex),
writing
\[
v(t):=\mathcal H^k\bigl((P+t)\cap Q\bigr),
\qquad
a(t):=\mathcal H^{k-1}\bigl((P+t)\cap \partial Q\bigr),
\]
one has the boundary--volume estimate
\[
a(t)\ \le\ C\,\bigl(v(t)\bigr)^{\frac{k-1}{k}}.
\]
\end{lemma}

\begin{proof}[Sketch]
After translating/rotating inside $P$, write the slice as a polytope in $P\cong\R^k$ cut out by the $d$ halfspaces defining $Q$.
On the event $(P+t)\cap Q\subset B_{c h}(0)$ only the halfspaces through the vertex $0$ are active at leading order, so the slice is comparable
(via a linear isomorphism with distortion controlled by $\Lambda$) to a truncated orthant in $\R^k$.
In such a simplicial-cone model, the slice polytopes are homothetic as the translate moves toward/away from the vertex, so
$v\asymp s^k$ and $a\asymp s^{k-1}$ for a single scale parameter $s$.
Eliminating $s$ gives $a\lesssim v^{(k-1)/k}$.
\end{proof}

\noindent
If one can implement the Bergman-scale sliver pieces so that, in each cell, their plane-like models concentrate near such uniformly transverse
vertices (rather than near flat face regions), Lemma~\ref{lem:cube-vertex-slice-boundary} suggests that the boundary budget
Hypothesis~\ref{hyp:BB} could be proved even for cubical partitions.
\end{remark}

\begin{remark}[Heuristic route to proving Hypothesis~\ref{hyp:BB}]\label{rem:bb-strategy}
Hypothesis~\ref{hyp:BB} is a \emph{boundary-budget} statement: it asserts that one can choose the sliver pieces so that ``small mass'' forces
``small boundary slices'' in a quantitative way \emph{on the actual interfaces used for gluing}.
The uniformly convex model estimate (Proposition~\ref{prop:sliver-boundary-budget}) shows the mechanism in the cleanest setting.
For a general $C^2$ cell decomposition, the corresponding statement is not automatic and should be viewed as the remaining geometric content of the
microstructure theorem.

\smallskip
\noindent
One plausible strategy is to concentrate the Bergman-scale mass $M_Q$ of each cell into regions of the interface network that are locally
``ball-like'' at scale $h$ (for instance, near rounded neighborhoods of the codimension-$\ge2$ skeleton where several interfaces meet),
so that the relevant sliver intersections are controlled by a local comparison to the ball model (Lemma~\ref{lem:ball-section-scaling}).
Since $M_Q\lesssim m\,h^{2n}$ at Bergman scale, such localization would not change the pairing against smooth test forms by more than $O(h)M_Q$,
yet could enforce the desired boundary shrinkage for the chosen pieces.
Making this precise amounts to constructing an interface network with controlled curvature and a supply of local ``rounded pockets'' shared across adjacent
cells, together with a template-based selection of translations supported in those pockets.
\end{remark}

\begin{proposition}[Weighted sliver gluing: a reduction statement]\label{prop:weighted-sliver-glue}
Assume the cell decomposition is at scale $h$ and admits uniform tubular-face charts with $(1+O(h^2))$ metric distortion
(Lemma~\ref{lem:normal-distortion}).
Assume moreover that the sliver boundary-budget hypothesis holds on each cell (Hypothesis~\ref{hyp:BB}),
and assume each cell $Q$ is filled by a sliver microstructure with total mass
$M_Q\sim m\int_Q\beta\wedge\psi$ written as $N_Q$ equal-mass calibrated pieces.
Assume that across each interior interface $F=Q\cap Q'$ the face-slice parameterizations are \emph{template-coherent} in the sense that,
after pairing pieces crossing $F$, the induced transverse measures arise from the same template by face maps whose variation is $O(h)$:
equivalently, the hypotheses of Lemma~\ref{lem:template-displacement} hold on each $F$
(or more generally Lemma~\ref{lem:template-displacement-edits}, with the unpaired term small enough to be absorbed as in Remark~\ref{rem:template-edits}).
Then, summing over all interfaces and using the boundary-budget estimate (Hypothesis~\ref{hyp:BB}) on each $Q$,
one obtains the global bound
\[
\mathcal F(\partial T^{\mathrm{raw}})\ \le\ C\,h^2\sum_Q M_Q^{\frac{k-1}{k}}\,N_Q^{\frac1k}\ +\ O(\varepsilon\,m),
\qquad k:=2n-2p.
\]
In particular, at the Bergman scale $h\asymp m^{-1/2}$ with uniformly bounded $M_Q\asymp m\,h^{2n}$, if $N_Q\le m^\beta$ for all $Q$ then
\[
\mathcal F(\partial T^{\mathrm{raw}})\ \lesssim\ m^{\frac{n-1+\beta}{k}}\ +\ O(\varepsilon\,m),
\]
which is $o(m)$ provided $\beta<k-(n-1)$ and $\varepsilon=o(1)$.
\end{proposition}

\begin{proof}
Write the raw current as a sum of cell pieces $T^{\mathrm{raw}}=\sum_Q S_Q$ as in Substep~4.2, so that
$\partial T^{\mathrm{raw}}$ is supported on the union of interior interfaces.  Decompose
\[
\partial T^{\mathrm{raw}}=\sum_F B_F,
\]
where each $F=Q\cap Q'$ is an interior interface and $B_F$ is the corresponding mismatch current on $F$.
...
\smallskip\noindent
\textbf{Per-face estimate.}
By the template-coherence hypothesis on each $F$ (Lemma~\ref{lem:template-displacement}, or the edited version
Lemma~\ref{lem:template-displacement-edits}), one has
\[
\mathcal F(B_F)\ \le\ C\,h^2\Bigl(\Mass(\partial S_Q\llcorner F)+\Mass(\partial S_{Q'}\llcorner F)\Bigr)\ +\ O(\varepsilon\,M_F),
\]
where $M_F\lesssim M_Q+M_{Q'}$ and $\varepsilon$ is the small-angle/graph parameter from
Proposition~\ref{prop:transport-flat-glue}.

\smallskip\noindent
\textbf{Summation over interfaces.}
Summing the boundary-mass term over all interfaces and reindexing by cells gives
\[
\sum_F \Bigl(\Mass(\partial S_Q\llcorner F)+\Mass(\partial S_{Q'}\llcorner F)\Bigr)
\ \le\ C_{\deg}\sum_Q \Mass(\partial S_Q),
\]
where $C_{\deg}$ depends only on the local combinatorics of the decomposition (a uniform bound on the number of interfaces meeting a cell).
Applying Hypothesis~\ref{hyp:BB} on each $Q$ yields
\[
\Mass(\partial S_Q)\ \le\ C\,M_Q^{\frac{k-1}{k}}\,N_Q^{\frac1k},
\qquad k:=2n-2p,
\]
up to a $(1+O(\varepsilon^2))$ factor absorbed into the constant.  Therefore
\[
\sum_F C\,h^2\Bigl(\Mass(\partial S_Q\llcorner F)+\Mass(\partial S_{Q'}\llcorner F)\Bigr)
\ \le\ C\,h^2\sum_Q M_Q^{\frac{k-1}{k}}\,N_Q^{\frac1k}.
\]

For the error term, since each interface contributes $O(\varepsilon(M_Q+M_{Q'}))$ and each cell meets only $O(1)$ interfaces,
one has $\sum_F M_F\lesssim \sum_Q M_Q\sim m\int_X\beta\wedge\psi\asymp m$, hence the total error is $O(\varepsilon\,m)$.
Combining these estimates yields the first displayed bound.

\smallskip\noindent
\textbf{Bergman-scale estimate.}
At the Bergman scale $h\asymp m^{-1/2}$, the number of cells is $\asymp h^{-2n}\asymp m^n$.
Moreover $M_Q\sim m\int_Q\beta\wedge\psi\lesssim m\,h^{2n}$ uniformly (since $\beta$ is bounded), so $M_Q\lesssim m^{1-n}$.
If in addition $N_Q\le m^\beta$ for all $Q$, then
\[
h^2\sum_Q M_Q^{\frac{k-1}{k}}\,N_Q^{\frac1k}
\ \lesssim\ 
m^{-1}\cdot m^n\cdot (m^{1-n})^{\frac{k-1}{k}}\cdot m^{\beta/k}
\ =\ m^{\frac{n-1+\beta}{k}}.
\]
This gives the stated bound and the $o(m)$ criterion.
\end{proof}

\begin{remark}[What this reduction buys]\label{rem:weighted-sliver-meaning}
Proposition~\ref{prop:weighted-sliver-glue} packages the ``unconditional feasible'' strategy into two explicit, local analytic inputs:
\begin{enumerate}
\item a \emph{sliver filling theorem} (Proposition~\ref{prop:sliver-local} / Corollary~\ref{cor:holomorphic-flat-sliver-local}) producing many tiny
calibrated holomorphic pieces per Bergman-scale cell, and
\item a \emph{template displacement estimate} at scale $h^2$ comparing the induced face slices across neighboring cells.
\end{enumerate}
Once these hold, the rest of the SYR chain (flat-norm filling, cohomology rounding, and calibrated compactness) proceeds exactly as written.
\end{remark}
...
\begin{lemma}[Flat-norm stability under small $C^0$ displacement]\label{lem:flat-small-displacement}
Let $S$ be an integral $\ell$-cycle in $\R^d$ with finite mass.
Let $f:\R^d\to\R^d$ be a Lipschitz map with $\mathrm{Lip}(f)\le L$ and
\[
\sup_{x\in\R^d}\|f(x)-x\|\ \le\ \delta.
\]
Then
\[
\mathcal F\!\bigl(f_\#S-S\bigr)\ \le\ C(\ell)\,\delta\,L^{\ell}\,\Mass(S).
\]
\end{lemma}

\begin{proof}[Sketch]
Consider the straight-line homotopy $H:[0,1]\times\R^d\to\R^d$, $H(t,x)=(1-t)x+t f(x)$.
Then $\partial H_\#([0,1]\times S)=f_\#S-S$.
The mass of $H_\#([0,1]\times S)$ is controlled by $\sup\|f-\mathrm{Id}\|$ times a uniform Jacobian bound depending on $\mathrm{Lip}(f)$ and $\ell$,
giving $\Mass(H_\#([0,1]\times S))\le C(\ell)\,\delta\,L^\ell\,\Mass(S)$.
Taking $R=0$ in the flat norm definition yields the estimate.
\end{proof}
...
Since $S$ is a cycle, $Q:=H_\#([0,1]\times S)$ satisfies $\partial Q = S - (x_0)_\#S = S$ (the pushforward under the constant map is $0$ in positive dimension).
...
\mathcal F(B_F)\ \le\ C\,h^2\,\Bigl(\Mass(\partial S_Q\llcorner F)+\Mass(\partial S_{Q'}\llcorner F)\Bigr)\ +\ O(\varepsilon\,M_F),
\]
where $M_F\lesssim M_Q+M_{Q'}$ and $\varepsilon$ is the small-angle/graph parameter from Proposition~\ref{prop:transport-flat-glue}\textnormal{(a)}.
\end{lemma}
...
Summing over atoms (with weights $w_a$) yields
$\mathcal F(B_F)\le C h^2\,\Mass(\partial S_Q\llcorner F)$ in the flat model, and symmetrizing gives the stated bound with both sides.
...
\begin{remark}[Allowing small template edits across a face]\label{rem:template-edits}
Lemma~\ref{lem:template-displacement} treats the cleanest situation where the two sides use the \emph{same} discrete template $\nu$.
In practice one may only be able to enforce template coherence up to a small number of insertions/deletions (as in
Lemma~\ref{lem:w1-template-edit} for the dense-sheet regime).
In the sliver regime, the same idea is available provided the \emph{unpaired} face slices are supported in a patch of diameter $\lesssim h$:
each unpaired slice is a $(k-1)$--cycle in $F$ and thus has flat norm $\lesssim h$ times its slice mass by Lemma~\ref{lem:flat-diameter}.
If the inserted/deleted pieces are chosen to be \emph{very small slivers} (hence with very small boundary slices), this unpaired contribution
is of the same $h^2\times(\text{boundary mass})$ order as the matched displacement term and can be absorbed into the bookkeeping of
Proposition~\ref{prop:weighted-sliver-glue}.
\smallskip

One convenient sufficient condition is that the edits constitute an $O(h)$ fraction of the total boundary mass on the face;
Lemma~\ref{lem:template-edits-oh} then shows they are automatically $O(h^2)$ in flat norm and can be absorbed.
\end{remark}
...
\]
In a flat smooth convex model (e.g.\ a ball cell), if $M_Q$ is split into $N_Q$ \emph{equal} sliver pieces of mass $M_Q/N_Q$, then the
...
\]
If, across a shared interface, the corresponding face slices are displaced by $\asymp h^2$ (as in the template/face-map variation heuristics),
then a crude cylinder filling gives a per-face flat-norm mismatch of order
\[
...
\]
In particular, since $(n-1)/k<1$ for $k=2n-2p\ge 2$, this is automatically sublinear in $m$ provided $N_Q$ grows at most polynomially in $m$
with exponent $<k-(n-1)$.  Making any version of this calculation rigorous inside the cubical/face framework requires precisely the weighted
bookkeeping estimate flagged in Remark~\ref{rem:sliver-vs-template}.
\end{remark}

\begin{remark}[Explicit parameter choices]\label{rem:template-params}
For a target flat-norm error fraction $\theta\in(0,1)$, Proposition~\ref{prop:template-flat-glue} gives
$\mathcal F(\partial T^{\mathrm{raw}})\le C_3(h+\varepsilon)m$ (when $\Mass(T^{\mathrm{raw}})\asymp m$).
Thus it suffices to choose, for example,
\[
h:=\frac{\theta}{10C_3},\qquad \varepsilon:=\frac{\theta}{10C_3},
\]
to obtain $\mathcal F(\partial T^{\mathrm{raw}})\le \theta\,m$ and hence a gluing correction current $R_{\mathrm{glue}}$ with
$\Mass(R_{\mathrm{glue}})\lesssim \theta m$.
\smallskip

\noindent
Equivalently, if one wants an explicit $m$--dependent schedule, one may take for any $\alpha>0$,
\[
h:=m^{-\alpha},\qquad \varepsilon:=m^{-\alpha},
\]
which yields $\mathcal F(\partial T^{\mathrm{raw}})\lesssim m^{1-\alpha}$, i.e.\ $\mathcal F(\partial T^{\mathrm{raw}})=o(m)$ as $m\to\infty$.
\smallskip

\noindent\textbf{What remains genuinely unproved.}
The proposition is conditional on verifying \textnormal{(T1)}--\textnormal{(T2)} for the \emph{holomorphic} sheet families produced in Substep~3.5:
uniform tubular-face charts and an $O(h)$ control of the face-slice maps (T1), and a mechanism enforcing slow variation of multiplicities (T2).
This proposition should be viewed as the clean quantitative closure of regime \textnormal{(I)} (dense sheets / constant-mass pieces).
In middle codimension and at Bergman scale (regime \textnormal{(II)}), one replaces the counting step by a boundary-budget estimate and a
template-displacement control, as summarized by Proposition~\ref{prop:weighted-sliver-glue} (built on Proposition~\ref{prop:sliver-boundary-budget}
and the local sliver targets of Proposition~\ref{prop:sliver-local} / Corollary~\ref{cor:holomorphic-flat-sliver-local}).
\end{remark}
...
\item use a true “sliver’’ mechanism (Proposition~\ref{prop:sliver-local}) that splits the target cube mass into many much smaller local pieces,
...
\]
is continuous in the translation parameter $t\in P^\perp\cong\R^{2p}$ and its image contains an interval $[0,A_{\max}]$ with $A_{\max}\asymp h^{2(n-p)}$.
...
For smooth convex $Q$, the $(2n-2p)$-dimensional cross-sectional volume of $Q$ by translates of a fixed plane depends continuously
on the translation parameter, and it vanishes when the translate becomes disjoint from $Q$.
Taking a translate that passes through the “thick” region of $Q$ gives a cross-section of size $\asymp h^{2(n-p)}$, yielding $A_{\max}\asymp h^{2(n-p)}$.
\end{proof}
...
$\lesssim r\,2^{-k}\cdot 1$, yielding $W_1\lesssim r\,2^{-k}\asymp r\,N^{-1/(d-1)}$.
...
\begin{proposition}[Flat ball model: volumetric transverse densities via radial shells]\label{prop:flat-sliver-local-ball}
Work in the same flat setting as Proposition~\ref{prop:flat-sliver-local}, with transverse space $P^\perp\cong\R^{2p}$.
Let $\Omega:=B_R(0)\subset P^\perp$ with $R\le c\,h$ and let $\rho:\Omega\to\R_{\ge0}$ be Lipschitz with total mass
$M=\int_{\Omega}\rho\,dt$.
Fix a radial partition $0=R_0<R_1<\cdots<R_L=R$ and set the annuli
$A_\ell:=\{t\in\Omega: R_{\ell-1}\le \|t\|\le R_\ell\}$ and masses $M_\ell:=\int_{A_\ell}\rho\,dt$.
Fix integers $N_\ell\ge 1$ and assume $M_\ell/N_\ell\le \Mass([P]\llcorner Q)$ for each $\ell$.
Then there exist radii $r_\ell\in[0,h)$ and translations $t_{\ell,1},\dots,t_{\ell,N_\ell}\in S^{2p-1}(r_\ell)$ such that:
\begin{enumerate}
\item[\textnormal{(i)}] (\textbf{Prescribed sliver masses}) for each $\ell$ and each $a$,
\[
\Mass\bigl([P+t_{\ell,a}]\llcorner Q\bigr)\ =\ \frac{M_\ell}{N_\ell};
\]
\item[\textnormal{(ii)}] (\textbf{Volumetric $W_1$ approximation}) with the induced weighted atomic measure
\[
\mu:=\sum_{\ell=1}^L\ \sum_{a=1}^{N_\ell} \Mass\bigl([P+t_{\ell,a}]\llcorner Q\bigr)\,\delta_{t_{\ell,a}},
\]
one has the estimate
\[
W_1(\mu,\rho\,dt)
\ \le\
M\,\max_{\ell}(R_\ell-R_{\ell-1})
\ +\ 
C(p)\sum_{\ell=1}^L r_\ell\,\Bigl(M_\ell+\mathrm{Lip}(\rho)\,R^{2p}\,(R_\ell-R_{\ell-1})\Bigr)\,N_\ell^{-\frac{1}{2p-1}}.
\]
\end{enumerate}
\end{proposition}

\begin{proof}[Sketch]
\textbf{Step 1 (match the per-piece masses by choosing radii).}
By Lemma~\ref{lem:ball-section-scaling}, the slice mass $r\mapsto \Mass([P+t]\llcorner Q)$ depends only on $r=\|t\|$ and is continuous and strictly
decreasing from $\Mass([P]\llcorner Q)=c_k h^{k}$ at $r=0$ to $0$ at $r=h$.
Thus for each $\ell$ there exists $r_\ell\in[0,h)$ such that $\Mass([P+t]\llcorner Q)=M_\ell/N_\ell$ for all $t\in S^{2p-1}(r_\ell)$.

\smallskip\noindent
\textbf{Step 2 (push each annulus radially to a sphere).}
For each $\ell$, choose a measurable map $\pi_\ell:A_\ell\to S^{2p-1}(r_\ell)$ that moves points only in the radial direction
(e.g.\ $\pi_\ell(t)=(r_\ell/\|t\|)\,t$ for $t\neq 0$, and $\pi_\ell(0)$ arbitrary when $R_{\ell-1}=0$).
Then $\|\pi_\ell(t)-t\|\le |r_\ell-\|t\||\le R_\ell-R_{\ell-1}$, hence transporting $\rho\,dt\llcorner A_\ell$ by $\pi_\ell$ costs at most
$M_\ell(R_\ell-R_{\ell-1})$ in $W_1$.
Summing over $\ell$ gives the first term $M\max_\ell(R_\ell-R_{\ell-1})$.

\smallskip\noindent
\textbf{Step 3 (quantize each spherical measure).}
Let $\rho_\ell\,d\sigma_{r_\ell}:=(\pi_\ell)_\#(\rho\,dt\llcorner A_\ell)$ be the pushed-forward measure on $S^{2p-1}(r_\ell)$; its total mass is $M_\ell$.
Apply Lemma~\ref{lem:sphere-quantize} (on $S^{2p-1}(r_\ell)$) to choose points $t_{\ell,1},\dots,t_{\ell,N_\ell}$ so that
the equal-weight atomic measure $\mu_\ell:=\sum_{a=1}^{N_\ell}(M_\ell/N_\ell)\,\delta_{t_{\ell,a}}$ satisfies
\[
W_1(\mu_\ell,\rho_\ell\,d\sigma_{r_\ell})
\ \le\
C(p)\,r_\ell\,\Bigl(M_\ell+\mathrm{Lip}(\rho_\ell)\,r_\ell^{2p-1}\Bigr)\,N_\ell^{-\frac{1}{2p-1}}.
\]
Estimating $\mathrm{Lip}(\rho_\ell)$ in terms of $\mathrm{Lip}(\rho)$ and the shell thickness yields the stated form.

\smallskip\noindent
\textbf{Step 4 (assemble shells).}
Define $\mu:=\sum_\ell \mu_\ell$; by construction each atom weight equals $\Mass([P+t_{\ell,a}]\llcorner Q)$, giving (i).
The $W_1$ estimate follows from the triangle inequality and summing the shell errors.
\end{proof}
...
Then Proposition~\ref{prop:finite-template} produces $\psi$-calibrated holomorphic complete intersections $Y^1,\dots,Y^N$ whose restricted
pieces on $Q$ are $C^1$ graphs over $P+t_a$ with
\[
...
\begin{corollary}[Holomorphic upgrade for volumetric transverse densities]
\label{cor:holomorphic-flat-sliver-local-ball}
In the setting of Proposition~\ref{prop:flat-sliver-local-ball}, assume $Q$ lies in a holomorphic chart and that $P$ is a calibrated complex
$(n-p)$-plane in those coordinates with normal covectors $\lambda_1,\dots,\lambda_p$.
Fix $\varepsilon>0$ and choose $m\ge m_1(\varepsilon)$ (Lemma~\ref{lem:bergman-control}) with $\mathrm{diam}(Q)\le c\,m^{-1/2}$.
Then, for any choice of radial partition $\{R_\ell\}$ and integers $\{N_\ell\}$ as in Proposition~\ref{prop:flat-sliver-local-ball}, one may choose
translations $t_{\ell,a}\in P^\perp$ and construct $\psi$-calibrated holomorphic complete intersections $Y^{\ell,a}$ such that each restricted piece
$[Y^{\ell,a}]\llcorner Q$ is a $C^1$ graph over $P+t_{\ell,a}$ with
\[
\Mass([Y^{\ell,a}]\llcorner Q)=(1+O(\varepsilon^2))\,\Mass([P+t_{\ell,a}]\llcorner Q).
\]
Consequently, the induced transverse measure
\[
\sum_{\ell,a} \Mass([Y^{\ell,a}]\llcorner Q)\,\delta_{t_{\ell,a}}
\]
approximates $\rho\,dt$ in $W_1$ with error bounded by the right-hand side of Proposition~\ref{prop:flat-sliver-local-ball} plus an additional
$O(\varepsilon^2)\,M$ term, where $M=\int_\Omega\rho$.
\end{corollary}

\begin{remark}[Interpretation]
Proposition~\ref{prop:flat-sliver-local} (sphere case) and Proposition~\ref{prop:flat-sliver-local-ball} (volumetric case) show that the
\emph{transverse-measure approximation} part of Proposition~\ref{prop:sliver-local} is achievable in a clean flat ball model using exact affine
calibrated pieces (with weights tied to the plane-slice mass through the choice of radius).
Corollaries~\ref{cor:holomorphic-flat-sliver-local} and \ref{cor:holomorphic-flat-sliver-local-ball} show that, by the standard Bergman/peak-section
$C^1$ control encoded in Lemma~\ref{lem:bergman-control}, these affine sliver templates admit a holomorphic complete-intersection upgrade on Bergman-scale cells.
The remaining nontrivial step for unconditional closure is thus the \emph{cube/face compatibility for gluing} (template coherence across the entire adjacency graph),
not the local realization on a single cell.
\end{remark}

\begin{proposition}[Local sliver-sheet realizability on a ball cell]\label{prop:sliver-local}
Fix a sufficiently small \emph{ball-like} coordinate cell $Q$ of diameter $h$ inside a holomorphic chart
(e.g.\ a geodesic ball in normal coordinates, so the flat ball model applies up to $(1+O(h^2))$ distortion as in Lemma~\ref{lem:normal-distortion}).
Fix a calibrated complex $(n-p)$-plane direction $P\in K_{n-p}(x_Q)$ with normal space $P^\perp\cong\R^{2p}$ in that chart.
Let $\Omega:=B_R(0)\subset P^\perp$ with $R\le c\,h$ and let $\rho:\Omega\to\R_{\ge 0}$ be Lipschitz with total mass $M=\int_\Omega\rho\,dt$.
Then for every choice of a radial partition $0=R_0<\cdots<R_L=R$ and integers $N_\ell\ge 1$ as in Proposition~\ref{prop:flat-sliver-local-ball},
setting $N:=\sum_{\ell=1}^L N_\ell$, there exist $\psi$-calibrated holomorphic complete intersections
$Y^{\ell,a}\subset X$ ($1\le \ell\le L$, $1\le a\le N_\ell$) such that:
\begin{enumerate}
\item[\textnormal{(i)}] (\textbf{Small-angle / graph control})
each $Y^{\ell,a}\cap Q$ is a $C^1$ graph over an affine translate $P+t_{\ell,a}$ with slope $\le C\,\varepsilon$ (hence
$\sup_{y\in Q}\angle(T_yY^{\ell,a},P)\le C\,\varepsilon$), where $\varepsilon\to 0$ as $h\to 0$ through Lemma~\ref{lem:bergman-control};
\item[\textnormal{(ii)}] (\textbf{Sliver masses})
\[
\Mass([Y^{\ell,a}]\llcorner Q)\ =\ (1+O(\varepsilon^2))\,\frac{M_\ell}{N_\ell},
\qquad
M_\ell:=\int_{A_\ell}\rho\,dt,
\]
and in particular $\sum_{\ell,a}\Mass([Y^{\ell,a}]\llcorner Q)=M+O(\varepsilon^2)M$;
\item[\textnormal{(iii)}] (\textbf{Transverse measure approximation})
the induced transverse measure
\[
\mu:=\sum_{\ell=1}^L\ \sum_{a=1}^{N_\ell}\Mass([Y^{\ell,a}]\llcorner Q)\,\delta_{t_{\ell,a}}
\]
satisfies
\[
W_1(\mu,\rho\,dt)\ \le\ \tau(\{R_\ell\},\{N_\ell\},h)\ +\ O(\varepsilon^2)\,M,
\]
where $\tau(\{R_\ell\},\{N_\ell\},h)$ is the explicit right-hand side of Proposition~\ref{prop:flat-sliver-local-ball}.
\end{enumerate}
In particular, by choosing the shell thickness $\max_\ell(R_\ell-R_{\ell-1})\to 0$ and $N_\ell\to\infty$ as $h\to 0$, one obtains
$W_1(\mu,\rho\,dt)\to 0$.
\end{proposition}

\begin{proof}[Sketch]
Work in the holomorphic chart and apply the flat ball model up to the $(1+O(h^2))$ distortion from Lemma~\ref{lem:normal-distortion}.
Proposition~\ref{prop:flat-sliver-local-ball} produces translations $t_{\ell,a}$ and affine calibrated pieces $[P+t_{\ell,a}]\llcorner Q$
with the stated masses and $W_1$ error $\tau(\{R_\ell\},\{N_\ell\},h)$.
Proposition~\ref{prop:finite-template} (based on Lemma~\ref{lem:bergman-control}) upgrades each affine piece to a holomorphic complete intersection
$Y^{\ell,a}$ which is a $C^1$ graph over $P+t_{\ell,a}$ on $Q$, with mass distortion factor $(1+O(\varepsilon^2))$.
The change of weights contributes at most $O(\varepsilon^2)M$ to $W_1$, and the rest is the flat-model error term.
\end{proof}
...
\begin{remark}[Why Proposition~\ref{prop:sliver-local} helps close the fixed-$m$ tension]\label{rem:sliver-close}
If Proposition~\ref{prop:sliver-local} holds uniformly on each cube (with $M\sim m\int_Q\beta\wedge\psi$),
then one can keep $N$ large even when $m$ is fixed and $h\to 0$, because each individual piece can have arbitrarily small mass.
This restores the “many degrees of freedom per face’’ regime needed for transport averaging and discrepancy rounding,
while preserving total mass $O(m)$.  Combined with the flat-norm gluing criterion (Lemma~\ref{lem:flat-glue-criterion})
and the transport estimates in Substep~4.2, this would yield an unconditional realization step.
\end{remark}
...
Let $t_1,\dots,t_N\in P^\perp\cong\R^{2p}$ be translation vectors.
...
\item[\textnormal{(ii)}] $\Mass([Y^a]\llcorner Q)=(1+O(\varepsilon^2))\,\Mass([P+t_a]\llcorner Q)$.
\item[\textnormal{(iii)}] (\textbf{Optional disjointness}) If additionally $\|t_a-t_b\|\ge 10\,\varepsilon\,\mathrm{diam}(Q)$ for $a\neq b$,
then the pieces $Y^a\cap Q$ are pairwise disjoint.
\end{enumerate}
\end{proposition}
...
Then Lemma~\ref{lem:graph-from-grad} gives (i) (graph control) and Lemma~\ref{lem:sliver-stability} gives (ii) (mass comparability).
If the optional separation condition holds, Lemma~\ref{lem:sliver-stability}(ii) implies disjointness on $Q$.
\end{proof}
...
One concrete quantitative implementation of the “fixed template’’ route is Proposition~\ref{prop:template-flat-glue}, which shows that
template coherence plus $O(h)$ stability of face-slice maps yields $\mathcal F(\partial T^{\mathrm{raw}})\lesssim (h+\varepsilon)\,m$.
In the Bergman-scale sliver regime \textnormal{(II)}, the analogous bookkeeping is summarized by Proposition~\ref{prop:weighted-sliver-glue},
which replaces the raw sheet-count estimate by a boundary-budget bound on $\Mass(\partial S_Q)$.
...
\begin{remark}[Compatibility with prefix templates and slow variation]\label{rem:cohomology-vs-prefix}
In Proposition~\ref{prop:cohomology-match} the only freedom is to choose, for each variable $(Q,j)$, whether to round $n_{Q,j}$ up or down:
$N_{Q,j}=\lfloor n_{Q,j}\rfloor$ or $\lfloor n_{Q,j}\rfloor+1$.
In the global template scheme (Remark~\ref{rem:nested-template-scheme}), this corresponds exactly to adding or removing a \emph{bounded} number of template atoms
(a bounded prefix correction).
Thus, once the cellwise template sizes satisfy $N_{Q,j}\gtrsim h^{-1}$ (many atoms/slivers per cell), the cohomology correction does not interfere with the
$O(h)$ edit fraction needed for Proposition~\ref{prop:prefix-template-coherence}; see Remark~\ref{rem:bounded-corrections}.
\end{remark}
...
\begin{theorem}[Automatic SYR for cone-valued forms (conditional on microstructure)]\label{thm:automatic-syr}
...
\begin{proof}
Assume the Microstructure/flat-norm gluing theorem (Theorem~\ref{thm:microstructure-flatglue}).
Apply it to the effective class $[\gamma]$ and its cone-valued representative $\beta$.
This produces a sequence of closed integral $\psi$-calibrated currents in the desired homology class with the stated mass and Young-measure properties.
Harvey--Lawson and Chow then yield algebraicity.
\end{proof}
...
\begin{theorem}[Effective classes are algebraic (conditional on microstructure)]\label{thm:effective-algebraic}
...
% ============================================================
\subsection*{Main theorem: Hodge conjecture (conditional on microstructure)}
% ============================================================

\begin{theorem}[Hodge Conjecture for rational $(p,p)$ classes (conditional)]
...
\begin{corollary}[Full Hodge conjecture (conditional)]\label{cor:full-hodge}
