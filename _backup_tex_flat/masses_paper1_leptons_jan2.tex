...
\title{\textbf{Charged Fermion Masses from Octave Closure and $\phig$-Ladder Geometry}\\[0.25em]
\large A single-anchor, no-per-flavor-tuning mass framework with explicit transport hygiene}
...
\begin{abstract}
The Standard Model treats the charged-fermion masses as inputs (Yukawa couplings) rather than outputs.
This paper presents a structural mass framework in which charged fermion masses are organized by two ingredients:
(i) an eight-tick closure that supplies a canonical ``octave'' reference for ladder coordinates, and (ii) a $\phig$-ladder scale coordinate
that encodes multiplicative hierarchy by fixed rung steps.
At a single common anchor scale $\muStar$, each charged fermion is assigned an integer rung within its sector and a charge-derived band label;
the mass at the anchor is then determined by sector-global yardsticks together with these discrete coordinates.
No per-species fitting is permitted: the only inputs at the model layer are sector-wide integers and the charge-to-band map shared across families.

To compare to experimental conventions at other scales or schemes, Standard-Model renormalization-group running is used \emph{only} as
bookkeeping transport under a declared policy (\CERT), and is never conflated with the structural band coordinate (\PROVED).
We report reproducible tables for the charged-lepton sector (electron, muon, tau), including accuracy against PDG values (\VAL),
and provide explicit ablations and falsifiers that would refute the framework if observed.
\end{abstract}
...
In the Standard Model (SM), charged-fermion masses arise from Yukawa couplings.
Those couplings are empirical inputs: the SM is extraordinarily successful once the numbers are specified, but it does not explain why the
electron, muon, and tau have the numerical values they do, nor why the charged-fermion spectrum spans many orders of magnitude.
This motivates the search for \emph{structure}: a small set of organizing principles that constrain the spectrum without introducing
per-particle tuning knobs.

\subsection{Framework overview: discrete closure and ladder coordinates}
The framework developed here treats stable particles as stable \emph{recognition boundaries} whose persistence is tied to discrete closure.
Two ingredients organize the mass hierarchy:
\begin{itemize}
  \item \textbf{Octave (eight-tick closure).} A minimal closure in three dimensions yields an 8-tick cycle. In this paper, that cycle supplies a canonical
  reference for ladder coordinates: the appearance of a universal ``$-8$'' offset is an octave reference (a coordinate origin), not a fitted correction.
  \item \textbf{$\phig$-ladder (multiplicative scale coordinate).} We represent scale hierarchies by a ladder coordinate in which integer rung shifts correspond to
  fixed multiplicative factors. The choice of $\phig$ is structural within the framework: it is the unique self-similar scaling factor compatible with the closure
  and cost constraints that define the model.
\end{itemize}
At a single common anchor scale $\muStar$, each charged fermion is described by:
(i) a sector-wide yardstick (shared across all members of a sector), (ii) an integer rung, and (iii) a charge-derived band label shared across equal-charge families.

\subsection{Claim contract and non-circularity (referee-facing)}
This paper enforces a strict claim contract.
\begin{itemize}
  \item \textbf{No per-species fitting.} The model layer permits only sector-global integers and shared maps (e.g.\ a common charge-to-band rule).
  There are no particle-by-particle offsets or ``hand-tuned'' exponents.
  \item \textbf{Transport hygiene.} Any comparison to Particle Data Group (PDG) conventions requires an explicit choice of scheme and scale.
  Standard-Model renormalization-group running is used \emph{only} as bookkeeping transport under a declared policy and is never identified with the
  structural band coordinate.
  \item \textbf{No circular testing.} No measured mass may appear on the right-hand side of its own prediction. Where external conventions enter (e.g.\ PDG values),
  they enter only on the validation side.
\end{itemize}
Throughout, we distinguish structural derivations from declared transport conventions and from validation comparisons against external data.

\subsection{What this paper delivers (and what it does not)}
The main deliverables are:
\begin{itemize}
  \item a single-anchor mass framework for charged fermions built from sector-global yardsticks and discrete coordinates,
  \item an explicit derivation of the charged-lepton masses (electron, muon, tau) as a reproducible absolute prediction pipeline,
  \item ablations and falsifiers designed to make the framework refutable rather than merely descriptive.
\end{itemize}
We will cite PDG values for comparison \cite{PDG2024}; those comparisons are labeled as validation rather than derivation.

This paper does \emph{not} claim that a structural band coordinate is invariant under SM running away from the anchor, and it does \emph{not} claim that
bookkeeping transport exponents are the same object as the structural band map.

\subsection{Notation and classical correspondences}
Table~\ref{tab:classical_dict} summarizes the key terms used in this paper
and their correspondences to standard physics concepts.
This dictionary is intended to help readers familiar with SM phenomenology
navigate the structural framework without requiring prior exposure to the
underlying discrete-geometry program.
...
\begin{table}[h]
  ...
  \begin{tabular}{llll}
    \toprule
    Term & Classical Equivalent & Status & Notes \\
    \midrule
    $\phig$-ladder & log-scale mass coordinate & Bridge & Self-similar scaling; $\phig^2=\phig+1$ \\
    Octave (8-tick) & --- & Novel & Minimal 3-bit closure; no classical analog \\
    Rung ($r$) & discrete scale index & Bridge & Integer step on $\phig$-ladder \\
    gap$(Z)$ & family band correction & Bridge & Closed-form from charge; see Eq.~\eqref{eq:gap_def} \\
    $f^{\mathrm{RG}}$ & SM RG running & Twin & Standard QED/QCD transport \\
    Sector yardstick & SM sector mass scale & Bridge & Discrete cube-derived input \\
    $\muStar$ & anchor scale (GeV) & Bridge & Common reference for all species \\
    \bottomrule
  \end{tabular}
  \caption{Dictionary of framework terms and their classical correspondences.
    \emph{Twin}: mathematically identical to the classical object.
    \emph{Bridge}: corresponds via an explicit mapping or limit.
    \emph{Novel}: no direct classical analog exists.}
  \label{tab:classical_dict}
\end{table}
...
\input{../../out/masses/lepton_chain_pred_vs_pdg.tex}
...
\input{../../out/masses/rg_transport_policy_table.tex}
...
\section{Conclusions}
This paper presented a parameter-free structural organization of the charged-fermion mass spectrum at a single anchor scale \(\muStar\),
and a fully specified charged-lepton mass pipeline derived from the same discrete geometry.
The organizing principle is that once the charge-derived family label \(Z\) is fixed, the residual correction is a \emph{family-level} band
\(\mathrm{gap}(Z)\), not a particle-by-particle fit. \PROVED

\subsection{What is structural, and what is a declared convention}
\paragraph{Structural layer (model claims).}
The construction of the integerized charge \(\tildeQ\), the family label \(Z(\tildeQ)\), and the band map \(\mathrm{gap}(Z)\) is structural and contains no adjustable parameters. \PROVED
The sector yardsticks and rung conventions used in the skeleton factor \(m_{\mathrm{skel}}(i;\muStar)\) are likewise sector-global rather than species-specific. \PROVED

\paragraph{Display layer (bookkeeping for comparison).}
Two kinds of declarations are required for comparison to external datasets:
\begin{itemize}
  \item an explicit absolute-scale convention (a calibration seam) for reporting values in eV/MeV, \CERT
  \item and an explicit transport convention for mapping external scheme/scale choices back to the anchor. \CERT
\end{itemize}
Neither declaration is allowed to modify the structural band map \(\mathrm{gap}(Z)\); transport is bookkeeping only and must not be conflated with band structure. \CERT

\subsection{What was validated numerically in this paper}
With the declarations above fixed and stated, the charged-lepton chain produces the electron/muon/tau masses with the quoted accuracy (Table~\ref{tab:lepton_chain_pred_vs_pdg}). \VAL
The same framework yields a falsifiable diagnostic for whether external charged-fermion data clusters by equal \(Z\) at the anchor (Sec.~7). \VAL

\subsection{How to break the framework}
Section~8 provides a set of ablations (remove one ingredient and observe failure) and falsifiers (observations that would refute the proposal).
The most direct falsifier is the failure of equal-\(Z\) clustering at \(\muStar\) under any explicitly declared transport policy, because this tests the central claim:
that the family label \(Z\) is the correct coordinate for the spectrum at the anchor. \VAL

\subsection{Roadmap for the companion papers}
Paper~2 develops the CKM/PMNS mixing predictions from the same ledger geometry and fixes the claim tags for those equations.
Paper~3 develops the neutrino sector (deep-ladder structure and mass-splitting predictions), including the same calibration seam discipline for eV reporting.


\appendix

\section{Claim-to-Symbol Map}
This appendix is a cross-reference for notation used in Paper~1.
It also records the \emph{claim class} of each object: whether it is derived structurally, declared as a convention/certificate,
introduced as a modeling hypothesis, or used only in validation against external data.

\subsection{Claim tags}
The paper uses four required tags (attached to displayed equations throughout):
\begin{itemize}
  \item \PROVED: structural derivation in the mathematical layer (no per-species fitting),
  \item \CERT: declared convention or certified numerical value used for bookkeeping/reporting,
  \item \HYP: modeling hypothesis or choice not asserted as a proved structural consequence,
  \item \VAL: comparison to external data or convention-dependent diagnostic.
\end{itemize}

\subsection{Ladder and band coordinates}
\begin{table}[h]
  \centering
  \small
  \begin{tabular}{lllp{0.46\linewidth}}
    \toprule
    Symbol & Role & First definition & Notes / claim class \\
    \midrule
    \(\phig\) & ladder base & Eqs.~\eqref{eq:phi_quadratic}--\eqref{eq:phi_def} &
      \PROVED\ (mathematical definition); \HYP\ (use as the canonical scaling base) \\
    \(\log_{\phig}(x)\) & base-\(\phig\) log & Eq.~\eqref{eq:logphi_def} & \PROVED \\
    \(r_i\) & rung (integer ladder coordinate) & Sec.~3.4; used in Eq.~\eqref{eq:masslaw_anchor} &
      \HYP\ (assignment convention at the anchor) \\
    \(-8\) & octave reference offset & Sec.~2; used in Eq.~\eqref{eq:masslaw_anchor} &
      \HYP\ (interpretation as a forced coordinate origin, not a fit knob) \\
    \(Q\) & electric charge (in units of \(e\)) & Sec.~5.2 & \PROVED\ (physical input, not a fitted parameter) \\
    \(\tildeQ\) & integerized charge & Eq.~\eqref{eq:charge_integerization} & \HYP \\
    \(\Zidx(Q,\mathrm{sector})\) & charge\(\to\)band map & Eq.~\eqref{eq:Zmap} & \HYP \\
    \(Z_\ell,Z_u,Z_d\) & family band labels & Eq.~\eqref{eq:Z_family_values} & \PROVED\ (given \(\tildeQ\) and the rule) \\
    \(\mathrm{gap}(Z)\) & band exponent shift & Eq.~\eqref{eq:gap_def} & \HYP\ (choice of closed form); \PROVED\ (shared within equal-\(Z\) families) \\
    \(\fRec(Z)\) & structural band coordinate & Sec.~7.2 & \PROVED\ (defined as \(\mathrm{gap}(Z)\)) \\
    \bottomrule
  \end{tabular}
\end{table}

\subsection{Yardsticks and anchor display}
\begin{table}[h]
  \centering
  \small
  \begin{tabular}{lllp{0.46\linewidth}}
    ...
    Symbol & Role & First definition & Notes / claim class \\
    \midrule
    \(A_{\mathrm{sector}}\) & sector yardstick & Eq.~\eqref{eq:yardstick_form} &
      \HYP\ (yardstick form); \PROVED\ (no per-species tuning within a sector) \\
    \(B_{\mathrm{pow}}(\mathrm{sector})\) & binary exponent & Eqs.~\eqref{eq:lepton_yardstick_exponents}--\eqref{eq:ew_yardstick_exponents} &
      \PROVED\ (sector formulas) \\
    \(r_0(\mathrm{sector})\) & \(\phig\)-origin exponent & Eqs.~\eqref{eq:lepton_yardstick_exponents}--\eqref{eq:ew_yardstick_exponents} &
      \PROVED\ (sector formulas) \\
    \(\Ecoh\) & coherence unit & Sec.~4.2 & \CERT\ (declared reporting seam when converting to eV/MeV) \\
    \(m_{\mathrm{skel}}(i;\muStar)\) & rung skeleton at anchor & Eq.~\eqref{eq:skeleton_mass} & \PROVED\ (factorization identity) \\
    \(\mRS(i;\muStar)\) & anchor display mass & Eq.~\eqref{eq:masslaw_anchor} & \HYP\ (anchor law as a structural hypothesis) \\
    \bottomrule
  \end{tabular}
\end{table}

\subsection{Lepton chain and transport bookkeeping}
\begin{table}[h]
  \centering
  \small
  \begin{tabular}{lllp{0.46\linewidth}}
    ...
    Symbol & Role & First definition & Notes / claim class \\
    \midrule
    \(\delta_e\) & electron break exponent & Eq.~\eqref{eq:delta_e} & \HYP \\
    \(S_{e\to\mu}\) & generation step & Eq.~\eqref{eq:step_emu} & \HYP \\
    \(S_{\mu\to\tau}\) & generation step & Eq.~\eqref{eq:step_mutau} & \HYP \\
    \(m_e^{\mathrm{pred}},m_\mu^{\mathrm{pred}},m_\tau^{\mathrm{pred}}\) & lepton absolute predictions & Eqs.~\eqref{eq:me_pred}--\eqref{eq:mtau_pred} &
      \HYP\ (pipeline); \VAL\ (comparison in Table~\ref{tab:lepton_chain_pred_vs_pdg}) \\
    \(\muT\) & target scale for comparison & Sec.~7.3 & \CERT\ (declared) \\
    \(\fRG_i(\mu_1,\mu_2)\) & RG transport exponent & Eq.~\eqref{eq:frg_def} & \CERT\ (bookkeeping only; scheme/scale dependent) \\
    \(\mPred(i;\muT)\) & transported display & Eq.~\eqref{eq:transport_display} & \CERT \\
    \(\mdata(i;\muStar)\) & transported data at anchor & Eq.~\eqref{eq:data_to_anchor} & \VAL \\
    \(f_i^{\mathrm{exp}}(\muStar)\) & inferred band from data & Eq.~\eqref{eq:fexp_def} & \VAL \\
    \bottomrule
  \end{tabular}
\end{table}

\section{Reproducibility}
This paper is designed to be reproducible from a clean checkout of the repository.
All numerical tables included in the PDF are generated from scripts and written to \texttt{out/masses/} as \LaTeX{} snippets, which are then included via \verb|\input|.
No table in this paper is edited by hand. \PROVED

\subsection{Generated inputs used by this paper}
The PDF includes exactly two auto-generated table files:
\begin{itemize}
  \item \texttt{out/masses/lepton\_chain\_pred\_vs\_pdg.tex} (Sec.~6): lepton chain prediction and PDG comparison,
  \item \texttt{out/masses/rg\_transport\_policy\_table.tex} (Sec.~7): pinned transport-policy certificate table.
\end{itemize}

\subsection{Lepton chain table (T9--T10)}
From the repository root, regenerate the lepton table via:
\begin{verbatim}
python3 tools/lepton_chain_table.py
\end{verbatim}
This produces:
\begin{itemize}
  \item \texttt{out/masses/lepton\_chain\_pred\_vs\_pdg.tex} (included in the paper),
  \item plus \texttt{.csv} and \texttt{.json} companion artifacts for auditing.
\end{itemize}

\paragraph{Calibration seam (explicit, single-scalar).}
The script reports numbers in MeV under a declared single-anchor calibration seam, provided by a JSON file that contains the single scalar
\texttt{tau0\_seconds} (seconds per RS tick). \CERT
To make that declaration explicit (or to swap the seam for a different reporting choice), run:
\begin{verbatim}
python3 tools/lepton_chain_table.py --calibration-json data/calibration_tau0_seconds_particle_mev.json
\end{verbatim}
The PDG reference targets used for validation are loaded from \texttt{data/masses.json}. \VAL

\subsection{Pinned SM RG transport certificate table}
The transport-policy table is generated from a pinned, auditable certificate file:
\begin{itemize}
  \item certificate: \texttt{data/certificates/rg\_transport/canonical\_2025\_q4.json} \CERT
  \item policy inputs/knobs: \texttt{tools/rg\_transport\_policy.json} \CERT
\end{itemize}
To regenerate the certificate from the declared policy (convention knobs are fixed by the policy file), run:
\begin{verbatim}
...
\end{verbatim}
Then regenerate the \LaTeX{} table snippet used by the paper:
\begin{verbatim}
python3 tools/rg_transport_table.py
\end{verbatim}
Transport is used only for scheme/scale bookkeeping in comparisons and must not be conflated with the structural band coordinate. \CERT

\subsection{Compiling the PDF}
After regenerating the tables above, compile Paper~1 from `papers/tex/`:
\begin{verbatim}
cd papers/tex
pdflatex -interaction=nonstopmode -output-directory=../pdf masses_paper1_leptons.tex
pdflatex -interaction=nonstopmode -output-directory=../pdf masses_paper1_leptons.tex
\end{verbatim}
The resulting PDF is written to \texttt{papers/pdf/masses\_paper1\_leptons.pdf}. \PROVED

\input{masses_bibliography.tex}
