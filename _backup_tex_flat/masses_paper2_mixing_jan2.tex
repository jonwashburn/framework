...
\title{\textbf{CKM and PMNS Mixing from Cubic Ledger Topology}\\[0.25em]
\large A structural account of flavor mixing with explicit claim hygiene}
...
\begin{abstract}
The Standard Model treats the CKM and PMNS mixing matrices as empirical unitary inputs.
This paper develops a structural alternative: flavor mixing is modeled as a consequence of discrete ``ledger'' topology on the 3-cube,
so that mixing weights reduce to simple functions of cube combinatorics (8 vertices, 12 edges, 6 faces, and 24 vertex--edge slots) together with the
shared constants \(\phig\) and \(\alpha\). \HYP

We separate three layers with explicit claim hygiene.
First, several integer coefficients that appear in the mixing-angle expressions are forced by cube counting (no tuning). \PROVED
Second, a small set of closed-form angle/element formulas is proposed from the ledger geometry, with no per-experiment or per-channel fitting. \HYP
Third, we compare the resulting CKM/PMNS values to PDG and NuFIT summaries and label all numerical agreement strictly as validation. \VAL

The paper also gives a falsifiable sign-structure for CP violation (via the Jarlskog invariant) and a referee-facing checklist for what constitutes a valid
objection (e.g., which convention and which experimental summary is being used). \HYP
\end{abstract}
...
\subsection{What the Standard Model does and does not explain about mixing}
In the Standard Model (SM), the CKM and PMNS matrices are empirical unitary inputs:
their angles and phases are measured and then treated as free parameters of the effective theory.
The SM then succeeds spectacularly, but it does not explain \emph{why} the observed mixing has the particular size hierarchy it does,
nor why certain integers and near-rational patterns repeatedly appear in global fits.
This motivates the same kind of question as in the mass spectrum: is there a small, rigid piece of structure that organizes the data without per-channel tuning?

\subsection{Framework overview: mixing as ledger geometry on the 3-cube}
This paper develops the mixing sector of the same discrete-geometry program used in Paper~1.
The core modeling premise is that flavor transitions are constrained by a finite combinatorial ``ledger'' associated with the 3-cube:
8 vertices, 12 edges, 6 faces, and 24 vertex--edge slots. \HYP
These integers supply candidate normalizations and coefficients in mixing expressions.
The resulting formulas are then expressed using the shared constants \(\phig\) and \(\alpha\) (already used in Paper~1), rather than introducing
new mixing-specific fit parameters. \HYP

\subsection{Claim contract (referee-facing)}
We enforce a strict claim contract aligned with Paper~1:
\begin{itemize}
  \item \textbf{No per-channel fitting.} The model layer may use only cube-derived integers and shared constants (\(\phig\), \(\alpha\)).
  There is no tuning of separate coefficients for CKM vs PMNS, no per-angle offsets, and no per-dataset adjustments. \PROVED
  \item \textbf{Claim hygiene.} Every displayed equation is tagged as \PROVED, \CERT, \HYP, or \VAL.
  Structural claims are separated from convention-dependent reporting and from empirical comparison. \PROVED
  \item \textbf{Dataset clarity.} Comparisons to experiment must state the target summary being used (PDG for CKM; NuFIT for PMNS) and the ordering/assumptions (e.g.\ normal ordering for PMNS when applicable). \VAL
\end{itemize}

\subsection{What this paper delivers (and what it does not)}
The deliverables are:
\begin{itemize}
  \item a cube-geometry account of several integer coefficients that appear in mixing-angle corrections (no tuning), \PROVED
  \item proposed closed-form expressions for selected CKM/PMNS elements and mixing-angle combinations in terms of cube counts and shared constants, \HYP
  \item a validation section comparing these expressions to PDG and NuFIT summaries, with explicit falsifiers. \VAL
\end{itemize}
This paper does \emph{not} claim that the SM itself is replaced; it provides a structural hypothesis for why the observed mixing matrices take their form,
and it is judged solely by whether the proposed formulas survive increasingly precise experimental tests. \HYP

\subsection{Notation and classical correspondences}
Table~\ref{tab:classical_dict_p2} summarizes the key terms used in this paper
...
\begin{table}[h]
  ...
  \begin{tabular}{llll}
    ...
    \midrule
    Cubic ledger & 3-cube combinatorics & Bridge & $V{=}8$, $E{=}12$, $F{=}6$, $S{=}24$ \\
    $|V_{cb}|=1/24$ & CKM 2--3 mixing & Bridge & Slot normalization \\
    $|V_{us}|=\phig^{-3}$ & Cabibbo mixing & Bridge & Ladder exponent \\
    $\sin^2\theta_{13}=\phig^{-8}$ & PMNS reactor angle & Bridge & Octave-forced \\
    $\alpha$-corrections & radiative corrections & Twin & QED coupling \\
    Jarlskog $J$ & CP-violation invariant & Twin & Rephasing-invariant \\
    \bottomrule
  \end{tabular}
...
\section{Conclusions}
This paper proposed a structural, no-per-channel-tuning account of flavor mixing based on cubic ledger topology.
The central modeling premise is that a small set of cube-derived normalizations and coefficients, together with the shared constants \(\phig\) and \(\alpha\),
organize both CKM and PMNS mixing in closed form. \HYP

\subsection{What is structural vs what is hypothesized}
\paragraph{Structural (combinatorics).}
The cube counts \(V=8\), \(E=12\), \(F=6\), and the vertex--edge slot count \(S=24\) are fixed combinatorics. \PROVED
Several coefficients used in the correction layer are fixed by these counts (e.g.\ \(C_{\mathrm{atm}}=F\), \(C_{\mathrm{Cab}}=F/4\)). \PROVED

\paragraph{Model hypotheses (physics).}
The identification of specific CKM/PMNS observables with particular ledger normalizations and $\phig$-powers is a falsifiable hypothesis:
\(|V_{cb}|=1/24\), \(|V_{ub}|=\alpha/2\), \(\sin^2\theta_{13}=\phig^{-8}\), and the $\alpha$-correction forms are not derived from the SM but proposed as structural laws. \HYP

\subsection{What the validation indicates (and what is tense)}
The PDG comparison indicates that the CKM hypotheses have the correct magnitude hierarchy and that the corrected Cabibbo expression
\(|V_{us}|_{\mathrm{pred,corr}}=\phig^{-3}-\tfrac32\alpha\) is strongly preferred over the leading-order \(\phig^{-3}\) value. \VAL
The CKM CP-violation scale obtained from the same ingredients,
\(J_{\mathrm{CKM}}^{\mathrm{pred}}=|V_{us}||V_{cb}||V_{ub}|\),
also lands at the correct order of magnitude. \VAL

For PMNS, \(\sin^2\theta_{13}\) and \(\sin^2\theta_{12}\) are in reasonable agreement with current NuFIT summaries at present uncertainties. \VAL
The main near-term risk is the atmospheric angle: the hypothesis \(\sin^2\theta_{23}=1/2+6\alpha\) predicts the upper octant and is therefore sharply falsifiable
if future global fits converge decisively to the lower octant. \VAL

\subsection{Falsifiers and next steps}
The core falsifiers are:
\begin{itemize}
  \item CKM: failure of \(|V_{cb}|\) to remain consistent with the slot normalization \(1/24\) as uncertainties tighten, \VAL
  \item CKM: inconsistency of the Jarlskog magnitude with the predicted scale from Eq.~\eqref{eq:Jckm_pred_closed} (or its corrected Cabibbo variant), \VAL
  \item PMNS: decisive confirmation of a lower-octant \(\theta_{23}\) incompatible with Eq.~\eqref{eq:pmns_theta23_pred}. \VAL
\end{itemize}
The remaining tasks for completing this paper are to consolidate the numerical targets and uncertainties in a compact table,
and to record the exact audited fields used for comparisons in Appendix~A. \PROVED

\appendix

\section{Certificate Fields (for auditing)}
This appendix records a compact set of audit fields used to check the paper’s claims.
Each field is listed with: (i) the closed-form definition in the paper, and (ii) the external comparison target and tolerance (PDG/NuFIT),
where applicable.
The purpose is to make the validation layer auditable without re-reading the entire text. \PROVED

\subsection*{A.1 CKM audit fields (PDG)}
\begin{itemize}
  \item \texttt{vcb\_geometric\_origin}: the CKM 2--3 mixing hypothesis
  \( |V_{cb}|_{\mathrm{pred}} := 1/S = 1/24 \) (Eq.~\eqref{eq:Vcb_pred}). \HYP

  \item \texttt{vub\_origin}: the CKM 1--3 mixing hypothesis
  \( |V_{ub}|_{\mathrm{pred}} := \alpha/2 \) (Eq.~\eqref{eq:Vub_pred}). \HYP

  \item \texttt{vus\_leading}: the leading Cabibbo hypothesis
  \( |V_{us}|_{\mathrm{pred}} := \phig^{-3} \) (Eq.~\eqref{eq:Vus_pred}). \HYP

  \item \texttt{vus\_corrected}: the corrected Cabibbo hypothesis
  \( |V_{us}|_{\mathrm{pred,corr}} := \phig^{-3} - \tfrac{3}{2}\alpha \) (Eq.~\eqref{eq:Vus_pred_corr}). \HYP

  \item \texttt{vcb\_match} (validation): compare \(|V_{cb}|_{\mathrm{pred}}\) to a PDG reference value
  (see Sec.~7; PDG summary \cite{PDG2024}). \VAL

  \item \texttt{vub\_match} (validation): compare \(|V_{ub}|_{\mathrm{pred}}\) to a PDG reference value
  (see Sec.~7; PDG summary \cite{PDG2024}). \VAL

  \item \texttt{vus\_match} (validation): compare either the leading or corrected Cabibbo form to a PDG reference value
  (see Sec.~7; PDG summary \cite{PDG2024}). \VAL
\end{itemize}

\subsection*{A.2 PMNS audit fields (NuFIT)}
\begin{itemize}
  \item \texttt{theta13\_pred}: \( \sin^2\theta_{13}^{\mathrm{pred}} := \phig^{-8} \) (Eq.~\eqref{eq:pmns_theta13_pred}). \HYP
  \item \texttt{theta12\_pred}: \( \sin^2\theta_{12}^{\mathrm{pred}} := \phig^{-2} - 10\alpha \) (Eq.~\eqref{eq:pmns_theta12_pred}). \HYP
  \item \texttt{theta23\_pred}: \( \sin^2\theta_{23}^{\mathrm{pred}} := \tfrac12 + 6\alpha \) (Eq.~\eqref{eq:pmns_theta23_pred}). \HYP

  \item \texttt{theta13\_match} (validation): compare \(\sin^2\theta_{13}^{\mathrm{pred}}\) to a NuFIT reference value
  for a stated ordering and release (Sec.~7; NuFIT \cite{NuFIT}). \VAL

  \item \texttt{theta12\_match} (validation): compare \(\sin^2\theta_{12}^{\mathrm{pred}}\) to a NuFIT reference value
  (Sec.~7; NuFIT \cite{NuFIT}). \VAL

  \item \texttt{theta23\_match} (validation): compare \(\sin^2\theta_{23}^{\mathrm{pred}}\) to a NuFIT reference value,
  with the octant convention stated explicitly (Sec.~7; NuFIT \cite{NuFIT}). \VAL
\end{itemize}

\subsection*{A.3 CP-violation audit fields}
\begin{itemize}
  \item \texttt{jarlskog\_definition}: \( J(W) := \left|\operatorname{Im}(W_{11}W_{22}W_{12}^\ast W_{21}^\ast)\right| \)
  (Eq.~\eqref{eq:J_def}). \PROVED

  \item \texttt{jarlskog\_ckm\_pred}: \( J_{\mathrm{CKM}}^{\mathrm{pred}} := |V_{us}|_{\mathrm{pred}}|V_{cb}|_{\mathrm{pred}}|V_{ub}|_{\mathrm{pred}} \)
  (Eq.~\eqref{eq:Jckm_pred_closed}). \HYP

  \item \texttt{jarlskog\_ckm\_pred\_corr} (optional): the same product using \(|V_{us}|_{\mathrm{pred,corr}}\)
  (Eq.~\eqref{eq:Jckm_pred_corr}). \HYP

  \item \texttt{jarlskog\_match} (validation): compare the predicted CKM Jarlskog magnitude to a PDG reference value
  (Sec.~7; PDG summary \cite{PDG2024}). \VAL
\end{itemize}

\section{Reproducibility}
This paper contains no fitted parameters and no per-channel adjustments.
All numerical values used in Sec.~7 are obtained by evaluating the closed-form expressions in Secs.~3--6 under the explicitly pinned constant
\(\alpha^{-1}=137.036\) (Eq.~\eqref{eq:alpha_pin_p2}). \CERT

\subsection{Reproducing the numerical evaluations}
From the repository root, the following one-shot calculation reproduces the headline predictions used in the PDG/NuFIT comparison:
\begin{verbatim}
python3 - <<'PY'
import math
phi = (1 + math.sqrt(5))/2
...
print("|Vub|", Vub)
print("|Vus|", Vus)
print("|Vus_corr|", Vus_corr)
print("sin^2 th13", th13)
print("sin^2 th12", th12)
print("sin^2 th23", th23)
print("J_ckm", J)
print("J_ckm_corr", Jc)
PY
\end{verbatim}
Reference targets and uncertainty windows should be taken from the stated external summaries (PDG, NuFIT) at the cited releases. \VAL

\subsection{Compiling the PDF}
Compile Paper~2 from \texttt{papers/tex/}:
\begin{verbatim}
cd papers/tex
pdflatex -interaction=nonstopmode -output-directory=../pdf masses_paper2_mixing.tex
pdflatex -interaction=nonstopmode -output-directory=../pdf masses_paper2_mixing.tex
\end{verbatim}
The resulting PDF is written to \texttt{papers/pdf/masses\_paper2\_mixing.pdf}. \PROVED
