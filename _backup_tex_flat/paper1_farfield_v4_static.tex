...
For referee convenience, we also include independent rigorous ball-arithmetic artifacts on representative low-height rectangles in the repository (not used in the proof).
\end{abstract}
...
The repository also contains rigorous ball-arithmetic rectangle checks and finite Pick artifacts on low-height regions; these are included as independent numerical corroboration but are not used in the all-heights proof.
...
\textcolor{blue}{Note that $\xi$ has the forced zeros at $s=0$ and $s=1$ coming from the prefactor $s(s-1)$; these do not correspond to zeros of $\zeta$. In this paper all ``zeros'' refer to zeros of $\zeta$ in $\Omega$.}
...
\end{theorem}
\begin{proof}
This is proved by a quantitative phase--velocity identity for $\Arg \mathcal J_{\rm out}$ together with a Cauchy--Riemann/Green pairing on Whitney boxes and an unconditional Carleson energy bound.
For a complete proof (including the explicit Carleson bound $C_{\rm box}^{(\zeta)}<\infty$ and the quantitative wedge criterion), see the repository manuscript \texttt{Riemann-active.txt} (Section ``Carleson energy and boundary BMO (unconditional)'' and the theorem ``Boundary wedge from the product certificate (atom-safe)'').
\end{proof}
...
The artifacts in Table~\ref{tab:artifact-data} are generated by the repository verifier \texttt{verify\_attachment\_arb.py} using ARB ball arithmetic (via \texttt{python-flint}).
...
All computational artifacts used for optional cross-checks are included in the repository:
