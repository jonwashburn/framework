...
We introduce four independently motivated ``gates''---interaction, entanglement, curvature, and d'Alembert structure---that separate the canonical cost $J(x)=\tfrac12(x+x^{-1})-1$ from this counterexample family. The fourth gate (d'Alembert structure) is the keystone: it requires that the shifted log-lift $H=G+1$ satisfies the classical d'Alembert functional equation $H(t+u)+H(t-u)=2H(t)H(u)$, which combined with calibration forces the unique solution $G=\cosh-1$.

We prove that under the four gates, full inevitability follows unconditionally: $F=J$ and $P(u,v)=2uv+2u+2v$ on $[0,\infty)^2$, with \emph{no} regularity assumptions on $P$. Core results are machine-verified in Lean 4.
\end{abstract}
...
This paper takes a different approach. Instead of assuming properties of $P$, we identify four \emph{minimal requirements}---gates that a meaningful comparison law must satisfy. Each gate is independently motivated, and each is satisfied by the canonical cost while violated by the counterexample.
...
4 & d'Alembert & Shifted log-lift satisfies $H(t+u)+H(t-u)=2H(t)H(u)$ \\
\bottomrule
\end{tabular}
}
\end{center}

The power of this approach is \emph{quadrilateral convergence}: four independent lines of reasoning converge on the same target structure. The first three gates are necessary but insufficient; the fourth gate (d'Alembert structure) completes the chain by pinning down the exact coefficient in the ODE.
...
\subsection{Why Three Gates Are Insufficient}

The first three gates---interaction, entanglement, and curvature---distinguish $\Jcost$ from the quadratic counterexample. However, they are \emph{insufficient} to uniquely determine the RCL.

The problem is that the structural axioms + interaction allow a \emph{family} of solutions parameterized by a coefficient $a > 0$:
\begin{itemize}
\item $a = 0$: Flat ODE $G'' = 1$ (ruled out by interaction)
\item $a = 2$: Hyperbolic ODE $G'' = G + 1$ (the RCL)
\item $a = 8$: ODE $G'' = 4G + 1$ (also has interaction, but different structure)
\end{itemize}

Interaction only rules out $a = 0$; it does not force $a = 2$. The fourth gate closes this gap.
...
The d'Alembert gate is the \emph{keystone} of the inevitability argument. While gates 1--3 distinguish $\Jcost$ from the counterexample, they do not uniquely determine the RCL because there is a family of valid solutions. Gate 4 pins down the \emph{exact coefficient} by requiring the classical d'Alembert structure.
...
With the fourth gate, the proof is now complete. No hypothesis remains:

\begin{enumerate}
\item \textbf{d'Alembert structure} (Gate 4 assumption)
\item \textbf{d'Alembert + calibration $\Rightarrow$ $G = \cosh - 1$} (Theorem~\ref{thm:dAlembert-forces-cosh}---machine-verified)
\item \textbf{$G = \cosh - 1$ $\Rightarrow$ $F = \Jcost$} (by definition of log-lift)
\item \textbf{$F = \Jcost$ $\Rightarrow$ $P = $ RCL} (Theorem~\ref{thm:P_forced_from_J}---machine-verified with \emph{no} assumptions on $P$)
\end{enumerate}

Every step is either definitional or machine-verified. The fourth gate is not a hypothesis to be proved---it is an independently motivated physical/mathematical requirement, just like the first three gates.
...
\node[gate, below right=1.8cm and 1.5cm of conclusion] (gate4) {Gate 4\\d'Alembert};

\draw[->, thick] (gate1) -- (conclusion);
\draw[->, thick] (gate2) -- (conclusion);
\draw[->, thick] (gate3) -- (conclusion);
\draw[->, thick, MidnightBlue] (gate4) -- (conclusion);
...
Each gate excludes the quadratic-log counterexample family. Gates 1--3 are necessary but insufficient; \textbf{Gate 4 (d'Alembert structure)} completes the chain by pinning down the exact coefficient $\lambda = 1$ in $H = \cosh(\lambda t)$.
...
With the fourth gate (d'Alembert structure), the inevitability theorem is now fully machine-verified with no remaining hypotheses. The only axiom used is the classical d'Alembert classification theorem (Acz\'{e}l's theorem), which is a standard result in functional equation theory.
...
IndisputableMonolith/Foundation/DAlembert/
  ...
  FourthGate.lean           -- Gate 4: d'Alembert structure
...
4 & d'Alembert structure & \checkmark & $\times$ \\
\bottomrule
\end{tabular}
}
\end{center}

The four-gate structure provides robust evidence: four independent lines of reasoning, four independent motivations, all converging on:
\[
F(x) = \frac{1}{2}\left(x + \frac{1}{x}\right) - 1, \qquad P(u,v) = 2uv + 2u + 2v.
\]

An alternative combiner compatible with multiplicative consistency must violate at least one of the four gate conditions. The counterexample shows this is possible, but requires abandoning interaction, entanglement, hyperbolic curvature, and d'Alembert structure simultaneously.

Under the four gates, the Recognition Composition Law is the \emph{unique} combiner compatible with multiplicative consistency. No further hypothesis is required.
