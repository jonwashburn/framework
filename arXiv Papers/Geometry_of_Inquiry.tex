...
We emphasize:
\begin{itemize}
    \item The d'Alembert law is a \emph{postulate}. The framework shows what follows, not why this postulate is necessary.
...
\begin{enumerate}
    \item \texttt{Jcost\_dalembert}: d'Alembert law verification.
...
\begin{itemize}
    ...
    \item The d'Alembert law uniquely determines $\Jcost(x) = \frac{1}{2}(x + 1/x) - 1$.
...
\begin{itemize}
    \item The d'Alembert law is postulated, not derived.
...
\end{align}
These are equal. The d'Alembert law for $\Jcost = g - 1$ follows by algebra.
\end{proof}
