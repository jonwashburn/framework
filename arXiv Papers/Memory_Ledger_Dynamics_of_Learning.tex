...
Our starting point is Recognition Science (RS), a theoretical framework that derives physics from a single primitive: the d'Alembert composition law for a cost functional $\Jcost$ \cite{washburn2024rs}. This law uniquely determines:
\begin{equation}
...
\begin{definition}[d'Alembert Composition Law]
A cost functional $F: \mathbb{R}_+ \to \mathbb{R}$ satisfies the d'Alembert composition law if:
\begin{equation}
...
\item $\Jcost(x) = \frac{1}{2}(x + x^{-1}) - 1$: Unique solution to d'Alembert law
