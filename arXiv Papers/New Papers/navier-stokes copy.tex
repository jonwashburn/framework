...
\[
\mathcal{W}(x,t;r):=\frac{1}{r}\iint_{Q_r(x,t)} |\omega|^{3/2}\,dx\,ds,\qquad
...
\item \textbf{Threshold constants and safe window.} Let \(\varepsilon_A>0\) be the absorption threshold in Lemma~A (Section~2). Let \(C_B\) be the constant in the \(L^{3/2}\!\to BMO^{-1}\) slice bridge (Lemma~B, Section~3), as proved under the square--Carleson hypothesis (C2S$^{\square}$). Define the working threshold
\[
\varepsilon_0:=\min\Big\{\varepsilon_A,\ \big(\varepsilon_{\mathrm{SD}}/C_B\big)^{3/2}\Big\}.
\]
Under (C2S$^{\square}$), the \emph{safe-window criterion} is: if, on a unit window $[t_0-1,t_0]$, one has
\[
\sup_{x\in\mathbb{R}^3,\ r>0}\ \frac{1}{r}\int_{t_0-r^2}^{t_0}\!\Big(\int_{B_r(x)} |\omega|^{3/2}\Big)^{\!4/3} ds\ \le\ \varepsilon_0^{4/3},
\]
...
then
\[
\sup_{(x,t)\in\mathbb{R}^3\times[t_0-1,t_0]}\ \sup_{r>0}\ \frac{1}{r}\int_{t-r^2}^{t}\!\Big(\int_{B_r(x)} |\omega|^{3/2}\Big)^{\!4/3} ds\ \le\ C\,\varepsilon^{4/3},
\]
...
\[
\mathcal{W}(x,t;r):=\frac{1}{r}\iint_{Q_r(x,t)}|\omega|^{3/2}\,dx\,ds,\qquad \omega=\nabla\times u,
\]
...
\[
\mathcal{W}(x_0,t_0;r):=\frac1r\iint_{Q_r(x_0,t_0)} \theta^{3/2}\,dx\,dt.
\]

\begin{lemma}[Lemma A: critical $\varepsilon$--regularity (uses (SA))]\label{lem:A}
There exist absolute constants $\varepsilon_A>0$ and $C_A<\infty$ such that if the slice-controlled absorption hypothesis (SA) holds on $Q_{r_0}(x_0,t_0)$,
\[
\sup_{s\in[t_0-r_0^2,t_0]}\ \|\omega(\cdot,s)\|_{L^{3/2}(B_{2r_0}(x_0))}\ \le\ \varepsilon_A,
\]
and
\[
...
\begin{proof}
\emph{Caccioppoli--De~Giorgi under (SA).} By scaling, reduce to $r_0=1$, $(x_0,t_0)=(0,0)$ and write $Q_1=B_1\times[-1,0]$. The slice hypothesis (SA) yields
\[
\sup_{s\in[-1,0]}\ \|\omega(\cdot,s)\|_{L^{3/2}(B_2)}\ \le\ \varepsilon_A,
\]
and by assumption $\mathcal{W}(0,0;1)\le\varepsilon_A$. Let $\theta=|\omega|$ and set $w:=(\theta-\kappa_0)_+$ with $\kappa_0=K_0\,\mathcal{W}(0,0;1)^{2/3}$ and universal $K_0\ge1$. Choose the cutoff chain and exponent ladder as in Appendix~A.

Under (SA), the vortex--stretching term is absorbed in the Caccioppoli inequality (see \eqref{A:caccioppoli-absorbed}). The De~Giorgi iteration on shrinking cylinders (Appendix~A.4--A.5) then yields a uniform bound for $w$ and, in particular,
\[
...
\paragraph{Remarks.}
(1) The absorption of stretching uses (SA): a uniform slice bound $\sup_{s\in[t_0-r_0^2,t_0]}\|\omega(\cdot,s)\|_{L^{3/2}(B_{2r_0})}\le\varepsilon_A$. No smallness of the drift $u$ is needed; $\operatorname{div}u=0$ moves advection entirely onto the cutoff, and the iteration tolerates the resulting lower–order contribution.
...
\[
\mathcal{W}(x,t;r):=\frac1r\iint_{Q_r(x,t)}|\omega|^{3/2},\qquad Q_r(x,t)=B_r(x)\times[t-r^2,t],
\]
...
\[
\sup_{(x,t)\in\mathbb{R}^3\times[-1,0]}\ \sup_{r>0}\ \frac{1}{r}\int_{t-r^2}^{t}\!\Big(\int_{B_r(x)} |\omega|^{3/2}\Big)^{\!4/3}\! ds\ \le\ \varepsilon^{4/3}.
\]
...
\;\le\; C\,\varepsilon^{4/3}\,r^{-5/3}\sum_{k\ge0}2^{-k/3}
\;\le\; C\,\varepsilon^{4/3}.
\]
This bound is uniform in $(t,x,r)$.

\medskip
\emph{Step 3 (Nonlinear piece via heat smoothing and $L^3$ control).}
By the $L^{3/2}\!\to L^2$ smoothing of the heat semigroup and boundedness of $\mathbb{P}$,
\[
\|e^{\nu(\tau-s)\Delta}\mathbb{P}\nabla\!\cdot F\|_{L^2(\mathbb{R}^3)} \;\le\; C\,(\nu(\tau-s))^{-3/4}\,\|F\|_{L^{3/2}(\mathbb{R}^3)}\qquad(0<s<\tau).
\]
With $F=u\otimes u$ we have $\|F\|_{L^{3/2}}\le \|u\|_{L^3}^2$. Repeating the dyadic near/far estimate \eqref{eq:BS} on each time slice,
\[
\|u(\cdot,t+s)\|_{L^3(B_{2^{k+1}r})}\ \le\ C\sum_{m\ge0}2^{-m}\|\omega(\cdot,t+s)\|_{L^{3/2}(B_{2^{m+k+2}r})},
\]
so $\|u\otimes u\|_{L^{3/2}}$ is controlled by the \emph{square} of the right–hand side. Using the convolution kernel $(\tau-s)^{-3/4}\mathbf{1}_{0<s<\tau}$ in Young's inequality on $s\in(0,\tau)$, and then the same critical interpolation as in Step~2 (now applied to $\|\omega\|_{L^{3/2}}^2$ on $B_{2^{m+k+2}r}$ over time intervals of length $(2^{m+k+2}r)^2$), one obtains
\[
\mathbf{E}[N](t;x,r)\ \le\ C\,\varepsilon^{4/3},
\]
uniformly in $(t,x,r)$. (The only inputs are the $L^{3/2}\!\to L^2$ heat gain, dyadic Biot–Savart, and the uniform bound $\sup_{(x,s),\rho}\mathcal{W}\le\varepsilon$.)

\medskip
\emph{Step 4 (Good time selection and conclusion).}
By \eqref{eq:duhamel} and the triangle inequality for $\mathbf{E}[\cdot]^{1/2}$,
\[
\mathbf{E}[e^{\nu\cdot\Delta}u(t)](x,r)^{1/2}\ \le\ \mathbf{E}[L](t;x,r)^{1/2}+\mathbf{E}[N](t;x,r)^{1/2}.
\]
Integrating over $t\in[-\tfrac12,0]$ and using the uniform bounds from Steps~2–3,
\[
\int_{-1/2}^{0}\mathbf{E}[e^{\nu\cdot\Delta}u(t)](x,r)\,dt\ \le\ C\,\varepsilon^{4/3}.
\]
Therefore there exists $t_*\in[-\tfrac12,0]$ such that
\[
\mathbf{E}[e^{\nu\cdot\Delta}u(t_*)](x,r)\ \le\ C\,\varepsilon^{4/3}\qquad\text{for all }(x,r).
\]
Taking the supremum over $(x,r)$ and the square root exactly gives
\[
\|u(\cdot,t_*)\|_{BMO^{-1}} \;\le\; C_B\,\varepsilon^{2/3},
\]
with $C_B:=\sqrt{C}$. This is the claimed estimate, and $t_*\in[-\tfrac12,0]$.

In particular, the bound is \emph{uniform} over all $(x,r)$ and depends only on $\sup_{(x,t),r}\mathcal{W}(x,t;r)$ over $[-1,0]$.
\end{proof}

(References for Section 3: Biot–Savart and dyadic split \cite{MajdaBertozzi2002,Stein1993}; heat-kernel smoothing and time convolution are standard; the Carleson/tent-space perspective is from \cite{KochTataru2001}.)
\paragraph{Remarks.}
...
\[
\mathcal{W}(x,t;r):=\frac{1}{r}\iint_{Q_r(x,t)}|\omega|^{3/2}\,dx\,ds,
\qquad
\mathcal{M}(t):=\sup_{x\in\mathbb{R}^3,\ r>0}\mathcal{W}(x,t;r),
\]
...
\[
\mathcal C_{U}(s):= \sup_{x\in\mathbb{R}^3,\ r>0}\ \frac{1}{r}\int_{s-r^2}^{s}\!\Big(\int_{B_r(x)} |\Omega(y,\tau)|^{3/2}\,dy\Big)^{\!4/3} d\tau.
\]
...
\]
Divide by $r$ and take the supremum over $(x,r)$; then take $\liminf_{n}$ to obtain the claim.
\end{proof}

\begin{lemma}[Unit-scale bound for $\mathcal W$ from $\mathcal C$]\label{lem:W-from-C}
For every $s$ and every $x\in\mathbb{R}^3$, if
\[
\frac{1}{1}\int_{s-1}^{s}\!\Big(\int_{B_1(x)} |\omega|^{3/2}\Big)^{\!4/3} d\tau\ \le\ \Lambda^{4/3},
\]
then
\[
...
\begin{proof}
Since $\nabla U^{(n)}=\mathcal{R}\,\Omega^{(n)}$ (Riesz transform matrix), boundedness $\Omega^{(n)}\in L^{3/2}$ on fixed cylinders implies $\nabla U^{(n)}\in L^{3/2}$ there; the strong $L^3$ convergence of $U^{(n)}$ then yields weak convergence $\Omega^{(n)}\rightharpoonup \Omega$ in $L^{3/2}$ on each fixed cylinder. The map $f\mapsto\int|f|^{3/2}$ is convex on $L^{3/2}$, hence weakly lower semicontinuous, proving the first claim. Taking suprema gives the second.
\end{proof}
...
\[
\mathcal{W}(0,0;r)=\frac1r\iint_{Q_r}\theta^{3/2}.
\]
...
\[
\sup_{r\in(0,2]}\ \frac{1}{r}\int_{-1}^{0}\!\Big(\int_{B_r} |\omega|^{3/2}\Big)^{\!4/3}\! ds\ \le\ \varepsilon_0^{4/3},
\]
and
\[
...
Note that $w=(\theta-\kappa_0)_+\le \theta$ and hence $\iint_{Q_1} w^{3/2}\le \iint_{Q_1}\theta^{3/2}$. Using the hypothesis $\iint_{Q_1}\theta^{3/2}\le \varepsilon_0+\eta$ together with \eqref{eq:thetaQv_simplified} and the definition of the critical functional (remember $\mathcal{W}(0,0;r)=r^{-1}\iint_{Q_r}\theta^{3/2}$), we obtain
\begin{equation}\label{eq:pre_density_drop}
\mathcal{W}(0,0;\vartheta)\ \le\ \underbrace{\frac{A}{\vartheta}\,\kappa_0^{3/2}|Q_\vartheta|}_{\text{baseline}}\ +\ \rho\,B\,\frac{1}{\vartheta}\,(\varepsilon_0+\eta).
\end{equation}
By scaling, $|Q_\vartheta|=\vartheta^5|Q_1|$ and $\kappa_0^{3/2}=K_0^{3/2}\varepsilon_0$. Thus the baseline term equals
\[
\frac{A}{\vartheta}\,\kappa_0^{3/2}|Q_\vartheta|\ =\ A\,K_0^{3/2}\,\vartheta^{4}\,\varepsilon_0.
\]
Fix $K_0$ \emph{large} so that $A\,K_0^{3/2}\,\vartheta^{4}\le \tfrac12$; with our choice $\vartheta=\tfrac14$ this is harmless (constants are universal), and gives
\[
\frac{A}{\vartheta}\,\kappa_0^{3/2}|Q_\vartheta|\ \le\ \tfrac12\,\varepsilon_0.
\]
...
\]
for some universal $\tilde B>0$ (absorbing $\vartheta^{-1}$). Increase $K_0$ further if needed so that $\tfrac12+\rho\,\tilde B\le 1$; this is possible because $\rho,\tilde B$ are fixed numbers, independent of $\varepsilon_0,\eta$. With this choice,
...
\paragraph{Remarks.}
(1) The proof is scale-free: the same argument at general radius $r$ gives the identical contraction on $Q_{\vartheta r}$, and dividing by $r$ preserves the functional $\mathcal{W}$.  
...
\[
\mathcal{C}(t)
:= \sup_{x\in\mathbb{R}^3,\ r>0}\ \frac{1}{r}\int_{t-r^2}^{t}\!\Big(\int_{B_r(x)} |\omega(y,s)|^{3/2}\,dy\Big)^{\!4/3} ds.
\]
...
\]
Undo the scaling to $(x_n,t_n;r_n)$ and divide by $\vartheta$ (definition of $\mathcal{W}$) to conclude that at the \emph{same} center $(x_n,t_n)$ one has, at the smaller radius $\vartheta r_n$,
\[
...
\[
\frac{1}{R}\int_{\tau-R^2}^{\tau}\!\Big(\int_{B_R(x)} |\omega|^{3/2}\Big)^{\!4/3}\! ds\ \le\ \varepsilon^{4/3}
...
\begin{aligned}
&\sup_{t}\int \eta^2 w^{p+1}\,dx\ +\ \frac{\nu}{4}\iint \eta^2\left|\nabla\!\left(w^{\frac{p+1}{2}}\right)\right|^2\,dx\,dt\\
&\qquad\le\ C\iint \Big(|\partial_t\eta|\,\eta+|\nabla\eta|^2\Big)\,w^{p+1}\,dx\,dt\ +\ C\,\varepsilon^{4/3}\,r_0^{1/3}\,\kappa_0^{p+1}\,|B_{r_0}|,
\end{aligned}
\end{equation}
...
\]
forces $w\equiv 0$ on $Q_{r_6/2}$ by \eqref{A:Linfty}. Returning to the original cylinder $Q_{r_0}(x_0,t_0)$ by parabolic scaling (recall that $|\omega|$ scales like $r_0^{-2}$ and $\|\theta\|_{L^{3/2}(Q_{r_0})}^{2/3}$ is invariant once divided by $r_0$) gives the pointwise estimate of Lemma~\ref{lem:A} with a constant $C_A$ depending only on the data fixed in \eqref{A:epsA}–\eqref{A:cutoff-bounds} and on the dimension.

\paragraph{Summary for implementation.}
\begin{itemize}
\item \emph{Absorption threshold (SA).} Fix $\varepsilon_A=\nu/(8C_{\mathrm{CZ}}C_S^2)$ (see \eqref{A:epsA}); whenever the slice smallness
\[
\sup_{s\in[t_0-r_0^2,\,t_0]}\ \|\omega(\cdot,s)\|_{L^{3/2}(B_{2r_0}(x_0))}\ \le \varepsilon_A
\]
holds on the working cylinder, the stretching term in \eqref{A:pre-cacc} is absorbed, yielding \eqref{A:caccioppoli-absorbed}.
...
\item \emph{Level choice.} The truncation level $\kappa_0=K_0\,\varepsilon^{2/3}$ (with $K_0$ universal and $\varepsilon:=\mathcal{W}(0,0;1)$) is used to quantify the De~Giorgi iteration baseline; absorption still relies on (SA).
\end{itemize}
...
\[
\frac{1}{r}\int_{I}\!\Big(\int_{B_r}\theta^{3/2}\Big)^{\!1+\sigma}\!dt\ \le\ C\,\varepsilon^{1+\sigma}.
\]
...
\]
Since $\sup\mathcal W\le\varepsilon$ on the unit window, $\iint_{B_R\times I}|\omega|^{3/2}\le C\,\varepsilon\,R$. Hence
\[
\int a(t)^2\,dt\ \lesssim\ \int_I\!\|\omega\|_{L^{3/2}(B_R)}^{2}\,dt\ \le\ C\,\varepsilon^{4/3}\,R^{1/3}\ \lesssim\ C\,\varepsilon^{4/3}.
\]
...
\[
\frac{1}{r}\int_{-r^2}^{0}\!F(t)\,dt\ \le\ C\,\varepsilon\qquad\text{and}\qquad \frac{1}{r}\int_{-r^2}^{0}\!F(t)^{1+\sigma_0}\,dt\ \le\ C\,\varepsilon^{1+\sigma_0}
\]
...
\[
\sup_{(x,t)\in\mathbb{R}^3\times[t_0-1,t_0]}\ \sup_{r>0}\ \mathcal{W}(x,t;r)\ \le\ \varepsilon\ \Longrightarrow\ \sup_{(x,t),r}\ \frac{1}{r}\int_{t-r^2}^{t}\!\Big(\int_{B_r(x)} |\omega|^{3/2}\Big)^{\!4/3}\!ds\ \le\ C\,\varepsilon^{4/3}.
\]
\end{theorem}
...
\[
h(s):=\int_{B_r(x)} |\omega(y,s)|^{3/2}\,dy,\qquad A_p:=\frac{1}{r}\int_I h(s)^p\,ds,\quad p\ge1.
\]
...
\]
Divide by $r$ and note $\int_I h= r A_1$, $\int_I h^{1+\sigma}= r A_{1+\sigma}$; the factor of $r$ cancels since $(1-\alpha)p+\alpha p/(1+\sigma)=1$. Hence
\[
...
\[
\frac{1}{r}\int_{t-r^2}^{t}\!\Big(\int_{B_r(x)} |\omega|^{3/2}\Big)^{\!4/3}\!ds\ \le\ C\,\varepsilon^{4/3}.
\]
...
(3) Absorption of vortex-stretching uses the \emph{slice} hypothesis (SA): a uniform bound $\sup_{s\in[t_0-r_0^2,t_0]}\|\omega(\cdot,s)\|_{L^{3/2}(B_{2r_0})}\le\varepsilon_A$ on the working cylinder. Smallness of the scale-invariant mass $\mathcal W$ is used to set the truncation level and close the De~Giorgi iteration; it does not by itself imply absorption.
...
\[
\mathcal{W}(x,t;r):=\frac{1}{r}\iint_{Q_r(x,t)}|\omega|^{3/2}\,,
