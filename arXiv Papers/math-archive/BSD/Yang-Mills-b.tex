...
\hypersetup{
  ...
  breaklinks=true,
  pdftitle={Yang--Mills Mass Gap: Unconditional Lattice Gap and a Conditional AF--free Continuum Construction},
...
% Title and authors
\title[Yang--Mills Mass Gap: Lattice Gap and Conditional AF--free Continuum]{\Large\textbf{Yang--Mills Mass Gap}\\[8pt]
\large Unconditional Lattice Gap and a Conditional AF--free Continuum Construction}
...
\smallskip
For the continuum, we give an AF--free norm--resolvent convergence (NRC) framework on fixed regions. The OS/GNS embedding and comparison identities are proved in-text, while the key quantitative inputs needed for operator-norm NRC (graph--defect and low--energy projector control) are isolated explicitly as Assumption~\ref{assump:U2b}. Consequently, the continuum mass-gap/persistence statements are conditional on the fixed-region U1/OS1 inputs and Assumption~\ref{assump:U2b} (while the lattice gap is unconditional), with constants tracked and volume-uniform on fixed slabs. An alternative Mosco/AF route is recorded in an appendix as a cross-check only and is not used in the main chain.
\end{abstract}
...
\vspace{8pt}
\begin{itemize}
  \item[\textbullet] $(\theta_*,t_0)$: interface smoothing constants (active closure: UCIS$_{\rm SW}$ along the scaling window \eqref{eq:ucis-sw-window}; uniform in $L$ and boundary data on fixed slabs)
...
\textit{(lattice strong-coupling gap unconditional; continuum construction conditional on UCIS$_{\rm SW}$ + fixed-region U1/OS1 + Assumption~\ref{assump:U2b})}
\end{center}
\vspace{10pt}
\begin{enumerate}
...
\begin{itemize}
  ...
  \item \textbf{AF--free NRC/persistence (conditional; U2b required)}: Assumption~\ref{assump:U2b}, Thm.~\ref{thm:strong-semigroup-core}, Prop.~\ref{prop:collective-compactness}, Thm.~\ref{thm:nrc-operator-norm}, Lem.~\ref{lem:af-free-cauchy}, Thm.~\ref{thm:gap-persist-cont}.
...
\begin{itemize}
  ...
  \item \textbf{AF--free NRC/persistence (fixed regions; conditional on U1/OS1 and U2b)}: Thms.~\ref{thm:U1-lsi-uei}, \ref{thm:os1-unconditional}; comparison identity Lem.~\ref{lem:U2-comparison}; quantitative NRC Thm.~\ref{thm:nrc-quant} and operator--norm NRC Thms.~\ref{thm:nrc-operator-norm}, \ref{thm:nrc-embeddings} (conditional on Assumption~\ref{assump:U2b}); one-point input Prop.~\ref{prop:one-point-resolvent} and uniqueness Thm.~\ref{thm:U2-nrc-unique}; compactness/core tools Thm.~\ref{thm:strong-semigroup-core}, Prop.~\ref{prop:collective-compactness}, Lem.~\ref{lem:af-free-cauchy}; and gap persistence Thm.~\ref{thm:gap-persist-cont}. (A calibrated fixed-slab comparison theorem with explicit constants is recorded as Thm.~\ref{thm:quant-calibrated-af-free-nrc}, but is not used in the main UCIS$_{\rm SW}$ chain.)
...
\begin{enumerate}
  ...
  \item \textbf{NRC quantitative inputs (RG-grade)}: Assumption~\ref{assump:U2b} (graph-defect + low-energy projector control for the OS/GNS embeddings).
  \item \textbf{Operator-norm NRC + uniqueness (conditional)}: Thm.~\ref{thm:nrc-operator-norm}, Thm.~\ref{thm:nrc-embeddings}, Lem.~\ref{lem:af-free-cauchy}, Thm.~\ref{thm:U2-nrc-unique}.
...
For quick verification against the Clay YM statement (honest conditional map):
\begin{itemize}
  \item \textbf{Existence (OS0--OS5; conditional):} Thm.~\ref{thm:uei-fixed-region} (OS0 on fixed regions), Prop.~\ref{prop:os0os2-closure} (OS0/OS2 closure), Thm.~\ref{thm:os1-unconditional} (OS1), Prop.~\ref{prop:os35-limit} (OS3/OS5 in the UCIS$_{\rm SW}$ route), and OS reconstruction to Wightman: Thm.~\ref{thm:os-to-wightman}.
  ...
  \item \textbf{Mass gap (continuum; conditional):} scaling-window slab contraction via UCIS$_{\rm SW}$ (Thm.~\ref{thm:ucis-sw}, Prop.~\ref{prop:os35-limit}) and AF--free NRC + gap persistence (conditional on Assumption~\ref{assump:U2b}; Thm.~\ref{thm:nrc-operator-norm}, Thm.~\ref{thm:gap-persist-cont}); global gap operator: Thm.~\ref{thm:global-gap-operator}.
...
\item \textbf{AF--free NRC to the continuum.} Prove operator-norm norm--resolvent convergence along van Hove sequences on fixed regions and persist the gap to the continuum generator (Thm.~\ref{thm:strong-semigroup-core}, Thm.~\ref{thm:nrc-operator-norm}, Thm.~\ref{thm:nrc-embeddings}, Lem.~\ref{lem:af-free-cauchy}, Thm.~\ref{thm:U2-nrc-unique}, Thm.~\ref{thm:gap-persist-cont}).
  ...
  \item \textbf{Conclusion.} Conclude $\operatorname{spec}(H)\subset\{0\}\cup[\gamma_*,\infty)$ with $\gamma_*>0$, uniform in $N\ge 2$ and uniform in $L$ on fixed physical slabs along admissible scaling-window schedules (and conditional on the explicit NRC inputs, Assumption~\ref{assump:U2b}).
\end{itemize}
\medskip
...
\begin{itemize}
  ...
  \item \textbf{AF–free NRC (existence/uniqueness).} On fixed regions: UEI/LSI (U1), OS/GNS embeddings and the comparison identities (U2a/U2c), and the quantitative operator-norm inputs isolated as Assumption~\ref{assump:U2b}; then the Cauchy resolvent criterion and holomorphic functional calculus yield uniqueness of local limits. Embedding and boundary independence and unitary equivalence hold.
...
\begin{theorem}[Single global Hamiltonian; exhaustion/schedule independence (conditional)]\label{thm:global-gap-operator}
Assume the AF--free NRC uniqueness/identification package on fixed regions (in particular Assumption~\ref{assump:U2b}, so that Theorem~\ref{thm:U2-nrc-unique} applies along the van Hove window). 
...
\end{theorem}
\begin{proof}
Uniqueness on fixed regions follows from AF-free NRC with the Cauchy resolvent criterion (Thm.~\ref{thm:U2-nrc-unique}, conditional on Assumption~\ref{assump:U2b}) and overlap consistency (Prop.~\ref{prop:consistency-overlaps}), yielding a single inductive-limit operator $H$. Unitary equivalence across admissible embeddings is Prop.~\ref{prop:unitary-equivalence} and Cor.~\ref{cor:scheme-independence-global}. Boundary/exhaustion independence is Prop.~\ref{prop:bc-robust-app}. Independence of the scaling schedule within the admissible class follows because the one-point resolvent convergence (Prop.~\ref{prop:one-point-resolvent}, conditional on Assumption~\ref{assump:U2b}) and the AF--free uniqueness theorem (Thm.~\ref{thm:U2-nrc-unique}) force the same operator-valued resolvent limit for any admissible schedule. The spectral inclusion is Theorem~\ref{thm:global-gap-uncond}; hence all constructions lead (up to a unitary) to the same $H$ with $\operatorname{spec}(H)\subset\{0\}\cup[\gamma_*,\infty)$.
\end{proof}
...
\]
\noindent\emph{Notational caution.} Do not confuse $\Delta_a^{\rm iso}$ with the generator defect $D_{a,L}:=H I_{a,L}-I_{a,L}H_{a,L}$ in Assumption~\ref{assump:U2b}.
...
This remark is bookkeeping for the \emph{optional} commutator cross-check route to OS1; it invokes the outline-only fixed-core sandwich input Theorem~\ref{TS:sandwich_main} and is not used in the main calibrator-based OS1 proof.
One may take
\[
  C_{\rm comm}(R,G)
  \ \le\ C_{\rm Strang}(R)\ +\ C_{\rm mag}(R,G)
\]
with $C_{\rm Strang}(R)$ the Strang remainder constant from the local sandwich (Theorem~\ref{TS:sandwich_main}, outline-only) and
\[
...
$C_{\rm gd},\,C_\Lambda$ & U2b defect/projector constants for operator-norm NRC; Assumption~\ref{assump:U2b}. \\
...
\begin{proposition}[Non-Gaussianity: nonzero truncated 4-point for local fields (conditional)]\label{prop:nonzero-cumulant4}
Assume the continuum local fields have been constructed on fixed regions and the local Schwinger limits are unique (e.g.\ Corollary~\ref{cor:unique-schwinger-local}, hence conditional on Assumption~\ref{assump:U2b}). Then there exist compactly supported smooth test functions $f_1,\ldots,f_4\in C_c^\infty(\mathbb R^4,\wedge^2\mathbb R^4)$, supported in a fixed bounded region $R\Subset\mathbb R^4$, such that the truncated 4-point function of the clover field $\Xi$ satisfies
\[
...
By Lemma~\ref{lem:local-fields-tempered} and Corollary~\ref{cor:unique-schwinger-local}, $\Xi_a(f)\to \Xi(f)$ in $L^2$ and joint moments converge along van Hove sequences (conditional on Assumption~\ref{assump:U2b}). Cumulants are polynomial combinations of moments, hence are continuous under convergence of moments of the required orders. Therefore, the nonzero truncated 4\,–\,point persists in the continuum limit:
\[
...
\begin{assumption}[U2b$_R$: quantitative NRC inputs on fixed regions (RG-grade)]\label{assump:U2bR}
Fix a bounded Lipschitz region $R\Subset \mathbb R^4$. Let $H_R\ge 0$ be the continuum generator on the OS/GNS space $\mathcal H_R$ and $H_{a,R}\ge 0$ the lattice generator on $\mathcal H_{a,R}$ (time-zero algebra supported in $R$), with the isometric embeddings $I_{a,R}$ of Lemma~\ref{lem:isometric-embeddings}. Set
\[
  P_{a,R}:=I_{a,R}I_{a,R}^*,\qquad D_{a,R}:=H_R I_{a,R}-I_{a,R}H_{a,R}.
\]
Assume:
\begin{itemize}
...
\end{itemize}
\end{assumption}
\noindent\emph{Remark (status).} This is a fixed-region analogue of the quantitative NRC input isolated globally as Assumption~\ref{assump:U2b}. Discharging it from in-text lattice Yang--Mills material requires a quantitative bridge lemma relating the OS/GNS embedding-side objects to a concrete comparison discretization where $O(a)$ defect bounds are proved (cf.\ the \emph{BLOCKER} remark after Assumption~\ref{assump:U2b}).

\begin{theorem}[Quantitative operator--norm resolvent comparison on fixed regions (conditional)]\label{thm:nrc-operator-norm-fixed}
Fix a bounded Lipschitz region $R\Subset\mathbb R^4$, $z\in\mathbb C\setminus\mathbb R$, and $\Lambda\ge 1$. Let $I_{a,R}$ be the isometric embeddings of Lemma~\ref{lem:isometric-embeddings} and set $P_{a,R}:=I_{a,R}I_{a,R}^*$ and $D_{a,R}:=H_R I_{a,R}-I_{a,R}H_{a,R}$. Assume Assumption~\ref{assump:U2bR}. Then there exists $C(z,\Lambda;R)<\infty$ independent of $(a,L)$ such that, uniformly in the lateral size $L$ along the scaling window,
\[
...
\]
using Assumption~\ref{assump:U2bR}(ii) and $\|R(z)\|=|\Im z|^{-1}$. For the defect term, use Assumption~\ref{assump:U2bR}(i) and Lemma~\ref{lem:weighted-resolvent} (applied to $H_{a,R}$):
\[
  \|R(z)\,D\,R_a(z)\,I_{a,R}^*\|
...
\begin{proof}
Set $B_a:=H_R - I_{a,R} H_{a,R} I_{a,R}^*$ on $\mathcal H_R$. By Assumption~\ref{assump:U2bR}, $\|B_a(H_R+1)^{-1/2}\|\le C_R a$. On the low-energy sector, $\|(H_R+1)^{1/2} R_H(z)\|$ and $\|(H_R+1)^{1/2} R_{H_{a,R}}(z)\|$ are bounded uniformly on a contour separating $[0,\Lambda]$ from $[\Lambda+\Delta,\infty)$; the second resolvent identity yields
\[
...
\begin{theorem}[Global OS3 (clustering) from a spectral gap (conditional)]\label{thm:global-os3-clustering}
Let $G$ be compact simple. Assume the hypotheses of Theorem~\ref{thm:global-gap-uncond} (in particular the scaling-window slab contraction input and Assumption~\ref{assump:U2b}). Then the global continuum Schwinger functions satisfy OS3 on $\mathbb R^4$ with exponential clustering rate at least the global Euclidean gap $\gamma_*>0$ from Theorem~\ref{thm:global-gap-uncond}. Precisely, for any gauge-invariant local observables $A,B$ with compact supports separated by Euclidean time $t\ge 0$,
\[
...
\begin{theorem}[Global OS0--OS5 with gap (conditional; any compact simple $G$)]\label{thm:global-OS0-5}
Assume the hypotheses of Theorem~\ref{thm:main-af-free} (in particular UCIS$_{\rm SW}$ under \eqref{eq:ucis-sw-window} and Assumption~\ref{assump:U2b}). Then the continuum Schwinger functions satisfy OS0--OS5 on $\mathbb R^4$, and the reconstructed generator $H\ge 0$ obeys
\[
...
\begin{theorem}[Global operator-norm NRC and projectors (conditional; see Clay pack)]\label{thm:nrc-global-Oa2}
Assume the hypotheses of Theorem~\ref{thm:main-af-free} (in particular Assumption~\ref{assump:U2b}). Then the global operator-norm NRC and the associated spectral projector convergence statements hold as recorded in Theorem~\ref{thm:global-nrc-clay}. No $O(a^2)$ rate is claimed in the main chain.
\end{theorem}
\begin{proof}
...
\begin{theorem}[\texorpdfstring{OS$\to$Wightman export (conditional)}{OS-to-Wightman export (conditional)}]\label{thm:os-wightman-export}
Assume the hypotheses of Theorem~\ref{thm:main-af-free} (in particular Assumption~\ref{assump:U2b}). Then the OS$\to$Wightman reconstruction yields a Poincar\'e covariant, local Wightman theory with Minkowski mass gap $\ge \gamma_*$ and nontrivial gauge-invariant local fields, as recorded in Theorem~\ref{thm:gap-wightman-clay} (see also Theorem~\ref{thm:os-to-wightman}).
\end{theorem}
\begin{proof}
See Theorem~\ref{thm:gap-wightman-clay} and Theorem~\ref{thm:os-to-wightman}.
\end{proof}

\begin{theorem}[Non-Gaussianity at the Wightman level (conditional)]\label{thm:wightman-nongaussian}
...
\begin{theorem}[Unitary uniqueness and independence (conditional)]\label{thm:global-independence}
Assume the hypotheses of Theorem~\ref{thm:main-af-free} (in particular Assumption~\ref{assump:U2b}). Then the resulting continuum OS/Wightman theory is independent (up to unitary equivalence) of the embedding scheme, van Hove exhaustion, and boundary conditions, as recorded in Theorem~\ref{thm:independence-clay}.
\end{theorem}
\begin{proof}
...
\subsection*{\texorpdfstring{AF--free: Time-Slice $O(a^2)$ Control and NRC (Auxiliary Outline)}{AF--free: Time-Slice O(a2) Control and NRC (Auxiliary Outline)}}
On fixed physical regions, one may derive norm--resolvent convergence from a short-time slice comparison; the commutator/resolvent hypotheses are proved in Lemma~\ref{lem:commutator-Oa2} and Corollary~\ref{cor:resolvent-commutator}. We record the resulting $O(a^2)$ time-slice comparison as an \emph{auxiliary outline} only (it is not used in the main AF--free NRC chain, which is routed instead through the comparison identity and the quantitative inputs isolated as Assumption~\ref{assump:U2b}).
\begin{itemize}
...
\begin{lemma}[Local centralization: single-link heat-kernel sandwich under near-identity staples (target)]\label{lem:single-link-heat-sandwich}
...
This last tail-to-operator-norm step is part of the present \emph{auxiliary outline}: it requires an explicit conversion of the small probability of $\mathcal B_{r_0}^c$ into an operator bound for the corresponding tail contribution on the local core (e.g. via a kernel regularity/Hilbert--Schmidt estimate on the fixed stencil), which we do not attempt to formalize here.
...
This fixed-core sandwich is \emph{not} used for the slab/interface contraction (the odd-cone cut gap and UCIS$_{\rm SW}$ bookkeeping use the interface heat--kernel convex split with explicit $(\theta_*,t_0)$). It \emph{is} invoked in the commutator route to OS1 on fixed regions (Lemma~\ref{lem:local-commutator-Oa2}) to justify an $O(a^2)$ Strang envelope on local cores.
It also provides a natural closure target for the remaining generator-match part of the U2b bridge: an $O(a^2)$ one-tick transfer match on a fixed local core implies an $O(a\,t)$ small-time semigroup match (Lemma~\ref{lem:bridge-semigroup-from-one-tick}), and hence the $O(a)$ generator-match bound required in (B2) of Lemma~\ref{lem:bridge-U2b-target} (Lemma~\ref{lem:bridge-generator-from-semigroup}).
As stated here it remains an auxiliary \emph{outline}; treat it as an explicit input until a full proof is written and audited.
\end{mdframed}
\begin{proof}
\emph{Step 1: Laplace representation for $\Re z > 0$.} For $w$ with $\Re w > 0$, the resolvent admits the representation
\[
...
\begin{lemma}[\boldmath Legacy coarse-refresh slab minorization on the coarse interface (target / proof sketch; not used in main chain)]\label{lem:beta-L-independent-minorization}
...
\end{lemma}
\begin{proof}[Proof sketch / status]
...
For concreteness we record a monotone scaling schedule $\beta(a)$ and prove tightness/precompactness of local Schwinger functions along any van Hove net with $a\downarrow 0$ and $L(a) a\to\infty$. Any \emph{uniqueness} (no subsequences) statement along such a schedule additionally requires an AF--free NRC/uniqueness input on fixed regions (e.g. Proposition~\ref{prop:af-free-uniqueness} or Theorem~\ref{thm:U2-nrc-unique}, conditional on Assumption~\ref{assump:U2b}).
...
If, in addition, the AF--free uniqueness criterion holds on $R$ (e.g.\ Proposition~\ref{prop:af-free-uniqueness}, or Theorem~\ref{thm:U2-nrc-unique} under Assumption~\ref{assump:U2b}), then all subsequential limits coincide, yielding pointwise convergence on $R$.
\end{proof}

\begin{corollary}[Convergence on $\mathbb R^4$ (conditional)]\label{cor:global-convergence}
Assume the hypotheses ensuring uniqueness of local limits on fixed regions (e.g.\ Proposition~\ref{prop:af-free-uniqueness} on each fixed region, or Theorem~\ref{thm:U2-nrc-unique} under Assumption~\ref{assump:U2b}). Then along $\beta(a)$, the global construction of Section~\ref{sec:global-R4} produces the same Schwinger functions as any other admissible monotone schedule satisfying the same fixed-region hypotheses. In particular, the global measure $\mu_{\mathrm{YM}}$ is independent of the schedule within this class.
\end{corollary}
...
\begin{theorem}[AF-free calibrated NRC and uniqueness (conditional; optional)]\label{thm:af-free-calibrated}
...
\begin{theorem}[Global Euclidean spectral gap, boundary/region independent (conditional; any compact simple $G$)]\label{thm:global-gap-uncond}
Let $G$ be a compact simple group. Assume the hypotheses of Theorem~\ref{thm:main-af-free} (in particular the scaling-window slab contraction input and Assumption~\ref{assump:U2b} for the operator-norm NRC step). With the global OS construction of Section~\ref{sec:global-R4}, there exists $\gamma_*>0$ (depending only on $(R_*,a_0,G)$ via $(\theta_*,t_0,\lambda_1)$) such that the Euclidean generator $H$ on the global OS/GNS Hilbert space satisfies
\[
...
Step 3 (continuum limit on fixed regions). On any fixed $R\Subset\mathbb R^4$, UEI and OS0 yield tightness and equicontinuity; under Assumption~\ref{assump:U2b} the AF--free NRC engine gives operator-norm resolvent convergence and uniqueness of local limits along van Hove nets (Theorems~\ref{thm:nrc-operator-norm}, \ref{thm:U2-nrc-unique}). Gap persistence (Theorem~\ref{thm:gap-persist-cont}) transports the uniform lower bound $\gamma_*$ from local lattices to the continuum generator $H_R$ on $R$.
...
All constants depend only on the slab geometry $(R_*,a_0)$ and group data; the conclusion uses the fixed-region inputs (in particular Assumption~\ref{assump:U2b}) and OS1 on fixed regions as stated earlier.
\end{proof}
...
\begin{theorem}[Clay--Jaffe--Witten compliance: existence and mass gap on $\mathbb R^4$ (conditional; any compact simple $G$)]\label{thm:clay-compliance}
Let $G$ be any compact simple Lie group. Assume the hypotheses of Theorem~\ref{thm:main-af-free} (in particular Assumption~\ref{assump:U2b} for the operator-norm NRC step), and assume the short-distance and stress-tensor packages cited below. Then the resulting continuum theory has the following properties:
\begin{itemize}
...
\end{itemize}
\end{lemma}
\begin{corollary}[Verification of OS0--OS5 assumptions in this manuscript (conditional)]\label{cor:os-assumptions-verified}
In the setting of this paper, the hypotheses (i)–(iii) of Lemma~\ref{lem:os-continuum} hold on fixed regions, uniformly along van Hove sequences, under the fixed-region inputs used in the continuum construction (in particular Assumption~\ref{assump:U2b}):
\begin{itemize}
...
\smallskip
\noindent\emph{Closure route J (smoothing/universality).} Alternatively, use the Track-J route on $R$: Lemmas~\ref{lem:U1-J1-smoothed-lsi} and \ref{lem:U1-J2-universality} hold (proved above via heat-kernel LSI tensorization and uniform convergence).
...
\end{lemma}
\begin{proof}
Work on a fixed gauge-invariant local core $\mathcal C_U\subset \mathcal V^{\rm loc}_{0,a,L}(R)$ that is dense in the time-zero local OS/GNS space. As an additional outline-only input (of the same “fixed-core Strang envelope” type discussed around Theorem~\ref{TS:sandwich_main}), assume that for $t\in[0,t_0]$ one has a Strang envelope
\[
...
\smallskip
\noindent\emph{Outline-only input.} The Strang-envelope identity with an $O(a^2 t)$ remainder is precisely the kind of time-continuous strengthening of the fixed-core sandwich discussed around Theorem~\ref{TS:sandwich_main}; we treat it here as part of the same optional outline and do not use this commutator route in the main OS1 closure.
...
\begin{proposition}[Collective compactness calibrator (conditional)]\label{prop:collective-compactness}
Assume Assumption~\ref{assump:U2b}. Fix $z_0\in\mathbb C\setminus\mathbb R$ and $\Lambda>0$. There exists a finite-rank operator $Q=Q(z_0,\Lambda)$ on $\mathcal H$ with $\|Q\|\le 1$ and spectral support in $E_H([0,\Lambda])$ such that for all large $k$,
\[
...
\begin{theorem}[NRC for all nonreal $z$ along a scaling sequence (conditional)]\label{thm:nrc-embeddings}
Let $\{\mu_{a,L}\}$ be the OS-positive Wilson lattice measures with one-tick transfer $T_{a,L}=e^{-aH_{a,L}}$ and OS/GNS Hilbert spaces $\mathcal H_{a,L}$. Assume UEI on fixed regions and locality as above, and assume Assumption~\ref{assump:U2b}. Then along any van Hove scaling sequence $(a_k,L_k)$ there exists a subsequence (not relabeled), a Hilbert space $\mathcal H$, and a positive self-adjoint $H\ge 0$ such that for every nonreal $z$,
\[
  \big\|(H-z)^{-1} - I_{a_k,L_k}\,(H_{a_k,L_k}-z)^{-1}\,I_{a_k,L_k}^*\big\|\;\xrightarrow[k\to\infty]{}\;0,
\]
...
Let $K\subset \mathbb C\setminus\mathbb R$ be compact. Suppose Assumption~\ref{assump:U2b} holds and that for some $z_0\in K$ the NRC estimate of Theorem~\ref{thm:nrc-quant} holds with rate $\le C(z_0)\,a$. Then there exists $C_K>0$ such that for all $z\in K$ and van Hove pairs $(a,L)$, $(a',L')$,
\[
...
\]
where $\Xi(z,w)=I+(z-w)\,I_{a'}R_{a'}(z)I_{a'}^*$ on the right and similarly bounded on the left. On $K$, resolvent norms are uniformly bounded by $\operatorname{dist}(K,\mathbb R)^{-1}$. Choosing $w=z_0$ and applying the one-point estimate of Theorem~\ref{thm:nrc-quant} at $z_0$ (under Assumption~\ref{assump:U2b}) yields the $O(a+a')$ bound at $z_0$. Uniform boundedness of the multipliers over $K$ transfers the Cauchy rate from $z_0$ to all $z\in K$ with a constant $C_K$.
\end{proof}
\end{lemma}
\begin{proof}[Proof sketch of Theorem~\ref{thm:nrc-embeddings}]
...
\emph{Quantitative defect/projector inputs.} Assumption~\ref{assump:U2b} supplies the graph-defect bound $\|(H+1)^{-1/2}D_{a,L}(H_{a,L}+1)^{-1/2}\|\le C_{\rm gd}a$ and the low-energy projection control $\|(I-P_{a,L})E_H([0,\Lambda])\|\le C_\Lambda a$ needed for operator-norm NRC on fixed regions.
...
\]
implies the one-point $O(a)$ estimate at $z=z_0$ by Theorem~\ref{thm:nrc-quant} (using Assumption~\ref{assump:U2b}); the bootstrap to all nonreal $z$ follows by the second resolvent identity as in Theorem~\ref{thm:U2-nrc-unique}.
...
\begin{theorem}[Quantitative operator-norm NRC for all nonreal $z$ (conditional)]\label{thm:nrc-quant}
Fix $z\in\mathbb C\setminus\mathbb R$ and $\Lambda>0$. Let $P_{a,L}:=I_{a,L}I_{a,L}^*$ and $D_{a,L}:=H I_{a,L}-I_{a,L}H_{a,L}$ as in Lemma~\ref{lem:U2-comparison}. Assume the quantitative U2b inputs below.
...
\end{theorem}
\begin{assumption}[U2b: quantitative NRC inputs for the OS/GNS embedding framework (RG-grade)]\label{assump:U2b}
With $P_{a,L}:=I_{a,L}I_{a,L}^*$ and $D_{a,L}:=H I_{a,L}-I_{a,L}H_{a,L}$:
\begin{itemize}
...
\end{itemize}
\end{assumption}
\noindent\emph{Remark (status/analogy).} For a fixed-slab discretization with the cell-averaging injection $J_a$, an energy-weighted generator defect bound and the corresponding low-energy spectral projector stability are proved in Proposition~\ref{lem:graph-defect-Oa} and Lemma~\ref{lem:low-energy-proj}. A comparison-side low-energy approximation for $J_a^*J_a$ is Lemma~\ref{lem:cell-avg-lowenergy}. Assumption~\ref{assump:U2b} is the corresponding quantitative input for the OS/GNS embeddings $I_{a,L}$ used in the AF--free continuum construction.
\noindent\emph{BLOCKER (what remains to prove U2b).} To discharge Assumption~\ref{assump:U2b} from in-text material, one needs a quantitative \emph{bridge lemma} identifying the OS/GNS embedding-side objects $(I_{a,L},H_{a,L},H)$ with a concrete discretization/comparison model where $O(a)$ defect/projector bounds are proved. The in-text target `lem:bridge-U2b-target` isolates the remaining RG-grade inputs as the quasi-unitary matching estimates (B1)--(B2) (the comparison low-energy piece is supplied by Lemma~\ref{lem:cell-avg-lowenergy}).
\begin{lemma}[Bridge lemma for Assumption~\ref{assump:U2b} (target; RG-grade)]\label{lem:bridge-U2b-target}
Fix a bounded region $R\Subset\mathbb R^4$ and a cutoff $\Lambda\ge 1$. Let $(\mathcal H^{\rm cmp}_a,H^{\rm cmp}_a,J_a)$ denote a concrete fixed-slab discretization/comparison model on $R$ (cell-averaging injection $J_a$) for which one has proved quantitative defect/projector bounds, e.g. Proposition~\ref{lem:graph-defect-Oa} and Lemma~\ref{lem:low-energy-proj}, together with the low-energy approximation property of $J_a^*J_a$ (Lemma~\ref{lem:cell-avg-lowenergy}). Assume moreover that $J_a$ is uniformly bounded in the corresponding graph norms: there exists $C_{\rm inj}<\infty$ such that for all sufficiently small $a$,
\[
...
\begin{itemize}
  ...
  \]
\end{itemize}
Then the embedding-side bounds in Assumption~\ref{assump:U2b} follow on $R$ (with constants inherited from the proved $J_a$-side bounds, Lemma~\ref{lem:cell-avg-lowenergy}, and the bridge constants in (B1)--(B2)).
\end{lemma}
\begin{proof}[Proof sketch]
...
\begin{itemize}
  ...
  A convenient CPM-style reformulation is to target instead a small-time semigroup match: by Lemma~\ref{lem:bridge-generator-from-semigroup}, it suffices to prove that for some $t_0>0$,
  \[
  ...
  In particular, a directly checkable “one-tick” target is an $O(a^2)$ intertwining estimate
  \[
...
\end{itemize}
\begin{lemma}[Tree-gauge $L^2$ density-isometry bridge criterion (target)]\label{lem:bridge-density-isometry-target}
...
%
%
\begin{lemma}[Density-isometry commutator identity and a checkable sufficient condition for the generator-match gate (B2) (target)]\label{lem:bridge-B2-density-derivatives}
...
\end{remark}
%
\begin{lemma}[Per-cell $O(a)$ consistency from primitive $C^2$ remainders (deterministic bookkeeping)]\label{lem:primitive-replacement-consistency}
Work on a fixed region $R\Subset\mathbb R^4$ in tree-gauge chord coordinates $u\in G^{m(R,a)}$ with product Haar $\pi^{\otimes m}$.
Assume there is a finite index set $\mathcal Q_a(R)$ of \emph{primitive interactions} and, for each $q\in\mathcal Q_a(R)$,
a stencil $S(q)\subset\{1,\dots,m(R,a)\}$ of uniformly bounded cardinality such that the two strictly positive densities $\rho_{a,L}$ and $\rho^{\rm cmp}_a$ admit factorizations on the cylinder core of the form
\[
  \rho_{a,L}(u)\ \propto\ \prod_{q\in\mathcal Q_a(R)} \Phi^{(1)}_{q,a,L}\!\big(u|_{S(q)}\big),
  \qquad
  \rho^{\rm cmp}_{a}(u)\ \propto\ \prod_{q\in\mathcal Q_a(R)} \Phi^{(2)}_{q,a}\!\big(u|_{S(q)}\big),
\]
with $\log\Phi^{(1)}_{q,a,L},\log\Phi^{(2)}_{q,a}$ of class $C^2$ in the stencil coordinates.

Let $\mathcal I_a(R)$ be any $a$-mesh cell index set in $R$ (so $|\mathcal I_a(R)|=O(a^{-4})$) and fix an assignment map
$\alpha:\mathcal Q_a(R)\to\mathcal I_a(R)$ attaching each primitive $q$ to a nearby cell $\alpha(q)$.
For each cell $\alpha\in\mathcal I_a(R)$ define $S(\alpha):=\bigcup_{q:\ \alpha(q)=\alpha} S(q)$ and define cell terms by
\[
  a^4\,\mathcal L^{(j)}_{\alpha,\bullet}\!\big(u|_{S(\alpha)}\big)
  \ :=\ -\sum_{q:\,\alpha(q)=\alpha}\log \Phi^{(j)}_{q,\bullet}\!\big(u|_{S(q)}\big),
  \qquad j\in\{1,2\}.
\]
Assume the primitive replacements satisfy a uniform first-order remainder bound for some $C_{\rm prim}(R)<\infty$:
for all $q,a,L$,
\[
  \big\|\log\Phi^{(1)}_{q,a,L}-\log\Phi^{(2)}_{q,a}\big\|_\infty
  +\max_{e\in S(q)}\Big(
  \big\|\nabla_e\big(\log\Phi^{(1)}_{q,a,L}-\log\Phi^{(2)}_{q,a}\big)\big\|_\infty
  +\big\|\Delta_e\big(\log\Phi^{(1)}_{q,a,L}-\log\Phi^{(2)}_{q,a}\big)\big\|_\infty
  \Big)\ \le\ C_{\rm prim}(R)\,a^5,
\]
and assume each cell contains at most $J_0(R)<\infty$ primitives:
\[
  \sup_{\alpha\in\mathcal I_a(R)} \#\{q\in\mathcal Q_a(R):\alpha(q)=\alpha\}\ \le\ J_0(R).
\]
Then the corresponding cell terms satisfy the per-cell first-order consistency bounds:
for every $\alpha$ and every $e\in S(\alpha)$,
\[
  \big\|\mathcal L^{(1)}_{\alpha,a,L}-\mathcal L^{(2)}_{\alpha,a}\big\|_\infty
  +\big\|\nabla_e\big(\mathcal L^{(1)}_{\alpha,a,L}-\mathcal L^{(2)}_{\alpha,a}\big)\big\|_\infty
  +\big\|\Delta_e\big(\mathcal L^{(1)}_{\alpha,a,L}-\mathcal L^{(2)}_{\alpha,a}\big)\big\|_\infty
  \ \le\ J_0(R)\,C_{\rm prim}(R)\,a.
\]
In particular, these hypotheses imply the per-cell $O(a)$ consistency input required in Lemma~\ref{lem:local-expansion-logr}.
\end{lemma}

\begin{proof}
Fix a cell $\alpha$. By definition,
\[
  \mathcal L^{(1)}_{\alpha,a,L}-\mathcal L^{(2)}_{\alpha,a}
  \ =\ \frac{1}{a^4}\sum_{q:\,\alpha(q)=\alpha}\Big(-\log\Phi^{(1)}_{q,a,L}+\log\Phi^{(2)}_{q,a}\Big).
\]
Taking $L^\infty$ norms and using the uniform primitive remainder bound gives
\[
  \big\|\mathcal L^{(1)}_{\alpha,a,L}-\mathcal L^{(2)}_{\alpha,a}\big\|_\infty
  \ \le\ \frac{1}{a^4}\sum_{q:\,\alpha(q)=\alpha}\big\|\log\Phi^{(1)}_{q,a,L}-\log\Phi^{(2)}_{q,a}\big\|_\infty
  \ \le\ \frac{J_0(R)}{a^4}\,C_{\rm prim}(R)\,a^5
  \ =\ J_0(R)\,C_{\rm prim}(R)\,a.
\]
The same estimate applies after applying $\nabla_e$ or $\Delta_e$ for any $e\in S(\alpha)$, since $\nabla_e$ and $\Delta_e$ are derivations
acting coordinatewise and only primitives with $e\in S(q)$ contribute; bounding by all primitives in the cell gives the stated bound.
\end{proof}
%
\begin{lemma}[Tick primitive $C^2$ remainder from calibrated quartic mismatch and commutator scaling (template)]\label{lem:tick-primitive-C2-a5}
Fix a primitive index $q$ and write $S(q)$ for its (finite) stencil.
Assume the primitive mismatch can be written in the form
\[
  \Delta_{q,a,L}(u)\ :=\ \log\Phi^{(1)}_{q,a,L}\!\big(u|_{S(q)}\big)-\log\Phi^{(2)}_{q,a}\!\big(u|_{S(q)}\big)
  \ =\ f_a\!\big(\Psi_{q,a}(u)\big)
\]
for a smooth map $\Psi_{q,a}:G^{|S(q)|}\to G$ (the ``tick holonomy'' map) and a smooth function $f_a:G\to\mathbb R$.
Assume that on a near-identity window $W_{q,a}\subset G^{|S(q)|}$ there is a normal neighborhood $\exp(B(0,r))\subset G$ such that $\Psi_{q,a}(W_{q,a})\subset \exp(B(0,r))$ and the following two bounds hold uniformly in $(q,L)$:
\begin{itemize}
  \item[(A)] \textbf{Calibrated quartic mismatch.} Writing $g=\exp X$ on $\exp(B(0,r))$, there exists $C_f<\infty$ such that for all $a$ and all $\|X\|\le r$,
  \[
    |f_a(\exp X)|\ \le\ C_f\,\beta(a)\,\|X\|^4,\qquad
    \|\nabla f_a(\exp X)\|\ \le\ C_f\,\beta(a)\,\|X\|^3,\qquad
    \|\nabla^2 f_a(\exp X)\|\ \le\ C_f\,\beta(a)\,\|X\|^2,
  \]
  where $\nabla,\nabla^2$ denote left-invariant first and second derivatives on $G$ (measured in the bi-invariant metric).
  \item[(B)] \textbf{Commutator scaling for the tick holonomy.} Writing $X(u):=\log(\Psi_{q,a}(u))\in\mathfrak g$ on $W_{q,a}$, there exists $C_\Psi<\infty$ such that for all $u\in W_{q,a}$,
  \[
    \|X(u)\|\ \le\ C_\Psi\,a^2,\qquad
    \max_{e\in S(q)}\|\nabla_e X(u)\|\ \le\ C_\Psi\,a,\qquad
    \max_{e_1,e_2\in S(q)}\|\nabla_{e_1}\nabla_{e_2} X(u)\|\ \le\ C_\Psi,
  \]
  where $\nabla_e$ denotes the left-invariant derivative in the $e$-th stencil coordinate.
\end{itemize}
Then on $W_{q,a}$ one has the uniform bound
\[
  \|\Delta_{q,a,L}\|_{C^2(W_{q,a})}\ \le\ C\,\beta(a)\,a^6,
\]
for a constant $C<\infty$ depending only on $(C_f,C_\Psi,r,G)$ and the stencil size $|S(q)|$.
In particular, along any schedule with $\sup_{a\in(0,a_0]} \beta(a)\,a<\infty$ (e.g.\ under the scaling window \eqref{eq:ucis-sw-window}, or under the AF-style schedule Definition~\ref{def:beta-schedule}),
there is $C'<\infty$ such that
\[
  \|\Delta_{q,a,L}\|_{C^2(W_{q,a})}\ \le\ C'\,a^5.
\]
\end{lemma}

\begin{proof}
Write $g(u):=\Psi_{q,a}(u)=\exp(X(u))$ on $W_{q,a}$.
The $C^0$ bound follows directly from (A) and $\|X(u)\|\le C_\Psi a^2$:
\[
  |\Delta_{q,a,L}(u)|\ =\ |f_a(g(u))|\ \le\ C_f\,\beta(a)\,\|X(u)\|^4\ \le\ C\,\beta(a)\,a^8.
\]
For first derivatives, the chain rule gives $\nabla_e\Delta=(\nabla f_a)(g)\cdot \nabla_e g$, and on the normal neighborhood $\|\nabla_e g\|\lesssim \|\nabla_e X\|$ with a constant depending only on $(r,G)$.
Using (A) and (B),
\[
  \|\nabla_e\Delta\|\ \lesssim\ \|\nabla f_a(\exp X)\|\,\|\nabla_e X\|
  \ \le\ C_f\,\beta(a)\,\|X\|^3\cdot (C_\Psi a)
  \ \le\ C\,\beta(a)\,a^7.
\]
For second derivatives, the chain rule yields two terms (Hessian term plus gradient$\times$second derivative term); schematically,
\[
  \nabla_{e_1}\nabla_{e_2}\Delta
  =(\nabla^2 f_a)(g)[\nabla_{e_1}g,\nabla_{e_2}g]\ +\ (\nabla f_a)(g)\cdot \nabla_{e_1}\nabla_{e_2}g.
\]
Again on the normal neighborhood $\|\nabla g\|\lesssim\|\nabla X\|$ and $\|\nabla^2 g\|\lesssim \|\nabla^2 X\|+\|\nabla X\|^2$ with constants depending only on $(r,G)$.
Using (A) and (B) gives
\[
  \|\nabla_{e_1}\nabla_{e_2}\Delta\|
  \ \lesssim\ \|\nabla^2 f_a(\exp X)\|\,\|\nabla X\|^2\ +\ \|\nabla f_a(\exp X)\|\,\big(\|\nabla^2 X\|+\|\nabla X\|^2\big)
  \ \le\ C\,\beta(a)\,a^6,
\]
since $\|\nabla^2 f_a(\exp X)\|\lesssim \beta(a)\|X\|^2\lesssim \beta(a)a^4$, $\|\nabla X\|\lesssim a$, and $\|\nabla^2 X\|\lesssim 1$.
Combining the bounds gives $\|\Delta_{q,a,L}\|_{C^2(W_{q,a})}\le C\,\beta(a)a^6$.
The final $a^5$ bound follows under $\sup_a \beta(a)a<\infty$.
\end{proof}
%
...
For the continuum step, by Thms.~\ref{thm:U1-lsi-uei}, \ref{thm:os1-unconditional}, Lem.~\ref{lem:U2-comparison}, Prop.~\ref{prop:one-point-resolvent}, and Thm.~\ref{thm:U2-nrc-unique}, the AF--free NRC engine (Thm.~\ref{thm:nrc-operator-norm}) together with the Cauchy criterion (Lem.~\ref{lem:af-free-cauchy}) gives operator-norm resolvent convergence on fixed regions and identifies a unique limit, conditional on Assumption~\ref{assump:U2b}. Gap persistence to the continuum then follows from Thm.~\ref{thm:gap-persist-cont}. UEI and limit closures establish OS0--OS3; local fields exist and are non-Gaussian (Prop.~\ref{prop:nonzero-cumulant4}).
...
\begin{theorem}[Continuum YM on $\mathbb R^4$ with OS0--OS5 and positive mass gap (conditional on U1/OS1/NRC inputs)]\label{thm:main-af-free}
Fix a compact simple gauge group $G$ (default $\mathrm{SU}(N)$, $N\ge 2$) and a weak--coupling scaling window $a\downarrow 0$ with a schedule $\beta=\beta(a)$ and van Hove volumes.
Assume the following fixed-region/continuum inputs (RG-grade in $d=4$):
\begin{itemize}
  \item \textbf{(U1)} fixed-region tightness/UEI along the scaling window (Theorem~\ref{thm:uei-fixed-region});
  \item \textbf{(OS1)} Euclidean invariance on fixed regions in the limit (Theorem~\ref{thm:os1-unconditional});
  \item \textbf{(U2/NRC)} AF--free NRC uniqueness/identification of the continuum generator (Theorem~\ref{thm:U2-nrc-unique}, conditional on Assumption~\ref{assump:U2b});
  \item \textbf{(Interface smoothing)} scaling-window interface smoothing on fixed slabs via UCIS$_{\rm SW}$ (Theorem~\ref{thm:ucis-sw} under \eqref{eq:ucis-sw-window}).
\end{itemize}
Then there exists a nontrivial Euclidean quantum Yang--Mills theory on $\mathbb R^4$ whose Schwinger functions satisfy OS0--OS5, with local gauge-invariant fields. Let $H\ge 0$ be the corresponding Euclidean generator. There exists a constant $\gamma_*>0$, depending only on the fixed slab geometry and group data together with the UCIS$_{\rm SW}$ scaling-window parameters (i.e. on $(G,\varepsilon,T_{\rm phys},C_{\rm SW})$ through the constants produced in Theorem~\ref{thm:ucis-sw}), such that
\[
  \operatorname{spec}(H)\subset\{0\}\cup[\gamma_*,\infty)\,.
\]
Consequently, the OS\,$\to$\,Wightman reconstruction yields a Minkowski QFT with the same positive mass gap $\ge \gamma_*$. In particular, one may take $\gamma_* := 8\,c_{\mathrm{cut,phys}} = 8\,\big(-\log(1-\theta_*(1-e^{-\lambda_1(G) t_0}))\big)$ with $(\theta_*,t_0)$ as produced by UCIS$_{\rm SW}$ (Theorem~\ref{thm:ucis-sw} under \eqref{eq:ucis-sw-window}).
\end{theorem}
\begin{proof}
...
\begin{corollary}[Non-Gaussianity of the continuum local fields (conditional)]\label{cor:nonGaussian-main}
Assume the hypotheses ensuring existence and uniqueness of the fixed-region continuum limits (in particular Assumption~\ref{assump:U2b}). Then there exist compactly supported smooth test functions $f_1,\ldots,f_4\in C_c^\infty(\mathbb R^4,\wedge^2\mathbb R^4)$ such that the truncated 4-point function of the clover field is nonzero in the continuum limit:
\[
  \langle \Xi(f_1)\,\Xi(f_2)\,\Xi(f_3)\,\Xi(f_4)\rangle_c\ \neq\ 0.
\]
...
\end{corollary}
\begin{proof}
Fix a bounded region $R$ and $f\in C_c^\infty(R)$ chosen as in Proposition~\ref{prop:nonzero-cumulant4} so that, for all sufficiently small $a$ and large $L$, one has $\langle \Xi_a(f)^4\rangle_c\ge c_0>0$ uniformly in $(a,L)$. By Lemma~\ref{lem:local-fields-tempered} and Corollary~\ref{cor:unique-schwinger-local}, $\Xi_a(f)\to \Xi(f)$ in $L^2$ on fixed regions and the required moments converge along van Hove sequences (conditional on Assumption~\ref{assump:U2b}). Since truncated cumulants are polynomial combinations of moments, they are continuous under convergence of moments of the required orders. Therefore
\[
...
% ============================
\medskip
\begin{theorem}[Clay–critical Global OS pack on $\mathbb R^4$ (OS0--OS5, explicit constants; conditional)]\label{thm:global-os-clay}
Assume the hypotheses of Theorem~\ref{thm:main-af-free} (in particular Assumption~\ref{assump:U2b}). Then there exist global Schwinger functions $\{S_n\}_{n\ge1}$ on the cylinder $\sigma$--algebra of gauge--invariant observables on $\mathbb R^4$ such that OS0--OS5 hold globally, with explicit constants and no dependence on the choice of van Hove exhaustion, embedding scheme, or boundary conditions. More precisely:
\begin{itemize}
...
\begin{theorem}[Uniform global NRC with explicit constants; spectral projectors (conditional)]\label{thm:global-nrc-clay}
Assume Assumption~\ref{assump:U2b} so that Theorem~\ref{thm:nrc-quant} applies.
...
\]
where $\varepsilon(a)=O(a)$ under Theorem~\ref{thm:nrc-quant} (conditional on Assumption~\ref{assump:U2b}), and under the fixed–core normalization of \S\ref{thm:quant-calibrated-af-free-nrc} one has $\varepsilon(a)\le a^2$ (Cor.~\ref{cor:NRC-explicit}). In particular, on any compact $K\subset\mathbb C\setminus\mathbb R$ the convergence is uniform with the same $K(z)$.
...
\begin{theorem}[Gap persistence, OS$\to$Wightman, microcausality and nontriviality (conditional)]\label{thm:gap-wightman-clay}
Assume the hypotheses of Theorem~\ref{thm:main-af-free} (in particular Assumption~\ref{assump:U2b}). For the resulting global continuum theory:
\begin{itemize}
...
\begin{theorem}[Group generality and global independence/uniqueness (conditional)]\label{thm:independence-clay}
Let $G$ be any compact simple Lie group. Assume the hypotheses of Theorem~\ref{thm:main-af-free} (in particular Assumption~\ref{assump:U2b}). Then:
\begin{itemize}
...
\]
The resulting gap floor is transported to the continuum generator by Theorem~\ref{thm:global-gap-uncond} (under the remaining fixed-region inputs, in particular Assumption~\ref{assump:U2b}). A schedule-free, $\beta$--uniform one-step Doeblin minorization is \emph{not} used in the main chain.
\end{mdframed}
...
\begin{itemize}
  ...
  \]
  with $\varepsilon(a)=O(a)$ under Theorem~\ref{thm:nrc-quant} (conditional on Assumption~\ref{assump:U2b}) and $\varepsilon(a)\le a^2$ under the fixed-core normalization (Cor.~\ref{cor:NRC-explicit}).
...
Assume the hypotheses of Theorem~\ref{thm:main-af-free} (in particular UCIS$_{\rm SW}$ under \eqref{eq:ucis-sw-window} and Assumption~\ref{assump:U2b}). Then the global continuum generator $H$ obtained by NRC and OS reconstruction satisfies
\[
  \operatorname{spec}(H)\subset\{0\}\cup[\gamma_*,\infty)\,,
\]
...
\begin{theorem}[Global Minkowski mass gap (conditional)]\label{thm:global-minkowski-gap}
Assume the hypotheses of Theorem~\ref{thm:main-af-free} (in particular UCIS$_{\rm SW}$ under \eqref{eq:ucis-sw-window} and Assumption~\ref{assump:U2b}). For the global continuum OS measure constructed in Section~\ref{sec:global-R4}, let $H\ge 0$ be the Euclidean generator and $H_{\mathrm{Mink}}$ the Minkowski Hamiltonian obtained by OS\,$\to$\,Wightman. Then there exists $\gamma_*>0$ such that
\[
...
\begin{lemma}[Nontriviality: positive variance witness (conditional)]\label{lem:nontrivial-variance}
...
\begin{itemize}
  ...
  \item \textbf{NRC/embedding constants.} From Theorems~\ref{thm:nrc-embeddings}, \ref{thm:nrc-quant} together with Assumption~\ref{assump:U2b}: graph-defect bound $C_{\rm gd}$, low-energy projection control $C_\Lambda$, and resolvent rate $C(z_0,\Lambda)$.
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Mass gap (continuum)}: Scaling-window interface closure: Thm.~\ref{thm:ucis-sw} (under \eqref{eq:ucis-sw-window}), Cor.~\ref{cor:ucis-sw-L2-contraction}, Thm.~\ref{thm:ucis-sw-odd-subspace}, and bookkeeping via Prop.~\ref{prop:os35-limit}; persistence to the limit: Thm.~\ref{thm:gap-persist-cont} together with AF--free NRC (conditional on Assumption~\ref{assump:U2b}): Thm.~\ref{thm:nrc-operator-norm}, Thm.~\ref{thm:nrc-embeddings}.
  \item \textbf{AF/Mosco framework}: Assumption~\ref{assump:AF-Mosco}; Semigroup\,$\Rightarrow$\,resolvent: Thm.~\ref{thm:NRC-allz}; quantitative NRC: Thm.~\ref{thm:nrc-quant} (conditional on Assumption~\ref{assump:U2b}); embeddings/core: Thm.~\ref{thm:strong-semigroup-core}, Prop.~\ref{prop:collective-compactness}; defects/projections: Assumption~\ref{assump:U2b} (OS/GNS embedding-side inputs), and for the fixed-slab $J_a$ discretization analogues see Lem.~\ref{lem:graph-defect-Oa}, Lem.~\ref{lem:low-energy-proj}.
...
\begin{itemize}
  ...
  \item \textbf{Unique projective limit (conditional).} Tightness (UEI) and equicontinuity yield subsequential limits on fixed regions; uniqueness (no subsequences) requires an AF--free NRC/uniqueness input, e.g.\ Proposition~\ref{prop:af-free-uniqueness} (which assumes an operator-norm resolvent Cauchy input) or Theorem~\ref{thm:U2-nrc-unique} under Assumption~\ref{assump:U2b}.
...
\begin{theorem}[Tightness of loop $n$-point functions; uniqueness conditional]\label{thm:c1a-tight}
...
\noindent If, in addition, the AF--free NRC/uniqueness package applies on the fixed region containing $\{\Gamma_i\}$ (e.g.\ Theorem~\ref{thm:U2-nrc-unique}, with one-point input supplied by Proposition~\ref{prop:one-point-resolvent} under Assumption~\ref{assump:U2b}), then the corresponding loop Schwinger functions converge uniquely (no subsequences) to consistent limits $\{S_n\}_n$.
\end{theorem}
\begin{proof}
...
We record a \emph{conditional} packaging statement for an asymptotic-freedom (AF) scaling schedule $\beta(a)$. This section is \emph{not} used in the main chain: it does not prove existence of a 4D weak-coupling scaling limit from first principles, but only explains how the AF--free fixed-region NRC/uniqueness package would be applied along a concrete monotone schedule once the fixed-region inputs (in particular Assumption~\ref{assump:U2b}) are available.
...
Along schedules satisfying \eqref{eq:ucis-sw-window}, UCIS$_{\rm SW}$ supplies a fixed-physical-time contraction (Cor.~\ref{cor:ucis-sw-L2-contraction}, Thm.~\ref{thm:ucis-sw-odd-subspace}) and hence a slab gap input transported to the continuum by gap persistence (Thm.~\ref{thm:gap-persist-cont}), conditional on the NRC/uniqueness package (in particular Assumption~\ref{assump:U2b}).
...
\end{itemize}
\noindent To remove subsequences, one uses the AF--free uniqueness theorem on fixed regions (Theorem~\ref{thm:U2-nrc-unique}), which upgrades one-point operator--norm resolvent convergence (e.g.\ Proposition~\ref{prop:one-point-resolvent}, conditional on Assumption~\ref{assump:U2b}) to uniform convergence on compact $K\subset\mathbb C\setminus\mathbb R$ and hence uniqueness of the limiting generator.
...
Assume the hypotheses of the main AF--free continuum construction on fixed regions (in particular the scaling-window interface input UCIS$_{\rm SW}$ and Assumption~\ref{assump:U2b} for NRC/uniqueness). Fix any monotone schedule $\beta(a)$ satisfying (AF1)--(AF4) and any van Hove choice $L(a)$ with $L(a)\,a\to\infty$. Then along this schedule the resulting continuum limits are unique (no subsequences) and satisfy OS0--OS5; moreover the limiting generator $H\ge 0$ obeys
\[
...
\begin{itemize}
  ...
  \item U2b graph-defect bound $\|(H+1)^{-1/2}D_{a,L}(H_{a,L}+1)^{-1/2}\|\le C_{\rm gd}a$ and low-energy projection control $\|(I-P_{a,L})E_H([0,\Lambda])\|\le C_\Lambda a$: Assumption~\ref{assump:U2b} (U2b; currently conditional/target in the OS/GNS embedding framework).
...
\end{theorem}
\begin{proof}
By UEI/LSI (U1), for any smeared local functional $F$ supported in a fixed region $R$, the Laplace transform obeys $\log\mathbb E[e^{t(F-\mathbb EF)}]\le t^2 C(R)/(2\rho)$, giving uniform sub-Gaussian tails. Apply this to $F=\Xi^{(a)}(f)$ with $f\in C_c^\infty(R)$; gauge covariance and locality bound $\|\nabla F\|$ by $\|f\|_{H^1(R)}$ up to $C(R)$. Thus $\sup_a \mathbb E[\,|\Xi^{(a)}(f)|^2\,]\le C(R)\,\|f\|_{H^1}^2$. Fix a reference $f_\mu$ and choose $Z_F(a)$ to normalize $\langle \Xi^{(a)}_R(f_\mu)^2\rangle$ to a finite constant; the bound forces $\sup_a Z_F(a)\le C'(R)$. Tightness and the AF-free NRC (conditional on Assumption~\ref{assump:U2b}) yield convergence of $\Xi^{(a)}_R$ in $\mathsf S'$ along van Hove. Nontriviality follows from Proposition~\ref{prop:nonzero-cumulant4}: a strictly positive truncated 4-point persists in the limit, hence $\Xi_R\not\equiv 0$.
\end{proof}
...
\medskip
\noindent\emph{Conclusion.} Under these U1 hypotheses, the Herbst argument yields centered subgaussian Laplace bounds on $R$ and hence uniform exponential integrability/tightness for the corresponding local cylinder laws. (See also Theorem~\ref{thm:uei-fixed-region} and the operational decomposition in \texttt{U1\_OPERATIONAL\_PLAN.md}.)
\end{theorem}
...
\begin{proposition}[One–point resolvent estimate at a nonreal $z_0$ (conditional; cutoff form)]\label{prop:one-point-resolvent}
...
\begin{itemize}
  ...
  \item Assumption~\ref{assump:U2b} (U2b quantitative inputs);
...
\section{Appendix: beta-independent interface minorization (legacy target; proof sketch)}
...
\begin{proposition}[Explicit Doeblin constants, $\beta$\,–\,independent (legacy target; proof sketch)]\label{prop:explicit-doeblin-constants-appendix}
\noindent\emph{Target statement (not used in the main chain).} There exist $t_0=t_0(G)>0$ and $\theta_*=\theta_*(R_*,a_0,G)>0$, independent of $\beta$, $a\in(0,a_0]$, and $L$, such that
\[
