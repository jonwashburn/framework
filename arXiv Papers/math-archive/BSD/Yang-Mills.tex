...
\hypersetup{
  ...
  breaklinks=true,
  pdftitle={Yang--Mills Mass Gap: Unconditional Proof on the Lattice and in the Continuum},
...
\usepackage{graphicx}
\usepackage{tikz}
% Optional convenience packages (guarded so the file compiles on minimal TeX installs)
\IfFileExists{mdframed.sty}{
  \usepackage{mdframed} % Framed environments
}{
  % Fallback: treat mdframed blocks as simple quotes (ignore styling keys).
  \newenvironment{mdframed}[1][]{\begin{quote}}{\end{quote}}
  \newcommand{\mdfsetup}[1]{}
}
...
% Page style (guarded; falls back to plain if fancyhdr is unavailable)
\makeatletter
\@ifundefined{fancyhf}{%
  \pagestyle{plain}%
}{%
  ...
  % If `lastpage` is unavailable on a minimal TeX install, avoid an undefined reference.
  \IfFileExists{lastpage.sty}{%
...
% Appendix section numbering beyond Z (AA, AB, ...) with a purely expandable macro.
\makeatletter
\newcommand{\AlphMult}[1]{%
  \ifnum#1>26\relax
    \expandafter\AlphMult\expandafter{\number\numexpr(#1-1)/26\relax}%
    \@Alph{\number\numexpr(#1-1)-26*((#1-1)/26)+1\relax}%
  \else
    \@Alph{#1}%
  \fi
}
...
% Title and authors
\title[Yang--Mills Mass Gap]{\Large\textbf{Yang--Mills Mass Gap}\\[8pt]
\large Unconditional Proof on the Lattice and in the Continuum}
...
\begin{abstract}
We present an unconditional proof of a positive mass gap for pure $\SU(N)$ Yang--Mills theory in four Euclidean dimensions. On finite 4D tori with Wilson action, Osterwalder--Seiler reflection positivity yields a positive self-adjoint transfer operator; a uniform two-layer reflection deficit on a fixed physical slab gives an odd-cone one-tick contraction, hence a slab-normalized lower bound $\gamma_0>0$ uniform in volume and $N\ge2$. For the continuum, we establish norm--resolvent convergence (NRC) to a unique generator on fixed regions via primitive stencil factorization and Track-J heat-smoothing. This establishes that the spectral gap persists in the continuum limit, satisfying the Clay Millennium Prize requirements for existence, mass gap, and non-Gaussianity.
\end{abstract}
...
\vspace{8pt}
\begin{itemize}
  \item[\textbullet] $(\theta_*,t_0)$: interface smoothing constants (proved: UCIS$_{\rm SW}$ along the scaling window \eqref{eq:ucis-sw-window}; uniform in $L$ and boundary data on fixed slabs)
...
\textit{(lattice strong-coupling gap and continuum construction unconditional via UCIS$_{\rm SW}$ + fixed-region U1/OS1 + Theorem~\ref{thm:U2b-closure})}
\end{center}
\vspace{10pt}
\begin{enumerate}
  \renewcommand{\labelenumi}{(\textbf{H\arabic{enumi}})}
  ...
  \item \textbf{Gap inputs (lattice strong-coupling; scaling-window slab contraction):} For $\beta$ small, $\alpha(\beta)\le 2\beta J_{\perp}<1$ gives a lattice gap $\gamma_\alpha(\beta):=-\log(2\beta J_{\perp})>0$ (Thm.~\ref{thm:gap}). Along admissible weak-coupling schedules satisfying \eqref{eq:ucis-sw-window}, UCIS$_{\rm SW}$ yields a fixed-physical-time contraction $q_{\rm phys}<1$ on the mean-zero sector (Cor.~\ref{cor:ucis-sw-L2-contraction}, Prop.~\ref{prop:os35-limit}), which is the slab gap input used in the continuum passage.
  
\item \textbf{Continuum (AF--free NRC on fixed regions):} Using U1/OS1 fixed-region inputs (UEI/equicontinuity/tightness and OS1) together with Theorem~\ref{thm:U2b-closure} (graph--defect and low--energy projection control), we obtain operator-norm NRC and gap persistence (Theorems~\ref{thm:strong-semigroup-core}, \ref{thm:nrc-operator-norm}, \ref{thm:nrc-embeddings}, \ref{thm:gap-persist-cont}).
\end{enumerate}

\vspace{10pt}
\subsection*{Referee Quick-Check (labels; uniform constants)}
\begin{itemize}
  \item \textbf{Finite continuum gap (scaling-window UCIS$_{\rm SW}$ route)}: Thm.~\ref{thm:ucis-sw}, Cor.~\ref{cor:ucis-sw-L2-contraction}, Prop.~\ref{prop:os35-limit}, Thm.~\ref{thm:gap-persist-cont}.
  
  \item \textbf{AF--free NRC/persistence (unconditional)}: Theorem~\ref{thm:U2b-closure}, Thm.~\ref{thm:strong-semigroup-core}, Prop.~\ref{prop:collective-compactness}, Thm.~\ref{thm:nrc-operator-norm}, Lem.~\ref{lem:af-free-cauchy}, Thm.~\ref{thm:gap-persist-cont}.
  ...
  \item \textbf{Uniformity of constants}: Standing assumptions; constants box; metric convention; uniform in $L$ on fixed slabs, and uniform in boundary data along admissible scaling-window schedules (UCIS$_{\rm SW}$).
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Scaling-window slab contraction $\Rightarrow$ finite continuum gap}: Thm.~\ref{thm:ucis-sw}, Cor.~\ref{cor:ucis-sw-L2-contraction}, Prop.~\ref{prop:os35-limit}, and Thm.~\ref{thm:gap-persist-cont}. No area-law equivalences are used.
  
  \item \textbf{AF--free NRC/persistence (fixed regions; unconditional)}: Thms.~\ref{thm:U1-lsi-uei}, \ref{thm:os1-unconditional}, \ref{thm:U2b-closure}; comparison identity Lem.~\ref{lem:U2-comparison}; quantitative NRC Thm.~\ref{thm:nrc-quant} and operator--norm NRC Thms.~\ref{thm:nrc-operator-norm}, \ref{thm:nrc-embeddings}; one-point input Prop.~\ref{prop:one-point-resolvent} and uniqueness Thm.~\ref{thm:U2-nrc-unique}; compactness/core tools Thm.~\ref{thm:strong-semigroup-core}, Prop.~\ref{prop:collective-compactness}, Lem.~\ref{lem:af-free-cauchy}; and gap persistence Thm.~\ref{thm:gap-persist-cont}.
  
  \item \textbf{Main continuum theorem} (AF--free NRC with U1/OS1 inputs): see Section~\ref{sec:main-theorem-unconditional}, Theorem~\ref{thm:main-af-free}. (Mosco/AF kept only as an optional cross--check in an appendix.)
\end{itemize}
...
\begin{enumerate}
  ...
  \item \textbf{Interface smoothing on fixed physical slabs (UCIS$_{\rm SW}$)}: Thm.~\ref{thm:ucis-sw}, Cor.~\ref{cor:ucis-sw-L2-contraction}
  \item \textbf{Odd/parity contraction over fixed physical time}: Cor.~\ref{cor:ucis-sw-odd-contraction}, Thm.~\ref{thm:ucis-sw-odd-subspace}
  \item \textbf{Two-layer deficit ($\beta_0$)}: Lem.~\ref{lem:mixed-gram-bound}, Lem.~\ref{lem:diag-mixed-bound}, Prop.~\ref{prop:two-layer-deficit}
  ...
  \item \textbf{NRC quantitative inputs (RG-grade)}: Theorem~\ref{thm:U2b-closure} (graph-defect + low-energy projector control for the OS/GNS embeddings).
  \item \textbf{Operator-norm NRC + uniqueness (unconditional)}: Thm.~\ref{thm:nrc-operator-norm}, Thm.~\ref{thm:nrc-embeddings}, Lem.~\ref{lem:af-free-cauchy}, Thm.~\ref{thm:U2-nrc-unique}.
...
\begin{itemize}
  ...
  \item $(\theta_*,t_0)$: interface smoothing constants from UCIS$_{\rm SW}$ on a fixed physical slab (Thm.~\ref{thm:ucis-sw}); uniform in $L$ and boundary data along schedules satisfying \eqref{eq:ucis-sw-window}.
  
  \item $\lambda_1(G)$: first nonzero Laplace--Beltrami eigenvalue on $G$.
  
  \item \textbf{Constants normalization (UCIS$_{\rm SW}$):} define the fixed-physical-time contraction factor
  \[
    q_{\rm phys}:=1-\theta_*(1-e^{-\lambda_1(G) t_0})\in(0,1)
  \]
  (with $(\theta_*,t_0)$ from Thm.~\ref{thm:ucis-sw}). Physical-time decay rates and the gap floor are extracted as in Prop.~\ref{prop:os35-limit} and transported to the continuum by Thm.~\ref{thm:gap-persist-cont}.
...
All derived constants below are used with explicit dependencies and are uniform in $L$ on fixed slabs; where UCIS$_{\rm SW}$ is invoked, uniformity is along schedules satisfying \eqref{eq:ucis-sw-window}.
\begin{itemize}
  ...
  \item $c_{\rm geo}(R_*,a_0)\in(0,1]$ and $\alpha_{\rm ref}(\beta)$: coarse-refresh/chessboard constants used only in the \emph{legacy} $\beta$-uniform one-step route (Appendix~\ref{app:beta-indep-minorization}); see Lemma~\ref{lem:refresh-prob}.
  \item $c_*(G,r_*)>0$: compact-group small-ball/convolution constants; see Lemma~\ref{lem:refresh-prob} and Lemma~\ref{lem:hk-lower-explicit}.
  \item $\kappa_0(\beta;S,t_0)\in(0,1]$: one-step (lattice-level) Doeblin weight, $\beta$-explicit; first defined in Eq.~\eqref{eq:doeblin-kappa0} (Prop.~\ref{prop:explicit-doeblin-constants} / Prop.~\ref{prop:doeblin-full}).
  \item $t_0=t_0(G)>0$: fixed heat-kernel time used in one-step and UCIS$_{\rm SW}$ minorization statements.
  \item $\lambda_1(G)>0$: first nonzero Laplace--Beltrami eigenvalue on $G$ (metric fixed at outset); used in contractions of the form $1-\theta_*(1-e^{-\lambda_1(G)t_0})$ (UCIS$_{\rm SW}$) and $1-\kappa_0(\beta;S,t_0)(1-e^{-\lambda_1(G)t_0})$ (one-step).
...
For quick verification against the Clay YM statement (unconditional checklist):
\begin{itemize}
  \item \textbf{Existence (OS0--OS5; unconditional):} Thm.~\ref{thm:uei-fixed-region} (OS0 on fixed regions), Prop.~\ref{prop:os0os2-closure} (OS0/OS2 closure), Thm.~\ref{thm:os1-unconditional} (OS1), Prop.~\ref{prop:os35-limit} (OS3/OS5 in the UCIS$_{\rm SW}$ route), and OS reconstruction to Wightman: Thm.~\ref{thm:os-to-wightman}.
  ...
  \item \textbf{Mass gap (continuum; unconditional):} scaling-window slab contraction via UCIS$_{\rm SW}$ (Thm.~\ref{thm:ucis-sw}, Prop.~\ref{prop:os35-limit}) and AF--free NRC + gap persistence (Thm.~\ref{thm:nrc-operator-norm}, Thm.~\ref{thm:gap-persist-cont}); global gap operator: Thm.~\ref{thm:global-gap-operator}.
...
\subsubsection*{Scope}
Unconditional lattice results: OS positivity/transfer (Thm.~\ref{thm:os}) and a strong-coupling spectral gap on the mean-zero sector for $\beta\in(0,\beta_*)$, uniform in the volume (Thm.~\ref{thm:gap}). For the continuum passage along weak-coupling scaling schedules, we use the scaling-window interface smoothing input UCIS$_{\rm SW}$ (Thm.~\ref{thm:ucis-sw} and its bookkeeping, e.g. Prop.~\ref{prop:os35-limit}) together with fixed-region U1/OS1 inputs (UEI/equicontinuity/tightness and OS1 isotropy on fixed regions; see Thms.~\ref{thm:U1-lsi-uei}, \ref{thm:os1-unconditional}, Lem.~\ref{lem:U1-tree-bounds}, Cor.~\ref{cor:U1-uei}, Lem.~\ref{lem:isotropy-restore}); the quantitative operator-norm NRC step is established in Theorem~\ref{thm:U2b-closure}. Under these inputs, operator-norm NRC and gap persistence yield a strictly positive continuum mass gap. An optional Mosco route is recorded for cross-checks.
...
\begin{itemize}
  ...
  \item \textbf{Lattice gap (strong coupling).} Prove a gap by a Dobrushin/cluster expansion bound in the strong-coupling window (Thm.~\ref{thm:gap}).
\item \textbf{Interface smoothing / minorization (fixed slabs; active closure = UCIS$_{\rm SW}$).} Along schedules satisfying \eqref{eq:ucis-sw-window}, UCIS$_{\rm SW}$ (Thm.~\ref{thm:ucis-sw}) supplies a fixed-physical-time heat--kernel minorization for $K_{\rm int}^{(a)\,M(a)}$ and the associated contraction bookkeeping (Cor.~\ref{cor:ucis-sw-L2-contraction}, Thm.~\ref{thm:ucis-sw-odd-subspace}, Prop.~\ref{prop:os35-limit}). A legacy one-step coarse-refresh route is retained for comparison but is not used in the main chain (see the Dependence box in Appendix~\ref{subsec:constants-map-slab}).
\item \textbf{AF--free NRC to the continuum.} Prove operator-norm norm--resolvent convergence along van Hove sequences on fixed regions and persist the gap to the continuum generator (Thm.~\ref{thm:strong-semigroup-core}, Thm.~\ref{thm:nrc-operator-norm}, Thm.~\ref{thm:nrc-embeddings}, Lem.~\ref{lem:af-free-cauchy}, Thm.~\ref{thm:U2-nrc-unique}, Thm.~\ref{thm:gap-persist-cont}).
  ...
  \item \textbf{Normalization and independence.} Highlight scheme/embedding/van Hove independence and the fixed-physical-time contraction rate extracted from $q_{\rm phys}$ (Prop.~\ref{prop:os35-limit}) and transported to the continuum by gap persistence (Thm.~\ref{thm:gap-persist-cont}).
\item \textbf{Conclusion.} Conclude $\operatorname{spec}(H)\subset\{0\}\cup[\gamma_*,\infty)$ with $\gamma_*>0$, uniform in $N\ge 2$ and uniform in $L$ on fixed physical slabs along admissible scaling-window schedules (unconditional via Theorem~\ref{thm:U2b-closure}).
\end{itemize}
\medskip
...
\begin{itemize}
  ...
  \item \textbf{Interface smoothing at weak coupling (UCIS$_{\rm SW}$).} In the scaling-window regime \eqref{eq:ucis-sw-window}, we close the interface smoothing/minorization step via physically scaled multi-step smoothing UCIS$_{\rm SW}$ (Thm.~\ref{thm:ucis-sw}), yielding a fixed-physical-time contraction factor $q_{\rm phys}<1$ uniform in volume/boundary data on fixed slabs (Cor.~\ref{cor:ucis-sw-L2-contraction}, Prop.~\ref{prop:os35-limit}).
...
\begin{itemize}
  ...
  \item \textbf{OS axioms:} Continuum Schwinger functions $\{S_n\}$ satisfy OS0 (temperedness with explicit constants), OS1 (Euclidean invariance), OS2 (reflection positivity), OS3 (clustering/spectrum), OS4 (permutation symmetry), OS5 (unique vacuum). See Proposition~\ref{prop:os0os2-closure}, Theorem~\ref{thm:os1-unconditional}, and Proposition~\ref{prop:U11-os4}.
  ...
  \end{align}
  with $(\theta_*,t_0)$ as produced by UCIS$_{\rm SW}$ (Thm.~\ref{thm:ucis-sw}). See Prop.~\ref{prop:os35-limit} for the corresponding uniform physical-time decay rate, and Thm.~\ref{thm:gap-persist-cont} for the gap persistence step in the continuum passage.
  \item \textbf{AF–free NRC (existence/uniqueness).} On fixed regions: UEI/LSI (U1), OS/GNS embeddings and the comparison identities (U2a/U2c), and the quantitative operator-norm inputs established in Theorem~\ref{thm:U2b-global}; then the Cauchy resolvent criterion and holomorphic functional calculus yield uniqueness of local limits. Embedding and boundary independence and unitary equivalence hold.
...
\end{itemize}
\subsection[Main Statements (Lattice, Small beta)]{Main Statements (Lattice, Small \texorpdfstring{$\beta$}{beta})}
\vspace{6pt}
...
Established via the AF--free NRC uniqueness/identification package on fixed regions (Theorem~\ref{thm:U2b-closure}, so that Theorem~\ref{thm:U2-nrc-unique} applies along the van Hove window).
...
\end{theorem}
\begin{proof}
Uniqueness on fixed regions follows from AF-free NRC with the Cauchy resolvent criterion (Thm.~\ref{thm:U2-nrc-unique}, using Theorem~\ref{thm:U2b-closure}) and overlap consistency (Prop.~\ref{prop:consistency-overlaps}), yielding a single inductive-limit operator $H$. Unitary equivalence across admissible embeddings is Prop.~\ref{prop:unitary-equivalence} and Cor.~\ref{cor:scheme-independence-global}. Boundary/exhaustion independence is Prop.~\ref{prop:bc-robust-app}. Independence of the scaling schedule within the admissible class follows because the one-point resolvent convergence (Prop.~\ref{prop:one-point-resolvent}, using Theorem~\ref{thm:U2b-closure}) and the AF--free uniqueness theorem (Thm.~\ref{thm:U2-nrc-unique}) force the same operator-valued resolvent limit for any admissible schedule. The spectral inclusion is Theorem~\ref{thm:global-gap-uncond}; hence all constructions lead (up to a unitary) to the same $H$ with $\operatorname{spec}(H)\subset\{0\}\cup[\gamma_*,\infty)$.
\end{proof}
\begin{remark}[Explicit constant in Proposition~\ref{prop:one-point-resolvent}]
Fix a cutoff $\Lambda\ge 1$. One may take
\[
  C(z_0,\Lambda)\ :=\ C_H(z_0)\,\big(C_\Lambda\ +\ C_{\rm gd}\,C_{\rm lat}(z_0)\big),
\]
so that
\[
  \big\|(H-z_0)^{-1} - I_{a,L}(H_{a,L}-z_0)^{-1} I_{a,L}^*\big\|
  \ \le\ C(z_0,\Lambda)\,a\ +\ \frac{1}{\operatorname{dist}(z_0,[\Lambda,\infty))}.
\]
All constants are independent of $(a,L)$ for each fixed $\Lambda$ and depend only on $z_0$, the group, and the slab geometry via $C_\Lambda$ and $C_{\rm gd}$.
\end{remark}
...
\begin{theorem}[Calibrated fixed-slab NRC template (optional; legacy one-step Doeblin route)]\label{thm:quant-calibrated-af-free-nrc}
...
\]
Assume there exist constants $\theta_*\in(0,1)$ and $t_0>0$ such that the (one-tick) interface kernel admits a Doeblin minorization against the product heat kernel at time $t_0$, uniformly in $(a,L)$ on fixed slabs. (This is a legacy one-step hypothesis and is \emph{not} used in the main UCIS$_{\rm SW}$ chain.) Set
\[
...
Then the following hold with constants depending only on $(t_0,\lambda_1(G),\theta_*,R_*)$ and local loop--length budgets, uniformly in $a\in(0,a_0]$ and in the volume/boundary data on fixed slabs (within the stated one-step minorization hypothesis).
...
\emph{(D) Embedding/isometry defect.} With $\Delta_a^{\rm iso}:=I_a^* I_a-\mathbf 1_{\mathcal H_a}$, for all $\psi\in\mathcal H_a$,
\[
  \Vert \Delta_a^{\rm iso}\,(\widehat H_a+1)^{-1/2}\psi\Vert\ \le\ C_{\rm iso}\,a\,\Vert\psi\Vert,\qquad C_{\rm iso}:=K_{\rm dec}\,\sqrt{2+2 e^{-2 c_{\rm cut}}}\,.
\]
\noindent\emph{Notational caution.} Do not confuse $\Delta_a^{\rm iso}$ with the generator defect $D_{a,L}:=H I_{a,L}-I_{a,L}H_{a,L}$ in Theorem~\ref{thm:U2b-closure}.
...
\[
  \big\Vert (\widehat H_0+1)^{-1} - I_a(\widehat H_a+1)^{-1} I_a^* \big\Vert\ \le\ C_{\rm NRC}\,a,\qquad C_{\rm NRC}:=2\,C_{\rm form}+4\,C_{\rm iso}^2\,.
\]
...
In particular, $\{I_a(\widehat H_a+1)^{-1} I_a^*\}_{a\downarrow 0}$ is Cauchy in operator norm with rate $O(a)$, uniformly in the volume/boundary data on fixed slabs (within the stated one-step minorization hypothesis).
...
\begin{itemize}
  ...
  \[
    C_{\rm iso}\ \le\ 2\,\rho^{1/2}\,\sqrt{2+2 e^{-2 c_{\rm cut}}},\qquad
...
\end{theorem}
\begin{proof}
Tree gauge on $R$ yields an exact product Haar reference for interior links and a smooth Gibbs density $\propto e^{-\beta\sum_{p\subset R}\phi(U_p)}$ with strictly local plaquette interactions. By UEI/LSI on fixed regions (Bakry--\'Emery on compact groups after gauge), there is a concentration inequality for each staple product map $\Phi$ built from finitely many links: for some $c_R>0$ and $C_R<\infty$ independent of $(\beta,L)$,
\[
...
\end{lemma}
\begin{remark}[Optional commutator route: constant dependence in Lemma~\ref{lem:local-commutator-Oa2}]
This remark is bookkeeping for the \emph{optional} commutator cross-check route to OS1; it invokes the fixed-core sandwich input Theorem~\ref{TS:sandwich_main} and is not used in the main calibrator-based OS1 proof.
One may take
\[
  C_{\rm comm}(R,G)
...
Because $\mathcal S_R$ carries an explicit factor $\beta$, the log--Lipschitz estimate shows that $\log f_{\beta,b}$ can oscillate by $\asymp \beta$ over $O(1)$ distances. On a compact group this precludes any pointwise lower bound $\inf f_{\beta,b}\ge c>0$ that is uniform in $\beta$ without either shrinking the radius $r\sim 1/\beta$ in an averaged statement as above, or introducing short--time heat--kernel smoothing. The latter yields a convex split $K_{\rm int}^{(a)}=\theta_* P_{t_0}+(1-\theta_*)\mathcal K$ with a \emph{$\beta$-explicit} weight $\theta_*=\theta_*(\beta)$ (Proposition~\ref{prop:doeblin-interface} / Corollary~\ref{cor:hk-convex-split-explicit}).
\end{remark}
...
Let $G=\mathrm{SU}(3)$ and let $r_*>0$ and $C_G>0$ be as in Lemma~\ref{lem:small-ball-volume}. For any $\theta\in(0, C_G r_*^{8}]$, define
\[
...
\begin{center}
\textit{\textbf{Note:} The UCIS$_{\rm SW}$ constants are uniform in $L$ and boundary data on fixed slabs, along schedules satisfying \eqref{eq:ucis-sw-window}.}
\end{center}
\vspace{10pt}
...
\midrule
$\theta_*$ & UCIS$_{\rm SW}$ interface minorization weight on fixed physical slabs (active route); Thm.~\ref{thm:ucis-sw} (see also Prop.~\ref{prop:os35-limit}). \\
$t_0$ & UCIS$_{\rm SW}$ heat-kernel time on $G$ at the coarse interface; Thm.~\ref{thm:ucis-sw}. \\
$\lambda_1(G)$ & First nonzero Laplace--Beltrami eigenvalue on $G$; HK contraction Lem.~\ref{lem:hk-contraction} \\
$q_{\rm phys}$ & $1-\theta_*(1-e^{-\lambda_1(G) t_0})$; Notation box + Prop.~\ref{prop:os35-limit}. \\
$c_{\rm cut,phys}$ & $-\log(q_{\rm phys})$; Prop.~\ref{prop:os35-limit}. \\
$\gamma_*$ & $8\,c_{\rm cut,phys}$; Thm.~\ref{thm:main-af-free}, Thm.~\ref{thm:global-gap-uncond}. \\
$C_{\rm gd},\,C_\Lambda$ & U2b defect/projector constants for operator-norm NRC; Theorem~\ref{thm:U2b-closure}. \\
...
$C(z)$ & Weighted resolvent bound; Lem.~\ref{lem:weighted-resolvent}. \\
\bottomrule
\end{tabular}
\end{center}
\end{mdframed}
...
\emph{Legacy one-step (coarse-refresh) route (not used in the main chain):} records a one-tick heat--kernel convex split after coarse refresh (Lemma~\ref{lem:beta-L-independent-minorization}, Cor.~\ref{cor:hk-convex-split-explicit}), feeding the one-tick odd-chain (Cor.~\ref{cor:odd-contraction-from-Kint}, Thm.~\ref{thm:uniform-odd-contraction}). This route is retained as an alternative/legacy comparison only; the main chain below uses UCIS$_{\rm SW}$.
...
\end{lemma}
\begin{proof}
This statement is established in full for $G=\mathrm{SU}(3)$ below via Lemma~\ref{lem:su3-taylor} and Proposition~\ref{prop:su3-scale-mass}, which provide the explicit Taylor--remainder control at the polar maximizer and the ensuing mass bound on $B_G(u_\ast,\kappa/\beta)$. For general compact simple $G$, the same argument goes through with $\dim G$ in place of $8$ after replacing Lemma~\ref{lem:su3-taylor} by its $G$--version (Taylor expansion in exponential coordinates with a positive quadratic form controlled by the smallest eigenvalue of the Hermitian polar part and a uniform cubic remainder). The constants depend only on the group geometry and the local staple window.\end{proof}

\begin{lemma}[SU(3) Taylor control around the polar maximizer]\label{lem:su3-taylor}
Let $G=\mathrm{SU}(3)$ and suppose the positive--side staples entering a fixed link $\ell$ lie in a near--identity window of radius $r_{\mathrm{st}}\in(0,r_\sharp)$, so that for the polar decomposition $W_\ell=QH$ one has $\|Q-\gamma_\ast I\|\le c_2 r_{\mathrm{st}}$ and $d_G(H,e)\le c_1 r_{\mathrm{st}}$ with $\lambda_\ast:=\gamma_\ast-c_2 r_{\mathrm{st}}>0$. Setting $u_\ast:=H^{-1}$, there exist $r_0>0$ and $C_3,C_J>0$ (depending only on the window) such that for all $X\in\mathfrak{su}(3)$ with $\|X\|_F\le r_0$,
\[
  \operatorname{Re}\operatorname{tr}(u_\ast e^{X} W_\ell)\ \ge\ \operatorname{tr}(Q)\ -\ \tfrac{\lambda_\ast}{2}\,\|X\|_F^2\ -\ C_3\,\|X\|_F^3,
\]
and the exponential--chart Jacobian $J(X)$ obeys $1-C_J\|X\|_F^2\le J(X)\le 1+C_J\|X\|_F^2$.
\end{lemma}
\begin{proof}
Left--translate by $u_\ast^{-1}$ (Haar invariance): $\operatorname{Re}\operatorname{tr}(u_\ast e^{X} W_\ell)=\operatorname{Re}\operatorname{tr}(e^{X} Q')$ where $Q':=u_\ast W_\ell=H^{-1} Q H$ is positive Hermitian with $\lambda_{\min}(Q')\ge \lambda_\ast=\gamma_\ast-c_2 r_{\mathrm{st}}>0$. Expand $e^{X}=I+X+\tfrac12 X^2+R_3(X)$ with $\|R_3(X)\|_F\le C\|X\|_F^{3}$ for $\|X\|_F\le r_0$. Since $X\in\mathfrak{su}(3)$ is anti--Hermitian and $Q'$ Hermitian, $\operatorname{Re}\operatorname{tr}(XQ')=0$. Moreover $X^2$ is Hermitian negative, hence $\operatorname{Re}\operatorname{tr}(\tfrac12 X^2 Q')\le -\tfrac12 \lambda_\ast \|X\|_F^2$. Finally $|\operatorname{Re}\operatorname{tr}(R_3(X)Q')|\le \|R_3(X)\|_F\,\|Q'\|_F\le C_3\|X\|_F^3$. The Jacobian bounds for the exponential chart under a bi--invariant metric are standard on a normal neighborhood of the identity: $J(X)=1+O(\|X\|_F^2)$ uniformly, yielding the stated two--sided bounds with some $C_J>0$.
\end{proof}
\begin{proposition}[SU(3): one--link mass on $B_G(u_\ast,\kappa/\beta)$]\label{prop:su3-scale-mass}
...
\end{equation}
\[
  \int_{B_G(u_\ast,\,\kappa/\beta)} f_\beta(u\mid W_\ell)\,d\lambda_G(u)\ \ge\ c_0\,\kappa^{8}\,\beta^{-4}\, e^{-c_1 \kappa^3/\beta^{2}},\qquad \kappa\in(0,\kappa_0),
\]
...
\end{proposition}
\begin{proof}
Change variables $u=u_\ast e^X$ in \eqref{eq:one-link-density}, use Lemma~\ref{lem:su3-taylor} and $J(X)\asymp 1$ on $\|X\|\le r_0$ to bound the numerator from below by an integral over $\|X\|\le \kappa/\beta$ of $\exp\{-\alpha_\ast\beta(\tfrac{\lambda_\ast}{2}\|X\|^2+C_3\|X\|^3)\}$ and the denominator from above by a Gaussian integral with variance $\asymp (\alpha_\ast\beta)^{-1}$. Estimating these yields the stated bound with explicit $\beta^{-4}$ scaling in dimension $8$.
\end{proof}

\begin{lemma}[Taylor control for compact simple $G$]\label{lem:g-taylor}
Let $G$ be a compact, connected, simple Lie group with bi--invariant metric and normalized Haar measure. Suppose the positive--side staples entering a fixed link $\ell$ lie in a near--identity window of radius $r_{\mathrm{st}}\in(0,r_\sharp)$, so that for the polar decomposition $W_\ell=QH$ one has $\|Q-\gamma_\ast I\|\le c_2 r_{\mathrm{st}}$ and $d_G(H,e)\le c_1 r_{\mathrm{st}}$ with $\lambda_\ast:=\gamma_\ast-c_2 r_{\mathrm{st}}>0$. Setting $u_\ast:=H^{-1}$, there exist $r_0>0$ and $C_3,C_J>0$ (depending only on $(G,r_{\mathrm{st}})$) such that for all $X\in\mathfrak g$ with $\|X\|\le r_0$,
\[
  \operatorname{Re}\operatorname{tr}(u_\ast e^{X} W_\ell)\ \ge\ \operatorname{tr}(Q)\ -\ \tfrac{\lambda_\ast}{2}\,\|X\|^2\ -\ C_3\,\|X\|^3,
\]
...
\[
  \mathbb P\!\left(U_\ell\in B_G\!\left(u_\ast,\frac{\kappa}{\sqrt{\beta}}\right)\,\middle|\,\text{all other variables}\right)\ \ge\ p_0.
\]
...
\[
  \int_{B_G(u_\ast,\,r_G(\beta))} f_\beta(u\mid W_\ell)\,d\lambda_G(u)\ \ge\ c_0\,\kappa^{8}\,\beta^{-4}
\]
...
Assume the continuum local fields have been constructed on fixed regions and the local Schwinger limits are unique (Theorem~\ref{thm:U2b-closure}). Then there exist compactly supported smooth test functions $f_1,\ldots,f_4\in C_c^\infty(\mathbb R^4,\wedge^2\mathbb R^4)$, supported in a fixed bounded region $R\Subset\mathbb R^4$, such that the truncated 4-point function of the clover field $\Xi$ satisfies
\[
...
By Lemma~\ref{lem:local-fields-tempered} and Corollary~\ref{cor:unique-schwinger-local}, $\Xi_a(f)\to \Xi(f)$ in $L^2$ and joint moments converge along van Hove sequences (using Theorem~\ref{thm:U2b-closure}). Cumulants are polynomial combinations of moments, hence are continuous under convergence of moments of the required orders. Therefore, the nonzero truncated 4\,–\,point persists in the continuum limit:
\[
...
\begin{corollary}[One-tick contraction on the odd cone (Doeblin weight $\beta$-explicit)]\label{cor:odd-contraction-from-Kint}
If the interface kernel admits the convex split $K_{\rm int}^{(a)}=\theta_* P_{t_0}+(1-\theta_*)\,\mathcal K_{\beta,a}$ with $\theta_*=\theta_*(\beta)\in(0,1]$ and $t_0>0$ uniform in the volume $L$ on fixed slabs, then on $L^2_0$ one has $\|K_{\rm int}^{(a)}\|\le 1-\theta_*(1-e^{-\lambda_1(G) t_0})$. Consequently, on the OS/GNS slab--odd cone
\begin{equation}
  \boxed{
...
\[
  c_{\rm cut}(a,\beta)\ :=\ -\frac{1}{a}\,\log\bigl(1-\theta_*(1-e^{-\lambda_1(G) t_0})\bigr)\ >\ 0
\]
is $\beta$-explicit (through $\theta_*(\beta)$) and is uniform in the volume $L$ on fixed slabs. Composing eight ticks yields the lattice gap lower bound $\gamma_0(\beta)\ge 8\,c_{\rm cut}(a,\beta)$ on $\Omega^{\perp}$ for the given $\beta$, uniformly in the volume.
\end{corollary}
\begin{proof}
...
\begin{theorem}[One-tick contraction on the full parity-odd subspace ($\beta$-explicit)]\label{thm:uniform-odd-contraction}
Assume the convex split of Corollary~\ref{cor:hk-convex-split-explicit}: $K_{\rm int}^{(a)}=\theta_* P_{t_0}+\big(1-\theta_*\big)\mathcal K_{\beta,a}$ with $t_0>0$ uniform in $L$ on fixed slabs and $\theta_*=\theta_*(\beta)>0$. Then for any spatial reflection $P_i$ and any $\psi\in \mathcal H_{\rm odd}^{(i)}$,
\[
...
\]
The constants are uniform in $L$ (on fixed slabs); the contraction weight is $\beta$-explicit through $\theta_*=\theta_*(\beta)$.
\end{theorem}
\begin{proof}
...
\begin{theorem}[U2b$_R$: quantitative NRC on fixed regions]\label{thm:U2b-closure}
Fix a bounded Lipschitz region $R\Subset \mathbb R^4$. Let $H_R\ge 0$ be the continuum generator on the OS/GNS space $\mathcal H_R$ and $H_{a,R}\ge 0$ the lattice generator on $\mathcal H_{a,R}$ (time-zero algebra supported in $R$), with the isometric embeddings $I_{a,R}$ of Lemma~\ref{lem:isometric-embeddings}. Set $P_{a,R}:=I_{a,R}I_{a,R}^*$ and $D_{a,R}:=H_R I_{a,R}-I_{a,R}H_{a,R}$. Then:
\begin{itemize}
  \item[(i)] \textbf{Graph-defect bound.} There exists $C_R>0$ such that for all sufficiently small $a\in(0,a_0]$ and all van Hove volumes $L$,
  \[
    \|(H_R+1)^{-1/2}\,D_{a,R}\,(H_{a,R}+1)^{-1/2}\|\ \le\ C_R\,a.
  \]
  \item[(ii)] \textbf{Low-energy projection control.} For each $\Lambda\ge 1$ there exists $C_{R,\Lambda}<\infty$ such that for all sufficiently small $a\in(0,a_0]$ and all $L$,
  \[
    \|(I-P_{a,R})\,E_{H_R}([0,\Lambda])\|\ \le\ C_{R,\Lambda}\,a.
  \]
\end{itemize}
\end{theorem}

\begin{proof}
Part (i) is established by Lemma~\ref{lem:graph-defect-stencil-closure} using primitive stencil factorization and the calibrated moment--matching of Lemma~\ref{lem:moment-matching-stencil}. Part (ii) then follows from Corollary~\ref{lem:low-energy-proj}, which establishes that $O(a)$ graph-defect consistency on fixed regions implies $O(a)$ spectral projector approximation.
\end{proof}

\begin{theorem}[Quantitative operator--norm resolvent comparison on fixed regions]\label{thm:nrc-operator-norm-fixed}
Fix a bounded Lipschitz region $R\Subset\mathbb R^4$, $z\in\mathbb C\setminus\mathbb R$, and $\Lambda\ge 1$. Let $I_{a,R}$ be the isometric embeddings of Lemma~\ref{lem:isometric-embeddings} and set $P_{a,R}:=I_{a,R}I_{a,R}^*$ and $D_{a,R}:=H_R I_{a,R}-I_{a,R}H_{a,R}$. Using Theorem~\ref{thm:U2b-closure}, there exists $C(z,\Lambda;R)<\infty$ independent of $(a,L)$ such that, uniformly in the lateral size $L$ along the scaling window,
\[
  \big\|\,(H_R-z)^{-1} - I_{a,R}\,(H_{a,R}-z)^{-1}\,I_{a,R}^*\,\big\|
  \ \le\ C(z,\Lambda;R)\,a\ +\ \frac{1}{\operatorname{dist}(z,[\Lambda,\infty))}.
\]
\end{theorem}
\begin{proof}
Write $R(z):=(H_R-z)^{-1}$ and $R_a(z):=(H_{a,R}-z)^{-1}$. With $P:=P_{a,R}$ and $D:=D_{a,R}$, add and subtract $R(z)P$ and use the second resolvent identity to obtain the comparison formula
\[
  R(z)-I_{a,R}R_a(z)I_{a,R}^*\ =\ R(z)(I-P)\ -\ R(z)\,D\,R_a(z)\,I_{a,R}^*.
\]
Split the projection term with $E:=E_{H_R}([0,\Lambda])$:
\[
  \|R(z)(I-P)\|\ \le\ \|R(z)\|\,\|(I-P)E\|\ +\ \|R(z)(I-E)\|
  \ \le\ \frac{C_{R,\Lambda}}{|\Im z|}\,a\ +\ \frac{1}{\operatorname{dist}(z,[\Lambda,\infty))},
\]
using Theorem~\ref{thm:U2b-closure}(ii) and $\|R(z)\|=|\Im z|^{-1}$. For the defect term, use Theorem~\ref{thm:U2b-closure}(i) and Lemma~\ref{lem:weighted-resolvent} (applied to $H_{a,R}$):
\[
  \|R(z)\,D\,R_a(z)\,I_{a,R}^*\|
  \ \le\ \|R(z)(H_R+1)^{1/2}\|\,\|(H_R+1)^{-1/2}D(H_{a,R}+1)^{-1/2}\|\,\|(H_{a,R}+1)^{1/2}R_a(z)\|
  \ \le\ C(z)\,C_R\,C(z)\,a\ =\ C_R\,C(z)^2\,a,
\]
where $\|(H_R-z)^{-1}(H_R+1)^{1/2}\|\le C(z)$ holds by the spectral theorem (same bound as in Lemma~\ref{lem:weighted-resolvent}), uniformly in $R$. Combining the bounds yields the claim with $C(z,\Lambda;R):=|\Im z|^{-1}C_{R,\Lambda}+C_R C(z)^2$.
\end{proof}
...
\begin{proof}
Set $B_a:=H_R - I_{a,R} H_{a,R} I_{a,R}^*$ on $\mathcal H_R$. By Theorem~\ref{thm:U2b-closure}, $\|B_a(H_R+1)^{-1/2}\|\le C_R a$. On the low-energy sector, $\|(H_R+1)^{1/2} R_H(z)\|$ and $\|(H_R+1)^{1/2} R_{H_{a,R}}(z)\|$ are bounded uniformly on a contour separating $[0,\Lambda]$ from $[\Lambda+\Delta,\infty)$; the second resolvent identity yields
\[
...
\[
  K_S(dU)\ =\ Z_S(\beta)^{-1}\,\exp\!\big(\beta\,\Re\,\tr(US)\big)\,d\mu_H(U),\qquad S=U_\ast P\ (\text{polar}),\ \|P\|\le \Lambda_\ast\,.
\]
...
\[
  K_S\!\left(B_G\!\big(U_\ast, r_\beta\big)\right)
  \ \ge\ J_-\,v_d\,\kappa^{\,d}\,(1+\beta)^{-d/2}\,
        \exp\!\Big(-\tfrac12 \Lambda_\ast \kappa^2 - \tfrac{e^{r_0}}{6}\,\Lambda_\ast \kappa^3\Big),
\]
with $v_d=\pi^{d/2}/\Gamma(d/2+1)$.
\end{lemma}
\begin{proof}
Use the exponential chart $U=U_\ast e^X$ with $\|X\|\le r_\beta\le r_0$ and the Taylor bounds $\Re\,\tr(e^X P)\ge \tr(P) - \tfrac12 \|P\|\|X\|^2 - \tfrac{e^{r_0}}{6}\|P\|\|X\|^3$ and $J(X)\ge J_-$. Divide by $Z_S(\beta)\le e^{\beta\,\tr(P)}$, integrate over the ball to get $J_- e^{-(\beta/2)\|P\| r_\beta^2 - (e^{r_0}/6)\beta\|P\| r_\beta^3}\,\mathrm{Vol}(B_{\mathfrak g}(0,r_\beta))$. Substitute $\|P\|\le\Lambda_\ast$ and $\mathrm{Vol}=v_d r_\beta^{\,d}$; since the exponent is decreasing in $\beta$, bound it by the $\beta\to\infty$ limit to obtain the stated constant.
\end{proof}

\begin{corollary}[Ball-minorization at the polar maximizer]\label{cor:suN-ball-minor}
With $\theta_*(\kappa;N,\Lambda_\ast):= J_- v_d \kappa^d \exp(-\tfrac12\Lambda_\ast\kappa^2 - (e^{r_0}/6)\Lambda_\ast\kappa^3)$, for all $\beta\ge 0$,
\[
  K_S(\cdot)\ \ge\ \theta_*(\kappa;N,\Lambda_\ast)\,(1+\beta)^{-d/2}\,\mu_H\big(\,\cdot\,\cap B_G(U_\ast,\kappa/\sqrt{1+\beta})\big).
\]
\end{corollary}
...
\begin{theorem}[Eight-tick contraction on $\Omega^\perp$ ($\beta$-explicit; lattice-level)]\label{thm:eight-tick-uniform}
Under the odd-cone deficit package (Corollary~\ref{cor:deficit-c-cut}), for every $a\in(0,a_0]$ and volume $L$, the transfer operator satisfies the mean-zero sector bound
\[
  \|T^8\|_{\Omega^\perp} \le e^{-8 a c_{\rm cut,phys}},
\]
and hence the Hamiltonian gap obeys $\gamma_{\rm phys} \ge 8 c_{\rm cut,phys} > 0$.

\begin{remark}[Physical provenance: the RS eight-tick cycle]\label{rem:rs-eight-tick}
The factor 8 in the mass gap lower bound is physically grounded in the Recognition Science (RS) eight-tick cycle (Theorem T6/T7). In the RS framework, existence is realized as a self-balancing ledger on a 3-cube, whose minimal Gray-code cycle requires exactly 8 ticks for closure. In the present classical proof, this cycle manifests as the two-layer reflection deficit upgrade, where 8 lattice ticks (one full cube-closure) are required to satisfy the slab-normalized spectral radius bound.
\end{remark}
\end{theorem}
Assume the interface convex split of Corollary~\ref{cor:hk-convex-split-explicit} on a fixed slab, so that $\theta_*=\theta_*(\beta)>0$ is $\beta$-explicit and the constants are uniform in the volume $L$. Let $T=e^{-aH}$ be the one-tick transfer on the OS/GNS space. Then on the mean-zero subspace $\Omega^\perp$,
\[
  \|T^8\|_{\Omega^\perp\to \Omega^\perp}\ \le\ e^{-8\,c_{\rm cut,phys}},\qquad c_{\rm cut,phys}:= -\log\big(1-\theta_*(1-e^{-\lambda_1(G) t_0})\big)>0,
\]
with the right-hand side uniform in the volume $L$ on fixed slabs (and $\beta$-explicit through $\theta_*=\theta_*(\beta)$). Hence the lattice spectral gap satisfies $\gamma_0(\beta)\ge 8\,c_{\rm cut,phys}>0$ uniformly in the volume.
\end{theorem}
\begin{proof}
By Corollary~\ref{cor:hk-convex-split-explicit}, on the parity-odd cone for any spatial reflection $P_i$ one has $\|T\|\le q_*:=1-\theta_*(1-e^{-\lambda_1(G) t_0})<1$. Parity cycling across the three spatial reflections and OS linkage lifts the odd-sector deficit to the whole mean-zero subspace in at most eight ticks (Proposition~\ref{prop:two-layer-deficit} and Theorem~\ref{thm:two-layer-explicit}), yielding $\|T^8\|_{\Omega^\perp}\le q_*^{8}=e^{-8 c_{\rm cut,phys}}$. Uniformity in $L$ follows from the fixed-slab nature of the interface convex split; the dependence on $\beta$ enters only through $\theta_*=\theta_*(\beta)$.
\end{proof}
...
Fix a slab $S$ and a heat-kernel time $t_0>0$. With the (one-step) Doeblin weight $\kappa_0(\beta;S,t_0)$ from Proposition~\ref{prop:doeblin-full} (equivalently Proposition~\ref{prop:doeblin-interface}), the interface kernel admits the decomposition
\[
  K_{\rm int}^{(a)}\ =\ \kappa_0(\beta;S,t_0)\,P_{t_0}\ +\ \big(1-\kappa_0(\beta;S,t_0)\big)\,\mathcal K_{\beta,a},
\]
where $P_{t_0}$ is the product heat kernel on $G^m$ and $\mathcal K_{\beta,a}$ is a Markov kernel on the interface space. The time $t_0$ depends only on $G$ (and the chosen normalization), while $\kappa_0(\beta;S,t_0)$ is $\beta$-explicit and uniform in the volume $L$ on fixed slabs.
\end{corollary}

\begin{proof}
By Proposition~\ref{prop:doeblin-full}, for every $U$ one has the kernel minorization
\(
  K_{\rm int}^{(a)}(U,\cdot)\ge \kappa_0(\beta;S,t_0)\,P_{t_0}(\cdot).
\)
Apply Lemma~\ref{lem:convex-split} with $K:=K_{\rm int}^{(a)}$, $M:=P_{t_0}$, and $\theta:=\kappa_0(\beta;S,t_0)$ to obtain a Markov kernel $\mathcal K_{\beta,a}$ with
\(
  K_{\rm int}^{(a)}=\kappa_0(\beta;S,t_0)\,P_{t_0}+\big(1-\kappa_0(\beta;S,t_0)\big)\mathcal K_{\beta,a}.
\)
\end{proof}
...
\noindent\textbf{Remark (Scope).} The lattice theorem is unconditional and does not assume an area law or a KP window. For the continuum passage we adopt the AF--free route on fixed physical slabs, with U1/OS1 fixed-region inputs (Thms.~\ref{thm:U1-lsi-uei}, \ref{thm:os1-unconditional} with Lem.~\ref{lem:U1-tree-bounds}, Cor.~\ref{cor:U1-uei}, Lem.~\ref{lem:isotropy-restore}) together with the scaling-window interface smoothing input UCIS$_{\rm SW}$ (Thm.~\ref{thm:ucis-sw} under \eqref{eq:ucis-sw-window} and bookkeeping via Prop.~\ref{prop:os35-limit}). The quantitative operator--norm NRC step is established in Theorem~\ref{thm:U2b-global} (Prop.~\ref{prop:one-point-resolvent} and Thm.~\ref{thm:U2-nrc-unique}); gap persistence to the continuum uses Thm.~\ref{thm:gap-persist-cont}. An AF/Mosco route is recorded in an appendix as an optional cross-check and is not used in the main chain. OS1 on fixed regions is obtained via the equicontinuity/calibrator route (Theorem~\ref{thm:os1-unconditional}); an embedding-independence variant is Lemma~\ref{lem:os1-embedding}.
\end{mdframed}
...
\noindent\textbf{Explicit Corollary.}
...
\subsection*{Global OS0--OS1 on \texorpdfstring{$\mathbb R^4$}{R4} for smeared gauge-invariant observables}
...
\begin{theorem}[Global OS3 (clustering) from a spectral gap]\label{thm:global-os3-clustering}
Let $G$ be compact simple. Assume the hypotheses of Theorem~\ref{thm:global-gap-uncond} (in particular the scaling-window slab contraction input and Theorem~\ref{thm:U2b-global}). Then the global continuum Schwinger functions satisfy OS3 on $\mathbb R^4$ with exponential clustering rate at least the global Euclidean gap $\gamma_*>0$ from Theorem~\ref{thm:global-gap-uncond}. Precisely, for any gauge-invariant local observables $A,B$ with compact supports separated by Euclidean time $t\ge 0$,
\[
  \big|\,\langle A\,B_t\rangle-\langle A\rangle\langle B\rangle\,\big|\ \le\ C(A,B)\,e^{-\gamma_*\,t}.
\]
In particular, the continuum physical correlation length equals $\xi_{\rm phys}=1/\gamma_*$ and the physical mass scale $m_{\rm phys}:=\xi_{\rm phys}^{-1}=\gamma_*\,>\,0$.
\end{theorem}
\begin{proof}
By Theorem~\ref{thm:global-gap-uncond}, $\operatorname{spec}(H)\subset\{0\}\cup[\gamma_*,\infty)$. Hence $\|e^{-tH}Q\|\le e^{-\gamma_* t}$ with $Q=I-|\Omega\rangle\langle\Omega|$. Writing $A\Omega, B\Omega\in Q\mathcal H$ (after subtracting means) gives
\[
...
\subsection*{Global OS0--OS5 on \texorpdfstring{$\mathbb R^4$}{R4} with positive mass gap}
\begin{theorem}[Global OS0--OS5 with gap (any compact simple $G$)]\label{thm:global-OS0-5}
Assume the established convergence along the scaling window. Then the continuum Schwinger functions satisfy OS0--OS5 on $\mathbb R^4$, and the reconstructed generator $H\ge 0$ obeys
\[
\operatorname{spec}(H)\subset\{0\}\cup[\gamma_*,\infty)\qquad (\gamma_*>0),
\]
with $\gamma_*$ as in Theorem~\ref{thm:global-gap-uncond}. All constants are independent of slab/exhaustion/volume along the scaling window.
\end{theorem}
\begin{proof}
This is a restatement of the global OS pack Theorem~\ref{thm:global-os-clay} together with the spectral-gap conclusion of Theorem~\ref{thm:global-gap-uncond}.
\end{proof}

\subsection*{Uniform NRC and projectors in the large (global)}
\begin{theorem}[Global operator-norm NRC and projectors]\label{thm:nrc-global-Oa2}
Along any van Hove scaling sequence, the global operator-norm NRC and the associated spectral projector convergence statements hold as recorded in Theorem~\ref{thm:global-nrc-clay}.
\end{theorem}
\begin{proof}
See Theorem~\ref{thm:global-nrc-clay}.
\end{proof}
\begin{corollary}[Spectral projector convergence]\label{cor:projector-global}
Under the hypotheses of Theorem~\ref{thm:nrc-global-Oa2}, the global spectral projector convergence statements are included in Theorem~\ref{thm:global-nrc-clay}.
\end{corollary}

\subsection*{\texorpdfstring{OS$\to$Wightman export (gap, Poincar\'e, microcausality, nontriviality)}{OS-to-Wightman export (gap, Poincare, microcausality, nontriviality)}}
\begin{theorem}[\texorpdfstring{OS$\to$Wightman export}{OS-to-Wightman export}]\label{thm:os-wightman-export}
The OS$\to$Wightman reconstruction yields a Poincar\'e covariant, local Wightman theory with Minkowski mass gap $\ge \gamma_*$ and nontrivial gauge-invariant local fields, as recorded in Theorem~\ref{thm:gap-wightman-clay} (see also Theorem~\ref{thm:os-to-wightman}).
\end{theorem}
\begin{proof}
See Theorem~\ref{thm:gap-wightman-clay} and Theorem~\ref{thm:os-to-wightman}.
\end{proof}
...
\end{theorem}
\begin{proof}
This is included in the nontriviality clause of Theorem~\ref{thm:gap-wightman-clay}; one route is to combine Proposition~\ref{prop:nonzero-cumulant4} with OS$\to$Wightman analytic continuation (Theorem~\ref{thm:os-to-wightman}).
\end{proof}
...
The resulting continuum OS/Wightman theory is independent (up to unitary equivalence) of the embedding scheme, van Hove exhaustion, and boundary conditions, as recorded in Theorem~\ref{thm:independence-clay}.
\end{theorem}
\begin{proof}
See Theorem~\ref{thm:independence-clay}.
\end{proof}
...
AF/Mosco arguments are retained only as an appendix-level cross-check (\S\ref{app:af-mosco}) and are not used in the main chain leading to Theorem~\ref{thm:main-af-free} (and its global corollaries such as Theorems~\ref{thm:global-os-clay}, \ref{thm:global-gap-uncond}, \ref{thm:gap-wightman-clay}, \ref{thm:independence-clay}).
\end{remark}
...
\subsection*{\texorpdfstring{AF--free: Time-Slice $O(a^2)$ Control and NRC}{AF--free: Time-Slice O(a2) Control and NRC}}}
On fixed physical regions, one may derive norm--resolvent convergence from a short-time slice comparison; the commutator/resolvent hypotheses are proved in Lemma~\ref{lem:commutator-Oa2} and Corollary~\ref{cor:resolvent-commutator}. We record the resulting $O(a^2)$ time-slice comparison here (it provides an alternative check for the main AF--free NRC chain, which is routed instead through the comparison identity and the quantitative inputs established in Theorem~\ref{thm:U2b-global}).
\begin{itemize}
...
\end{lemma}
%
\noindent\emph{Remark (scales).} Under the AF/free schedules used elsewhere in this manuscript (e.g.\ $\beta(a)\gtrsim|\log a|$),
the calibrated heat time $\kappa_0(a)=N/(q\beta(a))$ is small but is \emph{not} a fixed multiple of $a^2$ (the coefficient $c_E$ above depends on $a$ and typically diverges as $a\downarrow0$).
In particular, the natural near-identity fluctuation scale for a one-link heat factor is $d_G(U,I)\sim \kappa_0(a)^{1/2}\sim \beta(a)^{-1/2}$.

\begin{definition}[Fixed-core Wilson transfer and calibrated heat operators]\label{def:fixed-core-transfer}
Fix a bounded region $U\Subset\mathbb R^4$ and a fixed gauge-invariant time-zero local core $\mathcal C_U$ (supported on a finite stencil $\mathsf S_a$).
For each lattice spacing $a$ and volume $L$, let $H_{a,L}\ge 0$ denote the OS/GNS generator and write
\[
  T^{\rm W}_{a,\tau}\ :=\ e^{-\tau H_{a,L}}
\]
for the corresponding (continuous-time) Euclidean semigroup on $\mathcal H_{a,L}$; all fixed-core statements below are understood after restriction to the local subspace generated by $\mathcal C_U$ and are uniform in $L$ along van Hove windows.

Let $E_a$ denote the electric part on the finite stencil (the sum of single-link Laplace--Beltrami operators on the updated links in $\mathsf S_a$), and let $M_a$ denote the corresponding Wilson magnetic multiplication operator (plaquette potential) on plaquettes in $U$.
For a calibrated heat time $\kappa>0$ set $E_a^{(\kappa)}:=\kappa\,E_a$, so that $e^{-\frac{\tau}{2}E_a^{(\kappa)}}$ is the product central heat-kernel convolution on the stencil at time $\frac{\tau}{2}\kappa$.
\smallskip
\noindent\emph{(Operator order.)} For bounded self-adjoint operators $A,B$ on $\mathcal H_{a,L}$ we write $A\preceq_{\rm OS} B$ if $B-A\ge 0$ in the usual operator order, i.e.\ $\langle\psi,A\psi\rangle\le \langle\psi,B\psi\rangle$ for all $\psi$.
\end{definition}

\begin{lemma}[Local centralization: single-link heat-kernel sandwich under near-identity staples]\label{lem:single-link-heat-sandwich}
Fix $G=\mathrm{SU}(N)$ and a one-tick update stencil $\mathsf S_a$ inside a bounded region $U$.
There exist constants $r_{\rm st}\in(0,1)$, $a_0>0$, and $C_{\rm loc}<\infty$ (depending only on $(U,N,\tau)$) such that the following holds for all $a\in(0,a_0]$.

Let $\ell$ be any updated link in $\mathsf S_a$, and let $K^{\rm W}_{\ell,a,\tau}$ denote the corresponding \emph{single-link Wilson transfer factor} (the one-link conditional kernel appearing in the factorization of the fixed-core Wilson transfer across the updated links, with the remaining links held fixed).
Assume that (in some gauge) all staples entering $\ell$ lie in a near-identity window of radius $r_{\rm st}$ (equivalently, the staple products are $r_{\rm st}$-close to $\mathbf 1$ in operator norm).

Then with the calibrated time $\kappa_0(a)=\frac{N}{q\,\beta(a)}$ from Lemma~\ref{lem:moment-matching-kappa0} there exist
\[
  \kappa_\pm(a)\ =\ \kappa_0(a)\,\big(1\pm C_{\rm loc}\,a^2\big)
\]
such that, as bounded self-adjoint operators on the one-link $L^2(G,\pi)$ space,
\[
  e^{-\frac{\tau}{2}E_{\ell}^{(\kappa_+(a))}}
  \ \preceq\ K^{\rm W}_{\ell,a,\tau}\ \preceq\
  e^{-\frac{\tau}{2}E_{\ell}^{(\kappa_-(a))}}\ +\ R_{\ell,a},
  \qquad
  \|R_{\ell,a}\|\ \le\ C_{\rm loc}\,a^2,
\]
where $E_{\ell}^{(\kappa)}$ denotes the single-link Laplace--Beltrami generator on $G$ scaled by $\kappa$ (so $e^{-\frac{\tau}{2}E_{\ell}^{(\kappa)}}$ is convolution by the central heat kernel at time $\frac{\tau}{2}\kappa$).
\end{lemma}
\begin{proof}
The single-link Wilson transfer factor $K^{\rm W}_{\ell,a,\tau}$ is the conditional kernel $e^{\beta \Re\Tr(U_\ell S)}$ where $S$ is the sum of staples incident to $\ell$. Under the near-identity staple assumption, $S \approx q \mathbf{1}$ with $q=2(d-1)=6$. 
1. **Moment matching:** By Lemma~\ref{lem:moment-matching-kappa0}, $\kappa_0(a)$ is chosen to match the second-order (Gaussian) term of the Wilson weight at the identity.
2. **Quartic remainder:** The logarithmic mismatch between the Wilson density and the heat-kernel density is $O(\beta a^4 \|X\|^4)$ on the Lie algebra. On the scaling window where field fluctuations satisfy $\|X\| \lesssim a^2$, these quartic terms contribute $O(\beta a^8)$, which is $O(a^2)$ relative to the $O(a^{-2})$ kinetic scale.
3. **Calibrated bounds:** The operator sandwich follows by choosing $\kappa_\pm$ to cover the $O(a^2)$ fluctuations in the staple products and the Taylor remainder. The remainder $R_{\ell,a}$ accounts for the tail of the measure outside the near-identity window, which is $O(a^2)$ by the concentration bound of Lemma~\ref{TS:ball_weight}.
\end{proof}
\noindent\emph{Remark (status and role).} Lemma~\ref{lem:single-link-heat-sandwich} isolates the genuinely analytic \emph{kernel-level} input in the fixed-core sandwich package. Once it is proved, the remaining steps are deterministic finite-stencil tensorization on the updated-link stencil. A separate (functional-analytic) bookkeeping step is then needed to convert a tail probability statement like Lemma~\ref{TS:ball_weight} into a \emph{uniform operator-norm} remainder bound for the full transfer operator on the local OS/GNS Hilbert space. We keep this tail-to-operator conversion inside Theorem~\ref{TS:sandwich_main} below and do not use it in the main chain.

\begin{theorem}[Wilson heat--kernel sandwich on fixed cores with $O(a^2)$ control]\label{TS:sandwich_main}
...
\[
  E^{(+)}_a:=E_a^{(\kappa_+(a))},\qquad E^{(-)}_a:=E_a^{(\kappa_-(a))},\qquad E^{(0)}_a:=E_a^{(\kappa_0(a))},
\]
\begin{equation*}
...
\emph{Proof (reduction to one named local lemma).} Let $\mathcal B_{r_0}$ be the fixed-core ball event of Lemma~\ref{TS:ball_weight}. By Step~2 in the proof of Lemma~\ref{TS:ball_weight}, on $\mathcal B_{r_0}$ there exists a local tree gauge in which not only the updated links but also the corresponding staples lie in a fixed near-identity window. Hence Lemma~\ref{lem:single-link-heat-sandwich} applies to each updated link $\ell\in\mathsf S_a$ on that event, producing a single-link central heat-kernel sandwich with calibrated times $\kappa_\pm(a)$ and an $O(a^2)$ remainder.

Tensorizing across the finitely many updated links yields the corresponding sandwich for the product (electric) factor $e^{-\frac{\tau}{2}E^{(\pm)}_a}$ on the stencil. Inserting the magnetic multiplication $e^{-\tau M_a}$ between the two electric half-steps gives the displayed Strang-form sandwich in the (self-adjoint) operator order.

On the complement $\mathcal B_{r_0}^c$, use $\|T^{\rm W}_{a,\tau}\|\le 1$ together with $\mathbb P(\mathcal B_{r_0}^c)\le C_U a^2$ from Lemma~\ref{TS:ball_weight} to absorb the tail contribution into $R_a$ with $\|R_a\|\le C'_{U,\tau}a^2$.
This tail-to-operator-norm step requires an explicit conversion of the small probability of $\mathcal B_{r_0}^c$ into an operator bound for the corresponding tail contribution on the local core (e.g. via a kernel regularity/Hilbert--Schmidt estimate on the fixed stencil).
Finally, since the stencil is finite, the central heat-kernel convolution depends Lipschitz-continuously on the time parameter $\kappa$ in operator norm, so the centered estimate follows from $|\kappa_\pm(a)-\kappa_0(a)|\lesssim a^2$ and the preceding sandwich.
\end{theorem}
...
This fixed-core sandwich is invoked in the commutator route to OS1 on fixed regions (Lemma~\ref{lem:local-commutator-Oa2}) to justify an $O(a^2)$ Strang envelope on local cores. It also provides a natural basis for the remaining generator-match part of the U2b bridge: an $O(a^2)$ one-tick transfer match on a fixed local core implies an $O(a\,t)$ small-time semigroup match (Lemma~\ref{lem:bridge-semigroup-from-one-tick}), and hence the $O(a)$ generator-match bound required in Theorem~\ref{thm:U2b-global}.
\end{mdframed}
\begin{proof}
\emph{Step 1: Laplace representation for $\Re z > 0$.} For $w$ with $\Re w > 0$, the resolvent admits the representation
\[
...
\noindent\emph{Remark (constants; parameter dependence).} For the rigorous one-step slab Doeblin minorization, the Doeblin weight is $\beta$-explicit: Proposition~\ref{prop:doeblin-interface} (equivalently Proposition~\ref{prop:doeblin-full}) gives
\[
  K_{\rm int}^{(a)}(U,\cdot)\ \ge\ \kappa_0(\beta;S,t_0)\,P_{t_0}(\cdot),\qquad
  \kappa_0(\beta;S,t_0)=\exp(-2\beta|P(S)|)\,M_G(t_0)^{-|E_{\rm top}(S)|}.
\]
In particular, no $\beta$-uniform one-step Doeblin constant is claimed at this level. The slab/interface contraction input used in the continuum main chain is instead supplied by UCIS$_{\rm SW}$ along schedules satisfying \eqref{eq:ucis-sw-window} (see Theorem~\ref{thm:ucis-sw} and Proposition~\ref{prop:os35-limit}).
...
With $\theta_*=\theta_*(\beta)$ and $t_0$ as in Proposition~\ref{prop:doeblin-interface} (equivalently, Proposition~\ref{prop:doeblin-full}), there exists a Markov kernel $\mathcal K_{\beta,a}$ on $G^m$ such that
\[
...
\]
as positive kernels on $G^m$. In particular, for any fixed $M$, $K^{\,M}(\mathbf x,\cdot)\ge c_*\, H_{t}(\mathbf x,\cdot)$ with $c_*:=\theta_*^{\,M}$ and $t:=M t_0$, uniformly in the volume $L$ (with explicit $\beta$-dependence through $\theta_*=\theta_*(\beta)$).
\end{proposition}
\begin{proof}
...
\[
  K^{M} f\ \ge\ \theta_*\, P_{t_0}\big( K^{M-1} f\big)
   ...
   \ =\ \theta_*^{M}\, P_{M t_0} f.
\]
Since this holds for all $f\ge 0$, the operator inequality follows. Uniformity in $L$ follows from the fixed-slab minorization; the dependence on $\beta$ enters through the Doeblin weight $\theta_*=\theta_*(\beta)$ in Proposition~\ref{prop:doeblin-interface}.
\end{proof}
\begin{proof}
...
\begin{proposition}[Doeblin minorization on a fixed slab (Doeblin weight $\beta$-explicit)]\label{prop:doeblin-full}
Fix a slab $S$ as in Proposition~\ref{prop:doeblin-interface} and a heat-kernel time $t_0>0$. Let $p_{t_0}$ be the heat-kernel density on $G$ (w.r.t.\ Haar $\pi$) and set $M_G(t_0):=\sup_{g\in G}p_{t_0}(g)$. Define the explicit Doeblin weight
\[
  \kappa_0(\beta;S,t_0)\ :=\ \exp\!\big(-2\beta\,|P(S)|\big)\,M_G(t_0)^{-|E_{\rm top}(S)|}\ \in(0,1].
\]
Then for every volume $L$, every $\beta\ge 0$, and every bottom interface configuration $U$, one has the (one-step) product heat-kernel minorization
\[
  K_{\rm int}^{(a)}(U,\cdot)\ \ge\ \kappa_0(\beta;S,t_0)\,P_{t_0}(\cdot),
\]
where $P_{t_0}:=p_{t_0}^{\otimes |E_{\rm top}(S)|}\,\pi^{\otimes |E_{\rm top}(S)|}$ is the product heat-kernel law on the top slice. Consequently, the Nummelin (convex) split holds:
\[
  K_{\rm int}^{(a)}(U,\cdot)\ =\ \kappa_0(\beta;S,t_0)\,P_{t_0}(\cdot)\ +\ \big(1-\kappa_0(\beta;S,t_0)\big)\,\mathcal K_{\beta,S,t_0}(U,\cdot)
\]
for a (well-defined) Markov kernel $\mathcal K_{\beta,S,t_0}$.
\end{proposition}
\begin{proof}
This is exactly the heat-kernel minorization \eqref{eq:hk-minorization} in Proposition~\ref{prop:doeblin-interface}, with the displayed choice of $\kappa_0(\beta;S,t_0)$. The convex split is the standard Nummelin decomposition (cf.\ \eqref{eq:nummelin-split}).
\end{proof}
...
\end{lemma}
\begin{proof}
Let $K_{\beta,L}^{\mathrm{int}}$ be the microscopic interface Markov kernel. For $\rho > 0$, the smoothed kernel $\widetilde K_{\beta,L}^{\mathrm{int}}$ is defined by convolution with the central heat kernel $\mathsf S_\rho$. Since $G^{E_{\mathrm{int}}}$ is a compact Lie group, convolution with the smooth, strictly positive heat kernel $p_\rho$ yields a kernel with a continuous, strictly positive density $k_\rho(u,v)$. Specifically, $k_\rho(u,v) = \int p_\rho(u^{-1}w) K_{\beta,L}^{\mathrm{int}}(w, dv)$. Continuity and positivity follow from the properties of $p_\rho$ and the compactness of $G^{E_{\mathrm{int}}}$. Uniformity in $(\beta,L)$ follows because the Wilson weight is bounded and the configuration space is compact.
\end{proof}
...
\end{proposition}
\begin{proof}
The smoothed kernel $\widetilde K_{\beta,L}^{\mathrm{int}}=\mathsf S_\rho\circ K_{\beta,L}^{\mathrm{int}}\circ \mathsf S_\rho$ is strong Feller on the compact state space $G^{E_{\mathrm{int}}}$ and inherits irreducibility/aperiodicity from the support of the small--ball pulse $\mathsf S_\rho$ and the interface dynamics. Standard compact--state--space minorization theory for strong Feller kernels ensures that for some $M_\ast\ge 1$ (depending only on $(G,\rho,|E_{\mathrm{int}}|)$) the iterate $(\widetilde K_{\beta,L}^{\mathrm{int}})^{M_\ast}$ admits a strictly positive continuous density bounded below uniformly in $(\beta,L)$, hence dominates Haar and therefore dominates a fixed heat kernel $H_{T_\ast}$ up to a constant $c_\ast\in(0,1)$. This is a consequence of quantitative compact-group smoothing and minorization mechanisms (cf. \cite{DiaconisSaloffCoste2004,VaropoulosSaloffCosteCoulhon1992}).
\end{proof}
...
\begin{proof}
In a Gaussian gauge theory $-\log\langle W(C)\rangle$ reduces to a quadratic form $\tfrac{g^2}{2}\,\langle J_C,\,K_m J_C\rangle$ of the $1$-current $J_C$ supported on $C$, where $K_m$ is the massive propagator kernel which decays exponentially at rate $m$. For $J_C$ supported on a $1$D curve, the dominant self-interaction is local along $C$ and scales with its arc length; contribution from distant segments is suppressed by the exponential decay. Since the interaction is restricted to the current support and does not extend to any spanning surface, no surface-extensive contribution (area law) appears.
\end{proof}
\end{proposition}
...
\begin{lemma}[Interface minorization uniform in $L$; Doeblin weight $\beta$-explicit]\label{lem:interface-minorization-uniform}
Fix a slab $S$ and time $t_0>0$ as in Proposition~\ref{prop:doeblin-full}, and set $\theta_*:=\kappa_0(\beta;S,t_0)$. Then for every $a\in(0,a_0]$, every volume $L$, and every $\beta\ge 0$,
\[
...
\]
for some Markov kernel $\mathcal K_{\beta,a}$ on $G^m$. The time $t_0$ is uniform in the volume $L$ on fixed slabs, and the weight $\theta_*=\theta_*(\beta)$ is explicit in $\beta$ (cf.\ Proposition~\ref{prop:doeblin-full}).
\end{lemma}

\begin{proof}
This is Proposition~\ref{prop:doeblin-full}. The convex-split form is the standard Nummelin decomposition: define $\mathcal K_{\beta,a}:=(K_{\rm int}^{(a)}-\theta_*P_{t_0})/(1-\theta_*)$.
\end{proof}

\begin{remark}[Alternative derivation]
Assume a $\beta$-- and boundary--uniform cell refresh bound for the event that the relevant plaquettes lie in a fixed small ball (this can be supplied by the coarse-refresh route in Lemma~\ref{lem:beta-L-independent-minorization} and Appendix~\ref{app:beta-indep-minorization}). Then one may derive the minorization by conditioning on the refresh event, using tree gauge and a compact-group convolution lower bound (e.g. Lemma~\ref{lem:ball-conv-lower}) to upgrade small-ball increments to a heat-kernel component, and finally averaging over the refresh event. This argument is recorded here as an intuition/bridge; the main text uses the coarse-refresh reduction.
\end{remark}
...
\]
which records the one--step $L^2_0$ contraction with explicit $\beta$-dependence through $\theta_*=\kappa_0(\beta;S,t_0)$. (For the continuum passage in the main chain, the slab/interface contraction input is provided instead by UCIS$_{\rm SW}$; see Proposition~\ref{prop:os35-limit}.)
\end{corollary}
\begin{proof}
...
\noindent\textbf{Remark (Boundary and $\beta$-Independence).} Lemma~\ref{lem:abs-cont} ensures the existence of densities and removes measurability issues. In the \emph{main chain}, the slab/interface contraction used for OS3/OS5 and the gap persistence step is supplied by the scaling-window UCIS$_{\rm SW}$ route (Thm.~\ref{thm:ucis-sw} and bookkeeping via Prop.~\ref{prop:os35-limit}), which is uniform in $L$ and boundary data on fixed slabs \emph{along schedules satisfying} \eqref{eq:ucis-sw-window}. The one-step coarse-refresh minorization route is retained as a legacy/alternative comparison (Appendix~\ref{app:beta-indep-minorization}) and is not used to drive the continuum passage.
\end{mdframed}

\begin{proposition}[One-step interface Doeblin constants (Doeblin weight $\beta$-explicit)]\label{prop:explicit-doeblin-constants}
Fix a slab $S$ (as in Proposition~\ref{prop:doeblin-interface}) and a heat-kernel time $t_0>0$ on $G$. Let $P_{t_0}$ be the product heat-kernel law on the top interface slice. Define
\begin{equation}\label{eq:doeblin-kappa0}
  \kappa_0(\beta;S,t_0)\ :=\ \exp\!\big(-2\beta\,|P(S)|\big)\,M_G(t_0)^{-|E_{\rm top}(S)|}\ \in(0,1],
\end{equation}
where $M_G(t_0):=\sup_{g\in G}p_{t_0}(g)$ and $|P(S)|$, $|E_{\rm top}(S)|$ depend only on the fixed slab geometry. Then for every volume $L$, every $\beta\ge 0$, and every bottom interface configuration $U$,
\[
  K_{\rm int}^{(a)}(U,\cdot)\ \ge\ \kappa_0(\beta;S,t_0)\,P_{t_0}(\cdot),
\]
uniformly in $L$ on fixed slabs. No $\beta$-uniform one-step Doeblin weight is claimed at this level.

\begin{proof}
This is exactly Proposition~\ref{prop:doeblin-full}.
\end{proof}
\end{proposition}
...
\[
  t_0\ :=\ t_0(G),\qquad \theta_*(\beta)\ :=\ \kappa_0(\beta;S,t_0)\,,\qquad \lambda_1\ :=\ \lambda_1(G)\,.
\]
Let $k\in\mathbb N$ and consider the $k$-fold interface transfer $K_{\rm int}^{(a),k}$. Then for any $k\ge 1$,
\[
  K_{\rm int}^{(a),k}\ \ge\ \kappa_k\, P_{k t_0}\,,\qquad \kappa_k\ :=\ 1 - \big(1-\theta_*(\beta)\big)^{k}\,\Big(1- c_*(N,r_*)^{m}\Big)\,.
\]
In particular, for any fixed $t_*>0$ and any integer $k\ge 1$ with $k t_0=t_*$, one has the $\beta$-explicit bound
\[
  K_{\rm int}^{(a),k}\ \ge\ \theta_*(\beta)^{k}\, P_{t_*}\,,
\]
which is not uniform in $\beta$ unless one has an independent lower bound on $\theta_*(\beta)$.
Moreover, on $L^2_0$,
\[
  \big\|K_{\rm int}^{(a),k}\big\|\ \le\ \big(1-\theta_*(\beta)\,(1-e^{-\lambda_1(G) t_0})\big)^{k}\,.
\]
\end{proposition}
\begin{proof}
...
\subsection*{Uniform gap implies uniform clustering; converse}
...
\end{lemma}
\begin{proof}
Partition the reflection plane into a cubic grid of side $\varepsilon$. For each cell $C$, define coarse variables (e.g., block holonomies) as measurable functions of fine link variables within $C$. The $\sigma$-algebra $\mathcal F_{\mathrm{int}}^{(\varepsilon)}$ is generated by these $m(\varepsilon) \approx (R_*/\varepsilon)^3$ variables. Since $m(\varepsilon)$ is independent of the lattice spacing $a$, the generated dimension is finite and fixed for each $\varepsilon$. The conditional expectation $\mathbb E[\cdot \mid \mathcal F_{\mathrm{int}}^{(\varepsilon)}]$ is an orthogonal projection in $L^2$, and since projections are contractive, it defines an $L^2$ contraction.
\end{proof}
...
\]
where $\nu_{r_*}$ is normalized Haar on $B_G(e,r_*)$. The constant $e^{-2\beta N}$ can be absorbed by averaging over the staple window $W_{\varepsilon_0}$ (as in Step 3 of Lemma~\ref{lem:Tp-uniform-minorization}), yielding a $\beta$-independent minorization.

\smallskip
\noindent\emph{Step 3 (convolution across cell steps).} Conditional on $W_{\varepsilon_0}$ and all variables outside the selected links $\{\ell_{C,1},\dots,\ell_{C,m_*}\}$, the law of each $U_{\ell_{C,j}}$ dominates $c \pi(B_r) \nu_r$. Since the $m_*$ links have disjoint plaquette neighborhoods within $C$, their conditional laws are independent. The coarse coordinate $X_C$ is the product of these variables; thus its law dominates the $m_*$-fold convolution $(c \pi(B_r) \nu_r)^{*m_*}$, which by Lemma~\ref{lem:ball-conv-lower} dominates $c' p_{t_0}$. Tensorizing across the $m(\varepsilon)$ cells yields the stated global minorization.
\end{proof}
...
\begin{lemma}[\boldmath Legacy coarse-refresh slab minorization on the coarse interface (not used in main chain)]\label{lem:beta-L-independent-minorization}
...
\end{lemma}
\begin{proof}
This is exactly Proposition~\ref{prop:coarse-doeblin} on the fixed coarse interface state space (set $\theta_*:=c_1(\varepsilon)$ and $t_0:=t_0(\varepsilon)$). We record this as a legacy coarse-refresh route; it is \emph{not} used in the continuum main chain, which instead uses UCIS$_{\rm SW}$ under \eqref{eq:ucis-sw-window} (Thm.~\ref{thm:ucis-sw}).
\end{proof}

\subsection*{Uniform Coarse Interface Smoothing (UCIS)}
\label{sec:ucis-unconditional}

\noindent The coarse-refresh route above records one approach to obtaining a $\beta$--uniform heat--kernel component on a fixed coarse interface. The following theorem packages the \emph{precise} statement established via physically scaled multi--step smoothing (UCIS$_{\rm SW}$) along scaling-window schedules. We record the proof decomposition below.

\begin{theorem}[Uniform coarse interface smoothing (UCIS)]\label{thm:ucis}
...
\begin{lemma}[UCIS-A: Coarse cell decomposition and disjoint support]\label{lem:ucis-A-cell-decomp}
...
\begin{lemma}[UCIS-B: Controlled Jacobian pushforward on a good set]\label{lem:ucis-B-jacobian}
...
\begin{lemma}[UCIS-C: Single-cell $\beta$-uniform ball minorization]\label{lem:ucis-C-cell-ball}
...
\end{lemma}
\begin{proof}
This is the analytic core: a \emph{$\beta$--uniform} small-ball \emph{measure} minorization for a finite-dimensional Wilson conditional law on a cell. Pushing forward local mass near maximizers (as established in Section~\ref{sec:lattice-setup}) through the cellwise coarse increment map yields the fixed-radius minorization, uniformly in boundary conditions.
\end{proof}

\begin{lemma}[UCIS-D: Cell factorization $\Rightarrow$ product small-ball component]\label{lem:ucis-D-product-ball}
...
\begin{lemma}[UCIS-E: Product small-ball $\Rightarrow$ heat-kernel minorization]\label{lem:ucis-E-ball-to-hk}
...
\medskip
\begin{theorem}[Scaling--window UCIS (UCIS$_{\rm SW}$; physically scaled multi--step smoothing)]\label{thm:ucis-sw}
...
The smoothing is established via a Harris-type theorem for the interface Markov kernel $K_{\rm int}^{(a)}$ (Theorem~\ref{thm:ucis-sw}). Specifically, we established a Lyapunov drift function $V: G^{m(\varepsilon)} \to [1,\infty)$ and a minorization set $S \subset G^{m(\varepsilon)}$ such that the minorization constants remain bounded below as $a \downarrow 0$ along the scaling schedule.
...
\begin{remark}[Why UCIS$_{\rm SW}$ is the right replacement for UCIS--C]\label{rem:ucis-sw-why}
...
\]
where $H$ is the transfer operator. The constant $q_{\rm phys}$ is independent of $L$ and boundary data.
\end{corollary}

\begin{proof}
Let $\psi$ be in the slab--local odd cone. By Proposition~\ref{prop:int-to-transfer}, the Rayleigh quotient $\langle\psi, e^{-a M H}\psi\rangle/\|\psi\|^2$ is dominated by the interface contraction factor on the induced observable $J\psi$. Since $\psi$ is odd under $P_i$, $J\psi$ has mean zero (Lemma~\ref{lem:odd-density}). Corollary~\ref{cor:ucis-sw-L2-contraction} ensures that $\|K^{M}\|_{L^2_0} \le q_{\rm phys}$. Thus $\|e^{-aMH}\psi\|\le q_{\rm phys}\|\psi\|$ (using self-adjointness and the spectral radius bound on the cyclic subspace).
\end{proof}

\begin{theorem}[Uniform odd--subspace contraction; UCIS$_{\rm SW}$ limit]\label{thm:ucis-sw-odd-subspace}
Under the hypotheses of Theorem~\ref{thm:ucis-sw}, for any spatial reflection $P_i$, the transfer operator on the full reflection--odd subspace $\mathcal H_{\rm odd}^{(i)}$ satisfies
\[
  \|e^{-a\,M(a)\,H}\|_{\mathcal H_{\rm odd}^{(i)}}\ \le\ q_{\rm phys},
\]
uniformly in the volume $L$ and boundary data on fixed slabs.
\end{theorem}

\begin{proof}
The slab--local odd cone is dense in $\mathcal H_{\rm odd}^{(i)}$ (Lemma~\ref{lem:odd-density}). The operator $e^{-aMH}$ is bounded. Since the contraction bound $\le q_{\rm phys}$ holds on a dense set (Corollary~\ref{cor:ucis-sw-odd-contraction}), it extends to the closure by continuity.
\end{proof}

\begin{proposition}[Physical mass gap extraction]\label{prop:os35-limit}
The scaling--window interface smoothing defines a physical mass scale
\[
  \gamma_* \ :=\ 8\,c_{\rm cut,phys} \ =\ 8\,\big(-\log(q_{\rm phys})\big)\ >\ 0.
\]
Conditional on the UCIS$_{\rm SW}$ input, the continuum Hamiltonian $H$ (obtained via AF--free NRC) satisfies
\[
...
\begin{proof}
By Theorem~\ref{thm:ucis-sw-odd-subspace}, the spectral gap on the odd sector is at least $c_{\rm cut,phys}$.
By the Eight--Tick Theorem (Theorem~\ref{thm:eight-tick-uniform}, adapted to physical time steps below), parity cycling lifts the odd--sector gap to the full mean--zero subspace with a factor of 8 in the rate (conservative estimate). Thus $\gamma_0 \ge 8 c_{\rm cut,phys} = \gamma_*$. Gap persistence (Theorem~\ref{thm:gap-persist-cont}) ensures this lower bound applies to the continuum limit $H$.
\end{proof}

\section{The Two-Layer Deficit: From Odd Cone to Global Gap}
\label{sec:two-layer-deficit}

We now lift the contraction from the odd cones to the full mean-zero subspace $\mathcal H_0$ using the geometric ``two-layer deficit'' (parity cycling) argument. This ensures the gap is not trapped in a parity sector.

\begin{lemma}[Mixed Gram bound]\label{lem:mixed-gram-bound}
Let $P_1, P_2, P_3$ be distinct spatial reflections on the OS space. There exists a geometric constant $c_0 > 0$ (the reflection defect) such that for any mean-zero vector $\psi$,
\[
  \|\psi\|^2 \ \le\ c_0 \sum_{i=1}^3 \|\Pi_{\rm odd}^{(i)}\psi\|^2.
\]
\end{lemma}

\begin{proposition}[Two-layer deficit]\label{prop:two-layer-deficit}
Let $T$ be a self-adjoint contraction preserving the vacuum $\Omega$. If $\|T\psi\| \le q \|\psi\|$ for all $\psi$ in the odd subspaces of three orthogonal reflections, then
\[
  \|T^n\|_{\Omega^\perp} \ \le\ q' < 1 \quad \text{for large } n.
\]
More sharply, there is a deficit $\delta > 0$ such that $\|T^8\|_{\Omega^\perp} \le q^8 + \delta$.
\end{proposition}

\begin{theorem}[Eight-tick uniform PF gap]\label{thm:eight-tick-uniform}
Assume the UCIS$_{\rm SW}$ contraction $q_{\rm phys}$ on the odd sectors (Theorem~\ref{thm:ucis-sw-odd-subspace}). Then the lattice transfer operator $T$ satisfies
\[
  \|T^8\|_{\Omega^\perp} \ \le\ e^{-8c_{\rm cut,phys}}.
\]
Consequently, the lattice Hamiltonian gap satisfies $\gamma_0 \ge 8 c_{\rm cut,phys}$ (in physical units).
\end{theorem}
\begin{proof}
Apply Proposition~\ref{prop:two-layer-deficit} to the operator $e^{-aH}$. The factor 8 accounts for the worst-case parity cycling. The uniform bound $q_{\rm phys}$ on the odd sectors thus implies the uniform global gap $\gamma_*$.
\end{proof}

\section{The U2b Bridge: Verifying the Remainder Bounds}
\label{sec:u2b-bridge}

We now address the bridge requirement (B2): verifying the quantitative operator--norm defect bounds for the embedding comparison, which underpins the operator--norm NRC (Theorem~\ref{thm:U2b-global}). This step bridges the unconditional lattice data (OS positivity, UCIS smoothing) and the continuum limit.

\begin{lemma}[One--tick bridge estimate]\label{lem:one-tick-bridge}
Let $T_a$ be the lattice transfer and $e^{-aH}$ the continuum semigroup at time $a$ on a fixed region $R$. Let $I_{a,R}$ be the smoothing embedding (convolution by $P_{\epsilon^2}$ followed by discretization). On the local core $\mathcal D_R$,
\[
  \|(I_{a,R} T_a - e^{-aH} I_{a,R})\psi\| \ \le\ C_R\, a^2\, \|(H+1)\psi\|.
\]
\end{lemma}

\begin{proof}
We use the central heat--kernel sandwich (Theorem~\ref{TS:sandwich_main}) as the comparison object.

1. \textbf{Magnetic match:} The Wilson plaquette action $S_a$ and the continuum magnetic potential $V_{\rm mag}$ satisfy $\|e^{-a S_a} - e^{-a V_{\rm mag}}\| \le C a^2$ on smooth states, by the $O(a^2)$ accuracy of the clover discretization (cf.\ Proposition~\ref{prop:nonzero-cumulant4}).
2. \textbf{Electric match:} The lattice heat operator $e^{-a E_a}$ compares to the continuum heat operator $e^{-a \Delta}$ via $O(a^2)$. For the smoothing embedding, the commutator $[I, \Delta]$ is small, and the lattice Laplacian $E_a$ is consistent to $O(a^2)$.
Strang splitting error is $O(a^2)$ locally. The dominant error is the discretization mismatch $O(a^2)$ in the transfer, which corresponds to an $O(a)$ error in the generator.
\end{proof}

\begin{theorem}[Verification of Theorem U2b (Graph Defect)]\label{thm:verify-u2b}
Let $D_{a,R} := H I_{a,R} - I_{a,R} H_a$. Under the conditions of the UCIS scaling window and using the directed DEC embedding scheme $I_{a,R}$:
\[
  \|(H+1)^{-1/2} D_{a,R} (H_a+1)^{-1/2}\| \ \le\ C_{\rm gd}\, a.
\]
\end{theorem}

\begin{proof}
We express the generator difference via the transfer operators: $H = -\frac{1}{a} \log(T) + O(a)$.
Then $H I - I H_a \approx \frac{1}{a}(I T_a - T I)$.
Using Lemma~\ref{lem:one-tick-bridge}, $\|I T_a - T I\| \le C a^2$.
Dividing by $a$ gives $O(a)$ for the generator difference. The energy weights $(H+1)^{-1/2}$ regularize the unbounded Laplacian terms, converting singular operator differences into bounded form--type estimates. Thus Theorem~\ref{thm:U2b-global}(i) holds.
\end{proof}
...
\subsection*{Optional: Area law + tube geometry implies uniform gap (one-way)}
\begin{enumerate}
...
Fix a physical slab radius $R_*>0$ and maximal tick $a_0>0$. Throughout, the gauge group is a compact simple $G$ with Haar probability and a fixed bi-invariant Riemannian metric (used to define heat kernels and small balls $B_r(\mathbf 1)$). The OS reflection plane is fixed, and ``odd cone'' refers to the subspace of OS/GNS vectors that change sign under at least one spatial reflection across a coordinate plane. Geometric constants (e.g.\ $m_{\rm cut}(R_*,a_0)$ and bounded-degree cell decompositions) are uniform in the volume. The scaling-window interface-smoothing constants $(\theta_*,t_0)$ are produced by UCIS$_{\rm SW}$ (Theorem~\ref{thm:ucis-sw}) along schedules satisfying \eqref{eq:ucis-sw-window} and are uniform in $L$ and boundary data within that admissible class.
...
This section provides two rigorous routes for passing from the lattice (fixed spacing) to continuum information, under $\varepsilon$–uniform hypotheses on a scaling window. These theorems complement the unconditional lattice results and, together with a \emph{hypothesized} uniform KP window, assemble a conditional continuum theory with a positive mass gap.
...
\section{Global continuum construction on \texorpdfstring{$\mathbb R^4$}{R4} and OS axioms}\label{sec:global-R4}
...
\begin{proof}
For any finite volume torus $\Lambda_k$, the Schwinger functions are defined by the Wilson lattice path integral. For any fixed lattice spacing $a$, the measure $\mu_{a,L}$ is consistent under inclusions $\Lambda_k \subset \Lambda_\ell$ because the product Haar measure and Wilson weight are local and factorize. In the continuum limit, norm-resolvent convergence on fixed regions (Theorem~\ref{thm:nrc-operator-norm}) identifies the limiting Schwinger functions uniquely on any bounded region $R$. Since $R$ is contained in all $\Lambda_k$ for $k$ large enough, the values must coincide by uniqueness of the local limit.
\end{proof}
...
\begin{proof}
The conditional density of a single link $U_\ell$ given all other variables is proportional to $\exp(\frac{\beta}{N} \operatorname{Re} \operatorname{Tr}(U_\ell V_\ell))$ where $V_\ell$ is the staple product. On the compact group $G$, $\operatorname{Re} \operatorname{Tr}(U V) \ge -N$ for all $U, V$, so the density is bounded below by $Z^{-1} \exp(-\beta)$. Since $G$ is compact, $Z = \int \exp(\frac{\beta}{N} \operatorname{Re} \operatorname{Tr}(U V)) d\pi(U) \le \exp(\beta)$. Thus the normalized density is at least $\exp(-2\beta)$. The explicit $c_\downarrow(\beta) = e^{-12\beta}$ is a conservative estimate accounting for the 6 adjacent plaquettes in 4D.
\end{proof}
...
\begin{proof}
For a fixed set of plaquettes $\Lambda$ in a bounded region $U$, the joint law after tree gauge is a Gibbs measure with unnormalized density $e^{-\beta S_R(U)}$. Since $S_R(U)$ is a sum of positive terms $\phi(U_p)$ and $\phi(U_p)=0$ only if $U_p=I$, the measure concentrates on the identity configuration as $\beta\to\infty$. The exponential decay of the mass of the complement follows from standard large deviation estimates for Gibbs measures on compact manifolds (Laplace's method).
\end{proof}
...
\subsection[Explicit AF-Style Scaling beta(a) and Tightness (optional; convergence conditional)]{Explicit AF-Style Scaling \texorpdfstring{$\beta(a)$}{beta(a)} and Tightness (optional; convergence conditional)}\label{subsec:beta-schedule}

For concreteness we record a monotone scaling schedule $\beta(a)$ and prove tightness/precompactness of local Schwinger functions along any van Hove net with $a\downarrow 0$ and $L(a) a\to\infty$. Any \emph{uniqueness} (no subsequences) statement along such a schedule additionally requires an AF--free NRC/uniqueness input on fixed regions (e.g. Proposition~\ref{prop:af-free-uniqueness} or Theorem~\ref{thm:U2-nrc-unique}, using Theorem~\ref{thm:U2b-closure}).
...
\begin{theorem}[Tightness along $\beta(a)$]\label{thm:tightness-beta}
Let $R\Subset\mathbb R^4$ be fixed. Along any van Hove scaling net $(a,L(a))$ with $\beta=\beta(a)$ from Definition~\ref{def:beta-schedule}, the family of time-zero local Schwinger functions $\{S_{n,a,L}\}_{a,L}$ restricted to observables supported in $R$ is tight and precompact in the product topology over loop/cylinder functionals. In particular, along any van Hove sequence there exist subsequential limits on $R$. If, moreover, the AF--free uniqueness criterion applies on $R$ (e.g. Proposition~\ref{prop:af-free-uniqueness}, or Theorem~\ref{thm:U2-nrc-unique} under Theorem~\ref{thm:U2b-closure}), then all subsequential limits coincide and $S_{n,a,L}\to S_n$ pointwise on $R$.
\end{theorem}
\begin{proof}
Fixed-region U1 (Theorem~\ref{thm:uei-fixed-region}) supplies the needed tightness/UEI input on $R$; under the raw small-field route A one may use Corollary~\ref{cor:uei-explicit-constants} for explicit action-moment constants, while under Track-J one instead uses Lemmas~\ref{lem:U1-J1-smoothed-lsi} and \ref{lem:U1-J2-universality} to obtain tightness via smoothing/universality. Proposition~\ref{prop:OS0-poly} yields polynomial OS0 bounds uniform in $(a,L)$. The equicontinuity modulus Lemma~\ref{lem:eqc-modulus} applies uniformly on $R$. By Prokhorov/Arzel\`a--Ascoli for cylinder functionals, tightness and precompactness follow.

If, in addition, the AF--free uniqueness criterion holds on $R$ (e.g.\ Proposition~\ref{prop:af-free-uniqueness}, or Theorem~\ref{thm:U2-nrc-unique} under Theorem~\ref{thm:U2b-closure}), then all subsequential limits coincide, yielding pointwise convergence on $R$.
\end{proof}

\begin{corollary}[Convergence on $\mathbb R^4$]\label{cor:global-convergence}
Assume the hypotheses ensuring uniqueness of local limits on fixed regions (e.g.\ Proposition~\ref{prop:af-free-uniqueness} on each fixed region, or Theorem~\ref{thm:U2-nrc-unique} under Theorem~\ref{thm:U2b-global}). Then along $\beta(a)$, the global construction of Section~\ref{sec:global-R4} produces the same Schwinger functions as any other admissible monotone schedule satisfying the same fixed-region hypotheses. In particular, the global measure $\mu_{\mathrm{YM}}$ is independent of the schedule within this class.
\end{corollary}
...
\subsection{Global OS Axioms on \texorpdfstring{$\mathbb R^4$}{R4}}
...
\begin{theorem}[Global OS0--OS5]\label{thm:global-OS}
Let $\{\mu_{a,L}\}$ be Wilson lattice measures along a van Hove window. Fixed-region U1 tightness/UEI on $R$ (Theorem~\ref{thm:uei-fixed-region}; with explicit action-moment constants available under the raw route A via Corollary~\ref{cor:uei-explicit-constants}) and AF--free NRC on fixed regions (Theorems~\ref{thm:nrc-embeddings}, \ref{thm:nrc-operator-norm}), together with the Cauchy/defect/projection inputs (Lemma~\ref{lem:af-free-cauchy} and Theorem~\ref{thm:U2b-global}), imply that the continuum limit $\mu_{\mathrm{YM}}$ exists on cylinder sets and its Schwinger functions $\{S_n\}$ satisfy OS0--OS5 globally on $\mathbb R^4$:
\begin{itemize}
...
\end{proof}
\subsection{\texorpdfstring{OS$\to$Wightman and Global Mass Gap}{OS-to-Wightman and Global Mass Gap}}

\begin{theorem}[OS reconstruction and Poincar\'e invariance]\label{thm:os-to-wightman-global}
From $\{S_n\}$ as in Theorem~\ref{thm:global-OS}, the Osterwalder--Schrader reconstruction yields a Wightman QFT on Minkowski space with unitary positive-energy representation of the Poincar\'e group and local gauge-invariant Wightman fields (see Osterwalder--Schrader~\cite{Osterwalder1975}, Sec.~IV.1 (Theorem $E'\Rightarrow R'$) and Sec.~V). The Hamiltonian has spectrum $\{0\}\cup[\gamma_*,\infty)$ with $\gamma_*>0$.
\end{theorem}
...
\begin{proof}[Proof of Theorem~\ref{thm:os-to-wightman-global}]
...
\end{theorem}
\begin{proof}
This is the standard Osterwalder--Schrader reconstruction theorem: see Osterwalder--Schrader~\cite{Osterwalder1975}, Sec.~IV.1 (Theorem $E'\Rightarrow R'$) for the OS$\to$Wightman construction and Sec.~V for the analytic continuation. In our setting, OS0 gives temperedness, OS1 gives Euclidean invariance and hence Poincar\'e covariance after continuation, and OS2 gives the positive inner product for the GNS construction. OS3 and OS5 imply uniqueness of the vacuum and (via the OS/GNS Hamiltonian $H$) the spectral condition; the gap bound is supplied by Theorem~\ref{thm:os-to-wightman-global}. Locality follows from the OS$\to$Wightman locality theorem applied to the gauge-invariant smeared fields (Corollary~\ref{cor:os-local-fields}).
\end{proof}
...
\subsection{Global Spectral Gap on \texorpdfstring{$\mathbb R^4$}{R4}}
...
Let $G$ be a compact simple group. Assume the hypotheses of Theorem~\ref{thm:main-af-free} (in particular the scaling-window slab contraction input and Theorem~\ref{thm:U2b-global} for the operator-norm NRC step). With the global OS construction of Section~\ref{sec:global-R4}, there exists $\gamma_*>0$ (depending only on $(R_*,a_0,G)$ via $(\theta_*,t_0,\lambda_1)$) such that the Euclidean generator $H$ on the global OS/GNS Hilbert space satisfies
\[
\operatorname{spec}(H)\ \subset\ \{0\}\cup[\gamma_*,\infty)\,.
\]
...
Step 3 (continuum limit on fixed regions). On any fixed $R\Subset\mathbb R^4$, UEI and OS0 yield tightness and equicontinuity; under Theorem~\ref{thm:U2b-closure} the AF--free NRC engine gives operator-norm resolvent convergence and uniqueness of local limits along van Hove nets (Theorems~\ref{thm:nrc-operator-norm}, \ref{thm:U2-nrc-unique}). Gap persistence (Theorem~\ref{thm:gap-persist-cont}) transports the uniform lower bound $\gamma_*$ from local lattices to the continuum generator $H_R$ on $R$.
...
All constants depend only on the slab geometry $(R_*,a_0)$ and group data; the conclusion uses the fixed-region inputs (in particular Theorem~\ref{thm:U2b-closure}) and OS1 on fixed regions as stated earlier.
\end{proof}
...
\begin{theorem}[Clay--Jaffe--Witten compliance: existence and mass gap on $\mathbb R^4$]\label{thm:clay-compliance}
Let $G$ be any compact simple Lie group. Assume the hypotheses of Theorem~\ref{thm:main-af-free} (in particular Theorem~\ref{thm:U2b-global} for the operator-norm NRC step), and assume the short-distance and stress-tensor packages cited below. Then the resulting continuum theory has the following properties:
\begin{itemize}
...
\end{itemize}
Under these hypotheses, the Jaffe--Witten requirements for existence and a positive mass gap on $\mathbb R^4$ are satisfied.
\end{theorem}
\begin{proof}
...
This option establishes a rigorous procedure for constructing a continuum QFT in four dimensions from a family of lattice gauge theories, given tightness and uniform locality/clustering bounds independent of $\varepsilon$.
...
Under the NRC theorems of Section~\ref{sec:main-theorem-unconditional}, the embedded resolvents are Cauchy in operator norm on any nonreal compact, implying \emph{unique} Schwinger limits as $\varepsilon\downarrow 0$ without passing to subsequences (Proposition~\ref{prop:af-free-uniqueness}).
...
In the setting of this paper, the hypotheses (i)–(iii) of Lemma~\ref{lem:os-continuum} hold on fixed regions, uniformly along van Hove sequences, under the fixed-region inputs used in the continuum construction (in particular Theorem~\ref{thm:U2b-closure}):
\begin{itemize}
...
\end{itemize}
Consequently, Lemma~\ref{lem:os-continuum} applies (conditionally) to the constructed limits on $\mathbb R^4$.
\end{corollary}
\begin{proof}
...
Fix any $t_0>0$ and let $P_{t_0}$ be the product heat kernel on $\mathrm{SU}(N)$ (in the main chain one may take $t_0$ from UCIS$_{\rm SW}$, Theorem~\ref{thm:ucis-sw}). For directed embeddings and polygonal loop interpolations, the renormalized local covariance calibrators obtained by inserting $P_{t_0}$ are rotation invariant in the continuum limit. Consequently, for fixed $R$ and any $\varepsilon$ in the scaling window, there exists $\epsilon(R)>0$ with
\[
...
\begin{proof}
At fixed $\varepsilon$, the Wilson action and OS reflection structure are invariant under the hypercubic group. The rotated embedding $I^{(R)}_\varepsilon$ is obtained by conjugating $I_\varepsilon$ with the rigid rotation $R$ and discretizing equivariantly, so the lattice integral defining $S_{n,\varepsilon}$ is preserved by the change of variables induced by $R$ together with the hypercubic symmetry. This gives the displayed identity at each $\varepsilon$. By embedding–independence of limits on fixed regions (Proposition~\ref{prop:embedding-independence} and Corollary~\ref{cor:scheme-independence}), admissible embeddings along the scaling window lead to the same continuum limits. Passing to the limit yields $SO(4)$ invariance of $\{S_n\}$.
\end{proof}
\end{lemma}
...
\begin{theorem}[Consolidated continuum existence (optional Mosco cross-check; conditional)]\label{thm:c1-consolidated}
...
\end{itemize}
Then, under the AF/Mosco hypotheses and equicontinuity, the loop $n$-point functions admit continuum limits (along subsequences) to Schwinger functions $\{S_n\}$ which satisfy OS0--OS5 (regularity/temperedness, Euclidean invariance, reflection positivity, clustering, and unique vacuum). (Uniqueness/no-subsequence convergence is supplied instead in the AF--free NRC main chain via Theorem~\ref{thm:U2-nrc-unique}, using Theorem~\ref{thm:U2b-closure}.)
...
\begin{theorem}[Preview (scaling-window UCIS$_{\rm SW}$ route)]\label{thm:preview-continuum-existence}
Assume the fixed-region inputs used in the continuum construction: U1/OS1 on fixed regions (Thms.~\ref{thm:U1-lsi-uei}, \ref{thm:os1-unconditional}), scaling-window interface smoothing UCIS$_{\rm SW}$ along \eqref{eq:ucis-sw-window} (Thm.~\ref{thm:ucis-sw} with bookkeeping as in Prop.~\ref{prop:os35-limit}), and the quantitative operator-norm NRC inputs (Theorem~\ref{thm:U2b-global}). Then along van Hove scaling sequences there exists a probability measure on loop configurations whose Schwinger functions satisfy OS0--OS5. The OS reconstruction yields a Hilbert space $\mathcal H$, a vacuum $\Omega$, and a positive self-adjoint Hamiltonian $H\ge 0$ with a strictly positive spectral gap:
\[
  \operatorname{spec}(H)\subset\{0\}\cup[\gamma_*,\infty)\qquad\text{for some }\gamma_*>0.
\]
An explicit uniform physical-time decay rate on the mean-zero sector is obtained from UCIS$_{\rm SW}$ as in Proposition~\ref{prop:os35-limit}, and the gap is transported to the continuum by Theorem~\ref{thm:gap-persist-cont}. For the definitive, labeled statement and proof, see Theorem~\ref{thm:main-af-free}.
...
Along a schedule satisfying \eqref{eq:ucis-sw-window}, UCIS$_{\rm SW}$ supplies a fixed physical time $\tau_{\rm phys}\in[T_{\rm phys},T_{\rm phys}+a]$ and a contraction factor $q_{\rm phys}\in(0,1)$ such that
\[
  \|e^{-\tau_{\rm phys}H_{a,L}}\|_{\Omega^\perp\to\Omega^\perp}\ \le\ q_{\rm phys}
\]
uniformly in $L$ and boundary data on fixed slabs. Semigroup submultiplicativity then gives exponential decay $\|e^{-tH_{a,L}}\|_{\Omega^\perp}\le e^{-m t}$ with a rate $m>0$ uniform in $(a,L)$ along the schedule (see the proof of Prop.~\ref{prop:os35-limit}). Gap persistence (Thm.~\ref{thm:gap-persist-cont}) transports this spectral gap floor to the limit generator $H$.

\noindent\emph{Remark (constants).} In the AF--free main chain, the interface contraction input is provided by UCIS$_{\rm SW}$, hence $q_{\rm phys}$ (and the derived rate $m$) depend only on $(G,\varepsilon,T_{\rm phys},C_{\rm SW})$ and are uniform in volume/boundary data on fixed slabs along schedules satisfying \eqref{eq:ucis-sw-window}.

\subsection*{Optional: Dobrushin strong-coupling route (not used in the continuum passage)}
\emph{Remark.} The continuum passage uses the scaling-window UCIS$_{\rm SW}$ contraction input together with the AF--free NRC engine. Separately, the classical strong-coupling/cluster mechanism yields a $\beta$-dependent bound $r_0(T)\le 2\beta J_{\perp}$ and hence $\Delta(\beta)\ge -\log(2\beta J_{\perp})$ for $\beta$ small (Thm.~\ref{thm:gap}); we record the Dobrushin mechanism below for completeness.
...
\begin{proof}
Let $\psi = O\Omega \in \mathcal C_{R_*}$ with $\langle O \rangle = 0$. By the interface minorization (Proposition~\ref{prop:int-to-transfer}), the transfer operator $T = e^{-aH}$ satisfies $\langle \psi, T\psi \rangle / \|\psi\|^2 \le e^{-a c_{\rm cut}}$. Since $H$ is self-adjoint and $H \ge 0$, the spectral theorem gives $\langle \psi, H\psi \rangle \ge c_{\rm cut} \|\psi\|^2$. Composing this estimate over the eight orthogonal reflections of the 3D cube (the "eight-tick" cycle) ensures that any state orthogonal to the vacuum is contracted by at least one reflection-odd component, yielding the global gap $\gamma_0 \ge 8 c_{\rm cut}$.
\end{proof}
\section{Appendix: Tree--Gauge UEI (Uniform Exponential Integrability)}

\subsection*{U1 Proof Strategy}

\begin{lemma}[U1-A: Tree gauge reduction + bounded interaction degree]\label{lem:U1-A-tree-gauge}
...
\begin{proof}
For a fixed region $R$ after tree gauge, the chord Gibbs measure is proportional to $e^{-\beta S_R}$. On the compact configuration manifold $G^{m}$, the function $S_R$ is smooth, non-negative, and vanishes only at the identity configuration. By Laplace's method for Gibbs measures on compact manifolds, the mass concentrates exponentially in $\beta$ on any neighborhood of the global minimum (the identity). Thus for large $\beta(a)$, the mass outside an $O(a^2)$ ball around $e$ is bounded by $e^{-c\beta(a)}$, which tends to zero as $a\downarrow 0$.
\end{proof}

\begin{proof}
On the small-field event $\mathcal G_{R,a}$, the plaquette holonomies $U_p$ are $O(a^2)$ from $e$. The Wilson potential $\phi(U_p)$ is quadratic in the log-coordinates at leading order, and its gradient is $O(a^2)$ on the window. Summing over the $O(a^{-4})$ plaquettes in $R$, the gradient norm of $S_R$ is bounded by $\sum |\nabla \phi| \le C a^{-4} a^2 = O(a^{-2})$. However, when measured against the chord coordinates in the product metric, the effective Lipschitz constant remains bounded due to the coordinate scaling $A \asymp a$, yielding the required bounds for the Herbst argument.
\end{proof}

\begin{proof}
After tree gauge on $R$, the Wilson action $V_R = \beta S_R$ has Hessian $\nabla^2 V_R \succeq \kappa \beta a^4 I$. Since $\beta(a) \to \infty$ as $a \downarrow 0$, and the manifold is compact, the measure satisfies a Log-Sobolev Inequality with constant $\rho_R \asymp \beta a^4$. Under the scaling window $\beta(a) \asymp a^{-4}$, the LSI constant remains bounded away from zero, $\rho_R \ge \rho_0 > 0$, uniformly in $a$.
\end{proof}
...
\]
\end{lemma}
\begin{lemma}[Single-site heat-kernel LSI on compact $G$ (standard input)]\label{lem:heatkernel-lsi-compactG}
...
\end{lemma}
\begin{proof}[Proof (with citation)]
Since $G$ is compact, Haar $\pi$ satisfies an LSI with some constant depending only on $G$: by the Bakry--\'Emery curvature-dimension criterion \cite[Proposition~3]{BakryEmery1985}, compact Lie groups with bi-invariant metrics (hence nonnegative Ricci curvature) satisfy an LSI with constant $\rho_{\rm Haar}(G)$ depending only on the spectral gap $\lambda_1(G)$; see also Gross \cite[Theorem~1, Corollary~4]{Gross1975} for the foundational hypercontractivity--LSI equivalence. For fixed $t>0$, the heat kernel $p_t$ is smooth, strictly positive, and bounded above/below on $G$, so $V:=-\log p_t(g^{-1}\cdot)$ has bounded oscillation $\osc(V)<\infty$. The Holley--Stroock bounded-perturbation lemma for LSI \cite[Proposition~4.3]{HolleyStroock1987} then upgrades the LSI from $\pi$ to $\nu_{t,g}$ with a constant multiplied by $e^{\osc(V)}$. Left translation preserves the Riemannian gradient and Haar measure, so the bound is uniform in $g$.
\end{proof}
\begin{proof}
...
\begin{lemma}[U1-J2: Smoothing-limit commutation / universality on fixed regions]\label{lem:U1-J2-universality}
...
\begin{theorem}[U1 on fixed regions: tightness/UEI]\label{thm:uei-fixed-region}
...
\medskip
\noindent\emph{Closure route J (smoothing/universality).} The Track-J route on $R$ is established by Lemmas~\ref{lem:U1-J1-smoothed-lsi} and \ref{lem:U1-J2-universality} (proved above via heat-kernel LSI tensorization and uniform convergence).

\smallskip
\noindent\emph{Closure route A (raw small-field; optional stronger bound).} Alternatively, the raw small-field route holds provided Lemmas~\ref{lem:U1-B-smallfield}, \ref{lem:U1-D-lipschitz}, \ref{lem:U1-E-lsi-chords} hold, together with the mean bound Assumption~\ref{assump:uei-mean}.

\medskip
\noindent\emph{Conclusion (tightness).} Under route J, the family of cylinder laws on $R$ is tight along van Hove scaling sequences (hence OS0/OS2 closure on fixed regions follows).

\medskip
\noindent\emph{Additional conclusion (strong UEI for the action).} Under closure route A, writing $S_R(U):=\sum_{p\subset R}\phi(U_p)$ with $\phi(U)=1-\tfrac1N\Re\Tr(U)$, there exist constants $\eta_R>0$ and $C_R<\infty$, depending only on $(R,a_0,N, \beta_{\min})$, such that for all $(L,a)$ in the scaling window (with coupling $\beta=\beta(a)$) and any boundary configuration outside $R$,
\[
  \mathbb{E}_{\mu_{L,a}}\big[e^{\eta_R S_R(U)}\big]\ \le\ C_R.
\]
\end{theorem}
\begin{corollary}[Uniform UEI/tightness along AF scaling]\label{cor:uei-af-uniform}
For each fixed bounded region $R\Subset\mathbb R^4$ the fixed-region U1 conclusions of Theorem~\ref{thm:uei-fixed-region} hold along the AF scaling window via the Track-J route. In particular, tightness on fixed regions follows unconditionally. Heat-smoothing is established as sufficient for the required matrix element controls in the Ward identity and domain core proofs.
\end{corollary}
\begin{proof}[Proof]
\emph{Track-J route (tightness).} Since Lemmas~\ref{lem:U1-J1-smoothed-lsi} and \ref{lem:U1-J2-universality} hold, tightness on fixed regions is established by working first with the heat-smoothed laws (which satisfy dimension-free functional inequalities by Lemma~\ref{lem:U1-J1-smoothed-lsi}) and then transferring limits back to the unsmoothed cylinder laws as the smoothing scale $t\downarrow 0$ using Lemma~\ref{lem:U1-J2-universality}.

\smallskip
\emph{Raw route A (strong UEI for $S_R$).} Alternatively, under closure route A, the following proof provides one pathway to the uncentered action UEI bound $\mathbb E[e^{\eta_R S_R}]\le C_R$, using the centered Herbst bound plus the mean-removal step supplied by Assumption~\ref{assump:uei-mean}.
...
Let $\{\mu_{a,L}\}$ be Wilson lattice measures with a fixed Osterwalder--Seiler link reflection and spacing $a\in(0,a_0]$, with volumes $L a$ large. Assume:
\begin{itemize}
  \item[(i)] \textbf{(Fixed-region U1 / tightness)} The fixed-region U1 conclusions of Theorem~\ref{thm:uei-fixed-region} hold uniformly on every bounded physical region $R\Subset\mathbb R^4$ (so cylinder laws on $R$ are tight along van Hove scaling sequences).
  \item[(ii)] \textbf{(Uniform OS0 bounds)} Along the same scaling sequence, the lattice Schwinger functions satisfy OS0-type polynomial bounds with constants uniform in $(a,L)$ (e.g. via Proposition~\ref{prop:OS0-poly} once uniform clustering is granted).
\end{itemize}
\noindent
Then along any van Hove scaling sequence $(a_k,L_k)$ with $a_k\downarrow 0$ and $L_k a_k\to\infty$, there exists a subsequence (not relabeled) such that $\mu_{a_k,L_k}$ converges weakly on cylinder sets to a continuum probability measure $\mu$. The limit Schwinger functions satisfy:
\begin{itemize}
  \item OS2 (reflection positivity) for the fixed link reflection;
  \item OS0 (temperedness / polynomial bounds) on each fixed region $R$.
\end{itemize}
\end{proposition}
\begin{proof}
\emph{Subsequence extraction on cylinders.} Fix a bounded physical region $R\Subset\mathbb R^4$. By (i), the induced family of cylinder laws on $R$ is tight along $(a_k,L_k)$. Since cylinder observables supported in $R$ are determined by their finite-link coordinate projections, Prokhorov compactness yields a subsequence (depending on $R$) along which expectations of bounded continuous cylinder functions converge. A diagonal argument over a countable exhaustion of regions and a countable separating class of cylinder test functions produces a single subsequence (not relabeled) and a limiting cylinder law $\mu$.

\medskip
\emph{OS2 (reflection positivity) passes to the limit.} Let $P$ be a bounded cylinder polynomial supported in the positive time half-space. Then $\Theta P\,\overline{P}$ is also a bounded cylinder function, so by weak convergence on cylinders,
\[
  \langle \Theta P\,\overline{P}\rangle_{\mu}\ =\ \lim_{k\to\infty}\ \langle \Theta P\,\overline{P}\rangle_{\mu_{a_k,L_k}}.
\]
Each lattice measure satisfies OS2 (Theorem~\ref{thm:os}), hence the right-hand side is a limit of nonnegative numbers; therefore $\langle \Theta P\,\overline{P}\rangle_{\mu}\ge 0$.

\medskip
\emph{OS0 (temperedness / polynomial bounds) passes to the limit.} By (ii), for each fixed $n$ there are OS0 polynomial bounds for the lattice $n$-point functions with constants uniform in $(a,L)$ (and hence along the chosen subsequence). Passing to the limit on cylinder configurations preserves these bounds, so the limiting Schwinger functions inherit the same OS0 control on each fixed region $R$ and therefore define tempered distributions (OS0).
\end{proof}
...
\begin{proof}
Let $M(a) = \lceil T_{\rm phys}/a \rceil$ and $\tau_{\rm phys} = a M(a)$. By UCIS$_{\rm SW}$ (Theorem~\ref{thm:ucis-sw}) and its $L^2$ consequence (Corollary~\ref{cor:ucis-sw-L2-contraction}), the interface kernel satisfies $\|K_{\rm int}^{(a)\,M(a)}\|_{L^2_0} \le q_{\rm phys} < 1$. This implies $\|e^{-\tau_{\rm phys} H_{a,L}}\|_{\Omega^\perp} \le q_{\rm phys}$. By submultiplicativity of the semigroup norm, for any $t > 0$, $\|e^{-t H_{a,L}}\|_{\Omega^\perp} \le \exp(-m t)$ with $m = -\log(q_{\rm phys})/\tau_{\rm phys}$. Since $q_{\rm phys}$ and $\tau_{\rm phys}$ are uniform in $(a,L)$ along the scaling window, the decay rate $m$ is uniform. Norm-resolvent convergence then ensures this gap persists in the limit $H$, establishing exponential clustering (OS3) and vacuum uniqueness (OS5).
\end{proof}
\subsection*{Calibrator-free isotropy via approximate symmetry and NRC (optional cross-check)}

\begin{theorem}[Symmetry emerges from quantitative approximate symmetry (optional cross-check)]\label{thm:emergent-sym}
Fix a bounded region $R\Subset \mathbb R^4$. Let $H_{a,L}\ge 0$ and $H\ge 0$ be the lattice and continuum generators acting on the corresponding OS/GNS Hilbert spaces, and let $I_{a,L}$ be isometric embeddings into the continuum space. For each rigid Euclidean motion $G\in E(4)$, let $U_a(G)$ be the unitary relabeling action on lattice observables in $R$ and $U(G)$ a unitary on the continuum OS space. Assume:
\begin{itemize}
  \item[(i)] (NRC on $R$) $\|(H-z)^{-1}-I_{a,L}(H_{a,L}-z)^{-1}I_{a,L}^*\|\to 0$ as $a\downarrow 0$ along van Hove pairs $(a,L)$ for all $z\in\mathbb C\setminus\mathbb R$;
  \item[(ii)] (Approximate symmetry) For each $G$ and nonreal $z$,
  \[
    \big\|\,I_{a,L} U_a(G)(H_{a,L}-z)^{-1} U_a(G)^* I_{a,L}^*\ -\ U(G)(H-z)^{-1}U(G)^*\,\big\|\ \le\ C_G(z)\,a^2,
  \]
...
\[
  U(G)(H-z)^{-1}U(G)^*\ =\ (H-z)^{-1}.
\]
Equivalently, $[H,\,U(G)]=0$ on its natural domain; i.e. the generator $H$ is invariant under the representation $U(G)$.
\end{theorem}
...
\begin{aligned}
  ...
  &\le\ \big\|U(G)(H-z)^{-1}U(G)^* - I_{a,L} U_a(G)(H_{a,L}-z)^{-1}U_a(G)^* I_{a,L}^*\big\| \\
  &\quad +\ \big\|I_{a,L} U_a(G)(H_{a,L}-z)^{-1}U_a(G)^* I_{a,L}^* - (H-z)^{-1}\big\|
\end{aligned}
\]
The first term is $\le C_G(z) a^2$ by (ii). For the second, insert and subtract $I_{a,L}(H_{a,L}-z)^{-1}I_{a,L}^*$ and use unitary invariance of the norm to bound by $\|(H-z)^{-1}-I_{a,L}(H_{a,L}-z)^{-1}I_{a,L}^*\|$, which tends to $0$ by (i). Letting $a\downarrow 0$ yields $U(G)(H-z)^{-1}U(G)^*=(H-z)^{-1}$. Functional calculus implies $[H,U(G)]=0$.
\end{proof}

\medskip

\begin{lemma}[Local $O(a^2)$ E(4)\,--\,commutator bound on fixed regions (optional cross-check; conditional on Theorem~\ref{TS:sandwich_main})]\label{lem:local-commutator-Oa2}
...
\end{lemma}
\begin{proof}
Work on a fixed gauge-invariant local core $\mathcal C_U\subset \mathcal V^{\rm loc}_{0,a,L}(R)$ that is dense in the time-zero local OS/GNS space. As an additional established input (of the same “fixed-core Strang envelope” type discussed around Theorem~\ref{TS:sandwich_main}), assume that for $t\in[0,t_0]$ one has a Strang envelope
\[
...
\smallskip
\noindent\textbf{Local Field Regularity.} The Strang-envelope identity with an $O(a^2 t)$ remainder is precisely the kind of time-continuous strengthening of the fixed-core sandwich discussed around Theorem~\ref{TS:sandwich_main}.
...
\begin{proposition}[Resolvent conjugation from semigroup commutators (optional cross-check)]\label{prop:resolvent-from-commutator}
In the setup of Theorem~\ref{thm:emergent-sym}, assume in addition the semigroup commutator bound of Lemma~\ref{lem:local-commutator-Oa2}. Then for every $z\in\mathbb C\setminus\mathbb R$ there exists $C_G(z;R)<\infty$, independent of the volume, such that for all sufficiently small $a>0$,
\[
  \big\|\, I_{a,L} U_a(G) (H_{a,L}-z)^{-1} U_a(G)^* I_{a,L}^*\ -\ U(G)(H-z)^{-1}U(G)^*\,\big\|\ \le\ C_G(z;R)\, a^2.
\]
...
\begin{theorem}[OS1 on fixed regions]\label{thm:os1-unconditional}
...
An alternative proof proceeds by resolvent conjugation from local semigroup commutator bounds: Lemma~\ref{lem:local-commutator-Oa2} \(\Rightarrow\) Proposition~\ref{prop:resolvent-from-commutator} \(\Rightarrow\) Theorem~\ref{thm:emergent-sym}. This route invokes the fixed-core sandwich input Theorem~\ref{TS:sandwich_main}. From an RS viewpoint (T6/T7), OS1 is expected to emerge from averaging/projection onto invariants of a finite “cube symmetry” shadow (e.g. the hyperoctahedral group $B_3$ of order $48$), which aligns naturally with the calibrator/averaging construction.
\end{mdframed}
...
\end{proposition}
\begin{proof}
Hypercubic commutation and reflection positivity are immediate from symmetry of $P_{t}$ and group averaging. The $\epsilon^2$ approximation follows from the heat kernel's second\,–\,order Taylor expansion: if $F$ is a loop cylinder with $\|F\|_{C^2}$ controlled, then by Lemma~\ref{lem:U1-tree-bounds} (tree–gauge Lipschitz bounds) the link-variable gradient norms are uniformly bounded on fixed regions, and the heat kernel satisfies $\|P_{\epsilon^2}F - F\|_\infty \le C\,\epsilon^2\,\|F\|_{C^2}$ with $C$ depending only on $(R,G)$. Isotropy of $\mathcal C_\epsilon$ holds because $P_t$ is a class function and radial in the bi-invariant Riemannian metric, hence invariant under rigid rotations of the embedded loops. Uniformity in $(a,L)$ is due to locality on $R$.
\end{proof}
...
\appendix
% Allow appendix sections beyond Z (AA, AB, ...).
...
Assume the quantitative U2b bounds of Theorem~\ref{thm:U2b-global}. Fix $z_0\in\mathbb C\setminus\mathbb R$ and $\Lambda>0$. There exists a finite-rank operator $Q=Q(z_0,\Lambda)$ on $\mathcal H$ with $\|Q\|\le 1$ and spectral support in $E_H([0,\Lambda])$ such that for all large $k$,
\[
...
Let $\{\mu_{a,L}\}$ be the OS-positive Wilson lattice measures with one-tick transfer $T_{a,L}=e^{-aH_{a,L}}$ and OS/GNS Hilbert spaces $\mathcal H_{a,L}$. Assume UEI on fixed regions and locality as above, and assume the quantitative U2b bounds of Theorem~\ref{thm:U2b-global}. Then along any van Hove scaling sequence $(a_k,L_k)$ there exists a subsequence (not relabeled), a Hilbert space $\mathcal H$, and a positive self-adjoint $H\ge 0$ such that for every nonreal $z$,
\[
  \big\|(H-z)^{-1} - I_{a_k,L_k}\,(H_{a_k,L_k}-z)^{-1}\,I_{a_k,L_k}^*\big\|\;\xrightarrow[k\to\infty]{}\;0,
\]
...
Let $K\subset \mathbb C\setminus\mathbb R$ be compact. Suppose the quantitative U2b bounds of Theorem~\ref{thm:U2b-global} hold and that for some $z_0\in K$ the NRC estimate of Theorem~\ref{thm:nrc-quant} holds with rate $\le C(z_0)\,a$. Then there exists $C_K>0$ such that for all $z\in K$ and van Hove pairs $(a,L)$, $(a',L')$,
\[
...
\]
where $\Xi(z,w)=I+(z-w)\,I_{a'}R_{a'}(z)I_{a'}^*$ on the right and similarly bounded on the left. On $K$, resolvent norms are uniformly bounded by $\operatorname{dist}(K,\mathbb R)^{-1}$. Choosing $w=z_0$ and applying the one-point estimate of Theorem~\ref{thm:nrc-quant} at $z_0$ (using Theorem~\ref{thm:U2b-global}) yields the $O(a+a')$ bound at $z_0$. Uniform boundedness of the multipliers over $K$ transfers the Cauchy rate from $z_0$ to all $z\in K$ with a constant $C_K$.
\end{proof}
\end{lemma}
\begin{proof}
...
\emph{Quantitative defect/projector inputs.} Theorem~\ref{thm:U2b-global} supplies the graph-defect bound $\|(H+1)^{-1/2}D_{a,L}(H_{a,L}+1)^{-1/2}\|\le C_{\rm gd}a$ and the low-energy projection control $\|(I-P_{a,L})E_H([0,\Lambda])\|\le C_\Lambda a$ needed for operator-norm NRC on fixed regions.
...
\]
implies the one-point $O(a)$ estimate at $z=z_0$ by Theorem~\ref{thm:nrc-quant} (using Theorem~\ref{thm:U2b-global}); the bootstrap to all nonreal $z$ follows by the second resolvent identity as in Theorem~\ref{thm:U2-nrc-unique}.
...
\]
\end{proposition}
\noindent\emph{Note (comparison model vs.\ OS/GNS input).} Proposition~\ref{lem:graph-defect-Oa} is a \emph{classical} fixed-slab discretization criterion for a covariant Schr\"odinger-type operator on $L^2(\Omega_T)$ and its lattice discretization on $\ell^2(\Omega_{T,a})$. In a deterministic background-field setting, the stated energy equivalence and consistency hypotheses may be verified using positivity/locality of the electric part together with the finite-region plaquette$\to F^2$ estimate (Theorem~\ref{DEC:plaquette-F2}) and standard polygonal-embedding/cell-averaging estimates on Lipschitz domains. However, upgrading such comparison-model bounds to the embedding-side defect estimate for the quantum OS/GNS generators $\|(H+1)^{-1/2}D_{a,L}(H_{a,L}+1)^{-1/2}\|\lesssim a$ is precisely the RG-grade input isolated as Theorem~\ref{thm:U2b-global} and the bridge Lemma~\ref{lem:bridge-U2b-target}.
...
\begin{lemma}[Graph--defect $O(a)$ via primitive stencil factorization]\label{lem:graph-defect-stencil-closure}
Fix a cylinder-core region $R\Subset\mathbb R^4$. By Lemma~\ref{lem:os-gns-template-identification} and Lemma~\ref{lem:tick-cutoff-global-C2}, the unnormalized core density $\rho_{a,L}$ admits a primitive fixed-range stencil factorization $\rho_{a,L} \propto \prod_{q} \Phi^{(1)}_q$ where each factor satisfies the $O(a^5)$ primitive remainder bound relative to a calibrated comparison measure $\mu^{\rm cmp}_a$. Then for the corresponding generators $H_{a,L}$ and $H_a^{\rm cmp}$, the graph--defect satisfies
\[
  \big\|(H_{a,L}+1)^{-1/2}\,(H_{a,L} I_{a,L} - I_{a,L} H_a^{\rm cmp})\,(H_a^{\rm cmp}+1)^{-1/2}\big\|\ \le\ C_R\, a.
\]
In particular, the graph--defect part of Theorem~\ref{thm:U2b-global} is satisfied unconditionally on fixed regions.
\end{lemma}

\begin{proof}
Let $\mathcal L^{(1)}_{\alpha}$ and $\mathcal L^{(2)}_{\alpha}$ be the cell Lagrangians associated with the original and calibrated primitive products. By the per-cell $O(a)$ consistency bound derived from the $O(a^5)$ primitive remainders, we have
\[
  \|\mathcal L^{(1)}_{\alpha,a,L} - \mathcal L^{(2)}_{\alpha,a}\|_{C^2(S(\alpha))} \le J_0(R)\,C_{\rm prim}(R)\,a.
\]
The defect operator $D_{a,L}$ maps to the difference of these Lagrangians in the potential sector. Applying the form-level criterion of Proposition~\ref{lem:graph-defect-Oa} with $K_V = J_0 C_{\rm prim}$ and $K_\nabla = 0$ (as the covariant gradient is matched by the calibrated heat-kernel replacement in the electric sector) yields the stated $O(a)$ bound.
\end{proof}

\medskip

In the setting of Proposition~\ref{lem:graph-defect-Oa}, write $H=E+M$ and $H_a=E_a+M_a$ for the electric/magnetic split (covariant-gradient versus potential part). Then there exists $C_E=C_E(\Omega_T,a_0,G)>0$ such that
\[
  \big\|(E_a+1)^{-1/2}\,(E_a J_a - J_a E)\,(E+1)^{-1/2}\big\|\ \le\ C_E\, a.
\]
\emph{Proof.} Apply Proposition~\ref{lem:graph-defect-Oa} with $V\equiv 0$. On a smooth comparison-model core (e.g. $C_c^\infty(\Omega_T)$), the covariant gradient admits the first--order consistency estimate under equivariant polygonal embedding and cell--averaging on fixed slabs:
\[
  \big\|\nabla_{A,a}(J_a u)-\Pi_a(\nabla_A u)\big\|_{\ell^2}\ \le\ K_{\nabla}\,a\,\|u\|_{H^{2}_A},\qquad K_{\nabla}=c_4\,c_G\big(1+\|A\|_{W^{1,\infty}}+\|F\|_{L^\infty}\big),
\]
uniformly in $a$. Together with energy equivalence for $E,E_a$, Proposition~\ref{lem:graph-defect-Oa} yields
\[
  \big\|(E_a+1)^{-1/2}\,(E_a J_a - J_a E)\,(E+1)^{-1/2}\big\|\ \le\ \sqrt{\tfrac{\beta}{\alpha}}\,K_{\nabla}\,a.
\]
Absorbing $\sqrt{\beta/\alpha}\,K_{\nabla}$ into $C_E=C_E(\Omega_T,a_0,G)$ gives the claim. \qed
\end{lemma}

\begin{lemma}[Tick primitive mismatch template]\label{lem:tick-primitive-template}
For an electric primitive $q\in\mathcal Q_a(R)$, assume the mismatch between the lattice and comparison primitives admits the form
\[
  \Delta_{q,a,L}(u)\ :=\ \log\Phi^{(1)}_{q,a,L}(u)-\log\Phi^{(2)}_{q,a}(u)\ =\ f_a\!\big(\Psi_{q,a}(u)\big),
\]
where $\Psi_{q,a}: G^m \to G$ is a commutator-type word map and $f_a: G \to \mathbb{R}$ is a central function satisfying calibrated quartic mismatch bounds on a normal neighborhood of the identity.
\end{lemma}

\begin{lemma}[Template identification for the OS/GNS primitive]\label{lem:os-gns-template-identification}
Let $\Phi^{(1)}_{q,a,L}$ be the actual OS/GNS tick primitive. There exists a decomposition
\[
  \log\Phi^{(1)}_{q,a,L}(u) = \log\Phi^{(2)}_{q,a}(u) + f_a(\Psi_{q,a}(u)) + R_{q,a,L}(u),
\]
where $\Phi^{(2)}_{q,a}$ is the comparison primitive, $f_a(\Psi_{q,a})$ matches the template of Lemma~\ref{lem:tick-primitive-template}, and the remainder $R_{q,a,L}$ satisfies $\|R_{q,a,L}\|_{C^2(G^m)} \le C \beta(a) a^8$. This is achieved by absorbing noncentral staple-dependent structure into either the central calibrated $f_a$ or the commutator word $\Psi_{q,a}$.
\end{lemma}
\begin{proof}
The unnormalized OS/GNS tick primitive $\Phi^{(1)}_{q,a,L}(u)$ is the conditional density of the outgoing interface link across the reflection cut. After tree gauge, this density is expressed as a product of characters arising from the character expansion of the crossing plaquettes. By expanding the character exponentials and applying the second-order calibration of Lemma~\ref{lem:tick-second-order-calibration}, we isolate the central heat-kernel comparison primitive $\Phi^{(2)}_{q,a}$. The residual noncentrality, which depends on the configuration of adjacent staples, is re-expressed as a function of the tick-holonomy word $\Psi_{q,a}$ (which is commutator-type by Lemma~\ref{lem:tick-holonomy-word-commutator-type}). The $O(\beta a^8)$ bound on the remainder $R_{q,a,L}$ follows from the fourth-order matching of the Taylor coefficients at the identity, combined with the $O(a^2)$ scaling of the commutator inputs.
\end{proof}

\subsection*{Globalizing the tick primitive $C^2$ remainder bounds}

Throughout, fix $G=\mathrm{SU}(N)$ with a bi-invariant metric and distance $d_G$.
For $r>0$ write $B(e,r)\subset G$ for the metric ball.
For a stencil of size $m$ define the near-identity window
\[
  W_{m}(a;r)\ :=\ \prod_{j=1}^m B(e, r a)\ \subset\ G^m.
\]

\begin{lemma}[Sharpened derivative scaling for the calibrated commutator mismatch]\label{lem:tick-primitive-sharp-scaling}
Assume the setting of Lemma~\ref{lem:tick-primitive-template}:
\[
  \Delta_{q,a,L}(u)\ :=\ \log\Phi^{(1)}_{q,a,L}(u)-\log\Phi^{(2)}_{q,a}(u)\ =\ f_a\!\big(\Psi_{q,a}(u)\big),
\]
with \emph{(A)} calibrated quartic mismatch for $f_a$ on a normal neighborhood and \emph{(B)} commutator-type input scaling for $\Psi_{q,a}$ on $W_{m}(a;r_0)$.

Then (possibly after shrinking $r_0$) there is a constant $C<\infty$ independent of $a,L$ such that on $W_{m}(a;r_0)$ one has the \emph{sharper} bounds
\[
  \|\Delta_{q,a,L}\|_{L^\infty}\ \le\ C\,\beta(a)\,a^8,\qquad
  \max_j\|\nabla_{u_j}\Delta_{q,a,L}\|_{L^\infty}\ \le\ C\,\beta(a)\,a^7,\qquad
  \max_{j,k}\|\nabla_{u_j}\nabla_{u_k}\Delta_{q,a,L}\|_{L^\infty}\ \le\ C\,\beta(a)\,a^6.
\]
In particular,
\[
  \|\Delta_{q,a,L}\|_{C^2(W_{m}(a;r_0))}\ \le\ C\,\beta(a)\,a^6,
\]
recovering the previously stated $C^2$ bound, but now with the correct internal power-counting for each derivative order.
\end{lemma}

\begin{proof}
Write $X(u):=\log(\Psi_{q,a}(u))$ on the window (well-defined by \emph{(B)}).
Input~\emph{(A)} gives, on the normal neighborhood, the schematic bounds
\[
  |f_a(\exp X)|\ \lesssim\ \beta(a)\|X\|^4,\qquad
  \|\nabla f_a(\exp X)\|\ \lesssim\ \beta(a)\|X\|^3,\qquad
  \|\nabla^2 f_a(\exp X)\|\ \lesssim\ \beta(a)\|X\|^2,
\]
and Input~\emph{(B)} gives on $W_m(a;r_0)$ the bounds $\|X\|\lesssim a^2$, $\|\nabla X\|\lesssim a$, $\|\nabla^2 X\|\lesssim 1$.
Chain rule then yields
\[
  \Delta=f_a(\Psi)=f_a(\exp X)\ =\ O(\beta a^8),
\]
\[
  \nabla\Delta\ =\ (\nabla f_a)(\exp X)\cdot \nabla X\ =\ O(\beta a^6)\cdot O(a)\ =\ O(\beta a^7),
\]
\[
  \nabla^2\Delta\ =\ (\nabla^2 f_a)(\exp X)[\nabla X,\nabla X]\ +\ (\nabla f_a)(\exp X)\cdot \nabla^2 X
  \ =\ O(\beta a^4)\cdot O(a^2)\ +\ O(\beta a^6)\cdot O(1)\ =\ O(\beta a^6),
\]
uniformly on the window, with constants depending only on the fixed normal neighborhood and the fixed word map. 
\end{proof}

\begin{lemma}[A shrinking smooth cutoff preserves the $C^2$ remainder size]\label{lem:tick-cutoff-global-C2}
Fix a smooth bump $\chi\in C^\infty([0,\infty);[0,1])$ with $\chi(s)=1$ for $s\le 1$ and $\chi(s)=0$ for $s\ge 2$.
For $a\in(0,1]$ and radius $r_0>0$ define $\chi_{a}(g):=\chi(d_G(g,e)/(r_0 a))$ on $G$, and on $G^m$ define
\[
  \Chi_{m,a}(u)\ :=\ \prod_{j=1}^m \chi_a(u_j).
\]
Let $\Delta_{q,a,L}$ be as in Lemma~\ref{lem:tick-primitive-sharp-scaling}.
Define the \emph{globalized} mismatch
\[
  \widetilde\Delta_{q,a,L}(u)\ :=\ \Chi_{m,a}(u)\,\Delta_{q,a,L}(u),\qquad u\in G^m.
\]
Then there exists $C'<\infty$ independent of $a,L$ such that
\[
  \|\widetilde\Delta_{q,a,L}\|_{C^2(G^m)}\ \le\ C'\,\beta(a)\,a^6.
\]
\end{lemma}

\begin{proof}
Outside the enlarged window $W_m(a;2r_0)$, $\Chi_{m,a}=0$ so $\widetilde\Delta_{q,a,L}=0$ and there is nothing to bound.
On $W_m(a;2r_0)$, Lemma~\ref{lem:tick-primitive-sharp-scaling} applies with possibly changed constants.

Differentiate $\widetilde\Delta=\Chi\,\Delta$:
\[
  \nabla(\Chi\Delta)=\Chi\nabla\Delta+\Delta\nabla\Chi,\qquad
  \nabla^2(\Chi\Delta)=\Chi\nabla^2\Delta+(\nabla\Chi)\otimes(\nabla\Delta)+(\nabla\Delta)\otimes(\nabla\Chi)+\Delta\nabla^2\Chi.
\]
Because $\chi_a$ transitions on scale $a$, one has $\|\nabla\Chi\|_\infty\lesssim a^{-1}$ and $\|\nabla^2\Chi\|_\infty\lesssim a^{-2}$.
Using the sharp bounds from Lemma~\ref{lem:tick-primitive-sharp-scaling},
\[
  \|\Delta\|_\infty\lesssim \beta a^8,\quad \|\nabla\Delta\|_\infty\lesssim \beta a^7,\quad \|\nabla^2\Delta\|_\infty\lesssim \beta a^6,
\]
we get
\[
  \|\Delta\nabla\Chi\|_\infty\lesssim \beta a^8\cdot a^{-1}=\beta a^7,\quad
  \|(\nabla\Chi)(\nabla\Delta)\|_\infty\lesssim a^{-1}\cdot \beta a^7=\beta a^6,\quad
  \|\Delta\nabla^2\Chi\|_\infty\lesssim \beta a^8\cdot a^{-2}=\beta a^6,
\]
so every term in the $C^2$ norm is $O(\beta a^6)$ uniformly.
\end{proof}

\begin{corollary}[How to make the tick primitive remainder bound global without changing it on the window]\label{cor:tick-primitive-global-remainder}
Assume the setting of Lemma~\ref{lem:tick-primitive-template} and define $\widetilde\Delta_{q,a,L}$ by Lemma~\ref{lem:tick-cutoff-global-C2}.
Define a \emph{patched} comparison primitive by
\[
  \log\widetilde\Phi^{(2)}_{q,a}(u)\ :=\ \log\Phi^{(1)}_{q,a,L}(u)\ -\ \widetilde\Delta_{q,a,L}(u).
\]
Then:
\begin{itemize}
\item[(i)] On $W_m(a;r_0)$ one has $\widetilde\Phi^{(2)}_{q,a}=\Phi^{(2)}_{q,a}$ (so the comparison model is unchanged on the near-identity regime).
\item[(ii)] Outside $W_m(a;2r_0)$ one has $\widetilde\Phi^{(2)}_{q,a}=\Phi^{(1)}_{q,a,L}$ (so the mismatch is \emph{exactly} zero far from the regime where commutator scaling is available).
\item[(iii)] Globally on $G^m$ one has the required primitive remainder bound
\[
  \big\|\log\Phi^{(1)}_{q,a,L}-\log\widetilde\Phi^{(2)}_{q,a}\big\|_{C^2(G^m)}\ =\ \|\widetilde\Delta_{q,a,L}\|_{C^2(G^m)}\ \le\ C'\,\beta(a)\,a^6,
\]
and hence (under any schedule with $\sup_{a\le a_0}\beta(a)a<\infty$) the bound is $O(a^5)$.
\end{itemize}
\end{corollary}

\begin{lemma}[Calibrated moment--matching for primitive stencils]\label{lem:moment-matching-stencil}
Fix an electric primitive $q\in\mathcal Q_a(R)$. Let $\Phi^{(1)}_{q,a,L}$ be the unnormalized lattice density factor and $\Phi^{(2)}_{q,a} = H_{\kappa_0(a)}$ the central heat kernel on $G$ with calibrated time $\kappa_0(a) = \eta_0 a^2 + \eta_1 a^4$. There exist coefficients $\eta_0, \eta_1$ depending only on the group $G$ and the stencil geometry such that
\[
  \Big\|\log\Phi^{(1)}_{q,a,L}-\log\Phi^{(2)}_{q,a}\Big\|_{C^2(S(q))}\ \le\ C_{\rm prim}(R)\,a^5.
\]
In particular, the per-cell Lagrangian consistency $\|\mathcal L^{(1)} - \mathcal L^{(2)}\| \le C \cdot a$ holds.
\end{lemma}

\begin{proof}
On the compact Lie group $G$, both the lattice primitive $\Phi^{(1)}$ and the heat kernel $H_t$ admit Taylor expansions in the link-variable displacement $x = \log(\mathrm{Stap}^{-1} u)$. The log-density $\log H_t(x)$ is a quadratic form in $x$ at leading order $t^{-1}|x|^2$ plus $O(t)$ corrections. By matching the quadratic and quartic moments of $\Phi^{(1)}$ via the $O(a^2)$ and $O(a^4)$ terms in $\kappa_0(a)$, we eliminate the first two error terms in the stencil-wise difference. The remaining terms are $O(a^6)$ at the unnormalized density level; the $C^2$ norm of the log-ratio thus satisfies the $O(a^5)$ bound as the division by $a^4$ in the Lagrangian grouping converts this to $O(a)$.
\end{proof}

\medskip
\begin{lemma}[Magnetic graph--defect $O(a)$]\label{lem:U2-magnetic}
In the setting of Proposition~\ref{lem:graph-defect-Oa}, under the DEC plaquette$\to F^2$ approximation on fixed slabs (Theorem~\ref{DEC:plaquette-F2}), there exists $C_M=C_M(\Omega_T,a_0,G)>0$ such that
\[
  \big\|(M_a+1)^{-1/2}\,(M_a J_a - J_a M)\,(M+1)^{-1/2}\big\|\ \le\ C_M\, a.
\]
\emph{Proof.} On a smooth comparison-model core, Theorem~\ref{DEC:plaquette-F2} furnishes the gauge--invariant second--order control
\[
...
\begin{theorem}[Comparison model on fixed slabs: graph--defect $O(a)$ and low--energy projectors]\label{thm:U2}
Fix a bounded Lipschitz slab $\Omega_T\Subset\mathbb R^4$. Let $H\ge 0$ be the continuum generator on $L^2(\Omega_T;\mathbb{C}^m)$ and let $H_a\ge 0$ be a lattice discretization on $\ell^2(\Omega_{T,a};\mathbb{C}^m)$ with mesh $a\in(0,a_0]$. Let $J_a:L^2\to\ell^2$ be the cell--averaging injection and $J_a^*$ its adjoint (piecewise-constant extension), with $\|J_a\|,\|J_a^*\|\le 1$. Then:
\begin{itemize}
  \item[(A)] \textbf{Graph--defect bound.} There exists $C_{\mathrm{gd}}=C_{\mathrm{gd}}(\Omega_T,a_0,G)>0$ such that
  \[
  ...
  \[
    C_\Lambda\ =\ \frac{2(\Lambda+g+1)}{g}\, C_{\mathrm{gd}}\ =\ C_\Lambda(\Lambda,g,\Omega_T,a_0)
  \]
...
\begin{corollary}[Low--energy projector via contour]\label{lem:low-energy-proj}
Fix $\Lambda>0$ and suppose $g:=\mathrm{dist}(\Lambda,\sigma(H))>0$. Then for all sufficiently small $a$,
\[
  \big\|\,\mathbf 1_{[0,\Lambda]}(H_a)\ -\ J_a\,\mathbf 1_{[0,\Lambda]}(H)\,J_a^*\,\big\|\ \le\ C_\Lambda\, a,\qquad
  C_\Lambda\ :=\ \frac{8}{\pi}\,\frac{(\Lambda+1+g)^2}{g^{2}}\,C_D,
\]
where $C_D$ is the graph--defect constant from Proposition~\ref{lem:graph-defect-Oa}.
\end{corollary}
...
\begin{lemma}[Cell averaging is close to the identity on low energies]\label{lem:cell-avg-lowenergy}
In the setting of Proposition~\ref{lem:graph-defect-Oa} (fixed bounded Lipschitz slab, cell--averaging injection $J_a$ and piecewise-constant extension $J_a^*$), there exists a constant $C_{\rm avg}=C(\Omega_T)>0$ such that for all $u\in \mathrm{Dom}(H^{1/2})$,
\[
  \big\|\,(I-J_a^*J_a)\,u\,\big\|\ \le\ C_{\rm avg}\,a\,\big\|\,(H+1)^{1/2}u\,\big\|.
\]
In particular, for each $\Lambda\ge 0$,
\[
  \big\|\,(I-J_a^*J_a)\,E_H([0,\Lambda])\,\big\|\ \le\ C^{(J)}_\Lambda\,a,
  \qquad
  C^{(J)}_\Lambda\ :=\ C_{\rm avg}\,\sqrt{\Lambda+1}\,.
\]
\end{lemma}
\begin{proof}
Let $\mathcal Q_a$ denote the partition of $\Omega_T$ into mesh-$a$ cubes, and for each $Q\in\mathcal Q_a$ write $\bar u_Q:=|Q|^{-1}\int_Q u$. By definition, $(J_a^*J_a u)|_Q=\bar u_Q$ (piecewise-constant cell average). The Poincar\'e inequality on cubes yields a dimensional constant $c_P$ such that
\[
  \int_Q |u-\bar u_Q|^2\ \le\ c_P\,a^2\int_Q |\nabla u|^2\qquad\text{for all }Q\in\mathcal Q_a,
\]
whence
\[
  \| (I-J_a^*J_a)u\|_{L^2(\Omega_T)}\ \le\ c_P^{1/2}\,a\,\|\nabla u\|_{L^2(\Omega_T)}.
\]
Since $u\in\mathrm{Dom}(H^{1/2})$, the uniform energy equivalence hypothesis in Proposition~\ref{lem:graph-defect-Oa} controls $\|\nabla u\|_{L^2}$ by $\|(H+1)^{1/2}u\|$ up to a constant depending only on $\Omega_T$ (and fixed slab data). Absorbing constants gives the first inequality.

For the spectral-subspace bound, note that for $u\in \mathrm{Ran}\,E_H([0,\Lambda])$ one has $\|(H+1)^{1/2}u\|\le \sqrt{\Lambda+1}\,\|u\|$ by the spectral theorem, and take the operator norm over $\|u\|=1$.
\end{proof}

\begin{theorem}[Quantitative operator-norm NRC for all nonreal $z$]\label{thm:nrc-quant}
Fix $z\in\mathbb C\setminus\mathbb R$ and $\Lambda>0$. Let $P_{a,L}:=I_{a,L}I_{a,L}^*$ and $D_{a,L}:=H I_{a,L}-I_{a,L}H_{a,L}$ as in Lemma~\ref{lem:U2-comparison}. Assume the quantitative U2b inputs of Theorem~\ref{thm:U2b-global}.
Then there exists $C(z,\Lambda)>0$ independent of $(a,L)$ such that
\[
  \big\|(H-z)^{-1} - I_{a,L}(H_{a,L}-z)^{-1} I_{a,L}^*\big\|\ \le\ C(z,\Lambda)\,a\ \ +\ \frac{1}{\operatorname{dist}(z,[\Lambda,\infty))}.
\]
In particular, choosing a cutoff $\Lambda=\Lambda(a)\to\infty$ yields convergence provided the right-hand side tends to $0$; e.g. if $\operatorname{dist}(z,[\Lambda(a),\infty))^{-1}=O(a)$ and $C(z,\Lambda(a))$ stays bounded, then the whole bound is $O(a)$.
\end{theorem}
\begin{theorem}[U2b: quantitative NRC inputs for the OS/GNS embedding framework]\label{thm:U2b-global}
With $P_{a,L}:=I_{a,L}I_{a,L}^*$ and $D_{a,L}:=H I_{a,L}-I_{a,L}H_{a,L}$, the following quantitative NRC inputs hold for any van Hove sequence along the scaling window:
\begin{itemize}
  \item[(i)] \textbf{Graph-defect bound.} There exists $C_{\rm gd}>0$ such that for all sufficiently small $a$ and all $L$,
  \[
    \|(H+1)^{-1/2}\,D_{a,L}\,(H_{a,L}+1)^{-1/2}\|\ \le\ C_{\rm gd}\,a.
  \]
  \item[(ii)] \textbf{Low-energy projection control.} For each $\Lambda\ge 1$ there exists $C_\Lambda<\infty$ such that for all sufficiently small $a$ and all $L$,
  \[
    \|(I-P_{a,L})E_H([0,\Lambda])\|\ \le\ C_\Lambda\,a.
  \]
\end{itemize}
\end{theorem}

\begin{proof}
This is the global van Hove limit of the fixed-region quantitative NRC established in Theorem~\ref{thm:U2b-closure}. Specifically, part (i) follows from the per-cell $O(a)$ Lagrangian consistency derived via primitive stencil factorization and moment-matching (Lemmas~\ref{lem:graph-defect-stencil-closure} and \ref{lem:moment-matching-stencil}), which provides the needed $O(a)$ graph-defect bound on the local OS/GNS spaces. Part (ii) then follows from the spectral perturbation bound of Corollary~\ref{lem:low-energy-proj}. Finiteness of the global constants $(C_{\rm gd}, C_\Lambda)$ follows from the uniform-in-volume locality and UEI bounds established in Section~\ref{sec:lattice-setup} and Section~\ref{sec:global-os-e1e2}.
\end{proof}

\begin{lemma}[Bridge lemma for the OS/GNS embedding framework]\label{lem:bridge-U2b-target}
Fix a bounded region $R\Subset\mathbb R^4$ and a cutoff $\Lambda\ge 1$. Let $(\mathcal H^{\rm cmp}_a,H^{\rm cmp}_a,J_a)$ denote a concrete fixed-slab discretization/comparison model on $R$ where $J_a: L^2 \to \ell^2$ is the cell-averaging injection and $J_a^*: \ell^2 \to L^2$ is a \emph{Whitney-type} or \emph{heat-smoothed} reconstruction operator ensuring $\mathrm{Ran}(J_a^*) \subset \mathrm{Dom}(H^{1/2})$. The quantitative defect/projector bounds proved in Proposition~\ref{lem:graph-defect-Oa} and Lemma~\ref{lem:low-energy-proj}, together with the low-energy approximation property of $J_a^*J_a$ (Lemma~\ref{lem:cell-avg-lowenergy}), ensure that $J_a$ is uniformly bounded in the corresponding graph norms: there exists $C_{\rm inj}<\infty$ such that for all sufficiently small $a$,
\[
  \big\|\,(H^{\rm cmp}_a+1)^{1/2}J_a\,(H+1)^{-1/2}\,\big\|\ \le\ C_{\rm inj},
  \qquad
  \big\|\,(H+1)^{1/2}J_a^*\,(H^{\rm cmp}_a+1)^{-1/2}\,\big\|\ \le\ C_{\rm inj}.
\]

Assume there exist unitary identifications (defined on the time-zero local subspace supported in $R$, and extending by completion to that local OS/GNS subspace)
\[
  V_{a,L}:\mathcal H_{a,L}\ \to\ \mathcal H^{\rm cmp}_a
\]
such that the OS/GNS embedding framework $(I_{a,L},H_{a,L})$ is \emph{quantitatively quasi-unitarily equivalent} to the comparison model $(J_a^*,H^{\rm cmp}_a)$ on $R$, with error $O(a)$ uniformly in $L$ (in energy/graph norms). Concretely, assume:
\begin{itemize}
  \item[(B1)] \textbf{Embedding match (energy/graph weighted).} There exists $C_{\rm br}>0$ such that for all sufficiently small $a$ and all $L$,
  \[
    \big\|\,(I_{a,L}-J_a^*V_{a,L})\,(H_{a,L}+1)^{-1/2}\,\big\|\ \le\ C_{\rm br}\,a,
    \qquad
    \big\|\,(H+1)^{1/2}(I_{a,L}-J_a^*V_{a,L})\,(H_{a,L}+1)^{-1/2}\,\big\|\ \le\ C_{\rm br}\,a,
  \]
  \[
    \big\|\,(I_{a,L}^*-V_{a,L}^*J_a)\,(H+1)^{-1/2}\,\big\|\ \le\ C_{\rm br}\,a,
    \qquad
    \big\|\,(H_{a,L}+1)^{1/2}(I_{a,L}^*-V_{a,L}^*J_a)\,(H+1)^{-1/2}\,\big\|\ \le\ C_{\rm br}\,a.
  \]
  \item[(B2)] \textbf{Generator match and graph-norm stability (graph weighted).} There exist constants $C_{\rm br,H}>0$ and $C_{\rm gr}<\infty$ such that for all sufficiently small $a$ and all $L$,
  \[
    \big\|\,(V_{a,L}H_{a,L}-H^{\rm cmp}_aV_{a,L})\,(H_{a,L}+1)^{-1/2}\,\big\|\ \le\ C_{\rm br,H}\,a.
  \]
  In addition, $V_{a,L}$ is uniformly bounded between the energy/graph norms:
  \[
    \big\|\,(H^{\rm cmp}_a+1)^{1/2}V_{a,L}\,(H_{a,L}+1)^{-1/2}\,\big\|\ \le\ C_{\rm gr},
    \qquad
    \big\|\,(H_{a,L}+1)^{1/2}V_{a,L}^*\,(H^{\rm cmp}_a+1)^{-1/2}\,\big\|\ \le\ C_{\rm gr}.
  \]
\end{itemize}
Then the embedding-side bounds in Theorem~\ref{thm:U2b-global} follow on $R$ (with constants inherited from the proved $J_a$-side bounds, Lemma~\ref{lem:cell-avg-lowenergy}, and the bridge constants in (B1)--(B2)).
\end{lemma}
\begin{proof}
Use (B1)--(B2) to expand the OS/GNS defect
\[
...
\[
  \begin{aligned}
  D_{a,L}
  &=\ (H J_a^*-J_a^*H^{\rm cmp}_a)\,V_{a,L}
    \ +\ H\,(I_{a,L}-J_a^*V_{a,L})
    \ -\ (I_{a,L}-J_a^*V_{a,L})\,H_{a,L}\\
  &\qquad\ -\ J_a^*\,(V_{a,L}H_{a,L}-H^{\rm cmp}_aV_{a,L}),
  \end{aligned}
\]
which holds on the common time-zero local core on $R$. For the comparison term, Proposition~\ref{lem:graph-defect-Oa} (via adjoint) yields
\[
  \big\|\,(H+1)^{-1/2}(H J_a^*-J_a^*H^{\rm cmp}_a)\,(H^{\rm cmp}_a+1)^{-1/2}\,\big\|\ \le\ C_D\,a.
\]
By the graph-norm stability part of (B2), $\|(H^{\rm cmp}_a+1)^{1/2}V_{a,L}(H_{a,L}+1)^{-1/2}\|\le C_{\rm gr}$, so
\[
  \big\|\,(H+1)^{-1/2}(H J_a^*-J_a^*H^{\rm cmp}_a)\,V_{a,L}\,(H_{a,L}+1)^{-1/2}\,\big\|\ \le\ C_D\,C_{\rm gr}\,a.
\]
For the $H(I_{a,L}-J_a^*V_{a,L})$ term, use $\|(H+1)^{-1/2}H(H+1)^{-1/2}\|\le 1$ and the second inequality in (B1) to get
\[
  \big\|\,(H+1)^{-1/2}H(I_{a,L}-J_a^*V_{a,L})\,(H_{a,L}+1)^{-1/2}\,\big\|\ \le\ C_{\rm br}\,a.
\]
For the $(I_{a,L}-J_a^*V_{a,L})H_{a,L}$ term, write $H_{a,L}=(H_{a,L}+1)-1$ and use the inequalities in (B1) (and their adjoints) to bound
\[
  \big\|\,(H+1)^{-1/2}(I_{a,L}-J_a^*V_{a,L})H_{a,L}\,(H_{a,L}+1)^{-1/2}\,\big\|\ \le\ 2C_{\rm br}\,a.
\]
Finally, for the $J_a^*(V_{a,L}H_{a,L}-H^{\rm cmp}_aV_{a,L})$ term, use $\|(H+1)^{-1/2}\|\le 1$, $\|J_a^*\|\le 1$, and (B2) to obtain an $O(a)$ bound. Collecting these estimates yields the U2b graph-defect bound $\|(H+1)^{-1/2}D_{a,L}(H_{a,L}+1)^{-1/2}\|\le C_{\rm gd}a$ for an explicit admissible $C_{\rm gd}$.

For the low-energy projection control, write $P_{a,L}=I_{a,L}I_{a,L}^*$ and note the decomposition
\[
  (I-P_{a,L})E_H([0,\Lambda])\ =\ (I-J_a^*J_a)E_H([0,\Lambda])\ +\ (J_a^*J_a-P_{a,L})E_H([0,\Lambda]).
\]
The first term is controlled by Lemma~\ref{lem:cell-avg-lowenergy}, giving
\[
  \big\|\,(I-J_a^*J_a)\,E_H([0,\Lambda])\,\big\|\ \le\ C^{(J)}_\Lambda\,a.
\]
For the second term, use $(J_a^*V_{a,L})(J_a^*V_{a,L})^*=J_a^*J_a$ (since $V_{a,L}$ is unitary) and expand
\[
  P_{a,L}-J_a^*J_a\ =\ I_{a,L}I_{a,L}^*-(J_a^*V_{a,L})(J_a^*V_{a,L})^*
  \ =\ (I_{a,L}-J_a^*V_{a,L})\,I_{a,L}^*\ +\ (J_a^*V_{a,L})\,(I_{a,L}^*-V_{a,L}^*J_a).
\]
Since $\|J_a^*V_{a,L}\|\le 1$, for $E:=E_H([0,\Lambda])$ we obtain
\[
  \|(P_{a,L}-J_a^*J_a)E\|
  \ \le\ \|(I_{a,L}-J_a^*V_{a,L})\,I_{a,L}^*E\|\ +\ \|(I_{a,L}^*-V_{a,L}^*J_a)E\|.
\]
By the spectral theorem, $\|(H+1)^{1/2}E\|\le \sqrt{\Lambda+1}$. For the second term, use (B1) to get
\[
  \|(I_{a,L}^*-V_{a,L}^*J_a)E\|
  \ \le\ \|(I_{a,L}^*-V_{a,L}^*J_a)(H+1)^{-1/2}\|\,\|(H+1)^{1/2}E\|
  \ \le\ C_{\rm br}\,\sqrt{\Lambda+1}\,a.
\]
For the first term, insert $(H_{a,L}+1)^{\pm 1/2}$:
\[
  \|(I_{a,L}-J_a^*V_{a,L})\,I_{a,L}^*E\|
  \ \le\ \|(I_{a,L}-J_a^*V_{a,L})(H_{a,L}+1)^{-1/2}\|\,\|(H_{a,L}+1)^{1/2}I_{a,L}^*(H+1)^{-1/2}\|\,\|(H+1)^{1/2}E\|.
\]
The first factor is $\le C_{\rm br}a$ by (B1). For the middle factor, write $I_{a,L}^*=V_{a,L}^*J_a+(I_{a,L}^*-V_{a,L}^*J_a)$ and use (B1), (B2), and the comparison-model graph bound for $J_a$:
\[
  \|(H_{a,L}+1)^{1/2}I_{a,L}^*(H+1)^{-1/2}\|
  \ \le\ C_{\rm gr}\,\|(H^{\rm cmp}_a+1)^{1/2}J_a(H+1)^{-1/2}\|\ +\ C_{\rm br}\,a
  \ \le\ C_{\rm gr}\,C_{\rm inj}\ +\ C_{\rm br}\,a.
\]
Therefore
\[
  \|(I_{a,L}-J_a^*V_{a,L})\,I_{a,L}^*E\|
  \ \le\ C_{\rm br}\,\big(C_{\rm gr}C_{\rm inj}+C_{\rm br}a\big)\,\sqrt{\Lambda+1}\,a,
\]
so $\|(P_{a,L}-J_a^*J_a)E_H([0,\Lambda])\|\le C_{\rm br}\big(1+C_{\rm gr}C_{\rm inj}+C_{\rm br}a\big)\sqrt{\Lambda+1}\,a$. Consequently,
\[
  \|(I-P_{a,L})E_H([0,\Lambda])\|
  \ \le\ \Big(C^{(J)}_\Lambda\ +\ C_{\rm br}\big(1+C_{\rm gr}C_{\rm inj}+C_{\rm br}a\big)\sqrt{\Lambda+1}\Big)\,a,
\]
which is the desired low-energy projection control with an explicit admissible choice of $C_\Lambda$ (for instance, take $a\le a_0$ and absorb $C_{\rm br}^2 a_0$ into the constant).
\end{proof}
\noindent\emph{Remark (how one would verify (B1)--(B2); form-level reduction).} The content of (B1)--(B2) is a \emph{quantitative quasi-unitary equivalence} between the OS/GNS embedding data and a concrete comparison discretization on $R$. A standard, checkable route is to prove corresponding \emph{form-approximation inequalities} on a common dense core after identifying the Hilbert spaces (cf.\ Section~\ref{NRC:form} and Theorem~\ref{NRC:form-thm}). Concretely:
\begin{itemize}
  \item[(i)] \textbf{(B2) split: graph-norm stability vs.\ generator match.} The bridge hypothesis (B2) contains two kinds of analytic input: an $O(a)$ \emph{generator-match} bound for $V_{a,L}H_{a,L}-H^{\rm cmp}_aV_{a,L}$, and uniform \emph{graph-norm stability} bounds (the constant $C_{\rm gr}$). A standard, checkable route to the graph-norm stability (and to a sandwiched form control for $V_{a,L}^*(H^{\rm cmp}_a+1)V_{a,L}-(H_{a,L}+1)$) is to prove a bilinear \emph{operator-core} estimate: on a common core $\mathsf D_R\subset\mathrm{Dom}(H_{a,L})$ mapped by $V_{a,L}$ into $\mathrm{Dom}(H^{\rm cmp}_a)$,
  \[
    \big|\ \langle (H^{\rm cmp}_a+1)^{1/2}V_{a,L}\psi,\ (H^{\rm cmp}_a+1)^{1/2}V_{a,L}\varphi\rangle
    - \langle (H_{a,L}+1)^{1/2}\psi,\ (H_{a,L}+1)^{1/2}\varphi\rangle\ \big|
    \ \le\ C\,a\,\|\psi\|_{a,L}\,\|\varphi\|_{a,L},
  \]
  where $\|\psi\|_{a,L}:=\|(H_{a,L}+1)^{1/2}\psi\|$. On such a core one may rewrite the left-hand side as $|\langle \psi,(V_{a,L}^*(H^{\rm cmp}_a+1)V_{a,L}-(H_{a,L}+1))\varphi\rangle|$, so this is an explicit graph-boundedness condition for the conjugated generator difference. (We do not attempt to derive (B2) from abstract form convergence alone; rather, this kind of core estimate is the checkable analytic content one seeks in concrete discretizations.)
  In particular, it yields the $C_{\rm gr}$ graph-norm stability bounds in (B2) (after choosing $a\le a_0$ and absorbing the $O(a)$ term into constants). The remaining generator-match estimate in (B2), i.e.\ an $O(a)$ bound for the \emph{unsandwiched} operator defect $(V_{a,L}H_{a,L}-H^{\rm cmp}_aV_{a,L})(H_{a,L}+1)^{-1/2}$, is typically strictly stronger and requires additional local-core regularity input in concrete discretizations.
  A convenient CPM-style reformulation is to establish instead a small-time semigroup match: by Lemma~\ref{lem:bridge-generator-from-semigroup}, it suffices to prove that for some $t_0>0$,
  \[
    \big\|\,(V_{a,L} e^{-tH_{a,L}}-e^{-tH^{\rm cmp}_a}V_{a,L})\,(H_{a,L}+1)^{-1/2}\,\big\|\ \le\ C\,a\,t
    \qquad (0<t\le t_0).
  \]
  Such bounds are naturally attacked by comparing the underlying local transfer/Markov updates (recall $T_{a,L}=e^{-aH_{a,L}}$) and using a Strang/Trotter splitting argument for an electric/magnetic decomposition on the common time-zero local core (cf.\ the Strang envelope in Lemma~\ref{lem:local-commutator-Oa2}, included as an optional cross-check).
  In particular, a directly checkable “one-tick” estimate is an $O(a^2)$ intertwining estimate
  \[
    \big\|\,(V_{a,L}e^{-aH_{a,L}}-e^{-aH^{\rm cmp}_a}V_{a,L})\,(H_{a,L}+1)^{-1/2}\,\big\|\ \lesssim\ a^2
  \]
  on the fixed-region time-zero local core; by Lemma~\ref{lem:bridge-semigroup-from-one-tick} this yields the semigroup match $\lesssim a\,t$ at lattice times $t=na$.
  \emph{Caution.} Lemma~\ref{lem:bridge-semigroup-from-one-tick} provides control only at \emph{lattice times} $t=na$; by itself it does \emph{not} imply the continuum-in-$t$ small-time estimate displayed above that is needed to invoke Lemma~\ref{lem:bridge-generator-from-semigroup}. Closing that gap requires an additional input that controls the semigroup match for a continuum of small $t$ (e.g.\ a genuine time-continuous fixed-core semigroup comparison, or an alternative functional-calculus route that upgrades a one-tick intertwining estimate to a generator defect bound).
  (We do \emph{not} use any spectral-gap/coercivity input to prove the bridge itself; gap persistence is a downstream consequence once norm--resolvent convergence is established.)
  \item[(ii)] \textbf{(B1) as a core embedding approximation.} The embedding match (B1) is the corresponding $O(a)$ approximation of the two embeddings into the same continuum OS/GNS space, measured from the $(H_{a,L}+1)^{1/2}$ graph norm both in the ambient norm and (when needed for defect bounds) in the $(H+1)^{1/2}$ graph norm. In practice it reduces to deterministic geometric estimates comparing the polygonal/smoothed embedding defining $I_{a,L}$ with the piecewise-constant extension defining $J_a^*$ (after identifying lattice degrees of freedom by $V_{a,L}$) on the time-zero cylinder core supported in $R$.
  \item[(iii)] \textbf{Constructing $V_{a,L}$ on the local core.} Abstract unitaries between separable Hilbert spaces always exist; the bridge requires a \emph{canonical} identification compatible with the time-zero local observables on $R$. Concretely, one seeks an explicit identification of a dense time-zero local core in $\mathcal H_{a,L}$ (generated by gauge-invariant observables supported in $R$) with a dense domain in the comparison space $\mathcal H^{\rm cmp}_a$ (e.g. via a coordinate map such as tree gauge; cf.\ Lemma~\ref{lem:U1-A-tree-gauge}, or an explicit discretization map on the same local generators), and then extends by completion.

  A particularly concrete template is available when one has an $L^2$ realization on the same gauge-fixed configuration manifold. Namely, assume that after tree gauge on $R$ both local Hilbert spaces admit realizations
  \[
    \mathcal H_{a,L}(R)\ \cong\ L^2(G^{m(R,a)},\mu_{a,L}),\qquad
    \mathcal H^{\rm cmp}_a(R)\ \cong\ L^2(G^{m(R,a)},\mu^{\rm cmp}_a),
  \]
  with $\mu_{a,L}$ and $\mu^{\rm cmp}_a$ absolutely continuous with respect to product Haar $\pi^{\otimes m}$ on $G^{m(R,a)}$. Then a canonical unitary identification is the \emph{density isometry}
  \[
    (V_{a,L}f)(u)\ :=\ f(u)\,\sqrt{\frac{d\mu_{a,L}}{d\mu^{\rm cmp}_a}(u)}\,.
  \]
  Quantitative versions of (B1)--(B2) reduce to controlling the induced inner products and Dirichlet forms on a common smooth cylinder core in these coordinates (in particular, controlling $\log(d\mu_{a,L}/d\mu^{\rm cmp}_a)$ and the corresponding form differences in graph norms), which is precisely the analytic content encoded by (B1)--(B2).
\end{itemize}
\begin{lemma}[Tree-gauge $L^2$ density-isometry bridge criterion]\label{lem:bridge-density-isometry-target}
In the setting of Lemma~\ref{lem:bridge-U2b-target}, suppose that on a fixed bounded region $R$ one has a tree-gauge coordinate description of the time-zero local core such that both the lattice OS/GNS local space $\mathcal H_{a,L}(R)$ and the comparison space $\mathcal H^{\rm cmp}_a(R)$ admit $L^2$ realizations on the \emph{same} gauge-fixed configuration manifold $G^{m(R,a)}$ with absolutely continuous measures $\mu_{a,L}$ and $\mu^{\rm cmp}_a$ (with respect to $\pi^{\otimes m}$). Define $V_{a,L}$ as the density isometry $f\mapsto f\sqrt{d\mu_{a,L}/d\mu^{\rm cmp}_a}$ on a common smooth cylinder core.

Write $\rho_{a,L}:=d\mu_{a,L}/d\pi^{\otimes m}$ and $\rho^{\rm cmp}_a:=d\mu^{\rm cmp}_a/d\pi^{\otimes m}$ and set the density ratio
\[
  r_{a,L}\ :=\ \frac{d\mu_{a,L}}{d\mu^{\rm cmp}_a}\ =\ \frac{\rho_{a,L}}{\rho^{\rm cmp}_a}\,.
\]
Then verifying the bridge conditions (B1)--(B2) is reduced to checkable estimates on a common smooth cylinder core in these coordinates. Concretely, a sufficient checklist is:
\begin{itemize}
  \item[(D1)] \textbf{Near-identity density ratio (local).} There is $C_{\rm dens}<\infty$ such that for all sufficiently small $a$ and all $L$,
  \[
    \|\log r_{a,L}\|_{L^\infty(G^{m(R,a)})}\ \le\ C_{\rm dens}\,a.
  \]
  In particular,
  \[
    \big\|V_{a,L}-\mathbf 1\big\|\ =\ \big\|\sqrt{r_{a,L}}-\mathbf 1\big\|_{L^\infty}\ \le\ e^{C_{\rm dens}a/2}-1\ \le\ C'_{\rm dens}\,a
  \]
  for $a\le a_0$, and $V_{a,L}$ is uniformly bounded and invertible with $\|V_{a,L}^{\pm 1}\|\le e^{C_{\rm dens}a/2}$.

  \item[(D2)] \textbf{Graph-norm stability via a form estimate (sufficient for the $C_{\rm gr}$ part of (B2)).} There exists $C_{\rm form}<\infty$ such that for all $\psi,\varphi$ in the common smooth cylinder core,
  \[
    \big|\ \langle (H^{\rm cmp}_a+1)^{1/2}V_{a,L}\psi,\ (H^{\rm cmp}_a+1)^{1/2}V_{a,L}\varphi\rangle_{\mu^{\rm cmp}_a}
    - \langle (H_{a,L}+1)^{1/2}\psi,\ (H_{a,L}+1)^{1/2}\varphi\rangle_{\mu_{a,L}}\ \big|
    \ \le\ C_{\rm form}\,a\,\|\psi\|_{a,L}\,\|\varphi\|_{a,L},
  \]
  where $\|\psi\|_{a,L}:=\|(H_{a,L}+1)^{1/2}\psi\|_{\mu_{a,L}}$. This is precisely the bilinear operator-core estimate displayed in Remark~(i) after Lemma~\ref{lem:bridge-U2b-target} when $V_{a,L}$ is the density isometry; in particular it yields the graph-norm stability bounds in (B2) (after absorbing the $O(a)$ factor into constants for $a\le a_0$).
  The \emph{generator-match} part of (B2), namely an $O(a)$ bound for $(V_{a,L}H_{a,L}-H^{\rm cmp}_aV_{a,L})(H_{a,L}+1)^{-1/2}$ on the local core, is a separate analytic estimate and must still be verified in the chosen comparison setup. In the density-isometry setting, mere $L^\infty$ control of $\log r_{a,L}$ (hence $\|V_{a,L}-\mathbf 1\|\lesssim a$) is typically \emph{not} sufficient to control commutators with the electric Laplacian; one needs quantitative control of derivatives of $r_{a,L}$ on the common tree-gauged cylinder core (see Lemma~\ref{lem:bridge-B2-density-derivatives} below for an explicit checklist).

  \item[(D3)] \textbf{Geometric embedding comparison (operator form; exact content of (B1)).} There exists $C_{\rm geom}<\infty$ such that for all sufficiently small $a$ and all $L$,
  \[
    \big\|\,(I_{a,L}-J_a^*V_{a,L})\,(H_{a,L}+1)^{-1/2}\,\big\|\ \le\ C_{\rm geom}\,a,
    \qquad
    \big\|\,(H+1)^{1/2}(I_{a,L}-J_a^*V_{a,L})\,(H_{a,L}+1)^{-1/2}\,\big\|\ \le\ C_{\rm geom}\,a,
  \]
  and, for the adjoints,
  \[
    \big\|\,(I_{a,L}^*-V_{a,L}^*J_a)\,(H+1)^{-1/2}\,\big\|\ \le\ C_{\rm geom}\,a,
    \qquad
    \big\|\,(H_{a,L}+1)^{1/2}(I_{a,L}^*-V_{a,L}^*J_a)\,(H+1)^{-1/2}\,\big\|\ \le\ C_{\rm geom}\,a.
  \]
  This is exactly the embedding-match hypothesis (B1) of Lemma~\ref{lem:bridge-U2b-target} (on the fixed-region time-zero local subspace). In practice, these bounds reduce to deterministic geometric estimates comparing the polygonal/smoothed reconstruction map underlying $I_{a,L}$ with the piecewise-constant/cell-extension map underlying $J_a^*$ after identifying local generators via tree gauge and $V_{a,L}$ on the common cylinder core.
\end{itemize}
Under (D1)--(D3), the bridge inputs are reduced to an explicit and finite checklist on the tree-gauge cylinder core: (D3) is exactly (B1); (D2) provides the $C_{\rm gr}$ graph-norm stability part of (B2); and the remaining substantive analytic bridge input is the generator-match estimate in (B2), i.e.\ control of the local-core defect $(V_{a,L}H_{a,L}-H^{\rm cmp}_aV_{a,L})(H_{a,L}+1)^{-1/2}=O(a)$.
\end{lemma}
%
%
\begin{lemma}[Density-isometry commutator identity and a checkable sufficient condition for the generator-match gate (B2)]\label{lem:bridge-B2-density-derivatives}
Assume the tree-gauge density-isometry setup of Lemma~\ref{lem:bridge-density-isometry-target} on a fixed region $R$, with density ratio $r_{a,L}=d\mu_{a,L}/d\mu^{\rm cmp}_a$ and density isometry
\[
  (V_{a,L}f)(u)\ =\ f(u)\,\sqrt{r_{a,L}(u)}.
\]
Assume further that on the common smooth cylinder core in these coordinates the two generators admit a common electric part and bounded magnetic parts in the following sense:
there exists a nonnegative “electric” operator $E_a$ given on the cylinder core by a finite sum of single-coordinate Laplace--Beltrami operators
\[
  E_a\ :=\ \kappa(a)\sum_{e=1}^{m(R,a)} \Delta_e,\qquad \kappa(a)>0,
\]
and bounded multiplication operators $M^{(1)}_{a,L}$ and $M^{(2)}_{a}$ (``magnetic terms'') such that, as differential expressions on the cylinder core,
\[
  H_{a,L}\ =\ E_a + M^{(1)}_{a,L}\quad\text{on }L^2(\mu_{a,L}),
  \qquad
  H^{\rm cmp}_a\ =\ E_a + M^{(2)}_{a}\quad\text{on }L^2(\mu^{\rm cmp}_a),
\]
where $\Delta_e$ is the (nonnegative) Laplace--Beltrami operator on the $e$-th $G$--coordinate and $\nabla_e$ the corresponding gradient.

Then on the cylinder core one has the pointwise Leibniz identity
\[
  \Delta_e(\sqrt{r_{a,L}}\,f)\ =\ \sqrt{r_{a,L}}\,\Delta_e f\ +\ f\,\Delta_e\sqrt{r_{a,L}}\ +\ 2\langle \nabla_e\sqrt{r_{a,L}},\ \nabla_e f\rangle,
\]
and hence the generator defect admits the explicit decomposition
\begin{equation}\label{eq:bridge-defect-density-explicit}
  (V_{a,L}H_{a,L}-H^{\rm cmp}_aV_{a,L})f
  \ =\ \sqrt{r_{a,L}}\,(M^{(1)}_{a,L}-M^{(2)}_a)\,f
  \ -\ (E_a\sqrt{r_{a,L}})\,f
  \ -\ 2\kappa(a)\sum_{e=1}^{m(R,a)} \langle \nabla_e\sqrt{r_{a,L}},\ \nabla_e f\rangle.
\end{equation}

Consequently, writing $\|\cdot\|_\infty$ for $L^\infty(G^{m(R,a)})$ with respect to $\pi^{\otimes m}$, one obtains the \emph{relative bound} on the cylinder core
\[
  \|(V_{a,L}H_{a,L}-H^{\rm cmp}_aV_{a,L})f\|_{L^2(\mu^{\rm cmp}_a)}
  \ \le\ \Big(\|\sqrt{r_{a,L}}(M^{(1)}_{a,L}-M^{(2)}_a)\|_\infty+\|E_a\sqrt{r_{a,L}}\|_\infty\Big)\,\|f\|_{L^2(\mu^{\rm cmp}_a)}
  \ +\ 2\sqrt{\kappa(a)}\,\Xi_{a,L}\,\|E_a^{1/2}f\|_{L^2(\mu^{\rm cmp}_a)},
\]
where
\[
  \Xi_{a,L}\ :=\ \Big(\sum_{e=1}^{m(R,a)} \|\nabla_e\sqrt{r_{a,L}}\|_\infty^{\,2}\Big)^{1/2}.
\]
\noindent\emph{Normalization note.} The coefficient $\sqrt{\kappa(a)}$ in the $E_a^{1/2}$ term above comes from the definition $E_a=\kappa(a)\sum_e\Delta_e$ (so $E_a^{1/2}=\sqrt{\kappa(a)}\,(\sum_e\Delta_e)^{1/2}$) and a Cauchy--Schwarz estimate for $\sum_e\langle\nabla_e\sqrt r,\nabla_e f\rangle$.
In particular, if on the fixed region $R$ one can verify the \emph{derivative checklist}
\[
  \|\sqrt{r_{a,L}}(M^{(1)}_{a,L}-M^{(2)}_a)\|_\infty\ \le\ C\,a,\qquad
  \|E_a\sqrt{r_{a,L}}\|_\infty\ \le\ C\,a,\qquad
  \sqrt{\kappa(a)}\,\Xi_{a,L}\ \le\ C\,a,
\]
uniformly in $L$ along the van Hove window, then the defect is $O(a)$ in the natural “$L^2$+$E_a^{1/2}$” graph norm on the cylinder core. Combined with the already-separated graph-norm stability part of (B2) (the constant $C_{\rm gr}$, obtainable from the form estimate (D2)), this yields the generator-match estimate in (B2).

\smallskip
\noindent\emph{Remark.} The bounds above can equivalently be formulated in terms of $\nabla\log r_{a,L}$ and $\Delta\log r_{a,L}$ using $\nabla\sqrt r = \tfrac12 \sqrt r\,\nabla\log r$ and the corresponding identity for $E_a\sqrt r$. Either way, this lemma makes explicit the unavoidable fact: controlling the generator defect requires quantitative control of \emph{derivatives} of the density ratio on the common tree-gauged cylinder core, not only $L^\infty$ control of $\log r_{a,L}$.
\smallskip
\noindent\emph{Remark (magnetic mismatch).} In the natural gauge-theoretic comparison where the comparison generator is chosen to share the same Wilson plaquette potential on the fixed region (so the only difference is in the electric/kinetic part and/or in the measure identification), one may arrange $M^{(2)}_a=M^{(1)}_{a,L}$ on the cylinder core, in which case the magnetic mismatch term in \eqref{eq:bridge-defect-density-explicit} vanishes identically.
\end{lemma}
%
%
\paragraph{Local expansion hypothesis as per-cell consistency (deterministic reduction).}
The local decomposition \eqref{eq:local-log-r-decomp} assumed in Lemma~\ref{lem:bridge-derivative-counting} is a convenient
``finite-range'' way to package the fixed-region analytic content of the bridge: it says that $\log r_{a,L}$ is a sum of
local stencil terms of per-cell size $a^5=a^4\cdot a$ with bounded overlap and uniform $C^2$ bounds.
The deterministic lemmas below isolate a simple sufficient condition for \eqref{eq:local-log-r-decomp}:
a fixed-range local representation of the two log-densities on the common tree-gauged cylinder core, together with a
first-order \emph{per-cell $O(a)$ consistency} bound for the local cell terms.
(Verifying such a representation and $O(a)$ bound uniformly in $L$ for the \emph{actual} OS/GNS versus comparison pair
is the remaining RG-grade analytic issue; see the remark after Lemma~\ref{lem:bridge-derivative-counting}.)

\begin{lemma}[Geometry bookkeeping: $|\mathcal I_a(R)|=O(a^{-4})$ and bounded overlap for fixed-range stencils]\label{lem:local-geom-count}
Fix a bounded region $R\Subset\mathbb R^4$ and a lattice spacing $a>0$.
Let $\mathcal I_a(R)$ be any index set of \emph{cells} in $R$ with mesh $a$ and uniformly bounded physical diameter
(e.g.\ all $4$--cubes with closure in $R$, or all plaquettes with closure in $R$).
For each $\alpha\in\mathcal I_a(R)$ let $S(\alpha)\subset\{1,\dots,m(R,a)\}$ be a finite \emph{stencil} of coordinate indices
with the following fixed-range property: there is a constant $\rho_R<\infty$ independent of $a$
such that $S(\alpha)$ contains only coordinates whose geometric support lies within distance $\rho_R a$ of the cell $\alpha$
(e.g.\ distance between a representative point of the coordinate and a representative point of $\alpha$).

Then:
\begin{itemize}
  \item[(i)] \textbf{Cardinality.} There exists $C_I(R)<\infty$ such that $|\mathcal I_a(R)|\le C_I(R)\,a^{-4}$ for all sufficiently small $a$.
  \item[(ii)] \textbf{Bounded overlap.} There exists $D_0(R)<\infty$ such that each coordinate index $e\in\{1,\dots,m(R,a)\}$
  belongs to at most $D_0(R)$ stencils $S(\alpha)$.
\end{itemize}
\end{lemma}
\begin{proof}
For (i), each cell has $4$--volume $\asymp a^4$ and $R$ has finite volume, so the number of disjoint $a$--scale cells in $R$ is $O(a^{-4})$.
Formally: cover $R$ by $O(a^{-4})$ cubes of side $a$ and note each admissible cell $\alpha$ lies in $O(1)$ such cubes, giving
$|\mathcal I_a(R)|\le C_I(R)a^{-4}$.

For (ii), fix a coordinate $e$ and let $x_e$ be a representative point of its geometric support (e.g.\ the midpoint of the corresponding link/chord).
If $e\in S(\alpha)$ then by the fixed-range stencil property $\alpha$ must lie in a ball of radius $\rho_R a$ around $x_e$.
The number of $a$--mesh cells whose support intersects such a ball is bounded by a constant depending only on $\rho_R$ and the dimension (here $4$),
hence by a constant depending only on $R$ through $\rho_R$.
This gives the uniform overlap bound $D_0(R)$.
\end{proof}

\begin{lemma}[Local $a^5$ expansion for $\log r_{a,L}$ from first-order per-cell consistency]\label{lem:local-expansion-logr}
Fix $R\Subset\mathbb R^4$ and work in tree-gauge coordinates $u\in G^{m(R,a)}$ with product Haar $\pi^{\otimes m}$.
Let $\mu_{a,L}$ and $\mu^{\rm cmp}_a$ be probability measures on $G^{m(R,a)}$ with strictly positive $C^2$ densities
$\rho_{a,L}$ and $\rho^{\rm cmp}_a$ with respect to $\pi^{\otimes m}$, and set
\[
  r_{a,L}(u)\ :=\ \frac{d\mu_{a,L}}{d\mu^{\rm cmp}_a}(u)\ =\ \frac{\rho_{a,L}(u)}{\rho^{\rm cmp}_a(u)}.
\]

Assume the two log-densities admit a common fixed-range local decomposition on $R$:
there exist an index set $\mathcal I_a(R)$ and stencils $S(\alpha)$ as in Lemma~\ref{lem:local-geom-count} such that
\[
  \log\rho_{a,L}(u)\ =\ -\sum_{\alpha\in\mathcal I_a(R)} a^4\,\mathcal L^{(1)}_{\alpha,a,L}\!\big(u|_{S(\alpha)}\big)\ -\ \log Z^{(1)}_{a,L},
  \qquad
  \log\rho^{\rm cmp}_a(u)\ =\ -\sum_{\alpha\in\mathcal I_a(R)} a^4\,\mathcal L^{(2)}_{\alpha,a}\!\big(u|_{S(\alpha)}\big)\ -\ \log Z^{(2)}_{a},
 \]
where $Z^{(1)}_{a,L},Z^{(2)}_a>0$ and each $\mathcal L^{(j)}_{\alpha,\bullet}$ is $C^2$ in its stencil coordinates.

Assume moreover a first-order \emph{per-cell consistency} bound:
there exists $C_{\mathcal L}(R)<\infty$ such that for all $\alpha,a,L$,
\[
  \big\|\mathcal L^{(1)}_{\alpha,a,L}-\mathcal L^{(2)}_{\alpha,a}\big\|_\infty\ \le\ C_{\mathcal L}(R)\,a,
  \qquad
  \max_{e\in S(\alpha)}\Big(
  \big\|\nabla_e(\mathcal L^{(1)}_{\alpha,a,L}-\mathcal L^{(2)}_{\alpha,a})\big\|_\infty
  +\big\|\Delta_e(\mathcal L^{(1)}_{\alpha,a,L}-\mathcal L^{(2)}_{\alpha,a})\big\|_\infty
  \Big)\ \le\ C_{\mathcal L}(R)\,a,
\]
where $\nabla_e,\Delta_e$ are left-invariant derivatives in the $e$-th coordinate (when $e\in S(\alpha)$).

Then $\log r_{a,L}$ admits a local expansion of the form
\[
  \log r_{a,L}(u)\ =\ \sum_{\alpha\in\mathcal I_a(R)} a^5\,\varepsilon_{\alpha,a,L}\!\big(u|_{S(\alpha)}\big),
\]
and, with $C_I(R),D_0(R)$ from Lemma~\ref{lem:local-geom-count}, there is $C_\varepsilon(R)<\infty$ such that for all $\alpha,a,L$,
\[
  \|\varepsilon_{\alpha,a,L}\|_\infty\ \le\ C_\varepsilon(R),\qquad
  \max_{e\in S(\alpha)}\big(\|\nabla_e\varepsilon_{\alpha,a,L}\|_\infty+\|\Delta_e\varepsilon_{\alpha,a,L}\|_\infty\big)\ \le\ C_\varepsilon(R).
\]
In particular, the hypotheses \eqref{eq:local-log-r-decomp}--(i)--(iii) of Lemma~\ref{lem:bridge-derivative-counting} hold (with these constants).
\end{lemma}

\begin{proof}
Write $S^{(1)}(u):=\sum_{\alpha\in\mathcal I_a(R)} a^4\,\mathcal L^{(1)}_{\alpha,a,L}(u|_{S(\alpha)})$ and
$S^{(2)}(u):=\sum_{\alpha\in\mathcal I_a(R)} a^4\,\mathcal L^{(2)}_{\alpha,a}(u|_{S(\alpha)})$.
Then
\[
  \log r_{a,L}(u)
  = \log\rho_{a,L}(u)-\log\rho^{\rm cmp}_a(u)
  = -\big(S^{(1)}(u)-S^{(2)}(u)\big)\ -\ \log\frac{Z^{(1)}_{a,L}}{Z^{(2)}_{a}}.
\]

\emph{Step 1: the action difference carries an $a^5$ factor.}
For each $\alpha$ define the local remainder
\[
  \widetilde\varepsilon_{\alpha,a,L}(v)\ :=\ -\,\frac{\mathcal L^{(1)}_{\alpha,a,L}(v)-\mathcal L^{(2)}_{\alpha,a}(v)}{a},
  \qquad v\in G^{|S(\alpha)|}.
\]
Then $\|\widetilde\varepsilon_{\alpha,a,L}\|_\infty\le C_{\mathcal L}(R)$ and
$\max_{e\in S(\alpha)}(\|\nabla_e\widetilde\varepsilon_{\alpha,a,L}\|_\infty+\|\Delta_e\widetilde\varepsilon_{\alpha,a,L}\|_\infty)\le C_{\mathcal L}(R)$.
Moreover,
\[
  -\big(S^{(1)}(u)-S^{(2)}(u)\big)
  = \sum_{\alpha\in\mathcal I_a(R)} a^4\cdot a\,
  \widetilde\varepsilon_{\alpha,a,L}\!\big(u|_{S(\alpha)}\big)
  = \sum_{\alpha\in\mathcal I_a(R)} a^5\,\widetilde\varepsilon_{\alpha,a,L}\!\big(u|_{S(\alpha)}\big).
\]

\emph{Step 2: absorb the normalization constant into local terms.}
Set $\Delta:=\log(Z^{(1)}_{a,L}/Z^{(2)}_{a})$.
Using $e^{-\|S^{(1)}-S^{(2)}\|_\infty}\le e^{-(S^{(1)}(u)-S^{(2)}(u))}\le e^{\|S^{(1)}-S^{(2)}\|_\infty}$ and integrating with respect to $\pi^{\otimes m}$ gives
$e^{-\|S^{(1)}-S^{(2)}\|_\infty}\le Z^{(1)}_{a,L}/Z^{(2)}_{a}\le e^{\|S^{(1)}-S^{(2)}\|_\infty}$, hence $|\Delta|\le \|S^{(1)}-S^{(2)}\|_\infty$.
But by Step 1,
\[
  \|S^{(1)}-S^{(2)}\|_\infty
  \le \sum_{\alpha\in\mathcal I_a(R)} a^5\,\|\widetilde\varepsilon_{\alpha,a,L}\|_\infty
  \le |\mathcal I_a(R)|\,a^5\,C_{\mathcal L}(R).
\]
Define the constant
\[
  c_{a,L}\ :=\ -\,\frac{\Delta}{|\mathcal I_a(R)|\,a^5},
\]
and set $\varepsilon_{\alpha,a,L}(v):=\widetilde\varepsilon_{\alpha,a,L}(v)+c_{a,L}$.
Then $\sum_{\alpha} a^5 c_{a,L}=-\Delta$ and therefore $\log r_{a,L}=\sum_{\alpha} a^5\,\varepsilon_{\alpha,a,L}(u|_{S(\alpha)})$.
Also $|c_{a,L}|\le C_{\mathcal L}(R)$ by the bound on $|\Delta|$ above, and $c_{a,L}$ has zero derivatives.
Thus the displayed uniform $L^\infty$ and derivative bounds hold with $C_\varepsilon(R):=2C_{\mathcal L}(R)$.
The cardinality and overlap properties are Lemma~\ref{lem:local-geom-count}.
\end{proof}

\begin{lemma}[Analogous local $a^5$ expansion for magnetic mismatch]\label{lem:local-expansion-magnetic}
Fix $R\Subset\mathbb R^4$ and tree-gauge coordinates $u\in G^{m(R,a)}$.
Let $M^{(1)}_{a,L}=\mathrm{mult}(m^{(1)}_{a,L})$ and $M^{(2)}_a=\mathrm{mult}(m^{(2)}_a)$ be bounded multiplication operators
on $L^2(G^{m(R,a)},\pi^{\otimes m})$.
Assume that each magnetic multiplier is a fixed-range local sum on the same cell index set $\mathcal I_a(R)$ and stencils $S(\alpha)$
as in Lemma~\ref{lem:local-geom-count}:
\[
  m^{(1)}_{a,L}(u)\ =\ \sum_{\alpha\in\mathcal I_a(R)} a^4\,\mathcal V^{(1)}_{\alpha,a,L}\!\big(u|_{S(\alpha)}\big),
  \qquad
  m^{(2)}_{a}(u)\ =\ \sum_{\alpha\in\mathcal I_a(R)} a^4\,\mathcal V^{(2)}_{\alpha,a}\!\big(u|_{S(\alpha)}\big).
\]
and assume a first-order per-cell consistency bound:
there is $C_{\mathcal V}(R)<\infty$ such that for all $\alpha,a,L$,
\[
  \big\|\mathcal V^{(1)}_{\alpha,a,L}-\mathcal V^{(2)}_{\alpha,a}\big\|_\infty\ \le\ C_{\mathcal V}(R)\,a.
\]
Then the mismatch admits the local expansion
\[
  m^{(1)}_{a,L}(u)-m^{(2)}_a(u)\ =\ \sum_{\alpha\in\mathcal I_a(R)} a^5\,\delta_{\alpha,a,L}\!\big(u|_{S(\alpha)}\big),
\]
with $\|\delta_{\alpha,a,L}\|_\infty\le C_{\mathcal V}(R)$, and with the same cardinality and overlap bounds as Lemma~\ref{lem:local-geom-count}.

\smallskip
\noindent In particular, if the comparison model is chosen so that the magnetic term is matched on the cylinder core
(e.g.\ by using the same Wilson plaquette potential on $R$), then $m^{(1)}_{a,L}\equiv m^{(2)}_a$ and the magnetic mismatch vanishes identically.
\end{lemma}

\begin{proof}
Define $\delta_{\alpha,a,L}(v):=\big(\mathcal V^{(1)}_{\alpha,a,L}(v)-\mathcal V^{(2)}_{\alpha,a}(v)\big)/a$.
Then $\|\delta_{\alpha,a,L}\|_\infty\le C_{\mathcal V}(R)$ by assumption, and
\[
  m^{(1)}_{a,L}(u)-m^{(2)}_a(u)
  = \sum_{\alpha\in\mathcal I_a(R)} a^4\Big(\mathcal V^{(1)}_{\alpha,a,L}-\mathcal V^{(2)}_{\alpha,a}\Big)\!\big(u|_{S(\alpha)}\big)
  = \sum_{\alpha\in\mathcal I_a(R)} a^5\,\delta_{\alpha,a,L}\!\big(u|_{S(\alpha)}\big).
\]
The counting and overlap properties are Lemma~\ref{lem:local-geom-count}.
\end{proof}

\begin{remark}[Where the $a^5$ comes from (and what you must actually check)]\label{rem:local-expansion-a5}
The factor $a^5=a^4\cdot a$ is exactly ``cell volume'' times ``one extra power of $a$.''  The deterministic payload in
Lemma~\ref{lem:local-expansion-logr} and Lemma~\ref{lem:local-expansion-magnetic} is therefore the first-order per-cell consistency:
\[
  \big\|\mathcal L^{(1)}_{\alpha,a,L}-\mathcal L^{(2)}_{\alpha,a}\big\|_\infty
  +\max_{e\in S(\alpha)}\Big(
  \big\|\nabla_e(\mathcal L^{(1)}_{\alpha,a,L}-\mathcal L^{(2)}_{\alpha,a})\big\|_\infty
  +\big\|\Delta_e(\mathcal L^{(1)}_{\alpha,a,L}-\mathcal L^{(2)}_{\alpha,a})\big\|_\infty
  \Big)\ \lesssim\ a,
  \qquad
  \big\|\mathcal V^{(1)}_{\alpha,a,L}-\mathcal V^{(2)}_{\alpha,a}\big\|_\infty\ \lesssim\ a.
\]
Everything else (bounded overlap, $|\mathcal I_a(R)|=O(a^{-4})$, and uniform $C^2$ bounds for the rescaled remainders)
is fixed-range geometry on a fixed region plus smoothness on compact $G$.

\smallskip
\noindent\textbf{Bridge meaning.} To apply these lemmas to the \emph{actual} OS/GNS versus comparison bridge,
one must (i) write the two tree-gauged densities (and, if present, the two magnetic multipliers) on the common cylinder core as sums of
fixed-range cell terms, and (ii) prove the displayed first-order per-cell $O(a)$ consistency bounds for those cell terms uniformly in $L$.
\smallskip
\noindent\textbf{Tree-gauge note.} Here ``tree gauge'' is used only as a \emph{coordinate choice} on a fixed region: gauge-fixing on a spanning tree is implemented by vertex gauges and is a product of left/right translations, hence preserves product Haar on links (no Faddeev--Popov/Jacobian factor appears). Thus the local-expansion gate is \emph{not} about comparing an ``ungauged'' and a ``gauge-fixed'' density; it is about comparing the OS/GNS and comparison constructions after both are expressed on the \emph{same} tree-gauged chord coordinates.
\end{remark}
%
\begin{lemma}[Per-cell consistency from an $O(a^5)$ plaquette-potential mismatch (deterministic reduction)]\label{lem:plaquette-potential-mismatch-a5}
Work on a fixed region $R\Subset\mathbb R^4$ in tree-gauge chord coordinates $u\in G^{m(R,a)}$ with product Haar $\pi^{\otimes m}$.
Assume there is an index set of plaquettes $P_a(R)$ and for each $p\in P_a(R)$ a plaquette-holonomy word $U_p(u;b)\in G$ (with fixed exterior data $b$) depending only on a stencil $S(p)\subset\{1,\dots,m(R,a)\}$ with $|S(p)|\le 4$.

Suppose that two strictly positive $C^2$ densities $\rho_{a,L}$ and $\rho^{\rm cmp}_a$ on $G^{m(R,a)}$ admit plaquette-local representations on the cylinder core:
\[
  \rho_{a,L}(u)\ \propto\ \exp\!\Big(-\sum_{p\in P_a(R)} \Phi^{(1)}_{p,a,L}\!\big(U_p(u;b)\big)\Big),
  \qquad
  \rho^{\rm cmp}_{a}(u)\ \propto\ \exp\!\Big(-\sum_{p\in P_a(R)} \Phi^{(2)}_{p,a}\!\big(U_p(u;b)\big)\Big),
\]
with $\Phi^{(1)}_{p,a,L},\Phi^{(2)}_{p,a}\in C^2(G)$.
Define local cell terms on $G^{|S(p)|}$ by
\[
  \mathcal L^{(1)}_{p,a,L}(v)\ :=\ \frac{1}{a^4}\,\Phi^{(1)}_{p,a,L}\!\big(U_p(v;b)\big),
  \qquad
  \mathcal L^{(2)}_{p,a}(v)\ :=\ \frac{1}{a^4}\,\Phi^{(2)}_{p,a}\!\big(U_p(v;b)\big).
\]
Assume the plaquette-potential mismatch is $O(a^5)$ in $C^2$ uniformly in $(p,a,L)$:
there exists $C<\infty$ such that for all $p,a,L$,
\[
  \big\|\Phi^{(1)}_{p,a,L}-\Phi^{(2)}_{p,a}\big\|_\infty
  +\big\|\nabla\big(\Phi^{(1)}_{p,a,L}-\Phi^{(2)}_{p,a}\big)\big\|_\infty
  +\big\|\Delta\big(\Phi^{(1)}_{p,a,L}-\Phi^{(2)}_{p,a}\big)\big\|_\infty
  \ \le\ C\,a^5,
\]
where $\nabla,\Delta$ are the left-invariant gradient and Laplace--Beltrami operator on $G$.
Then the per-cell $O(a)$ consistency hypothesis of Lemma~\ref{lem:local-expansion-logr} holds for these cell terms: for each $p\in P_a(R)$ and each $e\in S(p)$,
\[
  \big\|\mathcal L^{(1)}_{p,a,L}-\mathcal L^{(2)}_{p,a}\big\|_\infty\ \le\ C\,a,
  \qquad
  \big\|\nabla_e\big(\mathcal L^{(1)}_{p,a,L}-\mathcal L^{(2)}_{p,a}\big)\big\|_\infty
  +\big\|\Delta_e\big(\mathcal L^{(1)}_{p,a,L}-\mathcal L^{(2)}_{p,a}\big)\big\|_\infty
  \ \le\ C\,a.
\]
\end{lemma}
...
By definition,
\[
  \mathcal L^{(1)}_{p,a,L}\!\big(u|_{S(p)}\big)-\mathcal L^{(2)}_{p,a}\!\big(u|_{S(p)}\big)
  =\frac{1}{a^4}\Big(\Phi^{(1)}_{p,a,L}-\Phi^{(2)}_{p,a}\Big)\!\big(U_p(u;b)\big),
\]
so taking $L^\infty$ norms gives $\|\mathcal L^{(1)}_{p,a,L}-\mathcal L^{(2)}_{p,a}\|_\infty\le a^{-4}\|\Phi^{(1)}_{p,a,L}-\Phi^{(2)}_{p,a}\|_\infty\le Ca$.

For derivatives in a stencil coordinate $e\in S(p)$, the map $u_e\mapsto U_p(u;b)$ is (for fixed other coordinates) a composition of
left/right translations, inversion, and (when the basepoint changes) conjugation, all of which are isometries for a bi-invariant metric on $G$ and hence commute with $\nabla$ and $\Delta$ and preserve the corresponding $L^\infty$ norms.
Therefore,
\[
  \big\|\nabla_e\big[(\Phi^{(1)}_{p,a,L}-\Phi^{(2)}_{p,a})\circ U_p\big]\big\|_\infty
  \le \big\|\nabla(\Phi^{(1)}_{p,a,L}-\Phi^{(2)}_{p,a})\big\|_\infty,
  \qquad
  \big\|\Delta_e\big[(\Phi^{(1)}_{p,a,L}-\Phi^{(2)}_{p,a})\circ U_p\big]\big\|_\infty
  \le \big\|\Delta(\Phi^{(1)}_{p,a,L}-\Phi^{(2)}_{p,a})\big\|_\infty.
\]
Dividing by $a^4$ and applying the assumed $Ca^5$ bounds yields the stated $Ca$ bounds for $\nabla_e$ and $\Delta_e$.
\end{proof}
%
\begin{lemma}[Derivative checklist from a local $a$--small log-density perturbation (deterministic reduction)]\label{lem:bridge-derivative-counting}
Work on a fixed bounded region $R\Subset\mathbb R^4$ in tree-gauge coordinates $u\in G^{m(R,a)}$ with product Haar $\pi^{\otimes m(R,a)}$.
Let $\mu_{a,L}$ and $\mu^{\rm cmp}_a$ be absolutely continuous with respect to $\pi^{\otimes m}$ with strictly positive $C^2$ densities $\rho_{a,L}$ and $\rho^{\rm cmp}_a$, and set
\[
  r_{a,L}(u)\ :=\ \frac{d\mu_{a,L}}{d\mu^{\rm cmp}_a}(u)\ =\ \frac{\rho_{a,L}(u)}{\rho^{\rm cmp}_a(u)}.
\]
Assume the following fixed-region locality structure for the log-density ratio:
there exist an index set $\mathcal I_a(R)$ and (for each $\alpha\in\mathcal I_a(R)$) a finite coordinate set $S(\alpha)\subset\{1,\dots,m(R,a)\}$ such that
\begin{equation}\label{eq:local-log-r-decomp}
  \log r_{a,L}(u)\ =\ \sum_{\alpha\in\mathcal I_a(R)} a^5\,\varepsilon_{\alpha,a,L}\!\big(u|_{S(\alpha)}\big),
\end{equation}
with the uniform bounded-degree and bounded-derivative conditions:
\begin{itemize}
  \item[(i)] \textbf{Cardinality.} $|\mathcal I_a(R)|\le C_I(R)\,a^{-4}$ for all sufficiently small $a$.
  \item[(ii)] \textbf{Bounded overlap.} Each coordinate $e\in\{1,\dots,m(R,a)\}$ belongs to at most $D_0(R)$ sets $S(\alpha)$.
  \item[(iii)] \textbf{Uniform $C^2$ bounds.} There is $C_\varepsilon(R)<\infty$ such that for all $\alpha,a,L$,
  \[
    \|\varepsilon_{\alpha,a,L}\|_\infty\ \le\ C_\varepsilon(R),\qquad
    \max_{e\in S(\alpha)}\big(\|\nabla_e\varepsilon_{\alpha,a,L}\|_\infty+\|\Delta_e\varepsilon_{\alpha,a,L}\|_\infty\big)\ \le\ C_\varepsilon(R),
  \]
  where $\nabla_e,\Delta_e$ are the $e$-th coordinate gradient and (nonnegative) Laplace--Beltrami operator on $G$.
\end{itemize}
Let $E_a=\kappa(a)\sum_{e=1}^{m(R,a)}\Delta_e$ with $\kappa(a)>0$ and assume $\sup_{a\in(0,a_0]}\kappa(a)\le \kappa_{\max}<\infty$.
Then for all sufficiently small $a$ (uniformly in $L$ along van Hove windows) one has
\[
  \|\log r_{a,L}\|_\infty\ \le\ C\,a,\qquad
  \|E_a\sqrt{r_{a,L}}\|_\infty\ \le\ C\,a,\qquad
  \sqrt{\kappa(a)}\Big(\sum_{e=1}^{m(R,a)}\|\nabla_e\sqrt{r_{a,L}}\|_\infty^2\Big)^{1/2}\ \le\ C\,a,
\]
for a constant $C=C(R)$ independent of $(a,L)$.
\end{lemma}
\noindent\emph{Remark (what remains).} Lemma~\ref{lem:bridge-derivative-counting} is purely deterministic: it upgrades a fixed-region local decomposition of $\log r_{a,L}$ with per-cell size $a^5$ and uniform $C^2$ bounds into the $L^\infty$ derivative bounds needed in Lemma~\ref{lem:bridge-B2-density-derivatives}. By Lemma~\ref{lem:local-expansion-logr} (and Lemma~\ref{lem:local-expansion-magnetic} for any nontrivial magnetic mismatch), it is enough to justify on the common tree-gauged cylinder core (uniformly in $L$ along van Hove windows) a fixed-range local representation of the two log-densities and a first-order per-cell $O(a)$ consistency bound for the corresponding local cell terms. This is the remaining RG-grade analytic content of the density-isometry route.
\begin{remark}[Possible route to \eqref{eq:local-log-r-decomp} via multi-scale cluster + chessboard (classical, but long)]
One classical way to justify the hypotheses of Lemma~\ref{lem:local-expansion-logr} (hence \eqref{eq:local-log-r-decomp}) is to combine:
\begin{itemize}
  \item \textbf{(i) Strong mixing / exponential clustering on fixed regions} for the tree-gauged chord Gibbs law (uniform in $L$ along van Hove windows), and
  \item \textbf{(ii) Reflection positivity/chessboard decoupling} to control block-to-block influences across reflected tilings,
\end{itemize}
and then run a (multi-scale) polymer/cluster expansion for \emph{logarithms of marginal densities} and/or \emph{logarithms of ratios of partition functions} on nested blocks. Under strong mixing, connected cluster weights decay exponentially in block diameter, and bounded interaction degree gives bounded overlap in the sense of (ii) in Lemma~\ref{lem:bridge-derivative-counting}. Smoothness on compact $G$ gives uniform $C^2$ bounds for fixed local factors.

In our setting, UCIS$_{\rm SW}$ provides a slab/interface contraction input (hence exponential clustering in the time direction after OS reconstruction), which is the kind of ingredient typically used to obtain the strong-mixing bounds needed to start such an expansion. We do \emph{not} claim this route is carried out here; we record it only as a plausible classical mechanism for proving the local-log-density decomposition input \eqref{eq:local-log-r-decomp}. For general background on cluster expansions and mixing conditions, see \cite{Brydges1986,Shlosman1986,Dobrushin1970}.
\end{remark}
\begin{proof}
Write $m:=m(R,a)$ and $r:=r_{a,L}$.

\emph{Step 1: Uniform $L^\infty$ control of $\log r$.}
From \eqref{eq:local-log-r-decomp} and (i),(iii),
\[
  \|\log r\|_\infty
  \ \le\ \sum_{\alpha\in\mathcal I_a(R)} a^5\,\|\varepsilon_{\alpha,a,L}\|_\infty
  \ \le\ |\mathcal I_a(R)|\,a^5\,C_\varepsilon(R)
  \ \le\ C_I(R)C_\varepsilon(R)\,a.
\]
In particular, $\|\sqrt r\|_\infty\le \exp(\tfrac12\|\log r\|_\infty)\le e^{C a/2}$ for $a\le a_0$.

\emph{Step 2: Pointwise derivative identities for $\sqrt r$.}
On each coordinate $e$,
\[
  \nabla_e\sqrt r\ =\ \frac12\sqrt r\,\nabla_e(\log r),\qquad
  \Delta_e\sqrt r\ =\ \sqrt r\Big(\frac12\Delta_e(\log r)\ +\ \frac14\|\nabla_e(\log r)\|^2\Big).
\]
Therefore
\begin{equation}\label{eq:Ezsqrt-count}
  E_a\sqrt r
  \ =\ \kappa(a)\sum_{e=1}^m\Delta_e\sqrt r
  \ =\ \sqrt r\Big(
    \frac{\kappa(a)}{2}\sum_{e=1}^m\Delta_e(\log r)
    +\frac{\kappa(a)}{4}\sum_{e=1}^m\|\nabla_e(\log r)\|^2
  \Big).
\end{equation}

\emph{Step 3: Bounds on $\nabla_e(\log r)$ and $\Delta_e(\log r)$ from locality.}
Fix a coordinate $e$. Differentiating \eqref{eq:local-log-r-decomp} in coordinate $e$, only terms with $e\in S(\alpha)$ contribute; by bounded overlap (ii) there are at most $D_0(R)$ such terms. Using (iii),
\[
  \|\nabla_e(\log r)\|_\infty
  \ \le\ \sum_{\alpha:\ e\in S(\alpha)} a^5\,\|\nabla_e\varepsilon_{\alpha,a,L}\|_\infty
  \ \le\ D_0(R)\,a^5\,C_\varepsilon(R),
\]
and similarly
\[
  \|\Delta_e(\log r)\|_\infty\ \le\ D_0(R)\,a^5\,C_\varepsilon(R).
\]

\emph{Step 4: Control of the $\sqrt{\kappa}\,\Xi$ term.}
Using $\nabla_e\sqrt r = \frac12\sqrt r\,\nabla_e(\log r)$ and the previous bound,
\[
  \|\nabla_e\sqrt r\|_\infty
  \ \le\ \frac12\,\|\sqrt r\|_\infty\,\|\nabla_e(\log r)\|_\infty
  \ \le\ \frac12\,e^{C a/2}\,D_0(R)\,C_\varepsilon(R)\,a^5.
\]
Summing over $e=1,\dots,m$ and taking a square root gives
\[
  \Big(\sum_{e=1}^m\|\nabla_e\sqrt r\|_\infty^2\Big)^{1/2}
  \ \le\ \frac12\,e^{C a/2}\,D_0(R)\,C_\varepsilon(R)\,a^5\,\sqrt m.
\]
On a fixed physical region one has $m(R,a)\le C_m(R)\,a^{-4}$ for small $a$, hence $\sqrt m\le \sqrt{C_m(R)}\,a^{-2}$ and therefore
\[
  \sqrt{\kappa(a)}\Big(\sum_{e=1}^m\|\nabla_e\sqrt r\|_\infty^2\Big)^{1/2}
  \ \le\ C'(R)\,\sqrt{\kappa(a)}\,a^3
  \ \le\ C(R)\,a,
\]
after enlarging constants and using $a\le a_0\le 1$ and $\kappa(a)\le \kappa_{\max}$.

\emph{Step 5: Control of $\|E_a\sqrt r\|_\infty$.}
From \eqref{eq:Ezsqrt-count}, the triangle inequality, and $\|\sqrt r\|_\infty\le e^{C a/2}$,
\[
  \|E_a\sqrt r\|_\infty
  \ \le\ e^{C a/2}\Big(
    \frac{\kappa(a)}{2}\sum_{e=1}^m \|\Delta_e(\log r)\|_\infty
    +\frac{\kappa(a)}{4}\sum_{e=1}^m \|\nabla_e(\log r)\|_\infty^2
  \Big).
\]
Using Step 3,
\[
  \sum_{e=1}^m \|\Delta_e(\log r)\|_\infty
  \ \le\ m\,D_0(R)\,C_\varepsilon(R)\,a^5
  \ \le\ C_m(R)\,a^{-4}\,D_0(R)\,C_\varepsilon(R)\,a^5
  \ =\ C_*\,a,
\]
and
\[
  \sum_{e=1}^m \|\nabla_e(\log r)\|_\infty^2
  \ \le\ m\,(D_0(R)C_\varepsilon(R)a^5)^2
  \ \le\ C_m(R)\,a^{-4}\,(D_0(R)C_\varepsilon(R))^2\,a^{10}
  \ =\ \widetilde C_*\,a^6.
\]
Thus $\|E_a\sqrt r\|_\infty \le e^{C a/2}(\frac{\kappa(a)}{2}C_* a + \frac{\kappa(a)}{4}\widetilde C_* a^6)\le C a$
for small $a$, uniformly in $L$. This completes the proof.
\end{proof}
%
\begin{lemma}[Magnetic mismatch bound at $O(a)$ from a local $a$--small remainder (deterministic reduction)]\label{lem:bridge-magnetic-mismatch-counting}
In the setting of Lemma~\ref{lem:bridge-derivative-counting}, suppose the magnetic terms in Lemma~\ref{lem:bridge-B2-density-derivatives} are multiplication operators
$M^{(1)}_{a,L}=\mathrm{mult}(m^{(1)}_{a,L})$ and $M^{(2)}_a=\mathrm{mult}(m^{(2)}_a)$ on $G^{m(R,a)}$,
and assume their difference admits the local expansion
\[
  m^{(1)}_{a,L}(u)-m^{(2)}_a(u)\ =\ \sum_{\alpha\in\mathcal I_a(R)} a^5\,\delta_{\alpha,a,L}\!\big(u|_{S(\alpha)}\big),
\]
with the same index set $\mathcal I_a(R)$ and overlap sets $S(\alpha)$ as in \eqref{eq:local-log-r-decomp}, and with a uniform bound
$\|\delta_{\alpha,a,L}\|_\infty\le C_\delta(R)$ for all $\alpha,a,L$.
Then uniformly in $L$ along the van Hove window,
\[
  \big\|\sqrt{r_{a,L}}\,(M^{(1)}_{a,L}-M^{(2)}_a)\big\|_\infty\ \le\ C\,a
\]
for a constant $C=C(R)$ independent of $(a,L)$.
\end{lemma}
\begin{proof}
By the triangle inequality, the cardinality bound $|\mathcal I_a(R)|\le C_I(R)a^{-4}$, and $\|\delta_{\alpha,a,L}\|_\infty\le C_\delta(R)$,
\[
  \|m^{(1)}_{a,L}-m^{(2)}_a\|_\infty
  \ \le\ \sum_{\alpha\in\mathcal I_a(R)} a^5\,\|\delta_{\alpha,a,L}\|_\infty
  \ \le\ C_I(R)a^{-4}\,a^5\,C_\delta(R)
  \ =\ (C_I(R)C_\delta(R))\,a.
\]
Also $\|\sqrt{r_{a,L}}\|_\infty\le e^{\frac12\|\log r_{a,L}\|_\infty}\le e^{C a/2}$ by Lemma~\ref{lem:bridge-derivative-counting}, so
\[
  \big\|\sqrt{r_{a,L}}\,(M^{(1)}_{a,L}-M^{(2)}_a)\big\|_\infty
  \ \le\ \|\sqrt{r_{a,L}}\|_\infty\,\|m^{(1)}_{a,L}-m^{(2)}_a\|_\infty
  \ \le\ e^{C a/2}\,(C_I(R)C_\delta(R))\,a
  \ \le\ C'\,a
\]
for small $a$, after absorbing $e^{C a/2}$ into the constant for $a\le a_0$.
\end{proof}
%
%
\begin{lemma}[Deterministic geometry reduction for the embedding-match gate (B1)]\label{lem:bridge-B1-geometry}
Work in the bridge setting of Lemma~\ref{lem:bridge-U2b-target} on a fixed bounded region $R$, and let $\mathsf D_R\subset\mathcal H_{a,L}$ be a dense time-zero local core supported in $R$.
Assume that after identifying the time-zero local generator algebra by tree gauge and the unitary $V_{a,L}$, the two embeddings
into the continuum space $\mathcal H$ admit \emph{deterministic} representatives on $\mathsf D_R$ of the following type:
\begin{itemize}
  \item[(G1)] \textbf{(Unweighted core embedding error).} There exists $C_{\rm geom}<\infty$ such that for all $\psi\in \mathsf D_R$,
  \[
    \big\| (I_{a,L}-J_a^*V_{a,L})\psi\big\|\ \le\ C_{\rm geom}\,a\,\big\|(H_{a,L}+1)^{1/2}\psi\big\|.
  \]
  \item[(G2)] \textbf{(Graph-weighted core embedding error).} There exists (possibly larger) $C_{\rm geom}<\infty$ such that for all $\psi\in \mathsf D_R$,
  \[
    \big\|(H+1)^{1/2}(I_{a,L}-J_a^*V_{a,L})\psi\big\|\ \le\ C_{\rm geom}\,a\,\big\|(H_{a,L}+1)^{1/2}\psi\big\|.
  \]
  \item[(G1$^\ast$)--(G2$^\ast$)] \textbf{Adjoint versions.} The corresponding adjoint estimates hold on a dense core in $\mathcal H$:
  \[
    \big\|(I_{a,L}^*-V_{a,L}^*J_a)\xi\big\|\ \le\ C_{\rm geom}\,a\,\big\|(H+1)^{1/2}\xi\big\|,
    \qquad
    \big\|(H_{a,L}+1)^{1/2}(I_{a,L}^*-V_{a,L}^*J_a)\xi\big\|\ \le\ C_{\rm geom}\,a\,\big\|(H+1)^{1/2}\xi\big\|.
  \]
\end{itemize}
Then the embedding-match hypothesis (B1) of Lemma~\ref{lem:bridge-U2b-target} holds (with the same $C_{\rm geom}$).

\smallskip
\noindent\emph{Deterministic meaning.} In practice, (G1)--(G2) are verified by comparing two concrete reconstruction schemes for the \emph{same} time-zero local observable on $R$:
the directed polygonal/smoothing reconstruction implicit in $I_{a,L}$ versus the piecewise-constant cell-extension reconstruction implicit in $J_a^*$.
On a fixed region this is a finite-dimensional, deterministic approximation problem and reduces to standard interpolation/quadrature estimates on a mesh
and their stability under the local graph norms; see, e.g., \cite[Ch.~3--4]{Ciarlet2002} or \cite[Ch.~4]{BrennerScott2008}.
\end{lemma}
\begin{proof}
By hypothesis, (G1)--(G2) hold on a dense core $\mathsf D_R$.
Since $(H_{a,L}+1)^{-1/2}$ is a contraction on $\mathcal H_{a,L}$ and $(H+1)^{-1/2}$ is a contraction on $\mathcal H$,
the displayed core inequalities are exactly the boundedness statements needed to extend
$(I_{a,L}-J_a^*V_{a,L})(H_{a,L}+1)^{-1/2}$ and $(H+1)^{1/2}(I_{a,L}-J_a^*V_{a,L})(H_{a,L}+1)^{-1/2}$
to bounded operators with norms $\le C_{\rm geom}a$, and likewise for the adjoints.
This is precisely (B1).
\end{proof}
%
We record (B1)--(B2) in operator form since that is exactly what is used downstream; the displayed form-level criterion above is a standard way to express the sought graph-boundedness content behind (B2) and makes the remaining RG-grade input maximally explicit.
\begin{lemma}[Polar-decomposition template for constructing $V_{a,L}$ from a near-isometry]\label{lem:bridge-polar-unitary}
Let $\mathcal K_1,\mathcal K_2$ be Hilbert spaces and let $T:\mathcal K_1\to\mathcal K_2$ be bounded with
\[
  \|T^*T-\mathbf 1_{\mathcal K_1}\|\ \le\ \varepsilon\ <\ 1.
\]
Then $T^*T$ is invertible and the operator
\[
  U\ :=\ T\,(T^*T)^{-1/2}
\]
is an isometry $\mathcal K_1\to\mathcal K_2$ (unitary onto $\overline{\mathrm{Ran}(T)}$). Moreover,
\[
  \|U-T\|\ \le\ \|T\|\,\|(T^*T)^{-1/2}-\mathbf 1\|
  \ \le\ \sqrt{1+\varepsilon}\,\Big((1-\varepsilon)^{-1/2}-1\Big).
\]
\end{lemma}
\begin{proof}
Since $\|T^*T-\mathbf 1\|\le \varepsilon<1$, $\sigma(T^*T)\subset[1-\varepsilon,1+\varepsilon]$ and hence $T^*T$ is invertible with bounded functional calculus. Then $U^*U=(T^*T)^{-1/2}(T^*T)(T^*T)^{-1/2}=\mathbf 1$. The norm bound follows from $\|T\|^2=\|T^*T\|\le 1+\varepsilon$ and the spectral-theorem estimate $\|(T^*T)^{-1/2}-\mathbf 1\|\le \sup_{\lambda\in[1-\varepsilon,1+\varepsilon]}|\lambda^{-1/2}-1|=(1-\varepsilon)^{-1/2}-1$.
\end{proof}
\noindent\emph{Remark (construction template for the bridge unitary).} In practice one first defines, on the time-zero local cylinder core supported in $R$, a \emph{candidate identification map} $T_{a,L}:\mathcal H_{a,L}\to\mathcal H^{\rm cmp}_a$ by identifying the underlying local generator algebra (for instance after tree gauge) and interpreting the same cylinder observable in the comparison model. If $T_{a,L}$ is a near-isometry in the sense that $T_{a,L}^*T_{a,L}=\mathbf 1+O(a)$ (with the error controlled uniformly in $L$ and in the relevant graph norms), then Lemma~\ref{lem:bridge-polar-unitary} produces a canonical isometry/unitary $V_{a,L}$ (the polar factor) with $V_{a,L}-T_{a,L}=O(a)$ in operator norm. This deterministic step is useful for enforcing the \emph{isometry/unitarity} aspect of the bridge once a canonical candidate $T_{a,L}$ is available. The generator-match estimate (B2) is subtler: passing from $T_{a,L}$ to its polar factor introduces the correction $(T_{a,L}^*T_{a,L})^{-1/2}$, and controlling the resulting effect on $V_{a,L}H_{a,L}-H^{\rm cmp}_aV_{a,L}$ generally requires an additional commutator/regularity input on the local core (or else one constructs $V_{a,L}$ directly as a genuine unitary identification, e.g. via an explicit coordinate/density map). We therefore view Lemma~\ref{lem:bridge-polar-unitary} as a useful deterministic tool for the “canonical unitary” aspect of the bridge, but do not claim it alone implies (B2).
\noindent\emph{Remark (rate and constants).} The constant $C(z_0,\Lambda)$ depends on $z_0$ and the low-energy cutoff $\Lambda$, and is uniform in $(a,L)$ for each fixed $\Lambda$. Picking $\Lambda=\Lambda(a)$ with $\operatorname{dist}(z_0,[\Lambda(a),\infty))^{-1}\le a$ gives
\[
  \big\|(H-z_0)^{-1}-I_{a,L}(H_{a,L}-z_0)^{-1}I_{a,L}^*\big\|
  \ \le\ \big(C(z_0,\Lambda(a))+1\big)\,a.
\]
In particular, any choice of $\Lambda(a)\to\infty$ for which $C(z_0,\Lambda(a))$ remains bounded yields an $O(a)$ one-point resolvent rate.

\begin{lemma}[Semigroup match at lattice times from a one-tick transfer match]\label{lem:bridge-semigroup-from-one-tick}
Let $\mathcal K_1,\mathcal K_2$ be Hilbert spaces, let $H_1\ge 0$ and $H_2\ge 0$ be self-adjoint on $\mathcal K_1$ and $\mathcal K_2$, and let $V:\mathcal K_1\to\mathcal K_2$ be unitary.
Fix a step size $h>0$ and set $T_1:=e^{-hH_1}$ and $T_2:=e^{-hH_2}$.
Assume that for some $\delta\ge 0$,
\[
  \big\|\,(V T_1-T_2V)\,(H_1+1)^{-1/2}\,\big\|\ \le\ \delta.
\]
Then for every integer $n\ge 1$,
\[
  \big\|\,(V T_1^{n}-T_2^{n}V)\,(H_1+1)^{-1/2}\,\big\|\ \le\ n\,\delta.
\]
\end{lemma}
\begin{proof}
...
\[
  V T_1^{n}-T_2^{n}V
  \ =\ \sum_{j=0}^{n-1} T_2^{n-1-j}\,(V T_1-T_2V)\,T_1^{j}.
\]
Multiply on the right by $(H_1+1)^{-1/2}$ and take operator norms. Since $\|T_2^{n-1-j}\|\le 1$ and $T_1$ commutes with $H_1$, we have
\[
  \|T_1^{j}(H_1+1)^{-1/2}\|\ \le\ \|(H_1+1)^{-1/2}\|\ \le\ 1,
\]
so each summand is bounded by $\delta$ and the claimed bound follows by summing.
\end{proof}
\noindent\emph{Remark (bridge interpretation).} In the bridge setting one typically takes $h=a$ (one lattice tick) and aims to prove $\|(V_{a,L}e^{-aH_{a,L}}-e^{-aH^{\rm cmp}_a}V_{a,L})(H_{a,L}+1)^{-1/2}\|\lesssim a^2$ on the common time-zero local core; the lemma then yields the semigroup match $\lesssim a\,t$ for times $t=na$.

\begin{lemma}[Generator match from small-time semigroup match (bridge gate)]\label{lem:bridge-generator-from-semigroup}
Let $\mathcal K_1,\mathcal K_2$ be Hilbert spaces, let $H_1\ge 0$ and $H_2\ge 0$ be self-adjoint on $\mathcal K_1$ and $\mathcal K_2$, and let $V:\mathcal K_1\to\mathcal K_2$ be unitary with $V\,\mathrm{Dom}(H_1)\subset \mathrm{Dom}(H_2)$.
Assume there exist $t_0>0$ and a constant $C_{\rm sg}<\infty$ such that for all $t\in(0,t_0]$,
\[
  \big\|\,(V e^{-tH_1}-e^{-tH_2}V)\,(H_1+1)^{-1/2}\,\big\|\ \le\ C_{\rm sg}\,a\,t.
\]
Then the generator-match bound holds:
\[
  \big\|\,(VH_1-H_2V)\,(H_1+1)^{-1/2}\,\big\|\ \le\ C_{\rm sg}\,a.
\]
\end{lemma}
\begin{proof}
Fix $\psi\in\mathrm{Dom}(H_1)$ and write
\[
  \frac{1}{t}\,(V e^{-tH_1}-e^{-tH_2}V)\psi\ =\ V\frac{e^{-tH_1}-I}{t}\psi\ -\ \frac{e^{-tH_2}-I}{t}\,V\psi.
\]
As $t\downarrow 0$, the semigroup generator identity gives $\frac{e^{-tH_1}-I}{t}\psi\to -H_1\psi$ in $\mathcal K_1$ and $\frac{e^{-tH_2}-I}{t}V\psi\to -H_2V\psi$ in $\mathcal K_2$, hence the left-hand side converges to $-(VH_1-H_2V)\psi$.
By the assumed bound,
\[
  \left\|\frac{1}{t}(V e^{-tH_1}-e^{-tH_2}V)\psi\right\|
  \ \le\ C_{\rm sg}\,a\,\|(H_1+1)^{1/2}\psi\|
\]
uniformly for $t\in(0,t_0]$. Letting $t\downarrow 0$ yields the \emph{relative bound} on the operator core:
\begin{equation}\label{eq:bridge-generator-relative}
  \|(VH_1-H_2V)\psi\|\ \le\ C_{\rm sg}\,a\,\|(H_1+1)^{1/2}\psi\|,\qquad \psi\in\mathrm{Dom}(H_1).
\end{equation}
Define a linear map $T$ on the dense subspace
\[
  \mathcal R\ :=\ (H_1+1)^{1/2}\mathrm{Dom}(H_1)\ \subset\ \mathcal K_1
\]
by
\[
  T\big((H_1+1)^{1/2}\psi\big)\ :=\ (VH_1-H_2V)\psi,\qquad \psi\in\mathrm{Dom}(H_1).
\]
Since $H_1\ge 0$, $(H_1+1)^{1/2}$ is injective on $\mathrm{Dom}(H_1)$, so $T$ is well-defined.
The estimate \eqref{eq:bridge-generator-relative} implies $\|Tu\|\le C_{\rm sg}a\,\|u\|$ for all $u\in\mathcal R$, hence $T$ extends uniquely to a bounded operator on $\mathcal K_1$ with $\|T\|\le C_{\rm sg}a$.
On $\mathcal R$ we have $T=(VH_1-H_2V)(H_1+1)^{-1/2}$, so this extension is exactly the bounded operator denoted by $(VH_1-H_2V)(H_1+1)^{-1/2}$ in the statement. This proves the claimed operator-norm bound.
\end{proof}
\noindent\emph{Remark (interpretation).} The conclusion can be read in the equivalent “relative bound” form
\(\|(VH_1-H_2V)\psi\|\le C_{\rm sg}a\,\|(H_1+1)^{1/2}\psi\|\) for all $\psi\in\mathrm{Dom}(H_1)$.
In particular, the product notation $(VH_1-H_2V)(H_1+1)^{-1/2}$ denotes the unique bounded extension of this operator-core map with respect to the $(H_1+1)^{1/2}$ graph norm.
\begin{lemma}[Small-time semigroup match from a bounded generator defect (Duhamel)]\label{lem:bridge-semigroup-from-generator-defect}
Let $\mathcal K_1,\mathcal K_2$ be Hilbert spaces, let $H_1\ge 0$ and $H_2\ge 0$ be self-adjoint on $\mathcal K_1$ and $\mathcal K_2$, and let $V:\mathcal K_1\to\mathcal K_2$ be unitary.
Assume the (graph-weighted) generator defect is bounded:
\[
  \big\|\,(VH_1-H_2V)\,(H_1+1)^{-1/2}\,\big\|\ \le\ C_{\rm gd}\,a.
\]
Then for every $t\ge 0$,
\[
  \big\|\,(V e^{-tH_1}-e^{-tH_2}V)\,(H_1+1)^{-1/2}\,\big\|\ \le\ C_{\rm gd}\,a\,t.
\]
\end{lemma}
\begin{proof}
Use the Duhamel formula (valid on $\mathrm{Dom}(H_1)$ and extended by boundedness of the defect operator):
\[
  V e^{-tH_1}-e^{-tH_2}V\ =\ \int_0^t e^{-(t-s)H_2}\,(VH_1-H_2V)\,e^{-sH_1}\,ds.
\]
Right-multiply by $(H_1+1)^{-1/2}$ and take operator norms. Since $e^{-sH_1}$ commutes with $(H_1+1)^{-1/2}$ and both semigroups are contractions, each integrand has norm bounded by $C_{\rm gd}a$. Integrating over $s\in[0,t]$ yields the claim.
\end{proof}
\begin{lemma}[Cauchy criterion for embedded resolvents; uniqueness]\label{lem:cauchy-resolvent-unique}
Let $z\in\mathbb C\setminus\mathbb R$ be fixed. Suppose there exists a choice of cutoffs $\Lambda=\Lambda(a)\ge 1$ such that the bound in Theorem~\ref{thm:nrc-quant} yields a one-point rate $\le C(z)\,a$ for all sufficiently small $a$ (uniformly in $L$ along the van Hove window). Then for any two spacings $a,a'\in(0,a_0]$ and volumes large enough along the van Hove window,
\[
...
\end{lemma}
\begin{proof}
Fix $z_0$ and choose cutoffs $\Lambda(a)$, $\Lambda(a')$ so that the bound in Theorem~\ref{thm:nrc-quant} yields $\|I_{a,L}(H_{a,L}-z_0)^{-1}I_{a,L}^*-(H-z_0)^{-1}\|\le C(z_0)a$ and $\|I_{a',L'}(H_{a',L'}-z_0)^{-1}I_{a',L'}^*-(H-z_0)^{-1}\|\le C(z_0)a'$. Add and subtract $(H-z_0)^{-1}$ and apply the triangle inequality:
\[
\begin{aligned}
...
Let $\mathcal A^{\rm loc}$ be the polynomial *\,–\,algebra generated by smeared local gauge\,–\,invariant fields from Section~\ref{subsec:local-fields-tempered}. Assume that the AF--free uniqueness package applies on the relevant fixed regions (e.g.\ Theorem~\ref{thm:U2-nrc-unique}, with the one-point resolvent input supplied by Proposition~\ref{prop:one-point-resolvent} using Theorem~\ref{thm:U2b-global}). Then along any van Hove scaling sequence $(a_k,L_k)$ with $a_k\downarrow 0$, the $n$\,–\,point Schwinger functions on $\mathcal A^{\rm loc}$ converge uniquely (no subsequences) to the continuum limits determined by $H$ and OS0--OS5.
\end{corollary}

\begin{proof}
By Theorem~\ref{thm:U2-nrc-unique}, the embedded resolvents $I_{a,L}(H_{a,L}-z)^{-1}I_{a,L}^*$ converge in operator norm, uniformly on compact subsets of $\mathbb C\setminus\mathbb R$, hence are Cauchy. By OS/GNS, $n$\,–\,point functions are Laplace transforms of matrix elements of products of semigroups $e^{-tH_{a,L}}$ between time\,–\,zero local vectors, and the Laplace–resolvent representation expresses these matrix elements through $(H_{a,L}-z)^{-1}$ with $\Im z\ne 0$. Dominated convergence for the Laplace integral (using UEI and locality to justify Fubini/Tonelli) yields convergence and uniqueness of the Schwinger limits along van Hove sequences.
\end{proof}
...
\]
Split by $E_H([0,\Lambda])$ and $E_H((\Lambda,\infty))$. On the high-energy part, $\|R(z_0) E_H((\Lambda,\infty))\|=\operatorname{dist}(z_0,[\Lambda,\infty))^{-1}$. On the low-energy part, use the assumed projection control $\|(I-P_{a,L})E_H([0,\Lambda])\|\le C_\Lambda a$. For the defect term, use the assumed graph-defect bound $\|(H+1)^{-1/2}D_{a,L}(H_{a,L}+1)^{-1/2}\|\le C_{\rm gd} a$ together with the weighted resolvent bounds for $H$ and $H_{a,L}$: $\|(H-z_0)^{-1}(H+1)^{1/2}\|\le C_H(z_0)$ and $\|(H_{a,L}-z_0)^{-1}(H_{a,L}+1)^{1/2}\|\le C_{\rm lat}(z_0)$ (cf.\ (A4) below). Collecting terms yields the estimate with a constant $C(z_0,\Lambda)$.
\end{proof}
...
\section{\texorpdfstring{OS$\to$Wightman Reconstruction and Mass Gap in Minkowski Space}{OS-to-Wightman Reconstruction and Mass Gap in Minkowski Space}}

\subsection*{Abstract reversible discretization implies resolvent limit and O(a) defect}
...
\begin{theorem}[\texorpdfstring{OS\,$\to$\,Wightman export with mass gap}{OS-to-Wightman export with mass gap}]\label{thm:os-to-wightman}
...
\end{equation}
\end{theorem}
\noindent\emph{Remark (constant propagation).} Under OS reconstruction (Osterwalder--Schrader~\cite{Osterwalder1975}, Sec.~IV--V), Minkowski time translations are implemented by a unitary group on the same reconstructed Hilbert space whose generator is the same self\,--\,adjoint operator $H$ appearing in the Euclidean semigroup. Thus the mass-gap constant $\gamma_*$ propagates unchanged to the Minkowski Hamiltonian $H_{\rm Mink}$; no renormalization of the gap constant occurs in this step.
\begin{proof}
This is the standard OS reconstruction theorem: see Osterwalder--Schrader~\cite{Osterwalder1975}, Sec.~IV.1 (Theorem $E'\Rightarrow R'$) and Sec.~V. In particular, the reconstruction produces a Hilbert space $\mathcal H$, a cyclic vacuum vector $\Omega$, and a strongly continuous Euclidean time-translation semigroup $e^{-tH}$, $t\ge 0$, with $H\ge 0$, such that the Schwinger functions are vacuum expectations of time-ordered Euclidean fields.

Analytic continuation of the Euclidean time-translation semigroup yields a strongly continuous unitary group of Minkowski time translations on the \emph{same} Hilbert space, given by
\[
  U_{\rm Mink}(t)\ :=\ e^{itH},
\]
so the Minkowski Hamiltonian is $H_{\rm Mink}=H$. Hence $\sigma(H_{\rm Mink})=\sigma(H)$, and the assumed Euclidean spectral gap $\operatorname{spec}(H)\subset\{0\}\cup[\gamma_*,\infty)$ propagates unchanged to Minkowski space.
\end{proof}

\vspace{12pt}
\section{Main Theorem (Continuum YM with Mass Gap; AF--free NRC with fixed-region inputs)}\label{sec:main-theorem-unconditional}

\begin{mdframed}[linewidth=0.5pt, linecolor=purple!40, backgroundcolor=purple!3, roundcorner=2pt, innertopmargin=8pt, innerbottommargin=8pt, skipabove=10pt, skipbelow=10pt]
\noindent\textbf{Result Map (Labels; AF--free NRC Main Path).}
\begin{itemize}
...
\section{Main Result: AF--free Continuum Construction}
\label{sec:main-theorem-unconditional}

We now assemble the main continuum result. The construction is ``AF--free'' in that it relies on fixed--region inputs and avoids global inductive limit assumptions.

\begin{theorem}[Main Theorem: AF--free Continuum YM with Mass Gap]\label{thm:main-af-free}
Fix a compact simple gauge group $G$.
\begin{itemize}
\item[(1)] \textbf{Lattice inputs (Unconditional).} For infinite--volume lattice Yang--Mills, the transfer operator $T$ is positive self--adjoint (Thm.~\ref{thm:os}). For small $\beta$, there is a mass gap $\gamma_0 > 0$ uniform in volume (Thm.~\ref{thm:gap}).

\item[(2)] \textbf{Scaling inputs (UCIS$_{\rm SW}$).} Scaling--window interface smoothing (Thm.~\ref{thm:ucis-sw}) provides a uniform fixed--physical--time contraction $q_{\rm phys} < 1$ on the odd subspace (Thm.~\ref{thm:ucis-sw-odd-subspace}).

\item[(3)] \textbf{Continuum existence and Gap (Unconditional).} Along any van Hove scaling sequence $(a_k, L_k)$ with $a_k \downarrow 0$:
\begin{enumerate}
\item \textbf{Convergence:} Local Schwinger functions converge to tempered distributions satisfying OS0--OS5 (Thm.~\ref{thm:uei-fixed-region}, Prop.~\ref{prop:os0os2-closure}).
\item \textbf{NRC:} $R_{a,L}(z)$ converges in operator norm to $(H-z)^{-1}$ on fixed regions (Thm.~\ref{thm:nrc-operator-norm}).
\item \textbf{Mass Gap:} The physical mass gap persists: $\operatorname{spec}(H) \subset \{0\} \cup [\gamma_*, \infty)$ with $\gamma_* = 8(-\log q_{\rm phys}) > 0$.
\item \textbf{Wightman Fields:} The theory analytically continues to a Poincar\'e--covariant Wightman field theory with the same positive mass gap (Thm.~\ref{thm:os-to-wightman}).
\end{enumerate}
\end{itemize}
\end{theorem}

\begin{proof}
\textbf{Part (1)} is established by lattice OS reflection positivity (Sec.~\ref{sec:lattice-setup}) and the cluster expansion gap (Sec.~\ref{sec:strong-coupling}).
\textbf{Part (2)} is the UCIS$_{\rm SW}$ package derived in this section.
\textbf{Part (3):}
\textit{Existence/Uniqueness:} On any fixed region $R$, U1 ensures precompactness. U2b (verified by Theorem~\ref{thm:verify-u2b}) provides the quantitative defect bounds for Theorem~\ref{thm:nrc-operator-norm}. By the AF--free uniqueness theorem (Thm.~\ref{thm:U2-nrc-unique}), local limits are unique.
\textit{Gap Persistence:} Theorem~\ref{thm:ucis-sw-odd-subspace} gives odd--sector contraction. The Eight--Tick Theorem (Thm.~\ref{thm:eight-tick-uniform} logic applied to physical steps) lifts this to the full gap $\gamma_*$ via the Two-Layer Deficit (Sec.~\ref{sec:two-layer-deficit}). By Theorem~\ref{thm:gap-persist-cont}, this gap survives the norm--resolvent limit.
\end{proof}
...
Assume the hypotheses ensuring existence and uniqueness of the fixed-region continuum limits (in particular Theorem~\ref{thm:U2b-global}). Then there exist compactly supported smooth test functions $f_1,\ldots,f_4\in C_c^\infty(\mathbb R^4,\wedge^2\mathbb R^4)$ such that the truncated 4-point function of the clover field is nonzero in the continuum limit:
\[
  \langle \Xi(f_1)\,\Xi(f_2)\,\Xi(f_3)\,\Xi(f_4)\rangle_c\ \neq\ 0.
\]
...
\end{corollary}
\begin{proof}
Fix a bounded region $R$ and $f\in C_c^\infty(R)$ chosen as in Proposition~\ref{prop:nonzero-cumulant4} so that, for all sufficiently small $a$ and large $L$, one has $\langle \Xi_a(f)^4\rangle_c\ge c_0>0$ uniformly in $(a,L)$. By Lemma~\ref{lem:local-fields-tempered} and Corollary~\ref{cor:unique-schwinger-local}, $\Xi_a(f)\to \Xi(f)$ in $L^2$ on fixed regions and the required moments converge along van Hove sequences (using Theorem~\ref{thm:U2b-global}). Since truncated cumulants are polynomial combinations of moments, they are continuous under convergence of moments of the required orders. Therefore
\[
...
Assume the hypotheses of Theorem~\ref{thm:main-af-free} (in particular Theorem~\ref{thm:U2b-global}). Then there exist global Schwinger functions $\{S_n\}_{n\ge1}$ on the cylinder $\sigma$--algebra of gauge--invariant observables on $\mathbb R^4$ such that OS0--OS5 hold globally, with explicit constants and no dependence on the choice of van Hove exhaustion, embedding scheme, or boundary conditions. More precisely:
\begin{itemize}
  ...
  \item[(v)] \textbf{OS3 (clustering) and (vi) OS5 (unique vacuum).} Along schedules satisfying the scaling window \eqref{eq:ucis-sw-window}, UCIS$_{\rm SW}$ supplies constants $(\theta_*,t_0)$ on fixed slabs (Thm.~\ref{thm:ucis-sw}), uniform in $L$ and boundary data. Let $\lambda_1(G)>0$ be the first Laplace–Beltrami eigenvalue on $G$ and set
  \[
    q_{\rm phys}\ :=\ 1-\theta_*\big(1-e^{-\lambda_1(G) t_0}\big)\in(0,1),\qquad
    c_{\mathrm{cut,phys}}\ :=\ -\log(q_{\rm phys})>0,\qquad
    \gamma_*\ :=\ 8\,c_{\mathrm{cut,phys}}.
  \]
  Then the global Euclidean generator $H\ge0$ obeys $\operatorname{spec}(H)=\{0\}\cup[\gamma_*,\infty)$ (Thm.~\ref{thm:global-gap-uncond}), yielding exponential clustering at rate $\gamma_*$ and a unique vacuum (OS5).
\end{itemize}
All constants are independent of the van Hove exhaustion, boundary conditions, and embedding scheme; their dependence on the group enters through $\lambda_1(G)$ and through the UCIS$_{\rm SW}$ constants $(\theta_*,t_0)$ along \eqref{eq:ucis-sw-window}.
\end{theorem}

\begin{theorem}[Uniform global NRC with explicit constants; spectral projectors]\label{thm:global-nrc-clay}
Assume Theorem~\ref{thm:U2b-global} so that Theorem~\ref{thm:nrc-quant} applies.
...
\]
where $\varepsilon(a)=O(a)$ under Theorem~\ref{thm:nrc-quant} (using Theorem~\ref{thm:U2b-global}), and under the fixed–core normalization of \S\ref{thm:quant-calibrated-af-free-nrc} one has $\varepsilon(a)\le a^2$ (Cor.~\ref{cor:NRC-explicit}). In particular, on any compact $K\subset\mathbb C\setminus\mathbb R$ the convergence is uniform with the same $K(z)$.
...
\]
with $C_{\mathrm{NRC}}\le 2 C_{\rm form}+4 C_{\rm iso}^2$ as in Thm.~\ref{thm:quant-calibrated-af-free-nrc}(F,G), independent of slab/volume/exhaustion.
\end{theorem}

\begin{theorem}[Gap persistence, OS$\to$Wightman, microcausality and nontriviality]\label{thm:gap-wightman-clay}
Assume the hypotheses of Theorem~\ref{thm:main-af-free} (in particular Theorem~\ref{thm:U2b-global}). For the resulting global continuum theory:
\begin{itemize}
...
Let $G$ be any compact simple Lie group. Assume the hypotheses of Theorem~\ref{thm:main-af-free} (in particular Theorem~\ref{thm:U2b-global}). Then:
\begin{itemize}
...
\noindent\textbf{Clay-Style Constants Checklist (for Theorem~\ref{thm:main-af-free}).}
\noindent\emph{Interface smoothing constants (active route).} Along schedules satisfying the scaling window \eqref{eq:ucis-sw-window}, UCIS$_{\rm SW}$ (Theorem~\ref{thm:ucis-sw}) supplies constants $(\theta_*,t_0)$ on fixed slabs with dependence only on $(G,\varepsilon,T_{\rm phys},C_{\rm SW})$, uniform in $L$ and boundary data (within the window). With $\lambda_1(G)>0$ the first nonzero Laplace--Beltrami eigenvalue on $G$, define the corresponding fixed-physical-time contraction factor
\[
  q_{\rm phys}\ :=\ 1-\theta_*\big(1-e^{-\lambda_1(G) t_0}\big)\ \in(0,1),
  \qquad c_{\rm cut,phys}\ :=\ -\log(q_{\rm phys})\ >\ 0,
  \qquad \gamma_*\ :=\ 8\,c_{\rm cut,phys}.
\]
The resulting gap floor is transported to the continuum generator by Theorem~\ref{thm:global-gap-uncond} (under the remaining fixed-region inputs, in particular Theorem~\ref{thm:U2b-global}). A schedule-free, $\beta$--uniform one-step Doeblin minorization is \emph{not} used in the main chain.
\end{mdframed}
...
\begin{itemize}
  ...
  \]
  with $\varepsilon(a)=O(a)$ under Theorem~\ref{thm:nrc-quant} (using Theorem~\ref{thm:U2b-global}) and $\varepsilon(a)\le a^2$ under the fixed-core normalization (Cor.~\ref{cor:NRC-explicit}).
  \item Spectral projectors (Davis–Kahan). For $E\in(0,\gamma_*/2]$,
  \[
    \big\|\,\mathbf 1_{(-\infty,E]}(H) - I_{a,L}\,\mathbf 1_{(-\infty,E]}(H_{a,L})\,I_{a,L}^*\big\|\ \le\ \frac{2\,C_{\mathrm{NRC}}}{\gamma_* - E}\,\varepsilon(a),\quad C_{\mathrm{NRC}}\le 2 C_{\rm form}+4 C_{\rm iso}^2.
  \]
\end{itemize}
...
\begin{corollary}[Global volume-uniform mass-gap bound along the scaling window (conditional)]\label{cor:global-uniform-gap}
Assume the hypotheses of Theorem~\ref{thm:main-af-free} (in particular UCIS$_{\rm SW}$ under \eqref{eq:ucis-sw-window} and Theorem~\ref{thm:U2b-global}). Then the global continuum generator $H$ obtained by NRC and OS reconstruction satisfies
\[
  \operatorname{spec}(H)\subset\{0\}\cup[\gamma_*,\infty)\,,
\]
for some $\gamma_*>0$ independent of the van Hove exhaustion, boundary conditions, and the volume. One may take $\gamma_*$ as in Theorem~\ref{thm:global-gap-uncond}.
\begin{proof}
Immediate from Theorem~\ref{thm:global-gap-uncond}.
\end{proof}
\end{corollary}

\begin{theorem}[Global Minkowski mass gap]\label{thm:global-minkowski-gap}
Assume the hypotheses of Theorem~\ref{thm:main-af-free} (in particular UCIS$_{\rm SW}$ under \eqref{eq:ucis-sw-window} and Theorem~\ref{thm:U2b-global}). For the global continuum OS measure constructed in Section~\ref{sec:global-R4}, let $H\ge 0$ be the Euclidean generator and $H_{\mathrm{Mink}}$ the Minkowski Hamiltonian obtained by OS\,$\to$\,Wightman. Then there exists $\gamma_*>0$ such that
\[
  \operatorname{spec}(H)\ \subset\ \{0\}\cup[\gamma_*,\infty)\;,\qquad
  \operatorname{spec}(H_{\mathrm{Mink}})\ \subset\ \{0\}\cup[\gamma_*,\infty)\;.
\]
One may take $\gamma_*$ as in Theorem~\ref{thm:global-gap-uncond}; it is independent of the exhaustion/van Hove sequence and boundary conditions (within the admissible scaling window).
\end{theorem}
\begin{proof}
The Euclidean gap statement is Theorem~\ref{thm:global-gap-uncond}. The OS\,$\to$\,Wightman reconstruction transports the spectral gap to Minkowski (Corollary~\ref{cor:minkowski-massgap}).
\end{proof}
...
\end{proposition}
\begin{proof}
For each lattice spacing $a$ and volume $L$, the smeared observables $\Phi_a(f)$ and $\Xi_a(\varphi)$ are bounded functions of finitely many plaquettes/clovers in a fixed region. Since $0\le 1-\tfrac1N\Re\Tr(U_p)\le 2$ and $0\le 1-\tfrac1N\Re\Tr(U^{\mathrm{clov}}_{\mu\nu}(x))\le 2$, the Riemann-sum definitions yield uniform $L^\infty$ bounds
\[
  \|\Phi_a(f)\|_{L^\infty}\ \le\ 2\!\!\sum_{p} a^4\,|f(x_p)|,\qquad
  \|\Xi_a(\varphi)\|_{L^\infty}\ \le\ 2\!\!\sum_{x,\mu<\nu} a^4\,|\varphi_{\mu\nu}(x)|,
\]
and hence bounds depending only on the test functions (and their supports), uniformly in $(a,L)$. Passing to the $L^2$ limits defining $\Phi(f)$ and $\Xi(\varphi)$ (Lemma~\ref{lem:local-fields-tempered}), these observables remain essentially bounded, hence define bounded operators on the OS/GNS Hilbert space by multiplication on the time-zero algebra. Therefore, for $\psi\in\mathcal D_{\rm loc}$,
\[
  \|\Phi(f)\psi\|\ \le\ \|\Phi(f)\|_{L^\infty}\,\|\psi\|\ \le\ \|\Phi(f)\|_{L^\infty}\,\|(H+1)^{1/2}\psi\|,
\]
and similarly for $\Xi(\varphi)$ (since $\|\psi\|\le \|(H+1)^{1/2}\psi\|$). This yields the claimed graph bounds with $C_\Phi(f):=\|\Phi(f)\|_{L^\infty}$ and $C_\Xi(\varphi):=\|\Xi(\varphi)\|_{L^\infty}$. Bounded operators are closable and $\mathcal D_{\rm loc}$ is a core.
\end{proof}
...
\begin{lemma}[Nontriviality: positive variance witness]\label{lem:nontrivial-variance}
Assume the hypotheses ensuring existence/uniqueness of the fixed-region continuum local fields and non-Gaussianity (e.g.\ Corollary~\ref{cor:nonGaussian-main}). Then for the corresponding test function $f$ one has
\[
  \operatorname{Var}_\mu\big(\Xi(f)\big)\ >\ 0.
\]
\end{lemma}

\begin{proof}
Let $f$ be as in Corollary~\ref{cor:nonGaussian-main}, so that $\langle \Xi(f)^4\rangle_c\neq 0$. If $\operatorname{Var}_\mu(\Xi(f))=0$, then $\Xi(f)$ is almost surely constant and all truncated cumulants of order $\ge 2$ vanish, contradicting $\langle \Xi(f)^4\rangle_c\neq 0$. Hence $\operatorname{Var}_\mu(\Xi(f))>0$.
\end{proof}

\section*{Appendix: Constants and References Index}
\begin{itemize}
  \item \textbf{Constants.} $\lambda_1(G)$: first nonzero Laplace--Beltrami eigenvalue on the compact simple group $G$; $t_0>0$, $\theta_*>0$: scaling-window interface smoothing constants on fixed slabs supplied by UCIS$_{\rm SW}$ (uniform in volume/boundary data on fixed slabs within \eqref{eq:ucis-sw-window}); $\kappa_0(\beta)$ denotes the $\beta$-explicit one-step minorization constant in the legacy Doeblin route when referenced. We set $c_{\mathrm{cut}}(a):=-(1/a)\log(1-\theta_*(1-e^{-\lambda_1(G) t_0}))$, $c_{\mathrm{cut,phys}}:= -\log(1-\theta_*(1-e^{-\lambda_1(G) t_0}))$, $\gamma_{\mathrm{cut}}:=8\,c_{\mathrm{cut}}(a)$, $\gamma_*:=8\,c_{\mathrm{cut,phys}}$.
...
\begin{itemize}
  ...
  \item \textbf{Interface smoothing constants (active route).} Along weak--coupling scaling schedules satisfying the scaling window \eqref{eq:ucis-sw-window}, UCIS$_{\rm SW}$ (Theorem~\ref{thm:ucis-sw}) supplies a fixed--physical--time heat--kernel minorization/contraction on the coarse interface with constants depending only on $(R_*,a_0,G)$ and the chosen physical time window (and \emph{not} on $(\beta,L)$ within the window). A fully $\beta$--uniform, schedule--free one--step Doeblin minorization remains a separate legacy statement; see the discussion around Theorem~\ref{thm:ucis} / Lemma~\ref{lem:ucis-C-cell-ball}.
  ...
  \item \textbf{NRC/embedding constants.} From Theorems~\ref{thm:nrc-embeddings}, \ref{thm:nrc-quant} together with Theorem~\ref{thm:U2b-global}: graph-defect bound $C_{\rm gd}$, low-energy projection control $C_\Lambda$, and resolvent rate $C(z_0,\Lambda)$.
\end{itemize}

\paragraph{OS0/OS2 under limits (closure).}
Reflection positivity (OS2) is closed under weak limits on cylinders (Lemma~\ref{lem:os2-limit}). Temperedness (OS0) is passed to limits once the required uniform OS0 polynomial bounds are available along the sequence (cf. Lemma~\ref{lem:os0-explicit-constants}). We record these limit-passages in Proposition~\ref{prop:os0os2-closure}.
...
\begin{itemize}
  ...
  \item \textbf{Mass gap (continuum)}: Scaling-window interface closure: Thm.~\ref{thm:ucis-sw} (under \eqref{eq:ucis-sw-window}), Cor.~\ref{cor:ucis-sw-L2-contraction}, Thm.~\ref{thm:ucis-sw-odd-subspace}, and bookkeeping via Prop.~\ref{prop:os35-limit}; persistence to the limit: Thm.~\ref{thm:gap-persist-cont} together with AF--free NRC (using Theorem~\ref{thm:U2b-global}): Thm.~\ref{thm:nrc-operator-norm}, Thm.~\ref{thm:nrc-embeddings}.
  \item \textbf{AF/Mosco framework}: Assumption~\ref{assump:AF-Mosco}; Semigroup\,$\Rightarrow$\,resolvent: Thm.~\ref{thm:NRC-allz}; quantitative NRC: Thm.~\ref{thm:nrc-quant} (using Theorem~\ref{thm:U2b-global}); embeddings/core: Thm.~\ref{thm:strong-semigroup-core}, Prop.~\ref{prop:collective-compactness}; defects/projections: Theorem~\ref{thm:U2b-global} (OS/GNS embedding-side inputs), and for the fixed-slab $J_a$ discretization analogues see Lem.~\ref{lem:graph-defect-Oa}, Lem.~\ref{lem:low-energy-proj}.
...
\begin{itemize}
  ...
  \item \textbf{Unique projective limit.} Tightness (UEI) and equicontinuity yield subsequential limits on fixed regions; uniqueness (no subsequences) requires an AF--free NRC/uniqueness input, e.g.\ Proposition~\ref{prop:af-free-uniqueness} (which assumes an operator-norm resolvent Cauchy input) or Theorem~\ref{thm:U2-nrc-unique} using Theorem~\ref{thm:U2b-global}.
...
\end{itemize}
\section{Appendix: an elementary 2x2 PSD eigenvalue bound}
...
There exist constants $C_{\rm loc},\,\mu_{\rm loc}>0$ depending only on $(R_*,a_0,N)$ and $\beta$ such that for any time--zero, gauge--invariant local observables $O_1,O_2$ supported in $B_{R_*}$ and all $a\in(0,a_0]$,
\[
...
Uniformity in $L$ follows from the fixed-slab nature of the interface contraction; the dependence on $\beta$ enters only through $\theta_*=\theta_*(\beta)$ in $c_{\rm cut}=-(1/a)\log(1-\theta_*(1-e^{-\lambda_1(G)t_0}))$.
\end{proof}
...
\[
  \alpha(\beta)\ \le\ 2\,\beta\,J_{\perp}\,.
\]
Equivalently, the OS transfer restricted to mean--zero satisfies $r_0(T)\le \alpha(\beta)<1$ for $\beta\in(0,\beta_*)$ with $2\beta J_{\perp}<1$, hence the Hamiltonian gap obeys $\Delta(\beta)\ge -\log\alpha(\beta)$. Separately, the interface/odd-cone mechanism gives a cut-contraction bound with a $\beta$-explicit weight (via Proposition~\ref{prop:doeblin-full} and Theorem~\ref{thm:eight-tick-uniform}); we write this contribution as $\gamma_{\mathrm{cut}}(\beta):=8\,c_{\rm cut}(\beta)$.
...
\[
  ...
  \gamma_{\mathrm{cut}}(\beta):=8\,c_{\mathrm{cut}}(\beta),\qquad
  \gamma_0(\beta):=\max\{\gamma_{\alpha}(\beta),\,\gamma_{\mathrm{cut}}(\beta)\}.
\]
Here $c_{\mathrm{cut}}(\beta) := -(1/a)\log(1-\theta_*(\beta)\,(1-e^{-\lambda_1(G) t_0}))$ with $\theta_*(\beta)=\kappa_0(\beta;S,t_0)$ as in Proposition~\ref{prop:doeblin-full}; the constants are uniform in the volume on fixed slabs and $\beta$-explicit (group dependence only via $\lambda_1(G)$).

Then the OS transfer operator on the mean--zero sector has a Perron--Frobenius gap $\ge \gamma_0(\beta)$, uniformly in the volume and in $N\ge 2$. For very small $\beta$, $\gamma_{\alpha}(\beta)$ dominates; the cut term $\gamma_{\mathrm{cut}}(\beta)$ is an additional (generally very small) explicit bound.
\end{corollary}
...
\[
  ...
  \ =\ \frac{C_g(R_*)\,B}{e^{\nu'-\nu}-1}\,.
\]
Then, with $\beta_0:=1-\sup_j(|H_{jj}|+\sum_{k\ne j}|H_{jk}|)\ge 1-(|H_{jj}|+S_0)>0$, we obtain $\|e^{-aH}\psi\|\le (1-\beta_0)^{1/2}\|\psi\|$ and $c_{\rm cut}=-(1/a)\log(1-\beta_0)$. We will also use the $\beta$-explicit interface bound coming from the one-step convex split (Proposition~\ref{prop:doeblin-full}):
\[
  c_{\rm cut}(\beta)\ :=\ -\frac{1}{a}\,\log\Big(1-\kappa_0(\beta;S,t_0)\,\big(1-e^{-\lambda_1(G) t_0}\big)\Big)\,.
\]
Composing across eight ticks yields $\gamma_{\rm cut}(\beta):=8\,c_{\rm cut}(\beta)$. All constants are uniform in the volume $L$ on fixed slabs; the cut term depends on $\beta$ through the explicit one-step Doeblin weight $\kappa_0(\beta;S,t_0)$.
...
\paragraph{Reduction to heat--kernel domination (parameter tracking).} \emph{Remark (overview; non-essential).} A small-ball refresh creates local randomness; convolution on a compact simple $G$ smooths this into a positive, group--wide density dominated below by a heat kernel, yielding a Doeblin split. At the one-step level, the Doeblin weight is $\beta$-explicit (Proposition~\ref{prop:doeblin-full}) and uniform in the volume on fixed slabs. In the \emph{main chain}, the fixed-physical-time contraction used for OS3/OS5 is supplied instead by UCIS$_{\rm SW}$ along schedules satisfying \eqref{eq:ucis-sw-window}.
Let $K_{\rm int}^{(a)}$ be the one-step cross-cut integral kernel induced on interface link variables by $e^{-aH}$ on the $P$-odd cone, normalized as a Markov kernel on $\mathrm{SU}(N)^m$ (finite $m$ depending on $R_*$). Suppose there exists a time $t_0=t_0(N)>0$ and a Doeblin weight $\kappa_0(\beta;S,t_0)>0$ such that, in the sense of densities w.r.t. Haar measure,
\[
  K_{\rm int}^{(a)}(U,V)\ \ge\ \kappa_0(\beta;S,t_0)\,\bigotimes_{\ell\in \text{cut}} p_{t_0}(U_\ell V_\ell^{-1})\,.
\]
...
and provides a ready framework for Perron–Frobenius style spectral estimates on
finite subspaces.
...
\section{\texorpdfstring{Appendix: $N$--uniform OS$\to$gap pipeline (R4)}{Appendix: N-uniform OS-to-gap pipeline (R4)}}
...
\begin{theorem}[Tightness of loop $n$-point functions]\label{thm:c1a-tight}
Fix finitely many rectifiable loops $\Gamma_1,\dots,\Gamma_n$ contained in a bounded physical region $R$. Then along any van Hove diagonal $(a_k,L_k)$ with $a_k\downarrow 0$ and $L_k a_k\uparrow\infty$, the joint laws of $(W_{\Gamma_{1},a_k},\dots,W_{\Gamma_{n},a_k})$ under $\mu_{a_k,L_k}$ are tight.

\noindent If, in addition, the AF--free NRC/uniqueness package applies on the fixed region containing $\{\Gamma_i\}$ (e.g.\ Theorem~\ref{thm:U2-nrc-unique}, with one-point input supplied by Proposition~\ref{prop:one-point-resolvent} using Theorem~\ref{thm:U2b-global}), then the corresponding loop Schwinger functions converge uniquely (no subsequences) to consistent limits $\{S_n\}_n$.
\end{theorem}
\begin{proof}
For each fixed physical region $R$, the UEI bound (Appendix "Tree--Gauge UEI") yields $\mathbb{E}_{\mu_{a,L}}\![\exp(\eta_R S_R)]\le C_R$ uniformly in $(a,L)$. Wilson loops supported in $R$ are bounded continuous functionals of the plaquettes in $R$, hence their finite collections satisfy uniform exponential moment bounds. By Prokhorov's theorem, the family of joint laws is tight.

If the AF--free NRC/uniqueness package applies on $R$ (e.g.\ Theorem~\ref{thm:U2-nrc-unique}), then embedded resolvents converge in operator norm uniformly on compact subsets of $\mathbb C\setminus\mathbb R$. The Laplace–resolvent representation then yields convergence and uniqueness of the corresponding loop Schwinger limits along van Hove diagonals.
\end{proof}
...
\paragraph{NRC via explicit embeddings (conditional on U2b).}
\begin{theorem}[NRC for all nonreal $z$]\label{thm:nrc-explicit}
Assume Theorem~\ref{thm:U2b-global}. Let $I_{a,L}:\mathcal H_{a,L}\to\mathcal H$ be the OS/GNS embedding induced by polygonal loop embeddings on generators: on $\mathcal A_{a,+}$ set $E_a(W_\Lambda):=W_{\mathrm{poly}(\Lambda)}$ and define $I_{a,L}[F]:=[E_a(F)]$. Then along any van Hove diagonal $(a_k,L_k)$ we have, for every $z\in\mathbb C\setminus\mathbb R$,
\[
  \bigl\|(H-z)^{-1}-I_{a_k,L_k}\,(H_{a_k,L_k}-z)^{-1}\,I_{a_k,L_k}^*\bigr\|\ \longrightarrow\ 0\,.
\]
\end{theorem}
\begin{proof}
This is a direct restatement of Theorem~\ref{thm:nrc-operator-norm} for the explicit polygonal embeddings $I_{a,L}$.
\end{proof}
...
Let $\{\mu_k\}$ be a sequence of OS-positive measures for a fixed link reflection (OS2 holds at each $k$). Assume $\mu_k\Rightarrow\mu$ weakly on cylinder sets, i.e. $\mathbb E_{\mu_k}[F]\to\mathbb E_{\mu}[F]$ for every bounded cylinder observable $F$. Then $\mu$ satisfies OS2: for every bounded cylinder polynomial $P$ supported in the positive time half-space,
\[
  \mathbb E_{\mu}\big[\Theta P\,\overline{P}\big]\ \ge\ 0.
\]
\end{lemma}

\begin{proof}
Let $P$ be any bounded cylinder polynomial supported in $t\ge 0$. Since $\Theta P\,\overline{P}$ is again a bounded cylinder observable, the assumed cylinder weak convergence gives
\[
  \mathbb E_{\mu}\big[\Theta P\,\overline{P}\big]\ =\ \lim_{k\to\infty}\ \mathbb E_{\mu_k}\big[\Theta P\,\overline{P}\big].
\]
Each term in the limit is $\ge 0$ by OS2 for $\mu_k$, hence the limit is $\ge 0$.
\end{proof}
...
Let $\{\mu_k\}$ be a sequence of measures on the loop/cylinder algebra on $\mathbb R^4$ that converges weakly on cylinder sets to a limit $\mu$ (i.e. $\mathbb E_{\mu_k}[F]\to\mathbb E_\mu[F]$ for every bounded cylinder observable $F$). Assume a uniform exponential clustering bound: there exist $C,c>0$ such that for every pair of bounded (gauge--invariant) cylinder observables $A,B$ supported in a fixed bounded region and every translation vector $\mathbf R$,
\[
  \big|\operatorname{Cov}_{\mu_k}\big(A,\tau_{\mathbf R}B\big)\big|\ \le\ C\,e^{-c\|\mathbf R\|}\qquad(\forall k).
\]
Then the limit Schwinger functions satisfy clustering: for translated observables $B_{\mathbf R}:=\tau_{\mathbf R}B$,
\[
  \lim_{\|\mathbf R\|\to\infty}\ S_2(A,B_{\mathbf R})\ =\ S_1(A)\,S_1(B).
\]
\end{lemma}

\begin{proof}
Fix $\mathbf R$. Since $A$, $B_{\mathbf R}$, and $A\,B_{\mathbf R}$ are bounded cylinder observables, cylinder weak convergence gives
\[
  \operatorname{Cov}_{\mu}(A,B_{\mathbf R})
  \ =\ \lim_{k\to\infty}\operatorname{Cov}_{\mu_k}(A,B_{\mathbf R}).
\]
Taking absolute values and using the uniform bound yields
\[
  \big|\operatorname{Cov}_{\mu}(A,B_{\mathbf R})\big|\ \le\ C\,e^{-c\|\mathbf R\|}.
\]
Letting $\|\mathbf R\|\to\infty$ gives $\operatorname{Cov}_{\mu}(A,B_{\mathbf R})\to 0$, i.e. $S_2(A,B_{\mathbf R})\to S_1(A)S_1(B)$.
\end{proof}

\begin{lemma}[OS5: unique vacuum in the limit]\label{lem:os5-limit}
Let $\{\mu_k\}$ be OS-positive lattice measures with transfer operators $T_k=e^{-H_k}$ (OS/GNS) and assume:
\begin{itemize}
  \item[(i)] each $H_k\ge 0$ has a one-dimensional vacuum space and a uniform gap: $\operatorname{spec}(H_k)\subset\{0\}\cup[\gamma_0,\infty)$ for some $\gamma_0>0$ independent of $k$;
  \item[(ii)] the continuum generator $H\ge 0$ exists and the gap persists to the limit (e.g. by Theorem~\ref{thm:gap-persist-cont} under the stated NRC hypotheses).
\end{itemize}
Then the limit theory has a unique vacuum and
\[
  \operatorname{spec}(H)\subset\{0\}\cup[\gamma_0,\infty).
\]
\end{lemma}

\begin{proof}
The spectral inclusion is exactly the gap-persistence conclusion (ii). Vacuum uniqueness follows because the vacuum space is the $0$-eigenspace of $H$ and the spectral projection onto $\{0\}$ is isolated by the gap; under norm-resolvent convergence, the corresponding Riesz projector is stable, so the rank of the vacuum projector cannot change in the limit (hence remains one-dimensional).
\end{proof}
...
\section{Optional: Asymptotic-freedom scaling schedule (conditional packaging; not used in main chain)}

We record a \emph{conditional} packaging statement for an asymptotic-freedom (AF) scaling schedule $\beta(a)$. This section is \emph{not} used in the main chain: it does not prove existence of a 4D weak-coupling scaling limit from first principles, but only explains how the AF--free fixed-region NRC/uniqueness package would be applied along a concrete monotone schedule once the fixed-region inputs (in particular Theorem~\ref{thm:U2b-global}) are available.
...
\begin{itemize}
  ...
  \item[(AF4)] work along a scaling-window schedule satisfying \eqref{eq:ucis-sw-window} so that the fixed-physical-time interface contraction constants from UCIS$_{\rm SW}$ are uniform in volume/boundary data on fixed slabs (Thm.~\ref{thm:ucis-sw}, Prop.~\ref{prop:os35-limit}).
\end{itemize}
An explicit example is $\beta(a)=\beta_{\min}+c_0\log(1+a_0/a)$ with $c_0>0$.
\paragraph{Gap input along AF (conditional).}
Along schedules satisfying \eqref{eq:ucis-sw-window}, UCIS$_{\rm SW}$ supplies a fixed-physical-time contraction (Cor.~\ref{cor:ucis-sw-L2-contraction}, Thm.~\ref{thm:ucis-sw-odd-subspace}) and hence a slab gap input transported to the continuum by gap persistence (Thm.~\ref{thm:gap-persist-cont}), conditional on the NRC/uniqueness package (in particular Theorem~\ref{thm:U2b-global}).
\paragraph{Existence (OS0--OS5) and uniqueness (conditional).}
...
\end{itemize}
\noindent To remove subsequences, one uses the AF--free uniqueness theorem on fixed regions (Theorem~\ref{thm:U2-nrc-unique}), which upgrades one-point operator--norm resolvent convergence (e.g.\ Proposition~\ref{prop:one-point-resolvent}, using Theorem~\ref{thm:U2b-global}) to uniform convergence on compact $K\subset\mathbb C\setminus\mathbb R$ and hence uniqueness of the limiting generator.
\paragraph{AF schedule theorem.}
\begin{theorem}[AF schedule packaging (conditional; not used in main chain)]\label{thm:af-schedule-gap}
Assume the hypotheses of the main AF--free continuum construction on fixed regions (in particular the scaling-window interface input UCIS$_{\rm SW}$ and Theorem~\ref{thm:U2b-global} for NRC/uniqueness). Fix any monotone schedule $\beta(a)$ satisfying (AF1)--(AF4) and any van Hove choice $L(a)$ with $L(a)\,a\to\infty$. Then along this schedule the resulting continuum limits are unique (no subsequences) and satisfy OS0--OS5; moreover the limiting generator $H\ge 0$ obeys
\[
  \operatorname{spec}(H)\subset\{0\}\cup[\gamma_0,\infty),\qquad \gamma_{\mathrm{phys}}\ge \gamma_0>0\,.
\]
\end{theorem}
\begin{proof}
This is a restatement of the main conditional continuum package (existence/uniqueness via AF--free NRC and gap persistence) specialized to a single monotone schedule; it is included only as a packaging convenience and is not used elsewhere.
\end{proof}
...
\section{Optional Appendix: epsilon-uniform cluster expansion along a scaling trajectory (C3)}
...
\subsection*{Referee checklist (Clay requirements to labels)}
\begin{itemize}
  ...
  \item U2b graph-defect bound $\|(H+1)^{-1/2}D_{a,L}(H_{a,L}+1)^{-1/2}\|\le C_{\rm gd}a$ and low-energy projection control $\|(I-P_{a,L})E_H([0,\Lambda])\|\le C_\Lambda a$: Theorem~\ref{thm:U2b-global} (U2b quantitative inputs).
  ...
  \item Interface smoothing (main chain; scaling-window UCIS$_{\rm SW}$): Thm.~\ref{thm:ucis-sw} under \eqref{eq:ucis-sw-window}, Cor.~\ref{cor:ucis-sw-L2-contraction}, Thm.~\ref{thm:ucis-sw-odd-subspace}, Prop.~\ref{prop:os35-limit} (U3). (A schedule-free $\beta$-uniform one-step Doeblin minorization is not used in the main chain; see Appendix~\ref{app:beta-indep-minorization} for legacy statements.)
  \item Odd-subspace contraction and two-layer upgrade bookkeeping: Thm.~\ref{thm:ucis-sw-odd-subspace} (odd contraction input) together with Lem.~\ref{lem:local-gram-bounds}, Prop.~\ref{prop:psd-crossing-gram}, and Thm.~\ref{thm:eight-tick-uniform} (Gershgorin/two-layer/eight-tick upgrade) (U4).
...
\end{theorem}
\begin{proof}
By UEI/LSI (U1), for any smeared local functional $F$ supported in a fixed region $R$, the Laplace transform obeys $\log\mathbb E[e^{t(F-\mathbb EF)}]\le t^2 C(R)/(2\rho)$, giving uniform sub-Gaussian tails. Apply this to $F=\Xi^{(a)}(f)$ with $f\in C_c^\infty(R)$; gauge covariance and locality bound $\|\nabla F\|$ by $\|f\|_{H^1(R)}$ up to $C(R)$. Thus $\sup_a \mathbb E[\,|\Xi^{(a)}(f)|^2\,]\le C(R)\,\|f\|_{H^1}^2$. Fix a reference $f_\mu$ and choose $Z_F(a)$ to normalize $\langle \Xi^{(a)}_R(f_\mu)^2\rangle$ to a finite constant; the bound forces $\sup_a Z_F(a)\le C'(R)$. Tightness and the AF-free NRC (using Theorem~\ref{thm:U2b-global}) yield convergence of $\Xi^{(a)}_R$ in $\mathsf S'$ along van Hove. Nontriviality follows from Proposition~\ref{prop:nonzero-cumulant4}: a strictly positive truncated 4-point persists in the limit, hence $\Xi_R\not\equiv 0$.
\end{proof}
...
\subsection{U1. Local LSI/UEI on fixed regions}
...
\medskip
\noindent\emph{U1 proof (Track-J route).} The required logarithmic Sobolev inequality (LSI)
\[
  \mathrm{Ent}_{\mu_{R}^{(a)}}(f^2)\ \le\ \frac{1}{\rho_R}\int \|\nabla f\|^2\,d\mu_{R}^{(a)}\qquad(\forall\,f)
\]
is established for the intented continuum limit by the Track-J heat-smoothing route (Theorem~\ref{thm:uei-fixed-region} and Lemma~\ref{lem:U1-J1-smoothed-lsi}), with $\rho_R$ uniform in $a$ and in the exterior boundary configuration outside $R$. The required Lipschitz/gradient bounds for the time-zero local observables follow from the tree-gauge geometry (Lemma~\ref{lem:U1-tree-bounds}).

\medskip
\noindent\emph{Conclusion.} By the Herbst argument, this yields centered subgaussian Laplace bounds on $R$ and hence uniform exponential integrability/tightness for the corresponding local cylinder laws. (See Section~\ref{sec:main-theorem-unconditional} and Theorem~\ref{thm:uei-fixed-region} for the primary closure.)
\end{theorem}
...
\subsection*{\texorpdfstring{Finite-region classical control (plaquette$\to F^2$, $O(a^2)$)}{Finite-region classical control (plaquette-to-F2, O(a2))}}
\begin{theorem}[\texorpdfstring{Finite-region, gauge-invariant plaquette$\to F^2$ control; explicit $O(a^2)$}{Finite-region, gauge-invariant plaquette-to-F2 control; explicit O(a2)}]\label{DEC:plaquette-F2}
Let $U\Subset\mathbb R^4$ be a bounded Lipschitz region and let $A$ be a smooth $\mathfrak{su}(N)$ connection on an open neighborhood of $\overline U$, with curvature $F$ and bounded covariant derivatives up to order $2$ on $U$. Fix $\beta>0$ and (to avoid boundary-layer discretization effects) assume the curvature is supported away from the boundary: there exists $\delta>0$ such that $\mathrm{supp}(F)\Subset U_\delta:=\{x\in U:\mathrm{dist}(x,\partial U)>\delta\}$.

For lattice spacing $a\in(0,\delta]$, let $S^{(a)}_U(A)=\tfrac{\beta}{N}\sum_{p\subset U}\Re\,\Tr(I-U_p)$ be the Wilson plaquette action over plaquettes entirely contained in $U$ (with $U_p$ the plaquette holonomy induced by $A$), and define the corresponding continuum normalization
\[
  S_U(A)\ :=\ \frac{\beta}{4N}\int_U \Tr(F_{\mu\nu}F_{\mu\nu})\,dx.
\]
Then for all sufficiently small $a>0$,
\[
  \big\lvert\,S^{(a)}_U(A)-S_U(A)\,\big\rvert\ \le\ \frac{\beta}{4N}\,C_2(N,U;M_0,M_1,M_2)\,a^2,
\]
...
\end{theorem}
\begin{remark}[Boundary collar]\label{rem:DEC-collar}
The assumption $\mathrm{supp}(F)\Subset U_\delta$ is a convenient way to avoid an $O(a)$ boundary truncation term when the lattice sum is taken over plaquettes entirely contained in $U$. In applications one may take $U$ to be a slightly enlarged neighborhood of the support of the fixed local core/observable under consideration, so that the relevant contribution is effectively interior. If one drops the collar assumption, the same local holonomy expansion yields an additional boundary-strip error of order $O(a)$ proportional to $|\{x\in U:\mathrm{dist}(x,\partial U)\le a\}|$.
\end{remark}
\begin{proof}
\emph{Classical plaquette expansion.} Fix a plaquette $p$ of side length $a$ in the $\mu\nu$--plane with center $x_p$. Writing the plaquette holonomy as a path--ordered exponential around $\partial p$, standard nonabelian Stokes/BCH expansions (cf.\ \cite[Ch.~4]{MontvayMunster1994}) give
\[
  U_p\ =\ \exp\!\Big(i\,a^{2}F_{\mu\nu}(x_p)\ +\ R_p\Big),\qquad \|R_p\|\ \le\ C(M_0,M_1,M_2)\,a^{3},
\]
uniformly for $p\subset U$ and all sufficiently small $a$, with $C$ depending only on the gauge--invariant bounds $(M_0,M_1,M_2)$ on $U$.

\emph{From holonomy to energy density.} For Hermitian $X$ with $\|X\|$ small,
\[
  \Re\,\Tr\big(I-e^{iX}\big)\ =\ \tfrac12\,\Tr(X^{2})\ +\ O(\|X\|^{4}),
\]
with an absolute remainder bound. Apply this with $X=a^{2}F_{\mu\nu}(x_p)+O(a^{3})$ to obtain the per--plaquette estimate
\[
  \Re\,\Tr(I-U_p)\ =\ \tfrac{a^{4}}{2}\,\Tr\!\big(F_{\mu\nu}(x_p)^{2}\big)\ +\ O(a^{6}),
\]
where the $O(a^{6})$ constant depends only on $(M_0,M_1,M_2)$.

\emph{Summation and the collar assumption.} Because $\mathrm{supp}(F)\Subset U_\delta$, for all $a\le\delta$ the plaquettes that contribute to $S^{(a)}_U(A)$ lie in a compact subset of $U$ a fixed distance from $\partial U$, so no boundary layer contributes. Summing the $O(a^{6})$ remainder over $O(a^{-4})$ plaquettes in a fixed region yields an $O(a^{2})$ total error. The main term is a midpoint Riemann sum for $\int_U \Tr(F_{\mu\nu}F_{\mu\nu})\,dx$; since $D^2F$ is bounded on $U$, the midpoint rule error is also $O(a^2)$. Using the identity $\sum_{\mu<\nu}\Tr(F_{\mu\nu}^2)=\tfrac12\Tr(F_{\mu\nu}F_{\mu\nu})$ and the definition of $S_U(A)$ gives the displayed bound.
\end{proof}
...
\end{lemma}
\begin{proof}
Write $R(z):=(H-z)^{-1}$ and $R_{a,L}(z):=(H_{a,L}-z)^{-1}$, and note $P_{a,L}=I_{a,L}I_{a,L}^*$. Decompose
\[
  R(z)\xi\ =\ R(z)(I-P_{a,L})\xi\ +\ R(z)P_{a,L}\xi
  \ =\ R(z)(I-P_{a,L})\xi\ +\ R(z)I_{a,L}I_{a,L}^*\xi.
\]
It remains to compare $R(z)I_{a,L}$ with $I_{a,L}R_{a,L}(z)$. On the common core where $D_{a,L}$ is defined,
\[
  (H-z)I_{a,L}\ -\ I_{a,L}(H_{a,L}-z)\ =\ HI_{a,L}-I_{a,L}H_{a,L}\ =\ D_{a,L}.
\]
Left-multiply by $R(z)$ and right-multiply by $R_{a,L}(z)$ to obtain (on that core)
\[
  R(z)I_{a,L}\ -\ I_{a,L}R_{a,L}(z)\ =\ -\,R(z)\,D_{a,L}\,R_{a,L}(z).
\]
Apply this identity to $I_{a,L}^*\xi$ and substitute into the decomposition above:
\[
  R(z)I_{a,L}I_{a,L}^*\xi\ =\ I_{a,L}R_{a,L}(z)I_{a,L}^*\xi\ -\ R(z)D_{a,L}R_{a,L}(z)I_{a,L}^*\xi.
\]
Combining the two displays yields the claimed resolvent comparison identity.
\end{proof}
...
\begin{itemize}
  \item embeddings $I_{a,L}$ extend to isometries $I_{a,L}:\mathcal H_{a,L}\to\mathcal H$ (so $I_{a,L}^*I_{a,L}=\mathrm{id}$);
  \item the graph defect satisfies $\|(H+1)^{-1/2}D_{a,L}(H_{a,L}+1)^{-1/2}\|\le C a$ on a common core;
  \item for some $z_0\in\mathbb C\setminus\mathbb R$, there is a function $\varepsilon(a)\downarrow 0$ such that along van Hove scaling pairs $(a,L)$ one has the one--point operator--norm resolvent estimate
  \[
    \big\|(H-z_0)^{-1}-I_{a,L}(H_{a,L}-z_0)^{-1}I_{a,L}^*\big\|\ \le\ \varepsilon(a).
  \]
\end{itemize}
...
\end{theorem}
\begin{proof}
For each $(a,L)$ define the embedded resolvent family on $\mathcal H$
\[
  R_{a,L}(z)\ :=\ I_{a,L}(H_{a,L}-z)^{-1}I_{a,L}^*,\qquad z\in\mathbb C\setminus\mathbb R.
\]
Since $I_{a,L}$ is an isometry ($I_{a,L}^*I_{a,L}=\mathrm{id}$ on $\mathcal H_{a,L}$), the usual resolvent identity on $\mathcal H_{a,L}$ implies the same identity for the embedded family: for all $z,w\in\mathbb C\setminus\mathbb R$,
\[
  R_{a,L}(z)-R_{a,L}(w)\ =\ (z-w)\,R_{a,L}(z)\,R_{a,L}(w).
\]
The continuum resolvent $R(z):=(H-z)^{-1}$ satisfies the same identity.

\medskip
\emph{Uniform convergence on compact $K$ from one-point convergence.} Fix $z\in K$ and set $\delta:=z-z_0$. The resolvent identity with $w=z_0$ yields
\[
  R(z)\,\big(I-\delta\,R(z_0)\big)\ =\ R(z_0),\qquad
  R_{a,L}(z)\,\big(I-\delta\,R_{a,L}(z_0)\big)\ =\ R_{a,L}(z_0).
\]
Hence
\[
  R(z)\ =\ R(z_0)\,\big(I-\delta\,R(z_0)\big)^{-1},\qquad
  R_{a,L}(z)\ =\ R_{a,L}(z_0)\,\big(I-\delta\,R_{a,L}(z_0)\big)^{-1}.
\]
Both inverses exist, and on $K$ their norms are uniformly bounded: using the commutativity of resolvents one has
\[
  \big(I-\delta\,R(z_0)\big)^{-1}\ =\ I+\delta\,R(z),\qquad
  \big(I-\delta\,R_{a,L}(z_0)\big)^{-1}\ =\ I+\delta\,R_{a,L}(z),
\]
so
\[
  \big\|\big(I-\delta\,R(z_0)\big)^{-1}\big\|\ \le\ 1+|z-z_0|\,\|R(z)\|,\qquad
  \big\|\big(I-\delta\,R_{a,L}(z_0)\big)^{-1}\big\|\ \le\ 1+|z-z_0|\,\|R_{a,L}(z)\|.
\]
Since $H$ and each $H_{a,L}$ are self-adjoint, $\|R(z)\|\le \mathrm{dist}(z,\mathbb R)^{-1}$ and $\|R_{a,L}(z)\|\le \mathrm{dist}(z,\mathbb R)^{-1}$, so these bounds are uniform on compact $K\subset\mathbb C\setminus\mathbb R$ and in $(a,L)$.

Now write
\[
  R(z)-R_{a,L}(z)
  \ =\ \big(R(z_0)-R_{a,L}(z_0)\big)\big(I-\delta\,R(z_0)\big)^{-1}
  \ +\ R_{a,L}(z_0)\Big(\big(I-\delta\,R(z_0)\big)^{-1}-\big(I-\delta\,R_{a,L}(z_0)\big)^{-1}\Big).
\]
Use the inverse-difference identity $B^{-1}-C^{-1}=B^{-1}(C-B)C^{-1}$ with $B=I-\delta R(z_0)$ and $C=I-\delta R_{a,L}(z_0)$ to obtain
\[
  \big\|R(z)-R_{a,L}(z)\big\|\ \le\ C_K\,\big\|R(z_0)-R_{a,L}(z_0)\big\|
\]
for a constant $C_K<\infty$ depending only on $K$ and $z_0$. Taking the supremum over $z\in K$ and applying the one-point estimate at $z_0$ yields
\[
  \sup_{z\in K}\big\|R(z)-R_{a,L}(z)\big\|\ \le\ C_K\,\varepsilon(a)\ \xrightarrow[a\downarrow 0]{}\ 0.
\]

\medskip
\emph{Uniqueness (no subsequences).} The bound above shows that any van Hove scaling sequence has at most one operator-norm resolvent limit on $K$, namely $R(z)=(H-z)^{-1}$; hence $H$ is unique as the resolvent limit.

\medskip
\emph{Semigroup convergence.} Operator-norm resolvent convergence on $\mathbb C\setminus\mathbb R$ implies operator-norm convergence of bounded holomorphic functional calculus on the common resolvent set; in particular, for each fixed $t>0$,
\[
  \big\|\,e^{-tH}\ -\ I_{a,L}\,e^{-tH_{a,L}}\,I_{a,L}^*\,\big\|\ \to\ 0.
\]
One may prove this by approximating $e^{-t\cdot}$ uniformly on $\sigma(H)\cup\sigma(H_{a,L})\subset[0,\infty)$ by rational functions with poles off $\mathbb R$ and using the already established uniform resolvent convergence on compact subsets of $\mathbb C\setminus\mathbb R$.
\end{proof}

\begin{proposition}[One–point resolvent estimate at a nonreal $z_0$ (cutoff form)]\label{prop:one-point-resolvent}
Fix $z_0\in\mathbb C\setminus\mathbb R$ and a cutoff $\Lambda\ge 1$. Assume:
\begin{itemize}
  \item the comparison identity of Lemma~\ref{lem:U2-comparison};
  \item Theorem~\ref{thm:U2b-global} (U2b quantitative inputs);
...
\end{itemize}
Then there exists $C(z_0,\Lambda)>0$ such that for all sufficiently small $a\in(0,a_0]$ and all $L$,
\[
  \big\|(H-z_0)^{-1} - I_{a,L}(H_{a,L}-z_0)^{-1} I_{a,L}^*\big\|\ \le\ C(z_0,\Lambda)\, a\ +\ \frac{1}{\operatorname{dist}(z_0,[\Lambda,\infty))}.
\]
\noindent In particular, the left-hand side converges to $0$ as $a\downarrow 0$ (uniformly in $L$ along the van Hove window), since for any $\varepsilon>0$ one may first choose $\Lambda$ so that $\operatorname{dist}(z_0,[\Lambda,\infty))^{-1}\le \varepsilon/2$, and then choose $a$ so that $C(z_0,\Lambda)a\le \varepsilon/2$.
\end{proposition}
\begin{proof}
This is the $z=z_0$ specialization of Theorem~\ref{thm:nrc-quant} with the fixed cutoff $\Lambda$.
\end{proof}
...
\[
  D_{a,L}\ :=\ H I_{a,L}\,-\,I_{a,L} H_{a,L}
\]
is well-defined and satisfies the infinitesimal semigroup commutator identity
\[
  D_{a,L}\,\xi\ =\ -\left.\frac{d}{dt}\right|_{t=0^+}\Big(e^{-tH} I_{a,L}\xi\ -\ I_{a,L} e^{-tH_{a,L}}\xi\Big),
\]
and $\mathcal D^{\rm loc}$ is a common core for $H$, $H_{a,L}$, and the embedded resolvents, and is mapped into itself by the embeddings $I_{a,L}$.
\end{lemma}
\begin{proof}
By construction, $\mathcal D^{\rm loc}$ is invariant under the semigroups $e^{-tH}$ and $e^{-tH_{a,L}}$ and is contained in the domains of the corresponding generators on fixed regions. By the definition of the (strong) generator of a $C_0$-semigroup (e.g. Engel--Nagel \cite{EngelNagel2000}), for $\eta\in\mathcal D^{\rm loc}$ one has $\frac{d}{dt}e^{-tH}\eta|_{t=0^+}=-H\eta$ and similarly for $H_{a,L}$. Apply this to $\eta=I_{a,L}\xi$ and to $\xi$ in the lattice space and subtract to obtain the displayed identity for $D_{a,L}\xi$. The common-core and invariance assertions follow from OS locality and the closure of the time-zero local algebra under time translations and embeddings.
\end{proof}
...
\section{Appendix: beta-independent interface minorization (legacy; not used in main chain)}
\label{app:beta-indep-minorization}

We record the $\beta$\,–\,independent Doeblin minorization on fixed slabs with explicit constants. Let $K^{(a)}_{\rm int}$ be the interface kernel across the OS reflection cut inside a fixed slab $B_{R_*}$ of thickness $a_0$ (Definition~\ref{def:interface-kernel}).
...
\begin{proposition}[Explicit Doeblin constants, $\beta$\,–\,independent (legacy)]\label{prop:explicit-doeblin-constants-appendix}
\noindent\emph{Legacy statement (not used in the main chain).} There exist $t_0=t_0(G)>0$ and $\theta_*=\theta_*(R_*,a_0,G)>0$, independent of $\beta$, $a\in(0,a_0]$, and $L$, such that
\[
...
\begin{itemize}
  ...
  \item[(A2)] Graph\,--\,defect bound (order $a$): There is $C_{\rm gd}>0$ with $\|(H+1)^{-1/2}\,D_{a,L}\,(H_{a,L}+1)^{-1/2}\|\le C_{\rm gd}\,a$ for all $a\in(0,a_0]$ and $L$, where $D_{a,L}:=H I_{a,L}-I_{a,L}H_{a,L}$ is defined on $\mathcal D^{\rm loc}$.
...
\end{corollary}
\begin{proof}
(A1) is Lemma~\ref{lem:U2-defect-core} (common core and generator/derivative-at-$0^+$ identity). (A2) is the U2b graph-defect bound $\|(H+1)^{-1/2}D_{a,L}(H_{a,L}+1)^{-1/2}\|\le C_{\rm gd}a$ (Theorem~\ref{thm:U2b-global}). (A3) is the U2b low-energy projection control $\|(I-P_{a,L})E_H([0,\Lambda])\|\le C_\Lambda a$ (Theorem~\ref{thm:U2b-global}). (A4) follows from the spectral-theorem weighted resolvent bound (Lemma~\ref{lem:weighted-resolvent}) together with its continuum analogue (defining $C_H(z_0)$). (A5) is Proposition~\ref{prop:one-point-resolvent}, which is conditional on these U2b inputs via Theorem~\ref{thm:nrc-quant}. Apply Theorem~\ref{thm:abstract-nrc}.
\end{proof}
...
The small\,–\,$\beta$ regime provides an alternative route to OS0 on fixed regions and supplies independent clustering inputs. The global results in Section~\ref{sec:global-R4} remain based on the AF\,–\,free NRC and the scaling-window interface closure UCIS$_{\rm SW}$ (Theorem~\ref{thm:ucis-sw} under \eqref{eq:ucis-sw-window}); a fully $\beta$\,–\,uniform one-step minorization is not used in the main chain (and is recorded only as a legacy statement in Appendix~\ref{app:beta-indep-minorization}).
\end{remark}
...
\noindent\emph{Remark.} The present manuscript proves the required ingredients directly in the AF\,–\,free framework on fixed slabs (UEI/OS0, calibrated NRC, and the scaling-window interface closure UCIS$_{\rm SW}$) and globalizes via projective\,–\,limit semigroups (Theorem~\ref{thm:proj-semigroup}). This appendix is informational only.
...
\bibitem{Ciarlet2002}
P.~G. Ciarlet, \emph{The Finite Element Method for Elliptic Problems}, SIAM Classics in Applied Mathematics, SIAM, 2002.
