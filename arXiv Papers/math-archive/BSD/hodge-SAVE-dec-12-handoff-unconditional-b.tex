...
% ==========================================================
% Edit highlighting
% - Legacy edits: blue (editblock / \editblue)
% - Referee-response pass edits: magenta (editrefblock / \editref)
% - Amir integration pass edits: green (editamirblock / \editamir)
% - p-range wiring / Lefschetz pass edits: cyan (editpblock / \editp)
% - cone/coercivity reconciliation pass edits: custom orange (editconeblock / \editcone)
% ==========================================================
...
% ==========================================================
% Final-mode switch (disable colored edit markup for submission)
% ==========================================================
% Set \finaltrue for a referee/journal PDF without colored change-tracking.
% Set \finalfalse for internal drafts (keep colors).
\newif\iffinal
\finaltrue
% \finalfalse
\iffinal
  \renewcommand{\editblue}[1]{#1}
  \renewenvironment{editblock}{}{}
  \renewcommand{\editref}[1]{#1}
  \renewenvironment{editrefblock}{}{}
  \renewcommand{\editamir}[1]{#1}
  \renewenvironment{editamirblock}{}{}
  \renewcommand{\editp}[1]{#1}
  \renewenvironment{editpblock}{}{}
  \renewcommand{\editcone}[1]{#1}
  \renewenvironment{editconeblock}{}{}
\fi
...
a smooth projective K\"ahler manifold and summarizes the proof strategy used in this manuscript.
...
\item \textbf{Spine theorem (explicit defect accounting)} (Theorem~\ref{thm:spine-quantitative}): packages the output as a fixed-class closed integral cycle sequence with the checkable estimate $0\le \Mass(T_j)-c_0\le 2\,\Mass(G_j)$, reducing the whole program to proving (H1)--(H2).
...
\Mass(T_j)\to c_0,
\]
for the corresponding closed integral cycles $T_j$ in class $A=\mathrm{PD}(m[\gamma])$.
\end{lemma}
\begin{proof}
By Proposition~\ref{prop:integer-transport} (and $\delta_j=o(h_j)$, $\varepsilon_j=o(1)$) one has $\mathcal F(\partial T^{\mathrm{raw}})=o(m)$ as $h_j\to 0$.
Since $m$ is fixed once and for all, this implies $\mathcal F(\partial T^{\mathrm{raw}})\to 0$.
Proposition~\ref{prop:glue-gap} then supplies correction currents $R_{\mathrm{glue},j}$ with $\partial(T^{\mathrm{raw}}+R_{\mathrm{glue},j})=0$ and
$\Mass(R_{\mathrm{glue},j})\to 0$.
Enforcing the global cohomology constraints (Substep~4.3 / Proposition~\ref{prop:cohomology-match}) yields $[T_j]=A$.
Finally, Proposition~\ref{prop:almost-calibration} gives
$0\le \Mass(T_j)-c_0\le 2\,\Mass(R_{\mathrm{glue},j})\to 0$, hence $\Mass(T_j)\to c_0$.
\end{proof}
...
\begin{definition}[Stationary Young–measure realizability (SYR)]\label{def:syr}
	...
	is SYR–realizable if there exists a sequence of integral $2n\!-\!2p$ cycles $T_k$
	with
	\begin{enumerate}
		\item $\partial T_k=0$ and $[T_k]=\mathrm{PD}(m[\gamma])$ for some fixed $m\ge 1$, and
		\item the \emph{calibration defect} satisfies
		\[
		\Def_{\mathrm{cal}}(T_k)\ :=\ \Mass(T_k)-\langle T_k,\psi\rangle\ \longrightarrow\ 0.
		\]
	\end{enumerate}
	Equivalently, $\Mass(T_k)\to c_0:=\langle \mathrm{PD}(m[\gamma]),[\psi]\rangle$ since $\langle T_k,\psi\rangle=c_0$ for all $k$ (closedness of $\psi$).
	
	\smallskip\noindent
	(The acronym SYR reflects that the construction can be organized via local plane-mixtures / Young-measure bookkeeping.  This extra bookkeeping is \emph{not} needed for the calibrated-limit closure step; only the defect condition above is used in Theorem~\ref{thm:syr}.)
\end{definition}
...
\begin{proof}
	By Definition~\ref{def:syr}, there exists a fixed-class sequence of integral cycles $T_k$ with $\Def_{\mathrm{cal}}(T_k)\to 0$, hence $\Mass(T_k)\to c_0=\langle \mathrm{PD}(m[\gamma]),[\psi]\rangle$.
	Apply Theorem~\ref{thm:realization-from-almost}.
\end{proof}
...
form equals $\omega$.
(Notation: we reserve $m$ for the homology multiple in $A=\mathrm{PD}(m[\gamma])$.  In this ``projective/Bergman'' step we write $M$ for the tensor power of $L$, so the Bergman control scale is $M^{-1/2}$.)
\smallskip
\noindent
For $M\in\N$ large, consider the complete linear system $|L^M|$.
...
many $\psi$-calibrated $(n-p)$-submanifolds $Y_1,\ldots,Y_{N_{\mathrm{dir}}}$ (each a
...
Since $[\gamma]$ is rational, all its periods lie in $(1/D)\Z$ for some
fixed integer $D$.  Choose $m\gg 1$ divisible by $D$.
...
\]
At the intrinsic Bergman cell size $h\sim M^{-1/2}$ (where $M$ is the line-bundle power in Step~2) this becomes
\[
\frac{\mathcal F(\partial T^{\mathrm{raw}})}{m}\ \lesssim\ m^{-1/k}\,M^{-(1-\frac{n}{k})}\;\varepsilon^{-\frac{2p}{k}}.
\]
In particular, for the Hodge-relevant range $p\le n/2$ one has $k\ge n$, so the right-hand side tends to $0$ as $m\to\infty$ (and for $p<n/2$ the additional factor $M^{-(1-n/k)}$ gives further decay as $M\to\infty$).
...
holomorphic control scale $h\sim M^{-1/2}$ (where $M$ is the line-bundle power in Step~2).
...
\[
M_Q\ \sim\ m\int_Q \beta\wedge\psi\ \asymp\ m\,h^{2n}\ \asymp\ m\,M^{-n}.
\]
...
\ \asymp\ m^{\frac{k-1}{k}}\,M^{\frac{n}{k}-1}\,N_Q^{\frac1k}.
\]
Since $(k-1)/k<1$ for $k=2n-2p\ge 2$, this is automatically sublinear in $m$ (and decays in $M$ when $k>n$, i.e.\ $p<n/2$) provided $N_Q$ grows at most polynomially in $m$ and $M$.
...
on balls of radius $\asymp M^{-1/2}$ (where $M$ is the line-bundle power).  If one chooses the cell size $h$ at this intrinsic scale to guarantee uniform $C^1$ graph control on each cell,
then
\[
h\ \lesssim\ M^{-1/2}
\qquad\Longrightarrow\qquad
N_Q\sim m\,h^{2p}\ \lesssim\ m\,M^{-p}.
\]
...
\]
gives $\partial R_{\mathrm{glue}}=-\partial T^{\mathrm{raw}}$.
\begin{proposition}[Microstructure/gluing estimate]\label{prop:glue-gap}
Let $T^{\mathrm{raw}}$ be a raw integral current obtained by summing calibrated holomorphic pieces cellwise (Substep~4.1), so that $\partial T^{\mathrm{raw}}$ is supported on the mesh interfaces.
Assume the weighted face-control hypotheses of Corollary~\ref{cor:global-flat-weighted} hold, so that
\[
\mathcal F\!\left(\partial T^{\mathrm{raw}}\right)\ \lesssim\ h^2\sum_Q\sum_{a\in\mathcal S(Q)} m_{Q,a}^{\frac{k-1}{k}}\ +\ O(\varepsilon\,m),
\qquad k:=2n-2p.
\]
Then the correction current $R_{\mathrm{glue}}$ constructed from the flat-norm decomposition satisfies the explicit estimate
\[
\Mass(R_{\mathrm{glue}})
\ \le\ 2\,\mathcal F(\partial T^{\mathrm{raw}})
\ +\ C\,\bigl(\mathcal F(\partial T^{\mathrm{raw}})\bigr)^{\frac{k}{k-1}},
\]
where $C$ is the Federer--Fleming isoperimetric constant in dimension $k-1$.
In particular, in any parameter regime where $\mathcal F(\partial T^{\mathrm{raw}})\to 0$, one has $\Mass(R_{\mathrm{glue}})\to 0$ and
\[
T^{(1)}:=T^{\mathrm{raw}}+R_{\mathrm{glue}}
\]
is a closed integral current with $[T^{(1)}]=[T^{\mathrm{raw}}]$.
\end{proposition}

\begin{proof}
By definition of $\mathcal F$ there exist integral currents $R$ and $Q$ with $\partial T^{\mathrm{raw}}=R+\partial Q$ and
$\Mass(R)+\Mass(Q)\le 2\,\mathcal F(\partial T^{\mathrm{raw}})$.
Since $\partial T^{\mathrm{raw}}$ is itself a boundary, $R$ is null-homologous; by the Federer--Fleming isoperimetric inequality there exists an integral filling
$Q_R$ with $\partial Q_R=R$ and $\Mass(Q_R)\le C\,\Mass(R)^{\frac{k}{k-1}}$.
Setting $R_{\mathrm{glue}}:=-(Q+Q_R)$ gives $\partial R_{\mathrm{glue}}=-\partial T^{\mathrm{raw}}$ and
\[
\Mass(R_{\mathrm{glue}})\ \le\ \Mass(Q)+\Mass(Q_R)
\ \le\ 2\,\mathcal F(\partial T^{\mathrm{raw}})\ +\ C\,\bigl(2\,\mathcal F(\partial T^{\mathrm{raw}})\bigr)^{\frac{k}{k-1}},
\]
which implies the stated estimate after adjusting $C$.
\end{proof}

\subsection*{A transport viewpoint and a discrete reduction (background)}
...
is the difference of the resulting face-slice currents.  The flat-norm dual constraint includes $\|d\eta\|_{\mathrm{comass}}\le 1$, so boundary integrals vary
Lipschitzly under small transverse shifts, suggesting control of $\mathcal F(B_F)$ by a transverse $W_1$ matching cost.
...
\[
h^{k-1}\cdot\delta\int_{\Omega_F}\rho_F
\ =\ O\!\left(h^{2n-2p-1}\cdot \delta\cdot mh^{2p}\right)
\ =\ O\!\left(m\,\delta\,h^{2n-1}\right),
\]
so summing over all faces gives $O(m\,\delta/h)=o(m)$ provided $\delta=o(h)$.
Likewise the $\varepsilon$ term yields a global bound $O(\varepsilon m)=o(m)$ when $\varepsilon=o(1)$.
\end{proof}
...
$\bigl|\int_{R_{\mathrm{glue}}}\eta_\ell\bigr|\le \|\eta_\ell\|_{\mathrm{comass}}\,\Mass(R_{\mathrm{glue}})$.
...
\]
Since $T^{(1)}$ is a closed integral current, Lemma~\ref{lem:integral-periods} gives $\int_{T^{(1)}}\eta_\ell\in\Z$.
As $mI_\ell\in\Z$ by the choice of $m$, the strict inequality $|\int_{T^{(1)}}\eta_\ell-mI_\ell|<1$ forces
$\int_{T^{(1)}}\eta_\ell=mI_\ell$ (Lemma~\ref{lem:lattice-discreteness}).
Hence
\[
...
Let $X$ be a compact oriented manifold and let $T$ be a closed integral $k$--current on $X$.
If $\Theta$ is a smooth closed $k$--form representing an integral cohomology class $[\Theta]\in H^k(X,\Z)$, then
\[
\int_T \Theta \in \Z.
\]
\end{lemma}
\begin{proof}
This is the Kronecker pairing $\langle[T],[\Theta]\rangle\in\Z$ between $H_k(X,\Z)$ and $H^k(X,\Z)$, expressed in de~Rham form.
\end{proof}

\begin{lemma}[Lattice discreteness]\label{lem:lattice-discreteness}
If $a,b\in\Z$ and $|a-b|<1$, then $a=b$.
\end{lemma}
\begin{proof}
The difference $a-b\in\Z$ and the strict inequality $|a-b|<1$ force $a-b=0$.
\end{proof}
...
\]
Consequently, for the sheet--sum $S:=\sum_Q S_Q$, if $U$ is any correction current with $\partial U=\partial S$ satisfying
$\bigl|\int_U\Theta_\ell\bigr|<\tfrac12$ for all $\ell$ (for example, any $U$ with sufficiently small mass), then the closed integral current
$T:=S-U$ satisfies
\[
\int_T\Theta_\ell = m\,I_\ell
\qquad\text{for all }\ell,
\]
and hence $[T]=\mathrm{PD}(m[\gamma])$.
\end{proposition}
...
\]
Now let $S:=\sum_Q S_Q$ and let $U$ be any correction current with $\partial U=\partial S$ and $|\int_U\Theta_\ell|<\frac12$ for all $\ell$.
Then $T:=S-U$ is a closed integral current, so Lemma~\ref{lem:integral-periods} gives $\int_T\Theta_\ell\in\Z$ for all $\ell$.
Moreover,
\[
\Bigl|\int_T\Theta_\ell - mI_\ell\Bigr|
\le \Bigl|\sum_Q S_Q(\Theta_\ell)-mI_\ell\Bigr|+\Bigl|\int_U\Theta_\ell\Bigr|
<1.
\]
Since $mI_\ell\in\Z$, Lemma~\ref{lem:lattice-discreteness} yields $\int_T\Theta_\ell=mI_\ell$ for each $\ell$, i.e.\ $[T]=\mathrm{PD}(m[\gamma])$.
\end{proof}
...
In the parameter regimes established earlier, the global coherence/transport estimates force
\[
\mathcal F(\partial S)\xrightarrow[]{}0
\]
(see Proposition~\ref{prop:global-coherence-all-labels}; and in the borderline bidegree $p=n/2$ the required decay is supplied by
Lemma~\ref{lem:borderline-p-half} via Proposition~\ref{prop:integer-transport}).
Proposition~\ref{prop:glue-gap} then converts this flat-norm decay into an actual filling with vanishing mass.
...
\]
By Proposition~\ref{prop:cohomology-match} (applied with $U=U_\epsilon$, choosing $\epsilon$ small so that
$\bigl|\int_{U_\epsilon}\Theta_\ell\bigr|<\tfrac12$ for all $\ell$), the homology class
$[T_\epsilon]=\mathrm{PD}(m[\gamma])$.  Moreover, calibratedness
...
In the setting above, let $T_\epsilon:=S-U_\epsilon$ be the closed integral current obtained by gluing the calibrated sheet-sum $S=\sum_Q S_Q$ with a correction current $U_\epsilon$ satisfying $\partial U_\epsilon=\partial S$ and $\Mass(U_\epsilon)\to 0$.
Then:
\begin{enumerate}
\item[\textnormal{(i)}] (\textbf{Exact calibration pairing}) Since $[T_\epsilon]=\mathrm{PD}(m[\gamma])$ and $\psi$ is closed,
\[
\int_{T_\epsilon}\psi\ =\ \bigl\langle [T_\epsilon],[\psi]\bigr\rangle
\ =\ \bigl\langle \mathrm{PD}(m[\gamma]),[\psi]\bigr\rangle
\ =\ m\int_X \beta\wedge\psi\ =:\ c_0,
\]
independently of $\epsilon$.
\item[\textnormal{(ii)}] (\textbf{Almost--calibration}) Writing the calibration defect
\[
...
\]
\item[\textnormal{(iii)}] (\textbf{Mass convergence}) In particular,
\[
...
\begin{proof}
Since each sheet piece $S_Q$ is a holomorphic (hence $\psi$--calibrated) current, their sum $S$ is $\psi$--calibrated and therefore
\[
\Mass(S)=\int_S\psi.
\]
By the triangle inequality for the mass norm and $T_\epsilon=S-U_\epsilon$ we have
\[
...
\]
Since $\psi$ has comass $\le 1$ (it is the K\"ahler calibration), we have
$\bigl|\int_{U_\epsilon}\psi\bigr|\le \Mass(U_\epsilon)$, hence
\[
...
\]
The identity in (i) is the cohomology--homology pairing for the fixed class $[T_\epsilon]=\mathrm{PD}(m[\gamma])$ and the closed form $\psi$, and (iii) follows by combining (i)--(ii).
\end{proof}
...
dimension $n$, and let $1\le p\le \frac{n}{2}$. \editp{(For the full Hodge conjecture in all bidegrees, this restriction is harmless by Hard Lefschetz; see Remark~\ref{rem:lefschetz-reduction}.)}
Every smooth closed cone-valued
...
\Def_{\mathrm{cal}}(T_k)=\Mass(T_k)-\langle T_k,\psi\rangle\ \longrightarrow\ 0.
\]
Equivalently, $\Mass(T_k)\to m\int_X \beta\wedge\psi$.
...
\begin{proof}
Fix $\beta$ as in the statement.  In the microstructure construction of Section~\ref{sec:realization}, for each small parameter $\epsilon>0$ one builds:
\begin{itemize}
\item a calibrated sheet--sum $S=\sum_Q S_Q$ (a sum of holomorphic pieces on the mesh) whose periods are already within a fixed lattice margin of the target
\[
\Bigl|\sum_Q S_Q(\Theta_\ell) - m\,I_\ell\Bigr|<\tfrac12
\qquad(\ell=1,\dots,b)
\qquad
\text{(see Proposition~\ref{prop:cohomology-match})},
\]
\item and a correction current $U_\epsilon$ with $\partial U_\epsilon=\partial S$ and $\Mass(U_\epsilon)\to 0$ as $\epsilon\to 0$
\[
\text{(see Proposition~\ref{prop:glue-gap})}.
\]
\end{itemize}
Define the closed integral cycles $T_\epsilon:=S-U_\epsilon$.
By Proposition~\ref{prop:cohomology-match} (applied with $U=U_\epsilon$, noting that $|\int_{U_\epsilon}\Theta_\ell|\le \|\Theta_\ell\|_{\mathrm{comass}}\Mass(U_\epsilon)\to 0$), one has $[T_\epsilon]=\mathrm{PD}(m[\gamma])$ for $\epsilon$ small.
Then Proposition~\ref{prop:almost-calibration} gives $\Def_{\mathrm{cal}}(T_\epsilon)\to 0$, hence $\Mass(T_\epsilon)\to c_0=\langle \mathrm{PD}(m[\gamma]),[\psi]\rangle$.
Applying Theorem~\ref{thm:realization-from-almost} yields a $\psi$-calibrated integral limit current $T$ with $[T]=\mathrm{PD}(m[\gamma])$; the algebraicity conclusion follows by Harvey--Lawson and Chow/GAGA.
\end{proof}
...
\begin{proof}
Fix $x\in X$.
Since $\omega$ is K\"ahler, $\omega^p/p!$ is the standard K\"ahler calibration at $x$.
Equivalently, $\omega^p(x)$ lies in the interior of the strongly positive cone $K_p(x)$: it pairs \emph{strictly} positively with every nonzero calibrated simple $(n-p)$--plane at $x$.
This is the standard characterization of strict positivity (cf.\ Harvey--Lawson or Demailly).

For the uniform radius, for each $x$ choose $r(x)>0$ with $B(\omega^p(x),r(x))\subset K_p(x)$ (possible since $\omega^p(x)\in\mathrm{int}\,K_p(x)$).
The map $x\mapsto \omega^p(x)$ is continuous and the cone field $x\mapsto K_p(x)$ varies continuously in the Grassmann bundle.
By compactness of $X$ we may take
\[
r_0\ :=\ \min_{x\in X} r(x)\ >\ 0,
\]
which yields the claimed uniform inclusion.
\end{proof}
\end{editamirblock}
...
Hodge class on a smooth projective K\"ahler manifold, and assume $p\le n/2$.
Then $\gamma^+$
is algebraic.
\end{theorem}

\begin{proof}
Since $\gamma^+$ is cone--positive, it admits a cone-valued representative $\beta$
...
\begin{proof}
\noindent\textbf{Referee map (main dependency chain).}
The proof of Theorem~\ref{thm:main-hodge} uses only the following labeled results:
(i) Hard Lefschetz reduction to $p\le n/2$ (Remark~\ref{rem:lefschetz-reduction});
(ii) signed decomposition $\gamma=\gamma^+-\gamma^-$ with $\gamma^\pm$ cone--positive and $\gamma^-=N[\omega^p]$ (Lemma~\ref{lem:signed-decomp});
(iii) algebraicity of $\gamma^-$ as a complete intersection (Lemma~\ref{lem:gamma-minus-alg});
(iv) algebraicity of cone--positive classes $\gamma^+$ (Theorem~\ref{thm:effective-algebraic}), whose proof uses
Automatic SYR (Theorem~\ref{thm:automatic-syr}) + calibrated-limit closure (Theorem~\ref{thm:realization-from-almost}) + Chow/GAGA (Remark~\ref{rem:chow-gaga}).
Inside Theorem~\ref{thm:automatic-syr}, the quantitative output is the spine theorem (Theorem~\ref{thm:spine-quantitative}) under the global parameter schedule
(\S\ref{sec:parameter-schedule}), with local holomorphic manufacturing supplied by Theorem~\ref{thm:local-sheets} (H1),
global face coherence supplied by the corner-exit coherence package (Proposition~\ref{prop:global-coherence-all-labels} feeding Corollary~\ref{cor:global-flat-weighted} and Proposition~\ref{prop:glue-gap}; and in the borderline case $p=n/2$ by Lemma~\ref{lem:borderline-p-half} via Proposition~\ref{prop:integer-transport}) (H2),
and exact homology/integrality enforced by Proposition~\ref{prop:cohomology-match} (using Lemmas~\ref{lem:integral-periods} and \ref{lem:lattice-discreteness});
vanishing defect is then Proposition~\ref{prop:almost-calibration}.
\medskip
