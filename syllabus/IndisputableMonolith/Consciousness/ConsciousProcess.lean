...
structure ConsciousProcess where
  ...
  /-- Dimensionless invariance: the bridge ratio (λ_kin/τ_rec) is units-independent. -/
  dimensionless : ∀ {U' : RSUnits}, U'.tau0 ≠ 0 → U'.ell0 ≠ 0 →
    (RSUnits.lambda_kin_display units / RSUnits.tau_rec_display units) =
    (RSUnits.lambda_kin_display U' / RSUnits.tau_rec_display U')
...
/-- A conscious process is dimensionally consistent.
    The dimensionless property ensures unit coherence. -/
lemma dimensional_consistency (cp : ConsciousProcess) (hτ : cp.units.tau0 ≠ 0) (hℓ : cp.units.ell0 ≠ 0) :
    ∀ {U' : RSUnits}, U'.tau0 ≠ 0 → U'.ell0 ≠ 0 →
    (RSUnits.lambda_kin_display cp.units / RSUnits.tau_rec_display cp.units) =
    (RSUnits.lambda_kin_display U' / RSUnits.tau_rec_display U') :=
