...
When the Θ-field density ρ exceeds the saturation threshold `Θcrit`
(`PhaseSaturation.SaturationThreshold`, currently `φ^45`),
...
open ThetaThermodynamics (ThetaDensity Θcrit ThetaPressure CostNonExist EmbodimentThreshold
...
/-- Above the embodiment density threshold, embodiment becomes thermodynamically favored
by **cost comparison** (non-existence cost exceeds embodiment cost). -/
theorem favoredBind {ρ : ThetaDensity}
    (h : EmbodimentThreshold embodimentCost < ρ) :
    embodimentCost < CostNonExist ρ := by
  have hC : 0 ≤ embodimentCost := le_of_lt embodimentCost_pos
  exact embodiment_favored_above_threshold (C_emb := embodimentCost) hC h
...
/-- Above the embodiment density threshold, non-existence cost exceeds embodiment cost -/
theorem above_threshold_embodiment_cost_exceeded {ρ : ThetaDensity}
    (h : embodimentDensityThreshold < ρ) :
    embodimentCost < CostNonExist ρ :=
  favoredBind h
