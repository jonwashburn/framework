...
theorem phiMag_anti_mono_R (debt : ℤ) {R₁ R₂ : ℕ} (h : R₁ ≤ R₂) :
  ...
  exact div_le_div_of_nonneg_left ha hpos₁ hden

theorem phiMag_zero_debt (R : ℕ) : PhiMag 0 R = 0 := by
  unfold PhiMag
  simp
...
theorem backward_projection_principle (pl : PhantomLight) (B : ℤ) (_hB : 0 ≤ B) :
  ...
  have habs_sum_le : |∑ i : Fin pl.remainingTicks, future i|
    ...
    simpa using
      (Finset.abs_sum_le_sum_abs (s := (Finset.univ : Finset (Fin pl.remainingTicks))) (f := future))
...
theorem phantom_inflates_cost (x : ℝ) (_hx : x > 0) (pl : PhantomLight)
    (penalty_scale : ℝ) (hscale : penalty_scale > 0) :
...
/-- Quantitative response-time law from the phantom magnitude definition:
    `tau_response = 1 / (PhantomMagnitude + 1)` is always positive. -/
theorem precognition_response_time_law (ps : PhantomSensing) :
    ∃ τ_response : ℝ,
      τ_response > 0 ∧
      τ_response = 1 / (PhantomMagnitude ps.phantomField + 1) := by
  refine ⟨1 / (PhantomMagnitude ps.phantomField + 1), ?_, rfl⟩
  have hmag_nonneg : 0 ≤ PhantomMagnitude ps.phantomField :=
    phantom_visibility_grows_with_debt ps.phantomField
  have hden_pos : 0 < PhantomMagnitude ps.phantomField + 1 := by linarith
  exact one_div_pos.mpr hden_pos

/-- The precognition response-time hypothesis is witnessed in the model. -/
theorem precognition_response_time_hypothesis_holds :
    precognition_response_time_hypothesis := by
  intro ps _hsens
  exact precognition_response_time_law ps
...
theorem PhantomLightCert.verified_any (c : PhantomLightCert) :
  ...
  · intro x pl hx
    exact phantom_inflates_cost x hx pl 1 (by norm_num)
