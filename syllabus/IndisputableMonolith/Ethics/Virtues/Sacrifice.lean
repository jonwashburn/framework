...
noncomputable def Sacrifice
  ...
  let sacrificer' : MoralState := {
    ...
    energy_pos := by
      ...
      have h_cap_le : energy_debit ≤ sacrificer.energy / Foundation.φ := min_le_right _ _
      have h_div_lt : sacrificer.energy / Foundation.φ < sacrificer.energy := by
        rw [div_lt_iff₀ hφ_pos]
        exact lt_mul_of_one_lt_right sacrificer.energy_pos hφ_gt_one
      have h_debit_lt : energy_debit < sacrificer.energy := lt_of_le_of_lt h_cap_le h_div_lt
      linarith
  }
