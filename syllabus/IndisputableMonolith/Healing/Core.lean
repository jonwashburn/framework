...
/-- THEOREM: Healer and patient are maximally coupled via GCIC when same extent

    When healer and patient boundaries have the same φ-ladder position,
...
theorem maximal_theta_coupling (session : HealingSession)
    ...
    (h_pos : session.healer.boundary.extent > 0) (hL : lam_rec > 0) :
    theta_coupling_strength session = 1 := by
  unfold theta_coupling_strength
  exact theta_coupling_eq_one_same_extent _ _ _ h_extent h_pos hL
...
/-- MASTER THEOREM: Energy healing is effective
    ...
    Given:
    - A healer in high-coherence state (same φ-ladder position as patient)
