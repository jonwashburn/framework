...
/-! ## WToken Data/Predicate Separation (WS5.2)

**Refactoring pattern**: Separate raw data from proof constraints.
This enables:
1. Working with unvalidated WToken candidates
2. Defining RS legality as a predicate (not data)
3. Cleaner code generation without Prop-as-data issues

The legacy `WToken` structure is preserved for backward compatibility.
New code should prefer `WTokenData` + `IsWTokenLegal`. -/

/-- Raw WToken data (without proof fields).
    This separates the data representation from validity constraints.

**WS5.2**: Avoids proof-as-data pattern by making constraints a separate predicate. -/
structure WTokenData where
  ...
  /-- Emergent phase -/
  phi_e : ℝ
  /-- 8-phase fingerprint (not necessarily normalized/neutral) -/
  basis : Fin tauZero → ℂ

/-- RS legality predicate for WToken data.
    A WTokenData is legal if:
    1. Neutral: mean-free (sum of basis = 0)
    2. Normalized: unit L² norm

**WS5.2**: Constraints as predicate, not data fields. -/
def IsWTokenLegal (w : WTokenData) : Prop :=
  (Finset.univ.sum w.basis) = 0 ∧
  (Finset.univ.sum fun i => Complex.normSq (w.basis i)) = 1

/-- The type of legal WToken data (subtype formulation). -/
def LegalWToken : Type := { w : WTokenData // IsWTokenLegal w }

/-- Extract neutrality from legal WToken. -/
lemma LegalWToken.neutral (w : LegalWToken) :
    (Finset.univ.sum w.val.basis) = 0 := w.property.1

/-- Extract normalization from legal WToken. -/
lemma LegalWToken.normalized (w : LegalWToken) :
    (Finset.univ.sum fun i => Complex.normSq (w.val.basis i)) = 1 := w.property.2

/-- WToken: 8-phase complex semantic atom (legacy structure)

**Note (WS5.2)**: This structure bundles data with proof fields.
For new code, prefer `WTokenData` + `IsWTokenLegal` or `LegalWToken`.
This structure is preserved for backward compatibility. -/
...
/-! ## WToken ↔ LegalWToken Bridge -/

/-- Convert legacy WToken to WTokenData. -/
def WToken.toData (w : WToken) : WTokenData :=
  { nu_phi := w.nu_phi
  , ell := w.ell
  , sigma := w.sigma
  , tau := w.tau
...
/-- Legacy WToken is always legal. -/
lemma WToken.isLegal (w : WToken) : IsWTokenLegal w.toData :=
  ⟨w.neutral, w.normalized⟩

/-- Convert legacy WToken to LegalWToken. -/
def WToken.toLegalWToken (w : WToken) : LegalWToken :=
  ⟨w.toData, w.isLegal⟩

/-- Convert LegalWToken to legacy WToken. -/
def LegalWToken.toWToken (w : LegalWToken) : WToken :=
  { nu_phi := w.val.nu_phi
  , ell := w.val.ell
  , sigma := w.val.sigma
  , tau := w.val.tau
...
/-- Roundtrip: WToken → LegalWToken → WToken is identity on data. -/
@[simp] theorem WToken.roundtrip_data (w : WToken) :
    (w.toLegalWToken.toWToken).toData = w.toData := rfl

/-- Roundtrip: LegalWToken → WToken → LegalWToken is identity on data. -/
@[simp] theorem LegalWToken.roundtrip_data (w : LegalWToken) :
    w.toWToken.toLegalWToken.val = w.val := rfl
