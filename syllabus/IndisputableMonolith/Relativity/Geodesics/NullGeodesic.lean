...
def straight_null_geodesic (ic : InitialConditions) : NullGeodesic minkowski_tensor where
    ...
    have hdir :
        Finset.sum (Finset.univ : Finset (Fin 4))
          (fun μ =>
            Finset.sum (Finset.univ : Finset (Fin 4))
              (fun ν =>
                minkowski_tensor.g (straight_line ic lam) (fun _ => 0)
...
theorem null_geodesic_exists_minkowski (ic : InitialConditions) :
    ∃ geo : NullGeodesic minkowski_tensor,
