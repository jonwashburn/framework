  ...
  christoffel_expansion_minkowski :
    ∀ (hWF : WeakFieldPerturbation) (x : Fin 4 → ℝ) (ρ μ ν : Fin 4),
      |(christoffel_from_metric
          (perturbed_metric minkowski_tensor hWF.base)).Γ x ρ μ ν -
        linearized_christoffel minkowski_tensor hWF.base x ρ μ ν|
        ≤ 40 * hWF.eps ^ 2
...
theorem christoffel_minkowski_expansion (h : MetricPerturbation) (x : Fin 4 → ℝ) (ρ μ ν : Fin 4)
  [ChristoffelExpansionFacts] :
  |(christoffel_from_metric (perturbed_metric minkowski_tensor h)).Γ x ρ μ ν -
   linearized_christoffel minkowski_tensor h x ρ μ ν| < 0.01 := by
  have h_zero := minkowski_christoffel_zero x ρ μ ν
  have h_exp := christoffel_expansion_minkowski minkowski_tensor h x ρ μ ν
...
theorem christoffel_small_when_h_small (hWF : WeakFieldPerturbation)
  ...
  |linearized_christoffel minkowski_tensor hWF.base x ρ μ ν| < 1 := by
  -- δΓ = (1/2) η^{ρσ} (∂_μ h_νσ + ∂_ν h_μσ - ∂_σ h_μν)
  -- With |η^{ρσ}| ≤ 1, |∂h| controlled by finite difference |h|/Δx
  -- Sum over 4 values of σ; each term ~ |h|
  have : |linearized_christoffel minkowski_tensor hWF.base x ρ μ ν|
       ≤ (1/2) * Finset.sum (Finset.univ : Finset (Fin 4)) (fun σ =>
           |(inverse_metric minkowski_tensor) x (fun i => if i.val = 0 then ρ else σ) (fun _ => 0)| *
  ...
  have hmetric :
      ∀ σ,
        |(inverse_metric minkowski_tensor) x (fun i => if i.val = 0 then ρ else σ) (fun _ => 0)| ≤ 1 := by
    intro σ
    have :
        (inverse_metric minkowski_tensor) x (fun i => if i.val = 0 then ρ else σ) (fun _ => 0)
  ...
  have hsum_bound :
    ...
    simpa [hnonneg, hcard, Finset.card_univ, Fintype.card_fin, bit0, one_mul, mul_comm, mul_left_comm, mul_assoc]
      using this
  have hfinal : |linearized_christoffel minkowski_tensor hWF.base x ρ μ ν| ≤ 0.06 := by
  ...
  have h_deriv :
      ∀ σ,
        |partialDeriv_v2 (fun y => g.g y (fun _ => 0)
          ...
          - (partialDeriv_v2 (fun y => hWF.base.h y (fun i => if i.val = 0 then ν else σ)) μ x
              + partialDeriv_v2 (fun y => minkowski_tensor.g y (fun _ => 0)
