...
/-- Derive qualia from a WToken (total function).

This mirrors `ULQ.Core.deriveQualia`, but returns the constructed `QualiaSpace`
directly (no `Option`) since `WToken` carries enough structure to determine a
non-DC mode via DFT. -/
noncomputable def deriveQualia' (w : WToken) : QualiaSpace :=
...
structure QualiaExperience where
  ...
  coherent_qualia : qtoken.qualia =
    deriveQualia' (DFTDecomposition.boundaryToWToken boundary field definite)
...
theorem experience_from_boundary (b : StableBoundary) (ψ : UniversalField)
  ...
  let mode : QualiaMode := qualiaModeOfWToken w
...
theorem THEOREM_experience_emergence (b : StableBoundary) (ψ : UniversalField) :
    ...
    (∀ (qe₁ qe₂ : QualiaExperience),
      qe₁.boundary = b → qe₂.boundary = b →
      qe₁.field = ψ → qe₂.field = ψ →
      qe₁.qtoken.qualia = qe₂.qtoken.qualia) := by
  constructor
  · exact experience_threshold b ψ
  constructor
  · exact superposition_exclusive b ψ
  · intro qe₁ qe₂ hb1 hb2 hf1 hf2
    apply experience_uniqueness qe₁ qe₂
    · rw [hb1, hb2]
    · rw [hf1, hf2]
