...
/-- **CERTIFICATE: Advanced Particle Physics**
    ...
    4. (Optional) Quark mass quarter-ladder positions (hypothesis lane; not part of the core parameter-free spectra model). -/
structure AdvancedParticlePhysicsCert where
  -- 1. Mixing Matrix
  mixing_verified : MixingCert
  -- 2. Lepton Ladder
  lepton_torsion_verified : LeptonTorsionCert
  -- 3. Quark Ladder (optional / hypothesis lane)
  quark_mass_verified : Option QuarkMassCert := none
  -- 4. Neutrino Ladder
...
@[simp] def AdvancedParticlePhysicsCert.verified (c : AdvancedParticlePhysicsCert) : Prop :=
  ...
  (match c.quark_mass_verified with
   | none => True
   | some qc => (qc.top_match ∧ qc.bottom_match ∧ qc.charm_match)) ∧
