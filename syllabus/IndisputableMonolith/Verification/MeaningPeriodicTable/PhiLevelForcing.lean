...
/-! ## Part 2b: Cube-Partition Arithmetic Bridge (D=3) -/

/-- In D=3, the boundary-element count of a cube is V + E + F = 8 + 12 + 6 = 26. -/
theorem cube_boundary_elements_count :
    AlphaDerivation.cube_vertices AlphaDerivation.D +
      AlphaDerivation.cube_edges AlphaDerivation.D +
      AlphaDerivation.cube_faces AlphaDerivation.D = 26 := by
  native_decide

/-- Removing face channels from the cube boundary count gives 20:
    (V + E + F) - F = V + E = 8 + 12 = 20. -/
theorem token_count_from_cube_minus_faces :
    (AlphaDerivation.cube_vertices AlphaDerivation.D +
        AlphaDerivation.cube_edges AlphaDerivation.D +
        AlphaDerivation.cube_faces AlphaDerivation.D) -
      AlphaDerivation.cube_faces AlphaDerivation.D = 20 := by
  native_decide

/-- Equivalent arithmetic bridge: 4 mode families times (4 level slots + one
    self-conjugate bonus slot) gives 20 addresses. -/
theorem token_count_as_four_by_five :
    modeFamilies.length * ((simplicial_grades DimensionForcing.D_physical).card + 1) = 20 := by
  simp [mode_families_count, physical_dimension_forces_4_grades]

/-- Fibonacci closure begins at level 4 on the phi ladder:
    `phi^4` is not a new primitive grade, but decomposes into lower levels. -/
theorem phi_four_closure :
    (Constants.phi : ℝ) ^ 4 = Constants.phi ^ 3 + Constants.phi ^ 2 := by
  have hsq : (Constants.phi : ℝ) ^ 2 = Constants.phi + 1 := IndisputableMonolith.PhiSupport.phi_squared
  have hcub : (Constants.phi : ℝ) ^ 3 = 2 * Constants.phi + 1 := by
    nlinarith [hsq]
  have hfour : (Constants.phi : ℝ) ^ 4 = 3 * Constants.phi + 2 := by
    nlinarith [hsq, hcub]
  have hrhs : (Constants.phi : ℝ) ^ 3 + Constants.phi ^ 2 = 3 * Constants.phi + 2 := by
    nlinarith [hsq, hcub]
  nlinarith [hfour, hrhs]
...
theorem phi_level_count_derived (_h : phi_level_topology_hypothesis) :
