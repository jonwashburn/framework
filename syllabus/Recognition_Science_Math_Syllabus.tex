...
\hfill\textbf{File:} \texttt{axioms-4140269.pdf}
\hfill\textcolor{green!50!black}{\textbf{PUBLISHED} -- \emph{Axioms} (MDPI), 2026}

\textbf{Statement.}
Characterizes the class of reciprocal convex cost functionals on
$\mathbb{R}_{>0}$ satisfying the d'Alembert equation. Shows
$f(u):=1+J(e^u)$ is continuous and satisfies the additive d'Alembert equation;
classification gives $J(x)=\cosh(a\log x)-1=\frac{1}{2}(x^a+x^{-a})-1$.
Geometric-mean decision boundaries, compositionality for product models,
sequential mediation via geometric mean. Washburn \& Rahnamai Barghi.

\textbf{Dependencies:} M2.

\subsection*{M4b.\; Recognition Algebra: The Unified Algebraic Framework}
\textbf{Syllabus paper:} 82.\ Recognition Algebra
\hfill\textbf{File:} \texttt{papers/01-root-foundation/Recognition\_Algebra.tex}
\hfill\textcolor{red}{\textbf{NEW --- Feb 2026}}

\textbf{Statement.}
The RCL forces four algebraic layers:
\begin{enumerate}[nosep]
  \item \textbf{Cost Algebra.}  Monoid $(\mathbb{R}_{\ge 0},\star,0)$ with
    $a\star b=2ab+2a+2b$; shifted form $H=J+1$ satisfies d'Alembert.
  \item \textbf{$\phig$-Ring.}  $\mathbb{Z}[\phig]$ with Galois conjugation
    $\sigma^2=\mathrm{id}$, multiplicative norm $N(\alpha\beta)=N(\alpha)N(\beta)$,
    and $\phig$ a unit ($N(\phig)=-1$).
  \item \textbf{Ledger Algebra.}  Graded abelian group of antisymmetric events
    with vertex-wise conservation $\sigma=0$.
  \item \textbf{Octave Algebra.}  $\mathbb{Z}/8\mathbb{Z}$ with DFT-8; 20 neutral
    basis modes from $3\times 4 + 2\times 4$.
\end{enumerate}
Category \textbf{RecAlg} defined; canonical $J$ is the initial object.
Self-contained proofs.  Lean-verified.

\textbf{Dependencies:} M2, M5, M8.

\subsection*{M4c.\; Three Attributes Framework (Core + Extended)}
\textbf{Syllabus paper:} 84.\ Inevitability From Three Attributes
\hfill\textbf{File:} \texttt{Inevitability\_From\_Three\_Attributes.tex}
\hfill\textcolor{red}{\textbf{NEW --- Feb 2026}}

\textbf{Statement.}
Formalizes omniscience, omnipotence, and omnipresence on a costed
comparison system with explicit metric data. Proves a \textbf{core}
equivalence:
\begin{itemize}[nosep]
  \item core axioms $\Longleftrightarrow$ canonical core framework
  (unique $J$ and $D=3$ memory/non-segregation geometry),
\end{itemize}
and records the $\phig$, 8-tick, and $W=17$ consequences as
\textbf{explicitly postulated extensions}, not as core forced results.

\textbf{Dependencies:} M2, M3, M6.
...
M4c & Three Attributes Framework          & 1 & M2,M3,M6 \\
